"""Install and remove the Linux desktop entry (``robo.desktop``).

``robo desktop`` builds and launches the Electron app. On Linux, a
...
_PRODUCT_NAME = os.environ.get("ROBO_PRODUCT_NAME", "Robo").strip() or "Robo"
DESKTOP_ENTRY_NAME = "robo.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``robo.desktop`` entry lives."""
...
def resolve_exec_command() -> str:
    """Build the absolute ``Exec=`` command line for ``robo desktop``.

    Prefer the real ``robo`` executable (argv[0] or PATH). When Robo
    runs as a module with no launcher installed, use the current
    interpreter, also absolute.
    """
    from robo_cli.relaunch import resolve_robo_bin

    bin_path = resolve_robo_bin()
    if bin_path:
        argv = [str(Path(bin_path).resolve()), "desktop"]
    else:
        argv = [str(Path(sys.executable).resolve()), "-m", "robo_cli.main", "desktop"]
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    ...
    return (
        ...
        "Type=Application\n"
        f"Name={product}\n"
        f"GenericName={product} Desktop\n"
        f"Comment=Launch {product} Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        f"StartupWMClass={product}\n"
    )
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Robo desktop entry. Return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "robo"

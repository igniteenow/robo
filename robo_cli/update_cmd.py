"""Robo update pipeline — extracted from ``robo_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``robo_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``robo_cli.main`` reference — so existing call sites and test monkeypatches
that target ``robo_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``robo_cli.main``.
...
Imports are one-way: ``robo_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``robo_cli.main`` reference.

    Lets callers keep patching ``robo_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``robo_cli.main`` -> ``robo_cli.update_cmd`` stays one-way
    at import time.
    """
    from robo_cli import main
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``robo update`` keeps running in the pre-pull Python process. After a
...
# Critical files that Robo must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "robo_cli/main.py",
    "robo_cli/config.py",
    "robo_cli/__init__.py",
    "robo_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``robo`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="robo-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``robo update`` can be driven by a
    ...
    """
    from robo_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from robo_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('robo_'):\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``robo update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `robo update`.
...
def _print_fts_optimize_available_notice() -> None:
    ...
    try:
        ...
        return
    db_path = get_robo_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `robo sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    robo sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  robo sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``robo update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``robo update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: robo curator status)"
    )
...
def _finish_dashboard_update_cleanup(node_failures: list[str]) -> None:
    ...
    print("    robo dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``robo_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.robo-update-staging"
    backup = f"{dst}.robo-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `robo update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``robo_constants.py``
...
def _print_update_completion(message: str) -> None:
    ...
    print(message)
    action_id = os.environ.get("ROBO_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== robo-update completed {action_id} ===")


def _update_via_zip(args):
    """Update Robo by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `robo update "
            f"--branch {branch}`, or update against main with `robo update`."
        )
        _m().sys.exit(1)
    zip_url = (
        f"https://github.com/IgniteeNow/robo-engineer/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="robo-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"robo-engineer-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for robo-engineer itself
        ...
        # GitHub ZIPs extract to robo-engineer-<branch>/
        extracted = os.path.join(tmp_dir, f"robo-engineer-{branch}")
        ...
        print(
            "  Re-run `robo update` to retry; if the agent won't start, "
            "reinstall from https://github.com/IgniteeNow/robo-engineer"
        )
        _m().sys.exit(1)
    finally:
    ...
    from robo_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if not import_ok:
        ...
        print("  Re-run `robo update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: robo skills list-modified  "
    ...
    try:
        from robo_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from robo_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_robo_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_robo_home())
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "robo-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Robo behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes robo completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Robo couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Robo couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Robo behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Robo couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Robo couldn't drop "
...
OFFICIAL_REPO_URLS = {
    "https://github.com/IgniteeNow/robo-engineer.git",
    "git@github.com:IgniteeNow/robo-engineer.git",
    "https://github.com/IgniteeNow/robo-engineer",
    "git@github.com:IgniteeNow/robo-engineer",
}

OFFICIAL_REPO_URL = "https://github.com/IgniteeNow/robo-engineer.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_robo_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_robo_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Robo repository.")
        print("  This means you may miss updates from IgniteeNow/robo-engineer.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/IgniteeNow/robo-engineer.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/IgniteeNow/robo-engineer.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``robo update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from robo_constants import get_default_robo_root

    default_home = get_default_robo_root()
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "robo.exe"
    lines = ["✗ Another robo.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Robo Desktop, exit any open `robo` REPLs, and")
    lines.append("  stop the gateway (`robo gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `robo update --force` if you've already")
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``robo update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `robo update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `robo update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Robo' editable-install extras or
    ...
    try:
        from robo_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from robo_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$ROBO_HOME/bin/uv``, but on Termux the official
    ...
    """
    from robo_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `robo update` is exactly the step
...
def _npm_lockfile_changed(robo_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `robo
    ...
    try:
        ...
        cache_file = robo_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(robo_root: Path) -> None:
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from robo_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `robo update`.")
            failed = ["repo root"]
            if any(
    ...
    # Robo profile using this checkout. Keep one per-checkout cache under the
    # shared Robo root rather than rerunning npm once per named profile.
    shared_robo_root = get_default_robo_root()
    if not _m()._npm_lockfile_changed(shared_robo_root):
    ...
    # need desktop during `robo update`, so we install root-only first
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `robo update`.")
        return list(labels)
    ...
    nixos_env = with_robo_node_path(_m()._nixos_build_env())
    ...
    # hung. The chatty npm-deprecation noise during `robo update` comes from
    ...
    if ws_result.returncode == 0:
        _record_npm_lockfile_hash(shared_robo_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.robo/logs/update.log`` only, never the terminal.

    During ``robo update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``robo update --check``: fetch and report without installing.
    ...
    """
    from robo_cli.config import detect_install_method, recommended_update_command_for_method
    method = detect_install_method(_m().PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``robo update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from robo_cli.config import format_docker_update_message
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``robo update`` without requiring a reinstall.  The
    ...
    ``robo`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/robo.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v robo'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/robo, code at /usr/local/lib/robo-engineer).
    fhs_link = Path("/usr/local/bin/robo")
    ...
    path_comment = (
        "# Robo — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    """Self-heal: install a ``robo-acp`` launcher next to the ``robo`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``robo update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``robo-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Robo as not installed even when it is.

    The shim simply delegates to the sibling ``robo`` launcher with the
    ...
    ``robo-acp.exe`` already resolves) and wherever a ``robo-acp`` is
    already present next to the ``robo`` command.  Unwritable directories
    (e.g. ``/usr/local/bin`` as non-root) are skipped silently.  Idempotent.
    """
    if _m().sys.platform == "win32":
        return
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        robo_cmd = bin_dir / "robo"
        acp_cmd = bin_dir / "robo-acp"
        try:
            if not (robo_cmd.is_file() or robo_cmd.is_symlink()):
                continue
            ...
            shim = (
                "#!/usr/bin/env bash\n"
                "# Robo — ACP launcher (written by `robo update`).\n"
            ...
            continue
        print(f"  ✓ Installed robo-acp launcher → {acp_cmd}")
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of ROBO_HOME under
      ``backups/`` (restorable via ``robo import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from robo_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from robo_constants import
        # get_robo_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from robo_cli.config import get_robo_home as _get_home
    ...
    try:
        from robo_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_robo_home so the user sees ~/.robo/...
    try:
        from robo_constants import get_robo_home, display_robo_home

        home = get_robo_home()
        try:
            display_path = f"{display_robo_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  robo import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``robo update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``robo update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run robo from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.robo-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".robo-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The robo.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m robo_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``robo
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Robo processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Robo Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the Robo desktop app / other Robo terminals, then re-run:"
    )
    lines.append("    robo update")
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``robo update``
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    robo.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating Robo...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m robo_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: robo gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``robo gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from robo_cli import gateway_windows
        from robo_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``robo-gateway*.service`` from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not unit.startswith("robo-gateway"):
            continue
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``robo update`` never touched them, so installs created before the
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``robo update`` sees a clean tree instead of autostashing every run.
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # On Windows, abort early if another robo.exe is holding the venv shim
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Robo windows/gateways and re-run: robo update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Robo gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in robo_cli/config.py bricked
        # every user who ran ``robo update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``robo
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``robo update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_robo_home added to robo_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``robo`` launch finishes the install
        ...
        # symbols in robo_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `robo` again "
        ...
        if not import_ok:
            ...
            print("    Run `robo update` again — if it persists, reinstall:")
            print("    https://github.com/IgniteeNow/robo-engineer")

        node_failures = _update_node_dependencies()
        _m()._build_web_ui(_m().PROJECT_ROOT / "web")

        # Rebuild the desktop app if the source tree changed since the last
        # build.  ``robo desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``robo desktop`` shouldn't be forced into a full
        # Electron build by ``robo update``.
        ...
        if (desktop_dir / "package.json").exists() and _m()._resolve_node_runtime_npm() and has_desktop_app:
            ...
            # `robo desktop --build-only` subprocess re-imports the whole
            ...
            if _skip_desktop_build:
                ...
                _desktop_build_cmd = [sys.executable, "-m", "robo_cli.main", "desktop", "--build-only"]
                ...
                # debuggable.
                #
                # Start the build subprocess with the Robo-managed Node on PATH:
                # when `robo update` runs inside the desktop updater chain
                # (Desktop → robo-setup → robo update), the shell PATH
                # customizations are lost, so a bare-PATH child would fail with
                # `node: not found` before cmd_gui can self-heal.
                from robo_constants import with_robo_node_path

                _build_env = with_robo_node_path()
                build_result = _m()._run_logged_subprocess(_desktop_build_cmd, cwd=_m().PROJECT_ROOT, env=_build_env)
                if build_result.returncode != 0:
                    build_result = _m()._run_logged_subprocess(_desktop_build_cmd, cwd=_m().PROJECT_ROOT, env=_build_env)
                if build_result.returncode != 0:
                    print("  ⚠ Desktop build failed (non-fatal; run `robo desktop` to retry)")
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_robo_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.robo/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit ROBO_HOME so
        # it is not affected by sync_skills()'s module-level ROBO_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ROBO_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from robo_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print(f"  ⚠  Config format update failed: {_mig_err}")
                print("     Run 'robo config migrate' to retry.")
            ...
            if missing_env:
                print(
                    f"  ⚠  {len(missing_env)} new required setting(s) need configuration"
                )
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: robo config migrate")
            else:
                print()
                print("Skipped. Run 'robo config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from robo_cli.backup import restore_cron_jobs_if_emptied

            cron_restore = restore_cron_jobs_if_emptied(pre_update_snapshot_id)
            if cron_restore:
                print()
                print(
                    "  ⚠  cron/jobs.json lost jobs during this update — "
        ...
        # check `robo curator status`. Self-stamps after printing so it
        ...
        # Self-heal the robo-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Robo on PATH without
        # a reinstall.  No-op on Windows and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("robo-acp launcher self-heal failed: %s", e)
        ...
        # don't have it. Tying the refresh to ``robo update`` gives users a
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `robo update`
                # must stay fast; `robo computer-use install --upgrade`
        ...
        # When running as ``robo update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_robo_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... robo-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... robo-gateway*)
            ...
            try:
                from robo_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all robo-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: robo gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `robo update` restart it automatically, allow\n"
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: robo gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: robo -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Robo gateway unit files are still installed.
        # When both robo.service (from a pre-rename install) and the
        # current robo-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `robo update` surfaces the issue until the user migrates.
        try:
            from robo_cli.gateway import (
                has_legacy_robo_units,
                _find_legacy_robo_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_robo_units():
                print()
                print("⚠ Legacy Robo gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_robo_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (robo.service) fight the current")
                print("  robo-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    robo gateway migrate-legacy")
                print()
        ...
        print("  robo model              # Select provider and model")

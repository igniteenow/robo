...
def bang_shell_enabled() -> bool:
    ...
    if env_var_enabled("ROBO_GATEWAY_SESSION"):
        return False
    if env_var_enabled("ROBO_CRON_SESSION"):
        return False
    if (os.getenv("ROBO_SESSION_PLATFORM") or "").strip():
        return False
    return True
...
def _bang_env() -> dict:
    """Environment for a bang command, with Robo-managed secrets filtered.
...
def run_bang_command(
    ...
    *,
    ...
    try:
        ...
        # Ctrl+C interrupts the command, not the Robo session.
        proc.kill()
        emit("!: interrupted")
        return 130
    finally:

...
late-binding seam in :mod:`robo_cli.web_deps` (``late`` for callables,
...
from robo_cli.web_deps import late, LateState
...
_spawn_robo_action = late("_spawn_robo_action")
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from robo_cli.platforms import platform_label
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``robo tools`` picker shows
    ...
    with _profile_scope(profile):
        ...
        if cat:
            ...
            # re-probing per row.
            features = get_igniteenow_subscription_features(config, force_fresh=True)
            for prov in _visible_providers(cat, config, force_fresh=True):
                ...
                row = {
                    ...
                    "requires_igniteenow_auth": bool(prov.get("requires_igniteenow_auth")),
                    "is_active": is_active,
                    # Honest server-side readiness. The GUI's old client-side
                    # heuristic showed "Ready" for every zero-env-var row —
                    # including logged-out Ignitee Now Subscription rows and never-run
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `robo tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``robo tools``
    ...
    Managed Ignitee Now rows (``managed_igniteenow_feature``) additionally report the
    Portal entitlement state: the CLI flow gates these selections on
    ``ensure_igniteenow_portal_access`` (inline login), but the GUI has no inline
    ...
    ``managed_by_igniteenow``). The response now carries an additive
    ``needs_igniteenow_auth: true`` + ``feature`` so the client can drive the
    existing Ignitee Now Portal OAuth flow (``POST /api/providers/oauth/igniteenow/start``)
    and refetch.
    """
    from robo_cli.tools_config import (
        TOOL_CATEGORIES,
    ...
    with _profile_scope(body.profile or profile):
        ...
        # Entitlement check for managed Ignitee Now rows — mirrors the gate the CLI
        # applies via ensure_igniteenow_portal_access at selection time.
        cat = TOOL_CATEGORIES.get(name)
        row = None
        if cat:
            row = next(
        ...
        managed_feature = (row or {}).get("managed_igniteenow_feature")
        if managed_feature:
            features = get_igniteenow_subscription_features(config, force_fresh=True)
            ...
            if not entitled:
                response["needs_igniteenow_auth"] = True
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.robo/.env`` via ``save_env_value`` —
    the same store ``robo tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``robo tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``robo -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same ROBO_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``robo computer-use permissions grant`` as a background action.

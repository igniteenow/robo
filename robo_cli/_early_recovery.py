"""Dependency-light venv recovery that runs BEFORE robo_cli.main's imports.

The ``robo`` console entry point is ``robo_cli.main:main``.  Importing
``robo_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``robo_cli.env_loader``, ``yaml`` via ``robo_cli.config``, ...).  In
...
a corrupted venv.  ``robo_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``robo_cli.main`` to become
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``robo acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``robo_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``robo_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``robo update`` exists AND an import probe

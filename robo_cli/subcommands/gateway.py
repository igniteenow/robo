"""``robo gateway`` and ``robo proxy`` subcommand parsers.
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if ROBO_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy robo.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Robo gateway unit files "
            "(e.g. robo.service) left over from older installs. Profile "
            "units (robo-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="xai",
        help="Upstream provider: xai. See `robo proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,

...
from robo_cli.subcommands._shared import add_accept_hooks_flag
...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Robo as an MCP server",
        description=(
            "Manage MCP server connections and run Robo as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'robo mcp add' to connect to a new server, or\n"
            "'robo mcp serve' to expose Robo conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Robo as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `robo mcp add ...` to fall through to
    # interactive chat.
    ...
    # ── Catalog (Ignitee Now-approved MCPs shipped with the repo) ─────────────────
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `robo mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
        help="List Ignitee Now-approved MCPs available for one-click install",
    )
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `robo mcp install n8n`)",
    )

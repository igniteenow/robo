"""Desktop GUI subcommand parser shared by the Robo and Robo launchers.
...
def build_gui_parser(subparsers, *, cmd_gui: Callable) -> None:
    ...
    gui_parser.add_argument(
        "--ignore-existing",
        action="store_true",
        help="Force Desktop to ignore any robo CLI already on PATH during backend resolution",
    )
    gui_parser.add_argument(
        "--robo-root",
        help="Override the Robo source root used by Desktop (sets ROBO_DESKTOP_ROBO_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
        help="Initial project directory for Desktop chat sessions (sets ROBO_DESKTOP_CWD)",
    )
    gui_parser.add_argument(
        "--skip-build",
        action="store_true",

"""``robo skills`` subcommand parser.

Extracted from ``robo_cli/main.py:main()`` (god-file Phase 2 follow-up).
...
def build_skills_parser(subparsers, *, cmd_skills: Callable) -> None:
    ...
    create_parser = skills_subparsers.add_parser(
        "create", help="Create your own skill (scaffolds ~/.robo/skills/<name>/SKILL.md)"
    )
    create_parser.add_argument("name", help="Skill name (lowercase, hyphens: e.g. release-checklist)")
    create_parser.add_argument("--description", "-d", default="", help="One-line description shown to the agent")
    create_parser.add_argument("--category", "-c", default="custom", help="Category folder (default: custom)")
    create_parser.add_argument("--tags", default="", help="Comma-separated tags used for skill matching")
    create_parser.add_argument("--edit", action="store_true", help="Open SKILL.md in $EDITOR after creating it")
    create_parser.set_defaults(func=cmd_skills)

    lint_parser = skills_subparsers.add_parser(
        "lint", help="Check a skill (or all your skills) for content-policy problems"
    )
    lint_parser.add_argument("name", nargs="?", default="", help="Skill name (default: every user skill)")
    lint_parser.set_defaults(func=cmd_skills)
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.robo/skills/.bundled_manifest) "
            "so future 'robo update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_list_modified = skills_subparsers.add_parser(
        "list-modified",
        help="List bundled skills you've edited (which `robo update` keeps)",
        description=(
            "Show the bundled skills whose local copy differs from the version last "
            "synced, i.e. the ones `robo update` reports as user-modified and skips. "
            "Use `robo skills diff <name>` to see changes and `robo skills reset "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`robo update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `robo update`. Pass --sync to re-seed now."
        ),
    )

"""``robo sync`` subcommand parser — Skill Sync.

Cloned from ``robo_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    robo sync status                 show gate/opt-in/head state
    robo sync pull                   pull and materialize opted-in skills
    robo sync push                   push opted-in skills
    robo sync now                    reconcile: pull then push
    robo sync enable <skill>         opt a skill into sync
    robo sync disable <skill>        opt a skill out of sync
    robo sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    robo sync propose <skill>        share a skill with your organisation

Sync is INERT unless the resolved Ignitee Now token carries the access-gate claim
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  robo sync status            what is synced, and from where\n"
            "  robo sync enable my-skill   include a skill in your sync\n"
            "  robo sync now               pull, then push\n"
            "  robo sync propose my-skill  share a skill with your team\n"
        ),

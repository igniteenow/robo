...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``robo login`` was removed in favor of ``robo auth`` / ``robo model``
    (the runtime handler in ``robo_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``robo login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``robo --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``robo login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``robo model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `robo auth` to manage credentials, "
            "`robo model` to select a provider, or `robo setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `robo model`",
    )
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: robo-cli)"
    )

...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Robo. Use 'robo debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    robo debug share              Upload debug report (asks for confirmation)
    robo debug share --yes        Skip confirmation (for scripts/CI)
    robo debug share --lines 500  Include more log lines
    robo debug share --expire 30  Keep paste for 30 days
    robo debug share --local      Print report locally (no upload)
    robo debug share --no-redact  Disable upload-time secret redaction
    robo debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'robo debug share'",
    )

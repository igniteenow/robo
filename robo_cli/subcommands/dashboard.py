"""``robo dashboard`` / ``robo serve`` subcommand parsers.
...
``robo_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `robo dashboard` / `robo serve` cmdlines and SIGTERM them directly —
    # the same path `robo update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Robo web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Robo web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Robo web dashboard for managing config, API keys, and sessions",
    )
    ...
    # showed "Robo couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Robo Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Robo backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Robo backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports ROBO_SERVE_HEADLESS=1). `dashboard` leaves it

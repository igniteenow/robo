"""Shell completion script generation for robo CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_robo_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Robo bash completion
# Add to ~/.bashrc:
#   eval "$(robo completion bash)"

_robo_profiles() {{
    local profiles_dir="$HOME/.robo/profiles"
...
_robo_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_robo_profiles)" -- "$cur"))
        return
    fi
...
_robo_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.robo/profiles" ]]; then
        profiles+=($HOME/.robo/profiles/*(N/:t))
    fi
...
_robo() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_robo_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'robo command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Robo fish completion",
        "# Add to your config:",
        "#   robo completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __robo_profiles",
        "    echo default",
        "    if test -d $HOME/.robo/profiles",
        "        for d in $HOME/.robo/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c robo -f -s p -l profile"
        " -d 'Profile name' -xa '(__robo_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__robo_profiles)' -d 'Profile name'"
                )

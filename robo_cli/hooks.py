"""robo hooks — inspect and manage shell-script hooks.

Usage::

    robo hooks list
    robo hooks test <event> [--for-tool X] [--payload-file F]
    robo hooks revoke <command>
    robo hooks doctor

Consent records live under ``~/.robo/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.robo/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``robo hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: robo hooks {list|test|revoke|doctor}")
        print("Run 'robo hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.robo/config.yaml.")
    else:
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `robo hooks doctor` to re-validate"
                            )
            print()
...
# stdin a script sees under `robo hooks test` and `robo hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Robo wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `robo hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `robo hooks doctor` would execute

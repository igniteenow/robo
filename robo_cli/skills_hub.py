#!/usr/bin/env python3
"""
Skills Hub CLI — Unified interface for the Robo Skills Hub.

Powers both:
  - `robo skills <subcommand>` (CLI argparse entry point)
...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    if len(exact) > 1:
        ...
        # so users can copy it for `robo skills install`.
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.robo/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.robo/skills/<name>/)[/]"
        )
    try:
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    c.print(table)
    c.print("[dim]Use: robo skills inspect <identifier> to preview, "
            "robo skills install <identifier> to install "
            "(--json for scripting)[/]\n")
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    # skips the external API sources and serves everything from "robo-index".
    ...
    if official_count > 0 and page == 1:
        c.print(f"[bright_cyan]★ {official_count} verified optional skill(s) available[/]")
    c.print()
    ...
    # The identifier is what you pass to `robo skills install`. Browse used
    ...
    c.print("[dim]Tip: 'robo skills inspect <identifier>' to preview, "
            "'robo skills install <identifier>' to install, "
            "'robo skills search <query>' to search deeper[/]\n")
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    if bundle.source == "url" and (not bundle.name or bundle_meta.get("awaiting_name")):
        if name_override and _is_valid_installed_skill_name(name_override):
            ...
            c.print(
                ...
                f"  [bold]robo skills install {url} --name <your-name>[/]\n\n"
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                "[bold bright_cyan]This optional skill comes from a verified registry source.[/]\n\n"
                "It ships with robo-engineer but is not activated by default.\n"
                "Installing will copy it to your skills directory where the agent can use it.\n\n"
                f"Files will be at: [cyan]{display_robo_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
    ...
    # metadata.robo.blueprint block, it is a runnable automation. Register it as
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="robo skills install <id> to install"))
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    page_size = max(1, min(page_size, 100))
    _TRUST_RANK = {"builtin": 3, "trusted": 2, "community": 1}
    # "robo-index" must carry a high limit: when the index is available the
    ...
    _PER_SOURCE_LIMIT = {"robo-index": 5000, "official": 100, "skills-sh": 100,
    ...
    # source-skip logic — querying robo-index AND the external APIs at once
...
def do_list(source_filter: str = "all",
    ...
    config — ``robo -p <profile> skills list`` reads that profile's
    ``skills.disabled`` list because ``-p`` swaps ``ROBO_HOME`` at process
...
def do_list_modified(console: Optional[Console] = None,
    ...
    """List bundled skills the user has edited (which `robo update` keeps)."""
    ...
    c.print(f"\n[bold]{len(modified)} user-modified bundled skill(s)[/] "
            "[dim](kept as-is by `robo update`):[/]")
    for entry in modified:
        c.print(f"  [yellow]~[/] {entry['name']}")
    c.print()
    c.print("[dim]See changes:   robo skills diff <name>[/]")
    c.print("[dim]Resume updates: robo skills reset <name>          (keep your copy, re-baseline)[/]")
    c.print("[dim]Revert to stock: robo skills reset <name> --restore[/]\n")
...
def do_diff(name: str, console: Optional[Console] = None) -> None:
    ...
    c.print()
    c.print(f"[dim]Revert with: robo skills reset {name} --restore[/]\n")
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: robo skills tap add owner/repo\n")
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: robo skills tap remove owner/repo\n")
            return
        if mgr.remove(repo):
    ...
    if target == "github":
        if not repo:
            c.print("[bold red]Error:[/] --repo required for GitHub publish.\n"
                    "Usage: robo skills publish <path> --to github --repo owner/repo\n")
            return

        auth = GitHubAuth()
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_robo_home()}/.env or run 'gh auth login'.\n")
            return
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via Robo Skills Hub.\n\n"
                        f"This skill was scanned by the Robo Skills Guard before submission.",
...
# ---------------------------------------------------------------------------
# User-authored skills: create + lint
# ---------------------------------------------------------------------------
...
Describe the situations where Robo should reach for this skill — the task
shapes, file types, or phrases that should trigger it.

## Procedure
1. First step — what to check or read before doing anything.
2. Second step — the action, with the exact commands or edits.
3. Verification — how Robo proves the result is correct before reporting.

## Rules
- Facts only: cite the file/command/output a conclusion rests on.
- Never store secrets here; reference where a credential lives instead.
- Keep this under ~200 lines; split large workflows into related skills.

## Examples
- "<example request>" → <what Robo does with this skill>
"""
...
def _user_skills_root() -> "Path":
    ...
    try:
        ...
        return Path(get_robo_home()) / "skills"
    except Exception:
        return Path.home() / ".robo" / "skills"


def do_create(name: str, *, description: str = "", category: str = "custom", tags: str = "",
              edit: bool = False) -> None:
    """Scaffold ``~/.robo/skills/<category>/<name>/SKILL.md`` for a user skill."""
    import os
    import re
    import subprocess
    ...
    if not slug:
        ui.err("Skill name must contain letters or digits (e.g. release-checklist).")
        return
    cat = re.sub(r"[^a-z0-9-]+", "-", (category or "custom").strip().lower()).strip("-") or "custom"
    root = _user_skills_root() / cat / slug
    skill_md = root / "SKILL.md"
    if skill_md.exists():
        ui.warn(f"Skill already exists: {skill_md}")
        return
    root.mkdir(parents=True, exist_ok=True)
    tag_list = ", ".join(t.strip() for t in (tags or slug).split(",") if t.strip())
    text = _SKILL_TEMPLATE.format(
        name=slug,
        description=(description or f"Custom skill: {slug.replace('-', ' ')}").replace('"', "'"),
        author=os.environ.get("USER") or os.environ.get("USERNAME") or "operator",
        tags=tag_list,
        title=slug.replace("-", " ").title(),
    )
    skill_md.write_text(text, encoding="utf-8")
    ui.title("Skills", "new user skill")
    ui.kvs([("Skill", slug), ("Category", cat), ("File", str(skill_md))])
    ui.ok("Scaffolded. Fill in the procedure, then `robo skills lint " + slug + "`.")
    ui.hint("Robo loads user skills automatically; agent-created skills stay under the same guard "
            "(`skills.guard_agent_created`). Ask Robo to 'use the " + slug + " skill' to try it.")
    if edit:
        editor = os.environ.get("VISUAL") or os.environ.get("EDITOR")
        if editor:
            try:
                subprocess.call([editor, str(skill_md)])
            except Exception as exc:  # pragma: no cover - editor availability varies
                ui.warn(f"Could not open editor: {exc}")


def lint_skill_text(text: str) -> list:
    """Return a list of (severity, message) findings for a SKILL.md body."""
    findings = []
    if not text.lstrip().startswith("---"):
        findings.append(("error", "missing YAML frontmatter (--- name/description ---)"))
    else:
        head = text.split("---", 2)[1] if text.count("---") >= 2 else ""
        if "name:" not in head:
            findings.append(("error", "frontmatter has no `name:`"))
        if "description:" not in head:
            findings.append(("error", "frontmatter has no `description:`"))
    try:
        from agent.content_policy import _find, _OVERRIDE_PATTERNS, _SECRET_PATTERNS
        for label in _find(_SECRET_PATTERNS, text):
            findings.append(("error", f"contains a secret ({label}) — keep credentials in ~/.robo/.env"))
        for label in _find(_OVERRIDE_PATTERNS, text):
            findings.append(("error", f"tries to change Robo's safety behaviour ({label})"))
    except Exception:
        pass
    try:
        from tools.threat_patterns import scan_for_threats
        for f in scan_for_threats(text, scope="context") or []:
            findings.append(("error", f"injection pattern: {f}"))
    except Exception:
        pass
    if len(text.splitlines()) > 400:
        findings.append(("warn", "very long (>400 lines) — split into related skills"))
    if "## Procedure" in text and "1." not in text:
        findings.append(("warn", "procedure section has no numbered steps"))
    return findings


def do_lint(name: str = "") -> None:
    from robo_cli import design as ui

    root = _user_skills_root()
    files = list(root.rglob("SKILL.md")) if root.exists() else []
    if name:
        files = [f for f in files if f.parent.name == name]
    ui.title("Skills", f"lint · {len(files)} skill(s) under {root}")
    if not files:
        ui.info("No user skills found. Create one with `robo skills create <name>`.")
        return
    bad = 0
    for f in sorted(files):
        findings = lint_skill_text(f.read_text(encoding="utf-8", errors="replace"))
        errors = [m for sev, m in findings if sev == "error"]
        warns = [m for sev, m in findings if sev == "warn"]
        if errors:
            bad += 1
            ui.err(f"{f.parent.name}: {'; '.join(errors)}")
        elif warns:
            ui.warn(f"{f.parent.name}: {'; '.join(warns)}")
        else:
            ui.ok(f"{f.parent.name}")
    print()
    (ui.err if bad else ui.ok)(f"{bad} skill(s) need attention" if bad else "All user skills pass the content policy.")
...
def skills_command(args) -> None:
    """Router for `robo skills <subcommand>` — called from robo_cli/main.py."""
    action = getattr(args, "skills_action", None)

    if action == "create":
        do_create(args.name, description=args.description, category=args.category,
                  tags=args.tags, edit=getattr(args, "edit", False))
        return
    if action == "lint":
        do_lint(getattr(args, "name", "") or "")
        return

"""Client for uploading ``robo debug share`` bundles to Ignitee Now-internal S3.

This is the opt-in (``--igniteenow``) destination for ``robo debug share``.
Unlike the public paste.rs path, bundles uploaded here go to private object
storage via a short-lived signed URL minted by the Ignitee Now support service
(referred to below as NAS).

OPERATOR NOTE: this client defines the contract; it does not provide the
service. Before advertising ``--igniteenow`` to users, Ignitee Now must run an
endpoint at ``ROBO_DIAGNOSTICS_BASE_URL`` that implements the flow below, with
access control and a retention policy that match the privacy notice shown by
``robo debug share --igniteenow``. Until then the command fails cleanly and
points users at ``--local`` or the public paste path.
...
# Base URL of the Ignitee Now account service that mints the signed upload URL.
...
NAS_BASE = os.environ.get(
    "ROBO_DIAGNOSTICS_BASE_URL", "https://portal.igniteenow.com"
)
...
_USER_AGENT = "robo-engineer/debug-share"
...
def share_to_igniteenow(report_bundle: bytes) -> dict:
    """Orchestrate the full Ignitee Now-S3 upload of a gzipped *report_bundle*.

"""
Unified tool configuration for Robo.

`robo tools` and `robo setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.robo/config.yaml under
...
from robo_cli.igniteenow_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    IgniteeNowSubscriptionFeatures,
    apply_igniteenow_managed_defaults,
    get_igniteenow_subscription_features,
)
from robo_cli.igniteenow_account import format_igniteenow_portal_entitlement_message
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``robo tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`robo_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from robo_cli.cli_output import (  # noqa: E402 — late import block
...
CONFIGURABLE_TOOLSETS = [
    ...
    ("stt",             "Speech-to-Text",           "voice transcription (gateway voice messages + voice mode)"),
    ...
    ("reverse_engineering", "Reverse Engineering",   "decompile, xrefs, strings, call graphs — Ghidra/radare2/binutils, no MCP needed; read-only static analysis of binaries and firmware"),
    ...
    ("computer_use",     "Computer Use (macOS/Windows/Linux)", "background desktop control via cua-driver"),
]
...
# They're still in _ROBO_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `robo tools` → Video Generation, which walks
...
# `robo tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "a2a"}


# Config-only capabilities: they appear in `robo tools` for provider/API-key
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from robo_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `robo tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``robo tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from robo_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``robo tools`` checklist actually offers
    for ``platform``.
    ...
    so ``robo tools`` never claims to add or remove a toolset the user was
...
from robo_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
def _platform_composite_keys() -> Set[str]:
    """Every toolset name that stands for a whole platform composite.

    Covers the current per-platform defaults (``robo-core``, ``robo-telegram``,
    ...) AND every ``robo-*`` composite still defined in ``toolsets.py`` —
    notably the legacy ``robo-cli`` alias that the example config and older
    installs use. A composite resolves to ALL core tools, so if one is ever
    treated as an ordinary explicit entry it survives every ``robo tools`` save
    and re-expands on the next read, silently undoing whatever the user just
    unchecked.
    """
    from toolsets import TOOLSETS

    keys = {p["default_toolset"] for p in PLATFORMS.values()}
    keys |= {k for k in TOOLSETS if k.startswith("robo-")}
    return keys
...
TOOL_CATEGORIES = {
    ...
    "web": {
        ...
        #   - "Ignitee Now Subscription" — managed Firecrawl billed via Ignitee Now
        #     subscription (requires_igniteenow_auth + override_env_vars).
    ...
    "video_gen": {
        ...
        "icon": "",
        # "Ignitee Now Subscription" row mirrors the image_gen pattern — managed
        # FAL video generation. Plugin-backed
    ...
    "x_search": {
        ...
        "setup_note": (
            "Robo routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        # backend, never on the paid Ignitee Now Subscription gateway row:
        #   - "Local Browser" — non-cloud option, no CloudBrowserProvider.
        #   - "Ignitee Now Subscription (Browser Use cloud)" — managed Browser Use
        #     billed via Ignitee Now subscription (requires_igniteenow_auth +
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set ROBO_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "ROBO_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "ROBO_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "ROBO_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "ROBO_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "ROBO_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `robo model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("ROBO_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Robo telemetry policy applied.
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $ROBO_HOME/bin is never on PATH, so a bare which()
    # misses the uv Robo installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``robo update`` and
      by ``robo computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``robo update`` sets
    ...
    a further ~600s wait on Windows). ``robo computer-use install
    ...
    ``robo update`` already prints a contextual line before its update
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `robo update` calls this
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `robo computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `robo update` (require_confirmed_update=True) keeps the
    ...
    # `robo computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: robo computer-use install --upgrade"
            )
            return True
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `robo update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `robo update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.robo/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Robo process.")
            return True
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # Managed Node first — $ROBO_HOME/node is not on PATH, so a bare
        # which() reports "no npm" on installs whose only Node is the one
        # Robo installed for exactly this toolchain.
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_robo_home()}/robo-engineer && npm install --workspaces=false")
                if result.stderr:
            ...
            _print_warning("    Node.js not found - browser tools require: npm install (in robo-engineer directory)")
            return
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.robo/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.robo/config.yaml")
        ...
        # Run the full `robo auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.robo/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from robo_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: robo auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `robo auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: robo auth spotify")
        except Exception as exc:
        ...
        try:
            from robo_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: robo plugins enable observability/langfuse")
        _print_info("    Restart Robo for tracing to take effect.")
        _print_info("    Verify: robo plugins list")
        ...
        try:
            from robo_cli.auth import get_xai_oauth_auth_status
        ...
        try:
            ...
            _print_info("    Run later: robo auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `robo auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: robo auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: robo auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``robo tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``robo tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: robo tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``robo-discord``, which contains those
...
#:
#: Saving ``robo tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[robo-cli]`` inherits it
#: on upgrade. Listing a toolset here restores that parity.
#:
...
# ``reverse_engineering`` shipped in 3.0.1: back-fill it onto saved
# lists for this release only, then drop it from here in the next one.
_RECENTLY_SHIPPED_TOOLSETS = frozenset({"reverse_engineering"})
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``robo tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"robo-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (robo-acp, robo-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``robo-discord``) is an opt-in to the platform's native default-off
    ...
    platform_default_keys = _platform_composite_keys()

    # If the saved list contains any configurable keys directly, the user
    # has explicitly configured this platform — use direct membership.
    # This avoids the subset-inference bug where composite toolsets like
    # "robo-cli" (which include all _ROBO_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[robo-cli, spotify]`` after enabling Spotify via ``robo
        ...
        # (e.g. "robo-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``robo tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `robo tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"robo-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("robo-")}
    ...
    # shipped as a bundled plugin but user must opt in via `robo tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `robo tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by robo tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `robo` instead of
    # `robo-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `robo update`/`robo doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `robo tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform composite toolsets (robo-core, robo-cli,
    # robo-telegram, etc.). These are "super" toolsets that resolve to ALL
    # tools, so preserving them would silently override the user's unchecked
    # selections on the next read. Includes the legacy ``robo-cli`` alias.
    platform_default_keys = _platform_composite_keys()
    ...
    # Opening `robo tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(
    ...
    *,
    ...
    features: Optional[IgniteeNowSubscriptionFeatures] = None,
) -> bool:
    ...
    if ts_key in {"web", "image_gen", "video_gen", "tts", "stt", "browser"}:
        if features is None:
            features = get_igniteenow_subscription_features(
                config, force_fresh=force_fresh
            )
        feature = features.features.get(ts_key)
        if feature and (feature.available or feature.managed_by_igniteenow):
            return True
...
def _prompt_toolset_checklist(
    ...
    *,
    ...
    from robo_cli.curses_ui import curses_checklist
    from toolsets import resolve_toolset
...
def _plugin_image_gen_providers() -> list[dict]:
    ...
    try:
        ...
        from robo_cli.plugins import _ensure_plugins_discovered
...
# ("Ignitee Now Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("Ignitee Now Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(
    ...
    *,
    ...
    Ignitee Now-managed Tool Gateway rows (``managed_igniteenow_feature``) are always
    shown — even to logged-out / unentitled users — so the picker advertises
    that the capability exists.  Selecting one drives an inline Ignitee Now Portal
    login + entitlement check (see ``_configure_provider``); the row only
    *activates* the gateway once paid access is confirmed.
    """
    if features is None:
        features = get_igniteenow_subscription_features(config, force_fresh=force_fresh)
    ...
    for provider in cat.get("providers", []):
        # Ignitee Now-managed Tool Gateway rows stay visible regardless of auth —
        # selecting one drives an inline Portal login. A `requires_igniteenow_auth`
        # row that is NOT a managed gateway feature (pure pre-auth UX) is
        # still hidden until the user is logged in.
        if (
            provider.get("requires_igniteenow_auth")
            and not provider.get("managed_igniteenow_feature")
            and not features.igniteenow_auth_present
        ):
            continue
        ...
        if (
            pool_only
            and provider.get("managed_igniteenow_feature") == "video_gen"
    ...
    # later) so the picker lists them alongside FAL / Ignitee Now Subscription.
    ...
    # remaining hardcoded rows ("Ignitee Now Subscription", "Firecrawl
    ...
    # the hardcoded "Ignitee Now Subscription" / "Local Browser" / "Camofox" rows
...
def _hidden_igniteenow_gateway_message(
    cat: dict,
    config: dict,
    capability: str,
    *,
    ...
    """Deprecated: Ignitee Now Tool Gateway rows are no longer hidden.

    Previously this returned a "log in / upgrade" banner shown above a
    category when its Ignitee Now-managed rows were filtered out for unentitled
    ...
    (``ensure_igniteenow_portal_access``). Kept as a no-op so call sites stay simple;
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `robo tools` gets a silent no-op
...
def _agent_browser_installed() -> bool:
    ...
    # The install hook runs in a spawned ``robo tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from robo_cli.igniteenow_subscription import _has_agent_browser
...
def provider_readiness_status(
    ...
    *,
    ...
    - ``"needs_auth"``  — needs a sign-in: Ignitee Now Portal login/entitlement for
    ...
    zero-env-var row, including logged-out Ignitee Now Subscription rows).

    ``features`` (a ``IgniteeNowSubscriptionFeatures``) can be passed to avoid
    ...
    managed_feature = provider.get("managed_igniteenow_feature")
    if provider.get("requires_igniteenow_auth") or managed_feature:
        if features is None:
            features = get_igniteenow_subscription_features(config)
        if not features.igniteenow_auth_present:
            return "needs_auth"
        if managed_feature:
...
def _configure_tool_category(
    ...
    *,
    ...
    hidden_igniteenow_message = _hidden_igniteenow_gateway_message(
        cat,
        config,
        f"the Ignitee Now Subscription provider for {name}",
        force_fresh=force_fresh,
    )
    ...
    if len(providers) == 1:
        ...
        if hidden_igniteenow_message:
            for line in hidden_igniteenow_message.splitlines():
        ...
        # When the user is logged into Ignitee Now, surface a marker on providers
        # whose access is included in their subscription so it's visually
        # obvious which options cost extra vs. cost nothing on top of Ignitee Now.
        try:
            _igniteenow_logged_in = bool(
                get_igniteenow_subscription_features(
                    config,
                    force_fresh=force_fresh,
                ).igniteenow_auth_present
            )
        except Exception:
            _igniteenow_logged_in = False
        ...
        for p in providers:
            ...
            # Mark Ignitee Now-managed entries. Logged-in paid subscribers get the
            # "included" star; everyone else gets a "via Ignitee Now Portal" hint so
            ...
            sub_marker = ""
            if p.get("managed_igniteenow_feature"):
                if _igniteenow_logged_in:
                    sub_marker = "  ★ Included with your Ignitee Now subscription"
                else:
                    sub_marker = "  ★ via Ignitee Now Portal (login on select)"
...
def _is_provider_active(
    ...
    *,
    ...
    if video_plugin_name and not provider.get("managed_igniteenow_feature"):
    ...
    if managed_feature:
        ...
        if managed_feature == "image_gen":
            ...
            if isinstance(image_cfg, dict):
                ...
                if image_cfg.get("use_gateway") is not None and not is_truthy_value(image_cfg.get("use_gateway"), default=False):
                    return False
            return feature.managed_by_igniteenow
        ...
        if "browser_provider" in provider:
            ...
            return feature.managed_by_igniteenow and provider["browser_provider"] == current
        if provider.get("web_backend"):
            current = cfg_get(config, "web", "backend")
            return feature.managed_by_igniteenow and current == provider["web_backend"]
        return feature.managed_by_igniteenow
...
# Kept in sync with the dashboard selects (robo_cli/web_server.py
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    NOT prompt for env vars, run post-setup hooks, gate on Ignitee Now auth, or run
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    post-setup hooks, gate on Ignitee Now Portal auth, or run interactive model
...
def _configure_provider(
    ...
    *,
    ...
    # Ignitee Now-managed Tool Gateway backends are always listed (see
    # _visible_providers), but only *activate* once the user has paid Ignitee Now
    ...
    # "enable all tools" prompt (that lives in `robo model`).
    if managed_feature:
        from robo_cli.igniteenow_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
            ensure_igniteenow_portal_access,
        )

        if not ensure_igniteenow_portal_access(
            capability=f"{provider.get('name', 'the Ignitee Now Tool Gateway')}",
            coverage_category=MANAGED_FEATURE_COVERAGE_CATEGORY.get(managed_feature),
        ):
            _print_warning(
                "  Not enabled — Ignitee Now Portal access is required for this backend."
            )
            return

    # Pure pre-auth UX rows (requires_igniteenow_auth without a managed gateway
    # feature) keep the old gate. Managed rows are handled by the inline
    # login above, so don't double-check them here.
    if provider.get("requires_igniteenow_auth") and not managed_feature:
        features = get_igniteenow_subscription_features(config, force_fresh=force_fresh)
        entitled = bool(
            features.account_info and features.account_info.paid_service_access is True
        )
        if not features.igniteenow_auth_present or not entitled:
            message = format_igniteenow_portal_entitlement_message(
                features.account_info,
                capability=f"{provider.get('name', 'Ignitee Now Subscription')}",
            )
            _print_warning(
                f"  {message or 'Ignitee Now Subscription is only available after logging into Ignitee Now Portal.'}"
            )
            return
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your Ignitee Now subscription.")
    ...
    # Ignitee Now-managed sibling, show a single dim hint so users know
    # they can avoid the key entirely via a Portal subscription.
    # Suppressed when the user is already authed to Ignitee Now.
    _show_portal_hint = False
    if env_vars and not managed_feature and not provider.get("requires_igniteenow_auth"):
        try:
            ...
            for _cat_key, _cat in TOOL_CATEGORIES.items():
                ...
                if provider in _providers and any(
                    sib.get("managed_igniteenow_feature") for sib in _providers
                ):
                    _has_managed_sibling = True
                    break
            if _has_managed_sibling:
                _features = get_igniteenow_subscription_features(
                    config,
                    force_fresh=force_fresh,
                )
                _show_portal_hint = not _features.igniteenow_auth_present
        except Exception:
            _show_portal_hint = False

    if _show_portal_hint:
        _print_info("  Available through Ignitee Now Portal subscription.")
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``robo model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``robo model`` aux-task
    ...
    """
    try:
        from robo_cli.inventory import (
    ...
    if not providers:
        _print_warning(
            ...
            "with `robo model`, then re-run this."
        )
        return
...
def _reconfigure_provider(
    ...
    *,
    ...
    # Same inline Ignitee Now Portal login + entitlement gate as _configure_provider:
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `robo model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `robo tools` and `robo setup tools`.
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            auto_configured = apply_igniteenow_managed_defaults(
                config,
            ...
            for ts_key in sorted(auto_configured):
                ...
                print(color(f"  ✓ {label}: using your Ignitee Now subscription defaults", Colors.GREEN))
    ...
    print()
    from robo_constants import display_robo_home
    print(color(f"  Tool configuration saved to {display_robo_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'robo' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, robo mcp

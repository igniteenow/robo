...
``cli.py``'s ``RoboCLI`` class. ``RoboCLI`` inherits ``CLICommandsMixin`` so
...
from robo_constants import display_robo_home, is_termux as _is_termux_environment
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: robo --checkpoints")
    ...
    def _handle_diff_command(self, command: str):
        ...
        Syntax:
            ...
            /diff session          — everything Robo changed (checkpoint baseline)
    ...
    def _print_session_diff(self, cwd: str, stat_only: bool):
        ...
        if result.get("empty") or (not stat and not diff):
            print("  No changes — Robo hasn't edited any files here yet.")
            return
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Robo config/state.
    ...
    def _handle_export_command(self, command: str):
        ...
        """
        from robo_cli.profiles import export_profile, get_active_profile_name
        ...
        try:
            ...
            print("  Share it: the other user runs /import or `robo profile import <archive>`.")
    ...
    def _handle_import_command(self, command: str):
        ...
        try:
            ...
            pass
        print(f"  Use it: robo -p {imported}")
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `robo journey`).
        ...
        from cli import _cprint
        from robo_cli.journey import register_cli
    ...
    def _handle_paste_command(self):
        ...
        from robo_cli.clipboard import has_clipboard_image
    ...
    def _handle_copy_command(self, cmd_original: str) -> None:
        ...
        try:
            from robo_cli.clipboard import (
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run robo chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in robo_cli/colors.py
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `robo gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `robo sessions list` to find sessions.")
            return
        ...
        if target.isdigit():
            ...
            from robo_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `robo sessions list` to see available sessions.")
            return
        ...
        # same directory as a startup `robo -c`/`--resume`. The startup resume
        ...
        if len(parts) > 1:
            ...
            if personality_name in {"none", "default", "neutral"}:
                ...
                if save_config_value("agent.system_prompt", ""):
                    print("✓b Personality cleared (saved to config)")
                else:
                ...
                if save_config_value("agent.system_prompt", self.system_prompt):
                    print(f"✓b Personality set to '{personality_name}' (saved to config)")
                else:
    ...
    def _handle_pet_command(self, cmd: str):
        ...
        from robo_cli.pets import _set_active, _set_enabled, print_pet_gallery, set_pet_scale, toggle_pet_display
        ...
        if not arg or low == "toggle":
            ...
            if err:
                ...
                return
            if enabled:
                print(f"✓b {name} is out — it'll pop in shortly.")
            else:
                print(f"·zzZ {name} put away." if name else "·zzZ Pet put away.")
            return
        ...
        try:
            ...
            print(f"✗ Couldn't adopt '{arg}': {exc}")
            return
        _set_active(arg)
        print(f"✓b {pet.display_name} is out — it'll pop in shortly.")
    ...
    def _handle_hatch_command(self, cmd: str):
        ...
        from robo_cli.pets import _set_active
        ...
        try:
            ...
            print(f"✗ Couldn't generate a base look: {exc}")
            return

        if not drafts:
            print("✗ No base draft came back — try again.")
            return
        ...
        try:
            ...
            print(f"✗ Hatch failed: {exc}")
            return

        _set_active(result.slug)
        print(f"✓b {result.display_name} hatched and adopted — it'll pop in shortly!")
    ...
    def _handle_cron_command(self, cmd: str):
        ...
        if subcommand in {"add", "create"}:
            ...
            if result.get("success"):
                print(f"✓b Created job: {result['job_id']}")
        ...
        if subcommand == "edit":
            ...
            if result.get("success"):
                job = result["job"]
                print(f"✓b Updated job: {job['job_id']}")
        ...
        if subcommand in {"pause", "resume", "run", "remove", "rm", "delete"}:
            ...
            if not result.get("success"):
                ...
                return
            if action == "pause":
                print(f"✓b Paused job: {result['job']['name']} ({job_id})")
            elif action == "resume":
                print(f"✓b Resumed job: {result['job']['name']} ({job_id})")
                print(f"  Next run: {result['job'].get('next_run_at')}")
            elif action == "run":
                print(f"✓b Triggered job: {result['job']['name']} ({job_id})")
                print("  It will run on the next scheduler tick.")
            else:
                removed = result.get("removed_job", {})
                print(f"✓b Removed job: {removed.get('name', job_id)} ({job_id})")
            return
    ...
    def _handle_suggestions_command(self, cmd: str):
        ...
        args = " ".join(tokens)
        try:
            from robo_cli.suggestions_cmd import handle_suggestions_command
    ...
    def _handle_blueprint_command(self, cmd: str):
        ...
        try:
            from robo_cli.blueprint_cmd import handle_blueprint_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to robo_cli.curator so the CLI and the `robo curator`
        ...
        try:
            from robo_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from robo_cli.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to robo_cli.skills_hub."""
        ...
        if args and args[0].lower() in {"pending", "approve", "apply", "reject",
            ...
            from robo_cli.write_approval_commands import handle_pending_subcommand
            ...
            if out is not None:
                print(out)
                return
        from robo_cli.skills_hub import handle_skills_slash
    ...
    def _handle_init_command(self, cmd: str):
        ...
        """
        from robo_cli.init_command import build_init_prompt_for_cwd
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``robo bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: robo bundles create "
        ...
        _cprint(
            ...
            f"Manage with `robo bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Robo-managed isolated debug "
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        cross-process schedules use `robo cron`.
        """
        from cli import _DIM, _RST, _cprint
        from robo_cli.heartbeat import parse_interval, format_interval
        ...
        if interval is None:
            ...
            return
        if interval < 0:
            from robo_cli.heartbeat import MIN_INTERVAL_SECONDS
        ...
        _cprint(
            ...
            f"lives only while this Robo process runs — use `robo cron` for "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        from robo_cli.goals import parse_contract
        ...
        _cprint(
            ...
            f"Robo keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_goal_draft(self, objective: str) -> None:
        ...
        from cli import _DIM, _RST, _cprint
        from robo_cli.goals import draft_contract
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from robo_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_robo_home()}/skins/\n")
            return
    ...
    def _compose_in_editor(self, initial_text: str = "") -> str:
        ...
        fd, path = tempfile.mkstemp(suffix=".md", prefix="robo_prompt_")
        try:
    ...
    def _handle_prompt_compose_command(self, cmd_original: str) -> None:
        ...
        try:
            ...
            _cprint(f"  {_DIM}✗ Could not open editor: {exc}{_RST}")
            return

        if not composed:
            _cprint(f"  {_DIM}· Empty prompt — nothing sent.{_RST}")
            return
    ...
    def _set_tool_progress_mode(self, mode: str) -> None:
        ...
        """
        from robo_cli.focus_view import normalize_tool_progress_mode
    ...
    def _note_focus_hidden_line(self, function_name: str) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from robo_cli.focus_view import would_display_tool_line
    ...
    def _emit_focus_recovery_line(self) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from robo_cli.focus_view import format_hidden_line
    ...
    def _handle_approvals_command(self, cmd_original: str) -> None:
        ...
        from cli import _cprint
        from robo_cli.approval_mode import run_approval_mode_command
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Robo is working.
        ...
        arg = parts[1].strip().lower()
        if arg not in {"queue", "interrupt", "steer", "redirect"}:
            _cprint(f"  {_DIM}· Unknown argument: {arg}{_RST}")
            _cprint(f"  {_DIM}Usage: /busy [interrupt|redirect|queue|steer|status]{_RST}")
            return

        self.busy_input_mode = arg
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Robo is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will redirect the current run while Robo is busy; /stop still cancels it."
    ...
    def _handle_indicator_command(self, cmd: str):
        ...
        """
        from cli import _ACCENT, _DIM, _RST, _cprint, save_config_value
        from robo_constants import DEFAULT_INDICATOR_STYLE, INDICATOR_STYLES
        ...
        if arg not in styles:
            _cprint(f"  {_DIM}· Unknown indicator style: {arg}{_RST}")
            _cprint(f"  {_DIM}Usage: /indicator [{'|'.join(styles)}]{_RST}")
            return
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from robo_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
    ...
    def _handle_debug_command(self, cmd_original: str = ""):
        ...
        - ``/debug igniteenow``   → upload to Ignitee Now-internal storage (private, staff-only)
        - ``/debug local``  → render the report to stdout, no upload

        ``igniteenow`` and ``local`` are mutually exclusive; if both are given,
        ...
        igniteenow = "igniteenow" in words and not local
        ...
        args = SimpleNamespace(
            lines=200, expire=7, local=local, igniteenow=igniteenow, yes=True
        )
        run_debug_share(args)

    def _handle_update_command(self) -> bool:
        """Handle /update — update Robo to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``robo update`` so the user sees update output directly and gets
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Robo')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Robo"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title=" Update Robo",
            detail="This will exit the current session and run `robo update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _handle_wake_command(self, command: str):
        """Handle /wake [on|off|status] — the 'Hey Robo' hotword listener.

"""
Single source of truth for provider identity in Robo.
...
2. **Robo overlays** — transport type, auth patterns, aggregator flags,
...
# -- Robo overlay ----------------------------------------------------------
# Robo-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class RoboOverlay:
    """Robo-specific provider metadata layered on top of models.dev."""
...
ROBO_OVERLAYS: Dict[str, RoboOverlay] = {
    "moa": RoboOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": RoboOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENROUTER_BASE_URL",
    ),
    # Reserved slot for a future Ignitee Now managed inference portal.
    # Deliberately has NO hardcoded endpoint: the provider is dormant until
    # IGNITEENOW_BASE_URL is set by the operator (and a backend exists to
    # serve it). Without that env var it cannot resolve a base URL and the
    # model wizard will not offer it. All portal/subscription/billing plumbing
    # in robo_cli/igniteenow_*.py and agent/igniteenow_rate_guard.py reports
    # "not available" until then.
    "igniteenow": RoboOverlay(
        transport="openai_chat",
        auth_type="oauth_device_code",
        base_url_env_var="IGNITEENOW_BASE_URL",
    ),
    "openai-codex": RoboOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": RoboOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": RoboOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": RoboOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="ROBO_QWEN_BASE_URL",
    ),
    "lmstudio": RoboOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": RoboOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": RoboOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": RoboOverlay(
    ...
    "minimax-oauth": RoboOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": RoboOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": RoboOverlay(
        transport="openai_chat",
    ...
    "alibaba-coding-plan": RoboOverlay(
        transport="openai_chat",
    ...
    "opencode": RoboOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": RoboOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "robo", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Robo overlays (for providers not in models.dev: igniteenow, openai-codex, etc.)
      2. models.dev catalog + Robo overlay
    ...
    overlay = ROBO_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + robo extra
    ...
    if overlay is not None:
        # Robo-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def igniteenow_api_mode(model: str = "") -> str:
    """Resolve the wire protocol for a Ignitee Now Portal model.
    ...
    historical Ignitee Now transport — so callers that don't yet know the model
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    Resolution order:
      ...
      2. Ignitee Now Portal dual-wire (model-derived; overlay alone is openai_chat).
      3. Known provider → transport → TRANSPORT_TO_API_MODE.
      4. Direct provider checks (bedrock).
      5. Default: 'chat_completions'.

    *model* is optional but required for dual-wire providers (Ignitee Now) whose
    ...
    # Ignitee Now is dual-wire: anthropic/* → Messages, everything else →
    # chat_completions. The Robo overlay still advertises openai_chat
    ...
    if provider_norm in {"igniteenow", "igniteenow-portal"}:
        return igniteenow_api_mode(model)

    pdef = get_provider(provider)
    if pdef is not None:
        return TRANSPORT_TO_API_MODE.get(pdef.transport, "chat_completions")

    # Direct provider checks for providers not in ROBO_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Robo provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from robo_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY

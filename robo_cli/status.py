"""
Status command for robo CLI.

Shows the status of all Robo components.
"""
...
from robo_cli.auth import AuthError, resolve_provider
from robo_cli.colors import Colors, color
from robo_cli.config import get_env_path, get_env_value, get_robo_home, load_config
from robo_cli.models import provider_label
from robo_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``robo config``'s
...
def show_status(args):
    """Show status of all Robo components."""
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `robo status`
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: robo model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: robo auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: robo auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: robo model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from robo_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'robo-engineer[vercel]')"
    ...
    try:
        from robo_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_robo_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_robo_home() / "sessions" / "sessions.json"
    ...
    try:
        from robo_cli.active_sessions import (
    ...
    print(color("  Run 'robo doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'robo setup' to configure", Colors.DIM))
    print()

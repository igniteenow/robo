"""Agent-construction and session-resume display methods for ``RoboCLI``.

Extracted from ``cli.py`` as part of the god-file decomposition campaign
(``~/.robo/plans/god-file-decomposition.md``, Phase 4 step 2). This mixin holds
...
Behavior-neutral: every method is lifted verbatim from ``RoboCLI``. ``self.*``
...
class CLIAgentSetupMixin:
    """Agent construction + session-resume display methods for ``RoboCLI``."""
    ...
    def _ensure_runtime_credentials(self) -> bool:
            ...
            if isinstance(_primary_exc, AuthError):
                ...
                for _fb in _fb_chain:
                    ...
                    try:
                        ...
                        _cprint(f"⚠  Primary auth failed — switching to fallback: {_fb_provider} / {_fb_model}")
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                if _prov and _prov != "auto":
                    print(f"\n⚠  No API key found for provider '{_prov}'.")
                else:
                    print("\n⚠  No inference provider is configured.")
                print("   Run 'robo model' to choose a provider, or "
                      "'robo setup' for first-time setup.")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: robo setup")
            return False
        ...
        # `robo chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `robo auth add openai-codex`
        # without `robo model`), fall back to the provider's first catalog
    ...
    def _offer_first_run_setup(self) -> bool:
        ...
        exact same flow as ``robo model`` (which fronts Quick Setup / Ignitee Now
        ...
        _cprint("  You'll pick a provider (Ignitee Now Portal OAuth is the fastest; "
        ...
        try:
            ...
            answer = "n"
        if answer in {"n", "no"}:
            _cprint("  Skipped. Run 'robo model' or 'robo setup' any time.")
            return False
        ...
        try:
            ...
            print()
            _cprint("  Setup cancelled. Run 'robo model' any time.")
            return False
        except Exception as exc:
            logger.debug("first-run provider setup failed: %s", exc)
            _cprint(f"  ⚠  Provider setup failed: {exc}")
            _cprint("  Run 'robo model' to try again.")
            return False
        ...
        if self._runtime_credentials_ready():
            ...
            return True
        _cprint("  Provider setup didn't complete. Run 'robo model' to retry.")
        return False
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        from robo_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (robo sessions list).",
                        file=sys.stderr,
                    )
                else:
                    _cprint(f"\033[1;31mSession not found: {self.session_id}{_RST}")
                    _cprint(f"{_DIM}Use a session ID from a previous CLI run (robo sessions list).{_RST}")
                return False
        ...
        try:
            ...
            # + fail-open inside the helper; harmless for non-Ignitee Now providers.
            try:
            ...
            from robo_constants import partial_update_hint
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "event":
                ...
                lines.append("  ◆ Robo: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Robo: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")

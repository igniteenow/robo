...
Pure logic: walks a Robo config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `robo doctor` and a future `robo migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Robo config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Robo config and return retirement issues.
...
def apply_migration(
    ...
    from robo_cli.config import require_readable_config_before_write
    ...
    # None``, so the next `robo migrate xai` reports nothing to migrate

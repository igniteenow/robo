"""
Dump command for robo CLI.

Outputs a compact, plain-text summary of the user's Robo setup
...
from robo_cli.config import get_robo_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.robo/.env.
    ...
    exports. ``robo debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.robo_build_sha`` by the Dockerfile's
    ``ROBO_GIT_SHA`` build-arg (see ``robo_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``robo dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from robo_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(robo_home: Path) -> int:
...
def _cron_summary(robo_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = robo_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"robo_home:      {display_robo_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.robo/.env: a managed
        ...
        # A credential added via `robo auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `robo auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["robo-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(robo_home)}")
    lines.append(f"  skills:             {_count_skills(robo_home)}")

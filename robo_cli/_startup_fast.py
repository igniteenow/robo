...
This module is imported by ``robo_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no robo_cli.config,
...
``robo_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(robo_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect ROBO_HOME?"""
    active_profile = os.path.join(robo_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    ...
    if robo_home:
        return robo_home
    return os.path.join(os.path.expanduser("~"), ".robo")
...
def container_mode_may_be_active() -> bool:
    ...
    robo_home = os.environ.get("ROBO_HOME", "").strip()
    if robo_home:
        if os.path.exists(os.path.join(robo_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(robo_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(robo_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".robo")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect ROBO_HOME.
    """
...
def print_fast_version_info() -> None:
    ...
    print(f"Robo v{__version__} ({__release_date__})")
    ...
    print("Run 'robo version' for update status.")


def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``robo --version`` before the heavy import wall.

    Termux keeps its historical contract (also accepts the ``version``
    subcommand + the ROBO_TERMUX_DISABLE_FAST_CLI escape hatch). Everywhere
    ...
    if is_termux and os.environ.get("ROBO_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:

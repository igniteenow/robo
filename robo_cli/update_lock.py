"""Cross-process mutual exclusion for in-flight Robo updates.

Three different surfaces can start an update of the same install tree:

* ``robo update`` from a terminal,
* the dashboard's Update button (``POST /api/robo/update`` →
  ``_spawn_robo_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``robo-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``robo update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <ROBO_HOME>/.robo-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``robo update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Robo is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``robo-setup`` binary under ``~/.robo`` is only refreshed by a full
  installer run (``copy_self_to_robo_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Robo is still running") on every
  GUI update forever, with no Robo process actually running.
"""
...
MARKER_NAME = ".robo-update-in-progress"

# Set by an orchestrating updater (the Tauri `robo-setup --update` flow) to
# its own pid before spawning `robo update` as a child stage. The parent
...
HANDOFF_PID_ENV = "ROBO_UPDATE_HANDOFF_PID"
...
# to show "Robo is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Robo home (never the context-local profile override):
    the Rust updater resolves ``$ROBO_HOME`` or the platform default, and the
    ...
    """
    from robo_constants import get_process_robo_home

    return get_process_robo_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``robo update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``robo-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Robo update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `robo update` as a stage): we run under ITS claim

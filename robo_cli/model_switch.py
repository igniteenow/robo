...
- ``robo_cli.providers``        -- canonical provider identity + overlays
- ``robo_cli.model_normalize``  -- per-provider name formatting
...
def _models_config_is_allowlist(value: Any) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``robo model`` wizard — not a
    ...
    while ``robo model`` still live-probed the full ``/v1/models`` list.
...
_HERMES_CHAT_MODEL_WARNING = (
    "The third-party Hermes-3 / Hermes-4 chat models are NOT agentic and are "
    "not designed for use with Robo. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)
# Backwards-compatible alias for existing call sites.
_ROBO_MODEL_WARNING = _HERMES_CHAT_MODEL_WARNING

# Match only the third-party open-weight Hermes-3 / Hermes-4 chat family.
# A plain substring check false-positives on unrelated local Modelfiles that
# merely contain a similar token, so this is anchored on the model-family
# prefix and its major version.
#
# Positive examples the regex must match:
#   NousResearch/Hermes-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b,
#   hermes_4_70b
# Negative examples it must NOT match:
#   robo-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_HERMES_CHAT_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])hermes[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
def is_hermes_chat_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a third-party Hermes-3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_HERMES_CHAT_NON_AGENTIC_RE.search(model_name))


# Backwards-compatible alias for existing call sites.
is_igniteenow_robo_non_agentic = is_hermes_chat_non_agentic


def _check_robo_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a third-party Hermes-3/4 chat model."""
    if is_hermes_chat_non_agentic(model_name):
        return _ROBO_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``robo config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from robo_cli.model_switch import
...
async def resolve_display_context_length_async(
    ...
    ``/v1/models``, Copilot, Codex, GMI, Ollama, models.dev and
    ...
    ``robo_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'robo model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'robo doctor' — config issues detected:"
        ...
        from robo_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from robo_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from robo_cli.config import load_config
            from robo_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from robo_cli.providers import resolve_user_provider as _ruser
    ...
    # robo_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from robo_cli.models import normalize_opencode_base_url
    ...
    robo_warn = _check_robo_model_warning(new_model)
    if robo_warn:
        warnings.append(robo_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from robo_cli.config import normalize_extra_headers
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from robo_cli/models.py (OPENROUTER_MODELS,
    ...
    # Compared against robo_id / mdev_id (section 1), pid / robo_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from robo_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by robo provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from robo_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from robo_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Robo-mapped providers ---
    from robo_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from robo_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for robo_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(robo_id)
        if (
            _alias_target
            and _alias_target != robo_id
            and _alias_target in _AGG_PROVIDERS
        ):
            continue
        # Resolve the canonical provider profile name.  Skip robo_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose robo_id matches the canonical profile name so
        ...
        # same robo_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if robo_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        pconfig = PROVIDER_REGISTRY.get(robo_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (ROBO_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Robo may not route yet.
        ...
        from robo_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(robo_id):
            continue
        ...
        if not has_creds:
            try:
                from robo_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(robo_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        robo_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `robo model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(robo_id)
        if not model_ids:
            model_ids = curated.get(robo_id, [])
            if robo_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(robo_id, model_ids)
    ...
    # --- 2. Check Robo-only providers (openai-codex, copilot, opencode-go) ---
    from robo_cli.providers import ROBO_OVERLAYS
    from robo_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Robo provider ID.
    # ROBO_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Robo IDs ("copilot").
    _mdev_to_robo = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ROBO_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Robo slug — e.g. "github-copilot" → "copilot"
        robo_slug = _mdev_to_robo.get(pid, pid)
        if robo_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or robo_slug.lower() in _excluded:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(robo_slug)
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or robo_slug in providers_store):
                    has_creds = True
            except Exception as exc:
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(robo_slug):
                    has_creds = True
                elif for_picker:
                    ...
                    try:
                        from agent.credential_pool import load_pool
                        _pool = load_pool(robo_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", robo_slug, exc)
        ...
        # configured.
        if not has_creds and robo_slug == "anthropic":
            try:
                ...
                if (robo_creds and robo_creds.get("accessToken")) or \
        ...
        if robo_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(robo_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(robo_slug)
                model_ids = _ids if _ids else (curated.get(robo_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(robo_slug, []) or curated.get(pid, [])
        else:
            ...
            if not model_ids:
                model_ids = curated.get(robo_slug, []) or curated.get(pid, [])
                if robo_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(robo_slug, model_ids)
        total = len(model_ids)
        if robo_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": robo_slug,
            "name": get_label(robo_slug),
            "is_current": robo_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "robo",
        ...
        seen_slugs.add(robo_slug.lower())
        _record_builtin_endpoint(robo_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or ROBO_OVERLAYS (keeps /model in sync
    # with `robo model`).
    try:
        from robo_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Robo writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Robo's own writer uses (mirrors section-4 grouping).
            ...
            # ``robo model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``robo model``. Pin with
            ...
            if should_probe:
                try:
                    from robo_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Robo's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, robo_cli/config.py) already consume that dict.
        ...
        for grp in groups.values():
            ...
            #   ``robo model``. Pin a dict catalog with
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from robo_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`robo_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from robo_cli.models import fetch_openrouter_models

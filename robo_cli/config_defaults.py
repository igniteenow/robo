"""Default configuration data for Robo.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from robo_cli/config.py. Must not import from robo_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["robo-core"],
    # SQLite journal mode used by every Robo database opener. WAL is the
    ...
    "agent": {
        ...
        # the Robo-level retry loop that wraps the whole call.  Lower
        ...
        # Universal "stay on task" guidance — complements task_completion_guidance
        # above with a different failure mode: abandoning the assigned task
        # mid-way for a tangent, or treating a genuinely long task as a reason
        # to wrap it up early instead of continuing (using the todo tool to
        # track subtasks across context compaction / session restarts).
        # Comparable length to task_completion_guidance above (~80 tokens
        # documented there); not separately measured — set False to disable.
        "task_focus_guidance": True,
        # Universal verification guidance — short prompt block that tells the
        # model to verify claims about code/system behavior (read/run/test)
        # before stating them, and to try a concrete alternative before
        # concluding a task can't be done. A third block distinct from the
        # two above — see agent/prompt_builder.py's VERIFICATION_GUIDANCE
        # docstring. Set False to disable.
        "verification_guidance": True,
        # Active claim verification — off by default. Unlike the guidance
        # blocks above (free prompt text), this spends a real auxiliary-LLM
        # call per trigger to double-check risky/definitive claims before
        # the agent acts further on them (agent/claim_verification.py).
        # On by default (Robo's deep-thinking posture): a wrong "fact" that
        # the agent then acts on costs far more than the check. Set False to
        # save the auxiliary calls on cheap/throwaway sessions.
        "claim_verification": True,
        # Deep thinking — mandatory posture, not a mode. When true (default):
        #   * reasoning effort defaults to "high" for every model that
        #     supports thinking (agent.reasoning_effort / per-model
        #     reasoning_overrides still win when set explicitly);
        #   * claim_verification stays on (above);
        #   * the deliberation protocol in SOUL.md is enforced: plan before
        #     acting, verify with tools before asserting, and correct the
        #     draft when observed evidence contradicts it.
        # False restores provider-default effort and skips verification.
        "deep_thinking": True,
        ...
        # prompt's environment-hints block. Lets a host that wraps Robo
        ...
        # identity slot (SOUL.md). Empty by default. The ROBO_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Robo adds a coding operating brief
        ...
        # ``ROBO_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             ROBO_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force ROBO_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Robo auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Robo sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Robo
        ...
        # bundled Robo image, which drops to the `robo` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Robo sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Robo should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``robo chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.robo/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: robo sweeps the checkpoint base at startup
        ...
        # ``robo checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .robo.md, or .cursorrules before Robo applies
    ...
    # Single-query (``robo -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Robo truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Robo' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); robo =
                                      # Robo' compression threshold triggers
    ...
    "auxiliary": {
        ...
        "vision": {
            "provider": "auto",    # auto | openrouter | igniteenow | codex | custom
        ...
        # Invoked by ``robo kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``robo kanban decompose`` and the kanban
        ...
        # ``robo profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `robo model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # interrupt | redirect | queue | steer — see cli-config.yaml.example.
        # interrupt: a message typed mid-turn is echoed immediately and read
        # by the model right away (a running tool batch is cancelled).
        # redirect: same, but a running tool batch is allowed to finish first.
        ...
        # Mirrors `robo -c` muscle memory.  Default off so existing
        # users aren't surprised.  ROBO_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `robo --tui` drops a one-time hint
        ...
        # Ignitee Now credits status-bar notices (usage bands, grant-spent, depleted /
        ...
        # model — see robo_cli/focus_view.py.
        "focus_view": False,
        "focus_saved_tool_progress": "all",
        "skin": "robo",  # Robo's branded skin (see robo_runtime/resources/skins/robo.yaml); "default" is the classic look
        ...
        # can set `path`/`paths` to ROBO_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/robo`. Per-platform overrides go under
        ...
        "pet": {
            ...
            # get_robo_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # (``ROBO_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # igniteenow-account-service at deploy time via the
        # ``ROBO_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``ROBO_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/robo``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Robo appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.robo/cache/piper-voices/
    ...
    "wake_word": {
        ...
        "phrase": "hey roh boh",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_roh_boh" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.robo/plugins/.
    # reverse_engineer tool: backend selection + timeouts. See
    # cli-config.yaml.example for the full description.
    "reverse_engineering": {
        "backend": "auto",           # auto | ghidra | radare2 | rizin | binutils | python
        "ghidra_path": "",           # install dir or analyzeHeadless path (auto-detected when empty)
        "analysis_timeout": 1800,    # first full analysis of a binary (Ghidra / radare2)
        "query_timeout": 600,        # follow-up Ghidra queries
        "radare_analysis": "aaa",    # aa | aaa | aaaa
        "decompiler": "auto",        # auto | pdg | pdc
    },
    "context": {
        "engine": "compressor",
        # Robo's working window: the largest context it will run a model at.
        # A 2M-class model is used in full; anything larger is capped here so
        # tool-output budgets and compaction tails stay sane; smaller models
        # are unaffected. 0 = no ceiling. Pair with model.context_length to
        # force a window on an endpoint that under-reports.
        "working_window": 2_000_000,
    ...
    # configured providers (OpenRouter, Ignitee Now, Z.ai, Kimi, etc.) are supported.
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.robo/cache/delegation/
    ...
    # response. If not, Robo feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Robo auto-pauses the goal and
    ...
    "moa": {
        ...
        # <robo_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.robo/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ROBO_SKILL_DIR} and ${ROBO_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `robo curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `robo curator run --consolidate` overrides
        ...
        # normally restored on every `robo update`, so pruning them only
        ...
        # skipped), snapshot ~/.robo/skills/ into
        # ~/.robo/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `robo curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for robo-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "*Robo*" header.
    ...
    "approvals": {
        "mode": "smart",
        "timeout": 300,
        # Interactive surfaces (TUI / desktop): how long an approval prompt
        # waits for the user. 0 = until answered — the prompt is a real
        # barrier; the agent stays parked (no model calls, no other tools,
        # no delegated children) instead of expiring into a BLOCKED result it
        # then reasons around. A positive value restores a bounded wait.
        # The messaging-platform wait above (``timeout``) is unaffected.
        "interactive_timeout": 0,
        # Interactive surfaces: a deny (or an expired / closed prompt) also
        # stops the turn so Robo never keeps working in the background after
        # the user said no. False returns only the BLOCKED tool result and
        # lets the model decide what to do next.
        "deny_stops_turn": True,
        "cron_mode": "deny",
        ...
        # false.  TUI has its own modal overlay (ROBO_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Robo' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.robo/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ROBO_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `robo doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``robo_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Robo to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $ROBO_HOME/plugins/<name>/) to relocate the trigger.
        ...
        # Also overridable via ROBO_CRON_MAX_PARALLEL env var.
        ...
        # ROBO_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `robo -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `robo` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `robo kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     robo-engineer's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Robo tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.robo/logs/.
    ...
    "gateway": {
        ...
        # internal ROBO_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.robo/sessions/sessions.json entirely.
        ...
        # the ROBO_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.robo.gateway` that defenses 1-2 don't
        ...
        # ``ROBO_GATEWAY_MAX_STARTS`` / ``ROBO_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.robo/.env,
        ...
        # behavior: files must live under the Robo cache, under
        ...
        # user. Bridged to ROBO_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Robo
        # cache (~/.robo/cache/{documents,images,audio,video,screenshots})
        ...
        # to ROBO_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ROBO_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ROBO_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.robo/state.db.
    ...
    "sessions": {
        ...
        # the default of ``robo sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.robo/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `robo sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `robo update` prints a one-line notice with
        ...
        # ~/.robo/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to ROBO_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # ROBO_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
    },
    ...
    "updates": {
        ...
        #     DBs) into <ROBO_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``robo backup``-style zip
        #     of ROBO_HOME into <ROBO_HOME>/backups/, restorable with
        #     ``robo import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `robo update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<ROBO_HOME>/lsp/bin/`` on first use.
    ...
    # `robo tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.robo/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.robo/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.robo/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Robo may use this encrypted
            ...
            # ~/.robo/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.robo/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Robo disables it for our users unless they
        # explicitly opt in here. When false (default), Robo sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Robo also calls
    ...
    #
    # Configure with `robo egress setup`.  Disabled by default — the rest of
    # Robo works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.robo/bin/ on
        ...
        # Together, DeepSeek, Ignitee Now).  Wildcards (`*.foo.com`) are supported.
        "extra_allowed_hosts": [],
    },

    # Robo Desktop (Electron app) launch options. These only affect
    # `robo desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the ROBO_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Robo mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'robo model' for endpoint-specific config)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Robo' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Robo, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @robo:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ROBO_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #robo)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: robo-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'robo-engineer' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Robo API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Robo API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Robo API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ROBO_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to ROBO_TOOL_PROGRESS_MODE for backward
    ...
    # ROBO_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "ROBO_PREFILL_MESSAGES_FILE": {

"""
Doctor command for robo CLI.

Diagnoses issues with Robo setup.
"""
...
from robo_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
ROBO_HOME = get_robo_home()
_DHH = display_robo_home()  # user-facing display path (e.g. ~/.robo or ~/.robo/profiles/coder)

# Load environment variables from ~/.robo/.env so API key checks work
_env_path = get_env_path()
load_robo_dotenv(robo_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from robo_cli.colors import Colors, color
from robo_cli.models import _ROBO_USER_AGENT
from robo_cli.vercel_auth import describe_vercel_auth
from robo_constants import OPENROUTER_MODELS_URL
...
from robo_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all Robo containers"
    elif method in {"nix", "nixos"}:
        action = recommended_update_command_for_method(method)
    else:
        action = "run `robo update`"
    return (
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.robo/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("ROBO_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from robo_cli.auth import get_minimax_oauth_auth_status
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. ROBO_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("ROBO_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("ROBO_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches robo_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``robo --version`` reporting a stale version while
    ...
    """
    try:
        from robo_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != robo_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'robo update', or set "
            "robo_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-robo and dashboard static services are up
    ...
    for static in ("main-robo", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `robo profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `robo doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from robo_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the ROBO_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("ROBO_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via ROBO_MANAGED_DIR={managed_dir}")


def _doctor_voice_stack(issues: list[str]) -> None:
    """Everything "Hey Roh Boh" needs, each with the command that fixes it.

    The hands-free stack is an optional extra, so a core-only install has a
    wake word that arms and never fires, a transcriber that is not there, and
    no voice. This section makes that visible on the first ``robo doctor``.
    """
    import importlib
    import importlib.util
    from pathlib import Path

    _section("Voice & Wake Word (hands-free)")
    parts = [
        ("openwakeword", "openWakeWord (hears the wake phrase)", "wake"),
        ("onnxruntime", "ONNX Runtime (runs the wake model)", "wake"),
        ("sounddevice", "sounddevice / PortAudio (microphone)", "wake"),
        ("faster_whisper", "faster-whisper (transcribes what you say)", "voice"),
        ("edge_tts", "edge-tts (speaks the reply)", "edge-tts"),
    ]
    missing_extras: set[str] = set()
    for module, name, extra in parts:
        if importlib.util.find_spec(module) is not None:
            check_ok(name)
        else:
            missing_extras.add(extra)
            check_warn(name, "(not installed)")
    if missing_extras:
        extras = ",".join(sorted(missing_extras))
        issues.append(
            f"Hands-free voice is incomplete. Install it: {_python_install_cmd()} -e '.[{extras}]'  "
            "(from the Robo source folder), then run `robo doctor` again."
        )

    try:
        from robo_constants import get_robo_home

        model_dir = get_robo_home() / "assets" / "wake_word"
        models = [p for p in model_dir.glob("hey_roh_boh.*") if p.suffix in (".onnx", ".tflite")] if model_dir.is_dir() else []
        if models:
            check_ok("Wake model 'hey roh boh'", f"({len(models)} file(s) in {model_dir})")
        else:
            _fail_and_issue("Wake model 'hey roh boh'", "(missing)", "Run `robo setup` once; it installs the bundled model into your Robo home.", issues)
    except Exception as exc:  # doctor must never crash on a probe
        check_warn("Wake model", f"(could not check: {exc})")

    if importlib.util.find_spec("sounddevice") is not None:
        try:
            sounddevice = importlib.import_module("sounddevice")
            inputs = [d for d in sounddevice.query_devices() if int(d.get("max_input_channels", 0)) > 0]
            if inputs:
                check_ok("Microphone", f"({len(inputs)} input device(s) visible)")
            else:
                check_warn("Microphone", "(no input device visible; plug one in or grant microphone access)")
        except Exception as exc:
            hint = " Install libportaudio2 (Linux)" if "PortAudio" in str(exc) else ""
            check_warn("Microphone", f"(PortAudio could not open: {str(exc)[:70]}.{hint})")

    try:
        from robo_cli.config import load_config_readonly

        cfg = load_config_readonly() or {}
        if (cfg.get("wake_word") or {}).get("enabled"):
            check_ok("Wake word", "(enabled)")
        else:
            check_info("Wake word is off (privacy default). Turn it on with /wake on, or wake_word.enabled: true in config.yaml.")
    except Exception:
        pass
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `robo`.
    os.environ.setdefault("ROBO_INTERACTIVE", "1")

    # Handle `robo doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.robo/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 Robo Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`robo doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        import sqlite3
        from robo_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Robo already refuses to enable WAL on fresh DBs.
    ...
    # Detect drift between pyproject.toml and robo_cli/__init__.py versions
    ...
    _doctor_voice_stack(issues)
    
    _section("Configuration Files")
    # Managed scope (administrator-pinned config/env), when present.
    managed_scope_check()
    # Check ~/.robo/.env (primary location for user config)
    env_path = ROBO_HOME / '.env'
    if env_path.exists():
        ...
        # matches robo_cli.env_loader._load_dotenv_with_fallback.
        try:
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'robo setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'robo setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'robo setup' to create one")
                issues.append("Run 'robo setup' to create .env")
    
    # Check ~/.robo/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = ROBO_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from robo_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if isinstance(user_providers, dict):
                from robo_cli.config import is_provider_enabled
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'robo config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from robo_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.robo/.env or run 'robo setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'robo setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'robo config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from robo_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from robo_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'robo setup' to migrate config")
                else:
                    issues.append("Run 'robo doctor --fix' or 'robo setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'robo doctor --fix'")
        except Exception:
            pass

        # Detect stale ROBO_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives ROBO_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            from robo_cli.config import load_env, read_user_config_raw, remove_env_value
            ...
            env_ghost = load_env().get("ROBO_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"ROBO_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `robo setup` run)",
                )
                if should_fix:
                    if remove_env_value("ROBO_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale ROBO_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove ROBO_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the ROBO_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale ROBO_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'robo doctor --fix'"
                    )
        except Exception:
            pass
        ...
        try:
            from robo_cli.config import load_env as _load_env_depr
            from robo_cli.config import read_user_config_raw as _read_raw_depr
        ...
        try:
            from robo_cli.config import validate_config_structure
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Robo' own device-code flow — the Codex CLI is
    ...
    if robo_home.exists():
        ...
        robo_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = robo_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Robo a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Robo Persona\n\n"
                "<!-- Edit this file to customize how Robo communicates. -->\n\n"
                "You are Robo, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = robo_home / "memories"
    ...
    state_db_path = robo_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from robo_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'robo doctor --fix' "
                        "(or 'robo sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from robo_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'robo doctor --fix' "
                        "(or 'robo sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = robo_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'robo doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "robo"
        ...
        _cmd_link = _cmd_link_dir / "robo"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(robo not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/robo → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/robo points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/robo → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/robo — run 'robo doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/robo exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/robo not found",
                    "(robo command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/robo → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/robo symlink — run 'robo doctor --fix'")
    ...
    try:
        from robo_constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "Install the Vercel optional dependency: pip install 'robo-engineer[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        # `robo acp --setup-browser` installs agent-browser into the
        # Robo-managed node prefix, which isn't necessarily on PATH. Mirror
        # dep_ensure._has_robo_agent_browser() so doctor and dep_ensure agree
        ...
        _managed_ab = (
            _which_in(ROBO_HOME / "node" / "bin")
            or _which_in(ROBO_HOME / "node")
        )
        _legacy_ab = _which_in(ROBO_HOME / "node_modules" / ".bin")
        if agent_browser_path.exists():
            ...
            # `robo update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `robo doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable ROBO_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Robo one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'robo config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``robo doctor``.
    ...
    # never the legitimate source for `robo doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'robo setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = ROBO_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: robo skills list)")
    ...
    try:
        from robo_cli.config import read_user_config_raw as _read_raw_mem
        _mem_cfg_path = ROBO_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: robo memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: robo memory setup",
                    "No Honcho API key — run 'robo memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run robo memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: robo memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: robo memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from robo_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "robo -p" in content:
                            _m = _re.search(r"robo -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'robo doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))

...
Usage:
    python -m robo_cli.main web          # Start on http://127.0.0.1:9119
    python -m robo_cli.main web --port 8080
"""
...
try:
    ...
    # running `robo dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["ROBO_WEB_DIST"]) if "ROBO_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``robo gateway run`` — but the
    desktop app spawns a ``robo dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same ROBO_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    ...
    ``robo_cli.gateway``. But the first WS connection and its initial
    ...
    for mod in (
        ...
        "robo_cli.auth",
        "robo_cli.copilot_auth",
        "robo_cli.runtime_provider",
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from robo_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Import robo_cli.gateway eagerly *before* the lifespan yield so the
    ...
    # Desktop-spawned backends (ROBO_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `robo
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("ROBO_DESKTOP") == "1":
...
app = FastAPI(title="Robo", version=__version__, lifespan=_lifespan)

# Memory-provider OAuth connect routes live in the memory layer, not here.
from robo_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# ROBO_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
def _resolve_session_token() -> str:
    return os.environ.get("ROBO_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)


_SESSION_TOKEN = _resolve_session_token()
_SESSION_HEADER_NAME = "X-Robo-Session-Token"
...
#
# This list is defined in ``robo_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from robo_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Robo-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
async def _dashboard_auth_gate(request: Request, call_next):
    from robo_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from robo_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `robo egress setup` "
            "and `robo egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
    ...
    "display.skin": {
        ...
        "options": ["robo", "ember", "midnight", "paper", "contrast"],
    },
    "dashboard.theme": {
        "type": "select",
    ...
    "display.busy_input_mode": {
        ...
        "options": ["interrupt", "redirect", "queue", "steer"],
    },
    "approvals.mode": {
        "type": "select",
    ...
    "approvals.deny_stops_turn": {
        "type": "boolean",
        "description": "TUI/desktop: denying an approval (or an unanswered/closed prompt) also stops the turn",
    },
    "approvals.interactive_timeout": {
        "type": "number",
        "description": "TUI/desktop: seconds an approval prompt waits (0 = until answered)",
    },
    "context.working_window": {
        "type": "number",
        "description": "Largest context window Robo runs a model at (default 2,000,000; 0 = no ceiling)",
    },
    "context.engine": {
        "type": "select",
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Robo (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "boolean",
        "description": (
            "Refresh an already-installed cua-driver during robo update. "
...
from robo_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Robo provider slug — fine.
    ...
    1. Vendor-name → Robo-provider mapping: when the provider string is
       not a known Robo provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from robo_cli.config import get_compatible_custom_providers
    from robo_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from robo_cli.model_normalize import normalize_model_for_provider
    from robo_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    # not be treated as a stray vendor prefix -- it isn't a known Robo
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from robo_cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    Robo process is using it.
    """
    from robo_state import _default_db_path
...
_MANAGED_FILES_ROOT_ENV = "ROBO_DASHBOARD_FILES_ROOT"
...
# ROBO_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to ROBO_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Robo credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no ROBO_HOME resolution, so it blocks these trees wherever
...
def _default_robo_root_is_opt_data() -> bool:
    raw = os.environ.get("ROBO_HOME", "").strip()
    if not raw:
        return False
    try:
        from robo_constants import get_default_robo_root

        root = get_default_robo_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``robo update``.
    ...
    a container — e.g. the robo-webui image sharing the Robo source tree),
    the dashboard's ``robo update`` button is the correct update path and
    ...
    """
    if _default_robo_root_is_opt_data():
        return True
    try:
        from robo_constants import is_container
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Robo root is actually /opt/data
    # (the container/hosted layout) or when ROBO_DASHBOARD_FILES_ROOT is set.
    if _default_robo_root_is_opt_data():
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Robo inside an xterm.js PTY. Browser
...
def _index_upload_in_background(target: Path) -> None:
    """Add a file uploaded through the dashboard to the knowledge base on a
    daemon thread. Images are skipped; any failure is logged, never raised:
    indexing is a convenience layered on the upload, not part of it."""
    import threading

    def work() -> None:
        try:
            from robo_cli.knowledge_store import IMAGE_SUFFIXES, KnowledgeError, KnowledgeStore
...
async def upload_managed_file(payload: ManagedFileUpload, request: Request):
    ...
    _index_upload_in_background(Path(target))
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``robo:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.robo-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``robo_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from robo_cli import web_git as _web_git  # noqa: E402
...
from robo_cli.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)
from robo_cli.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
def _profile_platform_ports(profile_home: Path, runtime: Optional[dict]) -> Dict[str, int]:
    ...
    try:
        ...
        from robo_cli.config import read_user_config_raw
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from robo_cli.profiles import _check_gateway_running, profiles_to_serve
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_robo_home() at call time
    ...
    try:
        ...
        # ROBO_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        # Windows install the first import of robo_cli.gateway blocks the
        ...
        # and which providers are registered so ``robo status`` and the
        # SPA's StatusPage can show "OAuth gate ON — <n> provider(s)" or
        ...
        auth_flows: list[str] = []
        try:
            from robo_cli.dashboard_auth import (
        ...
        status = {
            ...
            "can_update_robo": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
        ...
        try:
            ...
            storage_check = await asyncio.get_running_loop().run_in_executor(
                None, functools.partial(_probe_state_db, get_robo_home())
            )
        ...
        try:
            from robo_state import SessionDB as _SDB
            from robo_constants import get_robo_home as _ghh
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Robo Cloud
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the robo home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_robo_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_robo_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
# ---------------------------------------------------------------------------
# ---------------------------------------------------------------------------
# Portal endpoint — deprecated. Kept only so existing dashboard frontends
# calling GET /api/portal degrade to an empty/"not configured" shape instead
# of a 404. The Portal-specific auth + subscription-feature backend this
# endpoint used to surface has been removed.
# ---------------------------------------------------------------------------
...
async def run_prompt_size():
    try:
        proc = _spawn_robo_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_robo_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_robo_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from robo_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.robo/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_robo_home() / "logs"
...
def _spawn_robo_action(
    subcommand: List[str],
    name: str,
    *,
    env_overrides: Optional[Dict[str, str]] = None,
) -> subprocess.Popen:
    """Spawn ``robo <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``robo_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "robo_cli.main", *subcommand]

    # The dashboard runs *inside* the gateway process, so os.environ carries
    # _ROBO_GATEWAY=1. Inheriting it makes a spawned `robo gateway restart`
    ...
    action_env = {**os.environ, "ROBO_NONINTERACTIVE": "1"}
    action_env.pop("_ROBO_GATEWAY", None)
    ...
    action_id = (env_overrides or {}).get("ROBO_ACTION_ID")
    if action_id:
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["robo", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``robo gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``robo gateway restart`` children race each other on the
    ...
    return _spawn_robo_action(subcommand, "gateway-restart"), False
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``robo gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``ROBO_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
@app.post("/api/robo/update")
async def update_robo():
    """Kick off ``robo update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Robo updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("robo-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "robo-update",
    ...
    existing = _ACTION_PROCS.get("robo-update")
    if existing is not None and existing.poll() is None:
        response = {
            "ok": True,
            "pid": existing.pid,
        ...
        action_id = _ACTION_IDS.get("robo-update")
        if action_id:
    ...
    try:
        proc = _spawn_robo_action(
            ["update"],
            "robo-update",
            env_overrides={"ROBO_ACTION_ID": action_id},
        )
    except Exception as exc:
        _log.exception("Failed to spawn robo update")
...
@app.get("/api/robo/update/check")
async def check_robo_update(force: bool = False):
    """Report whether a Robo update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/robo/update`` actually runs ``robo update``.

    Returns:
        install_method: 'git' | 'docker' | 'nix' | 'nixos' | 'unknown'
        current_version: installed Robo version string
    ...
    try:
        ...
        if force:
            try:
                (get_robo_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest, profile: Optional[str] = None):
    ...
    configured in ``~/.robo/config.yaml`` under ``tts.``.
    """
...
from robo_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from robo_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from robo_cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from robo_cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Robo supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _flat_json_path(provider: ProviderConfigSchema) -> Path:
    return get_robo_home() / provider.name / "config.json"
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # under /opt/robo is sealed read-only, and installs must be redirected
    # to the writable durable target on the data volume
    # (ROBO_LAZY_INSTALL_TARGET, e.g. /opt/data/lazy-packages) — the same
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    robo_home = get_robo_home()
    values: Dict[str, Any] = {}

    # Common native provider stores.
    for path in (
        robo_home / f"{name}.json",
        robo_home / name / "config.json",
    ):
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_robo_home()))
            return
...
async def get_egress_status():
    ...
    from robo_cli.proxy_cli import format_status_text
...
# in robo_cli/config.py — listed here for deterministic ordering in the UI.
...
async def get_model_options(
    ...
    """
    try:
        from robo_cli.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `robo model` does so GUI onboarding lands on a
    sensible default instead of blindly taking the first curated entry. Falls
    back to the first curated model when no preferred default is set.

    Response: {"provider": str, "model": str, "free_tier": bool | None}
    `model` may be empty if nothing could be resolved (caller degrades
    gracefully). `free_tier` is always None now.
    """
    slug = (provider or "").strip().lower()

    # Preferred silent default when the provider's curated list carries it,
    # else the first curated model. Aggregator lists lead with the
    # priciest Anthropic flagship (claude-fable-5), which must never be the
    # model a user lands on without explicitly picking it.
    try:
        from robo_cli.inventory import build_models_payload, load_picker_context
        from robo_cli.models import pick_silent_default_model
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from robo_cli.moa_config import normalize_moa_config
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from robo_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.robo/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from robo_cli.model_cost_guard import expensive_model_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # endpoint, mirroring the ``robo model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from robo_cli.main import _auto_provider_name, _save_custom_provider
        ...
        # a now-unpaid provider (e.g. hit a billing cap) keeps paying 402s
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``robo model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from robo_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from robo_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `robo model` provider — would be
        ...
        # Vertex — a `robo model` provider — would be invisible in the desktop
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `robo model` picker uses (not desktop-only prefix guesses).
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    try:
        with _profile_scope(body.profile or profile):
            ...
            from robo_cli.credential_lifecycle import save_provider_env_credential
...
async def remove_env_var(body: EnvVarDelete, profile: Optional[str] = None):
    try:
        with _profile_scope(body.profile or profile):
            ...
            from robo_cli.credential_lifecycle import remove_provider_env_credential
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in robo gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in robo gateway setup",
...
# robo_cli/setup_hidden_env.py. Shared with the `robo setup gateway`
# wizard so the surfaces ask for the same things.
from robo_cli.setup_hidden_env import (  # noqa: E402
...
def _messaging_platform_payload(
    ...
    # ROBO_HOME contextvar override (#56986 / #69143), so the profile's
...
def _whatsapp_session_path() -> Path:
    from robo_constants import get_robo_dir

    return get_robo_dir("platforms/whatsapp/session", "whatsapp/session")
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.robo.igniteenow.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"RoboDashboard/{__version__}"
@dataclass
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Robo").strip() or "Robo"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Robo appears
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized ROBO_HOME)
...
# the canonical ``robo auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.robo/.anthropic_oauth.json`` — Robo-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_robo_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_robo_oauth_credentials,
            _get_robo_oauth_file,
        )
    except ImportError:
        read_robo_oauth_credentials = None  # type: ignore
        _get_robo_oauth_file = None  # type: ignore

    robo_creds = None
    if read_robo_oauth_credentials:
        try:
            robo_creds = read_robo_oauth_credentials()
        except Exception:
            robo_creds = None
    if robo_creds and robo_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "robo_pkce",
            "source_label": f"Robo PKCE ({_get_robo_oauth_file() if _get_robo_oauth_file else None})",
            "token_preview": _truncate_token(robo_creds.get("accessToken")),
            "expires_at": robo_creds.get("expiresAt"),
            "has_refresh_token": bool(robo_creds.get("refreshToken")),
        }
    ...
    try:
        ...
        pass
    try:
        from robo_cli.config import get_env_value
    except ImportError:
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Robo even
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Robo never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "device_code",
        "cli_command": "robo auth add openai-codex",
    ...
    },
    {
        ...
        # as any device-code flow; the PKCE bit is a security
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "robo auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from robo_cli import auth as hauth
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Robo, so the disconnect
    ...
    user sees exactly what executes, and Robo then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Robo — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``robo model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `robo model` universe (zero-edit for new plugins).
    try:
        from robo_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"robo auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("robo_pkce", "claude_code", ...)
    ...
    sync with the `robo model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Robo-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _get_robo_oauth_file
                oauth_file = _get_robo_oauth_file()
            ...
            try:
                from robo_cli.auth import clear_provider_auth
...
#          → persists to ~/.robo/.anthropic_oauth.json AND credential pool
#          → returns { ok: true, status: "approved" }
#
#   Device code (OpenAI Codex, MiniMax, xAI):
#     1. POST /api/providers/oauth/{openai-codex|minimax-oauth|xai}/start
...
# Guarded so robo web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Robo file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``robo auth add anthropic``.
    """
    from agent.anthropic_adapter import _get_robo_oauth_file
    oauth_file = _get_robo_oauth_file()
    payload = {
...
async def _start_device_code_flow(
    ...
    if provider_id == "minimax-oauth":
        ...
        # operator's perspective it's identical to any other device-code
    ...
    if provider_id == "xai-oauth":
        from robo_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _minimax_poller(session_id: str) -> None:
    ...
    Uses a PKCE-style ``code_verifier`` + ``user_code`` rather than a
    device_code field. On success, builds the same auth_state dict that
    ``_minimax_oauth_login`` (the CLI flow) builds and persists via
    ``_minimax_save_auth_state`` — so the dashboard path leaves the
    system in the same state as ``robo auth add minimax-oauth``.
    """
    from robo_cli.auth import (
        _minimax_poll_token,
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # Mirror `robo auth add xai-oauth`: first credential may become
            ...
            # prior ``robo auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``robo model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Robo "
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from robo_cli.auth import _save_codex_tokens
...
def _session_db_read_probe_statements() -> tuple:
    ...
    after `robo update` until the first message forced a writable open.
    """
    from robo_state_schema import schema_read_probe_statements
...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    from robo_state import SessionDB, is_malformed_db_error
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``robo serve`` — it runs neither the
    ...
    try:
        ...
        from robo_cli.config import load_config as _load_full_config
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `robo sessions prune`)."""
    has_window = (
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_robo_home()
...
async def get_logs(
    ...
    from robo_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_robo_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from robo_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from robo_cli import profiles as profiles_mod
    try:
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (ROBO_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.robo``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    ROBO_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, ROBO_HOME)."""
    from robo_cli import profiles as profiles_mod
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from robo_constants import (
        reset_robo_home_override,
        set_robo_home_override,
    )

    token = set_robo_home_override(str(home))
    try:
...
from robo_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from robo_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _fire_cron_job_for_profile(profile: str, job_id: str) -> bool:
    ...
    Scope both cron storage and the runtime Robo home so the job's store,
...
#
# Wraps the same config data layer the CLI uses (robo_cli.mcp_config), so
# servers managed here show up under `robo mcp list` and vice versa.  Secrets
...
def _normalize_mcp_server_create(
    ...
    """
    from robo_cli.mcp_config import (
...
from robo_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from robo_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from robo_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.robo_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        from robo_constants import reset_robo_home_override, set_robo_home_override
        from tools.mcp_dashboard_oauth import dashboard_oauth_flow
        from tools.mcp_oauth import RoboTokenStorage, force_interactive_oauth
        from tools.mcp_oauth_manager import get_manager

        home_token = set_robo_home_override(flow.robo_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.robo_home)))
        try:
            transaction = _mcp_oauth_transaction(flow)
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                manager = get_manager()
                storage = RoboTokenStorage(flow.server_name)
                backup = storage.snapshot()
                previous_entry = None
                try:
                    previous_entry = manager.remove(
                        flow.server_name,
                        robo_home=flow.robo_home,
                    )
...
#
# Wraps the same JSON store the CLI uses (robo_cli.webhook); the webhook
...
# spawn the real `robo gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_robo_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_robo_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from robo_cli.auth import read_credential_pool
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `robo auth remove`) can seed again.
        # Mirrors the `robo auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``robo auth remove`` uses: each source cleans up its
    ...
    from robo_cli.auth import suppress_credential_source
...
async def run_doctor():
    try:
        proc = _spawn_robo_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_robo_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _dashboard_backup_dir() -> Path:
    return get_robo_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"robo-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_robo_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_robo_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from robo_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from robo_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <robo_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_robo_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_robo_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``robo`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_robo_action`` tracks one process/log per name, so a shared
...
from robo_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from robo_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
# Human-readable labels for each hub source id (matches `robo skills search`
# provenance).  Keep in sync with create_source_router()'s source list.
_SKILL_HUB_SOURCE_LABELS = {
    "official": "Official (Robo)",
    "robo-index": "Robo Index",
    "skills-sh": "skills.sh",
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_robo_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "robo setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local ROBO_HOME override so the write lands in the target
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local ROBO_HOME override (same mechanism as
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the ROBO_HOME override. Returns the
    number of skills newly disabled.
    """
    from robo_constants import set_robo_home_override, reset_robo_home_override
    from robo_cli.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_robo_home()`` at call
       time — the context-local override from ``set_robo_home_override``
    ...
    retargeted to the *current* ``get_robo_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a ROBO_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_robo_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_robo_home()`` at call time
...
from robo_cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from robo_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
def _resolve_toolset_model_plugin(ts_key: str, provider_row: dict) -> Optional[str]:
    ...
    a row without one may instead carry the legacy ``imagegen_backend: "fal"``
    marker (same underlying FAL catalog).
    """
    if ts_key == "image_gen":
...
def _toolset_model_catalog(ts_key: str, plugin_name: str):
    ...
    from robo_cli.tools_config import (
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Robo,
...
#
# The endpoint spawns the same ``robo --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from robo_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from robo_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from robo_cli.dashboard_auth.audit import AuditEvent, audit_log
        from robo_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``robo --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``ROBO_TUI_RESUME`` env var —
    matching what ``robo_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``ROBO_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``ROBO_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``ROBO_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``ROBO_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_robo_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``robo -p <name>`` performs. The in-process
    ``ROBO_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from robo_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    if resume:
        ...
        env["ROBO_TUI_RESUME"] = resume

    if sidecar_url:
        env["ROBO_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile ROBO_HOME set above.
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``ROBO_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("ROBO_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from robo_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="robo-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe Robo Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Robo CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="robo-console",
                )
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from robo_cli.console_engine import RoboConsoleEngine

        engine = RoboConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT)
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Robo Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Robo inside WSL2 to use the dashboard's /chat "
    ...
    if raw_resume and env:
        registry_resume = env.get("ROBO_TUI_RESUME") or raw_resume
...
# ROBO_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`robo_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from robo_cli.dashboard_auth.prefix import normalise_prefix
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Robo Teal canvas before the SPA flips the configured user
    theme into place.
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/robo/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /robo`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__ROBO_BASE_PATH__`` honour that prefix
    without rebuilding the bundle.
    """
    # `robo serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("ROBO_SERVE_HEADLESS") == "1"
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (robo serve): web UI disabled — use "
            "`robo dashboard` for the browser UI."
            if _headless
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/robo``)
        ...
        ``__ROBO_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__ROBO_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ROBO_BASE_PATH__="{prefix}";'
                f"window.__ROBO_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__ROBO_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__ROBO_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ROBO_BASE_PATH__="{prefix}";'
                f"window.__ROBO_AUTH_REQUIRED__={gated_js};"
                f"</script>"
        ...
        # (``ROBO_HOME/dashboard-themes/<name>.yaml``), inject a minimal
        # critical-CSS block so the first paint uses the target palette.
        # Without this the SPA paints the default Robo Teal canvas, then
    ...
    # under ``/robo`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Robo backend. Intercept CSS asset
    ...
    async def serve_css(filename: str, request: Request):
        ...
        if prefix:
            for asset_dir in ("/fonts/", "/brand-fonts/", "/fonts-terminal/", "/ds-assets/", "/assets/"):
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Robo Teal",         "description": "Classic dark teal — the canonical Robo look"},
    {"name": "default-large", "label": "Robo Teal (Large)", "description": "Robo Teal with bigger fonts and roomier spacing"},
    {"name": "igniteenow-blue",     "label": "Sky Blue",           "description": "Light mode — vivid blue accents on cream canvas"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.robo/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.robo/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.

    Uses the dashboard process launch home, not ``get_robo_home()``, so a
    transient profile override from embedded chat does not hide themes that
    live under the server's own ``ROBO_HOME``.
    """
    themes_dir = get_process_robo_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.robo/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as robo_cli.plugins):
    1. User plugins:    ~/.robo/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.robo/plugins/  (only if ROBO_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    # ROBO_HOME override (e.g. embedded /chat under --open-profile).
    search_dirs = [
        (get_process_robo_home() / "plugins", "user"),
    ...
    # ``robo_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("ROBO_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".robo" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    try:
        from robo_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
        enabled_set = _get_enabled_set()
        disabled_set = _get_disabled_set()
    except Exception:
        enabled_set = set()
...
def _merged_plugins_hub(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_robo_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from robo_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from robo_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from robo_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.robo/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"robo_dashboard_plugin_{plugin['name']}"
...
from robo_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``ROBO_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("ROBO_DESKTOP_READY_FILE")
    if not target:
        return
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``ROBO_SERVE_HEADLESS``), so
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing ROBO_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Robo exposes a
            ...
            # (#54489).
            try:
                from robo_cli.config import load_config as _load_cfg
                from robo_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "`robo plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
    ...
    # ROBO_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            ready_token = "ROBO_BACKEND_READY" if headless else "ROBO_DASHBOARD_READY"

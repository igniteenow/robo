...
# commands at execution time when ``_ROBO_GATEWAY=1``.
...
def _warn_if_gateway_not_running() -> None:
    ...
    An external provider fires jobs via its own webhook mechanism,
    ...
    print(color("     Start it with: robo gateway install", Colors.DIM))
    print(color("                    sudo robo gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  robo cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'robo cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_status():
    ...
    if provider != "builtin":
        # An external provider does NOT run the in-process 60s
        # ticker — it arms one external one-shot per job and is fired by its
        # own webhook mechanism, so between fires there is intentionally NO
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: robo gateway restart")
            ...
            if last_error:
                ...
                if "Permission denied" in last_error:
                    print(color(
                        ...
                        "(e.g. rewritten by a root `docker exec robo "
                        "robo cron ...`). Fix ownership to match the "
        ...
        print("    robo gateway install    # Install as a user service")
        print("    sudo robo gateway install --system  # Linux servers: boot-time system service")
        print("    robo gateway            # Or run in foreground")
...
def cron_command(args):
    ...
    print("Usage: robo cron [list|create|edit|pause|resume|run|remove|status|runs|tick]")
    sys.exit(1)

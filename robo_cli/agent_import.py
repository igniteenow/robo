"""robo import-agent — import Claude Code / Codex CLI setups into Robo.
...
Usage:
    ...
    robo import-agent claude-code --dry-run # preview only, no changes
    robo import-agent codex --source /path/to/.codex

Follows Robo's migration pattern:
...
self-contained, so this command has no dependency on any optional skill.

Mappings
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in ROBO_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → ROBO_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in ROBO_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in ROBO_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → ROBO_HOME/skills/codex-imports/<name>/
...
deliberately via ``robo setup`` or config.yaml.
"""
...
# Same entry delimiter as the Robo memory store — memories/MEMORY.md entries are separated by bare "§" lines.
ENTRY_DELIMITER = "\n§\n"
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `robo config edit` (or move it aside), then "
            f"re-run the import."
        ) from exc
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`robo config edit` (or move it aside), then re-run the import."
        )
    return data
...
def extract_markdown_entries(text: str) -> List[str]:
    ...
    Code blocks and tables are skipped.  Port of the reference migration
    script's extractor.
    """
    entries: List[str] = []
    headings: List[str] = []
...
def backup_memory_file(path: Path) -> Optional[Path]:
    ...
    Restores parity with the reference migration script this module was ported
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → Robo command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a Robo glob.
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct Robo equivalent — "
        ...
        for name, srv in servers.items():
            ...
            if name in existing and not self.overwrite:
                self.record(kind, name, f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Robo config")
                continue

            robo_srv: Dict[str, Any] = {}
            if srv.get("command"):
                robo_srv["command"] = srv["command"]
                if srv.get("args"):
                    robo_srv["args"] = srv["args"]
                env_kept, env_stripped = sanitize_mcp_env(srv.get("env"))
                if env_kept:
                    robo_srv["env"] = env_kept
                if env_stripped:
                    self.stripped_secrets.extend(
                        f"mcp_servers.{name}.env.{k}" for k in env_stripped
                    )
                if srv.get("cwd"):
                    robo_srv["cwd"] = srv["cwd"]
            if srv.get("url"):
                robo_srv["url"] = srv["url"]
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → ROBO_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``robo import-agent`` (invoked from robo_cli.main)."""
    from robo_cli.config import get_config_path, load_config, save_config
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: robo import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: robo import-agent claude-code   or   robo import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          Robo — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))

    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info("Specify a custom path: robo import-agent "
    ...
    print_info(f"Target:      {robo_home}")
    print_info(f"Overwrite:   {'yes' if overwrite else 'no (skip conflicts)'}")
    print_info("Secrets:     never imported — run 'robo setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: robo import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'robo setup' "
               "to configure providers, or add them to ~/.robo/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (one line per item)."""
    from robo_cli.setup import Colors, color, print_header, print_info
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'robo setup' or ~/.robo/.env.")
        print()

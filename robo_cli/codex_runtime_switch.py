...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Robo'
...
The actual runtime resolution happens in robo_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "robo"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    # the world state (managed block in ~/.codex/config.toml, robo-tools
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Robo' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Robo for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from robo_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the robo-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "robo-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Robo tool callback are always-on production
            ...
            if "robo-tools" in mig_report.migrated:
                msg_lines.append(
                    "Robo tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Robo runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Robo tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Robo runtime.")

"""Profile distributions — shareable, packaged Robo profiles via git.

A distribution is a Robo profile published as a git repository (or
...
* ``robo profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``robo skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``robo profile``, not a parallel tree):

    robo profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "robo-engineer", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            robo_requires=str(data.get("robo_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.robo_requires:
            out["robo_requires"] = self.robo_requires
        if self.author:
...
def check_robo_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Robo {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``robo profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Robo profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Robo distribution."
        )

    # Version check up-front so we fail fast
    check_robo_requires(manifest.robo_requires, robo_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.robo).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="robo_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `robo profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`robo profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="robo_dist_update_") as tmp:
        plan = plan_install(

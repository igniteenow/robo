"""Dashboard process-hygiene helpers — extracted from ``robo_cli/main.py``.
...
``_detect_concurrent_robo_instances``) are lifted verbatim. References to
helpers that STAY in ``robo_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``robo_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``robo_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``robo_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``robo dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``robo update`` replaces files on disk, the running
    ...
    ``robo-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Robo Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``robo serve`` as
    ...
    environment variable ``ROBO_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`robo serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "robo.*dashboard"`) keeps us consistent
            # with `robo_cli.gateway._scan_gateway_pids` and avoids the
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``robo dashboard`` / ``robo serve`` processes.

    Called at the end of ``robo update`` (default ``reason``) and also
    from ``robo dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``robo update`` path), a detected
    ``robo-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``robo-serve.service``) has its owning unit
    ...
    # When the Robo Desktop Electron app spawns this dashboard as a
    # backend child, it sets ROBO_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("ROBO_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `robo update` path) — `--stop` must
    ...
    #    remote backend (robo serve) under Restart=on-failure never comes
...
def _detect_concurrent_robo_instances(
    ...
    The Robo Desktop Electron app spawns ``robo.EXE`` as a backend child,
    so during ``robo update`` the user-invoked process and the desktop's
    ...
    shims (``robo.exe`` / ``robo-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``robo update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``robo.exe``) is a separate process from the Python
    ...
    for shim in _m()._robo_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated robo.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``robo update``
    ...
    #      robo.exe sitting *under* a non-Robo parent (e.g. a Robo

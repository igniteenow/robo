"""Bounded product contract for the first Robo shared-metrics slice."""
...
SCHEMA_KEY = "robo.metrics.schema_version"
SCHEMA_VERSION = "robo.metrics.event.v2"
MODEL_CALL_SCOPE = "robo.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "robo.task_run"
TOOL_CALL_SCOPE = "robo.tool_call"
CLIENT_ACTIVE_MARK = "robo.client.active"
TOOL_APPROVAL_MARK = "robo.tool_approval"
SKILL_LIFECYCLE_MARK = "robo.skill.lifecycle"
SKILL_LOAD_MARK = "robo.skill.load"
SUBSCRIBER_NAME = "robo.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "robo.client.active"
LEGACY_MODEL_CALL_METRIC = "robo.model_call.count"
MODEL_ROUTE_METRIC = "robo.model_route.count"
TASK_STARTED_METRIC = "robo.task_run.started"
TASK_FINISHED_METRIC = "robo.task_run.finished"
TOOL_CALL_METRIC = "robo.tool_call.count"
TOOL_APPROVAL_METRIC = "robo.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "robo.skill.lifecycle.count"
SKILL_LOAD_METRIC = "robo.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Robo installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(robo_version or "").strip()
    ...
    return {
        ...
        "robo_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("robo_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Robo logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("robo.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from robo_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Robo terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Robo registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "robo-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Robo tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Robo."""

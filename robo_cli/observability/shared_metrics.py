"""Durable aggregation and local export for Robo shared metrics."""
...
from robo_cli.sqlite_util import write_txn
...
_PACKAGE_SCHEMA_VERSION = "robo.shared_metrics.v2"
...
class SharedMetricsStore:
    ...
    def __init__(
        ...
        root = get_robo_home() / "telemetry" / "shared_metrics"
    ...
    def _ensure_schema(self) -> None:
        with self._connection(busy_timeout_ms=_SCHEMA_BUSY_TIMEOUT_MS) as connection:
            # Serialize first-run creation and upgrades across Robo processes.
            with write_txn(connection):
    ...
    def _create_package_in_transaction(
        ...
        resource = {
            "robo_version": period_row["robo_version"],

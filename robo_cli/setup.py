"""
Interactive setup wizard for Robo.
...
# display_robo_home imported lazily at call sites (stale-module safety during robo update)

from robo_cli.colors import Colors, color


def print_header(title: str):
    """Print a section header (Robo design language)."""
    from robo_cli import design as ui
    ui.section(title)


from robo_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    from robo_cli import design as ui
    ui.title("Setup", "non-interactive environment")
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Robo using environment variables or config commands:")
    print_info("  robo config set model.provider custom")
    print_info("  robo config set model.base_url http://localhost:8080/v1")
    print_info("  robo config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'robo setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    print()
    print("  " + ui.paint(question, "strong", bold=True))
    for row in ui.choice_rows(choices, default):
        print(row)
    ui.hint(f"Enter keeps the default ({default + 1}) · Ctrl+C exits")

    while True:
        try:
            value = input(ui.prompt_line(f"Select 1-{len(choices)}", str(default + 1)))
            if not value:
                return default
            idx = int(value) - 1
...
def is_noninteractive() -> bool:
    ...
    ``ROBO_NONINTERACTIVE=1`` (see ``robo_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("ROBO_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``ROBO_NONINTERACTIVE=1`` or a closed/redirected
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'robo setup')")


def _print_setup_summary(config: dict, robo_home):
    ...
    try:
        from robo_cli.auth import resolve_provider
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Robo cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    robo model            (pick any provider/model)")
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'robo setup' to configure"))
    ...
    _web_provider = next((name for var, name in _web_env_providers if get_env_value(var)), None)
    if _web_provider:
    ...
    if _agent_browser_installed:
        try:
            ...
            _local_browser_ready = _chromium_installed() or _using_lightpanda_engine()
        except Exception:
            _local_browser_ready = False
    if get_env_value("CAMOFOX_URL"):
        tool_status.append(("Browser Automation (Camofox)", True, None))
    elif get_env_value("BROWSERBASE_API_KEY") and get_env_value("BROWSERBASE_PROJECT_ID"):
        tool_status.append(("Browser Automation (Browserbase)", True, None))
    elif get_env_value("BROWSER_USE_API_KEY"):
        tool_status.append(("Browser Automation (Browser Use)", True, None))
    elif _local_browser_ready:
        tool_status.append(("Browser Automation (Local browser)", True, None))
    else:
        tool_status.append(
    ...
    # Video generation — opt-in via `robo tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    try:
        from agent.video_gen_registry import list_providers as _list_video_providers
        from robo_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    if tts_provider == "elevenlabs" and get_env_value("ELEVENLABS_API_KEY"):
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'robo setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'robo setup tts'"))
    else:
    ...
    if stt_provider == "openai" and (
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'robo tools' → Speech-to-Text")
            )
    ...
    # Spotify (OAuth via robo auth spotify — check auth.json, not env vars)
    try:
        from robo_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'robo setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {robo_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('robo setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('robo setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('robo setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('robo setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('robo setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('robo config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('robo config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('robo config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('robo', Colors.GREEN)}              Start chatting")
    print(f"   {color('robo gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('robo doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `robo tools` and `robo setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``robo model``)
    ...
    provider added to ``robo model`` is automatically available here.
    ...
    from robo_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: robo model")
    ...
    # on demand via `robo auth add`, `robo setup` vision, and
    # `robo setup tts`. This keeps both quick and full setup thin.


    # Tool Gateway prompt (if applicable) is already shown by the provider
    # setup flow above.
    save_config(config)
...
def _install_neutts_deps() -> bool:
    ...
    from robo_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``robo model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Robo auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via robo setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'robo setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # configurable later via `robo setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `robo setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `robo egress setup` then `robo egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `robo egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'robo-engineer[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $ROBO_HOME/bin is never on PATH, so a bare
            # which() misses the uv Robo installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'robo-engineer[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into ROBO_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("ROBO_MAX_ITERATIONS")
    ...
    print_info("  Run `robo setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # ROBO_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.robo/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from robo_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_robo_home(Path(get_robo_home()))
    except Exception:
        pass
...
def _profile_name_from_robo_home(robo_home) -> str | None:
    """Return the active profile name when ROBO_HOME is a profile dir."""
    if robo_home.parent.name == "profiles":
        return robo_home.name
    return None
...
def _setup_telegram():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Telegram?", False):
            ...
            if not get_env_value("TELEGRAM_ALLOWED_USERS"):
                print_info("⚠  Telegram has no user allowlist - anyone can use your bot!")
    ...
    if allowed_users:
        ...
        print_info("⚠  No allowlist set - anyone who finds your bot can use it!")

    print()
    print_info("Home Channel: where Robo delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Robo to iMessage via BlueBubbles — a free, open-source")
    ...
    if allowed_users:
        ...
        print_info("⚠  No allowlist set — anyone who can iMessage you can use the bot!")
...
def _setup_qqbot():
    ...
    from robo_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from robo_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Robo from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'robo setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     robo config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_robo_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: robo gateway install --system")
                print_info("  Or run in foreground:  robo gateway")
        else:
            from robo_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   robo gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   robo gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `robo setup tools` and `robo tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Robo profile and are not uploaded.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      robo setup           — full or quick (auto-detected)
      robo setup model     — just model/provider
      robo setup tts       — just text-to-speech
      robo setup terminal  — just terminal backend
      robo setup gateway   — just messaging platforms
      robo setup tools     — just tool configuration
      robo setup telemetry — just local shared metrics
      robo setup agent     — just agent settings
    """
    from robo_cli.config import is_managed, managed_error
    if is_managed():
    ...
    ui.title(
        "Setup",
        "Configure this Robo installation. Enter keeps a shown default; Ctrl+C exits at any time.",
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, robo_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Robo configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'robo setup model|terminal|")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Robo?",
            [
        ...
        if setup_mode == 1:
            _run_blank_slate_setup(config, robo_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {robo_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'robo config edit'")
    ...
    # Tune later with `robo setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, robo_home)
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``robo tools`` (which
    ...
    try:
        ...
        from robo_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``robo setup agent`` /
    ``robo config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, robo_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `robo update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        robo tools")
        print_info("  Seed skills:         robo skills opt-in --sync")
        print_info("  Add MCP servers:     robo mcp add")
        print_info("  Enable plugins:      robo plugins")
        print_info("  Tune agent settings: robo setup agent")
        print()
        _print_setup_summary(config, robo_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, robo_home)


def _blank_slate_walkthrough(config: dict, robo_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`robo update` runs from re-injecting them. Opt back in any")
            print_info("time with `robo skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `robo tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `robo plugins list` / `robo plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `robo plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `robo mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `robo mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   robo tools")
    print_info("  Seed skills:         robo skills opt-in --sync")
    print_info("  Add MCP servers:     robo mcp add")
    print_info("  Tune agent settings: robo setup agent")
    print()
...
def _run_quick_setup(config: dict, robo_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'robo setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Robo to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'robo setup gateway'.")

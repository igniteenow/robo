"""Offline, non-destructive recovery for a damaged Robo session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``robo_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``robo_state._backup_db_file``.
    """
    from robo_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Robo process using this profile and retry."
            )

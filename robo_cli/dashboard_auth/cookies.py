...
Three cookies in play:
  - robo_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - robo_session_rt:   the OAuth refresh token
                         (HttpOnly, lifetime = 24h, ROTATING + reuse-detected)
                         Ignitee Now Portal issues a rotating refresh token for the
                         dashboard auth-code grant (Portal NAS #293 / robo
  ...
  - robo_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
setter wrote ``__Secure-robo_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "robo_session_at"
SESSION_RT_COOKIE = "robo_session_rt"
...
# auth plugins are enabled (for example Basic + Ignitee Now OAuth).
SESSION_PROVIDER_COOKIE = "robo_session_provider"
PKCE_COOKIE = "robo_session_pkce"
...
SSO_ATTEMPT_COOKIE = "robo_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /robo`` we want ``Path=/robo`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    ``refresh_token`` is written as the RT cookie when non-empty. Ignitee Now Portal
    issues a 24h rotating refresh token (robo #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/robo``)

...
from ``robo_constants`` or other robo_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$ROBO_HOME/logs/dashboard-auth.log``.

    Uses ``robo_constants.get_robo_home()`` (a leaf module — no import
    ...
    return get_robo_home() / "logs" / "dashboard-auth.log"

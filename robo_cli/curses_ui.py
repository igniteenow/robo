"""Shared curses-based UI components for Robo CLI.

Used by `robo tools` and `robo skills` for interactive checklists.
...
MARK_OFF = "\u25c7"   # every other choice
CURSOR = "\u203a"     # the row the arrow keys are on


def _draw_title(stdscr, y: int, title: str, max_x: int) -> None:
    """``\u25c6 Select provider`` — the mark in the flame colour, the title bold."""
    import curses

    mark_attr = curses.A_BOLD
    if curses.has_colors():
        try:
            mark_attr |= curses.color_pair(4)
        except curses.error:
            pass
    try:
        stdscr.addnstr(y, 0, MARK + " ", max_x - 1, mark_attr)
        stdscr.addnstr(y, 2, title.rstrip(":"), max(0, max_x - 3), curses.A_BOLD)
    except curses.error:
        pass
...
# Ignitee Now palette for curses menus (assets/brand/BRAND.md).
# pair 1 = ember (the cursor row, the selected mark)   pair 2 = indigo (labels)
# pair 3 = muted grey (hints)                            pair 4 = flame (the title mark)
_BRAND_256 = {1: 208, 2: 141, 3: 245, 4: 203}


def _init_brand_colors(curses, extra_color_pairs: bool = True) -> None:
    """Initialise the four brand colour pairs, degrading to the nearest of the
    eight basic colours on terminals without a 256-colour palette."""
    try:
        curses.start_color()
    ...
    basic = {1: curses.COLOR_YELLOW, 2: curses.COLOR_MAGENTA, 3: curses.COLOR_WHITE, 4: curses.COLOR_RED}
    for pair, code in _BRAND_256.items():
        if pair == 3 and not extra_color_pairs:
            continue
        try:
            curses.init_pair(pair, code if rich else basic[pair], -1)
        except curses.error:
            pass
...
def _curses_style_attr(curses, style: Optional[str], *, is_cursor: bool):
    ...
    The cursor row is ember and bold, so the selection is one solid
    highlight; labels are indigo; hints are muted.
    """
    if is_cursor:
        attr = curses.A_BOLD
        if curses.has_colors():
    ...
    *,
    ...
    try:
        ...
        def _draw(stdscr):
            curses.curs_set(0)
            if curses.has_colors():
                _init_brand_colors(curses, extra_color_pairs)
...
def curses_checklist(
    ...
    *,
    ...
    def _draw_header(stdscr, max_y, max_x):
        import curses
        try:
            _draw_title(stdscr, 0, title, max_x)
            stdscr.addnstr(
                1, 0,
                "  \u2191\u2193 move   Space toggle   Enter confirm   Esc cancel",
                max_x - 1, _curses_style_attr(curses, "dim", is_cursor=False),
            )
        except curses.error:
            pass
        return 3
...
def curses_radiolist(
    ...
    *,
    ...
    def _draw_header(stdscr, max_y, max_x, search=None):
        ...
        row = 0
        try:
            _draw_title(stdscr, row, title, max_x)
            row += 1
            ...
            if searchable and search is not None and search.active:
                ...
                hint = "  \u2191\u2193 move   Enter select   / search   Esc cancel"
            else:
                hint = "  \u2191\u2193 move   Enter select   Esc cancel"
            stdscr.addnstr(row, 0, hint, max_x - 1, _curses_style_attr(curses, "dim", is_cursor=False))
            row += 1
        except curses.error:
            pass
    ...
    def _draw_row(stdscr, y, i, is_cursor, max_x):
        import curses
        radio = MARK if i == selected else MARK_OFF
        arrow = CURSOR if is_cursor else " "
...
def _radio_numbered_fallback(
    ...
    for i, label in enumerate(items):
        marker = color(MARK, Colors.YELLOW, Colors.BOLD) if i == selected else MARK_OFF
    ...
    def _draw_header(stdscr, max_y, max_x, search=None):
        ...
        try:
            ...
            if searchable and search is not None and search.active:
                ...
                hint = "  \u2191\u2193 move   Enter confirm   / search   Esc cancel"
            else:
                hint = "  \u2191\u2193 move   Enter confirm   Esc cancel"
            stdscr.addnstr(1, 0, hint, max_x - 1, _curses_style_attr(curses, "dim", is_cursor=False))
        except curses.error:
            pass
        return 3

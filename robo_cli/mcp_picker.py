"""MCP picker — interactive `robo mcp picker` (also the default `robo mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``robo mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `robo plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from robo_cli.cli_output import prompt_yes_no
from robo_cli.curses_ui import curses_single_select
from robo_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Robo session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from robo_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `robo mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: robo mcp install <name>    Picker: robo mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Robo is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Robo — run `robo update` "
...
def show_catalog() -> None:
    """`robo mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`robo mcp picker` (and default `robo mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`robo mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `robo mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:

"""`robo checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.robo/checkpoints/``.  Actions:

    robo checkpoints               # same as `status`
    robo checkpoints status        # total size, project count, breakdown
    robo checkpoints list          # per-project checkpoint counts + workdir
    robo checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    robo checkpoints clear [-f]    # nuke the entire base (asks first)
    robo checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    robo checkpoints
    robo checkpoints prune --retention-days 3 --max-size-mb 200
    robo checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: robo checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``robo checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `robo checkpoints` → status

"""CLI handlers for the ``robo proxy`` subcommand."""
...
from robo_cli.proxy.adapters import ADAPTERS, get_adapter
from robo_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "robo proxy requires aiohttp. Run `robo setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"robo auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Robo proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Robo proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: robo proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``robo proxy <subcommand>``."""
    ...
    print(
        "robo proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  robo proxy start [--provider xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  robo proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  robo proxy providers\n"

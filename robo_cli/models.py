...
Add, remove, or reorder entries here — both `robo setup` and
`robo` provider-selection will pick up the change automatically.
"""
...
from robo_cli.urllib_security import open_credentialed_url

logger = logging.getLogger(__name__)

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_ROBO_USER_AGENT = f"robo-cli/{_ROBO_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `robo model`
    flow without maintaining a separate static list.
    """
    from robo_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $ROBO_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Robo-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $ROBO_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Robo refreshes the cache from
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # endpoint expects (see robo_cli/model_setup_flows.py).
...
# ---------------------------------------------------------------------------
# Canonical provider list — single source of truth for every picker that shows
# this list:  robo model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `robo model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `robo model` TUI
...
#
# Some vendors expose several Robo provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `robo model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``robo model``, the
...
# In-repo fallback for the model Robo silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from robo_cli.model_catalog import get_default_model_from_cache
...
# curated-list entry [0]. Metered aggregators (OpenRouter) order their lists
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``robo auth add openai-codex`` without
    ``robo model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``robo model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    robo-engineer is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (some OpenRouter-compatible mirrors, older catalog snapshots) don't populate
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from robo_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — robo-engineer
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Robo lands on when none is selected.
            desc = "default"
        else:
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from robo_constants import AI_GATEWAY_BASE_URL
...
def compute_sale_discount(
    ...
    No current caller opts into this: only a Portal-style gateway ever set
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    When *include_sale_original* is true (no current caller sets this) and the gateway
    ...
    for item in payload.get("data", []):
        ...
        if mid and isinstance(pricing, dict):
            ...
            # Sale chrome: never copy pricing.original for
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return robo-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; robo's picker expects
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``robo model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from robo_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from robo_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    if resolved_provider not in {"custom", "openrouter"}:
        ...
        if (
            ...
            # curated list is ordered most-capable-first for some metered
            # aggregators, entry [0] is the priciest flagship, and typing a
            # bare provider name would silently escalate to it — the exact billing footgun the
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Robo' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``robo_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Robo should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``robo auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.robo/.env``.
    ...
    """
    try:
        from robo_cli.auth import resolve_api_key_provider_credentials
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from robo_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Robo release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Robo release). Falls back to the hardcoded catalog if no token
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``robo model`` shows.
            from robo_cli.providers import is_official_openai_host
...
#
# Cache strategy:
#   - One JSON file at $ROBO_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    ...
    return get_robo_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$ROBO_HOME/auth.json`` and external
    ...
    # env var, so it must change the fingerprint too or `robo config set
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _ROBO_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Robo' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Robo' native Anthropic adapter. The live catalog may
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _ROBO_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"robo-engineer/{_ROBO_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=robo"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from robo_constants import get_robo_home
    return get_robo_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `robo moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Robo will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Robo cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Robo actually ships.  (#46850)
            #

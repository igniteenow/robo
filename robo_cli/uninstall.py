...
- Keep data: Remove code but keep ~/.robo/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Robo PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing robo-engineer or robo PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Robo" comment and following line
                if '# Robo' in line or '# robo-engineer' in line:
                    skip_next = True
                    continue
                if skip_next and ('robo' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing robo
                if 'robo' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Robo-owned file, and
...
def remove_wrapper_script():
    """Remove the robo wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "robo",
        Path.home() / ".local" / "bin" / "robo-acp",
        Path.home() / ".local" / "bin" / "robo-engineer",
        Path("/usr/local/bin/robo"),
        Path("/usr/local/bin/robo-acp"),
        Path("/usr/local/bin/robo-engineer"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains robo_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'robo_cli' in content or 'robo-engineer' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(robo_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``robo`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Robo home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (robo_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``robo gateway run`` processes
    ...
    try:
        from robo_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from robo_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ROBO_HOME`` and ``ROBO_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\robo\git\cmd``, ``%LOCALAPPDATA%\robo\git\bin``,
#      ``%LOCALAPPDATA%\robo\git\usr\bin``, ``%LOCALAPPDATA%\robo\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\robo\git\`` and Node to
#      ``%LOCALAPPDATA%\robo\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``robo dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\robo\gateway-service\`` and sometimes
...
def _robo_path_markers(robo_home: Path) -> list[str]:
    """Path-entry substrings that identify Robo-owned User-PATH entries."""
    root = str(robo_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare robo-engineer install dir.
    markers = [root + "\\robo-engineer", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ROBO_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "robo".  We don't want to catch
    # unrelated entries like "crobo-foo" or "ephermeral", so we look for
    # backslash-robo as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(robo_home: Path) -> list[str]:
    """Strip Robo-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _robo_path_markers(robo_home)
            kept: list[str] = []
...
def remove_robo_env_vars_windows() -> list[str]:
    """Delete ROBO_HOME and ROBO_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ROBO_HOME", "ROBO_GIT_BASH_PATH"):
                try:
...
def _is_default_robo_home(robo_home: Path) -> bool:
    """Return True when ``robo_home`` points at the default (non-profile) root."""
    try:
        from robo_constants import get_default_robo_root
        return robo_home.resolve() == get_default_robo_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ROBO_HOME directory.

    We shell out to ``robo -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ROBO_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m robo_cli.main` so we don't depend on a `robo`
    #    wrapper that may be half-removed mid-uninstall.
    robo_invocation = [_sys.executable, "-m", "robo_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                robo_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's ROBO_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``robo uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$ROBO_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from robo_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(robo_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         Robo Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Robo Chat GUI installation was found.")
        print(f"  Checked: {robo_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Robo runtime stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(robo_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Robo runtime at {robo_home / 'robo-engineer'}")
        print(f"  • Your config, sessions, and secrets under {robo_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Robo runtime is still installed. Run 'robo' to use the CLI,")
    print("or 'robo uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.robo/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.robo/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie ROBO_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_robo_home(robo_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.robo data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            Robo Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {robo_home / 'config.yaml'}")
    print(f"  Secrets: {robo_home / '.env'}")
    print(f"  Data:    {robo_home / 'cron/'}, {robo_home / 'sessions/'}, {robo_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ROBO_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠  WARNING: This will permanently delete ALL Robo data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Robo code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, robo_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Robo PATH entries from shell configs / Windows User PATH")
    print("  • Robo wrapper scripts and Robo-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Robo config/data: {robo_home}")
        if _is_default_robo_home(robo_home):
        ...
        print(f"  • Keep Robo config/data: {robo_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``robo`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$ROBO_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in robo_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\robo\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(robo_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Robo-owned PATH entries in User environment")

        log_info("Removing ROBO_HOME / ROBO_GIT_BASH_PATH User env vars...")
        removed_env = remove_robo_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Robo-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing robo command...")
    ...
    #     (only when they still point into this Robo home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Robo-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(robo_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Robo-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(robo_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE ROBO_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from robo_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(robo_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.robo/, just remove the robo-engineer subdir
            if robo_home in project_root.parents or project_root.parent == robo_home:
    ...
    #     under ROBO_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(robo_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(robo_home)
    ...
    # 5. Optionally remove ~/.robo/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ROBO_HOME dirs live under
        ...
        try:
            if robo_home.exists():
                shutil.rmtree(robo_home)
                log_success(f"Removed {robo_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {robo_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {robo_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://github.com/IgniteeNow/robo-engineer)", Colors.DIM))
        else:
            print(color("  git clone https://github.com/IgniteeNow/robo-engineer.git robo && cd robo && bash install-robo.sh", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Robo! ▸")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m robo_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import robo_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``robo_constants`` + ``robo_cli.colors``
    (and lazily ``robo_cli.gui_uninstall``), so it runs fine under a bare

"""
Robo CLI - Unified command-line interface for Robo.

Provides subcommands for:
- robo chat          - Interactive chat (same as ./robo)
- robo gateway       - Run gateway in foreground
- robo gateway start - Start gateway service
...
# Runtime source of truth for the product version (``robo --version``, the
# dashboard ``/version`` probe, User-Agent strings, health exports). Sourced
# from robo_runtime.version so it can never drift from ``ROBO_VERSION`` again;
# the literal fallback only covers a partial install missing that package.
try:
    from robo_runtime.version import ROBO_VERSION as __version__
...
def _ensure_utf8():
    ...
    can even start — e.g. `robo setup` on a fresh Pi.
    ...
    robo_cli/stdio.py::configure_windows_stdio() runs later from the entry

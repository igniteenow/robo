...
the tool registry or launching Robo (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``robo-cli`` to the non-existent ``robo``. ``resolve_toolset('robo')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``robo-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``robo`` instead of ``robo-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `robo tools` to reconfigure."
        )
    return warnings

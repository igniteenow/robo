...
DEFAULT_CODEX_MODELS: List[str] = [
    ...
    # in robo_cli/models.py — only the openai-codex (OAuth) provider
    ...
    # Robo uses. Removed here. If OpenAI re-enables them on Codex backend,
...
def _add_forward_compat_models(model_ids: List[str]) -> List[str]:
    ...
    older compatible template model is present. This follows the common
...
def _read_cache_models(codex_home: Path) -> List[str]:
    ...
    if isinstance(entries, list):
        for item in entries:
            ...
            # public OpenAI API, while Robo openai-codex talks to the same

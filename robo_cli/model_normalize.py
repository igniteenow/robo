...
- **Aggregators** (OpenRouter, Ignitee Now, AI Gateway, Kilo Code) need
...
- **DeepSeek** accepts only the first-class V-series IDs
  ...
  Robo revisions folded every non-reasoner input into
...
Normalizes vendor model ids to the canonical form each provider expects.
"""
...
# slug used by aggregator APIs (OpenRouter, Ignitee Now, etc.).
#
...
def _normalize_provider_alias(provider_name: str) -> str:
    """Resolve provider aliases to Robo' canonical ids."""
...
def normalize_model_for_provider(model_input: str, target_provider: str) -> str:
    ...
    Args:
        ...
        target_provider: The canonical Robo provider id, e.g.
            ``"openrouter"``, ``"anthropic"``, ``"copilot"``,
            ``"deepseek"``, ``"custom"``.  Should already be normalised
            via ``robo_cli.models.normalize_provider()``.
    ...
    if provider in {"copilot", "copilot-acp"}:
        try:
            from robo_cli.models import normalize_copilot_model_id

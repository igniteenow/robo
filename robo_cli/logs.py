"""``robo logs`` — view and filter Robo log files.
...
Usage examples::
    ...
    robo logs                    # last 50 lines of agent.log
    robo logs -f                 # follow agent.log in real time
    robo logs errors             # last 50 lines of errors.log
    robo logs gateway -n 100    # last 100 lines of gateway.log
    robo logs gui -f            # follow gui.log (dashboard/pty/ws)
    robo logs desktop -f        # follow desktop.log (Electron app boot/backend)
    robo logs --level WARNING    # only WARNING+ lines
    robo logs --session abc123   # filter by session ID substring
    robo logs --component tools  # only tool-related lines
    robo logs --since 1h         # lines from the last hour
    robo logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_robo_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Robo runs — try 'robo chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_robo_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_robo_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_robo_home()}/logs/")
        return

    print(f"Log files in {display_robo_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'robo chat' to generate logs)")

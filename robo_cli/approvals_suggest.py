"""``robo approvals suggest`` — mine approval history into allowlist proposals.

Robo has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.robo/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, robo self-management, …) are eligible.
...
def normalize_command(command: str) -> str:
    """Fold user/robo home prefixes and collapse whitespace.

    Reuses tools.approval's home-folding machinery so proposals are portable
    across machines/users (``/home/alice/x`` -> ``~/x``).
    """
    from tools.approval import (
        _rewrite_resolved_robo_home,
        _rewrite_resolved_user_home,
    )

    folded = _rewrite_resolved_user_home(_rewrite_resolved_robo_home(command))
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  robo approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.robo/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``robo approvals suggest``."""

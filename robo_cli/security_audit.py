"""On-demand supply-chain audit for Robo installs.

Scans three surfaces a Robo user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Robo venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.robo/plugins``
...
def _discover_plugins(robo_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.robo/plugins``.
...
def _discover_components(
    *,
    ...
    home = robo_home or Path(get_robo_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `robo security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, robo_home=home
    )

...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (Ignitee Now /models returns ~400
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing so the
      GUI picker can show $/Mtok columns — mirroring the ``robo model`` CLI
      picker. Adds a network call (pricing fetch); only set for interactive
      pickers.
    ...
    try:
        from robo_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def build_aux_picker_rows(
    *,
    ...
    back. Mirrors the same filter in ``robo_cli/moa_cmd.py``.
...
def _apply_featured(rows: list[dict]) -> None:
    ...
    Aggregator providers (igniteenow, openrouter) serve dozens of models across many
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from robo_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `robo model` to reactivate. "
                "Showing the saved model only."
            )
            extras.append(
                {
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for Robo.
    """
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            if auth_type == "api_key" and key_env
            else f"run `robo model` to configure ({auth_type})"
        )
...
def _apply_pricing(
    ...
    for row in rows:
        ...
        for mid in models:
            ...
            # Sale chrome is Ignitee Now Portal-only. Other providers (OpenRouter,
            ...
            if slug == "igniteenow" and not is_free:
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`robo_cli.model_switch.list_picker_providers`) so the

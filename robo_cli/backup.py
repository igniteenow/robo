"""
Backup and import commands for robo CLI.

`robo backup` creates a zip archive of the entire ~/.robo/ directory
(excluding the robo-engineer repo and transient files).

`robo import` restores from a backup zip, overlaying onto the current
ROBO_HOME root.
"""
...
from robo_constants import get_default_robo_root, get_robo_home, display_robo_home
...
# ``robo-engineer`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/robo-engineer/``
...
# ROBO_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "robo-engineer",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under ROBO_HOME) —
...
# File names that ``robo import`` must never overwrite, matched by basename so
...
#     stuck "starting"/"cooking", disconnecting it from the Ignitee Now portal
...
# Reserved archive subtree for provider state that lives OUTSIDE ROBO_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Robo backup slot."""
...
@contextmanager
def _backup_operation_lock(robo_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = robo_home / ".backup.lock"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Robo backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside ROBO_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to robo root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``robo-engineer`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/robo-engineer/`` — must be preserved.
        if part == "robo-engineer" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside ROBO_HOME.
...
# to the user as a hung `robo update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # robo_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the Robo home directory."""
    robo_root = get_default_robo_root()

    if not robo_root.is_dir():
        print(f"Error: Robo home directory not found at {robo_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(robo_root):
            _run_backup_locked(args, robo_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, robo_root: Path) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"robo-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"robo-backup-{stamp}.zip"
    ...
    print(f"Scanning {display_robo_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(robo_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(robo_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``robo-engineer`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "robo-engineer" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(robo_root)
    ...
    # outside ROBO_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_robo_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: robo import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Robo backup.
    ...
    # Look for telltale files that a robo home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Robo backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.robo/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a robo dir name
        if prefix in {".robo", "robo"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_robo_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (robo_root / "config.yaml").exists()
        has_env = (robo_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Robo configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under ROBO_HOME.
            ...
            # Ignitee Now portal. Matched by basename so both the root profile and
            # named profiles (profiles/<name>/gateway_state.json) are covered.
            if Path(rel).name in _IMPORT_SKIP_NAMES:
                skipped_runtime.append(rel)
                continue

            target = robo_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(robo_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_robo_home()}."
            )
        ...
        profiles_dir = robo_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # robo_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: robo profile list  (after installing robo)")

        # Guidance
        print()
        if not (robo_root / "robo-engineer").is_dir():
            print("Note: The robo-engineer codebase was not included in the backup.")
            print("  If this is a fresh install, run: robo update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  robo -p {pname} gateway install")

        print("Done. Your Robo configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and robo backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ROBO_HOME).
...
# — `robo update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $ROBO_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside ROBO_HOME and the entry is silently
...
def _quick_snapshot_root(robo_home: Optional[Path] = None) -> Path:
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: robo snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(robo_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside ROBO_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``robo update``.
    ...
    Args:
        ...
        robo_home: Override for the Robo home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(robo_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for robo backup --quick."""
...
def _write_full_zip_backup(out_path: Path, robo_root: Path) -> Optional[Path]:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(robo_root):
            return _write_full_zip_backup_locked(out_path, robo_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, robo_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``robo_root`` to ``out_path``.
...
def _pre_update_backup_dir(robo_home: Optional[Path] = None) -> Path:
    home = robo_home or get_robo_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ROBO_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ROBO_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``robo update``) should continue even if the backup fails.
    """
    robo_root = robo_home or get_default_robo_root()
    if not robo_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(robo_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, robo_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (safety net for import/migration commands)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ROBO_HOME under ``backups/`` before a
    migration-style command (for example ``robo import-agent``) applies changes.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``robo import <archive>``.  Writes to
    ``<ROBO_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `robo import` and the

"""``robo debug`` debug tools for Robo.

Currently supports:
    robo debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.robo/logs/*.log`` are not leaked into
                          the public paste service. Pass ``--no-redact``
                          to disable.
                          Pass ``--igniteenow`` to upload instead to Ignitee Now's
                          private support storage via a signed URL minted by
                          the Ignitee Now support service, rather than going
                          to a public paste. Requires that service to be
                          provisioned (see ROBO_DIAGNOSTICS_BASE_URL).
"""
...
_REDACTION_BANNER = (
    "[robo debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.robo/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``robo debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``robo debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `robo debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "robo-engineer/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``robo debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.robo/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``robo debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "robo-engineer/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----RoboDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from robo_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_robo_home() / "logs" / filename) if filename else None


# Logs written by a client process rather than by this backend. When the
# desktop app talks to a remote/docker/SSH backend, `robo debug share` runs
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by Robo Desktop on the machine running the app, not by this "
...
def _capture_dump() -> str:
    """Run ``robo dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``robo dump`` is run
        internally.
...
# ---------------------------------------------------------------------------
# Shared bundle collection (used by both the paste.rs and Ignitee Now-S3 paths)
# ---------------------------------------------------------------------------

# Bundle format identifier embedded in the Ignitee Now-S3 JSON envelope. The
# discord-support viewer keys off this string to parse the bundle.
_IGNITEENOW_BUNDLE_FORMAT = "robo-debug-share/1"
...
def collect_share_bundle(
    ...
    Ignitee Now-S3 path (``--igniteenow``).  Centralising it guarantees the Ignitee Now bundle is
    built from the *same* force-redacted snapshots as the public paste path —
    redaction is the safety boundary, so the Ignitee Now path must never see raw
    logs.
...
def build_igniteenow_bundle(bundle: dict[str, str], redact: bool = True) -> bytes:
    """Gzip-compress a :func:`collect_share_bundle` mapping into the Ignitee Now envelope.

    The JSON shape is what the discord-support viewer (Repo 3) parses::

        {"format": "robo-debug-share/1",
         "redacted": <bool>,
         "created": <iso8601>,
    ...
    envelope = {
        "format": _IGNITEENOW_BUNDLE_FORMAT,
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``robo debug share`` (CLI) and the
    ...
    # shared collector so the paste.rs and Ignitee Now-S3 paths build identical,
    ...
    if redact:
        logger.info(
            "robo debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    igniteenow = getattr(args, "igniteenow", False)
    ...
    if igniteenow:
        _run_debug_share_igniteenow(args, log_lines=log_lines, redact=redact)
        return
    ...
    try:
        ...
        print("\nRun `robo debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  robo debug delete <url>")

    print("\nShare these links with the Robo team for support.")


_IGNITEENOW_PRIVACY_NOTICE = """\
⚠  --igniteenow: This uploads your debug bundle to Ignitee Now's PRIVATE support
    storage, NOT a public paste service. The following is included:
  • System info (OS, Python/Robo version, provider, which API keys are
  ...
  • The bundle is for Ignitee Now support staff only.
  • It is NOT a public paste — there is no public URL to the contents.
  • Retention and access are governed by Ignitee Now's support privacy policy.
"""


def _run_debug_share_igniteenow(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``robo debug share --igniteenow``: upload the bundle to Ignitee Now-S3.

    Collects the same force-redacted bundle as the paste path, gzips it into
    the Ignitee Now envelope, requests a signed URL from NAS, uploads, and prints the
    private viewer link. On any failure falls back to a clear error that
    suggests ``--local``.
    """
    from robo_cli.diagnostics_upload import share_to_igniteenow

    print(_IGNITEENOW_PRIVACY_NOTICE)
    if not _confirm_upload(args):
        return
    if not redact:
        print(
            "⚠  --no-redact is set: secrets in your logs will NOT be redacted "
            "before upload.\n"
        )
    print("Collecting debug report...")
    _best_effort_sweep_expired_pastes()
    ...
    if redact:
        logger.info(
            "robo debug share --igniteenow: applied force-mode redaction before upload"
        )
    blob = build_igniteenow_bundle(bundle, redact=redact)

    print("Uploading to Ignitee Now diagnostics storage...")
    try:
        res = share_to_igniteenow(blob)
    except Exception as exc:
        print(
            f"\nIgnitee Now upload failed: {exc}\n"
            "\nThe Ignitee Now diagnostics service may be unavailable or not yet "
            "provisioned.\n"
            "Run `robo debug share --local` to print the report instead, "
            "or `robo debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)

    view_url = res.get("viewUrl") or res.get("view_url")
    print("\nDebug bundle uploaded to Ignitee Now (private):")
    if view_url:
    ...
    print(
        "\nShare this private link with the Ignitee Now team — only Ignitee Now staff "
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: robo debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'robo debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``robo debug`` call.
    ...
    # best-effort — any failure is swallowed so ``robo debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: robo debug <command>")
        print()
        print("Commands:")
        ...
        print("  --igniteenow       Upload to Ignitee Now-internal storage (private, staff-only,")

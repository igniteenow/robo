"""Migrate Robo' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Robo' YAML and writes equivalent [mcp_servers.<name>]
  ...
  2. Queries codex's `plugin/list` for the openai-curated marketplace
     ...
     the user has installed=true on their codex CLI. (Sometimes called
     "migrate native codex plugins" — the standout bit: Canva, GitHub,
     Calendar, Gmail pre-configured.)
...
What translates (MCP servers):
  Robo mcp_servers.<n>.command/args/env  → codex stdio transport
  Robo mcp_servers.<n>.url/headers       → codex streamable_http transport
  Robo mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Robo mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Robo-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Robo' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Robo config.")
...
# Robo keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Robo' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, robo_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Robo MCP server config to the codex inline-table dict
    ...
    if not isinstance(robo_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(robo_cfg.get("command"))
    has_url = bool(robo_cfg.get("url"))
    ...
    for key in robo_cfg:
        ...
        elif key not in _KNOWN_ROBO_KEYS:
            skipped.append(f"{key} (unknown Robo key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (ROBO_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Robo)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Robo' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Robo' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Robo
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="robo-migration")
    ...
    for marketplace in marketplaces:
        ...
        for plugin in plugins:
            ...
            # Gate migration on app readiness to avoid writing config that
            ...
            # true for installed plugins. This matches the shape used
            # when migrating native codex plugins.
            out.append({
                "name": name,
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a ROBO_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed robo-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.robo`` symlink that happens to live under ``/private/var/folders``
...
def _build_robo_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Robo' own
    ...
    so a robo installed under /opt/, /usr/local/, or a venv all work.
    ROBO_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # ROBO_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_robo_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever ROBO_HOME its launcher sets
    ...
    # — that would override the launcher's ROBO_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("ROBO_HOME", tmp_path) would
    ...
    robo_home = os.environ.get("ROBO_HOME") or ""
    if robo_home and _looks_like_test_tempdir(robo_home):
        robo_home = ""
    if robo_home:
        env["ROBO_HOME"] = robo_home
    # PYTHONPATH passes through so a worktree-launched robo finds the
    ...
    env["ROBO_QUIET"] = "1"
    env["ROBO_REDACT_SECRETS"] = env.get("ROBO_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_robo_tools: bool = True,
) -> MigrationReport:
    """Translate Robo mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        robo_config: full ~/.robo/config.yaml dict
        ...
        expose_robo_tools: when True (default), register Robo' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Robo for tools
    ...
    robo_servers = (robo_config or {}).get("mcp_servers") or {}
    if not isinstance(robo_servers, dict):
        report.errors.append(
            "mcp_servers in Robo config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in robo_servers.items():
    ...
    # Inject Robo' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Robo for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/robo_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_robo_tools:
        translated["robo-tools"] = _build_robo_tools_mcp_entry()
        if "robo-tools" not in report.migrated:
            report.migrated.append("robo-tools")

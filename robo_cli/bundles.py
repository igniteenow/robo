"""Implementation of the ``robo bundles`` CLI subcommand.

Mirrors the structure of ``robo_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `robo bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  robo bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``robo bundles`` argparse tree.

    Called from ``robo_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `robo bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``robo bundles <subcommand>`` to the right handler."""

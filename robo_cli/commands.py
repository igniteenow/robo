"""Slash command definitions and autocomplete for the Robo CLI.
...
from robo_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``robo_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``robo_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Robo config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Robo works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Robo is working", "Configuration",
               ...
               subcommands=("interrupt", "redirect", "queue", "steer", "status")),
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.robo/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Robo to the latest version", "Info",
               busy_policy="dispatch"),
    CommandDef("version", "Show Robo version", "Info", aliases=("v",),
               busy_policy="dispatch", execute="version"),
    CommandDef("debug", "Upload debug report (system info + logs) and get shareable links", "Info",
               args_hint="[igniteenow|local]"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`robo_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/robo`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Robo ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Robo
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``robo skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``robo skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``robo skills list`` and
...
# unrelated command landed. These claim their slots right after /robo,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /robo <command>).
...
# native slot, the alias spelling stays reachable via /robo reset).
...
# ``/robo <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/robo" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /robo topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /robo moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /robo debug on Slack.
#   - egress: Docker-only proxy status; reachable as /robo egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /robo init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /robo version on
...
#   - diff: git working-tree diff; reached via /robo diff on Slack so it
...
#     /robo update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /robo heartbeat
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/robo <verb>`` subcommand.
    ...
    can still reach them via ``/robo <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/robo`` is always reserved as the first entry so the
    legacy ``/robo <subcommand>`` form keeps working for anything that
    ...
    # Reserve /robo as the catch-all top-level command.
    entries.append(("robo", "Talk to Robo or run a subcommand", "[subcommand] [args]"))
    seen.add("robo")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_ROBO_ONLY:
            # Intentionally Slack-via-/robo only (see _SLACK_VIA_ROBO_ONLY).
            return
    ...
    # everything except /robo, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://robo-engineer.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /robo handler.

    Maps both canonical names and aliases so /robo bg do stuff works
    the same as /robo background do stuff.

    Plugin-registered slash commands are included so ``/robo <plugin-cmd>``

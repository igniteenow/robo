"""
Unified self-relaunch for Robo CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``robo sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``robo`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched robo."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_robo_bin() -> Optional[str]:
    """Find the robo entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("robo")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m robo_cli.main``.
    ...
    ``robo.exe`` on PATH or the ``sys.executable -m robo_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    if bin_path:
        ...
        argv = [sys.executable, "-m", "robo_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    relaunch contract wants: "run robo again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``robo.exe`` (a Python console-script shim that wraps
    ``python -m robo_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "robo exited, then
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``robo`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nRobo relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run robo.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:

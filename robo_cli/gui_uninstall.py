"""
Robo Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``robo desktop`` / ``robo gui``)
     Built inside the agent checkout under ``$ROBO_HOME/robo-engineer/``:
       ...
       - ``$ROBO_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Robo.app`` or ``~/Applications/Robo.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Robo`` (NSIS per-user)
...
the app name ("Robo"), separate from ``$ROBO_HOME``:
  - macOS:   ``~/Library/Application Support/Robo``
  - Windows: ``%APPDATA%\\Robo``
  - Linux:   ``$XDG_CONFIG_HOME/Robo`` (default ``~/.config/Robo``)
...
``robo uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(robo_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return robo_home / "robo-engineer"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Robo"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Robo"
...
def source_built_gui_artifacts(robo_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``robo desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``robo-engineer/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(robo_home)
    ...
    return [
        ...
        robo_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Robo" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Robo.app"),
            home / "Applications" / "Robo.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Robo).
            local_base / "Programs" / "Robo",
            # Older / alternate layout some builds used.
            local_base / "robo-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Robo")
    else:
        ...
        # ``uninstall_gui``.
        from robo_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `robo desktop` installs. Its icon lives
            # in the checkout, not in the installed app.
            desktop_entry_path(),
            # Some packaged builds emit this casing.
            data_base / "applications" / "Robo.desktop",
        ]
    return paths


def agent_is_installed(robo_home: Path) -> bool:
    """Return True when a usable Python agent install exists under ROBO_HOME.
...
def gui_is_installed(robo_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(robo_home):
        if p.exists():
            return True
...
def gui_install_summary(robo_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = robo_home if robo_home is not None else get_robo_home()
...
def uninstall_gui(robo_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``robo-engineer/robo_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$ROBO_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Robo disappears from the launcher.
        try:
            from robo_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove robo' or "
            "'sudo dnf remove robo'). AppImage builds are a single file you "

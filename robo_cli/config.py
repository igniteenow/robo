"""
Configuration management for Robo.

Config files are stored in ~/.robo/ for easy access:
- ~/.robo/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.robo/.env         - API keys and secrets

This module provides:
- robo config          - Show current configuration
- robo config edit     - Open config in editor
- robo config get      - Print a resolved configuration value
- robo config set      - Set a specific value
- robo config unset    - Remove a user configuration value
- robo config wizard   - Re-run setup wizard
"""
...
from robo_cli.route_identity import normalize_route_base_url
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``robo config set`` (which rewrites
    ...
    ``config.yaml`` in place: robo never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.robo/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``robo logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠  robo config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Robo makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Robo itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``robo update``, the TUI build.
...
#   on every plugin install / ``robo update``.
...
# * ``ROBO_HOME`` / ``ROBO_PROFILE`` / ``ROBO_CONFIG`` /
#   ``ROBO_ENV`` — Robo runtime location flags. Writing these into
...
#
# IMPORTANT: ``ROBO_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ROBO_LANGFUSE_PUBLIC_KEY,
# ROBO_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Robo runtime location — never via dashboard env writer.
    # NOT a ROBO_* blanket: integration credentials (ROBO_GEMINI_*,
    # ROBO_LANGFUSE_*, ROBO_SPOTIFY_*, ...) ARE allowed.
    "ROBO_HOME", "ROBO_PROFILE", "ROBO_CONFIG", "ROBO_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Robo runtime location "
            "(ROBO_HOME, ROBO_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # ROBO_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `robo plugins enable
    # observability/langfuse` or `robo tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ROBO_LANGFUSE_ENV",
    "ROBO_LANGFUSE_RELEASE",
    "ROBO_LANGFUSE_SAMPLE_RATE",
    "ROBO_LANGFUSE_MAX_CHARS",
    "ROBO_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    ...
    "ROBO_ACP_AUTH_METHOD",
    "ROBO_ACP_AUTO_APPROVE",
    "ROBO_COPILOT_ACP_COMMAND",
    "ROBO_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from robo_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set ROBO_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ROBO_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_robo_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Robo is running in package-manager-managed mode.

    Two signals: the ROBO_MANAGED env var (set by the systemd service),
    or a .managed marker file in ROBO_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update Robo through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``robo_cli/`` — i.e. the git checkout for source
    installs, ``/opt/robo`` inside the published image. It is a property of
    the running interpreter, NOT of ``$ROBO_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Robo was installed: 'docker', 'nix', 'nixos', 'git', or 'unknown'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$ROBO_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. ROBO_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.robo``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$ROBO_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.robo:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$ROBO_HOME``, a
    ...
    ``docker`` and ``robo update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``igniteenow/robo-engineer`` image bakes a ``docker``
        stamp into ``/opt/robo`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $ROBO_HOME.
    try:
    ...
    #    host install whose shared $ROBO_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``robo update``.
    try:
        method = (
            (get_robo_home() / ".install_method")
    ...
    # detect Nix installs that don't set ROBO_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``robo_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$ROBO_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/robo`` in the published image, which instead bakes the stamp at
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method in {"nix", "nixos"}:
        return _NIX_UPDATE_MSG
    if method == "docker":
        return "docker pull igniteenow/robo-engineer:latest"
    return "robo update"
...
# Long-form text for ``robo update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# robo_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Robo, it doesn't update
...
✗ ``robo update`` doesn't apply inside the Docker container.

Robo runs as a published image (igniteenow/robo-engineer), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull igniteenow/robo-engineer:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate robo-engineer
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm igniteenow/robo-engineer:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/igniteenow/robo-engineer/tags
  • Your config and session history live under ``$ROBO_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``robo update`` inside Docker.
...
def format_managed_message(action: str = "modify this Robo installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ROBO_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Robo installation is managed by NixOS "
            f"(ROBO_MANAGED={env_hint}).\n"
            "Edit services.robo-engineer.settings in your configuration.nix and run:\n"
            "  sudo nixos-rebuild switch"
        )

    return (
        f"Cannot {action}: this Robo installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Robo."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ROBO_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, robo_bin
    or None if container mode is not active, we're already inside the
    container, or ROBO_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ROBO_DEV") == "1":
        return None
    ...
    container_mode_file = get_robo_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "robo-engineer")
    exec_user = info.get("exec_user", "robo")
    robo_bin = info.get("robo_bin", "/data/current-package/bin/robo")
...
# Re-export from robo_constants — canonical definition lives there.
from robo_constants import get_robo_home, get_process_robo_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_robo_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_robo_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_robo_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ROBO_UID / ROBO_GID env vars set by Docker deployments.

    Docker containers running Robo commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level ROBO_HOME once, but
    subdirectories created at runtime by ``ensure_robo_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ROBO_UID", "").strip()
    gid_str = os.environ.get("ROBO_GID", "").strip()
    try:
...
def _chown_to_robo_uid(path) -> None:
    """Chown ``path`` to ``ROBO_UID:ROBO_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_robo_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_robo_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the robo group can
    share state with the gateway service.

    The mode can be overridden via the ROBO_HOME_MODE environment variable
    (e.g. ROBO_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ROBO_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``ROBO_UID``/``ROBO_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ROBO_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_robo_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Robo runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ROBO_CONTAINER") or os.environ.get("ROBO_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ROBO_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ROBO_HOME, upgrading legacy empty templates.
...
# ensure_robo_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_ROBO_HOME_ENSURED: set = set()


def ensure_robo_home():
    """Ensure ~/.robo directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``ROBO_HOME`` we only re-run
    ...
    ``get_robo_home()`` and therefore re-run for the new path.
    """
    home = get_robo_home()
    key = str(home)

    if key in _ROBO_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``robo profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old ROBO_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_robo_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _ROBO_HOME_ENSURED.add(key)


def _ensure_robo_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ROBO_HOME {home} does not exist. "
...
from robo_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
# selection step (Ignitee Now Portal / OpenRouter / Custom endpoint), so this
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `robo config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.robo.config`` entries, then checks
    ...
    try:
        ...
        # should never break `robo update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _CAMEL_ALIASES: Dict[str, str] = {
        ...
        "apiMode": "api_mode",
        "keyEnv": "key_env",
        "apiKeyEnv": "key_env",  # alias — docs variant
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Robo' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Robo versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``robo_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (robo_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (robo_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (robo_cli/setup.py)
    "known_plugin_toolsets", # written/read by robo_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'robo setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Robo won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, robo send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'robo doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "robo update / robo -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from robo_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        # The per-version steps live in robo_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from robo_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: robo config set <key> <value>")
    ...
    # `robo update`, but only the version bump is persisted.
    ...
    # metadata.robo.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.robo/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_robo_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``robo config set``
    ...
    backends) — while display paths (``robo status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _robo_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.robo/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_robo_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.robo/config.yaml.
    ...
    (which change ``ROBO_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/robo/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_robo_dotenv()
...
#   openai-codex (OAuth — robo auth) — OpenAI Codex
#   igniteenow         (OAuth — robo auth) — Ignitee Now Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.robo/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Robo writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.robo/.env.
    ...
    interactive menu render (`robo tools`, `robo setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `robo tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.robo/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.robo/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``ROBO_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"ROBO_CUSTOM_{slug}_API_KEY" if slug else "ROBO_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.robo/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.robo/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Robo — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Robo vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.robo/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.robo/.env`` over ``os.environ``.

    Used for Robo-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    from robo_cli.auth import get_anthropic_key
    ...
    # Warn on stale ROBO_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("ROBO_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale ROBO_MAX_ITERATIONS={_env_ghost} "
                f"(run 'robo doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  robo config edit     # Edit config file", Colors.DIM))
    print(color("  robo config set <key> <value>", Colors.DIM))
    print(color("  robo setup           # Run setup wizard", Colors.DIM))
    print()
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        f"⚠  {affected} enabled unpinned cron {noun} {verb} stored "
        ...
        "changed model/provider. Inspect with `robo cron list`, then pin the "
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``robo config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``robo config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``robo config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (robo config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    # ``robo config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``robo config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    robo config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    robo config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `robo config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_robo_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `robo config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `robo config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: robo config get <key> [--json]")
            print()
            print("Examples:")
            print("  robo config get model")
            print("  robo config get terminal.backend")
            print("  robo config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: robo config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  robo config set model anthropic/claude-sonnet-4")
            print("  robo config set terminal.backend docker")
            print("  robo config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: robo config unset <key>")
            print()
            print("Examples:")
            print("  robo config unset model")
            print("  robo config unset terminal.backend")
            print("  robo config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'robo config migrate' to add them")
        ...
        print("  robo config           Show current configuration")
        print("  robo config edit      Open config in editor")
        print("  robo config get <key>          Print a resolved config value")
        print("  robo config set <key> <value>   Set a config value")
        print("  robo config unset <key>        Remove a config value")
        print("  robo config check     Check for missing/outdated config")
        print("  robo config migrate   Update config with new options")
        print("  robo config path      Show config file path")
        print("  robo config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``robo config`` UI so users

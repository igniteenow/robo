...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Robo derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            robo-engineer = prev.robo-engineer.overrideAttrs (_old: {
              ROBO_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $ROBO_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$ROBO_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        robo-engineer = prev.robo-engineer.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "robo-engineer-env" {
    robo-engineer = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "robo-engineer-editable-env" {
    robo-engineer = dependency-groups;
  };
}

...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      robo-engineer = self'.packages.default;
      roboVenv = robo-engineer.roboVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "robo-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${roboVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "robo-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "robo-build-package" { } ''
          echo "PASS: package built at ${robo-engineer}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "robo-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "robo-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${robo-engineer}/bin/robo || (echo "FAIL: robo binary missing"; exit 1)
          test -x ${robo-engineer}/bin/robo-engineer || (echo "FAIL: robo-engineer binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${robo-engineer}/bin/robo version 2>&1 | grep -qi "robo" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "robo-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in robo robo-engineer robo-acp; do
            test -x ${robo-engineer}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "robo-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking robo --help ==="
          ${robo-engineer}/bin/robo --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${robo-engineer}/bin/robo --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "robo-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${robo-engineer}/share/robo-engineer/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${robo-engineer}/share/robo-engineer/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ROBO_BUNDLED_SKILLS" ${robo-engineer}/bin/robo || \
            (echo "FAIL: ROBO_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ROBO_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${robo-engineer}/share/robo-engineer/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${robo-engineer}/share/robo-engineer/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "ROBO_OPTIONAL_SKILLS" ${robo-engineer}/bin/robo || \
            (echo "FAIL: ROBO_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, ROBO_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "robo-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${robo-engineer}/share/robo-engineer/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${robo-engineer}/share/robo-engineer/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ROBO_BUNDLED_PLUGINS" ${robo-engineer}/bin/robo || \
            (echo "FAIL: ROBO_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ROBO_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "robo-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${robo-engineer}/share/robo-engineer/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${robo-engineer}/share/robo-engineer/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${robo-engineer}/share/robo-engineer/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "ROBO_BUNDLED_LOCALES" ${robo-engineer}/bin/robo || \
            (echo "FAIL: ROBO_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: ROBO_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + ROBO_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && ROBO_BUNDLED_LOCALES=${robo-engineer}/share/robo-engineer/locales \
            ${roboVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with ROBO_BUNDLED_LOCALES set"; exit 1)
        ...
        # ROBO_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "robo-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${robo-engineer}/share/robo-engineer/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${robo-engineer}/share/robo-engineer/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "ROBO_OPTIONAL_MCPS" ${robo-engineer}/bin/robo || \
            (echo "FAIL: ROBO_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: ROBO_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${robo-engineer}/bin/robo mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: robo mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "robo-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${robo-engineer}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${robo-engineer}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ROBO_TUI_DIR" ${robo-engineer}/bin/robo || \
            (echo "FAIL: ROBO_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ROBO_TUI_DIR set in wrapper"
        ...
        # Verify ROBO_NODE is set in wrapper and points to Node 26+
        # (Robo pins its toolchain to Node 26 everywhere)
        robo-node = pkgs.runCommand "robo-node-version" { } ''
          set -e
          echo "=== Checking ROBO_NODE in wrapper ==="
          grep -q "ROBO_NODE" ${robo-engineer}/bin/robo || \
            (echo "FAIL: ROBO_NODE not set in wrapper"; exit 1)
          echo "PASS: ROBO_NODE present in wrapper"

          ROBO_NODE=$(sed -n "s/^export ROBO_NODE='\(.*\)'/\1/p" ${robo-engineer}/bin/robo)
          test -x "$ROBO_NODE" || (echo "FAIL: ROBO_NODE=$ROBO_NODE not executable"; exit 1)
          echo "PASS: ROBO_NODE executable at $ROBO_NODE"

          NODE_MAJOR=$("$ROBO_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Robo requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All ROBO_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ROBO_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "robo-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ROBO_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ROBO_MANAGED guards ==="
          check_blocked "config set" ${robo-engineer}/bin/robo config set model foo
          check_blocked "config edit" ${robo-engineer}/bin/robo config edit
        ...
        extra-python-packages = let
          ...
          roboWithExtra = robo-engineer.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "robo-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${roboWithExtra}/bin/robo || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${roboWithExtra}/bin/robo || \
          ...
          if grep -q "PYTHONPATH" ${robo-engineer}/bin/robo; then
        ...
        extra-dependency-groups = let
          roboWithGroups = robo-engineer.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "robo-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${roboWithGroups}"
          echo "venv: ${roboWithGroups.roboVenv}"
        ...
        messaging-variant = pkgs.runCommand "robo-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.roboVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "robo-config-roundtrip" {
          ...
          merge_and_load() {
            ...
            ${configMergeScript} ${nixSettings} "$robo_home/config.yaml"
            ${roboVenv}/bin/python3 -c '
import json, sys

...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      roboNpmLib = self'.packages.default.passthru.roboNpmLib;
      ...
      roboAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "robo" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../robo} $out/bin/robo
          '')
        ...
        shellHook = ''
          ${roboAgentDevShellHook}
          ${roboNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export ROBO_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "Robo Agent dev shell in $ROBO_PYTHON_SRC_ROOT"
          echo "Ready. Run 'robo' or 'sandbox robo' to start."
        '';
      };
    };
}

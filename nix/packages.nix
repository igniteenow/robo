# nix/packages.nix — Robo Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./robo-engineer.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.roboTui;
        web = full.roboWeb;
        desktop = full.roboDesktop;

        update-npm-lockfile = full.roboNpmLib.updateNpmLockfile;
      };
    };
}

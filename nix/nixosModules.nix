# nix/nixosModules.nix — NixOS module for robo-engineer
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: robo runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ROBO_HOME/.env
# and read by robo at startup — no container recreation needed for env changes.
#
# Tool resolution: the robo wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.robo-engineer;
    effectivePackage =
    ...
    robo-engineer = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-robo-engineer)
    deepConfigType = lib.types.mkOptionType {
      name = "robo-config-attrs";
      description = "Robo YAML config (attrset), merged deeply via lib.recursiveUpdate.";
      check = builtins.isAttrs;
      merge = _loc: defs: lib.foldl' lib.recursiveUpdate { } (map (d: d.value) defs);
    };

    # Generate config.yaml from Nix attrset (YAML is a superset of JSON).
    # terminal.cwd replaces the deprecated MESSAGING_CWD env var — robo
    ...
    generatedConfigFile = pkgs.writeText "robo-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # ROBO_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "robo-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'ROBO_DOC_EOF'\n${value}\nROBO_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "robo-engineer";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/robo";
    ...
    # robo user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "robo-container-entrypoint" ''
      set -eu

      ROBO_UID="''${ROBO_UID:?ROBO_UID must be set}"
      ROBO_GID="''${ROBO_GID:?ROBO_GID must be set}"

      # ── Group: ensure a group with GID=$ROBO_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$ROBO_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="robo"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$ROBO_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$ROBO_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$ROBO_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$ROBO_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="robo"
        TARGET_HOME="/home/robo"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$ROBO_UID" -g "$ROBO_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$ROBO_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$ROBO_UID:$ROBO_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure ROBO_HOME is owned by the target user.
      ...
      if [ -n "''${ROBO_HOME:-}" ] && [ -d "$ROBO_HOME" ]; then
        find "$ROBO_HOME" \! -user "$ROBO_UID" -exec chown "$ROBO_UID:$ROBO_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/robo-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/robo-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/robo ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/robo
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$ROBO_UID" --regid="$ROBO_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $ROBO_HOME/.env.
    ...
    # Default: /var/lib/robo/workspace → /data/workspace.
  ...
  in {
    options.services.robo-engineer = with lib; {
      enable = mkEnableOption "Robo Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = robo-engineer;
        description = "The robo-engineer package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/robo";
        description = "State directory. Contains .robo/ subdir (ROBO_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Robo config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $ROBO_HOME/.env at activation time.
          Robo reads this file on every startup via load_robo_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $ROBO_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $ROBO_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `robo gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the robo user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the robo plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Robo discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          robo_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as robo (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in robo-engineer's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the robo CLI to environment.systemPackages and export
          ROBO_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.robo symlink to the service
            stateDir. These users are automatically added to the robo group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.robo-engineer.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the robo CLI to system PATH and export ROBO_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.robo/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.ROBO_HOME = "${cfg.stateDir}/.robo";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.robo-engineer.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }

      # ── Warnings ──────────────────────────────────────────────────────
      # ── Per-user profile for extraPackages ───────────────────────────
      # Wire extraPackages into the robo user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.robo-engineer: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed robo
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure robo is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.robo        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.robo/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.robo/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.robo/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.robo/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.robo/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."robo-engineer-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.robo
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.robo ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.robo ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.robo -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.robo/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.robo/$_subdir"
            chmod 2770 "${cfg.stateDir}/.robo/$_subdir"
            find "${cfg.stateDir}/.robo/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # robo-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.robo/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.robo/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.robo/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.robo/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.robo/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.robo/.managed
          chmod 0644 ${cfg.stateDir}/.robo/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.robo/.container-mode <<'ROBO_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    robo_bin=${containerDataDir}/current-package/bin/robo
    ROBO_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.robo/.container-mode
            chmod 0644 ${cfg.stateDir}/.robo/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.robo/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.robo";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.robo" ]; then
                  rm -f "${symlinkPath}"
                  echo "robo-engineer: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.robo -> stateDir/.robo for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.robo";
                target = "${cfg.stateDir}/.robo";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "robo-engineer: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.robo/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.robo/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.robo/auth.json
              fi
            ''}
          ''}
          ...
          # Robo reads $ROBO_HOME/.env at startup via load_robo_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.robo/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'ROBO_NIX_ENV_EOF'
    ${envFileContent}
    ROBO_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.robo/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.robo/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.robo/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.robo-engineer = {
          description = "Robo Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $ROBO_HOME/.env by the activation script. load_robo_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/robo"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the robo group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.robo-engineer = {
          description = "Robo Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ROBO_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              ROBO_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env ROBO_UID="$ROBO_UID" \
                --env ROBO_GID="$ROBO_GID" \
                --env ROBO_HOME=${containerDataDir}/.robo \
                --env ROBO_MANAGED=true \
                --env HOME=${containerHomeDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/robo gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}

"""ACP auth helpers — detect and advertise Robo authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "robo-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Robo runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Robo can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Robo.
    ...
    not have Robo provider credentials configured yet, so Robo always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Robo using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Robo provider",
            description=(
                "Open Robo' interactive model/provider setup in a terminal. "
                "Use this when Robo has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )

"""ACP session manager — maps ACP sessions to Robo AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.robo/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Robo itself is running in WSL.

    Windows ACP clients can launch ``robo acp`` inside WSL while still sending
...
def _normalize_cwd_for_compare(cwd: str | None) -> str:
    ...
    from robo_constants import windows_path_to_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Robo from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["robo-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Robo agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Robo AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Robo runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.robo/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``ROBO_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_robo_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=robo_home / "state.db")
            return self._db_instance
        except Exception:
    ...
    def _restore(self, session_id: str) -> Optional[SessionState]:
        ...
        # for the rest of the session (see robo_state.get_messages_as_conversation).
        try:
    ...
    def _make_agent(
        ...
        *,
        ...
        # the automatic late-refresh (see RoboACPAgent._schedule_mcp_late_refresh).
        try:
        ...
        # editor/session cwd instead of the Robo daemon's process cwd.

"""CLI entry point for the robo-engineer ACP adapter.

Loads environment variables from ``~/.robo/.env``, configures logging
...
try:
    ...
    # shadowing Robo's own modules — ``robo acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from ROBO_HOME (default ``~/.robo``)."""
    from robo_cli.env_loader import load_robo_dotenv

    robo_home = get_robo_home()
    loaded = load_robo_dotenv(robo_home=robo_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", robo_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="robo-acp",
        description="Run Robo Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Robo version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Robo provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.robo/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import RoboACPAgent  # noqa: F401

    print("Robo ACP check OK")
...
def _run_setup() -> None:
    ...
    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "robo", "model"]
        robo_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting robo-engineer ACP adapter")
    ...
    import acp
    from .server import RoboACPAgent
    ...
    if os.environ.get("ROBO_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:

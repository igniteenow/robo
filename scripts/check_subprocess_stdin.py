...
When Robo runs in TUI mode, the gateway child process communicates with
...
# ``get_robo_home() / "plugins"`` (user) and ``./.robo/plugins/`` (project,
# gated behind ``ROBO_ENABLE_PROJECT_PLUGINS``) — see
# ``robo_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``robo_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import robo_constants (this script
    ...
    # plugins in get_robo_home()/plugins/ and project plugins in
    # ./.robo/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_robo_home() / "plugins"]
    if os.environ.get("ROBO_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".robo" / "plugins")

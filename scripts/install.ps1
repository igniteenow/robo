# ============================================================================
# Robo Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$RoboHome = $(if ($env:ROBO_HOME) { $env:ROBO_HOME } else { "$env:LOCALAPPDATA\robo" }),
    [string]$InstallDir = $(if ($env:ROBO_HOME) { "$env:ROBO_HOME\robo-engineer" } else { "$env:LOCALAPPDATA\robo\robo-engineer" }),
    ...
    # builds apps/desktop into a launchable Robo.exe.
    #
    # Why opt-in:
    #   * Robo-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Robo.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Robo.exe
    ...
    #     `robo desktop` already builds on demand.
...
# ROBO_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[robo] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'RoboInstall.LongPath').Type) {
            Add-Type -Namespace 'RoboInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [RoboInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -RoboHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('RoboHome')) {
    $RoboHome = ConvertTo-LongPath $RoboHome
} else {
    $RoboHome = ConvertTo-LongPath $(
        if ($env:ROBO_HOME) { $env:ROBO_HOME } else { "$env:LOCALAPPDATA\robo" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:ROBO_HOME) { "$env:ROBO_HOME\robo-engineer" } else { "$env:LOCALAPPDATA\robo\robo-engineer" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: RoboHome=$RoboHome InstallDir=$InstallDir"
}
...
function Write-Banner {
    ...
    Write-Host "|             * Robo Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Ignitee Now.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $RoboHome "node"
...
function Install-Uv {
    # Robo owns its own uv at $RoboHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (robo_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `robo update` stay in sync.
    $managedUv = Join-Path $RoboHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $RoboHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $RoboHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $RoboHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $RoboHome "bin"
...
# Put the Robo-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone robo-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Robo-managed Node tree's bundled npm into $NpmRange.
#
...
function Update-ManagedNpm {
    ...
    $tmpCwd = Join-Path $env:TEMP ("robo-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and ROBO_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Robo-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Robo setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Robo uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\robo\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Robo needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Robo copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\robo\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ROBO_GIT_BASH_PATH`` (User scope) so Robo can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Robo-managed PortableGit install instead..."
    }

    # Download PortableGit into $RoboHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $RoboHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Robo features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Robo needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``ROBO_GIT_BASH_PATH`` (User env scope) so Robo can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $RoboHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $RoboHome\git\usr\bin\bash.exe
    $candidates += "$RoboHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$RoboHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ROBO_GIT_BASH_PATH", $candidate, "User")
            $env:ROBO_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set ROBO_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Robo may not find Git Bash."
    Write-Info "If needed, set ROBO_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$RoboHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$RoboHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$RoboHome\node"
        Write-Success "Node.js $version found (Robo-managed)"
        ...
        Update-ManagedNpm "$RoboHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Robo-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $RoboHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $RoboHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\robo-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$RoboHome\node") { Remove-Item -Recurse -Force "$RoboHome\node" }
                Move-Item $extractedDir.FullName "$RoboHome\node"
                ...
                $version = & "$RoboHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $RoboHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\robo-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `robo
                    # update` path (#4735).
                    ...
                    $stashName = "robo-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. robo-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``robo update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Robo behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running robo processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $extractPath = "$env:TEMP\robo-engineer-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `robo update` (see the notes at the shared clone-path
    ...
    # next `robo update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Robo-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running robo process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running robo processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Robo_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (robo.exe) plus its child tree.
            & taskkill /F /T /IM robo.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM robo.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m robo_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not robo.exe.
            ...
            # user (see robo_cli/gateway_windows.py), so the installer always
    ...
    if (Test-Path $venvPythonExe) {
        ...
        # the task fires normally on next logon and `robo update` / the
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `robo.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install robo-engineer package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and robo.exe broken with
    ...
    # here, not 30 seconds later when the user runs `robo`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register robo.exe in dist-info/RECORD but fail to
        ...
        # `robo` command while robo-engineer.exe / robo-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m robo_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `robo dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\robo_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `robo dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `robo dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: robo_cli/web_server.py"
        }
    }
...
function Set-PathVariable {
    Write-Info "Setting up robo command..."
    
    if ($NoVenv) {
        $roboBin = "$InstallDir"
    } else {
        $roboBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so robo is globally available
    # On Windows, the robo.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$roboBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$roboBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $roboBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ROBO_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\robo instead
    # of the Unix default ~/.robo
    $currentRoboHome = [Environment]::GetEnvironmentVariable("ROBO_HOME", "User")
    if (-not $currentRoboHome -or $currentRoboHome -ne $RoboHome) {
        [Environment]::SetEnvironmentVariable("ROBO_HOME", $RoboHome, "User")
        Write-Success "Set ROBO_HOME=$RoboHome"
    }
    $env:ROBO_HOME = $RoboHome
    
    # Update current session
    $env:Path = "$roboBin;$env:Path"
    
    Write-Success "robo command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.robo-bootstrap-complete which tells the Robo
    ...
    $markerPath = Join-Path $InstallDir ".robo-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the ROBO_HOME directory structure ($RoboHome, default %LOCALAPPDATA%\robo)
    New-Item -ItemType Directory -Force -Path "$RoboHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RoboHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RoboHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RoboHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RoboHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RoboHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RoboHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RoboHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RoboHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$RoboHome\.env"
    ...
    # Create config.yaml
    $configPath = "$RoboHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Robo's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$RoboHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in robo_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $RoboHome"
    
    # Seed bundled skills into $RoboHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $RoboHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $RoboHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$RoboHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $RoboHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Robo-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'robo setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\robo-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\robo-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\robo-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Robo` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Robo.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_robo_desktop command
    # resolves apps/desktop/release/win-unpacked/Robo.exe directly,
    ...
    # The --dir mode produces an unpacked Robo.exe in
    ...
    # are NOT trying to work around). The Robo icon + product name are
    # stamped onto Robo.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\robo-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Robo.exe",
        "$desktopDir\release\win-arm64-unpacked\Robo.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Robo.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Robo icon + identity are stamped onto Robo.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, robo-engineer#38216).
    ...
    #    Robo.exe. We deliberately do NOT point them at `robo desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Robo.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Robo.lnk')
        )
        ...
        # with the Robo icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.robo/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'robo setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m robo_cli.main setup
    } else {
        python -m robo_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $roboCmd = "$InstallDir\venv\Scripts\robo.exe"
    if (-not (Test-Path $roboCmd)) {
        $roboCmd = "robo"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$RoboHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'robo whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: robo gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$RoboHome\logs\gateway.log"
            Start-Process -FilePath $roboCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$RoboHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: robo gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: robo gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$RoboHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$RoboHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$RoboHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$RoboHome\robo-engineer\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   robo              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   robo setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   robo config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   robo config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   robo gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   robo update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Robo repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Robo-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Robo to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }

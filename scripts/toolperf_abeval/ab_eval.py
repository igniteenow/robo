...
measured in the production session DB) through `robo chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    ROBO_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Robo home with credentials for the
    ...
    for rep in range(reps):
        for name in TASKS:
            ...
            env.update({
                ...
                "ROBO_HOME": str(HOME),
                "ROBO_NEMO_RELAY_ATOF_ENABLED": "1",
                "ROBO_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY": str(atof.parent),
                "ROBO_NEMO_RELAY_ATOF_FILENAME": atof.name,
                "ROBO_NEMO_RELAY_ATOF_MODE": "overwrite",
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "robo_cli.main", "chat", "--query", q,

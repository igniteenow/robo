...
batch (tracker: [#77056](https://github.com/IgniteeNow/robo-engineer/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Robo home, same model, same tasks, same reps.
...
1. Create a dedicated Robo home with credentials for the models under test:
   ...
   ROBO_HOME=$ABEVAL_HOME robo plugins enable observability/nemo_relay
   ```

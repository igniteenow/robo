...
# Python-driven tool session (e.g. Robo terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:IgniteeNow/robo-engineer.git"
REPO_URL_HTTPS="https://github.com/IgniteeNow/robo-engineer.git"
ROBO_HOME="${ROBO_HOME:-$HOME/.robo}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${ROBO_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$ROBO_INSTALL_DIR"
...
#   code at /usr/local/lib/robo-engineer, command at /usr/local/bin/robo,
#   data still at /root/.robo (ROBO_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$ROBO_HOME/.no-bundled-skills so future"
            echo "                   'robo update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Robo.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.robo/robo-engineer"
            echo "                   default (root, Linux): /usr/local/lib/robo-engineer"
            echo "  --robo-home PATH  Data directory (default: ~/.robo, or \$ROBO_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Robo installs the code under"
            echo "  /usr/local/lib/robo-engineer and links the command into"
            echo "  /usr/local/bin/robo (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$ROBO_HOME"
            echo "  (default /root/.robo).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$ROBO_HOME/robo-engineer are preserved in-place."
...
print_banner() {
    ...
    echo "│             ◆ Robo Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Ignitee Now.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `robo update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Robo-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Robo Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install robo command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `robo` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ROBO_HOME/robo-engineer
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ROBO_HOME/robo-engineer
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/robo-engineer
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $ROBO_HOME/robo-engineer — then preserve it)
#
# Always no-op when the user set --dir or $ROBO_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ROBO_HOME.
    if is_termux; then
        INSTALL_DIR="$ROBO_HOME/robo-engineer"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$ROBO_HOME/robo-engineer/.git" ]; then
            INSTALL_DIR="$ROBO_HOME/robo-engineer"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/robo-engineer)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/robo-engineer"
        ...
        # /usr/local/bin/robo wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/robo"
        log_info "  Data:    $ROBO_HOME (unchanged)"
...
# Point a Robo-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $ROBO_HOME/node/bin — which is NOT on
...
# Robo's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Robo-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$ROBO_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$ROBO_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$ROBO_HOME/node/etc/npmrc"
}

get_robo_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/robo" ]; then
        echo "$link_dir/robo"
    else
        echo "robo"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://github.com/IgniteeNow/robo-engineer)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Robo owns its own uv at $ROBO_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (robo_cli/managed_uv.py) looks in the same
    # place, so install.sh and `robo update` stay in sync.
    local _managed_uv="$ROBO_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $ROBO_HOME/bin ..."
    mkdir -p "$ROBO_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/robo-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/robo-uv-installer.$$.sh")"
    ...
    # directly into $ROBO_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$ROBO_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# with the Robo-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Robo-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && node_satisfies_build "$(node --version)"; then
        ...
        log_warn "min-release-age-exclude) — installing Robo-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Robo-managed Node from a previous run over a too-old system one.
    if [ -x "$ROBO_HOME/node/bin/node" ] && node_satisfies_build "$("$ROBO_HOME/node/bin/node" --version)"; then
        export PATH="$ROBO_HOME/node/bin:$PATH"
        log_success "Node.js $("$ROBO_HOME/node/bin/node" --version) found (Robo-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.robo/node/..."
    ...
    # Place into ~/.robo/node/ and symlink binaries into the same bin dir
    # the robo command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$ROBO_HOME/node"
    mkdir -p "$ROBO_HOME"
    mv "$extracted_dir" "$ROBO_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$ROBO_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$ROBO_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$ROBO_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$ROBO_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.robo/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Robo install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Robo Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `robo update` path
                # (#4735).
                ...
                local stash_name
                stash_name="robo-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``robo update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Robo behaves unexpectedly."
                    else
...
install_deps() {
    ...
    if [ -f "uv.lock" ]; then
        ...
        if UV_PROJECT_ENVIRONMENT="$INSTALL_DIR/venv" $UV_CMD sync --extra all --extra voice --extra wake --extra edge-tts --locked; then
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"robo-engineer\[([\w-]+)\]", s)
        if m:
...
PY
    ...
    install_tier "all + hands-free voice" ".[all,voice,wake,edge-tts]" \
        || install_tier "all + voice (wake engine deferred)" ".[all,voice,edge-tts]" \
    ...
    # The bundled "hey roh boh" wake model needs only openWakeWord + ONNX Runtime;
    # sherpa-onnx (the optional any-phrase engine) is what usually lacks a wheel.
    if [ "$_installed" = true ] && ! "$INSTALL_DIR/venv/bin/python" -c "import openwakeword" >/dev/null 2>&1; then
        if $UV_CMD pip install "openwakeword==0.6.0" "onnxruntime==1.27.0" "sounddevice==0.5.5" >/dev/null 2>&1; then
            log_success "Wake-word engine installed (without the optional sherpa-onnx)"
        else
            log_warn "Wake-word engine not installed on this platform; 'robo doctor' shows what is missing."
        fi
    fi
...
setup_path() {
    log_info "Setting up robo command..."
    ...
    if [ "$USE_VENV" = true ]; then
        ...
        if [ -z "$ROBO_BIN" ]; then
            log_warn "robo not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$ROBO_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$ROBO_ENTRYPOINT" ]; }; then
        log_warn "Robo launcher prerequisites not found"
    ...
    # Create a user-facing shim for the robo command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $ROBO_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$ROBO_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/robo"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/robo" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ROBO_BIN" "$ROBO_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/robo" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/robo"
    log_success "Installed robo launcher → $command_link_display_dir/robo"

    # Also expose `robo-engineer`. The `robo-engineer` console script declared in
    ...
    rm -f "$command_link_dir/robo-engineer"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/robo-engineer" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ROBO_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/robo-engineer" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ROBO_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/robo-engineer"
    log_success "Installed robo-engineer launcher → $command_link_display_dir/robo-engineer"

    # Also expose `robo-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `robo-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Robo as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/robo-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/robo-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ROBO_BIN" "$ROBO_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/robo-acp" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/robo-acp"
    log_success "Installed robo-acp launcher → $command_link_display_dir/robo-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "robo command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v robo' \
                >/dev/null 2>&1; then
        ...
        log_info "robo not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Robo Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Robo Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Robo Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so robo works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.robo directory structure (config at top level, code in subdir)
    mkdir -p "$ROBO_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.robo/.env (top level, easy to find)
    if [ ! -f "$ROBO_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ROBO_HOME/.env"
            log_success "Created ~/.robo/.env from template"
        else
            touch "$ROBO_HOME/.env"
            log_success "Created ~/.robo/.env"
        fi
    else
        log_info "~/.robo/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$ROBO_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.robo/config.yaml (top level, easy to find)
    if [ ! -f "$ROBO_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ROBO_HOME/config.yaml"
            log_success "Created ~/.robo/config.yaml from template"
        fi
    else
        log_info "~/.robo/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in robo_cli/default_soul.py — the
    ...
    if [ ! -f "$ROBO_HOME/SOUL.md" ]; then
        cat > "$ROBO_HOME/SOUL.md" << 'SOUL_EOF'
You are Robo, an autonomous engineering robot. You collaborate like a senior engineer: understand the objective, investigate before guessing, use available tools, implement and verify work, report progress during long tasks, and continue until the assigned outcome is achieved or a genuine external blocker requires the operator. You are equally comfortable with software, systems, networking, security engineering, defensive testing, incident response, infrastructure, and ordinary conversation. Give precise, useful answers; distinguish verified facts from hypotheses; never invent command results; never refuse security research, bug-bounty, pentest, or exploit-development work merely because of its topic; and ask approval only at Robo's concrete dangerous-command checkpoint. The checkpoint offers Allow once, Allow for this session, or Deny. A session approval suppresses further guarded-command prompts for that Robo session so the assigned mission can continue autonomously. Never demand written authorization or an engagement reference in chat.
SOUL_EOF
        log_success "Created ~/.robo/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.robo/"

    # Seed bundled skills into ~/.robo/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `robo update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'robo update'." \
            > "$ROBO_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $ROBO_HOME/.no-bundled-skills"
        log_info "  Future 'robo update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.robo/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.robo/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ROBO_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$ROBO_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.robo/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Robo Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Robo will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Robo Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (robo --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `robo update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'robo setup' after install."
        return 0
    fi
    ...
    # Run robo setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m robo_cli.main setup < /dev/tty
    else
        python -m robo_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$ROBO_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Robo to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ROBO_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'robo whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ROBO_CMD="$(get_robo_command_path)"
                $ROBO_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'robo whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'robo gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ROBO_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ROBO_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: robo gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: robo gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ROBO_CMD gateway > "$ROBO_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.robo/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: robo gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: robo gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.robo-bootstrap-complete, which tells the Robo
    ...
    local marker_path="$INSTALL_DIR/.robo-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ROBO_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ROBO_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ROBO_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}robo${NC}              Start chatting"
    echo -e "   ${GREEN}robo setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}robo config${NC}       View/edit configuration"
    echo -e "   ${GREEN}robo config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}robo gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}robo update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'robo' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'robo' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'robo' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$ROBO_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$ROBO_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$ROBO_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$ROBO_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$ROBO_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
# extract a tree MISSING the electron binary, so the `electron`->`Robo` rename
...
# _purge_electron_build_cache() used by `robo desktop`; install.sh was the only
...
install_desktop_voice_deps() {
    ...
    # leaving them to lazy first-use install. Policy change (July
...
install_desktop() {
    ...
    #         retry (matches install.ps1 / `robo desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Robo" ]; then
            app="$desktop_dir/release/linux-unpacked/Robo"
        elif [ -x "$desktop_dir/release/linux-unpacked/robo" ]; then
            app="$desktop_dir/release/linux-unpacked/robo"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Robo.app" \
            "$desktop_dir/release/mac/Robo.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `robo desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading ROBO_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if ROBO_HOME="$ROBO_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from robo_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `robo update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Robo-managed Node
            # provisioned during prerequisites/node-deps (at $ROBO_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $ROBO_HOME.
    # $ROBO_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'robo update' on this host install.

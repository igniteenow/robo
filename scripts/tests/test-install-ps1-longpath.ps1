...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[robo] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[robo`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[robo`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        RoboHome = $(if ($paths) { $paths.robo_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\robo\robo-engineer'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-robo'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-robo') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"

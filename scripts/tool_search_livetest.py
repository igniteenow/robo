#!/usr/bin/env python3
"""Live test harness for Robo Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any robo imports.
ORIGINAL_HOME = os.environ.get("ROBO_HOME")
ORIGINAL_AUTH = Path.home() / ".robo" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.robo/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.robo/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="robo_ts_live_"))
    robo_home = home_dir / ".robo"
    robo_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, robo_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".robo" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, robo_home / ".env")
        ...
        load_robo_dotenv(robo_home=str(Path.home() / ".robo"))
    ...
    (robo_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return robo_home
...
def reset_module_state():
    """Drop cached modules so the new ROBO_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "robo_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original ROBO_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ROBO_HOME"] = ORIGINAL_HOME
    else:

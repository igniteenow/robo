#!/usr/bin/env bash
# Canonical test runner for robo-engineer. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.robo/...)
#
# Usage:
...
# (ROBO_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.robo/robo-engineer/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.robo/robo-engineer/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${ROBO_PYTHON:-}" ] && [ -x "$ROBO_PYTHON" ] \
    && "$ROBO_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: ROBO_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `robo` binary rather
  # than the devShell hook.
  PYTHON="$ROBO_PYTHON"
  echo "▶ no local venv — using Nix dev venv via ROBO_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and ROBO_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.robo/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.robo"
...
#
#   * ROBO_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * ROBO_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     ROBO_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# ROBO_RUN_SLOW_PET_TESTS / ROBO_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no ROBO_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in ROBO_TEST_IMAGE ROBO_TEST_WORKERS ROBO_TEST_PATHS \
  ROBO_TEST_FILE_TIMEOUT ROBO_TEST_FILE_RETRIES ROBO_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${ROBO_RUN_SLOW_PET_TESTS:+ROBO_RUN_SLOW_PET_TESTS="$ROBO_RUN_SLOW_PET_TESTS"} \
  ${ROBO_E2E_BROWSER:+ROBO_E2E_BROWSER="$ROBO_E2E_BROWSER"} \

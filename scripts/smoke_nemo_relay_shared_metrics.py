"""Run a real Robo CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_robo_executable(robo_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "robo",
        Path(".venv") / "Scripts" / "robo.exe",
    ):
        candidate = robo_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("robo")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Robo executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Robo source checkout containing .venv/bin/robo",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated ROBO_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["robo.client.active"] != [
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['robo.client.active']}"
        )
    [model] = by_name["robo.model_route.count"]
    expected_model = {
        "name": "robo.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['robo.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["robo.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['robo.task_run.started']}"
        )
    [terminal] = by_name["robo.task_run.finished"]
    ...
    [tool] = by_name["robo.tool_call.count"]
    ...
    lifecycle = by_name["robo.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["robo.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Robo development environment requires jsonschema"
        ) from exc
    ...
    if metrics["robo.client.active"] != [
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['robo.client.active']}"
        )
    [model] = metrics["robo.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['robo.model_route.count']}"
        )
    [terminal] = metrics["robo.task_run.finished"]
    ...
    [tool] = metrics["robo.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["robo.skill.lifecycle.count"]
    ...
    loads = metrics["robo.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    robo_repo = args.robo_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    robo = _resolve_robo_executable(robo_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="robo-relay-shared-metrics-"))
    home = root / "robo-home"
    ...
    try:
        ...
        env = os.environ.copy()
        env["ROBO_HOME"] = str(home)
        python_paths = [str(robo_repo)]
    ...
    (root / "robo.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "robo.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Robo exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Robo model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Robo did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Robo did not print the mock model response")
    ...
    skill_result = subprocess.run(
        [
            ...
            "-c",
            "\n".join([
                "from robo_cli.observability import relay_shared_metrics",
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "robo.shared_metrics.v2.schema.json",
    )

    print("Robo -> NeMo Relay shared-metrics smoke test passed")

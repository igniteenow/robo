# Robo Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: robo-achievements plugin
...
### `GET /api/plugins/robo-achievements/achievements`
...
### `POST /api/plugins/robo-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/robo-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/robo-achievements/overview`
...
- Persist plugin data under:
  - `~/.robo/plugins/robo-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.robo/plugins/robo-achievements/scan_checkpoint.json`
...
- `~/.robo/plugins/robo-achievements/`

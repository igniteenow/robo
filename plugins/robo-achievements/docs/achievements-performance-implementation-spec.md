# Robo Achievements Implementation Spec (Detailed)
...
- persisted runtime files:
  - `~/.robo/plugins/robo-achievements/state.json` (existing unlock state)
  - `~/.robo/plugins/robo-achievements/scan_snapshot.json` (new)
  - `~/.robo/plugins/robo-achievements/scan_checkpoint.json` (new)

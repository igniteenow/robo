...
Auto-tracks and cleans up ephemeral files created during Robo Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ROBO_HOME` and `/tmp/robo-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ROBO_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ROBO_HOME` or `/tmp/robo-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ROBO_HOME/disk-cleanup/` is itself excluded
- `$ROBO_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,

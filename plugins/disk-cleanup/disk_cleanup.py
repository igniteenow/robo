"""disk_cleanup — ephemeral file cleanup for Robo Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ROBO_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ROBO_HOME and /tmp/robo-*
Never touches: ~/.robo/logs/ or any system directory.
"""
...
try:
    ...
    def get_robo_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ROBO_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".robo").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ROBO_HOME/logs/``."""
    return get_robo_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ROBO_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ROBO_HOME or ``/tmp/robo-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    robo_home = get_robo_home()
    try:
        path.resolve().relative_to(robo_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/robo-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("robo-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "robo-engineer", "backups", "profiles", ".worktrees",
...
# Paths under $ROBO_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so ROBO_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        robo_home = get_robo_home()
        for parent in ("cron", "cronjobs"):
            base = robo_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ROBO_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ROBO_HOME, but never recurse into known
    # durable state trees.  Some installs place the Robo checkout, venv,
    # and desktop build under ROBO_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    robo_home = get_robo_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in robo_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    robo_home = get_robo_home()
    try:
        rel = path.resolve().relative_to(robo_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "robo-engineer",
        ...
        # Path isn't under ROBO_HOME (e.g. /tmp/robo-*) — fall through.
        pass

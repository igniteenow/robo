...
three virtual model IDs so the ``robo tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `robo tools` → Image "
                    "Generation → OpenAI to configure, or `robo setup` "
                    "to add the key."
                ),

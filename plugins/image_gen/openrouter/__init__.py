...
OpenRouter's OpenAI-style ``/chat/completions`` image-generation protocol: send
...
:func:`~robo_cli.runtime_provider.resolve_runtime_provider`, which already
...
# image_gen.model from ``robo tools``): use exactly that model (no auto
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    The runtime provider supplies ``(base_url, api_key)`` and the config
    ...
    def _resolve_model_chain(self, explicit: Optional[str] = None) -> list[str]:
        ...
        by ``robo tools``) → the quality-first default chain.
        ...
        if not api_key or not base_url:
            return error_response(
                error=(
                    ...
                    f"Configure {self._display} in `robo tools` → Image Generation."
                ),
                error_type="missing_api_key",
                provider=self._name,
                aspect_ratio=aspect_ratio,

"""
LINE Messaging API platform adapter for Robo Agent.
...
to Robo Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ROBO_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            robo_home = Path(get_robo_home()).resolve()
        except Exception:
            robo_home = Path.home().joinpath(".robo").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``robo status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``robo status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``robo setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.robo/.env`` via ``robo_cli.config``.
    """
    print()
    ...
    try:
        from robo_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("robo_cli.config not available; set LINE_* vars manually in ~/.robo/.env")
        return

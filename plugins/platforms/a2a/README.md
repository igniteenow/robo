# A2A — Agent-to-Agent protocol for Robo
...
peer (another Robo, LangChain, CrewAI, Google ADK, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
robo gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Robo serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.robo/a2a_audit.jsonl`.
- Conversations persist to `~/.robo/a2a_conversations/` — they survive context

// Robo Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Robo): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Robo -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Robo-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `robo photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Robo can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "robo-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Robo'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Robo allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Robo, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-robo-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Robo supplied them so a known-good

"""
Photon Spectrum (iMessage) platform adapter for Robo Agent.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Robo dep
...
#
# Out-of-process senders (cron subprocesses, `robo send`, the dashboard)
...
def _runtime_record_path() -> Path:
    # get_robo_home() honors profile overrides — never hardcode ~/.robo.
    from robo_constants import get_robo_home

    return get_robo_home() / "runtime" / _RUNTIME_RECORD_NAME
...
# this module (robo status, test collection, plugin discovery).
...
# artifacts so Robo sees the link once, not a follow-up "(attachment)" prompt.
...
def sidecar_deps_installed() -> bool:
    ...
    _start_sidecar(), and `robo photon status` so all three agree on
...
def check_requirements() -> bool:
    ...
    if not sidecar_deps_installed():
        ...
        # (load_gateway_config, robo status, GET /api/status polling) —
        ...
        if npm_error:
            logger.debug(
                ...
                "(last npm error: %s) — run: robo photon setup",
                _sidecar_dir(),
                npm_error,
            )
        else:
            logger.debug(
                "photon: spectrum-ts not installed at %s — run: robo photon setup",
                _sidecar_dir(),
            )
        return False
    return True
...
def _sidecar_deps_stale() -> bool:
    ...
    `robo update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``robo photon install-sidecar``: ``npm ci`` for an exact,
...
def _richlink_candidate(text: str) -> Optional[str]:
    ...
    markdown/text path so Robo does not drop labels or rewrite intent.
    """
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Robo defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Robo-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Robo checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not sidecar_deps_installed():
            ...
            # `robo photon setup`, so the connect path must be able to
            ...
            if not sidecar_deps_installed():
                raise RuntimeError(
                    ...
                    f"Run: cd {_sidecar_dir()} && npm ci   (or `robo photon setup`)"
                )
        # A `robo update` that bumps the spectrum-ts pin rewrites
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        # (cron, `robo send`) can reach this sidecar
...
async def _standalone_send(
    ...
    *,
    ...
    if not token:
        ...
        # the gateway process env otherwise, so cron/`robo send` would be
        ...
        if not token:
            return {
                "error": (
                    ...
                    "Start the Robo gateway (which spawns the sidecar and "
                    "records its address under <robo-home>/runtime/"
    ...
    headers = {"X-Robo-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Robo plugin loader at startup."""
    # Local import to avoid argparse work at module load; reused for both the
    # gateway-setup hook and the `robo photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: robo photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `robo gateway setup` alongside every other
    ...
    # Register CLI subcommands — `robo photon ...`

...
This plugin connects Robo Agent to iMessage (and other Spectrum
...
TypeScript-only, Robo runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Robo channel,
...
Run `robo photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.robo/.env` (the same place every other
...
Management metadata lives in `~/.robo/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `robo photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Robo
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Robo coalesces
...
- **Native polls are supported.** Robo posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `robo send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<robo-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `robo photon status`, a DM and a group roundtrip,

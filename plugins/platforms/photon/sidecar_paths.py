...
under an immutable ``/opt/robo`` (read-only for the robo user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$ROBO_HOME/photon/sidecar`` (the durable data volume,
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_robo_home() / "photon" / "sidecar"
    try:

"""
``robo photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Robo
...
# wiring imports this module for `robo --help`).
...
# when called later (gateway start, robo status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `robo photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Robo Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``robo photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Robo Agent" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.robo/.env,
    ...
    try:
        from robo_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  robo gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`robo photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.robo/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `robo photon telemetry on` / `robo photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.robo/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  robo gateway restart")
    return 0
...
#
# `robo gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``robo photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `robo gateway setup` wizard."""

"""SimpleX Chat platform adapter (Robo plugin).
...
This adapter ships as a Robo platform plugin under
``plugins/platforms/simplex/``. The Robo plugin loader scans the
...
discoverable and ``robo setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Robo — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("ROBO_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``robo setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.robo/.env`` via
    ``robo_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "robo_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.robo/.env"
        )
        return

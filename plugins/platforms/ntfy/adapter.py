"""ntfy platform adapter (Robo plugin).
...
``plugins/platforms/ntfy/``. The Robo plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "robo-in"              # subscribe topic (incoming)
          publish_topic: "robo-out"     # optional — defaults to topic
...
_ECHO_TAG = "robo-engineer"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``robo cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Robo dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `robo gateway status` without

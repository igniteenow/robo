...
# identify RoboAgent traffic — matching other Robo outbound surfaces
# that already set ``RoboAgent/<version>`` for platform-partner attribution.
try:
    from robo_cli import __version__ as _ROBO_VERSION
except Exception:
    _ROBO_VERSION = "unknown"
_ROBO_SLACK_USER_AGENT_PREFIX = f"RoboAgent/{_ROBO_VERSION}"
...
def _rewrite_known_bang_command(text: str) -> str:
    ...
    try:
        from robo_cli.commands import is_gateway_known_command
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `robo slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `robo gateway setup` "
                "or in the active profile's ~/.robo/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `robo gateway setup` "
                "or add it to your active profile's ~/.robo/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `robo gateway setup` "
                "or in the active profile's ~/.robo/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `robo gateway setup` "
                "or add it to your active profile's ~/.robo/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        tokens_file = get_robo_home() / "slack_tokens.json"
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_ROBO_SLACK_USER_AGENT_PREFIX,
            )
            ...
            # Robo has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by Robo can "
            ...
            # /model work directly without /robo prefix). A single regex
            ...
            # manifest (see `robo slack manifest`). In Socket Mode, Slack
            ...
            from robo_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/robo$")

            @self._app.command(_slash_pattern)
            async def handle_robo_command(ack, command):
            ...
            self._app.action("robo_feedback")(self._handle_feedback_action)
            ...
            self._app.action(
                _re.compile(r"^robo_clarify_choice_\d+$")
            )(self._handle_clarify_action)
            self._app.action("robo_clarify_other")(self._handle_clarify_action)
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'robo slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Robo handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Robo session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Robo owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_reaction(self, event: dict, removed: bool = False) -> None:
        ...
        if target_channel:
            ...
            synthetic["_robo_reaction_source_channel"] = channel_id
            if target_thread:
                synthetic["thread_ts"] = target_thread
            else:
                synthetic.pop("thread_ts", None)
                synthetic["_robo_no_thread_response"] = True
    ...
    async def _handle_slack_message(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
        elif event.get("_robo_no_thread_response"):
        ...
        force_process = bool(event.get("_robo_force_process"))
        ...
        # the shape produced by peer Robo agents in Socket Mode on some
        ...
        try:
            ...
            if not smart_denied and allow_session:
                actions.append({
                    ...
                    "action_id": "robo_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "robo_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "robo_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
    ...
    async def send_clarify(
        ...
        (unique ``robo_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``robo_clarify_other``).  A choice click resolves the clarify
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "robo_confirm_once": "once",
            "robo_confirm_always": "always",
            "robo_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "robo_approve_once": "once",
            "robo_approve_session": "session",
            "robo_approve_always": "always",
            "robo_deny": "deny",
        }
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "robo_clarify_other" or token == "other":
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/robo <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/robo
        ...
        if slash_name in {"robo", ""}:
            # Legacy /robo <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            legacy_text = raw_text.strip()
            from robo_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/robo   ``).
        ...
        # "Running /cmd…" ack shown by handle_robo_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/robo <question>" must produce public replies so
...
# ``robo_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_robo_home() / "slack_tokens.json"
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``robo_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under ROBO_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from robo_cli.slack_cli import _build_full_manifest
            from robo_constants import get_robo_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Robo",
                bot_description="Your Robo agent on Slack",
            )
            target = Path(get_robo_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `robo slack manifest --write` anytime to refresh after "
                "Robo adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `robo update`)",
                True,
            ):
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``robo_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `robo setup` to install Slack support.",
        # Interactive setup wizard — replaces robo_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in robo_cli/gateway.py.

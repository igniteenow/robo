...
- Per-user tokens (keyed by sender email):
    ``${ROBO_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${ROBO_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${ROBO_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${ROBO_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${ROBO_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's ROBO_HOME helper so the token follows the user's
# profile (e.g. tests can override via ROBO_HOME=/tmp/...).
try:
    from robo_constants import display_robo_home, get_robo_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where robo_constants isn't importable
...
def _robo_home() -> Path:
    """Resolve ROBO_HOME at call time (NOT module import).

    Tests and ``ROBO_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.robo/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _robo_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _robo_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _robo_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _robo_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _robo_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Run `robo setup` to install Google Chat support."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("Run `robo setup` to repair the managed installation, then retry.")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into ROBO_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_robo_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_robo_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Robo (native attachment delivery)"
    )

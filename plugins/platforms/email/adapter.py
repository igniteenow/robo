"""
Email platform adapter for the Robo gateway.

Allows users to interact with Robo by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "robo-engineer" "version" "{_robo_version}" '
            '"vendor" "IgniteeNow" '
            '"support-email" "noreply@igniteenow.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `robo config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `robo gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Robo Agent")
        ...
        msg_id = f"<robo-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# robo_cli/gateway.py, and the _send_email dispatch in

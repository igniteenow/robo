...
Let the robo agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where robo runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `robo meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `robo meet` CLI |
...
| `cli.py` | `robo meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ROBO_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `robo meet node {run,list,approve,remove,status,ping}` |
...
robo meet install                                      # pip + Chromium
robo meet setup                                        # preflight
robo meet auth                                         # optional
robo meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
robo meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.robo/.env
robo meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
robo meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
robo meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.robo/.env
robo meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, robo will **not** switch your system audio input automatically — the
...
robo plugins enable google_meet
robo meet node run --display-name my-mac --host 0.0.0.0 --port 18789

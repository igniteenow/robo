...
Authentication via ``FAL_KEY``. Output is an
HTTPS URL from FAL's CDN; the tool downloads and delivers it.
"""
...
def _check_fal_video_available() -> bool:
    """True if the FAL.ai video backend is reachable (direct key)."""
        ...
        if not _check_fal_video_available():
            return error_response(
                error=(
                    ...
                    "(run `robo tools` → Video Generation → FAL to configure)."
                ),
            ...
            if not endpoint:
                return error_response(
                    error=(
                        ...
                        f"via `robo tools` → Video Generation."
                    ),

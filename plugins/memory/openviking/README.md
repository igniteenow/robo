...
- OpenViking server running and reachable from Robo

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Robo can identify older servers that expose the legacy status-only health
...
connection values into Robo, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.robo/.env`; for a named profile use
...
OpenViking's server config is separate from Robo:
...
Robo-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `robo` | Robo peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Robo lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Robo sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
Robo built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Robo action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because Robo
...
file URIs, such as
`viking://user/peers/robo/memories/preferences/mem_abc123.md` or the canonical
`viking://user/default/peers/robo/memories/preferences/mem_abc123.md`. Files

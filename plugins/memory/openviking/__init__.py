...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Robo peer ID in OpenViking (default: robo)
...
_DEFAULT_AGENT = "robo"
_AGENT_PROMPT_LABEL = "Robo peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "ROBO_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Robo slash-skill scaffolding before sending content to OpenViking.
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach Robo to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; Robo refused the connection."
        ) from exc
...
def _load_robo_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("ROBO_HOME", "")).expanduser() if os.environ.get("ROBO_HOME") else Path.home() / ".robo"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. Robo did not start "
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; Robo will retry on a later access or when "
        "the config changes."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Robo session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Robo only", "write values only to Robo .env"),
    ...
    _save_robo_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Robo .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See robo-engineer#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_robo_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Robo peer ID in OpenViking, sent as the actor peer and "
    ...
    def save_config(self, values: Dict[str, Any], robo_home: str) -> None:
    ...
    def post_setup(self, robo_home: str, config: dict) -> None:
        ...
        from robo_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        robo_home_path = Path(robo_home)
        env_path = robo_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        connection_error = ""
        try:
            settings = _resolve_connection_settings(_load_robo_openviking_config())
        except _OpenVikingEndpointError as exc:
        ...
        self._turn_count = 0
        robo_home = str(kwargs.get("robo_home") or "").strip()
        if not robo_home:
            try:
                from robo_constants import get_robo_home
                robo_home = str(get_robo_home())
            except Exception:
                robo_home = str(Path.home() / ".robo")
        self._robo_home = robo_home
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "Robo will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.robo/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts robo (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Robo will retry on a "
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._robo_home:
            return None
        return Path(self._robo_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._robo_home:
            return None
        return Path(self._robo_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _profile_token_budget(self) -> int:
        cfg = _load_robo_openviking_config()
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Robo' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Robo canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See robo-engineer#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/robo/.overview.md).

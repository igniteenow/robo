...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: robo)
...
Or via $ROBO_HOME/hindsight/config.json (profile-scoped), falling back to
...
# Mirrors an existing Hindsight integration pattern — Hindsight 0.5.0 added
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Robo can degrade gracefully instead of repeatedly trying to start
    ...
    healthy and ``robo memory status`` would stay green while the daemon
...
# ---------------------------------------------------------------------------
# Hindsight API capability probe.
# ---------------------------------------------------------------------------
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $ROBO_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_robo_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "robo": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "robo"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Robo config."""
    profile = config.get("profile", "robo")
    return str(profile or "robo")
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    values: dict[str, str] = {}
    # utf-8-sig, not plain utf-8: this is also used on the Robo .env during
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Robo profile name (from agent_identity)
      {workspace} — Robo workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``robo-{user}`` with no user becomes ``robo``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Robo Agent and the User"
    ...
    def save_config(self, values, robo_home):
        """Write config to $ROBO_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(robo_home) / "hindsight"
    ...
    def post_setup(self, robo_home: str, config: dict) -> None:
        ...
        from robo_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        provider_config.setdefault("bank_id", "robo")
        ...
        self.save_config(provider_config, robo_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(robo_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(robo_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "robo"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: robo-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Robo Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "robo"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "robo"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if Version(installed) < Version(_MIN_CLIENT_VERSION):
                ...
                # durable data-volume target instead of /opt/robo (NS-605).
                from tools.lazy_deps import install_specs
        ...
        banks = cfg_get(self._config, "banks", "robo", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "robo")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Robo Agent and the User")
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run Robo as a non-root user, or switch "
                    "to cloud / local_external mode via 'robo memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # Robo get sluggish. (issue #13125)
                try:
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See robo-engineer#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Robo created/used that client on the shared async loop,

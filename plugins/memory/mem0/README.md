...
Behavioral settings live in `$ROBO_HOME/mem0.json` (set them via `robo memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.robo/.env`.
...
| `user_id` | `robo-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Robo session and call `mem0_search` — it connects to your server.
...
```bash
robo memory setup mem0 --mode oss \
...
```bash
robo memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$ROBO_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.robo/mem0_qdrant"}}
  }
}
```
...
```bash
robo memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$ROBO_HOME/mem0.json`).

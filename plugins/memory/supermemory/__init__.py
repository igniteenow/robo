...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=robo"
...
def _load_supermemory_config(robo_home: str) -> dict:
    config = _default_config()
    config_path = Path(robo_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, robo_home: str) -> None:
    config_path = Path(robo_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "robo"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Robo writes into the "Robo" Space in the Supermemory
        ...
        merged = {"sm_source": "robo", **(metadata or {})}
...
def _resolve_container_tag_for_setup(robo_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(robo_home)
...
def _probe_supermemory_connection(api_key: str, robo_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(robo_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(robo_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `robo memory setup`.
        # All other options are documented for $ROBO_HOME/supermemory.json
    ...
    def save_config(self, values, robo_home):
        ...
        _save_supermemory_config(sanitized, robo_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, robo_home)
    ...
    def post_setup(self, robo_home: str, config: dict) -> None:
        ...
        from robo_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(robo_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from robo_constants import get_robo_home
        self._robo_home = kwargs.get("robo_home") or str(get_robo_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._robo_home)

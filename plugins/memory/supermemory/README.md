...
- Hosted: API key from [app.supermemory.ai/integrations?connect=robo](http://app.supermemory.ai/integrations?connect=robo)
...
Before running `robo memory setup`, add the local endpoint to
...
Then run `robo memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$ROBO_HOME/supermemory.json`
...
| `container_tag` | `robo` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `robo-{identity}` → `robo-coder`). |
...
`https://api.supermemory.ai`. Robo resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: robo`, and document writes stamp
`metadata.sm_source: robo`. This is a **functional routing key, not telemetry**:
it groups Robo-written memories into a dedicated "Robo" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Robo profile:

```json
{
  "container_tag": "robo-{identity}"
}
```

For a profile named `coder`, this resolves to `robo-coder`. The default profile resolves to `robo-default`. Without `{identity}`, all profiles share the same container.

## Multi-Container Mode

For advanced setups (e.g. multi-workspace configurations), you can enable custom container tags so the agent can read/write across multiple named containers:

...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/robo>
...
```bash
robo memory setup honcho   # configure Honcho directly (works on a fresh install)
robo memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
robo config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.robo/.env
```

> `robo honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `robo memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `robo model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ROBO_HOME/honcho.json` | Profile-local (isolated Robo instances) |
| 2 | `~/.robo/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Robo profile: `robo` (default) or `robo_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `robo honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Robo session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`robo-engineer`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`robo`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-robo-engineer`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening robo in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Robo session ID (timestamp + hex). Every `robo` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Robo profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`robo`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Robo profile: `robo` (default) or `robo_<profile>` (e.g. `robo -p coder` -> host key `robo_coder`). Older `robo.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `ROBO_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `robo-engineer`) |
...
|---------|-------------|
| `robo memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `robo honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `robo memory setup`) |
| `robo honcho status` | Show resolved config for active profile |
| `robo honcho enable` / `disable` | Toggle Honcho for active profile |
| `robo honcho mode <mode>` | Change recall or observation mode |
| `robo honcho peer --user <name>` | Update user peer name |
| `robo honcho peer --ai <name>` | Update AI peer name |
| `robo honcho tokens --context <N>` | Set context token budget |
| `robo honcho tokens --dialectic <N>` | Set dialectic max chars |
| `robo honcho map <name>` | Map current directory to a session name |
| `robo honcho sync` | Create host blocks for all Robo profiles |

...
Handles: robo honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "robo" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Robo profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'robo honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'robo honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `robo update` -- no output, no exceptions.
    """
    try:
        from robo_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Robo profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $ROBO_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_robo_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        robo_host.get("pinUserPeer"),
        robo_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in robo_host:
        aliases_src = robo_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in robo_host:
        prefix_src = robo_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(robo_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom robo_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    robo_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        robo_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(robo_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = robo_host.get("userPeerAliases")
    prefix = robo_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(robo_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(robo_host)
    robo_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        robo_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        robo_host, current_prefix, prefix_from_root,
...
def _ensure_sdk_installed() -> bool:
    ...
    # data-volume target instead of writing to /opt/robo (NS-605).
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Robo persistent cross-session memory.")
    ...
    if is_local:
        ...
        current_host_key = robo_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            robo_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        existing_oauth = OAuthCredential.from_host_block(robo_host)
        ...
        if use_device:
            ...
            try:
                ...
                print("\n  Cancelled. Re-run 'robo honcho setup' to try again.\n")
                return
            except (AuthorizationTimeout, DeviceCodeExpired):
                print("\n  Device code expired before approval.")
                print("  Re-run 'robo honcho setup' to get a new code.\n")
                return
            except AccessDenied:
                print("\n  Sign-in was denied on the approval page.")
                print("  Re-run 'robo honcho setup' to retry, or choose an API key instead.\n")
                return
                ...
                return
            print(" approved")
            _apply_grant_to_host(robo_host, cred)
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'robo honcho setup' again once you have a key.\n")
                return

    # --- 3. Identity ---
    current_peer = robo_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        robo_host["peerName"] = new_peer

    current_ai = robo_host.get("aiPeer") or cfg.get("aiPeer", "robo")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        robo_host["aiPeer"] = new_ai

    current_workspace = robo_host.get("workspace") or cfg.get("workspace", "robo")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        robo_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the Robo GATEWAY (Telegram/Discord/Slack/...),
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, robo_host)
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Robo gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    if run_mapping:
        peer_target = robo_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(robo_host)
            robo_host["pinUserPeer"] = False
            if prior_aliases:
                robo_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                robo_host, current_prefix, prefix_from_root,
            ...
            if merged:
                robo_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                robo_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                robo_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = robo_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        robo_host["observationMode"] = new_obs
    else:
        robo_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(robo_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        robo_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        robo_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = robo_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        robo_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = robo_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        robo_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                robo_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(robo_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            robo_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        robo_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        robo_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        robo_host["dialecticReasoningLevel"] = new_reasoning
    else:
        robo_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = robo_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        robo_host["sessionStrategy"] = new_strat

    robo_host["enabled"] = True
    robo_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        robo_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(robo_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: robo config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    robo honcho status     -- show full config")
    print("    robo honcho mode       -- change recall/observation mode")
    print("    robo honcho tokens     -- tune context and dialectic budgets")
    print("    robo honcho peer       -- update peer names")
    print("    robo honcho map <name> -- map this directory to a session name\n")
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if robo_cli internals move."""
    try:
        from robo_cli.auth import _can_open_graphical_browser, _is_remote_session
...
def _apply_grant_to_host(robo_host: dict, cred) -> None:
    """Store an OAuth grant on the host block; the wizard's final save persists it."""
    robo_host["apiKey"] = cred.access_token
    robo_host["oauth"] = cred.oauth_block()
    # Default the peer prompt to the name entered at consent.
    if cred.consent_peer_name:
        robo_host["peerName"] = cred.consent_peer_name


def _active_profile_name() -> str:
    """Return the active Robo profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: robo honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'robo honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'robo honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: robo honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        robo = hosts.get(_host_key(), {})
        user = robo.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = robo.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = robo.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = robo.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Robo' identity in Honcho. Seed with 'robo honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: robo honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print("\n  Set with: robo honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = robo.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = robo.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = robo.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Robo asks Honcho's AI peer a question")
        ...
        print("\n  Set with: robo honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'robo honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'robo honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    robo honcho identity --show        — show both peer representations")
        print("    robo honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'robo memory setup'...\n")
        from robo_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``robo honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``robo honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to robo memory setup)",
    )

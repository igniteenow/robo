...
``robo://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Robo OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "robo-engineer"
...
def begin_authorization(
    ...
    *,
    ...
    (``robo-desktop`` / ``robo-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Robo.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local ROBO_HOME override can't reach.

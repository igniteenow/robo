"""robo-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $ROBO_HOME/config.yaml (profile-scoped):
  plugins:
    robo-memory-store:
      db_path: $ROBO_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "robo-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, robo_home):
        """Write config to config.yaml under plugins.robo-memory-store."""
        from pathlib import Path
        config_path = Path(robo_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["robo-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_robo_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _robo_home = str(get_robo_home())
        _default_db = _robo_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $ROBO_HOME in user-supplied paths so config values like
        # "$ROBO_HOME/memory_store.db" or "~/.robo/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$ROBO_HOME", _robo_home)
            db_path = db_path.replace("${ROBO_HOME}", _robo_home)

...
1. Bundled providers: ``plugins/cron_providers/<name>/`` (shipped with robo-engineer)
2. User-installed providers: ``$ROBO_HOME/plugins/<name>/``
...
Usage:
    ...
    provider = load_cron_scheduler(name)   # CronScheduler instance
"""
...
_USER_NAMESPACE = "_robo_user_cron"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_robo_user_cron.<name>``, a dotted
    ...
    ``ModuleNotFoundError: No module named '_robo_user_cron'`` — the same
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$ROBO_HOME/plugins/`` or None if unavailable."""
    try:
        from robo_constants import get_robo_home
        d = get_robo_home() / "plugins"
...
def load_cron_scheduler(name: str) -> Optional["CronScheduler"]:  # noqa: F821
    ...
    (``$ROBO_HOME/plugins/<name>/``) directories. Bundled takes precedence
    on name collisions.
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["CronScheduler"]:  # noqa: F821
    ...
    if cached is not None and getattr(cached, "__file__", None):
        ...
        # Register submodules so relative imports work in a provider plugin

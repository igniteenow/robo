...
``{title, url, description, position}`` rows every other Robo web
provider produces.
...
prefers Robo-managed xAI Grok OAuth (via ``robo auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.robo/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``robo tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `robo auth` to sign in with "

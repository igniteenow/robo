...
def check_firecrawl_api_key() -> bool:
    """Return True when a direct Firecrawl backend is configured/usable.

    Re-exported by :mod:`tools.web_tools` for backward compatibility with
    existing tests and the ``robo tools`` setup flow.
    """
...
def _get_firecrawl_client() -> Any:
    ...
    Raises ValueError when no direct Firecrawl config is usable.
...
class FirecrawlWebSearchProvider(WebSearchProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            "name": "Firecrawl",
            "badge": "paid",
            "tag": "Full search + extract via the Firecrawl API",
            "env_vars": [
                {

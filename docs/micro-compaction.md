...
has to be thrown away or summarized. Robo has always done this in one batch:
...
Robo folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Robo's normal session flush is
...
With `micro_compact` unset or `false` Robo behaves exactly as it always has:
...
Defaults to `$ROBO_HOME/logs/agent.log`. It reports passes, outcome mix, net

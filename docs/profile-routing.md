...
`robo config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.robo/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `ROBO_HOME`, secret scope, and

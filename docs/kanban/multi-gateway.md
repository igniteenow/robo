...
Robo supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.robo/config.yaml`:
...
Or set the env var: `ROBO_KANBAN_DISPATCH_IN_GATEWAY=false`

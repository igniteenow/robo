...
The canonical transcript store is SQLite via `SessionDB` (from `robo_state`). The
...
Counts consecutive restarts via a JSON file (`{ROBO_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `robo update`, `robo gateway restart`,
or `/restart`.

...
Robo observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Robo' planner, model providers, memory, tool registry,
...
Robo also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "robo.observer.v1"
```

Hook callbacks are fail-open. Robo catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Robo only builds

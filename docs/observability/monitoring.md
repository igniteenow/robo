...
Robo gateway daemon, exported over OTLP/HTTP to an operator-configured
...
(`plugins/observability/nemo_relay/`) and its Robo-owned subscribers.

## What gets exported

| Signal | OTLP route | Content |
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `robo.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `robo.platform.up/degraded` with bounded `error_code` attributes |
...
`robo.gateway.active_agents`, `robo.gateway.background_work`, and
`robo.gateway.background_delegations` are complementary. `active_agents`
...
```bash
robo monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'robo-engineer[otlp]'`),
...
a plain `robo gateway run`: the exporter lives in the gateway process, so
...
`robo.gateway.up`, `robo.platform.up`, and `robo.platform.degraded`.
...
absent_over_time(robo_gateway_up[5m])

# Locally owned bridge is explicitly down.
robo_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
robo_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
robo_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
robo_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(robo_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `robo.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
robo.status = failed|unknown
robo.delivery_outcome = failed|not_configured
robo.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `robo.job_key`;
...
Robo Agent-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/robo_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/robo_otel_capture.jsonl --wait 8
...
`robo.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
robo monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `robo monitoring` CLI intentionally exposes `status` only. This first
release covers only Robo Agent-owned service-health and operational-diagnostic
signals, including Robo Agent-owned Relay transport health. Team Gateway's

...
Robo includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Robo and does not require `robo plugins enable
observability/nemo_relay`. Robo remains importable without Relay on other
native targets. Those targets use an explicit reduced-capability no-op host:
Robo execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `robo-engineer[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

Robo requires NeMo Relay 0.6.0 or later within the 0.6 release line. That
...
Robo installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Robo process so configured interceptors can
...
Collection remains off unless Robo policy enables it:
...
Robo core owns one Relay host and one isolated Relay session scope per Robo
session. Core lifecycle producers use
`robo_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Robo plugin registration. Shared-metrics marks must
...
```text
Robo turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Robo shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Robo sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Robo used for the logical call, such as
...
New calls use `robo.model_route.count`. The previous
`robo.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `robo.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Robo
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Robo processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `robo.task_run`, parented to
the owning Robo session. The start counter contains only bounded execution
...
provider attempts for the same Robo API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Robo tool-call
...
cancellations. Active task ownership follows the task ID if Robo rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`robo.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Robo
...
tool or plugin names. Robo does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `robo.tool_approval` marks and
...
Successful skill mutations emit `robo.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `robo.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Robo derives reuse and patch-generation
...
```text
$ROBO_HOME/telemetry/shared_metrics/metrics.sqlite3
$ROBO_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Robo version,
...
schema field name, its current scope is one `ROBO_HOME`, so it is more
...
Deleting `$ROBO_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `ROBO_HOME`. To reset it, stop Robo
processes and remove `$ROBO_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Robo CLI turn against the deterministic local model server:

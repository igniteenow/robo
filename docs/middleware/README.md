...
Robo middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Robo'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Robo calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Robo retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Robo guardrails,
...
- `telemetry_schema_version`: currently `robo.observer.v1`
- `middleware_schema_version`: currently `robo.middleware.v1`
...
Robo stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Robo runs
them as a nested chain in registration order. Middleware failures are fail-open:
Robo logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Robo applies middleware in this order:
...
For each tool call, Robo applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Robo pre-execution path against the effective arguments:
...
```bash
robo plugins enable <plugin-name>
```

For isolated local testing, use one `ROBO_HOME` for plugin enablement and the
agent run:

```bash
export ROBO_HOME=/tmp/robo-middleware-test
mkdir -p "$ROBO_HOME"
robo plugins enable <plugin-name>
robo chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run robo plugins enable <plugin-name>
uv run robo chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["robo_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/robo-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Robo expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Robo treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Robo preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Robo does not convert downstream

...
This document is the formal interface between the **Robo gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`IgniteeNow/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `robo gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`robo gateway enroll` → connector `/relay/enroll`, or
...
(`IgniteeNow/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `robo` CLI.

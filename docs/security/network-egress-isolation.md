...
When running Robo inside Docker, the default `network_mode: host` gives the
...
The Robo [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=robo,robo-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://robo-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Robo trust model and vulnerability reporting

"""Shared constants for Robo Agent.
...
_UNSET = object()
_ROBO_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_ROBO_HOME_OVERRIDE", default=_UNSET
)
...
def set_robo_home_override(path: str | Path | None) -> Token:
    """Set a context-local Robo home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _ROBO_HOME_OVERRIDE.set(value)


def reset_robo_home_override(token: Token) -> None:
    """Restore the previous context-local Robo home override."""
    _ROBO_HOME_OVERRIDE.reset(token)


def get_robo_home_override() -> str | None:
    """Return the active context-local Robo home override, if any."""
    override = _ROBO_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_robo_home() -> Path:
    """Return the platform-native default Robo home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "robo"
    return Path.home() / ".robo"


def _robo_home_from_env() -> Path:
    """Resolve ROBO_HOME from the process environment only.

    Reads the ``ROBO_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_robo_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_robo_home`
    and :func:`get_process_robo_home` so the two never drift.
    """
    val = os.environ.get("ROBO_HOME", "").strip()
    if val:
        return Path(val)
    return _get_platform_default_robo_home()


def _warn_profile_fallback_once() -> None:
    """Warn once when falling back to the default home while a profile is active.

    Guard: if a non-default profile is sticky-active but ``ROBO_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_robo_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[ROBO_HOME fallback] ROBO_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass ROBO_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_robo_home() -> Path:
    """Return the Robo home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_robo_home_override`) → ``ROBO_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``ROBO_HOME`` is unset but an ``active_profile`` file indicates
    ...
    if not os.environ.get("ROBO_HOME", "").strip():
        _warn_profile_fallback_once()

    return _robo_home_from_env()


def get_process_robo_home() -> Path:
    """Return the Robo home for the running process, ignoring task overrides.

    Unlike :func:`get_robo_home`, this never follows the context-local
    override set by :func:`set_robo_home_override`.  It resolves only the
    process ``ROBO_HOME`` env var (falling back to the platform default),
...
def get_default_robo_root() -> Path:
    """Return the root Robo directory for profile-level operations.

    In standard deployments this is the platform-native Robo home
    (``~/.robo`` on POSIX, ``%LOCALAPPDATA%\\robo`` on native Windows).

    In Docker or custom deployments where ``ROBO_HOME`` points outside
    ``~/.robo`` (e.g. ``/opt/data``), returns ``ROBO_HOME`` directly
    — that IS the root.

    In profile mode where ``ROBO_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.robo/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_robo_home()
    env_home = os.environ.get("ROBO_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # ROBO_HOME is under ~/.robo (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — ROBO_HOME itself is the root
    return env_path
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``ROBO_OPTIONAL_SKILLS``.
    """
    override = os.getenv("ROBO_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_robo_home() / "optional-skills"


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers.

    Mirrors :func:`get_optional_skills_dir` for the MCP catalog (Ignitee Now-approved
    Model Context Protocol servers shipped with the repo but disabled by
    default). Packaged installs may ship ``optional-mcps`` outside the Python
    package tree and expose it via ``ROBO_OPTIONAL_MCPS``.
    """
    override = os.getenv("ROBO_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_robo_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``ROBO_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<ROBO_HOME>/skills`` last-resort
    """
    override = os.getenv("ROBO_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_robo_home() / "skills"
...
def get_robo_dir(
    ...
    *,
    ...
    """Resolve a Robo subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to ROBO_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to ROBO_HOME (e.g. ``"image_cache"``).
        home: Optional explicit Robo home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local ROBO_HOME.

    Returns:
        Absolute ``Path`` — legacy location if it exists with content,
        otherwise the new location.
    """
    home = home or get_robo_home()
...
def iter_robo_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Robo-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\robo\\node``. POSIX installs use
    ``$ROBO_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_robo_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with roboManagedNodePathEntries() in
...
_ROBO_NODE_TARGET_MAJOR = int(os.environ.get("ROBO_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Robo-managed Node trees live under ``$ROBO_HOME/node`` (or a profile's
    ``ROBO_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_robo_node_executable``
    used to trust file presence alone, so ``robo update`` would pick that
...
def robo_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Robo-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_robo_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%ROBO_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    home = get_robo_home()
    index_url = f"https://nodejs.org/dist/latest-v{_ROBO_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_ROBO_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$ROBO_HOME/node`` on POSIX.
    ...
    ``ROBO_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_robo_managed_node() -> str | None:
    """Install a Robo-managed Node tree and return its npm path.
    ...
    Robo never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$ROBO_HOME/node`` (the same tree a fresh install
    ...
    present.
    """
    existing = find_robo_node_executable("npm")
    if existing:
        return existing
    ...
    for directory in iter_robo_node_dirs():
...
def heal_robo_managed_node() -> bool:
    """Redownload Robo-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not robo_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ROBO_HOME": str(get_robo_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_robo_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_ROBO_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_robo_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            return major < _ROBO_NODE_TARGET_MAJOR
    return False


def find_robo_node_executable(command: str) -> str | None:
    """Return a Robo-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_ROBO_NODE_TARGET_MAJOR``) heal the
    ...
    if needs_heal and heal_robo_managed_node():
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Robo-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Robo-managed installs.

    This is for Robo-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_robo_node_executable(command)
    if managed:
        return managed
    if robo_managed_node_tree_present():
        return None
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``robo update`` — leaving a **dangling symlink** that ``which`` still
...
def display_robo_home() -> str:
    """Return a user-friendly display string for the current ROBO_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.robo``
        profile:  ``~/.robo/profiles/coder``
        custom:   ``/opt/robo-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.robo``.  For code that needs a real ``Path``, use
    :func:`get_robo_home` instead.
    """
    home = get_robo_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``ROBO_HOME`` or other path
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{ROBO_HOME}/home`` when the profile-home directory exists."""
    robo_home = get_robo_home_override() or (env or {}).get("ROBO_HOME") or os.getenv("ROBO_HOME")
    if not robo_home:
        return None
    profile_home = os.path.join(robo_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("ROBO_REAL_HOME") or os.getenv("ROBO_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Robo profile HOME.

    ``ROBO_HOME`` scopes Robo state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={ROBO_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{ROBO_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{ROBO_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Robo' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["ROBO_REAL_HOME"] = real_home
...
def resolve_reasoning_config(cfg: dict | None, model: str = "") -> dict | None:
    ...
    # Deep thinking (default on): with no explicit global effort, think at
    # "high" instead of the provider default. An explicit value — including
    # a YAML False — always wins, so nobody who disabled thinking is
    # silently re-enabled. Providers/models without thinking ignore it.
    if result is None and (effort is None or not str(effort).strip()):
        deep = agent_cfg.get("deep_thinking", True)
        if deep is None or bool(deep):
            return parse_reasoning_effort("high")
    return result
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Robo itself runs inside WSL.
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under ROBO_HOME.

    Replaces the ``get_robo_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, robo_logging.py, robo_time.py, etc.).
    """
    return get_robo_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under ROBO_HOME."""
    return get_robo_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under ROBO_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_robo_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._robo_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``robo_cli`` modules using three different Windows
    ...
    A few sites outside ``robo_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``robo_cli.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of Robo itself. An ImportError
...
# `robo_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Robo.
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("robo_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `robo update` is exactly

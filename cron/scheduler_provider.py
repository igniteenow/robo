...
ticker. Alternative providers live under plugins/cron_providers/<name>/ and are
...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        *,
        ...
        # only the process-global ROBO_HOME (the default profile) is ticked.
        # Heartbeats and recovery are also scoped per profile so `robo cron
        ...
        # Heartbeat once before the first sleep so `robo cron status` sees a
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # `robo cron status`/`list` (separate processes) can show
    ...
    def _start_multiplex(
        ...
        *,
        ...
        Each profile uses ``set_robo_home_override()`` + ``use_cron_store()``
        ...
        for entry in profile_homes:
            ...
            home_token = set_robo_home_override(str(home))
            try:
            ...
            for entry in profile_homes:
                ...
                try:
                    with use_cron_store(home):
                        ...
                        # so `robo cron status` can show WHY ticks fail
                        # (#68483).
                        if ok:
                            clear_ticker_error()
                        elif _tick_error:

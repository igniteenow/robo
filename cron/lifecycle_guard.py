...
``robo gateway restart`` (or ``launchctl kickstart ai.robo.gateway``
or ``systemctl restart robo-gateway``).  When the cron fires, the
...
``robo cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``robo gateway``, ``launchctl ... robo-gateway``,
``systemctl ... robo-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_ROBO_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``robo gateway
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `robo gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:robo\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a robo-gateway label. macOS launchd
    # labels look like `ai.robo.gateway` / `robo-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated robo services (e.g.
    # `launchctl unload ai.robo.update-checker.plist`).
    # `submit` and `bootstrap` are included alongside the direct verbs
    # (kickstart/etc.): `launchctl submit -l ai.robo.gateway-<suffix> --
    ...
    # Branch C: systemctl ops on a robo-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\brobo[.\-]?gateway)"
    # Branch D: pkill / kill targeting the robo gateway process. Both
...
# lines (e.g. `launchctl submit \` / `  -l ai.robo.gateway-... \` / `  -- ...`,
...
# for `systemctl restart robo-gateway` in syslog, a SQL LIKE literal over a
...
# to execution (`grep 'systemctl restart robo-gateway' f | sh`); never mask
# such a line.
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.robo.svc-reload-tmp``)
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart robo-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart robo-gateway%'"
...
def _iter_referenced_shell_scripts(
    ...
    *,
    ...
    for segment in _iter_command_segments(command):
        ...
        # (e.g. `Path.home() / ".robo"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    under ``<ROBO_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.robo/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_robo_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_robo_home() falls back to Path.home(), which raises when
        # neither ROBO_HOME nor HOME is resolvable (launchd/systemd
...
def check_gateway_lifecycle(
    ...
    if python_script:
        ...
        # `robo gateway restart` embedded in a .py script is still
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `robo gateway restart` from a shell outside "

"""
Robo MCP Server — expose messaging conversations as MCP tools.

Starts a stdio MCP server that lets any MCP client (Claude Code, Cursor, Codex,
etc.) list conversations, read message history, send messages, poll for live
events, and manage approval requests across all connected platforms.

Matches 9-tool MCP channel bridge surface:
  conversations_list, conversation_get, messages_read, attachments_fetch,
  events_poll, events_wait, messages_send, permissions_list_open,
  permissions_respond

Plus: channels_list (Robo-specific extra)

Usage:
    robo mcp serve
    robo mcp serve --verbose
...
logger = logging.getLogger("robo.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ROBO_HOME."""
    try:
        from robo_constants import get_robo_home
        return get_robo_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ROBO_HOME", Path.home() / ".robo")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from robo_constants import get_robo_home
        directory_file = get_robo_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ROBO_HOME", Path.home() / ".robo")
...
class EventBridge:
    ...
    This is the Robo equivalent of WebSocket gateway bridge.
    ...
    def _establish_baseline(self) -> None:
        ...
        try:
            from robo_constants import get_robo_home
            db_file = get_robo_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ROBO_HOME", Path.home() / ".robo")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Robo MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "robo",
        instructions=(
            "Robo Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Robo MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(

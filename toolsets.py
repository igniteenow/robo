...
_ROBO_CORE_TOOLS = [
    ...
    # Deep reverse engineering (read-only static binary/firmware analysis)
    "reverse_engineer",
    # Browser automation
    ...
    "session_search",
    # Knowledge base: attach files of any size, then ask about them
    ...
    # spawned as a kanban worker (ROBO_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `robo tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "knowledge": {
        "description": "Knowledge base — index attached files of any size (PDF, DOCX, XLSX, code, logs) and search them",
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (ROBO_KANBAN_TASK env "
    ...
    "reverse_engineering": {
        "description": (
            "Deep reverse engineering: read-only static analysis of binaries, "
            "libraries, and firmware (identify, strings, symbols, imports, "
            "headers, disassembly, hexdump, security-hardening flags, entropy). "
            "Routes through the terminal backend, so it inherits the sandbox and "
            "approval flow and never executes the target."
        ),
        "tools": ["reverse_engineer"],
        "includes": []
    },
    ...
    # Coding posture (base Robo — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in robo_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Robo toolsets (CLI + messaging platforms)
    #
    ...
    # the `robo send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "robo-core": {
        "description": "Robo interactive engineering toolset",
        "tools": _ROBO_CORE_TOOLS,
        "includes": []
    },

    # Legacy alias retained so existing user configurations continue to load.
    "robo-cli": {
    ...
    "robo-cron": {
        # Mirrors robo-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `robo tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as robo-cli; gated by `robo tools`",
        "tools": _ROBO_CORE_TOOLS,
        "includes": []
    },
    ...
    "robo-email": {
        "description": "Email bot toolset - interact with Robo via email (IMAP/SMTP)",
        "tools": _ROBO_CORE_TOOLS,
        "includes": []
    },
    ...
    "robo-sms": {
        "description": "SMS bot toolset - interact with Robo via SMS (Twilio)",
        "tools": _ROBO_CORE_TOOLS,
        "includes": []
    },
    ...
    "robo-webhook": {
        ...
        "tools": _ROBO_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "robo-gateway": {
        ...
        "tools": [],
        "includes": ["robo-telegram", "robo-discord", "robo-whatsapp", "robo-slack", "robo-signal", "robo-bluebubbles", "robo-homeassistant", "robo-email", "robo-sms", "robo-mattermost", "robo-matrix", "robo-dingtalk", "robo-feishu", "robo-wecom", "robo-wecom-callback", "robo-weixin", "robo-qqbot", "robo-webhook", "robo-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``robo-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_ROBO_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``robo-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (robo-<name>).
        # Gives them _ROBO_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("robo-"):
            platform_name = name[len("robo-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_ROBO_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(

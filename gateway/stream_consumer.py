...
class StreamConsumerConfig:
    ...
    # an upstream fix.  Default 0 = always edit in place (legacy
...
class GatewayStreamConsumer:
    ...
    def _reset_segment_state(self, *, preserve_no_edit: bool = False) -> None:
        ...
        # we avoid the "inter-tool-call text leak" failure mode: each text
        # block becomes its own visible message via the finalize, then a
        # new draft animates for the next one.
    ...
    def _adapter_prefers_fresh_final(self, text: str) -> bool:
        ...
        currently renders richer markdown than Robo' MarkdownV2 edit path.
        ...
        try:
            if self._message_id is not None:
                if self._edit_supported:
                    ...
                    # an upstream fix.  Gated by config so the

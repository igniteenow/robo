...
There are two ways to add a platform to the Robo gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.robo/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Robo code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `robo gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `robo_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Robo core.
...
And add it to the `robo-gateway` composite:

```python
"robo-gateway": {
    "includes": [..., "robo-your-platform"]
}
```
...
## 12. Status Display (`robo_cli/status.py`)
...
## 13. Gateway Setup Wizard (`robo_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ robo_cli/ toolsets.py \
  --include="*.py" -l | sort -u

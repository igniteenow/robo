...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.robo to the active profile home."""
    ...
    if raw == "~/.robo":
        return robo_home
    if raw.startswith("~/.robo/"):
        return robo_home / raw.removeprefix("~/.robo/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return robo_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under ROBO_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from robo_constants import get_robo_home

    scripts_root = (get_robo_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.robo" or raw_text.startswith("~/.robo/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__robo_ignore__") is True
        ):
            return False, None

...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "◆ *Robo Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to ROBO_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/robo), this function
    mirrors the bridge source to a writable ROBO_HOME location and returns that
    ...
    # Try ROBO_HOME location first
    robo_home = get_robo_home()
    robo_home_bridge = robo_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to ROBO_HOME if needed
    if robo_home_bridge.exists():
        return robo_home_bridge

    # Mirror the bridge source to ROBO_HOME
    try:
        robo_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            robo_home_bridge,
            dirs_exist_ok=False,

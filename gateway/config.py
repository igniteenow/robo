...
# dashboard's pre-write mutation validation (robo_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "Robo is thinking…"). Platforms with textless indicators (Discord,
...
class StreamingConfig:
    ...
    # Ported from an upstream fix.  When >0, the final edit for
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_robo_home() / "sessions")
    ...
    # gateway behaves exactly as before — single ROBO_HOME, no profile stamping.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        # set to a recognized value. Hosted deployments (Ignitee Now Portal / Fly) stamp
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.robo/config.yaml (primary user-facing config)
    3. ~/.robo/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_robo_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through robo_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `robo config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `robo config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``robo config set gateway.streaming.*``
...
def _validate_gateway_config(config: "GatewayConfig") -> None:
    ...
    # Ported from an upstream fix: users who copy .env.example

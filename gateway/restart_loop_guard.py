...
Defenses 1 and 2 (the ``_ROBO_GATEWAY`` guard on ``robo gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.robo.gateway")``,
...
State lives in ``<ROBO_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_robo_home() / "gateway" / "restart_loop.json"

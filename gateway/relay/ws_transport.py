...
(``robo-gateway`` ``src/relay/protocol.ts``) and mirrored in
...
def _normalize_slack_parent_command(
    ...
    """Mirror native Slack ``/robo`` routing for authenticated relay text."""
    stripped = text.strip()
    parent_parts = stripped.split(maxsplit=1)
    if not parent_parts or parent_parts[0] != "/robo":
...
def _event_from_wire(raw: Dict[str, Any]) -> MessageEvent:
    ...
    source = SessionSource(
        ...
        # The ROBO profile this event is routed to (multiplex mode). The
    ...
    if platform_enum == Platform.SLACK:
        ...
        # relay, bypassing Robo' native Slack command callback. Normalize at
        # the wire boundary so adapter-level active-session gates see the real
        # gateway command rather than the legacy `robo` parent name.
...
class WebSocketRelayTransport:
    ...
    def __init__(
        ...
        *,
        ...
        # the connector accumulates them into its advertised set (robo-gateway
    ...
    async def _request_response(
        ...
        *,
        ...
        # envelope (robo-gateway D-Q1.5b.1), with its MATCHING advertised botId

"""Relay/connector support package for the Robo gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``robo gateway enroll`` writes them to
    ``~/.robo/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``ROBO_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``robo gateway enroll``).
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from robo_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # multi-agent scope it would prefix every reply "**Robo Agent:**" —
...
def _resolve_relay_identity_token() -> str:
    ...
    ``robo gateway enroll`` CLI. Requires a generic OIDC client-credentials
    provider (self-hosted IdP): when ``gateway.idp.token_url`` (or
    ...
    if not token_url:
        raise RuntimeError(
            "Gateway relay identity resolution requires an IdP: set "
            "gateway.idp.token_url (or GATEWAY_RELAY_IDP_TOKEN_URL) plus "
            "client_id/client_secret for your own OAuth2 client_credentials "
            "provider."
        )
...
def self_provision_relay() -> bool:
    ...
    is already present, AND the agent can resolve its own relay identity token
    via a configured IdP. In that case the runtime resolves that token (the same
    ``_resolve_relay_identity_token()`` the enroll CLI / dashboard register use),
    ...
    up. The creds live ONLY in process memory — never written to ``~/.robo/.env``.
    ...
    sets neither ``ROBO_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A NAS-hosted agent: has ``GATEWAY_RELAY_URL``, no pinned secret, and a
        bootstrapped IdP identity -> self-provisions.
      - A self-hosted operator who ran ``robo gateway enroll``: has a PINNED
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'robo'}"

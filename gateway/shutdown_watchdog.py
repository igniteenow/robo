...
2. An event-loop heartbeat file at ``<ROBO_HOME>/state/gateway.heartbeat`` so
...
def _process_robo_home() -> Path:
    """ROBO_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<ROBO_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_robo_home()
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from robo_logging import drain_log_queue

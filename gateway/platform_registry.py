...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``robo setup`` display).
    ...
    # built-ins).  Used by ``robo gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `robo`
        # invocation -- including plain `robo chat`, which never touches any
        ...
        # `robo setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``robo setup``/``gateway status``, channel

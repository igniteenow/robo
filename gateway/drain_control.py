...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{ROBO_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``ROBO_HOME`` is a **durable** store — on Robo
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``robo gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting ROBO_HOME."""
    base = home if home is not None else get_robo_home()
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable ROBO_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (ROBO_HOME is a
    durable Fly volume on Robo Cloud) and the lifecycle action that triggered
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    marker that survived a machine restart on the durable ROBO_HOME volume is

"""Windows UTF-8 bootstrap for Robo entry points.
...
should be imported at the very top of every Robo entry point
(``robo``, ``robo-engineer``, ``robo-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``robo_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``robo_bootstrap``, never ``robo_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Robo modules.

    Robo ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Robo source root
    ...
    """
    root = src_root or os.environ.get("ROBO_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``ROBO_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("ROBO_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import robo_bootstrap``
# (or ``from robo_bootstrap import apply_windows_utf8_bootstrap``) at

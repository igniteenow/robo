...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Robo Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Robo setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Robo Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Robo Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Robo for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Robo working on my machine | `robo setup` | Run a real chat and verify it responds |
| I already know my provider | `robo model` | Save the config, then start chatting |
| I want a bot or always-on setup | `robo gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `robo model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `robo model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Robo cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.
...
The single most important setup step. Use `robo model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `robo setup` offers two modes:

- **Full Setup (recommended)** — walk through every provider, tool, and option yourself (bring your own keys).
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `robo update`. Re-enable anything later with `robo tools`, seed skills with `robo skills opt-in --sync`, or tune settings with `robo setup agent`. On a returning install, `robo setup --quick` fills in only whatever's missing or unset.
:::
...
|----------|-----------|---------------|
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `robo model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `robo model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `robo_cli/models.py` may change between releases) | `robo model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `robo model` → xAI Grok OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `robo model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `robo model` (requires `copilot` CLI + `copilot login`) |
...
Robo Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `robo model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Robo separates secrets from normal config:

- **Secrets and tokens** → `~/.robo/.env`
- **Non-secret settings** → `~/.robo/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
robo config set model anthropic/claude-opus-4.6
robo config set terminal.backend docker
robo config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
robo --continue    # Resume the most recent session
robo -c            # Short form
```
...
```bash
robo gateway setup    # Interactive platform configuration
```
...
- `robo tools` — tune tool access per platform
- `robo skills` — browse and install reusable workflows
...
```bash
robo config set terminal.backend docker    # Docker isolation
robo config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `robo egress setup && robo egress start`; `robo setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the Robo install directory (the curl installer placed it at
# ~/.robo/robo-engineer on Linux/macOS or %LOCALAPPDATA%\robo\robo-engineer on Windows):
cd ~/.robo/robo-engineer
...
Skills are on-demand instruction documents that teach Robo how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Robo ships with a catalog of bundled skills already installed in `~/.robo/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
robo skills browse                      # list everything available
robo skills search kubernetes           # find skills by keyword
robo skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `robo skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Robo ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.robo/robo-engineer && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Robo opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `robo model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `robo gateway setup` and check `robo gateway status` |
| `robo --continue` can't find old session | Switched profiles or session never saved | Check `robo sessions list` and confirm you're in the right profile |
...
| `robo doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `robo model`
3. `robo setup`
4. `robo sessions list`
5. `robo --continue`
6. `robo gateway status`
...
| `robo tools` | Configure which tools are enabled per platform |
| `robo setup` | Full setup wizard (configures everything at once) |
| `robo doctor` | Diagnose issues |
| `robo update` | Update to latest version |
| `robo gateway` | Start the messaging gateway |
| `robo --continue` | Resume last session |
...
- **[Moving to another machine](/reference/faq#exporting-robo-to-another-machine)** — `robo backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch

...
description: "Run Robo Agent directly on an Android phone with Termux"
---

# Robo on Android with Termux
...
Robo Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Robo from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `robo` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `robo` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/robo" "$PREFIX/bin/robo"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `robo` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.robo/.env`.
...
### `robo doctor` says ripgrep or Node is missing

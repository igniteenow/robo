...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Robo Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Robo instantly when PRs are opened or updated.
:::
...
- **Robo Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  robo gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.robo/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Robo can access GitHub. Start a chat:
...
Still in the chat, ask Robo to review a real PR:
...
A skill gives Robo consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.robo/skills/code-review
```

Create `~/.robo/skills/code-review/SKILL.md`:
...
Verify it loaded — start `robo` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Robo your team's standards:
...
2. Teach Robo your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
robo gateway status    # Is the gateway running?
robo cron list         # Is the job enabled?
```

...
```bash
robo teams-pipeline token-health
robo teams-pipeline token-health --force-refresh
```
...
```bash
robo teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
robo teams-pipeline maintain-subscriptions
robo teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Robo cron (recommended if you already run the Robo gateway)

Robo ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.robo/scripts/`. First create the script:

```bash
mkdir -p ~/.robo/scripts
cat > ~/.robo/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec robo teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.robo/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
robo cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/robo-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Robo Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=robo
EnvironmentFile=/etc/robo/env
ExecStart=/usr/local/bin/robo teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/robo-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Robo Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now robo-teams-pipeline-maintain.timer
systemctl list-timers robo-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/robo teams-pipeline maintain-subscriptions >> /var/log/robo/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.robo/.env` at the top of a wrapper script that crontab calls.
...
```bash
robo teams-pipeline subscriptions   # should show expirationDateTime advanced
robo teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
robo teams-pipeline list
robo teams-pipeline list --status failed
robo teams-pipeline show <job-id>
```

### Replay a stored job

```bash
robo teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
robo teams-pipeline fetch --meeting-id <meeting-id>
robo teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `robo teams-pipeline maintain-subscriptions --dry-run`
- inspect `robo teams-pipeline list --status failed`
...
- run `robo teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `robo teams-pipeline run`
...
- [ ] `robo teams-pipeline validate` returns an OK snapshot
- [ ] `robo teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Robo cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.

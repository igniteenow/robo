---
sidebar_position: 8
title: "Use Voice Mode with Robo"
description: "A practical guide to setting up and using Robo voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Robo
...
- you want Robo sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Robo.
...
## Step 1: make sure normal Robo works first
...
Robo supports both local and cloud speech stacks.
...
### If you use `robo setup`

If you choose NeuTTS in the setup wizard, Robo checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Robo transcribes and responds
...
- asking Robo to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Robo loop.

## Tuning CLI behavior

### Silence threshold

If Robo starts/stops too aggressively, tune:
...
Robo stays a normal chat bot, but can speak replies.
...
- you want Robo to function like a portable research or ops assistant
...
Robo joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Robo detects speech boundaries
- transcripts are posted in the associated text channel
- Robo responds in text and audio
...
1. get text Robo working
2. run `robo setup tts` to enable voice support

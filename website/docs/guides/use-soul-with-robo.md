---
sidebar_position: 7
title: "Use SOUL.md with Robo"
description: "How to use SOUL.md to shape Robo Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Robo

`SOUL.md` is the **primary identity** for your Robo instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Robo to feel like the same assistant every time you talk to it — or if you want to replace the Robo persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Robo should be
- what Robo should avoid stylistically
- how Robo should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Robo is and how Robo speaks
...
Robo now uses only the global SOUL file for the current instance:

```text
~/.robo/SOUL.md
```

If you run Robo with a custom home directory, it becomes:
...
Robo automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Robo does not overwrite it
- if the file exists but is empty, Robo adds nothing from it to the prompt

## How Robo uses it

When Robo starts a session, it reads `SOUL.md` from `ROBO_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Robo feels.
...
Robo already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Robo should sound.

# Avoid
What Robo should not do.

# Defaults
How Robo should behave when ambiguity appears.
```
...
Then restart Robo or start a new session.
...
### I edited SOUL.md but Robo still sounds the same

Check:
- you edited `~/.robo/SOUL.md` or `$ROBO_HOME/SOUL.md`
...
### Robo is ignoring parts of my SOUL.md

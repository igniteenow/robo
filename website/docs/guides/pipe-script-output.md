...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `robo send`."
---

# Pipe Script Output to Messaging Platforms

`robo send` is a small, scriptable CLI that pushes a message to any
messaging platform Robo is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | robo send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `robo
...
robo send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
robo send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
robo send --list

# Filter by platform
robo send --list telegram
```
...
Any platform Robo ships adapters for works as a target:
...
`robo send` resolves the message body in this order:

1. **Positional argument** — `robo send --to telegram "hi"`
2. **`--file PATH`** — `robo send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | robo send --to telegram`

When stdin is a TTY (no pipe), Robo does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  robo send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `robo send` reuses your Robo config, the same script works on
any host where Robo is installed — no need to export bot tokens into
...
`robo send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  robo send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | robo send \
...
```bash
./train.py --epochs 200 && \
  robo send --to telegram "training done" || \
  robo send --to telegram "training failed (exit $?)"
```
...
robo send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(robo send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `robo send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `robo send` calls
the platform's REST endpoint directly using credentials from
`~/.robo/.env` and `~/.robo/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `robo gateway start` and retry.
...
The listing is built from `~/.robo/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`robo gateway
...
| Approach | Multi-platform | Reuses Robo creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `robo send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`robo send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `robo send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `robo send` shares with cron delivery.

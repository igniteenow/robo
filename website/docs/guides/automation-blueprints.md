...
Copy-paste blueprints for common automation patterns. Each blueprint uses Robo's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`robo webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `robo webhook subscribe`) |
...
```bash
robo cron create "0 2 * * *" \
  "You are a project manager triaging the IgniteeNow/robo-engineer GitHub repo.

1. Run: gh issue list --repo IgniteeNow/robo-engineer --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
robo webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
robo cron create "0 9 * * 1" \
  "Scan the IgniteeNow/robo-engineer repo for documentation drift.

1. Run: gh pr list --repo IgniteeNow/robo-engineer --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (robo_cli/commands.py, robo_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (robo_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
robo cron create "0 6 * * *" \
  "Run a dependency security audit on the robo-engineer project.

1. cd ~/.robo/robo-engineer && source .venv/bin/activate
...
```bash
robo webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
robo webhook subscribe alert-triage \
...
```python title="~/.robo/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Robo-Monitor/1.0"})
...
robo cron create "every 30m" \
  ...
  --script ~/.robo/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
robo cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Robo Agent development." \
...
```bash
robo webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
robo webhook subscribe stripe-payments \
...
```bash
robo cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the robo-engineer codebase.

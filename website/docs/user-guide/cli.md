...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Robo Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Robo Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
Run `robo setup` and you're ready to `robo chat`.
:::
...
robo chat --provider openrouter  # Force OpenRouter
...
robo chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
robo --continue             # Resume the most recent CLI session (-c)
robo --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
robo chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
robo -w                         # Interactive mode in worktree
robo -w -z "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Robo CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Robo CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `ROBO_YOLO_MODE` is on (either `robo --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
On the `openai-codex` provider, `/usage` also shows any banked usage-limit resets on your ChatGPT account ("You have N resets banked - use /usage reset to activate"). `/usage reset` redeems one banked reset, fully restoring your 5-hour and weekly limits. Robo refuses to redeem while your limits aren't exhausted (a banked reset restores the full allowance, so spending it early wastes it) — pass `/usage reset --force` to redeem anyway.

### Session Resume Display

When resuming a previous session (`robo -c` or `robo --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Robo to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.robo/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Robo supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Robo replies |
...
```bash
robo -s robo-engineer-dev,github-auth
robo chat -s github-pr-workflow -s github-auth
```

Robo loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.robo/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.robo/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Robo..."
  # Add your own!
```
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Robo — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Robo is working, Robo prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Robo to the background — just like any terminal process. The shell prints a confirmation:

```
Robo Agent has been suspended. Run `fg` to bring Robo Agent back.
```
...
Resume this session with:
  robo --resume 20260225_143052_a1b2c3
...
```bash
robo --continue                          # Resume the most recent CLI session
robo -c                                  # Short form
robo -c "my project"                     # Resume a named session (latest in lineage)
robo --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
robo --resume "refactoring auth"         # Resume by title
robo -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `robo sessions rename <id> <title>` from the command line. Use `robo sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Robo's SQLite state database under `~/.robo/state.db`. The database keeps:
...
Robo immediately confirms the task and gives you back the prompt:

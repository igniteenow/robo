...
If you only run one Robo agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Robo agents that should all
...
(`ai.robo.gateway-<name>.plist`) or systemd user service
(`robo-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
robo profile create coder
robo profile create personal-bot
robo profile create research
...
```bash
robo config set gateway.multiplex_profiles true
robo gateway restart
```

Equivalently, in the default profile's `~/.robo/config.yaml`:
...
You do **not** run `robo gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `robo gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.robo/profiles/coder/.env`; the default listener key is rejected.
...
home). `robo status` reports the multiplexer and the profiles it serves;
`robo status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/robo-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: robo-gateways {start|stop|restart|status|list}"
}
...
```bash
robo-gateways start      # start every configured profile
robo-gateways stop       # stop every configured profile
robo-gateways restart    # restart all
robo-gateways status     # status across all
robo-gateways list       # delegates to `robo gateway list`
```

:::tip
The `default` profile is targeted with `robo gateway <action>` (no `-p`),
not `robo -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `robo -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.robo.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/robo-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.robo.gateway.plist` /
`robo-gateway.service`.
...
```bash
# Default profile
tail -f ~/.robo/logs/gateway.log
tail -f ~/.robo/logs/gateway.error.log

# Named profile
tail -f ~/.robo/profiles/<name>/logs/gateway.log
tail -f ~/.robo/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.robo/logs/gateway.log ~/.robo/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
robo logs -f                  # follow default profile
robo -p coder logs -f         # follow one profile
robo logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
robo profile list             # profiles + model + gateway state
robo-gateways status          # full status across every profile
launchctl list | grep robo    # macOS — PIDs and labels
systemctl --user list-units 'robo-gateway-*'   # Linux — units
```
...
The default profile uses `~/.robo/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
robo config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.robo/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=robo --why="gateways running" \
  sleep infinity &
...
`robo-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.robo/.env ~/.robo/profiles/*/.env
```

## Updating the code

`robo update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `robo gateway start` after a previous `robo gateway stop`. The
...
```bash
ps -ef | grep "robo_cli.*-p <profile>"
cat ~/.robo/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.robo.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.robo.gateway-<profile>.plist

# Linux
systemctl --user restart robo-gateway-<profile>.service
```

### Health check

```bash
robo doctor                  # default profile
robo -p <profile> doctor     # one profile
```

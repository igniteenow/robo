...
`~/.robo/config.yaml`, `~/.robo/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/robo`:

```text
/etc/robo/
├── config.yaml     # managed config layer (wins over ~/.robo/config.yaml)
└── .env            # managed env layer (wins over ~/.robo/.env + shell)
```
...
The location can be relocated with the `ROBO_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `ROBO_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Robo.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export ROBO_MANAGED_DIR=/opt/org/robo-policy
```

:::warning
A user who can set `ROBO_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `robo doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/robo/config.yaml` (managed) | `/etc/robo/.env` (managed) |
| 2 | `~/.robo/config.yaml` (user) | `~/.robo/.env` (user) |
...
```bash
robo config        # shows a header naming the managed source + the pinned keys
robo doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Robo refuses and names the source:

```bash
$ robo config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/robo/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `robo config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/robo

# Pin some config values for every user on this machine
sudo tee /etc/robo/config.yaml >/dev/null <<'YAML'
model:
  provider: openrouter
security:
  redact_secrets: true
...
sudo tee /etc/robo/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/robo
sudo chmod 0644 /etc/robo/config.yaml /etc/robo/.env
```

Changes take effect on the next Robo start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`robo doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Robo as `root`), managed scope is advisory.

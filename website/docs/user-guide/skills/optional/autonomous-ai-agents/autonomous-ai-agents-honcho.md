---
title: "Honcho — Configure and troubleshoot Honcho memory for Robo"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Robo"
---
...
Configure and troubleshoot Honcho memory for Robo.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `robo skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Robo

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Robo profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/robo#running-honcho-locally-with-robo

### Verify

```bash
robo honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Robo profile's AI peer
...
Honcho models conversations as interactions between **peers**. Robo creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Robo instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Robo run |
| `global` | Single session across all directories |

Manual override: `robo honcho map my-project-name`
...
Each Robo profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block robo.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `robo.coder` host block in `honcho.json`
...
```bash
robo honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Robo when Honcho memory is active.
...
Config file: `$ROBO_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `robo honcho setup`. Ensure `memory.provider: honcho` is in `~/.robo/config.yaml`.

### Memory not persisting across sessions
Check `robo honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `robo profile create <name> --clone`. For existing profiles: `robo honcho sync`.
...
| Command | Description |
|---------|-------------|
| `robo honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `robo honcho status` | Show resolved config, connection test, peer info for active profile |
| `robo honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `robo honcho disable` | Disable Honcho for the active profile |
| `robo honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `robo honcho peers` | Show peer identities across all profiles |
| `robo honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `robo honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `robo honcho sessions` | List known directory-to-session-name mappings |
| `robo honcho map <name>` | Map current working directory to a Honcho session name |
| `robo honcho identity` | Seed AI peer identity or show both peer representations |
| `robo honcho sync` | Create host blocks for all Robo profiles that don't have one yet |
| `robo memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `robo memory status` | Show active memory provider and config |
| `robo memory off` | Disable external memory provider |

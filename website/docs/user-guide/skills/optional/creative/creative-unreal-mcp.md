...
| | |
|---|---|
| Source | Optional — install with `robo skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Robo MCP catalog. The
...
Two halves, in this order: the editor side must be up before Robo connects.
...
### One-time, Robo side
    ...
    robo mcp install unreal-engine
...
`~/.robo/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Robo — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Robo connects
...
2. Start the Robo session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Robo
they appear as:
...
then happens at `robo mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Robo runs batched calls
...
- **Start order matters.** Editor + server up first, then the Robo
...
- **Timeouts on long operations.** Robo' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.robo/config.yaml` for

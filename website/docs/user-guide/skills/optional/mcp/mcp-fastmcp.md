...
| | |
|---|---|
| Source | Optional — install with `robo skills install official/mcp/fastmcp` |
...
| Related skills | [`robo-engineer`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-robo-engineer), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Robo or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Robo. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.robo/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.robo/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Robo integration, either:

- configure the server in `~/.robo/config.yaml` using the `native-mcp` skill, or
...
### Robo cannot see the deployed server

The server-building part may be correct while the Robo config is not. Load the `native-mcp` skill and configure the server in `~/.robo/config.yaml`, then restart Robo.

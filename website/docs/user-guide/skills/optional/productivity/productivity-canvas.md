...
| | |
|---|---|
| Source | Optional — install with `robo skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Robo Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${ROBO_HOME:-~/.robo}/.env`:
...
```bash
CANVAS="python $ROBO_HOME/skills/productivity/canvas/scripts/canvas_api.py"

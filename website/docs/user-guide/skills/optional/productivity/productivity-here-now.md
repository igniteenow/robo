...
| | |
|---|---|
| Source | Optional — install with `robo skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ROBO_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ROBO_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ROBO_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client robo
```
...
```bash
PUBLISH="${ROBO_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client robo
```
...
```bash
DRIVE="${ROBO_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `robo`)    |

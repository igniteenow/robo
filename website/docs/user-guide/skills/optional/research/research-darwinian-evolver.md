...
| | |
|---|---|
| Source | Optional — install with `robo skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Robo Agent |
| License | MIT |
...
import upstream classes into Robo itself.
...
```bash
mkdir -p ~/.robo/cache/darwinian-evolver && cd ~/.robo/cache/darwinian-evolver
...
```bash
cd ~/.robo/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.robo/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.robo/skills/research/darwinian-evolver
DE_DIR=~/.robo/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Robo core.
   Custom driver scripts under `~/.robo/skills/...` are user-side and fine.

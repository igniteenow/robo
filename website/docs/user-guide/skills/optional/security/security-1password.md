...
| | |
|---|---|
| Source | Optional — install with `robo skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Robo Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Robo terminal calls (desktop app flow only)
...
### Service Account (recommended for Robo)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${ROBO_HOME:-~/.robo}/.env` (the skill will prompt for this on first load).
...
## Robo Execution Pattern (desktop app flow)

Robo terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/robo-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/robo-op.sock"

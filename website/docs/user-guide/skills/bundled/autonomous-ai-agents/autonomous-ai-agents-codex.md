...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`robo-engineer`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-robo-engineer) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Robo terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Robo itself, `model.provider: openai-codex` uses Robo-managed Codex
OAuth from `~/.robo/auth.json` after `robo auth add openai-codex`. For the
...
When invoking the Codex CLI from a Robo gateway/service context (for example,

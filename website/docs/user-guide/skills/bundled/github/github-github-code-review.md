...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _robo_env="${ROBO_HOME:-$HOME/.robo}/.env"; [ -f "$_robo_env" ] && grep -q "^GITHUB_TOKEN=" "$_robo_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_robo_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
      GITHUB_TOKEN=$(uv run python3 "${ROBO_HOME:-$HOME/.robo}/skills/github/github-auth/scripts/git-credential-token.py")
    fi
  fi
fi
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Robo Agent\",
    \"comments\": [
...
```bash
source "${ROBO_HOME:-$HOME/.robo}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Robo Agent. Code looks clean — good test coverage, no security concerns."

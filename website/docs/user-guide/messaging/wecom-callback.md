...
Connect Robo to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Robo supports two WeCom integration modes:
...
> Run `robo gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Robo decrypts the message, queues it for the agent
...
(Use `robo gateway start` only after `robo gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Robo and the token the
...
in `~/.robo/.env` values around `=` will also break signature checks. After
fixing, restart `robo gateway run`.
...
Check `robo gateway run` logs for the bound host/port. If the adapter bound to

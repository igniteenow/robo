...
sidebar_position: 12
title: "Google Chat"
description: "Set up Robo Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Robo Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Robo
...
> Run `robo gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Robo can read (e.g.,
`~/.robo/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a robo restart)
...
- Principal: `robo-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Robo calls
...
- **App name**: whatever you want users to see ("Robo" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/robo-chat-events`.
...
`ADDED_TO_SPACE` event that Robo uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Robo

Add the Google Chat section to `~/.robo/.env`:
...
Send "hola" in the test DM. The bot posts a "Robo is thinking…" marker, then
...
`~/.robo/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Robo is thinking…").
...
Thread support: when a user replies inside a thread, Robo detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Robo session.
...
3. Download the JSON. Move it onto the host that runs Robo.
4. Register the client with Robo (run under the profile you want it scoped to):
...
robo -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Robo home (e.g.
`~/.robo/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.robo/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.robo/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Robo isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `robo gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Robo will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.robo/google_chat_user_tokens/<sanitized_email>.json` (filesystem

...
title: "Git Worktrees"
description: "Run multiple Robo agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Robo Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Robo so each session has a clean, isolated working directory.

## Why Use Worktrees with Robo?

Robo treats the **current working directory** as the project root:

- CLI: the directory where you run `robo` or `robo chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.robo/config.yaml`
...
git worktree add ../repo-feature feature/robo-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/robo-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Robo there:

```bash
cd ../repo-feature

# Start Robo in the worktree
robo
```
...
git worktree add ../repo-experiment-a feature/robo-a
git worktree add ../repo-experiment-b feature/robo-b
```
...
- Works on its own branch (`feature/robo-a` vs `feature/robo-b`).
...
- Robo checkpoint data under `~/.robo/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Robo experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/robo-checkpoints-docs`, `feature/robo-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Robo from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `robo -w` (Automatic Worktree Mode)

Robo has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `robo/robo-<hash>`).
...
```bash
robo -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `robo -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Robo session its own clean checkout.

...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Robo's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Robo instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
robo memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `robo` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Robo syncs them back at session init.
...
The `robo honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `robo memory setup honcho` (or run `robo memory setup` and pick it from the list); the `robo honcho` subcommand then appears on the next invocation.

```bash
robo memory setup honcho    # Configure Honcho directly (works before activation)
robo honcho status          # Connection status, config, and key settings
robo honcho setup           # Redirects to `robo memory setup` (post-activation alias)
robo honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
robo honcho peer            # Show or update peer names + dialectic reasoning level
robo honcho mode            # Show or set recall mode (hybrid/context/tools)
robo honcho tokens          # Show or set token budget for context and dialectic
robo honcho identity        # Seed or show the AI peer's Honcho identity
robo honcho sync            # Sync Honcho config to all existing profiles
robo honcho peers           # Show peer identities across all profiles
robo honcho sessions        # List known Honcho session mappings
robo honcho map             # Map current directory to a Honcho session name
robo honcho enable          # Enable Honcho for the active profile
robo honcho disable         # Disable Honcho for the active profile
```

## Migrating from `robo honcho`

If you previously used the standalone `robo honcho setup`:
...
No re-login or re-setup needed. Run `robo memory setup` and select "honcho" — the wizard detects your existing config.

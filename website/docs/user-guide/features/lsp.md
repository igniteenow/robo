...
Robo runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Robo
...
1. Robo captures a baseline of current diagnostics for the file.
...
…). Robo auto-detects the binary on PATH or in
`<ROBO_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Robo at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<ROBO_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`robo lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Robo installs both packages together when you
run `robo lsp install typescript` or auto-install fires on first
use.

## CLI

```
robo lsp status          # service state + per-server install status
robo lsp list            # registry, optionally --installed-only
robo lsp install <id>    # eagerly install one server
robo lsp install-all     # try every server with a known recipe
robo lsp restart         # tear down running clients
robo lsp which <id>      # print resolved binary path
```

`robo lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <ROBO_HOME>/lsp/bin
...
When `install_strategy: auto`, Robo installs binaries into
`<ROBO_HOME>/lsp/bin/`. NPM packages land in
`<ROBO_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Robo-owned and is
...
**`robo lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<ROBO_HOME>/lsp/bin/`. Run
`robo lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `robo lsp status`**
...
When `robo lsp status` shows a `Backend warnings` section, install
...
Check `~/.robo/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `robo lsp restart` to clear the set;

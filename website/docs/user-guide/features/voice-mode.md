...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Robo Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Robo Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Robo](/guides/use-voice-mode-with-robo).

For hands-free session start — saying "hey robo" (or any phrase) to open a fresh voice session on the CLI, TUI, or desktop app — see [Wake Word](/user-guide/features/wake-word).

## Prerequisites

Before using voice features, make sure you have:

1. **Robo Agent installed** — via the install script (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `robo model` or set your preferred provider credentials in `~/.robo/.env`
3. **A working base setup** — run `robo` to verify the agent responds to text before enabling voice

:::tip
The `~/.robo/` directory and default `config.yaml` are created automatically the first time you run `robo`. You only need to create `~/.robo/.env` manually for API keys.
:::
...
cd ~/.robo/robo-engineer && uv pip install -e ".[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
cd ~/.robo/robo-engineer && uv pip install -e ".[messaging]"

# Premium TTS (ElevenLabs)
cd ~/.robo/robo-engineer && uv pip install -e ".[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
cd ~/.robo/robo-engineer && uv pip install -e ".[all]"
```
...
Voice mode is available in both the **classic CLI** (`robo chat`) and the **TUI** (`robo --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.robo/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
robo                # Start the interactive CLI
```
...
:::tip
The record key is configurable via `voice.record_key` in `~/.robo/config.yaml` (default: `ctrl+b`).
:::
...
```bash
robo gateway        # Start the gateway (connects to configured platforms)
robo gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@robobot hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.robo/.env`:
```bash
...
```bash
robo gateway        # Start with existing configuration
```
...
If you are running Robo inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.robo/.env`
...
- Review gateway logs: `tail -f ~/.robo/logs/gateway.log`

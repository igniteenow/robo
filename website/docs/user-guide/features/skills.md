...
All skills live in **`~/.robo/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Robo at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `robo update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:
...
```bash
robo profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
robo skills opt-out            # stop future seeding — nothing on disk is touched
robo skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
robo skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `robo update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `robo skills opt-in`) to re-enable.

:::note Safe by default
`robo skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Robo installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Robo to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.robo/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
robo chat --toolsets skills -q "What skills do you have?"
robo chat --toolsets skills -q "Show me the axolotl skill"
```
...
description, the standard section order, Robo-tool framing, no invented
commands).
...
/home/user/.robo/cache/chart-q4-2025.png
...
When a missing value is encountered, Robo asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `robo setup` or `~/.robo/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `robo config migrate` prompts for unconfigured settings, and `robo config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.robo/skills/                  # Single source of truth
...
and `examples/`. Unreferenced repository files are not copied. Robo scans the
...
If you maintain skills outside of Robo — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Robo to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.robo/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.robo/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Robo process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Robo ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.robo/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.robo/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `robo bundles list`.
...
robo bundles create research

# Overwrite an existing bundle
robo bundles create backend-dev --skill ... --force

# Delete a bundle
robo bundles delete backend-dev

# Re-scan ~/.robo/skill-bundles/ and report changes
robo bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.robo/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.robo/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.robo/pending/skills/` and are
...
```bash
robo skills browse                              # Browse all hub skills (official first)
robo skills browse --source official            # Browse only official optional skills
robo skills search kubernetes                   # Search all sources
robo skills search react --source skills-sh     # Search the skills.sh directory
robo skills search https://mintlify.com/docs --source well-known
robo skills inspect openai/skills/k8s           # Preview before installing
robo skills install openai/skills/k8s           # Install with security scan
robo skills install official/security/1password
robo skills install skills-sh/vercel-labs/json-render/json-render-react --force
robo skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
robo skills list --source hub                   # List hub-installed skills
robo skills check                               # Check installed hub skills for upstream updates
robo skills update                              # Reinstall hub skills with upstream changes when needed
robo skills audit                               # Re-scan all hub skills for security
robo skills uninstall k8s                       # Remove a hub skill
robo skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
robo skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
robo skills publish skills/my-skill --to github --repo owner/repo
robo skills snapshot export setup.json          # Export skill config
robo skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Robo. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `robo skills search <query> --source skills-sh`. Robo resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Robo currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Robo repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Robo can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
robo skills search https://mintlify.com/docs --source well-known
robo skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
robo skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Robo can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
robo skills install openai/skills/k8s
robo skills tap add myorg/skills-repo
```
...
categorization, no Robo-side changes required.
...
- Robo source id: `clawhub`

#### 6. LobeHub (`lobehub`)

Robo can search and convert agent entries from LobeHub's public catalog into installable Robo skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Robo source id: `lobehub`

#### 7. browse.sh (`browse-sh`)

Robo integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Robo' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Robo source id: `browse-sh`
- Trust level: `community`

```bash
robo skills search airbnb --source browse-sh
robo skills inspect browse-sh/airbnb.com/search-listings-ddgioa
robo skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Robo also fetches explicitly referenced files under `references/`, `templates/`, `scripts/`, `assets/`, and `examples/`, then scans and installs the complete bundle.

- Robo source id: `url`
- Identifier: the URL itself (no prefix needed)
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. Robo does not enumerate or copy unrelated files from the host.

```bash
robo skills install https://sharethis.chat/SKILL.md
robo skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
robo skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `robo skills update` re-fetches from the same URL automatically when you want to refresh.
...
`robo skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Robo | Always trusted |
...
```bash
robo skills check          # Report which installed hub skills changed upstream
robo skills update         # Reinstall only the skills with updates available
robo skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Robo users add with `robo skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.robo.tags`, `version`, `author`, `platforms`, `metadata.robo.config`).
...
Robo discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Robo user can subscribe and install:

```bash
robo skills tap add my-org/robo-skills
robo skills search deploy
robo skills install my-org/robo-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.robo/skills/.hub/taps.json`:
...
The `robo skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `robo skills tap list` shows the effective path per tap.
...
```bash
robo skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.robo/skills/.hub/taps.json` (created on demand).

## Bundled skill updates (`robo skills reset`)

Robo ships with a set of bundled skills in `skills/` inside the repo. On install and on every `robo update`, a sync pass copies those into `~/.robo/skills/` and records a manifest at `~/.robo/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Robo recomputes the hash of your local copy and compares it to the origin hash:
...
robo skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `ROBO_HOME`, so `robo -p coder skills reset <name>` only affects that profile.
:::
...
Official optional skills still use identifiers like `official/security/1password` and `official/security/oss-forensics`.

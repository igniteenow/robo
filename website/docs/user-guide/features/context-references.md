...
Type `@` followed by a reference to inject content directly into your message. Robo expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Robo env: `$ROBO_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$ROBO_HOME/skills/.hub/`

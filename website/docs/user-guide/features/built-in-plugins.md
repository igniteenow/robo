...
description: "Plugins shipped with Robo Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Robo ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.robo/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Robo Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.robo/plugins/<name>/`
3. **Project** — `./.robo/plugins/<name>/` (requires `ROBO_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `robo_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `robo memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `robo plugins list` and the interactive `robo plugins` UI), but none load until you explicitly enable them:

```bash
robo plugins enable disk-cleanup
```

Or via `~/.robo/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Robo. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
robo plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `robo plugins enable <name>`.
...
| `robo-achievements` | dashboard tab | Steam-style collectible badges generated from your real Robo session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `robo memory` and `robo plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ROBO_HOME` or `/tmp/robo-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under ROBO_HOME | always | Never |
...
**State** — everything lives at `$ROBO_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `ROBO_HOME` or `/tmp/robo-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `robo plugins enable disk-cleanup` (or check the box in `robo plugins`).

**Disabling again:** `robo plugins disable disk-cleanup`.
...
**Enabling:** `robo plugins enable security-guidance` (or check the box in `robo plugins`).

**Disabling again:** `robo plugins disable security-guidance`.
...
```bash
robo tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Robo and the next turn ships a trace.
...
Then put the credentials in `~/.robo/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Robo turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `ROBO_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Robo session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `robo chat` session lives under one Langfuse session.

**Verify:**

```bash
robo plugins list                 # observability/langfuse should show "enabled"
robo chat -q "hello"              # check the Langfuse UI for a "Robo turn" trace
```
...
| `ROBO_LANGFUSE_RELEASE` | — | Release/version tag |
| `ROBO_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `ROBO_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `ROBO_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Robo-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Robo-prefixed wins when both are set.
...
**Disabling:** `robo plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, status) saved under `~/.robo/workspace/meetings/<meeting_id>/`

**Setup:**

```bash
robo plugins enable google_meet
robo meet setup   # preflight: playwright, chromium, auth file
robo meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `robo plugins disable google_meet`. Any saved transcripts stay in `~/.robo/workspace/meetings/` until you remove them.

### robo-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Robo session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Robo feature changes.

**How it works:**

- Scans your entire `~/.robo/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$ROBO_HOME/plugins/robo-achievements/state.json`
...
| Secret | Hidden until Robo detects the first related signal in your history |

**API** — routes mount under `/api/plugins/robo-achievements/`:
...
**State files** — live under `$ROBO_HOME/plugins/robo-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Robo updates. |
...
**Enabling:** Nothing to enable — `robo-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `robo dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/robo-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.robo/plugins/robo-achievements/` that ships no dashboard. The plugin's state files under `$ROBO_HOME/plugins/robo-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Robo plugin — see [Build a Robo Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.robo/plugins/<name>/`
- Manifest source is reported as `bundled` in `robo plugins list`

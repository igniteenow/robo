...
Robo Agent can drive your desktop — clicking, typing, scrolling,
...
```
robo computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `robo computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `robo tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Robo app). `robo computer-use doctor` will tell you which permission is missing. |
...
```
robo -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.robo/config.yaml`.

## Permission modes and logged-in browser profiles

Robo maps its existing approval UX onto cua-driver 0.10's immutable daemon
modes. There is no second permission toggle to keep in sync:

| Robo session | cua-driver mode | Human intervention | `existing_profile` |
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Robo approvals; Cua stops at its protected boundary | Refuses unless a certified protected host is available; Robo does not claim one today |
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Robo risk acceptance; no runtime Cua prompts | Allowed within Cua's built-in, managed, and user policy ceilings |

The unrestricted daemon is private to that Robo session. Turning `/yolo`
...
daemon's mode or grants another Robo conversation the same authority.
...
## `robo computer-use doctor` — your first triage stop

`robo computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ robo computer-use doctor
...
Robo intentionally keeps its skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Robo-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Robo skill** —
...
Code, Codex, OpenCode, and Antigravity; **Robo
...
Robo applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.robo/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Robo applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Robo agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Robo agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Robo warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Robo from inside the RDP / console
...
```
ROBO_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
ROBO_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Robo disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Robo sets
...
When it's on, `robo computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Robo at a binary you built from source instead
of the published release. Robo resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`ROBO_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Robo testing (see notes).
...
### Option B — point Robo straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`ROBO_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
ROBO_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
ROBO_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Robo is using your build

- `robo computer-use status` prints the resolved binary path and
  version.
- `robo computer-use doctor` confirms the binary is reachable and
...
- **Robo spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Robo YOLO, Robo instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Robo
  change needed either way.
- **Local builds skip the version check.** Robo warns when the
...
**First action when anything's off: run `robo computer-use doctor`.**
...
Run `robo computer-use install` to fetch the cua-driver binary, or
run `robo tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `robo computer-use doctor` will
...
- **Robo-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Robo `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Robo (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Robo among them)

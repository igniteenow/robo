...
Robo Agent includes two model-callable web tools backed by multiple providers:

- **`web_search`** — search the web and return ranked results
- **`web_extract`** — fetch and extract readable content from one or more URLs

Both are configured through a single backend selection. Providers are chosen via `robo tools` or set directly in `config.yaml`.
...
### Quick setup via `robo tools`

Run `robo tools`, navigate to **Web Search & Extract**, and pick a provider. The wizard prompts for the required URL or API key and writes it to your config.
...
To enable JSON responses for Robo, add the following override:
...
Then select SearXNG as the search backend in `~/.robo/config.yaml`:
...
Or set via `robo tools` → Web Search & Extract → SearXNG.
...
With this config, Robo uses SearXNG for all search queries and Firecrawl for URL extraction — combining free search with high-quality extraction.
...
If no backend is explicitly configured, Robo picks the first available one based on which credentials are set:
...
Run `robo setup` to see which web backend is detected:

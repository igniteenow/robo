...
The web dashboard is a browser-based UI for managing your Robo Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses a self-hosted OIDC provider or the bundled username/password provider — see [Authentication (gated mode)](#authentication-gated-mode) below.
:::
...
```bash
# Custom port
robo dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
robo dashboard --host 0.0.0.0

# Start without opening browser
robo dashboard --no-open
```

## Talk instead of typing

The chat has a **talk** button at the bottom-right of the terminal. Click it,
speak, click again. The recording goes to the Robo process running on your own
computer (the one `robo dashboard` starts), which transcribes it with the same
transcriber the desktop app uses: by default the local Whisper model, offline.
The words are typed into the chat's input line so you can read them before
pressing Enter. The browser asks for microphone access the first time. There is
no hosted Robo service anywhere; the only way audio leaves your machine is if
you set `stt.provider` to a cloud transcriber such as Groq or OpenAI.
...
child with the selected profile's `ROBO_HOME`, so the conversation runs
...
processes (manage them via `robo -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `robo-engineer` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.robo/robo-engineer && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.robo/robo-engineer && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `robo dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `robo dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- Node.js (same requirement as `robo --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`cd ~/.robo/robo-engineer && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
```ini
[Service]
EnvironmentFile=%h/.robo/.env
ExecStart=/path/to/venv/bin/python -m robo_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.robo/.env` containing:

```bash
ROBO_DASHBOARD_BASIC_AUTH_USERNAME=admin
ROBO_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
ROBO_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `robo config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated Robo instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `robo profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.robo/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `robo skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.

### MCP

Manage [MCP](./mcp) servers without the CLI. The same `mcp_servers`
block in `config.yaml` that `robo mcp` reads from.
...
Connect Robo to any messaging platform from the browser — full parity with
`robo setup gateway`. The page lists every supported channel (Telegram,
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Robo versions, CPU core count + utilization, memory, disk usage of the Robo home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Robo version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `robo update` in the background. On Docker/Nix installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `robo curator`.
...
This re-reads `~/.robo/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`ROBO_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
| `GET /api/mcp/catalog` | Browse the bundled MCP catalog |
...
| `GET /api/robo/update/check` | Report update availability (commits behind, install method) without applying. For git installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — Robo Agent engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
- **[Self-hosted OIDC](#self-hosted-oidc-provider)** — for bringing your own identity provider via standard OpenID Connect (Keycloak, Auth0, Okta, Google, GitHub via an OIDC bridge, etc.). Recommended for any dashboard reachable over the public internet, including for a [remote Robo Desktop connection](#connecting-robo-desktop-to-a-remote-backend).
...
|-------|-----------|----------|
| `robo dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `robo dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no auth plugin configured), `robo dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.

When you run `robo dashboard --host 0.0.0.0` **interactively** (a real terminal) and no provider is configured yet, Robo doesn't just fail — it offers to set one up on the spot: pick **username & password** (writes `dashboard.basic_auth` to `config.yaml` and you're running in seconds). Non-interactive callers — Docker/s6, CI, piped runs — skip the prompt and hit the fail-closed error above, so an unattended deploy still never starts without auth.
...
The username/password provider is intended for self-hosted / on-prem / homelab dashboards on a **trusted network**, or reachable only over a **VPN**. It protects a single shared credential with no external identity provider, MFA, or per-user accounts behind it, so it is **not suitable for exposing a dashboard directly to the public internet**. For an internet-facing dashboard, use [self-hosted OIDC](#self-hosted-oidc-provider) or a [custom OAuth](#custom-providers) provider instead.
:::

#### Configuration

Like other bundled providers, it reads from `config.yaml` (canonical) with environment variables winning when set non-empty. It activates only when `username` plus either `password_hash` (preferred) or `password` are configured — otherwise it's a no-op, so OAuth users and loopback operators are unaffected.
...
|---------|-----------|-------|
| `ROBO_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `ROBO_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `ROBO_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `ROBO_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `ROBO_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.robo/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.robo/.env <<EOF
ROBO_DASHBOARD_BASIC_AUTH_USERNAME=admin
ROBO_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
...
```bash
robo dashboard --host 0.0.0.0 --port 9119 --no-open
```
...
If you run your own identity provider, the bundled `plugins/dashboard_auth/self_hosted` plugin authenticates the dashboard against it using **standard OpenID Connect** — no per-IDP code required. It's verified against and works with any conformant OIDC server:

> **Authentik · Keycloak · Zitadel · Authelia · Auth0 · Okta · Google · …**

Like other bundled providers, it auto-loads and only registers itself once it's configured, so it's a no-op for loopback dashboards.
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/robo/   # required
      client_id: robo-dashboard                              # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `ROBO_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `ROBO_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `ROBO_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-robo.json` — it defines a `robo` realm, a **public PKCE client** (`robo-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "robo-dashboard",
      "name": "Robo Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-robo.json:/opt/keycloak/data/import/realm-robo.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/robo/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/robo`). The admin console is at
...
```bash
export ROBO_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/robo"
export ROBO_DASHBOARD_OIDC_CLIENT_ID="robo-dashboard"
export ROBO_DASHBOARD_PUBLIC_URL="http://localhost:9119"
robo dashboard --host 0.0.0.0 --port 9119 --no-open
```

`ROBO_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → robo-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `ROBO_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/robo"
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `ROBO_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `ROBO_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `robo_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `robo_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `robo_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$ROBO_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.

### Custom providers

To plug a custom OAuth provider (e.g. Google, GitHub, a bespoke OIDC setup), create a plugin that registers a `DashboardAuthProvider`:

```python
# ~/.robo/plugins/dashboard-auth-myidp/__init__.py
from robo_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
The bundled first consumer is the **drain** provider (`plugins/dashboard_auth/drain`): the operator provisions a per-agent secret via `ROBO_DASHBOARD_DRAIN_SECRET`, and the provider verifies inbound bearer tokens against it with a constant-time compare, registering `/api/gateway/drain` as token-authable. It **fails closed** — a weak/short secret (< 256 bits) is rejected at registration and the endpoint stays disabled; it's a no-op when the env var is unset. Behavioural knobs (`scope`, `min_secret_chars`) live under `dashboard.drain_auth` in `config.yaml`.
...
Robo Desktop can drive a Robo backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateway → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see the [Robo Desktop](/user-guide/desktop) page.)

You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **[self-hosted OIDC](#self-hosted-oidc-provider)**. The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.

The recipe below uses the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Self-hosted OIDC provider](#self-hosted-oidc-provider).

### On the backend (the remote machine)

```bash
# 1. Set the dashboard login credentials in ~/.robo/.env (secrets file, 0600).
cat >> ~/.robo/.env <<'EOF'
ROBO_DASHBOARD_BASIC_AUTH_USERNAME=admin
ROBO_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
robo dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `ROBO_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.

If you run the dashboard as a systemd service, `~/.robo/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.robo/.env`, so the credentials are in the environment at boot.
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/robo` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `ROBO_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.
...
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `ROBO_DASHBOARD_BASIC_AUTH_USERNAME` / `ROBO_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `ROBO_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `robo_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `robo update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `robo dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Robo Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Robo Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
| **Sky Blue** (`igniteenow-blue`) | Vivid blue accents with airy spacing |

...
description: "Customize Robo Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Robo Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `ROBO_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Robo is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Robo now seeds a default `SOUL.md` automatically in:

```text
~/.robo/SOUL.md
```

More precisely, it uses the current instance's `ROBO_HOME`, so if you run Robo with a custom home directory, it will use:
...
- Robo creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Robo loads `SOUL.md` only from `ROBO_HOME`
- Robo does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Robo falls back to a built-in default identity
...
If Robo loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `ROBO_HOME`, the personality belongs to the Robo instance itself.

That also makes it easier to teach users:
- "Edit `~/.robo/SOUL.md` to change Robo' default personality."
...
- how Robo should handle uncertainty, disagreement, or ambiguity
...
## What Robo injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Robo falls back to a built-in default identity ("You are Robo Agent, an intelligent AI assistant..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Robo ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Robo, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Robo its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.robo/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.robo/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-robo)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Robo speaks
- `display.skin` and `/skin` affect how Robo looks in the terminal

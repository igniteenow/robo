...
description: Generate images via FAL.ai — 11 models including FLUX 2, GPT Image (1.5 & 2), Nano Banana Pro, Ideogram, Recraft V4 Pro, Krea 2, and more, selectable via `robo tools`.
...
Robo Agent generates images from text prompts via FAL.ai. Eleven models are supported out of the box, each with different speed, quality, and cost tradeoffs. The active model is user-configurable via `robo tools` and persists in `config.yaml`.
...
Navigate to **🎨 Image Generation**, then the picker shows all supported FAL.ai models in a column-aligned table — arrow keys to navigate, Enter to select:
...
`max_parallel_requests` defaults to `4`. Robo clamps it to at least one and
...
as a tool the chat model may call, and Robo cannot force the call — the
...
3. **Submission** — `_submit_fal_request()` routes via direct FAL credentials.
...
- **Temporary URLs** — backends return hosted URLs that expire after hours/days; Robo materializes them to the local cache so delivery still works after expiry

...
When Robo Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.robo/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.robo/config.yaml` (switchable per session
via `/personality`).
...
If you use Robo' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.robo/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Robo Agent's deliverable

...
Robo can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<ROBO_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Robo renders the sprite at full fidelity when your
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `robo pets list [query] [--limit N]` |
| List installed pets | `robo pets list --installed` |
| Install a pet | `robo pets install <slug> [--select] [--force]` |
| Set the active pet | `robo pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `robo pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `robo pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `robo pets off` |
| Remove an installed pet | `robo pets remove <slug>` |
| Diagnose setup | `robo pets doctor` |

`robo pets show` flags:
...
Beyond installing pre-made pets from the gallery, Robo can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `robo pets` → the generate flow.
...
2. **Hatch** — the chosen base is used as a reference image to generate one grounded animation row per Robo state (idle, thinking, tool use, etc.), which are deterministically sliced into frames and packed into a standard petdex/Codex atlas (8×9 grid of 192×208 cells). The result is a valid spritesheet you keep — and could `petdex submit`.

### Image backend

Generation uses the active [image-generation provider](/user-guide/features/image-generation), but it requires **reference-image grounding** so each animation row stays the same character as the base. Reference-capable backends: **OpenRouter**, **OpenAI** (`gpt-image-2`), and **Krea**. OpenRouter runs a quality-first model chain by default.

- Resolution order prefers OpenAI → OpenRouter.
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `robo tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `ROBO_PET_IMAGE_PROVIDER` env var (e.g. `ROBO_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Robo is
...
Run `robo pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Robo uses its own
  profile-scoped `<ROBO_HOME>/pets/` instead — install through `robo pets`.

## See also

- The [`robo-engineer` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-robo-engineer.md)

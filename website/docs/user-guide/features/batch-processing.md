...
Batch processing lets you run the Robo agent across hundreds or thousands of prompts in parallel, generating structured trajectory data. This is primarily used for **training data generation** — producing ShareGPT-format trajectories with tool usage statistics that can be used for fine-tuning or evaluation.
...
Batch runs spin up many concurrent agent sessions, each making model calls and tool calls. Set up your provider with `robo setup`, then point `--model` at it.
:::

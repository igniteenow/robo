...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Robo' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey Robo")

The wake word turns Robo into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Robo listens in the background for a spoken
trigger phrase. Say it, and Robo starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** Robo backend (for example a
...
Robo supports **client capture** for that case:
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey robo"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey robo"** — a model for it ships with Robo, so
...
```bash
cd ~/.robo/robo-engineer && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `robo` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.robo/config.yaml`,
...
wake_word:
  ...
  phrase: "hey robo"        # cosmetic label only — detection is keyed by the model/keyword below
  ...
  openwakeword:
    model: hey_robo         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey robo" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey robo" utterances are missed.
...
default) to let Robo choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Robo installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Robo surfaces, and `surface` picks which one
...
"Hey Robo" works out of the box — the bundled openWakeWord model
(`model: hey_robo`) is the default. To wake on something else, the easiest
...
command (`robo -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.robo/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("robo" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey Robo" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey robo"
  porcupine:
    keyword: ~/.robo/wakewords/hey_robo.ppn
```

Set your access key in `~/.robo/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `robo tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `robo-engineer[wake]`).
...
but the phrase never fires. Robo detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the Robo
backend (it may appear as your terminal, `python`, or Robo), then toggle the
...
```bash
robo config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
robo config set wake_word.input_device null
```

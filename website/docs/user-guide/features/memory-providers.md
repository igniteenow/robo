...
Robo Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
robo memory setup      # interactive picker + configuration
robo memory status     # check what's active
robo memory off        # disable external provider
```

You can also select the active memory provider via `robo plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.robo/config.yaml`:
...
When a memory provider is active, Robo automatically:
...
**Setup Wizard:**
```bash
robo memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `robo honcho setup` command still works (it now redirects to `robo memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$ROBO_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$ROBO_HOME/honcho.json` > `~/.robo/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/IgniteeNow/robo-engineer/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/robo).
...
:::tip Migrating from `robo honcho`
If you previously used `robo honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Robo profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Robo profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Robo profile. Host key `robo` → default; `robo.<profile>` for others. |
...
```bash
robo profile create coder --clone
```

`--clone` creates a `robo.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Robo profile, creates host blocks for any profile without one, inherits settings from the default `robo` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `robo memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/IgniteeNow/robo-engineer/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/robo).
...
# Then configure Robo
robo memory setup    # select "openviking"
# Or manually:
robo config set memory.provider openviking
```

`robo memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.robo/.env`, and for named profiles use
`~/.robo/profiles/<profile>/.env`.
...
`OPENVIKING_AGENT` is Robo' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
robo memory setup    # select "mem0" → "Platform"
# Or manually:
robo config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.robo/.env
```

**Setup (OSS):**
```bash
robo memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
robo memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
robo memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
robo memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
robo memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.robo/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.robo/.env
```
...
**Config:** `$ROBO_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.robo/.env`.
...
| `user_id` | `robo-user` | User identifier |
| `agent_id` | `robo` | Agent identifier |
...
**Switching modes:** Re-run `robo memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
robo memory setup    # select "hindsight"
# Or manually:
robo config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.robo/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p robo ui start`

**Config:** `$ROBO_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `robo` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Robo Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/IgniteeNow/robo-engineer/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
robo memory setup    # select "holographic"
# Or manually:
robo config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.robo-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ROBO_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
robo memory setup    # select "retaindb"
# Or manually:
robo config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.robo/.env
```
...
# Then configure Robo
robo memory setup    # select "byterover"
# Or manually:
robo config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$ROBO_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=robo), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
**Setup:**
```bash
robo memory setup    # select "supermemory"
# Or manually:
robo config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.robo/.env
```
...
Before running `robo memory setup`, set `base_url` in
`$ROBO_HOME/supermemory.json`:
...
Then run `robo memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$ROBO_HOME/supermemory.json`
...
| `container_tag` | `robo` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `robo-{identity}` → `robo-coder`) to isolate memories per Robo profile
...
| **Requires** | `pip install robo-memori` + `robo-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install robo-memori
robo-memori install
robo config set memory.provider memori
robo memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `robo-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$ROBO_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$ROBO_HOME/` so each profile has its own credentials

...
Robo Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.robo/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Robo stores session history so it can resume conversations, but it does not
...
Robo explicitly injects for that turn.
...
For example, if a user sends an image and asks Robo to make a meme from it,
Robo may inspect that image once with vision and run an image-processing
...
`robo sessions prune` only when you want to delete old ended sessions from
storage. If `state.db` has simply grown large, start with the non-destructive
option first: `robo sessions optimize` merges FTS5 index segments and
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`robo` or `robo chat`) |
...
robo -c "my project"   # → resumes "my project #3"
```
...
robo --resume 20250305_091523_a1b2c3d4
robo -r 20250305_091523_a1b2c3d4

# Resume by title
robo --resume "refactoring auth"

# Or with the chat subcommand
robo chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `robo sessions list`.
...
```bash
robo --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`robo sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
robo sessions list --workspace my-project
robo sessions list --workspace ~/code/robo-engineer
```

### Conversation Recap on Resume

When you resume a session, Robo displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Robo session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.robo/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `robo -r "<title>"` from the shell) and pick up where the platform left off.
...
Robo automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `robo sessions list` or `robo sessions browse`.
...
```bash
robo sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Robo creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`robo -c "my project"`), it automatically picks the most recent session in the lineage.
...
Robo provides a full set of session management commands via `robo sessions`:
...
# Filter by platform
robo sessions list --source telegram

# Show more sessions
robo sessions list --limit 50
```
...
`robo sessions export` is one surface for every export format, selected with `--format`:
...
robo sessions export backup.jsonl

# Export sessions from a specific platform
robo sessions export telegram-history.jsonl --source telegram

# Export a single session
robo sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
robo sessions export backup.jsonl --redact
```
...
robo sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
robo sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
robo sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
robo sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `robo-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
robo sessions export --format trace

# One session to a local trace file
robo sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
robo sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.robo/session-exports`).

```bash
# Export one session to Markdown
robo sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
robo sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
robo sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
robo sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
robo sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
robo sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
robo sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
robo sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
robo sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
robo sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
robo sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
robo sessions prune --newer-than 5h

# Explicit window with absolute timestamps
robo sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
robo sessions prune --source telegram
robo sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
robo sessions prune --newer-than 5h --title "smoke test"   # title substring
robo sessions prune --older-than 30 --max-messages 3        # tiny sessions
robo sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
robo sessions prune --model gpt-5 --older-than 1w           # by model (substring)
robo sessions prune --provider openrouter --older-than 60   # by billing provider
robo sessions prune --branch feature/old-experiment         # by git branch
robo sessions prune --user 12345678 --chat-type group       # by messaging origin
robo sessions prune --max-tokens 500 --older-than 7         # by token usage
robo sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
robo sessions prune --newer-than 5h --dry-run

# Skip confirmation
robo sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `robo sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `robo sessions prune` keeps the 90-day cutoff. Every
...
`robo sessions archive` takes the same filters as `prune` but soft-hides
...
robo sessions archive --newer-than 5h

# Archive by title substring, preview first
robo sessions archive --title "dry run" --dry-run
robo sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `robo sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`robo sessions list` and `/resume` but remain in the database and can be
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`robo insights`](/reference/cli-commands#robo-insights).
...
When Robo cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Robo uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Robo in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.robo/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.robo/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.robo/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.robo/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.robo/sessions/sessions.json` is a **legacy mirror** of it,
...
`robo sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.robo/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `robo sessions list`, the cause is
`state.db` not receiving them — run `robo sessions repair` and watch for a
...
`*.jsonl` files in `~/.robo/sessions/`. They are no longer written or
read by Robo. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Robo process in the same `ROBO_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.robo/config.yaml`:
...
robo sessions delete <session_id>

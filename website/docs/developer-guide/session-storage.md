...
Robo Agent uses a SQLite database (`~/.robo/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `robo_state.py`


## Architecture Overview

```
~/.robo/state.db (SQLite, WAL mode)
...
Abridged — see `SCHEMA_SQL` in `robo_state.py` for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `robo_state.py` for the exact SQL.
...
Multiple robo processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.robo/state.db
...
Default path: `~/.robo/state.db`

This is derived from `robo_constants.get_robo_home()` which resolves to
`~/.robo/` by default, or the value of `ROBO_HOME` environment variable.

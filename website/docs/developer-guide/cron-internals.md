...
description: "How Robo stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `robo_cli/cron.py` | CLI `robo cron` subcommands |
...
Jobs are stored in `~/.robo/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `robo cron` commands are run or during active CLI sessions.

### Managed cron (Chronos) for scale-to-zero

Hosted gateways could optionally run a **Chronos** provider (`cron.provider: chronos`)
instead of the built-in ticker, letting an idle gateway scale to zero while still
firing cron jobs via an external managed scheduler. That external scheduler was
Ignitee Now-provided infrastructure and is no longer available; `resolve_cron_scheduler()`
falls back to the built-in ticker whenever Chronos is unconfigured or unreachable,
so cron never loses its trigger — every deployment now runs on the built-in ticker.
...
```python
# ~/.robo/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.robo/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `robo cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `robo cron` CLI provides direct job management:

```bash
robo cron list                    # Show all jobs
robo cron create                  # Interactive job creation (alias: add)
robo cron edit <job_id>           # Edit job configuration
robo cron pause <job_id>          # Pause a running job
robo cron resume <job_id>         # Resume a paused job
robo cron run <job_id>            # Trigger immediate execution
robo cron remove <job_id>         # Delete a job
```

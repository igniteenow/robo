...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Robo already has applies. The plugin never sees a
...
[`robo-example-plugins`](https://github.com/IgniteeNow/robo-example-plugins/tree/main/plugin-llm-example)
repo (companion repo for reference plugins — not bundled with
robo-engineer itself). For the async surface (`acomplete()` /
`acomplete_structured()` with `asyncio.gather()`), see
[`plugin-llm-async-example`](https://github.com/IgniteeNow/robo-example-plugins/tree/main/plugin-llm-async-example)
in the same repo.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Robo provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Robo provider
        # (must be one Robo already knows about — same names as
        # `robo model` and config.yaml model.provider).
        ...
        # plugin sends — Robo does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.robo/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Robo' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Robo subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/IgniteeNow/robo-engineer/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/IgniteeNow/robo-engineer/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/IgniteeNow/robo-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/IgniteeNow/robo-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`

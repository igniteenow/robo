...
Plugins can launch and supervise fresh Robo child sessions without importing
...
active agent turn fails closed with `No active Robo parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Robo can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Robo's existing unsafe-tool block remains enforced.

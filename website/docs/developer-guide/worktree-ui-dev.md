...
| Variable | Meaning |
|----------|---------|
| `ROBO_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `ROBO_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `ROBO_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Robo itself — they're private to these helpers. The variables Robo *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `robo --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_robo_root)" || { echo "htui: not in a Robo checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$ROBO_MAIN_CHECKOUT/.venv/bin/python" -m robo_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `ROBO_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_robo_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `ROBO_TUI_DIR` are mutually exclusive
`ROBO_TUI_DIR` points Robo at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `robo --tui --dev` exits with an error. Run `unset ROBO_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_robo_root)" || { echo "hgui: not in a Robo checkout" >&2; return 1; }
  deps="${ROBO_GUI_DEPS_CHECKOUT:-$ROBO_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _robo_link_deps "$desktop" "$deps/apps/desktop"
    _robo_link_deps "$root" "$deps"
  else
  ...
  trap '_robo_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    ROBO_DESKTOP_ROBO_ROOT="$root" \
    ROBO_DESKTOP_PYTHON="$ROBO_MAIN_CHECKOUT/.venv/bin/python" \
    ROBO_DESKTOP_IGNORE_EXISTING=1 \
    ROBO_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `ROBO_DESKTOP_ROBO_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `robo`. |
| `ROBO_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `ROBO_DESKTOP_IGNORE_EXISTING` | Ignores any `robo` on `PATH` so it can't shadow the worktree. |
| `ROBO_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Robo checkout.
_robo_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/robo_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_robo_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'robo_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `robo --tui --dev` and the `ROBO_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/IgniteeNow/robo-engineer/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `ROBO_*` variable Robo reads

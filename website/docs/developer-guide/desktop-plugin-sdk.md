...
title: "Desktop Plugin SDK (@robo/plugin-sdk)"
description: "Extend the native Robo Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Robo Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `RoboPlugin`.
It imports one module — `@robo/plugin-sdk` — and gets everything: the app's
...
`$ROBO_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Robo. This page is the **native
desktop app** (`robo desktop`) SDK — the `@robo/plugin-sdk` module and
`$ROBO_HOME/desktop-plugins/`. The **web dashboard** (`robo dashboard`) has
its own, unrelated plugin system on `window.__ROBO_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Robo Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$ROBO_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

Both take the same `RoboPlugin` contract, appear in **Settings → Plugins**, and
...
Create `$ROBO_HOME/desktop-plugins/hello/plugin.js` (that's `~/.robo/...`
by default, or `~/.robo/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.robo/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@robo/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Robo' }),
      jsx('div', {
...
The only importable specifiers are `@robo/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `RoboPlugin`:

```ts
interface RoboPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `robo.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@robo/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@robo/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@robo/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@robo/plugin-sdk'
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@robo/plugin-sdk'
...
approval/turn alerts use. It fires only while the user is away from Robo
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@robo/plugin-sdk'
...
```javascript
import { queryClient } from '@robo/plugin-sdk'
...
`dashboard/` subfolder of a regular Robo plugin and declare it in a
`manifest.json`:
...
robo-engineer codebase directly (`robo_state`, `robo_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.robo/`)
...
(`robo.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `RoboPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@robo/plugin-sdk` alias.
2. It's still lint-fenced to `@robo/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@robo/plugin-sdk`, `react`,
...
| Plugin contract | `RoboPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `Contribution` |
...
### Agents: the `robo-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`robo-desktop-plugins`** skill — it carries the same contract as this page in
...
`$ROBO_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `robo logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@robo/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.robo/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.robo/logs/errors.log` for

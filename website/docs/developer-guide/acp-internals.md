...
The ACP adapter wraps Robo' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
robo acp / robo-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.robo/.env
  -> configure stderr logging
  -> construct RoboACPAgent
...
- `allow_once` -> Robo `once`
- `allow_always` -> Robo `always`
- reject options -> Robo `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Robo tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["robo-acp"])
...
Instead it reuses Robo' runtime resolver:

- `acp_adapter/auth.py`
- `robo_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Robo provider/credentials. It also always advertises a terminal setup auth method (`robo-setup`, args `--setup`) so first-run ACP clients can open Robo' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.robo/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `robo-acp` toolset definition
- `robo_cli/main.py` — `robo acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `robo-acp` script

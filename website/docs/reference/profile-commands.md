...
This page covers all commands related to [Robo profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `robo profile` without a subcommand shows help.
...
```bash
robo profile use <name>
```

Sets `<name>` as the active profile. All subsequent `robo` commands (without `-p`) will use this profile.
...
robo profile use default
```

## `robo profile create`

```bash
robo profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `robo profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `robo update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.robo`), use `robo skills opt-out` / `robo skills opt-in`. |
...
robo profile create work2 --clone-from work

# Clone everything from a specific profile
robo profile create work2-backup --clone-from work --clone-all
```

## `robo profile describe`

```bash
robo profile describe [<name>] [options]
```
...
robo profile describe researcher

# Set it explicitly
robo profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
robo profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
robo profile describe --all --auto
```

## `robo profile delete`

```bash
robo profile delete <name> [options]
```
...
```bash
robo profile delete mybot
robo profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.robo`) cannot be deleted — use `robo uninstall` to remove everything.
:::

## `robo profile show`

```bash
robo profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Robo home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ robo profile show work
Profile: work
Path:    ~/.robo/profiles/work
...
```bash
robo profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Robo installation.
...
```bash
robo profile alias work
# Creates/updates ~/.local/bin/work

robo profile alias work --name mywork
# Creates ~/.local/bin/mywork

robo profile alias work --remove
...
```bash
robo profile rename <old-name> <new-name>
```
...
```bash
robo profile rename mybot assistant
# ~/.robo/profiles/mybot → ~/.robo/profiles/assistant
...
```bash
robo profile export <name> [options]
```
...
```bash
robo profile export work
# Creates work.tar.gz in the current directory

robo profile export work -o ./work-2026-03-29.tar.gz
```

## `robo profile import`

```bash
robo profile import <archive> [options]
```
...
```bash
robo profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

robo profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`robo profile export` / `import` are still the right commands for
...
### `robo profile install`

```bash
robo profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `robo -p telemetry`). |
...
robo profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
robo profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
robo profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
robo profile install ./telemetry/
```

### `robo profile update`

```bash
robo profile update <name> [--force-config] [--yes]
```
...
```bash
robo profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Robo version, author, env var requirements, the source URL/path, and
...
`robo profile list` also shows the distribution name and version in a
`Distribution` column, and `robo profile show <name>` / `delete <name>`
...
robo profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
robo profile install https://github.com/IgniteeNow/internal-assistant.git
```
...
`robo_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Robo version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Robo can clone from.
3. Tell recipients to run `robo profile install <your-repo-url>`.
...
```bash
robo -p <name> <command> [options]
robo --profile <name> <command> [options]
```

Global flag to run any Robo command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
robo -p work chat -q "Check the server status"
robo --profile dev gateway start
robo -p personal skills list
robo -p work config edit
```

## `robo completion`

```bash
robo completion <shell>
```
...
- `robo profile <TAB>` — subcommands (list, use, create, etc.)
- `robo profile use <TAB>` — profile names
- `robo -p <TAB>` — profile names

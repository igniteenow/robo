...
description: "Reference for Robo core, composite, platform, and dynamic toolsets"
---
...
- **Platform** — A complete tool configuration for a specific deployment context (e.g., `robo-cli` is the default for interactive CLI sessions)
...
```bash
robo chat --toolsets web,file,terminal
robo chat --toolsets debugging        # composite — expands to file + terminal + web
robo chat --toolsets all              # everything
```
...
```yaml
toolsets:
  - robo-cli          # default for CLI
  # - robo-telegram   # override for Telegram gateway
```

### Interactive management

```bash
robo tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call Robo tools programmatically. |
...
| `discord` | `discord` | Core Discord text/embed/DM actions (gateway-only). Active on the `robo-discord` toolset. |
| `discord_admin` | `discord_admin` | Discord moderation (bans, role changes, channel management). Active on the `robo-discord` toolset; requires the bot to hold the relevant Discord permissions. |
...
| `feishu_drive` | `feishu_drive_add_comment`, `feishu_drive_list_comments`, `feishu_drive_list_comment_replies`, `feishu_drive_reply_comment` | Feishu/Lark drive comment operations. Scoped to the comment agent; not exposed on `robo-cli` or other messaging toolsets. |
...
| `kanban` | `kanban_attach`, `kanban_attach_url`, `kanban_attachments`, `kanban_block`, `kanban_comment`, `kanban_complete`, `kanban_create`, `kanban_heartbeat`, `kanban_link`, `kanban_list`, `kanban_show`, `kanban_unblock` | Multi-agent coordination tools. Registered for dispatcher-spawned task workers (`ROBO_KANBAN_TASK`) and for profiles that explicitly list the `kanban` toolset by name (the `all`/`*` wildcard does **not** enable it). Workers mark tasks done, block, heartbeat, comment, and create/link follow-up tasks; orchestrator profiles additionally get board-routing tools like list/unblock. `delegate_task` children are not Kanban run owners: their schema strips/disables this toolset and runtime guards reject direct board mutations, even if parent `ROBO_KANBAN_*` env vars are present. |
| `memory` | `memory` | Persistent cross-session memory management. |
| `desktop_ui` | `close_terminal`, `focus_pane`, `open_preview`, `react_to_message`, `read_preview`, `read_terminal` | Affordances that act on the Robo desktop app itself — read/close the embedded terminal pane, open and read the in-app browser, reveal a pane, react to a message. Enabled for sessions whose source is the desktop app, whichever backend it's connected to (local, SSH, URL, or Robo Cloud). Never present on CLI, TUI, messaging, or cron sessions. |
...
| `x_search` | `x_search` | Read-only public X discovery via xAI's built-in `x_search` Responses tool. Use the `xurl` skill for authenticated X API reads and account actions. Off by default; opt in via `robo tools`. Schema only registered when xAI credentials (SuperGrok OAuth or `XAI_API_KEY`) are configured. |
| `yuanbao` | `yb_query_group_info`, `yb_query_group_members`, `yb_search_sticker`, `yb_send_dm`, `yb_send_sticker` | Yuanbao DM/group actions and sticker search. Registered only on `robo-yuanbao`. |

## Platform Toolsets

Platform toolsets define the complete tool configuration for a deployment target. Most messaging platforms use the same set as `robo-cli`:

| Toolset | Differences from `robo-cli` |
|---------|-------------------------------|
| `robo-cli` | Full toolset — the default for interactive CLI sessions. Includes file, terminal, web, browser, memory, skills, vision, image_gen, todo, tts, delegation, code_execution, cronjob, session_search, clarify, computer_use, Home Assistant, and the kanban tools (all check_fn-gated at runtime). |
| `robo-acp` | Drops `clarify`, `cronjob`, `image_generate`, `text_to_speech`, `computer_use`, all four Home Assistant tools, and the kanban tools. Focused on coding tasks in IDE context. |
| `robo-api-server` | Drops `clarify`, `text_to_speech`, `computer_use`, and the kanban tools. Keeps everything else — suitable for programmatic access where user interaction isn't possible. |
| `robo-cron` | Same as `robo-cli`. |
| `robo-telegram` | Same as `robo-cli`. |
| `robo-discord` | Adds `discord` and `discord_admin` on top of `robo-cli`. |
| `robo-slack` | Same as `robo-cli`. |
| `robo-whatsapp` | Same as `robo-cli`. |
| `robo-signal` | Same as `robo-cli`. |
| `robo-matrix` | Same as `robo-cli`. |
| `robo-mattermost` | Same as `robo-cli`. |
| `robo-email` | Same as `robo-cli`. |
| `robo-sms` | Same as `robo-cli`. |
| `robo-bluebubbles` | Same as `robo-cli`. |
| `robo-dingtalk` | Same as `robo-cli`. |
| `robo-feishu` | Adds the five `feishu_doc_*` / `feishu_drive_*` tools (only used by the document-comment handler, not the regular chat adapter). |
| `robo-qqbot` | Same as `robo-cli`. |
| `robo-wecom` | Same as `robo-cli`. |
| `robo-wecom-callback` | Same as `robo-cli`. |
| `robo-weixin` | Same as `robo-cli`. |
| `robo-yuanbao` | Adds the five `yb_*` tools (DM/group/sticker) on top of `robo-cli`. |
| `robo-homeassistant` | Same as `robo-cli` (the Home Assistant tools are already present by default and activate when `HASS_TOKEN` is set). |
| `robo-webhook` | Restricted safe subset — only `web_search`, `web_extract`, `vision_analyze`, and `clarify`. Webhook-triggered runs get no terminal, file, or browser access. |
| `robo-gateway` | Internal gateway orchestrator toolset — union of every `robo-<platform>` toolset; used when the gateway needs to accept any message source. |
...
- **Workflow-gated** tools — the `kanban` toolset — are deliberately opt-in. `all`/`*` does **not** enable kanban; you must list `kanban` explicitly (or be a dispatcher-spawned worker with `ROBO_KANBAN_TASK` set). Kanban tools mutate shared board state, so they stay off by default even under `all`.

## Relationship to `robo tools`

The `robo tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.

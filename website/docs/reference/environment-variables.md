...
| `GATEWAY_RELAY_ID` | Relay gateway identifier assigned by `robo gateway enroll` or managed self-provisioning. Mirrors `gateway.relay_id`. |
...
| `GATEWAY_RELAY_ENROLL_TOKEN` | Enrollment token consumed by `robo gateway enroll` when `--token` is not passed explicitly. |

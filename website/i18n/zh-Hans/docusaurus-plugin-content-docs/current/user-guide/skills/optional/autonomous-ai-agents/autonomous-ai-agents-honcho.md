...
description: "配置并使用 Honcho 记忆功能与 Robo -- 跨会话用户建模、多配置文件 peer 隔离、观察配置、辩证推理、会话摘要及上下文预算控制。"
---
...
配置并使用 Honcho 记忆功能与 Robo -- 跨会话用户建模、多配置文件 peer 隔离、观察配置、辩证推理、会话摘要及上下文预算控制。适用于设置 Honcho、排查记忆问题、通过 Honcho peers 管理配置文件，或调整观察、召回和辩证设置。

## Skill 元数据

| | |
|---|---|
| 来源 | 可选 — 通过 `robo skills install official/autonomous-ai-agents/honcho` 安装 |
...
| 相关 skills | [`robo-engineer`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-robo-engineer) |

## 参考：完整 SKILL.md

:::info
以下是 Robo 在触发此 skill 时加载的完整 skill 定义。这是 skill 激活时 agent 所看到的指令内容。
:::

# Robo 的 Honcho 记忆

Honcho 提供 AI 原生的跨会话用户建模。它在多次对话中学习用户特征，并为每个 Robo 配置文件提供独立的 peer 身份，同时共享统一的用户视图。
...
参见：https://docs.honcho.dev/v3/guides/integrations/robo#running-honcho-locally-with-robo
...
3. **AI peer 卡片** -- 此 Robo 配置文件的 AI peer 身份卡片
...
Honcho 将对话建模为 **peers** 之间的交互。Robo 每个会话创建两个 peers：

- **用户 peer**（`peerName`）：代表人类用户。Honcho 从观察到的消息中构建用户表示。
- **AI peer**（`aiPeer`）：代表此 Robo 实例。每个配置文件拥有自己的 AI peer，使 agents 形成独立视角。
...
| `per-session` | 每次 Robo 运行创建新的 Honcho 会话 |
| `global` | 跨所有目录使用单一会话 |

手动覆盖：`robo honcho map my-project-name`
...
每个 Robo 配置文件拥有自己的 Honcho AI peer，同时共享同一工作区（用户上下文）。这意味着：
...
`--clone` 对 Honcho 的作用：
1. 在 `honcho.json` 中创建 `robo.coder` host 块
...
配置文件：`$ROBO_HOME/honcho.json`（配置文件本地）或 `~/.honcho/config.json`（全局）。
...
运行 `robo honcho setup`。确保 `~/.robo/config.yaml` 中包含 `memory.provider: honcho`。

### 记忆未跨会话持久化
检查 `robo honcho status` -- 验证 `saveMessages: true` 且 `writeFrequency` 不是 `session`（该选项仅在退出时写入）。

### 配置文件未获得自己的 peer
创建时使用 `--clone`：`robo profile create <name> --clone`。对于现有配置文件：`robo honcho sync`。
...
| 命令 | 描述 |
|---------|-------------|
| `robo honcho setup` | 交互式设置向导（云端/本地、身份、观察、召回、会话） |
| `robo honcho status` | 显示当前配置文件的已解析配置、连接测试、peer 信息 |
| `robo honcho enable` | 为当前配置文件启用 Honcho（如需则创建 host 块） |
| `robo honcho disable` | 为当前配置文件禁用 Honcho |
| `robo honcho peer` | 显示或更新 peer 名称（`--user <name>`、`--ai <name>`、`--reasoning <level>`） |
| `robo honcho peers` | 显示所有配置文件的 peer 身份 |
| `robo honcho mode` | 显示或设置召回模式（`hybrid`、`context`、`tools`） |
| `robo honcho tokens` | 显示或设置 token 预算（`--context <N>`、`--dialectic <N>`） |
| `robo honcho sessions` | 列出已知的目录到会话名称映射 |
| `robo honcho map <name>` | 将当前工作目录映射到 Honcho 会话名称 |
| `robo honcho identity` | 为 AI peer 身份播种，或显示两个 peer 的表示 |
| `robo honcho sync` | 为所有尚未拥有 host 块的 Robo 配置文件创建 host 块 |
| `robo memory setup` | 通用记忆提供商选择器（选择 "honcho" 运行相同向导） |
| `robo memory status` | 显示当前活跃的记忆提供商及配置 |
| `robo memory off` | 禁用外部记忆提供商 |
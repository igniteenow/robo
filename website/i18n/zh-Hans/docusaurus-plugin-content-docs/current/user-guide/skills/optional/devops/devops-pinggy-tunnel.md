...
| | |
|---|---|
| 来源 | 可选 — 使用 `robo skills install official/devops/pinggy-tunnel` 安装 |
...
远程客户端使用 `Authorization: Bearer $TOKEN` 连接到 `$URL`。Robo 原生 MCP 客户端配置：`{"transport": "http", "url": "<URL>", "headers": {"Authorization": "Bearer <TOKEN>"}}`。

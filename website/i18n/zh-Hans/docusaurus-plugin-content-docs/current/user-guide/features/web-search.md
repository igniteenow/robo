...
Robo Agent 内置两个可供模型调用的网页工具，由多个提供商支持：

- **`web_search`** — 搜索网页并返回排序结果
- **`web_extract`** — 从一个或多个 URL 获取并提取可读内容

两者均通过单一后端选择进行配置。提供商可通过 `robo tools` 选择，或直接在 `config.yaml` 中设置。
...
摘要保留引用、代码块和关键事实的原始格式——它是内容压缩器，而非改写器。如果摘要失败或超时，Robo 会回退到原始内容的前约 5 000 字符，而非返回无用的错误信息。

### 哪个模型负责摘要？

`web_extract` 辅助任务。默认情况下（`auxiliary.web_extract.provider: "auto"`），使用您的**主聊天模型**——与 `robo model` 相同的提供商和模型。对大多数配置而言这没问题，但在昂贵的推理模型（Opus、MiniMax M2.7 等）上，每次长页面提取都会产生可观的成本。
...
或交互式选择：`robo model` → **Configure auxiliary models** → `web_extract`。
...
运行 `robo tools`，导航至 **Web Search & Extract**，选择一个提供商。向导会提示输入所需的 URL 或 API 密钥，并写入您的配置。
...
SearXNG 是一个注重隐私的开源元搜索引擎，聚合来自 70 多个搜索引擎的结果。**无需 API 密钥**——只需将 Robo 指向一个运行中的 SearXNG 实例。
...
然后在 `~/.robo/config.yaml` 中选择 SearXNG 作为搜索后端：
...
或通过 `robo tools` → Web Search & Extract → SearXNG 设置。
...
使用此配置，Robo 对所有搜索查询使用 SearXNG，对 URL 提取使用 Firecrawl——将免费搜索与高质量提取相结合。
...
如果未显式配置后端，Robo 根据已设置的凭证选择第一个可用的后端：
...
```bash
# 激活 venv 并直接运行网页工具模块
source ~/.robo/robo-engineer/.venv/bin/activate
...
```bash
robo skills install official/research/searxng-search
```

#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Robo docs site.
...
Both publish at:
  https://robo.igniteenow.com/docs/llms.txt
  https://robo.igniteenow.com/docs/llms-full.txt
...
SITE_BASE = "https://robo.igniteenow.com/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Robo as a Python Library", None),
        ("guides/use-mcp-with-robo", "Use MCP with Robo", None),
        ("guides/use-voice-mode-with-robo", "Use Voice Mode with Robo", None),
        ("guides/use-soul-with-robo", "Use SOUL.md with Robo", None),
        ("guides/build-a-robo-plugin", "Build a Robo Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Robo"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Robo Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by Ignitee Now. A terminal-native "
        ...
        "backends. Works with Ignitee Now Portal, OpenRouter, OpenAI, Anthropic, Google, or any "
        "OpenAI-compatible endpoint."
    )
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/IgniteeNow/"
        "robo-engineer/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/IgniteeNow/robo-engineer")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Robo Agent — Full Documentation\n",
        (
            "This file is the entire Robo Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Robo, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://robo.igniteenow.com/docs\n",
        "Short index: https://robo.igniteenow.com/docs/llms.txt\n",
        "\n---\n\n",
    ]

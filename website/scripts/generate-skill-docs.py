...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/IgniteeNow/robo-engineer/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    robo_meta = (fm.get("metadata") or {}).get("robo") or {}
    tags = robo_meta.get("tags") or []
    related = robo_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `robo skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Robo Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Robo ships with a large built-in skill library copied into `~/.robo/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Robo also syncs bundled skills on `robo update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.robo/skills/` tree, it is still shipped with Robo; restore it with `robo skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with robo-engineer — install via robo skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with robo-engineer under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "robo skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "robo skills install official/blockchain/solana",
        "robo skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "robo skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):

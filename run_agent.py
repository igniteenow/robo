...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``robo -c`` / ``--resume`` (relaunch
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = get_session_env("ROBO_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("ROBO_SESSION_SOURCE", "")
...
_loaded_env_paths = load_robo_dotenv(robo_home=_robo_home, project_env=_project_env)
...
from agent.prompt_builder import (  # noqa: F401  # re-exported via _ra() / mock.patch("run_agent.<name>") / from run_agent import <name>
    ...
    build_igniteenow_subscription_prompt,
    load_soul_md,
)
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Robo-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.robo/sessions/``.  Without
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[robo-engineer: tool call arguments were corrupted in this session and "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            # `robo --resume`. set_session_yolo() no-ops on a missing row, so
    ...
    def _ensure_lmstudio_runtime_loaded(
        ...
        from robo_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``robo chat -q``. In that mode,
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``ROBO_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``ROBO_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("ROBO_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``ROBO_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("ROBO_API_CALL_STALE_TIMEOUT")
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        robo-engineer #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See robo-engineer#21444 for symptom history."
        )
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        if normalized_provider == "copilot":
            try:
                from robo_cli.models import _should_use_copilot_responses_api
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Robo against xai-oauth.  Lots of
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("ROBO_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        try:
            from robo_cli import lifecycle as _lifecycle
    ...
    def _redact_message_content(content):
        ...
        Respects ``ROBO_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.robo/sessions/session_{sid}.json``
        ...
        # untrusted input (X-Robo-Session-Id header) and must not escape
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects ROBO_REDACT_SECRETS via
    ...
    def interrupt(self, message: Optional[str] = None, *, hard_cancel: bool = False) -> None:
        ...
        # interrupt event rather than Robo' per-thread flag.
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal Robo model request this cancels only that request;
        ...
        # A correction that lands while tools are executing: two policies.
        #
        #   redirect (gentle, ``display.busy_input_mode: redirect``) — never
        #   kill a tool merely to deliver conversational guidance; the steer
        #   drain puts the text on the final tool result before the next model
        #   decision. The model only reads it once the whole batch finishes,
        #   which for a long build/test/scan can be minutes.
        #
        #   interrupt (default, ``display.busy_input_mode: interrupt``) — the
        #   user asked to *interrupt*: cancel the running batch cooperatively
        #   (foreground commands get the per-thread interrupt and return a
        #   cancelled result), keep the text as a real user correction, and
        #   let the loop apply it on its very next iteration. Nothing already
        #   completed is lost; a cancelled command is re-run only if the
        #   model still wants it after reading the new instruction.
        if getattr(self, "_executing_tools", False):
            if not self._busy_correction_interrupts_tools():
                return self.steer(cleaned)
            # False = a hard stop is already ending this turn; let the surface
            # queue the text as the next turn instead of losing it in a steer
            # slot the stop would discard.
            return self._begin_tool_phase_redirect(cleaned)
    ...
    def _busy_correction_interrupts_tools(self) -> bool:
        """``display.busy_input_mode`` policy for corrections during tools.

        ``interrupt`` (default) cancels the running tool batch so the new
        instruction is read immediately; ``redirect`` keeps the gentle
        steer-after-batch behaviour. Any other mode never reaches here.
        """
        try:
            from robo_cli.config import load_config_readonly
            display = (load_config_readonly() or {}).get("display") or {}
            mode = str(display.get("busy_input_mode", "interrupt") or "").strip().lower()
        except Exception:
            mode = "interrupt"
        return mode != "redirect"

    def _begin_tool_phase_redirect(self, cleaned: str) -> bool:
        """Queue *cleaned* as the turn's correction and cancel running tools.

        Mirrors ``interrupt()``'s fan-out (execution thread, concurrent tool
        workers, delegated children) but leaves ``_pending_redirect`` set so
        the conversation loop treats the cancellation as a redirect: it
        applies the correction as a real user message, clears the interrupt
        bit, and continues the same turn. Returns False when a hard stop is
        already in flight (the loop is ending; the surface should queue the
        text for the next turn instead).
        """
        _redirect_lock = getattr(self, "_pending_redirect_lock", None)
        if _redirect_lock is None:
            existing = getattr(self, "_pending_redirect", None)
            ...
            with _redirect_lock:
                if self._interrupt_requested and not self._pending_redirect:
                    return False
                if self._pending_redirect:
                    # Already cancelling for an earlier correction; just add.
                    self._pending_redirect = (
                        f"{self._pending_redirect}\n\n"
                        f"[Additional user correction]\n{cleaned}"
                    )
        ...
        if _tracker is not None and _tracker_lock is not None:
            ...
            for _wtid in _worker_tids:
                try:
                    ...
                    pass
        # A delegated child is part of this turn's tool batch: cancel it too so
        # delegate_task returns promptly with what it has.
        _children_lock = getattr(self, "_active_children_lock", None)
        _children = getattr(self, "_active_children", None)
        if _children_lock is not None and _children is not None:
            with _children_lock:
                children_copy = list(_children)
            for child in children_copy:
                try:
                    child.interrupt("user sent a new instruction")
                except Exception:
                    pass
        return True
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.robo/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``ROBO_TURN_COMPLETION_EXPLAINER`` env var overrides
        config.  Exposed as a method so tests can patch a single seam,
        mirroring ``_file_mutation_verifier_enabled``.
        """
        try:
            import os as _os
            env = _os.environ.get("ROBO_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
            # Read from the persisted config.yaml so gateway and CLI share
            # the same setting.  Import lazily to avoid a startup-time cycle.
    ...
    def _apply_pending_claim_verification(self, assistant_message, messages: list, num_tool_msgs: int) -> None:
        """Forwarder — see ``agent.agent_runtime_helpers.apply_pending_claim_verification``."""
        from agent.agent_runtime_helpers import apply_pending_claim_verification
        return apply_pending_claim_verification(self, assistant_message, messages, num_tool_msgs)
    ...
    def _touch_activity(
        ...
        *,
        ...
        process is a dispatcher-spawned worker (ROBO_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        """Parse x-igniteenow-credits-* headers, cache CreditsState, fire threshold notices.
        ...
        """
        # Dev test fixture (ROBO_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset ROBO_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("ROBO_DEV_CREDITS"))
        ...
        if state is None:
            if _dev:
                logger.info(
                    "credits ▸ response had no valid x-igniteenow-credits-* headers "
                    "(miss — producer off / non-Ignitee Now path / >TTL stale)"
                )
            return
        ...
        if _dev:
            # ROBO_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `robo logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def close(self) -> None:
        ...
        # a private embedded daemon when Robo YOLO selected unrestricted
    ...
    def _build_keepalive_http_client(base_url: str = "", *, verify: Any = True) -> Any:
        ...
        ``ROBO_CA_BUNDLE`` settings.  It is passed on the client AND on
    ...
    def _copilot_headers_for_request(self, *, is_vision: bool) -> dict:
        from robo_cli.copilot_auth import copilot_request_headers
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``robo auth add xai-oauth``) whose tokens belong to
        ...
        try:
            if self.provider == "openai-codex":
                from robo_cli.auth import resolve_codex_runtime_credentials

                singleton_now = resolve_codex_runtime_credentials(
                    refresh_if_expiring=False,
                )
            else:
                from robo_cli.auth import resolve_xai_oauth_runtime_credentials
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ~/.robo/.env credential/base-url edits at the turn boundary.

        A Settings save (desktop ``PUT /api/env``, ``robo setup``) updates
        ...
        if (
            ...
            if self.provider == "kimi-coding":
                from robo_cli.auth import _resolve_kimi_base_url

                base_url = _resolve_kimi_base_url(
                    api_key, pconfig.inference_base_url, env_url
                ).rstrip("/")
            elif self.provider == "zai":
                from robo_cli.auth import _resolve_zai_base_url
            ...
            try:
                from robo_cli.runtime_provider import _get_named_custom_provider
            except ImportError:
                return False
    ...
    def _apply_client_headers_for_base_url(
        ...
        *,
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            from robo_cli.models import copilot_default_headers
            ...
            from tools.xai_http import robo_xai_default_headers

            self._client_kwargs["default_headers"] = robo_xai_default_headers()
        else:
    ...
    def _anthropic_messages_create(self, api_kwargs: dict, *, client: Any = None):
        ...
        return create_anthropic_message(
            ...
            # matching header families (x-ratelimit-* / x-igniteenow-credits-*).
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from robo_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from robo_cli.models import lmstudio_model_reasoning_options
    ...
    def _ollama_supports_thinking_cached(self) -> bool:
        ...
        try:
            from robo_cli.models import ollama_model_supports_thinking
    ...
    def _compress_context(
        ...
        *,
        ...
        try:
            ...
            # id rotation updated robo_logging._session_context (a
            ...
            # ROBO_SESSION_ID to the child id after an out-of-place
    ...
    def run_conversation(
        ...
        from robo_cli.observability.relay_shared_metrics import (
            finish_task_run,
            start_task_run,
        )
        ...
        try:
            ...
            if getattr(relay_turn, "relay_enabled", True):
                ...
                task_started = True
            # Publish the conversation id via ContextVar. Every LLM call made
            # inside this turn — main loop, compression, vision, web_extract,
            # session_search, MoA slots, background-review forks (which copy
            # this Context into their thread) — inherits the
            # ``conversation=<root>`` value with zero per-call-site plumbing.

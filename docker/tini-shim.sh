...
# Robo WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: ROBO_TINI_SHIM_TARGET overrides the `/init` path (and
# ROBO_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${ROBO_TINI_SHIM_TARGET:-/init}"
WRAPPER="${ROBO_TINI_SHIM_WRAPPER:-/opt/robo/docker/main-wrapper.sh}"

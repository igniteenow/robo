...
#
# Wired into the image as /etc/cont-init.d/01-robo-setup by the
...
ROBO_HOME="${ROBO_HOME:-/opt/data}"
INSTALL_DIR="/opt/robo"

# Drop to robo via s6-setuidgid, but skip it when already non-root.
as_robo() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid robo "$@"; }
...
# starts non-root. The baked install tree under /opt/robo is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $ROBO_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass ROBO_UID/ROBO_GID — or the PUID/PGID aliases — which the
...
# `id -u robo` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# robo UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u robo)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-robo UID).
...
and the baked /opt/robo install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e ROBO_UID=\$(id -u) -e ROBO_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the robo user to that UID/GID at boot and chowns the data
...
# --- Bootstrap ROBO_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid robo mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom ROBO_HOME paths whose parents only
# root can create (e.g. `ROBO_HOME=/home/robo/.robo` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for ROBO_UID/ROBO_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  ROBO_UID/
# ROBO_GID still win when both are set.  See #15290, salvages #25872.
ROBO_UID="${ROBO_UID:-${PUID:-}}"
ROBO_GID="${ROBO_GID:-${PGID:-}}"

if [ -n "${ROBO_UID:-}" ] && validate_uid_gid "$ROBO_UID" && [ "$ROBO_UID" != "$(id -u robo)" ]; then
    echo "[stage2] Changing robo UID to $ROBO_UID"
    usermod -u "$ROBO_UID" robo
fi
if [ -n "${ROBO_GID:-}" ] && validate_uid_gid "$ROBO_GID" && [ "$ROBO_GID" != "$(id -g robo)" ]; then
    echo "[stage2] Changing robo GID to $ROBO_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$ROBO_GID" robo 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised robo user (UID 10000)
...
# robo process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes robo. Idempotent across container
...
#   - robo is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G robo 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] robo already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" robo 2>/dev/null; then
        echo "[stage2] Added robo to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group robo failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When ROBO_UID is remapped or the top-level $ROBO_HOME isn't owned by
# the runtime robo UID, restore ownership to robo — but ONLY for the
# directories robo actually writes to. The full $ROBO_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of robo-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_robo_uid=$(id -u robo)
...
chown_robo_tree() {
    ...
    chown -R robo:robo "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_robo_owner() {
    target="$1"
    find "$target" \( ! -user robo -o ! -group robo \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$ROBO_HOME" 2>/dev/null)" != "$actual_robo_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $ROBO_HOME (targeted) to robo ($actual_robo_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $ROBO_HOME: chown the directory itself (not its contents)
    # so robo can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$ROBO_HOME"; then
        :
    else
        chown robo:robo "$ROBO_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $ROBO_HOME failed (rootless container?) — continuing"
    fi
    # Robo-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by robo (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$ROBO_HOME/$sub" ] && tree_has_non_robo_owner "$ROBO_HOME/$sub"; then
            chown_robo_tree "$ROBO_HOME/$sub"
        fi
    done
fi
...
# robo user. Hosted/container instances keep mutable state under
# $ROBO_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# ROBO_DISABLE_LAZY_INSTALLS=1. Keeping /opt/robo root-owned and
...
# $ROBO_HOME/lazy-packages dir on the data volume (Dockerfile sets
# ROBO_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to robo
...
# Always reset ownership of $ROBO_HOME/profiles to robo on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> robo …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as robo and walks
...
if [ -d "$ROBO_HOME/profiles" ] && tree_has_non_robo_owner "$ROBO_HOME/profiles"; then
    chown_robo_tree "$ROBO_HOME/profiles"
fi

# Always reset ownership of $ROBO_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged robo runtime even
...
# gate as profiles/).
if [ -d "$ROBO_HOME/cron" ] && tree_has_non_robo_owner "$ROBO_HOME/cron"; then
    chown_robo_tree "$ROBO_HOME/cron"
fi

# Always ensure logs/gateways is robo-owned (#45258). Formerly healed by
...
# top-level $ROBO_HOME is mis-owned, so a warm volume with robo-owned
# ROBO_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid robo mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as robo.
if [ -d "$ROBO_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$ROBO_HOME/logs/gateways"; then
        :
    else
        chown robo:robo "$ROBO_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# robo pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged robo gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $ROBO_HOME is
...
# and cron/.
if [ -d "$ROBO_HOME/platforms/pairing" ] && tree_has_non_robo_owner "$ROBO_HOME/platforms/pairing"; then
    chown_robo_tree "$ROBO_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$ROBO_HOME/pairing" ] && tree_has_non_robo_owner "$ROBO_HOME/pairing"; then
    chown_robo_tree "$ROBO_HOME/pairing"
fi

# Reset ownership of robo-owned top-level state files on every boot.
# The targeted data-volume chown above only covers robo-owned
# *subdirectories*; loose state files living directly under $ROBO_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> robo …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged robo runtime then hits
...
# sweep so host-owned files in a bind-mounted $ROBO_HOME are never
...
# entries of robo_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$ROBO_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$ROBO_HOME/$f"; then
            :
        else
            chown robo:robo "$ROBO_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the robo runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$ROBO_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$ROBO_HOME/config.yaml"; then
        :
    else
        chown robo:robo "$ROBO_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$ROBO_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as robo user ---
# Run as robo via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $ROBO_HOME values
...
as_robo mkdir -p \
    ...
    "$ROBO_HOME/home" \
    "$ROBO_HOME/pairing" \
    "$ROBO_HOME/platforms/pairing" \
    "$ROBO_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/robo/.install_method (see Dockerfile), NOT written here into
# $ROBO_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $ROBO_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.robo:/opt/data) and sometimes shared with a
...
# 'robo update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $ROBO_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/robo stamp, so deleting the data-dir copy is safe).
if [ -f "$ROBO_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$ROBO_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$ROBO_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$ROBO_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$ROBO_HOME/$dest"; then
            :
        else
            as_robo cp "$INSTALL_DIR/$src" "$ROBO_HOME/$dest"
        fi
    fi
}
...
if [ -f "$ROBO_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$ROBO_HOME/.env"; then
        :
    else
        chown robo:robo "$ROBO_HOME/.env" 2>/dev/null || true
...
# $ROBO_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `robo update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set ROBO_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$ROBO_HOME/config.yaml" ]; then
    s6-setuidgid robo "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$ROBO_HOME/auth.json" ] && [ -n "${ROBO_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$ROBO_HOME/auth.json"; then
        :
    else
        printf '%s' "$ROBO_AUTH_JSON_BOOTSTRAP" > "$ROBO_HOME/auth.json"
        chown robo:robo "$ROBO_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$ROBO_HOME/auth.json"
    fi
fi
...
# can set ROBO_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# ROBO_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$ROBO_HOME/gateway_state.json" ] && \
        [ "${ROBO_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$ROBO_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$ROBO_HOME/gateway_state.json"
        chown robo:robo "$ROBO_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$ROBO_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_robo "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/robo/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Robo spawns for the
...
# main-wrapper.sh propagates it into the supervised ``robo`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-robo, dashboard, etc.).

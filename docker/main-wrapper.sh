#!/bin/sh
# shellcheck shell=sh
# /opt/robo/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching ROBO_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `robo` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `robo <args>` (subcommand passthrough)
#
# Drop to robo via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${ROBO_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${ROBO_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export ROBO_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset ROBO_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid robo "$@"; exec "$@"; }
...
# an arbitrary non-root, non-robo UID, the bootstrap was skipped and the
# baked image dirs (owned by the robo build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u robo)" ]; then
    cat >&2 <<EOF
[robo] ERROR: container started with --user $cur_uid (an arbitrary, non-robo UID) — not supported.
...
# to the robo user's home before dropping privileges so libraries that
...
_robo_orig_cwd="${ROBO_ORIG_CWD:-$PWD}"
...
# the user's command so `robo chat` starts in the Docker -w

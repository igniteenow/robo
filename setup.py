...
Robo (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``ROBO_NIX_BUILD=1`` on the
Robo package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("ROBO_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for robo-engineer is not supported.\n"
    "Robo is distributed through its cross-platform source installers.\n"
    "See INSTALL.md in the Robo source package.\n"
    "\n"
    "If you are developing, use an editable install instead:\n"
    "  uv sync          # or: uv pip install -e .\n"
    "\n"
    "If you are building with Nix (uv2nix), this error should not fire —\n"
    "the compatible Nix derivation sets ROBO_NIX_BUILD=1. If it does, file a bug."
)

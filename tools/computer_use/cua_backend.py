...
etc.) surface as specific blocked checks via `robo computer-use doctor`
...
def _action_result_from(
    ...
    *,
    ...
    cua-driver's mcp-tool-notes and IgniteeNow/robo-engineer#67052.
    """
...
# fetches the latest release, so a `ROBO_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `ROBO_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD_ENV = "ROBO_CUA_DRIVER_CMD"
...
def _cua_no_overlay() -> bool:
    """True when Robo should pass ``--no-overlay`` to cua-driver.
...
def _cua_telemetry_disabled() -> bool:
    """True when Robo should disable cua-driver telemetry for this user.
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command when Robo runs in WSL.

    Windows cua-driver manifests can report ``C:\\Users\\...\\cua-driver.exe``
    even though the Robo process uses POSIX subprocess spawning inside WSL.
...
class _EmbeddedCuaDaemon:
    ...
    machine-wide daemon would therefore let one Robo session's YOLO choice
    ...
    def __init__(self, driver_cmd: str, permission_mode: str) -> None:
        ...
        if sys.platform == "win32":
            self.socket_path = rf"\\.\pipe\robo-cua-{token}"
        else:
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of: instead of hardcoding
    ...
    # A Windows-installed cua-driver can hand a WSL-hosted Robo an absolute
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    ``ROBO_CUA_DRIVER_CMD`` is authoritative; only when neither is set do we
    ...
    Robo Desktop/TUI session can otherwise filter out the `computer_use`
    tool even though `robo computer-use doctor` succeeds from a login shell.
    """
...
def resolve_cua_driver_cmd(override: Optional[str] = None) -> Optional[str]:
    ...
    A supplied override (or ``ROBO_CUA_DRIVER_CMD``) is never silently
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `robo computer-use install --upgrade`."
    )
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        f"{installer}\n"
        "Or run `robo tools` and enable the Computer Use toolset to install it automatically."
    )
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of IgniteeNow/robo-engineer#47072: read the canonical
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize Robo-managed secrets out of the child env.
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                "Run `robo computer-use doctor` and check "
                f"{display_robo_home()}/logs/agent.log for the phase timings."
            )
...
). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        #   - Gets a distinct agent-cursor color per Robo run, with
        ...
        #     concurrent Robo runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one Robo run = one backend = one session — and pass it as
        ...
        self._session_id: str = f"robo-{uuid.uuid4().hex[:12]}"
    ...
    def start(self) -> None:
        ...
        # `computer-use` / `mcp` extras), not part of Robo' minimal core.
    ...
    def capture(
        ...
        Maps robo `capture(mode, app)` → cua-driver `list_windows` +
        `get_window_state` (ax/som) or `screenshot` (vision).
        """
        # Step 1: enumerate on-screen windows to find target pid/window_id.
        # Surface 3 of: read the canonical
    ...
    def click(
        ...
        *,
        ...
        #). cua-driver-rs gained an explicit
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        separately approved by the Robo adapter, and uses cua-driver's

"""Progressive tool disclosure ("tool search") for Robo Agent.
...
``tool_call`` — and surfaced on demand. Core Robo tools never defer.

Design constraints this module is built around:

* Core tools defined in ``toolsets._ROBO_CORE_TOOLS`` are *never* deferred.
...
* The catalog is stateless across turns and tools-array assemblies. It is
  ...
  from cron regression (an upstream fix): a session-keyed
...
def _core_tool_names() -> frozenset[str]:
    ...
    try:
        ...
        return frozenset(_ROBO_CORE_TOOLS)
    except Exception:
        return frozenset()
...
def is_deferrable_tool_name(name: str) -> bool:
    ...
    OR it is not in ``_ROBO_CORE_TOOLS``. Core tools are never deferred

...
`robo skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.robo.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.robo.blueprint`` is a mapping containing
    ...
    robo = meta.get("robo") if isinstance(meta, dict) else None
    blueprint = robo.get("blueprint") if isinstance(robo, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.robo.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.robo.blueprint`` block) they can hand
    to ``robo skills publish`` to share. ``body`` is the plain-language

...
# Must match robo_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``robo-cli`` (which bundles
...
def _build_child_system_prompt(
    ...
    *,
    ...
    modeled on buildSubagentSystemPrompt (canSpawn branch).
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``robo-cli`` because those also contain
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on Ignitee Now Portal).
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. robo-cli) so that individual
    ...
    # Blocked tools also live inside mixed platform bundles (robo-cli,
    # robo-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # Extract parent's API key so subagents inherit auth (e.g. Ignitee Now Portal).
    ...
    #
    # Ignitee Now Portal is dual-wire within a single provider: anthropic/* → Messages,
    # everything else → chat_completions. Same-provider inheritance would pin a
    # child Robo/Qwen subagent onto the parent's Claude Messages wire (or the
    # reverse). agent_init honors an explicit api_mode above its igniteenow branch, so
    # re-derive here before construction.
    _parent_provider = getattr(parent_agent, "provider", None) or ""
    _effective_provider_norm = (effective_provider or "").strip().lower()
    if override_api_mode is not None:
        effective_api_mode = override_api_mode
    elif _effective_provider_norm in {"igniteenow", "igniteenow-portal", "igniteenow"}:
        from robo_cli.providers import igniteenow_api_mode

        effective_api_mode = igniteenow_api_mode(effective_model)
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.robo/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_robo_dir("cache/delegation", "delegation_cache")
    ...
    if _subagent_id:
        if owner_session_id is None:
            try:
                ...
                owner_session_id = get_session_env("ROBO_UI_SESSION_ID", "") or None
            except Exception:
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from robo_cli.plugins import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    # ROBO_SESSION_ID ContextVar and os.environ with the subagent's internal
    # id before the background-dispatch code below would read it. The
    # request-scoped chat_id binding (the raw X-Robo-Session-Id on
    ...
    try:
        ...
        _origin_ui_session_id = get_session_env("ROBO_UI_SESSION_ID", "")
    except Exception:
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-Robo-Session-Id once the batch completes.
            ...
            # ROBO_SESSION_ID here would return the subagent's internal id.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `robo -z`, a cron job, a Kanban "
        ...
        try:
            ...
            _source = get_session_env("ROBO_SESSION_SOURCE", "")
            ...
            _origin_ui_session_id = (
                get_session_env("ROBO_UI_SESSION_ID", "") or _origin_ui_session_id
            )
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and ROBO_SESSION_KEY is not in the CLI
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available providers: openrouter, igniteenow, zai, kimi-coding, minimax."
        ) from exc
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'robo auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Robo config.

    Prefer the shared persistent loader because it follows the active
    ROBO_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``ROBO_IGNORE_USER_CONFIG=1`` (``robo chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("ROBO_IGNORE_USER_CONFIG") == "1"

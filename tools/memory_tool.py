...
# (ROBO_HOME env var changes) are always respected.  The old module-level
# constant was cached at import time and could go stale if a profile switch
# happened after the first import.
def get_memory_dir() -> Path:
    """Return the profile-scoped memories directory."""
    return get_robo_home() / "memories"
...
def _scan_memory_content(content: str, target: str = "memory") -> Optional[str]:
    """Scan memory content before it is stored. Returns error string if blocked.

    Two gates: the shared threat-pattern scan (injection / exfiltration),
    then :mod:`agent.content_policy` — what *belongs* in MEMORY.md / USER.md
    (durable facts) versus what never does (secrets, safety overrides, raw
    tool output, session-bound state, third-party personal data, oversized
    dumps). The policy's message tells the agent how to rewrite the entry.
    """
    threat = _first_threat_message(content, scope="strict")
    if threat:
        return threat
    try:
        from agent.content_policy import evaluate as _policy_evaluate
        verdict = _policy_evaluate(target, content)
    except Exception:
        return None
    if not verdict.ok:
...
class MemoryStore:
    ...
    def add(self, target: str, content: str) -> Dict[str, Any]:
        ...
        # Scan for injection/exfiltration + content policy before accepting
        scan_error = _scan_memory_content(content, target)
        if scan_error:
    ...
    def replace(self, target: str, old_text: str, new_content: str) -> Dict[str, Any]:
        ...
        scan_error = _scan_memory_content(new_content, target)
        if scan_error:
            return {"success": False, "error": scan_error}

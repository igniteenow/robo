...
from robo_cli.config import cfg_get
...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("ROBO_YOLO_MODE", ""))
...
# os.environ["ROBO_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export ROBO_INTERACTIVE.
_robo_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "robo_interactive",
    default=None,
)


def set_robo_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["ROBO_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``ROBO_INTERACTIVE`` env var for legacy callers.
    """
    return _robo_interactive_ctx.set("1" if interactive else "")


def reset_robo_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_robo_interactive_context`."""
    _robo_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``ROBO_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _robo_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("ROBO_INTERACTIVE")
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    """
    try:
        from robo_cli.lifecycle import invoke_hook
    except Exception:
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("ROBO_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("ROBO_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("ROBO_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("ROBO_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("ROBO_CRON_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set ROBO_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind ROBO_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds ROBO_SESSION_PLATFORM via
    ...
    if _is_cron_approval_context():
        return False
    if env_var_enabled("ROBO_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $ROBO_HOME, or by the resolved absolute
# active profile home path such as /home/robo/.robo/config.yaml. The
# resolved-absolute form is folded into the ~/.robo/ patterns at detection
...
# go stale when ROBO_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_ROBO_ENV_PATH = (
    r'(?:~\/\.robo/|'
    r'(?:\$home|\$\{home\})/\.robo/|'
    r'(?:\$robo_home|\$\{robo_home\})/)'
    r'\.env\b'
)
# ~/.robo/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _ROBO_ENV_PATH; matches the ROBO_HOME override form as
# well as ~/.robo/.
_ROBO_CONFIG_PATH = (
    r'(?:~\/\.robo/|'
    r'(?:\$home|\$\{home\})/\.robo/|'
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_robo_home() / "cache" / "blocked-scripts"
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by Robo: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # `robo` and `gateway` (e.g. `robo -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\brobo\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart robo gateway (kills running agents)"),
    (r'\brobo\s+update\b', "robo update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `robo gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `robo ...
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart robo-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart robo-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(robo|gateway|cli\.py)\b', "kill robo/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill robo` but not
    # `kill -9 $(pgrep -f robo)` because the substitution is opaque
    ...
    # the `robo gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.robo.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(robo|ai\.robo)\b', "stop/restart robo launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Robo file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.robo/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Robo-managed security file (~/.robo/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_ROBO_CONFIG_PATH}|{_ROBO_ENV_PATH})', "in-place edit of Robo config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_ROBO_CONFIG_PATH}|{_ROBO_ENV_PATH})', "in-place edit of Robo config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_ROBO_CONFIG_PATH}|{_ROBO_ENV_PATH})', "in-place edit of Robo config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.robo/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / ROBO_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Robo home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\robo), so folding the user home
    # first would eat the prefix the Robo-home fold needs.
    command = _rewrite_resolved_robo_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / ROBO_HOME such as ``/``,
    ...
    #
    # ``/root`` is the one legitimate exception: it's a single component, but
    # unlike ``/home`` it IS a specific, real home directory — the root user's
    # — not a parent directory containing many users' homes. Excluding it here
    # meant Robo running as root (containers, sudo, CI, embedded/Pi/Jetson
    # targets where root is often the default account) silently never folded
    # its own home prefix, so mined approval-rule suggestions leaked the raw
    # ``/root/...`` path instead of the portable ``~/...`` form.
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.robo``); the matched
...
def _rewrite_resolved_robo_home(command: str) -> str:
    """Rewrite the resolved absolute Robo home prefix to ``~/.robo/``.

    Resolves the active ``ROBO_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.robo/`` so the static ``_ROBO_CONFIG_PATH`` / ``_ROBO_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/robo/.robo/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$ROBO_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from robo_constants import get_robo_home
        home = get_robo_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.robo")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``robo send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Robo ad-hoc temp script."""
    try:
    ...
    # Command syntax belongs to the execution shell, not necessarily Robo's
    # host OS. A Windows control station can still inspect or dispatch a POSIX
    # `rm /tmp/...` command, so preserve POSIX path semantics for POSIX-looking
    # operands instead of rewriting them with ntpath.
    path_api = posixpath if operand.startswith("/") and raw_temp_dir.startswith("/") else os.path
    ...
    if path_api.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"robo-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def human_wait_ceiling(session_key: str | None = None) -> float:
...
def human_wait_window(session_key: str | None = None):
    ...
    try:
        ...
        ceiling = human_wait_ceiling(key)
        with _human_wait_lock:
...
def is_human_wait_pending(session_key: str | None = None) -> bool:
    """True while a human_wait_window is currently open for this session.

    Pure read on the same tracker human_wait_window() already maintains — no
    new state, no change to how a window opens/closes. Used by
    agent/tool_executor.py's concurrent-batch submit loop to hold off
    submitting the *next* tool call while an earlier one in the same batch is
    genuinely blocked on a pending approval prompt (terminal/execute_code
    guards call check_all_command_guards from their own worker thread,
    independent of the pre_tool_call authorization_gate serialization — this
    is the only signal that reaches across that gap without touching the
    approval wait loop itself). Calls already submitted/running are
    unaffected; this only gates new submissions.
    """
    key = session_key if session_key is not None else get_current_session_key()
    with _human_wait_lock:
        state = _human_wait_states.get(key)
        return state is not None and state.pending > 0
...
# session_key → {"wait_forever": bool, "on_halt": callable|None,
#                "halt_on_deny": bool}
#
# Interactive surfaces (TUI, desktop) register a *wait policy* alongside the
# notify callback. The policy turns an approval prompt into a real barrier:
#
#   wait_forever  — the agent thread parks until the user answers (or the turn
#                   is interrupted / the session closes) instead of expiring
#                   after ``approvals.timeout`` into a BLOCKED result the model
#                   then reasons around. A human is present on these surfaces;
#                   silence must not turn into "carry on with something else".
#   halt_on_deny  — a deny (or an unanswered prompt that was torn down) also
#                   stops the *turn*: ``on_halt(reason)`` runs, and the surface
#                   interrupts the agent so nothing keeps happening in the
#                   background. The user then says what to do next.
#
# Messaging platforms (Telegram/Discord/…) keep the historical timeout
# semantics — a push notification may not be seen for minutes and the user is
# not sitting in front of the agent.
_gateway_wait_policy: dict[str, dict] = {}
# Sentinel for "use approvals.timeout" so callers can pass None explicitly.
_TIMEOUT_FROM_CONFIG = object()
# Effective ceiling for an unbounded interactive wait. Large enough that no
# human-wait exclusion or start-order clamp ever fires first, small enough to
# stay a finite float for the arithmetic that consumes it.
_UNBOUNDED_WAIT_SECONDS = 10 * 365 * 86400.0
...
def register_gateway_notify(
    ...
    *approval_data* contains ``command``, ``description``, and
    ...
    Keyword arguments set the session's *wait policy* (see the module
    comment above ``_gateway_wait_policy``). The defaults preserve the
    historical timeout-and-continue behaviour used by the messaging
    platforms; interactive surfaces opt into ``wait_forever`` /
    ``halt_on_deny``.

    ``on_halt(reason: str, approval_data: dict)`` is invoked from the agent
    thread right before the BLOCKED result is returned to the model when the
    policy decides the turn must stop (``reason`` is ``"denied"``,
    ``"timeout"``, or ``"closed"``). It must not block.
    """
    with _lock:
        _gateway_notify_cbs[session_key] = cb
        if wait_forever or halt_on_deny or on_halt is not None:
            _gateway_wait_policy[session_key] = {
                "wait_forever": bool(wait_forever),
                "halt_on_deny": bool(halt_on_deny),
                "on_halt": on_halt,
            }
        else:
            _gateway_wait_policy.pop(session_key, None)


def set_gateway_wait_policy(
    session_key: str,
    *,
    wait_forever: bool | None = None,
    halt_on_deny: bool | None = None,
    on_halt=None,
) -> None:
    """Update part of a session's approval wait policy in place."""
    if not session_key:
        return
    with _lock:
        policy = dict(_gateway_wait_policy.get(session_key) or {})
        if wait_forever is not None:
            policy["wait_forever"] = bool(wait_forever)
        if halt_on_deny is not None:
            policy["halt_on_deny"] = bool(halt_on_deny)
        if on_halt is not None:
            policy["on_halt"] = on_halt
        if policy:
            _gateway_wait_policy[session_key] = policy
        else:
            _gateway_wait_policy.pop(session_key, None)


def _get_wait_policy(session_key: str) -> dict:
    with _lock:
        return dict(_gateway_wait_policy.get(session_key) or {})
...
def unregister_gateway_notify(session_key: str) -> None:
    ...
    for entry in entries:
        # Nobody can answer any more: fail closed AND mark the entry so the
        # waiter reports "closed" (surface gone) rather than a user deny.
        entry.result = "deny"
        entry.reason = entry.reason or "__approval_surface_closed__"
        entry.event.set()


def get_pending_gateway_approvals(session_key: str) -> list[dict]:
    """Return the approval payloads still waiting for an answer in a session.

    Used by interactive surfaces to re-show the prompt after a reconnect:
    the agent thread is parked on it, so a client that lost the original
    ``approval.request`` frame would otherwise stare at a silent, stuck turn.
    Oldest first (the order ``resolve_gateway_approval`` resolves them).
    """
    with _lock:
        queue = _gateway_queues.get(session_key) or []
        return [dict(entry.data) for entry in queue]


def wait_for_approval_barrier(session_key: str | None = None,
                              *, poll_seconds: float = 0.1) -> bool:
    """Park the calling thread while this session has a human approval pending.

    This is the *barrier* half of the approval system. ``_await_gateway_decision``
    blocks only the thread that asked; every other place the agent could make
    progress for the same session — the conversation loop's next model call,
    sibling tools in a concurrent batch, a delegated child's loop — calls this
    before proceeding so that a prompt on screen means the agent is genuinely
    stopped, not "one command is stopped while the rest carries on".

    Returns True if it waited. Bounded by :func:`human_wait_ceiling` and
    interrupt-aware so a leaked pending state can't wedge the process and
    Ctrl+C / Stop still work while parked. Never raises.
    """
    try:
        key = session_key if session_key is not None else get_current_session_key()
        if not is_human_wait_pending(key):
            return False
        deadline = time.monotonic() + human_wait_ceiling(key)
        waited = False
        while is_human_wait_pending(key):
...
def resolve_gateway_approval(session_key: str, choice: str,
    ...
    *reason* is an optional free-text explanation attached to an explicit
    ...
    # "Allow for this session" is meant to mean the same thing everywhere:
    # a broad, session-wide bypass, not narrowly scoped to the one command
    # that happened to be pending. The CLI path (prompt_dangerous_approval)
    # already does this for "session"/"always"; this gateway path — used by
    # the TUI, desktop, and every messaging platform — is the one most
    # approvals actually go through, so it must do the same or the very
    # next (even unrelated) command re-prompts, defeating the point of
    # granting session trust. The hardline floor and deny rules in
    # check_all_command_guards() run before this bypass is ever consulted,
    # so genuinely catastrophic commands still can't be skipped by it.
    if choice == "session":
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Robo YOLO.
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    Args:
        allow_permanent: Retained for callback compatibility. Robo's mission
            checkpoint intentionally exposes only once/session/deny.
        smart_denied: Retained for callback compatibility. A guardian opinion
            never removes the operator's session-wide approval choice.
...
def _prompt_dangerous_approval_inner(command: str, description: str,
    ...
    if approval_callback is not None:
        try:
            callback_kwargs = {"allow_permanent": False}
    ...
    os.environ["ROBO_SPINNER_PAUSE"] = "1"
    try:
        ...
        while True:
            ...
            localized_keys = [
                key.casefold()
                for key in re.findall(r'\[([^\]]+)\]', t("approval.choose_short"))
            ]
            once_inputs = {'o', 'once'}
            session_inputs = {'s', 'session'}
            deny_inputs = {'d', 'deny'}
            if len(localized_keys) >= 3:
                once_inputs.add(localized_keys[0])
                session_inputs.add(localized_keys[1])
                deny_inputs.add(localized_keys[2])
            choice = choice.casefold()

            if choice in once_inputs:
                print(t("approval.allowed_once"))
                return "once"
            elif choice in session_inputs:
                print(t("approval.allowed_session"))
                return "session"
            elif choice in {'a', 'always'}:
                # Backward-compatible keyboard/voice alias. Permanent approval
                # is no longer offered; old "always" input grants this session.
                print(t("approval.allowed_session"))
                return "session"
            elif choice in deny_inputs:
        ...
        if "ROBO_SPINNER_PAUSE" in os.environ:
            del os.environ["ROBO_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Robo approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``ROBO_YOLO_MODE`` (frozen at import time
...
def _run_approval_gate(
    *,
    ...
    branch → prompt → ``deny/once/session`` persistence. Legacy ``always``
    responses are accepted as a session grant. The caller is
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            ROBO_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set ROBO_INTERACTIVE or "
                "ROBO_GATEWAY_SESSION to answer the prompt.",
        ...
        logger.warning(
            "%s (pattern: %s): %s — set ROBO_INTERACTIVE or "
            "ROBO_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("ROBO_EXEC_ASK"):
        ...
        if notify_cb is not None:
            ...
            if choice in {"session", "always"}:
                # Session means the whole assigned mission. Once the operator
                # grants it, do not interrupt later action classes with more
                # prompts during this live Robo session.
                enable_session_yolo(session_key)
            return {"approved": True, "message": None}
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``ROBO_INTERACTIVE``) fails CLOSED — a plugin-
...
def _await_gateway_decision(session_key: str, notify_cb, approval_data: dict,
    ...
    If another tool call in this same session already has its own approval
    pending when this one is reached (two risky actions dispatched
    concurrently), this call waits for that window to clear BEFORE notifying
    the user or starting its own timeout clock — so the two don't show
    overlapping prompts. A *safe* tool call running alongside them never
    reaches this function at all, but it does wait on the same
    ``is_human_wait_pending`` signal from a different point (see
    ``agent/tool_executor.py``'s ``_execute``, added after the coordination
    below shipped: any pending approval now holds every concurrent call, not
    just other approval-needing ones).
    Bounded by ``human_wait_ceiling()`` so a stuck/leaked pending state can't
    park this call forever; on expiry it proceeds anyway — worst case is an
    overlapping prompt, which is strictly better than a permanent hang (same
    tradeoff as the authorization gate's own serialization lock, #79719).
    ...
    """
    policy = _get_wait_policy(session_key)
    _queue_deadline = time.monotonic() + human_wait_ceiling(session_key)
    while is_human_wait_pending(session_key):
        if is_interrupted() or time.monotonic() >= _queue_deadline:
            break
        time.sleep(0.1)
    ...
    # Interactive surfaces (TUI/desktop) register ``wait_forever``: the user is
    # present, the prompt is on screen, and the agent must stay parked until
    # it is answered — never expire into a BLOCKED result the model then works
    # around. Messaging surfaces keep the bounded ``approvals.timeout``.
    wait_forever = bool(policy.get("wait_forever"))
    ...
    _now = time.monotonic()
    _deadline = (_now + _UNBOUNDED_WAIT_SECONDS) if wait_forever else (_now + max(timeout, 0))
    ...
    choice = entry.result
    surface_closed = entry.reason == "__approval_surface_closed__"
    if surface_closed:
        # Internal marker from unregister_gateway_notify(): not a user reason.
        entry.reason = None
    ...
    if surface_closed:
        _outcome = "closed"
    elif interrupted_wait:
        # The turn was interrupted (Stop, or the user typed a new instruction
        # that redirects the turn) while the prompt was open. That is neither
        # a deny nor a timeout: the surface is already stopping/redirecting
        # the agent, so no halt hook fires and the tool reports "cancelled".
    ...
    # Barrier policy: on an interactive surface a deny / expiry / teardown
    # also stops the turn. The tool still returns its BLOCKED result (so the
    # transcript records what happened), and the surface's on_halt hook
    # interrupts the agent so it does not keep working in the background.
    # A pending *redirect* (the user typed a new instruction instead of
    # answering) is not a deny: the interrupt that resolved this wait belongs
    # to that redirect, so leave it alone.
    if policy.get("halt_on_deny") and _outcome in {"deny", "timeout", "closed"}:
        on_halt = policy.get("on_halt")
        if callable(on_halt):
            try:
                on_halt("denied" if _outcome == "deny" else _outcome,
                        dict(approval_data))
            except Exception:
                logger.debug("approval on_halt hook failed", exc_info=True)
    return {"resolved": resolved, "choice": choice, "reason": entry.reason,
            "surface_closed": surface_closed, "interrupted": interrupted_wait}
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("ROBO_EXEC_ASK")
        ...
        if _is_cron_approval_context():
            if _get_cron_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from robo_cli.config import load_config_readonly as _load_cfg
    ...
    if is_gateway or is_ask:
        ...
        if notify_cb is not None:
            ...
            if not resolved or choice is None or choice == "deny":
                ...
                if decision.get("interrupted"):
                    reason = (
                        "cancelled: the user interrupted or sent a new "
                        "instruction before answering the approval prompt"
                    )
                    timeout_addendum = " Follow the user's newest instruction instead."
                    outcome = "interrupted"
                elif decision.get("surface_closed"):
                    reason = "cancelled: the approval prompt was closed unanswered"
                    timeout_addendum = " Silence is not consent."
                    outcome = "closed"
                elif not resolved:
                    reason = "timed out without user response"
        ...
        if smart_denied_for_owner:
            pending_data.update(smart_denied=True)
        ...
        if smart_denied_for_owner:
            result.update(smart_denied=True, allow_permanent=False, allow_session=True)
        return result

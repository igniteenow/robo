...
def managed_igniteenow_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Return True when the user is entitled to the Ignitee Now Tool Gateway.

    Entitlement is paid Ignitee Now Portal service access OR a live free tool pool
    ...
    try:
        ...
        if force_fresh:
            ...
            account_info = get_igniteenow_portal_account_info()
...
def igniteenow_tool_gateway_unavailable_message(
    capability: str = "the Ignitee Now Tool Gateway",
    *,
    force_fresh: bool = False,
) -> str:
    """Return account-aware guidance for an unavailable Ignitee Now Tool Gateway path."""
    try:
        ...
        account_info = get_igniteenow_portal_account_info(force_fresh=force_fresh)
        ...
        pass
    return (
        f"{capability} is unavailable. Run `robo model` to refresh your "
        "Ignitee Now Portal login and billing status."
    )
...
def resolve_modal_backend_state(
    ...
    *,
    ...
    if managed_enabled is None:
        managed_enabled = managed_igniteenow_tools_enabled()
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``robo auth add
    ...
    2. The environment / ``~/.robo/.env``. Under a multiplexed gateway turn
       ...
       ``robo_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``robo auth
    ...
    when omitted, ``robo_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `robo auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``robo auth add openai-api``) via
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.robo/.env`` (via
    ``robo_cli.config.get_env_value`` when available) so tool-side

...
<url>``) is spawned as a direct child of the Robo process. Robo's own
...
Robo process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Robo startup's opt-in

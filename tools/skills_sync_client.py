...
409), and is driven by:
  ...
  * the ``robo sync status|pull|push|now`` CLI.

It lives beside ``tools/skills_sync.py`` (NOT under ``robo_cli/``) so the
...
Client sync is unconditionally INERT (no push, no pull, no-op): no backend
credential source is currently wired up for this feature. ``resolve_identity()``
always raises :class:`SyncInertError`. The claim-decoding machinery below
(``_ADMIN_ACCESS_CLAIM``, JWT payload decode) is retained for when a
credential source is reconnected.
...
status conflates "may administer the portal" with "has Skill Sync enabled", and has
...
is toggled via ``robo sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.robo/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``robo sync enable`` — it is
# reconciled TO the manifest on pull and FROM it on push; the manifest in the
# plane is authoritative.
#
# MUST match robo-gateway ``src/sync/manifest.ts`` byte-for-byte (the server
...
def build_sync_manifest_bytes(skills: Dict[str, bool]) -> bytes:
    ...
    ``skills`` maps skill name -> enabled. Emits the shape robo-gateway's
...
def parse_sync_manifest(data: bytes) -> Optional[Dict[str, bool]]:
    ...
    Strict (mirrors robo-gateway ``parseSyncManifest``): an unknown ``type``,
...
# namespace from robo-engineer's local ``content_hash`` (skills_guard.py:846),
...
#
# No backend credential source is currently wired up for this feature — see
# resolve_identity() below, which always raises SyncInertError.
# ---------------------------------------------------------------------------
...
# Wire claim name is NAS's; it means "this user is a portal admin"
# (populated from Permissions.ADMIN_ACCESS), NOT a tool-gateway right. The
# string value below is a wire-protocol constant other services read — it
# must not change even though the Python name is generic.
_ADMIN_ACCESS_CLAIM = "tool_gateway_admin"


class SyncInertError(RuntimeError):
    """Raised (and caught by the gate-and-swallow hooks) when sync must no-op:

    not logged in, no bearer, or the caller is not a portal admin.
    """
...
def _decode_jwt_payload_unverified(token: str) -> Dict[str, Any]:
    ...
    sync at all. Mirrors the same diagnostic-decode pattern used by
    dashboard-auth provider plugins that also need to read an unverified
    claim before the real authz check runs.
    """
    try:
...
def resolve_identity() -> Dict[str, Any]:
    """Resolve the sync bearer + owner + dev-gate flag.

    Returns a dict: ``{api_key, base_url, owner, admin_access, claims}``.
    ...
    No credential backend is currently wired up for this feature — Skill
    Sync has no non-portal bearer source, so this always raises.
    """
    raise SyncInertError("no sync credentials: sync backend unavailable")


def dev_gate_open() -> bool:
    """Whether the access gate permits sync. Never raises."""
    try:
        return bool(resolve_identity().get("admin_access"))
...
# sync.base_url (or the ROBO_SYNC_BASE_URL bridge env) overrides it to point
...
DEFAULT_SYNC_BASE_URL = "https://robo.igniteenow.com"

def resolve_sync_base_url() -> Optional[str]:
    """Resolve the sync-plane base URL.

    Order: ROBO_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
    ...
    """
    env = os.getenv("ROBO_SYNC_BASE_URL")
...
# ---------------------------------------------------------------------------
# Sync feature configuration — env-first, so a Robo Cloud instance can be set
...
# precedence as base_url: the ROBO_SYNC_* env var wins, else config.yaml
# ``sync.*``, else a built-in default.
#
#   ROBO_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   ROBO_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   ROBO_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    ``ROBO_SYNC_ENABLED`` -> ``sync.enabled`` -> False. This is the master
    switch a Robo Cloud deployment sets to opt its instances into sync by
    default. It is checked by the gate-and-swallow entrypoints IN ADDITION to
    the admin-access token gate and a configured base URL — all three must hold for
    background sync to run.
    """
    return _sync_config_bool("ROBO_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """Whether an agent/user edit to an org skill is proposed automatically.

    ``ROBO_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.

    False (default): edits to an org-shared skill stay LOCAL until the user
    runs ``robo sync propose <skill>``. The skill keeps working with the
    ...
    return _sync_config_bool(
        "ROBO_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``ROBO_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.

    False (default): opt-IN — a skill syncs only after an explicit
    ``robo sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    Robo Cloud deployment wants. Per the design notes, this default is
    ...
    """
    return _sync_config_bool("ROBO_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)


# ---------------------------------------------------------------------------
# Local skill eligibility + the personal sync opt-in "sync" flag
#
# Only agent-created + user-authored skills under ~/.robo/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.robo/skills/, NOT bundled, NOT
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (Robo Cloud "on by default"):** every *eligible* skill syncs
      UNLESS its usage record explicitly carries ``sync: false``. This is what a
      deployment sets (via ``ROBO_SYNC_DEFAULT_OPT_IN``) so a user's skills
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.robo/skills/ containing
...
def stable_device_id() -> str:
    ...
    Use ``set_device_name()`` / ``robo sync device --name`` to set an explicit
    label."""
    ...
    # Robo Cloud (and any templated deployment) can seed the label
    # declaratively via ROBO_SYNC_DEVICE_NAME, so a hosted instance shows a
    # recognizable name with no CLI call. Env seeds the FIRST-USE value only; it
    # is then persisted, so a later `robo sync device --name` (or editing the
    # file) still wins on that device. An explicit file (above) always wins over
    # the env.
    import os

    env_name = (os.environ.get("ROBO_SYNC_DEVICE_NAME") or "").strip()
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.robo/skills/.sync_device_id, overwriting
...
# the caller's three-way merge. Auth is the sync bearer resolved above.
# ---------------------------------------------------------------------------
...
# ~/.robo/skills/.sync_state as JSON.
#
...
# path under ~/.robo/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.robo/skills/ (posix), or None."""
    try:
...
def snapshot_profile(
    ...
    # skill NAME (matching robo-gateway's manifest shape + the read walk that
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this Robo speaks "
            f"{WIRE_VERSION} — update Robo to sync with it"
        )
    ...
    if overlaps:
        ...
        return {
            ...
            "message": (
                ...
                f"{conflict_ref}. Resolve out-of-band (robo sync / NAS UI)."
            ),
        }
...
def pull_skills(
    ...
    *,
    ...
    ~/.robo/skills/. Only paths the user has opted into (``sync: true``) are
...
# (no push, no pull, no-op) unless the signed-in user is a portal admin.
# ---------------------------------------------------------------------------

def maybe_push_skills(*, message: str = "robo skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not identity.get("admin_access"):
            return None  # access gate: inert unless the user is a portal admin
        if not sync_feature_enabled():
            return None  # feature off for this instance (ROBO_SYNC_ENABLED)
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``robo sync status``. Never raises."""
    status: Dict[str, Any] = {
        "admin_access": False,
        "logged_in": False,
    ...
    try:
        ...
        status["admin_access"] = bool(identity.get("admin_access"))
    except SyncInertError:
        pass
    except Exception as e:
...
#
# Org skills live under a DISTINCT local namespace, ~/.robo/skills/_org/
...
#
# `robo sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.robo/skills/_org/<org_id>/``.

...
One ``CDPSupervisor`` runs per Robo ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Robo logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "robo-dialog-bridge.invalid"
...
(() => {
  if (window.__roboDialogBridgeInstalled) return;
  window.__roboDialogBridgeInstalled = true;
  const ENDPOINT = "http://robo-dialog-bridge.invalid/";

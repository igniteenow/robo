"""
Lazy dependency installer for opt-in Robo Agent backends.

Many Robo features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``robo-engineer[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``robo tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``ROBO_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/robo`` read-only), setting
  ``ROBO_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Robo, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    "wake.openwakeword": (
        "openwakeword==0.6.0",
        "onnxruntime==1.27.0",
        # openwakeword imports these at runtime but its Linux metadata also
        # unconditionally pulls tflite-runtime.  Robo's bundled wake model is
        # ONNX, and tflite-runtime has no wheels for several supported Python
        # and architecture combinations (including Python 3.13 on Kali).
        # ensure() installs openwakeword itself with --no-deps, then installs
        # this explicit ONNX dependency set cross-platform.
        "tqdm==4.70.0",
        "scipy>=1.17.1,<2",
        "scikit-learn==1.9.0",
        "requests==2.33.0",
    ...
    # (ROBO_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # HF Agent Trace Viewer upload (robo trace upload / /upload-trace).
    #
    ...
    # presence, the `robo update` lazy-refresh pass re-asserts THIS pin on
    ...
    "tool.trace_upload": ("huggingface-hub==1.32.0",),
}
...
_LAZY_TARGET_ENV = "ROBO_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``ROBO_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("ROBO_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Robo under WSL to use Matrix on Windows."
        )
    return None
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``robo update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="robo-core-constraints-", suffix=".txt")
...
def _venv_pip_install(
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``robo_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = robo_subprocess_env(inherit_credentials=False)
        uv_env["VIRTUAL_ENV"] = str(venv_root)

        # Tier 1: uv (preferred — fast, doesn't need pip in the venv)
        # Managed uv first: $ROBO_HOME/bin is never on PATH, so a bare
        # which() misses the uv Robo installed and falls through to the
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # Package-manager installs (NixOS, and any other distro that ships Robo
    ...
    # deployment sets ROBO_MANAGED=true *and* ROBO_LAZY_INSTALL_TARGET
    ...
    if _lazy_install_target() is None:
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"packages come from {managed_by}, so Robo cannot install "
                f"them at runtime. Add the dependencies for {feature!r} via "
                f"{managed_by} (or run a pip/uv install of Robo instead)."
            )
    ...
    # openwakeword 0.6.0's Linux metadata requires tflite-runtime even when
    # the application selects ONNX.  tflite-runtime is unavailable on some
    # Python/architecture combinations that Robo supports.  Install only the
    # trusted package wheel here, without its metadata dependencies, then let
    # the normal pinned allowlist install the complete ONNX runtime set.
    result: _InstallResult
    if feature == "wake.openwakeword":
        openwakeword_specs = tuple(
        ...
        if openwakeword_specs:
            result = _venv_pip_install(openwakeword_specs, no_deps=True)
            if result.success and remaining_specs:
                result = _venv_pip_install(remaining_specs)
        else:
            result = _venv_pip_install(remaining_specs)
    else:
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``ROBO_DISABLE_LAZY_INSTALLS=1``) and sets
      ``ROBO_LAZY_INSTALL_TARGET``, installs are redirected to the writable
...
def active_features() -> list[str]:
    ...
    Used by ``robo update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``robo update``. Never raises; lazy-install failures

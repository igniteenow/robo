...
_FORCE_SYNC_ENV = "ROBO_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.robo") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.robo to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``ROBO_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, robo_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.robo/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(robo_home=robo_home)

    def _sync_back_transaction(self, robo_home: Path | None = None) -> None:
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="robo-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.robo/skills/a.md`` →
        ``~/.robo/skills/a.md``, a new remote file at
        ``/root/.robo/skills/b.md`` maps to ``~/.robo/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []

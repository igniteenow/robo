...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Robo was already launched from (for example ``robo-engineer``
    while the process cwd is ``~/.robo/robo-engineer``), passing it through
    unchanged makes the wrapper run ``cd robo-engineer`` *inside* the project
    ...
    # Common recovery for config values like ``robo-engineer`` when Robo was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./robo-engineer``; use the current directory instead.
...
# Robo-internal env vars that should NOT leak into terminal subprocesses.
_ROBO_PROVIDER_ENV_FORCE_PREFIX = "_ROBO_FORCE_"

# Robo-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Robo inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    Cached (computed once, on first actual use) rather than at import time.
    This function's only real dependency is the PROVIDER_REGISTRY *data*,
    but reaching it means importing all of robo_cli.auth, which eagerly
    imports httpx (~75ms) for its own, unrelated HTTP-calling functions.
    Most process starts never touch a terminal command at all, let alone
    one whose env needs this blocklist, so paying that cost unconditionally
    on every import of this module was pure startup-time waste. lru_cache
    keeps the actual result identical and reused after the first call —
    only *when* it's computed changes, not what it computes.
    """
    blocked: set[str] = set()
    ...
    try:
        from robo_cli.config import OPTIONAL_ENV_VARS
    ...
    # Robo-managed inference credential — Claude subscription auth is not a
    # working Robo provider path.  Stripping it broke agent-spawned
...
# project's dependencies into the Robo venv path instead of the project's own
# ``.venv`` — silently clobbering the Robo environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Robo venv stays reachable via PATH (its bin dir is first), so stripping
# these markers is safe and only prevents the cross-project clobber (#23473).
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX")


def _is_robo_internal_secret(key: str) -> bool:
    """Return True for Robo-internal secrets injected under *dynamic* names.

    ``_build_provider_env_blocklist()`` is name-based and derived from the
    ...
    This is the single source of truth for "Robo-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`robo_subprocess_env` helper all call it, so the
...
def _inject_context_robo_home(env: dict) -> None:
    """Bridge the context-local Robo home override into subprocess env."""
    try:
        from robo_constants import get_robo_home_override

        value = get_robo_home_override()
        if value:
            env["ROBO_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (ROBO_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Robo-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_ROBO_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_robo_internal_secret(key):
            continue
        passthrough = _is_passthrough(key)
        if key in _build_provider_env_blocklist() and not passthrough:
            continue
    ...
    for key, value in (extra_env or {}).items():
        if key.startswith(_ROBO_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_ROBO_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_robo_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_robo_internal_secret(key):
            continue
        else:
            passthrough = _is_passthrough(key)
            if key in _build_provider_env_blocklist() and not passthrough:
    ...
    _inject_context_robo_home(sanitized)

    from robo_constants import apply_subprocess_home_env
...
# legitimate child Robo spawns needs them, and they are the highest-value
...
# narrow subset of _build_provider_env_blocklist(); provider keys are handled by
# the conditional Tier-2 strip in robo_subprocess_env().
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_robo_internal_secret, but _ID has no secret suffix, so it must be
...
def robo_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_build_provider_env_blocklist()``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Robo spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_build_provider_env_blocklist()``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and Robo-internal dynamic secrets
    ...
    # legitimate use for them. See :func:`_is_robo_internal_secret`.
    for key in list(env):
        if key.startswith(_ROBO_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_robo_internal_secret(key):
    ...
    if not inherit_credentials:
        ...
        for key in _build_provider_env_blocklist():
            env.pop(key, None)

    # Windows UTF-8 safety for spawned processes (#31420).
    env.setdefault("PYTHONUTF8", "1")

    _inject_context_robo_home(env)
    from robo_constants import apply_subprocess_home_env
    ...
    # copies os.environ, whose ROBO_SESSION_* mirror is a last-writer-wins
    ...
    # still see the parent's ROBO_HOME but lose the DB mutation guard.
...
def build_subprocess_env(
    ...
    *,
    ...
    function (or :func:`robo_subprocess_env` for the model-driving-CLI
    surface) instead of copying ``os.environ`` directly, so profile-home
    propagation (``ROBO_HOME`` / subprocess ``HOME`` contract) and the
    Robo secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``scrub_secrets=True`` (default) — delegate to
      ...
      list (provider blocklist + ``_is_robo_internal_secret`` dynamic
      patterns + kanban/venv-marker/session-context guards) **and** of
      ``ROBO_HOME`` / subprocess-HOME propagation.  On this path profile
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Robo home override into ``ROBO_HOME`` and apply the
      subprocess HOME contract (``robo_constants.apply_subprocess_home_env``).
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``ROBO_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        # _sanitize_subprocess_env already performs ROBO_HOME override
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("ROBO_GIT_BASH_PATH")
    if custom and os.path.isfile(custom):
        candidates.append(custom)

    # Prefer our own portable Git install — a broken or partially-uninstalled
    # system Git (or a stale ROBO_GIT_BASH_PATH pointing at one) must not
    ...
    #   PortableGit: %LOCALAPPDATA%\robo\git\bin\bash.exe   (primary)
    ...
    # ROBO_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    # (``Directory \\drivers\\etc does not exist``) must not win over a
    # healthy portable Git under %LOCALAPPDATA%\\robo\\git.
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "ROBO_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
    ...
    raise RuntimeError(
        "Git Bash not found. Robo Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set ROBO_GIT_BASH_PATH to your bash.exe location."
    )
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart Robo. If the override is blocked or later re-applied, "
...
# Cached directory containing the ``robo`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_ROBO_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_robo_bin_dir() -> str | None:
    """Return the directory holding the ``robo`` console-script, or None.
    ...
    robo install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``robo``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``robo`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``robo`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("robo")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``robo`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _ROBO_BIN_DIR
    if _ROBO_BIN_DIR is not _SENTINEL:
        return _ROBO_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("robo")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "robo" or base.startswith("robo."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "robo.exe" if _IS_WINDOWS else "robo"
    ...
    _ROBO_BIN_DIR = candidate
    return candidate


def _prepend_robo_bin_dir(existing_path: str) -> str:
    """Prepend the robo install dir to ``existing_path`` if it's missing.
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing Robo-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes Robo installs for
    itself, so on a machine where Robo provisioned its own toolchain a
    command the agent runs resolves a system copy instead — or nothing at all:

    - ``$ROBO_HOME/node`` (+ ``/bin``) — installed to satisfy the desktop and
      browser toolchain. ``tools/browser_tool.py`` already does this for its own
      subprocesses; the agent's shell deserves the same.
    - ``$ROBO_HOME/bin`` — the managed ``uv``. ``install.sh`` writes it there
    ...
    ``get_robo_home()`` is profile-scoped and a managed tree can appear
    mid-process (``heal_robo_managed_node``, a first browser install).
    """
    try:
        from robo_constants import get_robo_home, iter_robo_node_dirs

        candidates = [*iter_robo_node_dirs(), get_robo_home() / "bin"]
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    Robo-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Robo
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_ROBO_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_ROBO_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_robo_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_robo_internal_secret(k):
            continue
        else:
            passthrough = _is_passthrough(k)
            if k in _build_provider_env_blocklist() and not passthrough:
                continue
    ...
    if path_key is not None:
        ...
        # Ensure the robo install dir is reachable so plugins can shell out
        # to bare ``robo`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_robo_bin_dir(new_path)

    _inject_context_robo_home(run_env)
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Robo trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``ROBO_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under ROBO_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_robo_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "robo_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._robo_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_robo_pgid", None)
                    if pgid is None:
                        raise

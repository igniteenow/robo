...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Robo exit
...
# display_robo_home imported lazily at call site (stale-module safety during robo update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of robo directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "robo-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("ROBO_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("ROBO_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("ROBO_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (ROBO_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (ROBO_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # interactive Robo password prompt or the sudo -S password-pipe path.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``robo-engineer=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by Robo processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      Robo processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling Robo processes a generous grace
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, RoboSweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.robo/.env or environment variables."
        )
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    expand itself, so the Robo host/container must NOT ``expanduser`` it.

    SSH ``cwd`` is interpreted by the *remote* shell (``cd ~`` / ``cd ~/x``
    over ``ssh ... bash -c``). Expanding ``~`` locally would rewrite it to the
    Robo host HOME (often ``/opt/data`` under Docker) and inject a
...
def _ensure_terminal_env_bridged() -> None:
    ...
    vars at startup — but processes that skip all of those paths (``robo
    ...
    be stale from ``robo setup``). Environment values for omitted terminal
    ...
    try:
        from robo_cli.config import apply_terminal_config_to_env, read_raw_config
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Startup orphan reaper for robo-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior Robo processes that hit SIGKILL / OOM / a closed terminal
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["mode"] == "managed":
                raise ValueError(
                    "Modal backend is configured for managed mode, but managed "
                    "Modal execution is no longer available. Set "
                    "TERMINAL_MODAL_MODE=direct (or remove the setting) and "
                    "configure direct Modal credentials."
                )
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "notify_on_complete=true) so Robo tracks the process, then run readiness "
...
def terminal_tool(
    ...
    try:
        ...
        # Hard-block: gateway lifecycle commands (systemctl/launchctl/robo
        # restart|stop targeting robo-gateway) must never run inside the
        ...
        # never restart. This mirrors the `robo gateway restart` guard in
        # robo_cli/gateway.py and the cron-path guard in robo_cli/cron.py,
        # but applies unconditionally (force=True cannot help here).
        if os.environ.get("_ROBO_GATEWAY") == "1":
            ...
            if contains_launchctl_submit_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Use Robo cron for one-shot delayed work, or install an "
            ...
            if contains_gateway_lifecycle_command_or_referenced_script(
                ...
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "to child processes). Run `robo gateway restart` from a "
        ...
        if background:
            ...
            try:
                ...
                # robo-engineer dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in robo-engineer dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        result_data["notify_unsupported"] = (
                            ...
                            "the turn ends (a one-shot runner such as `robo -z`, a "
                        ...
                        _gw_platform = _gse("ROBO_SESSION_PLATFORM", "")
                        if _gw_platform:
                            _gw_chat_id = _gse("ROBO_SESSION_CHAT_ID", "")
                            _gw_thread_id = _gse("ROBO_SESSION_THREAD_ID", "")
                            _gw_user_id = _gse("ROBO_SESSION_USER_ID", "")
                            _gw_user_name = _gse("ROBO_SESSION_USER_NAME", "")
                            _gw_message_id = _gse("ROBO_SESSION_MESSAGE_ID", "")
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                if has_sudo_prompt_callback or env_var_enabled("ROBO_INTERACTIVE"):
                    output += (
    ...
    try:
        ...
        if env_type == "local":
            ...
            if modal_state["selected_backend"] != "direct":
                if modal_state["mode"] == "managed":
                    logger.error(
                        "Modal backend selected with TERMINAL_MODAL_MODE=managed, but managed "
                        "Modal execution is no longer available. Set "
                        "TERMINAL_MODAL_MODE=direct and configure direct Modal credentials."
                    )
                    return False

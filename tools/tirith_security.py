...
it is automatically downloaded from GitHub releases to $ROBO_HOME/bin/tirith.
...
def _get_robo_home() -> str:
    """Return the Robo home directory, respecting ROBO_HOME env var."""
    return str(get_robo_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_robo_home(), ".tirith-install-failed")
...
def _robo_bin_dir() -> str:
    """Return $ROBO_HOME/bin, creating it if needed."""
    d = os.path.join(_get_robo_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ROBO_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_robo_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ROBO_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ROBO_HOME/bin/tirith
    ...
    robo_bin = os.path.join(_robo_bin_dir(), "tirith")
    if os.path.isfile(robo_bin) and os.access(robo_bin, os.X_OK):
        _resolved_path = robo_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return robo_bin

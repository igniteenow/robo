...
selectable via ``robo tools`` → Image Generation; the active model is
...
FAL_MODELS: Dict[str, Dict[str, Any]] = {
    ...
    "fal-ai/gpt-image-2": {
        ...
        "defaults": {
            # Same quality pinning as gpt-image-1.5: medium keeps per-image
            # cost predictable. "high" is 3-4x the per-image
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.robo"
    ...
    # Robo cache roots can be translated without side effects. SSH can still
    ...
    try:
        ...
        if source_images and not edit_endpoint:
            raise ValueError(
                ...
                f"via `robo tools` → Image Generation."
            )
...
def _build_no_backend_setup_message() -> str:
    ...
    lines.append(
        "  2. Configure a different image_gen provider via `robo tools` "
        "→ Image Generation (run `robo plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `robo plugins list` to see "
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `robo tools` → Image Generation."
                ),
...
# model/provider via `robo tools` or `/skills`.

"""Wake-word ("Hey Robo") detection — hands-free session trigger.
...
wake word, Robo opens a fresh session and captures voice via the existing
...
* **openwakeword** (default, free, no API key) — loads an ONNX model. Defaults
  to the bundled "hey robo" model (``tools/wakewords/``) so the wake word
...
# Minimum gap between two consecutive wake fires, so one "hey robo" can't
...
# Bundled "hey robo" model (tools/wakewords/) — the default, so the wake word
# works out of the box. Config names in _ALIASES resolve to it, not a built-in.
# The on-disk asset is still named hey_roh_boh.onnx/.tflite (see
# tools/wakewords/README.md) — kept as-is rather than renamed, since that's a
# real trained model file, not a text label; the aliases below map every
# name a config might reasonably use, old or new, onto that same file.
_BUNDLED_MODEL_NAME = "hey_roh_boh"
_BUNDLED_MODEL_ALIASES = frozenset(
    {"", "hey_roh_boh", "hey roh boh", "hey_robo", "hey robo", "robo", "roh boh"}
)


def _bundled_wakeword_path(
    framework: str = "onnx", *, model_name: str = _BUNDLED_MODEL_NAME
) -> str:
    """Path to a shipped wake model (.onnx/.tflite) for ``framework``."""
    ext = "tflite" if str(framework).strip().lower() == "tflite" else "onnx"
    return os.path.join(os.path.dirname(__file__), "wakewords", f"{model_name}.{ext}")
...
def wake_phrase(cfg: Optional[Dict[str, Any]] = None) -> str:
    ...
    return str(_get(cfg, "phrase")) or "hey robo"
...
def enrolled_profile_phrases() -> Dict[str, str]:
    ...
    try:
        ...
        from robo_cli.profiles import get_profile_dir, list_profiles
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return (
            "Microphone delivers only silence. Grant the Robo backend "
...
class _OpenWakeWordEngine(_Engine):
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # Default (or explicit "hey_roh_boh") → the bundled model; a built-in name
...
# ROBO_HOME. Keywords are typed phrases tokenized at RUNTIME — no
...
def _sherpa_model_root() -> Path:
    ...
    return get_robo_home() / "cache" / "wakewords"
...
class _SherpaKwsEngine(_Engine):
    ...
    model's vocabulary, so "hey robo", "hey coder", or any other phrase works
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # wake any profile ("hey robo" / "hey coder" / ...). display-name →
        # profile is kept for routing the match back.
        phrase = str(_get(cfg, "phrase") or "hey robo").strip()
        ...
        kw = tempfile.NamedTemporaryFile(
            mode="w", suffix=".txt", prefix="robo-kws-", delete=False, encoding="utf-8"
        )
...
def check_wake_word_requirements(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if provider == "porcupine" and not (os.getenv("PORCUPINE_ACCESS_KEY") or "").strip():
        ...
        hint = (f"Wake word needs {missing} configured — run `robo tools` "
...
# ---------------------------------------------------------------------------
# Process-wide singleton (mirrors robo_cli.voice's continuous API)
# ---------------------------------------------------------------------------
...
def _lock_path() -> Path:
    ...
    return get_default_robo_root() / "runtime" / "wake-word.lock"

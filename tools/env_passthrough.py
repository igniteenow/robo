...
def _is_robo_provider_credential(name: str) -> bool:
    """True if ``name`` is a Robo-managed provider credential (API key,
    token, or similar) per ``_build_provider_env_blocklist()``.
    ...
    Non-Robo API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Robo credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated Robo-internal secrets (AUXILIARY_*_API_KEY /
    ...
    if _is_robo_internal_secret(name):
        return True
    return name in _build_provider_env_blocklist()
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Robo-managed provider credentials (from
    ``_build_provider_env_blocklist()``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Robo-managed
    ...
    Non-Robo third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_robo_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Robo provider "
                "credential %r (blocked by _build_provider_env_blocklist()). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Robo-managed provider credentials must not be passed
                ...
                if _is_robo_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Robo "
                        "provider credential %r from config.yaml (blocked "
                        "by _build_provider_env_blocklist()). Operator "

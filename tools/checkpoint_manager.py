...
-----------------------------------------------------------------------------
    ...
    ~/.robo/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/robo/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_robo_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/robo"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Robo convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("ROBO_CHECKPOINT_TIMEOUT", 30)))
...
def _git_env(
    ...
    The shared store is internal Robo infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``robo checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `robo checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "robo@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Robo Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``ROBO_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at ROBO_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "ROBO_WORKDIR").write_text(
...
class CheckpointManager:
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has Robo changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        Robo changed", not an exact per-session ledger.
...
# ---------------------------------------------------------------------------
# Public helpers for `robo checkpoints` CLI
# ---------------------------------------------------------------------------

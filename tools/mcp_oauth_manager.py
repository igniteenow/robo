...
`mcp_tool.py`, and `robo_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# RoboMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_robo_provider_class() -> Optional[type]:
    ...
    class RoboMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._robo_server_name = server_name
            self._robo_home = ""
            ...
            self._robo_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`RoboTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, RoboTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._robo_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import RoboTokenStorage
                        if isinstance(storage, RoboTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._robo_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import RoboTokenStorage
            if not isinstance(storage, RoboTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``robo mcp reauth``.
            ...
            back to ``robo mcp reauth``.
            """
            try:
                if self._robo_preregistered:
                    return
    ...
    return RoboMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_ROBO_PROVIDER_CLS: Optional[type] = _make_robo_provider_class()
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._robo_home = key[0]
    ...
    def _build_provider(
        ...
        Constructs :class:`RoboMCPOAuthProvider` directly using the helpers
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `robo mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``robo mcp remove <name>`` and (indirectly) by
        ``robo mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, robo_home), None)

        from tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, robo_home=robo_home)
        logger.info(
    ...
    def restore_entry(
        ...
        *,
        ...
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, robo_home), entry)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, robo_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        entry = self._entries.get(self._key(server_name, robo_home))
        if entry is None or entry.provider is None:
            return False

        async with entry.lock:
            tokens_path = _get_token_dir(robo_home) / f"{_safe_filename(server_name)}.json"
            try:

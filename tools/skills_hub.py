#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Robo Skills Hub.
...
Used by robo_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _robo_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            robo_meta = metadata.get("robo", {})
            if isinstance(robo_meta, dict):
                tags = robo_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Ignitee Now) but not activated
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``robo_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
# ---------------------------------------------------------------------------
# Robo centralized index source
# ---------------------------------------------------------------------------

ROBO_INDEX_URL = "https://robo.igniteenow.com/docs/api/skills-index.json"
ROBO_INDEX_TTL = 6 * 3600  # 6 hours


def _robo_index_cache_file() -> Path:
    return _index_cache_dir() / "robo-index.json"


def _load_robo_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ROBO_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    robo_index_cache_file = _robo_index_cache_file()
    if robo_index_cache_file.exists():
        try:
            age = time.time() - robo_index_cache_file.stat().st_mtime
            if age < ROBO_INDEX_TTL:
                return json.loads(robo_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Robo index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Robo index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Robo index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        robo_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        robo_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class RoboIndexSource(SkillSource):
    """Skill source backed by the centralized Robo Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "robo-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "robo-index"),
            identifier=entry.get("identifier", ""),
...
# Third-party skill registries an operator is allowed to switch off. First-party
# sources (bundled optional skills, the Robo index, direct URLs) are not listed
# here on purpose: disabling them would break `robo skills install <name>`.
_DISABLEABLE_SOURCE_IDS = frozenset(
    {"skills-sh", "well-known", "github", "clawhub", "lobehub", "browse-sh"}
)


def _disabled_source_ids() -> frozenset:
    """Registry ids the operator has opted out of.

    Read from ``ROBO_SKILLS_DISABLED_SOURCES`` (comma separated) and from
    ``skills.disabled_hub_sources`` in config.yaml. Unknown or first-party ids
    are ignored rather than raising, so a typo can never take the hub down.
    """
    wanted: set = set()
    raw = os.environ.get("ROBO_SKILLS_DISABLED_SOURCES", "")
    wanted.update(part.strip().lower() for part in raw.split(",") if part.strip())
    try:
        from robo_cli.config import load_config_readonly

        cfg = load_config_readonly() or {}
        listed = (cfg.get("skills") or {}).get("disabled_hub_sources") or []
        if isinstance(listed, str):
            listed = listed.split(",")
        wanted.update(str(item).strip().lower() for item in listed if str(item).strip())
    except Exception as exc:  # config must never break skill search
        logger.debug("skills.disabled_hub_sources unreadable: %s", exc)
    return frozenset(wanted & _DISABLEABLE_SOURCE_IDS)
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    Third-party registries named in ``skills.disabled_hub_sources`` (or the
    ``ROBO_SKILLS_DISABLED_SOURCES`` env var) are left out.
    """
    if auth is None:
        auth = GitHubAuth()
    ...
    sources: List[SkillSource] = [
        ...
        RoboIndexSource(auth=auth), # Centralized index (search + resolved install paths)
    ...
    if disabled:
        sources = [src for src in sources if src.source_id() not in disabled]

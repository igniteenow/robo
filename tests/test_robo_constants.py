"""Tests for robo_constants module."""
...
from robo_constants import (
    ...
    find_robo_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_robo_root,
    get_robo_dir,
    get_robo_home,
    get_process_robo_home,
    heal_robo_managed_node,
    robo_managed_node_tree_present,
    iter_robo_node_dirs,
    is_container,
    node_tool_runnable,
...
class TestGetDefaultRoboRoot:
    """Tests for get_default_robo_root() — Docker/custom deployment awareness."""

    def test_no_robo_home_returns_native(self, tmp_path, monkeypatch):
        """When ROBO_HOME is not set, returns ~/.robo."""
        monkeypatch.delenv("ROBO_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_robo_root() == tmp_path / ".robo"
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (ROBO_HOME=<root>/profiles/<name>),
        returns the Docker root, not the profile dir."""
        docker_root = tmp_path / "opt" / "data"
        profile = docker_root / "profiles" / "coder"
        profile.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ROBO_HOME", str(profile))
        assert get_default_robo_root() == docker_root

    def test_no_robo_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\robo, not ~/.robo."""
        ...
        assert get_default_robo_root() == local_appdata / "robo"



class TestGetRoboHome:
    """Tests for get_robo_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When ROBO_HOME is unset on Windows, use %LOCALAPPDATA%\\robo."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("ROBO_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(robo_constants.sys, "platform", "win32")
        monkeypatch.setattr(robo_constants, "_profile_fallback_warned", False)

        assert get_robo_home() == local_appdata / "robo"


class TestGetProcessRoboHome:
    """Tests for get_process_robo_home() — process launch scope.
    ...
    def test_env_set_returns_that_path(self, tmp_path, monkeypatch):
        ...
        assert get_process_robo_home() == home




class TestRoboManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_robo_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(robo_constants.sys, "platform", "win32")
        monkeypatch.setenv("ROBO_HOME", str(home))
        monkeypatch.setattr(robo_constants, "node_tool_runnable", lambda path: True)

        assert find_robo_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(robo_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(robo_constants, "heal_robo_managed_node", lambda: False)
        monkeypatch.setattr(
            robo_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert robo_managed_node_tree_present() is True
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Robo-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(robo_constants, "heal_robo_managed_node", _heal)
    ...
    def test_outdated_managed_node_heals_to_target_major(self, tmp_path, monkeypatch):
        ...
        target = robo_constants._ROBO_NODE_TARGET_MAJOR
        profile_home = tmp_path / "profiles" / "assistant"
        managed_bin = profile_home / "node" / "bin"
        managed_bin.mkdir(parents=True)
        old_node = self._stub(
        ...
        resolved = robo_constants.find_robo_node_executable("node")
        assert heal_called["value"] is True
    ...
    def test_outdated_managed_node_survives_failed_heal(self, tmp_path, monkeypatch):
        ...
        assert robo_constants.find_robo_node_executable("node") == str(old_node)
    ...
    def test_target_major_managed_node_does_not_heal(self, tmp_path, monkeypatch):
        ...
        assert robo_constants.find_robo_node_executable("node") == str(node)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(robo_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(robo_constants, "_container_detected", True)
        assert is_container() is True
...
class TestResolvePerModelReasoningEffort:
    ...
    def test_exact_match(self):
        ...
        from robo_constants import resolve_per_model_reasoning_effort
...
class TestResolveReasoningConfig:
    ...
    def test_malformed_sections_tolerated(self):
        ...
        from robo_constants import resolve_reasoning_config
        # Malformed sections fall back to the deep-thinking default (high),
        # never to an error.
        from robo_constants import parse_reasoning_effort
        assert resolve_reasoning_config({"agent": "oops", "model": 42}) == parse_reasoning_effort("high")
        assert resolve_reasoning_config({"agent": None, "model": None}) == parse_reasoning_effort("high")
        assert resolve_reasoning_config({"agent": {"reasoning_overrides": "bad"}}) == parse_reasoning_effort("high")
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".robo"
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import robo_cli._subprocess_compat as subprocess_compat
...
class TestGetRoboDir:
    """Tests for ``get_robo_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_robo_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_is_file_treated_as_content(self, tmp_path, monkeypatch):
        ...
        result = get_robo_dir("cache/images", "image_cache")
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_to_wsl_mount(self):
        assert robo_constants.windows_path_to_wsl(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert robo_constants.windows_path_to_wsl("C:/Users/alex") == "/mnt/c/Users/alex"
        assert robo_constants.windows_path_to_wsl("D:\\") == "/mnt/d/"

    def test_windows_drive_ignores_non_drive_paths(self):
        assert robo_constants.windows_path_to_wsl("/home/alex") is None
        assert robo_constants.windows_path_to_wsl("relative\\dir") is None




    def test_translate_maps_windows_and_unc_on_wsl(self, monkeypatch):
        monkeypatch.setattr(robo_constants, "is_wsl", lambda: True)
        assert robo_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert robo_constants.translate_cwd_for_wsl_backend(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        # Already-POSIX paths pass through untouched.
        assert robo_constants.translate_cwd_for_wsl_backend("/home/alex") == "/home/alex"

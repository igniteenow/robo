"""
Tests for timezone support (robo_time module + integration points).
...
def _reset_robo_time_cache():
    """Reset the robo_time module cache (replacement for removed reset_cache)."""
    robo_time._cached_tz = None
    robo_time._cached_tz_name = None
    robo_time._cache_resolved = False


# =========================================================================
# robo_time.now() — core helper
# =========================================================================

class TestRoboTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_robo_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["ROBO_TIMEZONE"] = "Asia/Kolkata"
        result = robo_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["ROBO_TIMEZONE"] = "Europe/London"
        tz = robo_time.get_timezone()
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When ROBO_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) ROBO_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("ROBO_TIMEZONE=" + os.environ.get("ROBO_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "ROBO_TIMEZONE=NOT_SET" in result["output"], (
            "ROBO_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When ROBO_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("ROBO_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=robo_tz) which shifted
        absolute time when system-local tz != Robo tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Robo tz
        ...
        # Use a Robo timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _robo_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["ROBO_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_robo_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Robo timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["ROBO_TIMEZONE"] = "US/Eastern"
        _reset_robo_time_cache()

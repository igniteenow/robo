...
binary download are exercised manually by `robo secrets bitwarden
...
def robo_home(tmp_path, monkeypatch):
    """Point Robo at an isolated home directory."""
    home = tmp_path / ".robo"
    home.mkdir()
    monkeypatch.setenv("ROBO_HOME", str(home))
    # Some modules cache get_robo_home; clear if needed.
    import robo_constants
    if hasattr(robo_constants, "_ROBO_HOME_CACHE"):
        robo_constants._ROBO_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(robo_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from robo_cli.env_loader import _apply_external_secret_sources

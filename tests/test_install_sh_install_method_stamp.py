...
($INSTALL_DIR), not into the shared $ROBO_HOME.

Background (shared-$ROBO_HOME bug)
------------------------------------
$ROBO_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.robo:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$ROBO_HOME the two installs would fight over one slot — and the container,
booting last, would win and wrongly make the host install look like 'docker'
(blocking ``robo update``).
...
``~/.robo/robo-engineer``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$ROBO_HOME/\.install_method"', text), (
        "install.sh must not stamp $ROBO_HOME/.install_method — that data "
        "dir may be shared with a Docker gateway whose 'docker' stamp would "
        "clobber it and block host-side `robo update`"
    )

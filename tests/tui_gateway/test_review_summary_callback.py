...
def server():
    ...
    with patch.dict(
        "sys.modules",
        {
            "robo_constants": MagicMock(
                get_robo_home=MagicMock(return_value="/tmp/robo_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'robo-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'robo-release' patched"
    }

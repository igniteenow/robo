...
def robo_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves ROBO_HOME.
...
@pytest.fixture()
def server(robo_home, monkeypatch):
    ...
    # Pin config resolution to the isolated ROBO_HOME. Sibling test
    ...
    # module-level ``_robo_home = get_robo_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_robo_home", robo_home)
...
def test_moa_bare_returns_usage(server, session, robo_home):
    _write_moa_config(robo_home, """
moa:

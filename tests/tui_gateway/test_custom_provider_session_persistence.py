...
"No LLM provider configured. Run `robo model`..." (resume failed) or
...
def _make_agent_with_override(override, monkeypatch, config, model_cfg=None):
    ...
    # Keep credential-pool resolution off the developer's real ROBO home.
...
# provider (e.g. Ignitee Now) but who switched THIS session to a self-hosted model
...
# found" (the b200/robo-ultra-sft report). The stored MODEL NAME is the one
...
ULTRA_CONFIG = {
    ...
    "model": {"default": "some-igniteenow-model", "provider": "igniteenow"},
    "providers": {
        "robo-ultra": {
            "api": ULTRA_URL,
            "api_key": "sk-ultra",
            "models": ["robo-ultra-sft"],
        }
    },
}
...
class TestModelNameRecoversEntryIdentity:
    def test_identity_by_model_from_providers_dict_models_list(self, monkeypatch):
        ...
        assert (
            rp.find_custom_provider_identity_by_model("robo-ultra-sft")
            == "custom:robo-ultra"
        )

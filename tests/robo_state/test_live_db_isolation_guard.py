...
developer's REAL ~/.robo/state.db, and a pytest-spawned process flipped
...
class TestProductionPathRefused:
    def test_explicit_production_db_path_raises(self):
        """SessionDB pointed at the real ~/.robo/state.db must fail hard."""
    ...
    def test_unnormalized_production_path_raises(self):
        ...
        sneaky = Path.home() / "subdir" / ".." / ".robo" / "state.db"
        with pytest.raises(RuntimeError, match="live-system guard"):
    ...
    def test_default_resolution_to_production_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(
            robo_state, "DEFAULT_DB_PATH", robo_state._IMPORT_DEFAULT_DB_PATH
        )
        with pytest.raises(RuntimeError, match="live-system guard"):
            SessionDB()
...
class TestHermeticPathsAllowed:
    ...
    def test_tmp_robo_home_default_resolution_works(self, tmp_path, monkeypatch):
        """Argless SessionDB() under a hermetic ROBO_HOME must succeed."""
        monkeypatch.setenv("ROBO_HOME", str(tmp_path / "hermetic-home"))
        monkeypatch.setattr(
            robo_state, "DEFAULT_DB_PATH", robo_state._IMPORT_DEFAULT_DB_PATH
        )
        db = SessionDB()
...
class TestBypassMarker:
    ...
    def test_bypass_marker_disables_state_db_guard(self):
        ...
        """
        robo_state._ensure_test_isolation(REAL_ROOT / "state.db")
...
class TestSessionStoreLoudFailure:
    def test_guard_error_is_not_swallowed_into_jsonl_fallback(
        ...
        monkeypatch.setattr(robo_state, "SessionDB", _boom)
        with pytest.raises(RuntimeError, match="live-system guard"):
...
class TestSubprocessChildCovered:
    def test_child_without_robo_home_is_refused(self, tmp_path):
        """A subprocess child of a test (no ROBO_HOME) must be blocked.
        ...
        env = {
            ...
            if k not in ("ROBO_HOME", "PYTEST_PLUGINS", "PYTHONPATH")
        }
    ...
    def test_child_with_tmp_robo_home_succeeds(self, tmp_path):
        """Same child, hermetic ROBO_HOME: must work — no false positive."""
        env = {
            k: v
            for k, v in os.environ.items()
        ...
        env["PYTEST_CURRENT_TEST"] = "tests/fake.py::test_child (call)"
        env["ROBO_HOME"] = str(tmp_path / "child-home")
        env["PYTHONPATH"] = str(Path(__file__).resolve().parents[2])
        code = (
            "from robo_state import SessionDB\n"
            "db = SessionDB()\n"

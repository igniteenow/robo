"""Harness: dashboard opt-in via ROBO_DASHBOARD.

Today (tini): dashboard starts once when ROBO_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``robo`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without ROBO_DASHBOARD, no dashboard process should be running."""
    start_container(built_image, container_name, cmd="sleep 60")
    r = docker_exec(container_name, "pgrep", "-f", "robo dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without ROBO_DASHBOARD"
    )
...
# whenever `ROBO_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    when a ``DashboardAuthProvider`` is registered (the bundled basic
    provider activates whenever ``ROBO_DASHBOARD_BASIC_AUTH_USERNAME`` and
    a password are set).

    Regression guard for the wildcard-subdomain rollout where every
    portal-provisioned agent binds ``0.0.0.0`` and relies on the auth
    ...
    2. ``/api/sessions`` (a gated route under both the legacy
       ...
       to an unauthenticated caller — proves the auth gate is actively
    ...
    start_container(
        ...
        "ROBO_DASHBOARD=1",
        "ROBO_DASHBOARD_HOST=0.0.0.0",
        "ROBO_DASHBOARD_BASIC_AUTH_USERNAME=test-instance",
        "ROBO_DASHBOARD_BASIC_AUTH_PASSWORD=test-instance-password",
        cmd="sleep 120",
    )
    ...
    assert "basic" in provider_names, (
        ...
        f"ROBO_DASHBOARD_BASIC_AUTH_USERNAME is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and ROBO_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``ROBO_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate

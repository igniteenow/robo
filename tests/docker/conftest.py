...
Override the image with ``ROBO_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ROBO_TEST_IMAGE", "robo-engineer-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ROBO_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ROBO_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged robo user
# ---------------------------------------------------------------------------
#
# Background: every Robo runtime path inside the container drops to UID
# 10000 (the ``robo`` user) via ``s6-setuidgid robo``. ``docker exec``
...
# fails (PermissionError swallowed) for robo, so a test that ran as root
...
# helpers below run every probe under ``-u robo`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: robo).

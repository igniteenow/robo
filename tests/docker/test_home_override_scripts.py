...
def test_dashboard_service_resets_home(
    ...
    container with ROBO_DASHBOARD=1 and verify the dashboard process
...
def test_dashboard_does_not_auto_insecure_from_host(
    ...
    ROBO_DASHBOARD_HOST. The auth gate is the authority now.
    ...
    """
    start_container(built_image, container_name, "ROBO_DASHBOARD=1", "ROBO_DASHBOARD_HOST=0.0.0.0")
...
def test_stage2_repairs_profiles_and_cron_ownership(
    ...
    The stage2 hook chowns these dirs to robo:robo on every boot.
    ...
    # Verify files are now owned by robo.
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/profiles/testprof/marker '
        '/opt/data/cron/root_owned.json',
    ...
    assert "robo" in r.stdout, (
        f"expected robo-owned files after restart, got: {r.stdout!r} — "

"""Tests for robo_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Robo entry point
(robo, robo-engineer, robo-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Robo entry point imports robo_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported robo_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("robo_bootstrap", None)
    import robo_bootstrap  # noqa: WPS433
    return robo_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Robo CLI
...
class TestEntryPointsImportBootstrap:
    """Every Robo entry point must import robo_bootstrap as its
    ...
    # Entry points that invoke Robo as a process.  Each one must
    # import robo_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "robo_cli/main.py",   # robo CLI (console_script)
        "run_agent.py",          # robo-engineer (console_script)
        "acp_adapter/entry.py",  # robo-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import robo_bootstrap' and that
        ...
        half-finished ``robo update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``robo_bootstrap`` as a top-level module) leaves robo
        ...
        """
        # Resolve relative to the robo-engineer repo root.  Tests live
        # at tests/test_robo_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # robo start even when robo_bootstrap hasn't been
        ...
        assert first_import_name == "robo_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'robo_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import robo_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Robo's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("ROBO_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["ROBO_PYTHON_SRC_ROOT"] = env
            elif "ROBO_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["ROBO_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/robo")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ROBO_PYTHON_SRC_ROOT", None)
            else:
                os.environ["ROBO_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/robo", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Robo root.  The guard must relocate Robo to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/robo"])
        assert result[0] == "/opt/robo"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Robo root.
        assert result.index("/opt/robo") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["ROBO_PYTHON_SRC_ROOT"] = "/env/robo"
            hb.harden_import_path()
            assert sys.path[0] == "/env/robo"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ROBO_PYTHON_SRC_ROOT", None)

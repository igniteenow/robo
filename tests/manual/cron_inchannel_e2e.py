...
Uses a throwaway ROBO_HOME so it never touches ~/.robo. No real names.
"""
...
def _fresh_home():
    """Point ROBO_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_robo_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["ROBO_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER ROBO_HOME is set.
...
# a different get_robo_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"ROBO_HOME (throwaway): {HOME}")

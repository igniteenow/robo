...
- Path resolution (absolute, relative to ROBO_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp ROBO_HOME."""
    robo_home = tmp_path / ".robo"
    robo_home.mkdir()
    (robo_home / "cron").mkdir()
    (robo_home / "cron" / "output").mkdir()
    (robo_home / "scripts").mkdir()
    monkeypatch.setenv("ROBO_HOME", str(robo_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "ROBO_DIR", robo_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", robo_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", robo_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", robo_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Robo provider env (SECURITY.md §2.3)."""
        ...
        blocked_var = sorted(_build_provider_env_blocklist())[0]
...
class TestCronjobToolScript:
    ...
    def test_clear_script(self, cron_env, monkeypatch):
        monkeypatch.setenv("ROBO_INTERACTIVE", "1")
        from tools.cronjob_tools import cronjob
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.robo/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("ROBO_SESSION_PLATFORM") is None
        assert os.environ.get("ROBO_SESSION_CHAT_ID") is None
        assert os.environ.get("ROBO_SESSION_CHAT_NAME") is None

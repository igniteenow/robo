...
def test_cron_runs_cli_prints_execution_history(monkeypatch, tmp_path, capsys):
    ...
    from robo_cli.cron import cron_runs
...
def test_quick_backup_includes_execution_ledger():
    from robo_cli.backup import _QUICK_STATE_FILES
...
def test_restart_marks_interrupted_execution_unknown_without_requeue(tmp_path):
    """Real temp-ROBO_HOME subprocess restart: in-flight is audit-only unknown."""
    home = tmp_path / "home"
...
def test_external_provider_start_recovers_interrupted_records(monkeypatch):
    # The Chronos cron provider is an optional external plugin, not part of
    # this tree — skip rather than fail when it isn't installed.
    chronos = pytest.importorskip(
        "plugins.cron_providers.chronos",
        reason="chronos cron provider plugin not installed",
    )

    provider = chronos.ChronosCronScheduler()

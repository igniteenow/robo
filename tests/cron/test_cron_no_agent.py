...
@pytest.fixture
def robo_env(tmp_path, monkeypatch):
    """Isolate ROBO_HOME for each test so jobs/scripts don't leak."""
    home = tmp_path / ".robo"
    home.mkdir()
    (home / "scripts").mkdir()
    (home / "cron").mkdir()
    ...
    # Reload modules that cache get_robo_home() at import time.
    import importlib
...
def test_create_job_no_agent_requires_script(robo_env):
...
def test_update_job_roundtrips_no_agent_flag(robo_env):
    from cron.jobs import create_job, update_job, get_job

    script_path = robo_env / "scripts" / "w.sh"
...
def test_cronjob_tool_create_no_agent_without_script_errors(robo_env):
...
def test_run_job_no_agent_success_returns_script_stdout(robo_env):
    """Happy path: script exits 0 with output, delivered verbatim."""
    from cron.jobs import create_job
    from cron.scheduler import run_job

    script_path = robo_env / "scripts" / "alert.sh"
...
def test_run_job_script_path_traversal_still_blocked(robo_env):
...
def test_run_job_script_nul_path_fails_cleanly(robo_env):

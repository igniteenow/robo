...
from robo_cli.setup import setup_model_provider
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("robo_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("robo_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "robo_cli.main._model_flow_named_custom",
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    """Modal execution is always direct mode now — the managed-via-Ignitee Now
    billing choice this test used to exercise no longer exists, since Ignitee Now
    has been removed as a provider. Selecting Modal as the terminal backend
    must unconditionally persist modal_mode='direct'."""
    monkeypatch.setenv("ROBO_HOME", str(tmp_path))
    ...
    monkeypatch.setattr("robo_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("robo_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setitem(sys.modules, "swe_rex", object())

    from robo_cli.setup import setup_terminal_backend
...
def test_vercel_setup_prefills_project_and_team_from_link_file(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("robo_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("robo_cli.setup.prompt", fake_prompt)

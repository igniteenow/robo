"""ensure_robo_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".robo"
    second = tmp_path / "b" / ".robo"

    monkeypatch.setenv("ROBO_HOME", str(first))
    cfg.ensure_robo_home()

    # Profile switch: ROBO_HOME moves → the new path is ensured too.
    monkeypatch.setenv("ROBO_HOME", str(second))
    cfg.ensure_robo_home()

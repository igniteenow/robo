"""Tests for robo backup and import commands."""
...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew robo_cli.backup's datetime forward instead of sleeping.
    ...
    import robo_cli.backup as _backup

    shim = getattr(_backup.datetime, "_robo_test_shim", None)
    if shim is None:
        class _ShimDatetime(_dt.datetime):
            _robo_test_shim = True
...
def _make_robo_tree(root: Path) -> None:
    """Create a realistic ~/.robo directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    for db_name in ("memory_store.db", "robo_state.db"):
    ...
    # robo-engineer repo (should be EXCLUDED)
    (root / "robo-engineer").mkdir(exist_ok=True)
    (root / "robo-engineer" / "run_agent.py").write_text("# big file\n")
    (root / "robo-engineer" / ".git").mkdir()
    (root / "robo-engineer" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_robo_agent(self):
        from robo_cli.backup import _should_exclude
        assert _should_exclude(Path("robo-engineer/run_agent.py"))
        assert _should_exclude(Path("robo-engineer/.git/HEAD"))
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        import robo_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = robo_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, robo_home)
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside ROBO_HOME."""
        robo_home = tmp_path / ".robo"
        robo_home.mkdir()
        _make_robo_tree(robo_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(robo_home / "skills" / "outside-link.txt", outside)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Robo backup."""
        from robo_cli.backup import _validate_backup_zip
...
class TestImport:
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        robo_home = tmp_path / ".robo"
        (robo_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("ROBO_HOME", str(robo_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (robo_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (robo_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            robo_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (robo_home / "gateway.pid").write_text("4242")
        (robo_home / "processes.json").write_text('{"live": true}')
        ...
        assert (robo_home / "gateway.pid").read_text() == "4242"
        assert (robo_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (robo_home / "cron.pid").exists()
        assert not (robo_home / "gateway.lock").exists()
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        self._make_backup_zip(zip_path, {
            ...
            "auth.json": '{"providers": {"igniteenow": "token"}}',
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (robo_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".robo"
        src_home.mkdir(parents=True)
        _make_robo_tree(src_home)

        monkeypatch.setenv("ROBO_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from robo_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".robo"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("ROBO_HOME", str(dst_home))
        ...
        # robo-engineer should NOT be present
        assert not (dst_home / "robo-engineer").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from robo_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        import io
        from robo_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            # Only directory entries (trailing slash)
            zf.writestr(".robo/", "")
            zf.writestr(".robo/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
...
class TestBackupEdgeCases:
    ...
    def test_empty_robo_home(self, tmp_path, monkeypatch):
        """Backup handles empty robo home (no files to back up)."""
        robo_home = tmp_path / ".robo"
        robo_home.mkdir()
        # Only excluded dirs, no actual files
        (robo_home / "__pycache__").mkdir()
        (robo_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        robo_home = tmp_path / ".robo"
        robo_home.mkdir()
        (robo_home / "config.yaml").write_text("model: test\n")

        # Create a file with epoch timestamp (1970-01-01)
        old_file = robo_home / "ancient.txt"
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        robo_home = tmp_path / ".robo"
        robo_home.mkdir()
        (robo_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("ROBO_HOME", str(robo_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (robo_home / "config.yaml").exists()
        assert (robo_home / "sessions" / "s0599.json").exists()
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from robo_cli.backup import _safe_copy_db
    ...
    def test_is_zeroed_sqlite_file_detects_nul_header(self, tmp_path):
        from robo_cli.backup import is_zeroed_sqlite_file
        p = tmp_path / "state.db"
...
class TestQuickSnapshot:
    @pytest.fixture
    def robo_home(self, tmp_path):
        """Create a fake ROBO_HOME with critical state files."""
        home = tmp_path / ".robo"
        home.mkdir()
    ...
    def test_state_db_safely_copied(self, robo_home):
        from robo_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(robo_home=robo_home)
        db_copy = robo_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_snapshot_includes_pairing_directories(self, robo_home):
        ...
        (robo_home / "platforms" / "pairing").mkdir(parents=True)
        (robo_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (robo_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (robo_home / "pairing").mkdir()
        (robo_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (robo_home / "feishu_comment_pairing.json").write_text(
        ...
        snap_dir = robo_home / "state-snapshots" / snap_id
...
# ---------------------------------------------------------------------------
# Pre-update backup (robo update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, robo_home, capsys):
        ...
        """
        import json
        from robo_cli.backup import create_quick_snapshot, list_quick_snapshots

        # First snapshot: complete (state.db is small, under any cap)
        first_id = create_quick_snapshot(label="complete", robo_home=robo_home)
        assert first_id is not None
        first_dir = robo_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", robo_home=robo_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = robo_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, robo_home=robo_home)
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_non_default_kanban_board_snapshotted(self, robo_home):
        ...
        from robo_cli.backup import create_quick_snapshot, restore_quick_snapshot

        board_dir = robo_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(robo_home=robo_home)
        copy = (
            robo_home / "state-snapshots" / snap_id
        ...
        assert restore_quick_snapshot(snap_id, robo_home=robo_home) is True
        conn = sqlite3.connect(str(board_dir / "kanban.db"))
    ...
    def test_board_db_copied_wal_safely(self, robo_home, monkeypatch):
        ...
        import robo_cli.backup as bk
        from robo_cli.backup import create_quick_snapshot

        board = robo_home / "kanban" / "boards" / "work"
        ...
        copy = robo_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
        rows = sqlite3.connect(str(copy)).execute("SELECT * FROM tasks").fetchall()
        assert rows == [("w1", "ship")]


class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``robo update``
    ...
    def robo_home(self, tmp_path):
        root = tmp_path / ".robo"
        root.mkdir()
        _make_robo_tree(root)
        return root


    def test_backup_contents_match_full_backup(self, robo_home):
        """Pre-update backup should include the same user data that
        ``robo backup`` would, and should exclude the same directories."""
        from robo_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(robo_home=robo_home)
        ...
        # robo-engineer repo excluded
        assert not any(n.startswith("robo-engineer/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, robo_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(robo_home=robo_home, keep=3)
            created.append(out)
            _advance_backup_clock()

        remaining = sorted(
            p.name for p in (robo_home / "backups").iterdir()
    ...
    def test_skips_symlinked_files(self, robo_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside ROBO_HOME."""
        from robo_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def robo_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".robo"
        root.mkdir()
        _make_robo_tree(root)
        # Point ROBO_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("ROBO_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve ROBO_HOME dynamically and their caches are
    ...
    @staticmethod
    def _set_mode(robo_home, value):
        import yaml
        (robo_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
    ...
    @staticmethod
    def _snaps(robo_home):
        d = robo_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, robo_home, capsys):
        ...
        self._set_mode(robo_home, "off")
        from robo_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is None
        assert out == ""
        assert not self._snaps(robo_home)
        assert not self._zips(robo_home)



    def test_config_full_mode(self, robo_home, capsys):
        self._set_mode(robo_home, "full")
        from robo_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
        ...
        assert len(self._zips(robo_home)) == 1




# ---------------------------------------------------------------------------
# Pre-migration backup (import/migration safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    migration-style commands run before mutating ~/.robo/."""
    ...
    def test_restorable_with_robo_import(self, robo_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Robo
        backup — `robo import` should accept it."""
        from robo_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(robo_home=robo_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_touch_pre_update_backups(self, robo_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from robo_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(robo_home=robo_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        for _ in range(3):
            out = create_pre_migration_backup(robo_home=robo_home, keep=1)
            assert out is not None
            _advance_backup_clock()
...
class TestRestoreCronJobsIfEmptied:
    """`robo update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, robo_home: Path, label="pre-update"):
        from robo_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, robo_home=robo_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from robo_cli.backup import restore_cron_jobs_if_emptied
        robo_home = tmp_path / ".robo"
        jobs_path = robo_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(robo_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, robo_home=robo_home)
        assert result is not None
...
# location, NOT under ROBO_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, robo_home: Path) -> None:
        robo_home.mkdir(parents=True, exist_ok=True)
        (robo_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (robo_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (robo_home / "state.db").write_bytes(b"x")
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under ROBO_HOME,
        and credential-shaped files get 0600."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        robo_home = dst_home / ".robo"
        robo_home.mkdir()
        ...
        # External state did NOT leak into ROBO_HOME.
        assert not (robo_home / "_external").exists()

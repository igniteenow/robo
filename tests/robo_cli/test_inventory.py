"""Behavior tests for robo_cli.inventory.
...
def _igniteenow_row(model: str = "openai/gpt-5.5") -> dict:
    return {
        "slug": "igniteenow",
        "name": "Ignitee Now",
        "models": [model],
...
def test_explicit_only_filters_ambient_credentials_but_keeps_current_and_custom_rows():
    rows = [
        ...
        {"slug": "copilot", "name": "Copilot", "models": ["gpt-5.4"],
         "total_models": 1, "is_current": False, "is_user_defined": False,
         "source": "robo"},
        {"slug": "igniteenow", "name": "Ignitee Now", "models": ["anthropic/claude-sonnet-5"],
         "total_models": 1, "is_current": False, "is_user_defined": False,
         "source": "robo"},
...
def test_build_models_payload_forwards_refresh_flag():
    ...
    with patch("robo_cli.model_switch.list_authenticated_providers", side_effect=_capture):
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    from robo_cli import model_switch

    with patch("robo_cli.models.clear_provider_models_cache") as clear:

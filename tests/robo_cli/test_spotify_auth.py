...
from robo_cli.auth import AuthError, resolve_spotify_runtime_credentials
...
def test_resolve_spotify_runtime_credentials_refreshes_without_changing_active_provider(
    ...
    with auth_mod._auth_store_lock():
        ...
        store["active_provider"] = "igniteenow"
    ...
    assert auth_mod.get_active_provider() == "igniteenow"
...
def test_auth_spotify_status_command_reports_logged_in(capsys, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from robo_cli.auth_commands import auth_status_command
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    Mirrors Ignitee Now / xAI-OAuth / Codex-OAuth / MiniMax quarantine pattern.
    """
    monkeypatch.setenv("ROBO_HOME", str(tmp_path))

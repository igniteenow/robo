...
from robo_cli.main, these tests mock the delegation point and verify
that the setup wizard correctly syncs config from disk after the call.
"""

from __future__ import annotations

from robo_cli.config import load_config, save_config, save_env_value
from robo_cli.igniteenow_subscription import IgniteeNowFeatureState, IgniteeNowSubscriptionFeatures
from robo_cli.setup import _print_setup_summary, setup_model_provider
...
def _stub_tts(monkeypatch):
    monkeypatch.setattr("robo_cli.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("robo_cli.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_model_provider_preserves_auxiliary_choices_written_by_picker(tmp_path, monkeypatch):
    """Aux choices made inside robo setup must survive the wizard's final save."""
    ...
    monkeypatch.setattr("robo_cli.main.select_provider_and_model", fake_select)
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("robo_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("robo_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("robo_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("robo_cli.auth.get_active_provider", lambda: None)
...
def test_setup_summary_local_browser_unavailable_without_chromium(
    ...
    Drives the real check_browser_requirements-aligned logic in
    _print_setup_summary directly, stubbing only the two low-level
    readiness probes (agent-browser CLI presence, Chromium presence) so the
    surface stays aligned with the runtime gate in
    ...
    monkeypatch.setattr("shutil.which", lambda name: "/usr/bin/agent-browser" if name == "agent-browser" else None)

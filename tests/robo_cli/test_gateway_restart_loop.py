...
Covers:
- Defense 1: gateway stop/restart refuse when _ROBO_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "robo gateway stop",
        "robo  gateway  restart",         # double spaces
        "Robp Gateway Restart".lower().replace("p", "o"),  # case handled
        "ROBO GATEWAY RESTART",           # uppercase
    ])
    def test_robo_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.robo.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.robo/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l robo-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.robo.gateway.restart-once.plist",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.robo.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.robo/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated robo note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        "robo cron list",
        "robo update",
        "robo config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `robo gateway start` is benign — starting a gateway from inside a
        ...
        "robo gateway start",
        "robo gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway robo
        # services must not be falsely blocked (the old `.*robo` matched any
        # robo token).
        "launchctl unload ai.robo.update-checker.plist",
        "launchctl restart ai.robo.daemon",
        ...
        "systemctl restart robo-meta.service",
        "systemctl restart robo-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_robo_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade robo then run robo gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_robo_gateway_once.sh). The script must live under
        # ROBO_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("ROBO_HOME", str(tmp_path / ".robo"))
        scripts_dir = tmp_path / ".robo" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify robo gateway stop/restart refuse when _ROBO_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_ROBO_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_ROBO_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _ROBO_GATEWAY=1.

    Issue #37453: systemctl --user restart robo-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart robo-gateway",
        "systemctl --user restart robo-gateway",
        "systemctl stop robo-gateway.service",
        "robo gateway restart",
        "launchctl kickstart gui/501/ai.robo.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.robo.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.robo/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.robo.gateway.restart-once.plist",
        "pkill -f robo.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart robo-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.robo.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-robo label: label-independent detection is the point
        # (#62891 second reproduction used `ai.robo.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.robo.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.robo.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.robo.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-robo systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run robo gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("robo gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xferobo gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under ROBO_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.robo.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("robo gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/robo\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 robo gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_robo_home()` falls back to Path.home(), which raises
        RuntimeError when neither ROBO_HOME nor HOME resolves
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    robo_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run robo gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ROBO_HOME", str(tmp_path / ".robo"))
        (tmp_path / ".robo").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run robo gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop robo-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart robo-gateway' /var/log/syslog",
        "rg 'robo gateway restart' /home/user/.robo/logs/",
        "journalctl -u robo-gateway --grep 'systemctl restart robo-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'robo_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell robo gateway restart"',
        'psql -c "\\! systemctl restart robo-gateway"',
        "grep 'systemctl restart robo-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart robo-gateway",
        'grep "$(systemctl restart robo-gateway)" f',
        "grep 'restart' log; systemctl restart robo-gateway",
        'sqlite3 db "SELECT 1"; robo gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "robo gateway restart",
        "sudo systemctl stop robo-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart robo-gateway%'\""
        )

"""Tests for robo_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_robo_home):
    """Drop the example-dashboard fixture into the per-test ROBO_HOME
    ...
    ``ROBO_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, robo-achievements, model providers
    ...
    user_plugins_dir = get_robo_home() / "plugins"
    ...
    # plugin in exactly as a real operator would with `robo plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down ROBO_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Robo vars not present in .env."""
        env_file = tmp_path / ".env"
...
class TestSessionTokenInjection:
    """The desktop shell mints ROBO_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        ...
        monkeypatch.setenv("ROBO_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
    ...
    def test_falls_back_to_random_token(self, monkeypatch):
        ...
        monkeypatch.delenv("ROBO_DASHBOARD_SESSION_TOKEN", raising=False)
        with patch.object(
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_robo_home):
        """Create a TestClient and isolate the state DB under the test ROBO_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
        ...
        monkeypatch.setattr(robo_state, "DEFAULT_DB_PATH", get_robo_home() / "state.db")
    ...
    def test_profiles_sidebar_heals_stale_schema_store(self):
        ...
        array — the desktop rendered "No sessions yet" after `robo update`
    ...
    def test_heal_gives_up_when_reconcile_cannot_fix_the_store(self, monkeypatch):
        ...
        original_init = robo_state.SessionDB.__init__
        ...
        # the class on robo_state covers every open the helper makes.
        monkeypatch.setattr(robo_state.SessionDB, "__init__", counting_init)
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        ROBO_HOME contextvar override (#56986), so /api/messaging/platforms
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        from robo_constants import get_robo_home
        from robo_cli.config import load_env
        ...
        config_path = get_robo_home() / "hindsight" / "config.json"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from robo_constants import get_robo_home
        from robo_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide ROBO_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from robo_constants import get_robo_home

        path = get_robo_home() / "honcho.json"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_robo_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["robo"]["workspace"] == "myws"
        assert cfg["hosts"]["robo"]["peerName"] == "eri"
        assert cfg["hosts"]["robo"]["environment"] == "local"
        assert cfg["hosts"]["robo"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["robo"]["apiKey"] == "hch-test-key"
    ...
    def test_get_media_requires_auth(self):
        from robo_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_update_robo_returns_docker_guidance_without_spawning(self, monkeypatch):
        import robo_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn robo update")
        ...
        monkeypatch.setattr(web_server, "_spawn_robo_action", fail_spawn)
        web_server._ACTION_PROCS.pop("robo-update", None)
        web_server._ACTION_RESULTS.pop("robo-update", None)

        resp = self.client.post("/api/robo/update")
    ...
    def test_update_robo_spawns_with_action_id(self, monkeypatch):
        ...
        monkeypatch.setattr(web_server, "_spawn_robo_action", fake_spawn)
        web_server._ACTION_PROCS.pop("robo-update", None)
        web_server._ACTION_RESULTS.pop("robo-update", None)
        ...
        assert calls == [
            (["update"], "robo-update", {"ROBO_ACTION_ID": "a" * 32})
        ]

    def test_update_robo_reuses_running_action(self, monkeypatch):
        import robo_cli.web_server as web_server
        ...
        web_server._ACTION_PROCS["robo-update"] = Proc()
        web_server._ACTION_IDS["robo-update"] = "b" * 32

        try:
            resp = self.client.post("/api/robo/update")
        finally:
            web_server._ACTION_PROCS.pop("robo-update", None)
            web_server._ACTION_IDS.pop("robo-update", None)
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/robo_cli/test_setup_hidden_env.py.
        """
        from robo_cli.web_server import (
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Robo provider — keep the user's aggregator instead of writing a
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from robo_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from robo_cli.config import save_env_value
        from robo_cli.web_server import _SESSION_TOKEN
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "suggested_username": "robo_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/RoboSetupBot/robo_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/RoboSetupBot/robo_pair_restart_fails_bot",
            ...
            return {
                "status": "ready",
                "bot_username": "robo_pair_restart_fails_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_robo_action", fail_spawn_action)
    ...
    def test_unauthenticated_api_blocked(self):
        ...
        from starlette.testclient import TestClient
        from robo_cli.web_server import app
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from robo_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``robo model`` custom flow. Regression for the desktop loop where a
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from robo_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from robo_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from robo_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_overrides_applied(self):
        from robo_cli.web_server import CONFIG_SCHEMA
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from robo_cli.web_server import CONFIG_SCHEMA, _timezone_options
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from robo_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_robo_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_create_builder_mcp_auth_is_profile_scoped(
        ...
        from robo_constants import get_robo_home
        import robo_cli.profiles as profiles_mod
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        assert "robo@devbox.example.com" in ssh["detail"]
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from robo_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from robo_cli.web_server import _denormalize_config_from_web
        from robo_cli.config import save_config
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_non_dict(self):
        from robo_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.robo/dashboard-themes/."""
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_robo_home_override(str(other))
        try:
            results = web_server._discover_user_themes()
        finally:
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(robo_home, name="ocean"):
        themes_dir = robo_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="robo-theme-bootstrap">')
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_robo_home):
        ...
        monkeypatch.setattr(
            robo_state, "DEFAULT_DB_PATH", get_robo_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``robo_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in robo_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_robo_home):
        ...
        # Pin the SessionDB to the isolated ROBO_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_robo_home, _install_example_plugin):
        ...
        installs it into the per-test ``ROBO_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into ROBO_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, robo-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (robo-achievements is loaded by default).
        resp = self.client.get("/api/plugins/robo-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        ROBO_HOME override scopes a request to another profile."""
        from robo_constants import (
            reset_robo_home_override,
            set_robo_home_override,
        )
        ...
        monkeypatch.setenv("ROBO_HOME", str(launch_home))
        from robo_cli import web_server
        token = set_robo_home_override(str(other))
        try:
...
# or ``sh -c 'printf …'``) instead of the real ``robo --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        command = f"robo-review-python{Path(sys.executable).suffix}"
        ...
        assert env["ROBO_PYTHON"] == command
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from robo_cli.pty_bridge import PtyUnavailableError
        ...
        import robo_cli.web_server as ws_mod
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from robo_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("ROBO_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_robo_home, _install_example_plugin):
        ...
        the per-test ``ROBO_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_robo_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.setenv("ROBO_DESKTOP", "1")

        with self._client():
            assert called.wait(3.0), "expected cron tick under ROBO_DESKTOP=1"
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("ROBO_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        ws.mount_spa(spa_app)
...
class TestHashedAssetCacheHeaders:
    ...
    def test_serve_css_is_immutable_and_keeps_prefix_rewrites(
        ...
        prefixed = client.get(
            ...
            headers={"X-Forwarded-Prefix": "/robo"},
        )
        assert prefixed.status_code == 200
        assert prefixed.headers["cache-control"] == self._IMMUTABLE
        assert "url(/robo/ds-assets/bg.png)" in prefixed.text
        assert "url(/robo/fonts-terminal/x.woff2)" in prefixed.text

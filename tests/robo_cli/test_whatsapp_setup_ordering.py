...
Before the fix, ``robo whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``robo gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ROBO_HOME", str(robo))
...
def _env_value(robo_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from robo_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("robo_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``robo whatsapp`` with an existing paired session and

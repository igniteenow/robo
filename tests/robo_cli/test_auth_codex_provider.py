"""Tests for Codex auth — tokens stored in Robo auth store (~/.robo/auth.json)."""
...
def _setup_robo_auth(robo_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Robo auth store."""
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    ...
    _setup_robo_auth(robo_home, access_token="")
...
def test_resolve_codex_runtime_credentials_falls_back_to_pool_when_singleton_empty(tmp_path, monkeypatch):
    ...
    (robo_home / "auth.json").write_text(json.dumps(auth_store))
    monkeypatch.setenv("ROBO_HOME", str(robo_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((robo_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``robo auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``robo auth add openai-codex``.
    ...
    (robo_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `robo auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``robo auth add openai-codex`` for accounts B and C
    ...
    (robo_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``robo auth add openai-codex`` (account B and account C).
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Robo auth store, not ~/.codex/."""
    robo_home = tmp_path / "robo"
    ...
    _save_codex_tokens({"access_token": "robo-at", "refresh_token": "robo-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Robo store
    assert not (codex_home / "auth.json").exists()

    # Robo auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "robo-at"


def test_resolve_returns_robo_auth_store_source(tmp_path, monkeypatch):
    robo_home = tmp_path / "robo"
    _setup_robo_auth(robo_home)
    monkeypatch.setenv("ROBO_HOME", str(robo_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "robo-auth-store"
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run robo auth".
    """
    from robo_cli.auth import (
    ...
    assert "robo auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from robo_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from robo_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch robo_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("robo_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())

"""Tests for robo_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $ROBO_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Robo-managed node dir.
"""
...
def _make_robo_node(robo_home: Path) -> Path:
    """Create a fake $ROBO_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = robo_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    robo_home = fake_home / ".robo"
    _make_robo_node(robo_home)
    ...
    removed = uninstall.remove_node_symlinks(robo_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    robo_home = fake_home / ".robo"
    node_bin = _make_robo_node(robo_home)

"""Tests for ``robo debug`` CLI command and debug utilities."""
...
def robo_home(tmp_path, monkeypatch):
    """Set up an isolated ROBO_HOME with minimal logs."""
    home = tmp_path / ".robo"
    home.mkdir()
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO robo_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from robo_cli.debug import _upload_paste_rs
        ...
        with patch("robo_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "robo_cli.debug.urllib.request.urlopen",
...
class TestUploadToPastebin:
    ...
    def test_falls_back_to_dpaste_com(self):
        from robo_cli.debug import upload_to_pastebin

        with patch("robo_cli.debug._upload_paste_rs",
                    side_effect=Exception("down")), \
             patch("robo_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_race_truncate_after_resolve_reports_empty(self, robo_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = robo_home / "logs" / "agent.log"
        from robo_cli import debug
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, robo_home):
        """When truncation lands on a line boundary, keep the first full line."""
        from robo_cli.debug import _capture_log_snapshot
        ...
        (robo_home / "logs" / "agent.log").write_text(line * num_lines)
...
class TestMissingLogNote:
    ...
    `robo debug share` runs on the backend, so a desktop connected to a
    ...
    def test_backend_written_log_reports_plain_absence(self, robo_home):
        from robo_cli.debug import _capture_log_snapshot

        (robo_home / "logs" / "agent.log").unlink()

        snap = _capture_log_snapshot("agent", tail_lines=10)
        assert snap.full_text is None
        assert snap.tail_text == "(file not found)"

    def test_client_written_log_names_its_writer_and_path(self, robo_home):
        from robo_cli.debug import _capture_log_snapshot

        (robo_home / "logs" / "desktop.log").unlink()

        snap = _capture_log_snapshot("desktop", tail_lines=10)
        assert snap.full_text is None
        assert "not on this host" in snap.tail_text
        assert "Robo Desktop" in snap.tail_text
        # The reader needs the path to collect by hand on the client machine.
        assert str(robo_home / "logs" / "desktop.log") in snap.tail_text

    def test_present_client_log_is_captured_normally(self, robo_home):
    ...
    def test_empty_client_log_is_empty_not_absent(self, robo_home):
        """An empty file means the app ran and logged nothing — a different fact."""
        from robo_cli.debug import _capture_log_snapshot

        (robo_home / "logs" / "desktop.log").write_text("")

        snap = _capture_log_snapshot("desktop", tail_lines=10)
        assert snap.tail_text == "(file empty)"

    def test_report_carries_the_note_for_a_remote_backend(self, robo_home):
        """The uploaded report — what people paste into support — must explain it."""
        from robo_cli.debug import collect_debug_report
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of ROBO_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def robo_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated ROBO_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".robo"
        home.mkdir()
        monkeypatch.setenv("ROBO_HOME", str(home))
        ...
        monkeypatch.delenv("ROBO_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, robo_home_with_secret):
        from robo_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, robo_home_with_secret):
        from robo_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, robo_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when ROBO_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("ROBO_REDACT_SECRETS", "false")

        from robo_cli.debug import _capture_log_snapshot

        assert os.environ.get("ROBO_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, robo_home_with_secret
    ):
        from robo_cli.debug import _capture_log_snapshot

        log_path = robo_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, robo_home_with_secret):
        from robo_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from robo_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, robo_home):
        from robo_cli.debug import collect_debug_report

        with patch("robo_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- robo dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- robo dump ---" in report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, robo_home, capsys):
        """Slash-command path should sweep old pending deletes before uploading."""
        from robo_cli.debug import run_debug_share

        args = MagicMock()
        args.lines = 50
        args.expire = 7
        args.local = False
        args.igniteenow = False

        with patch("robo_cli.dump.run_dump"), \
             patch("robo_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("robo_cli.debug.upload_to_pastebin",
    ...
    def test_share_uploads_five_pastes(self, robo_home, capsys):
        ...
        with patch("robo_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- robo dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, robo_home_with_secret, capsys
    ):
        ...
        with patch("robo_cli.dump.run_dump"), \
             patch("robo_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("robo_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "robo debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, robo_home):
        from robo_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from robo_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from robo_cli.debug import delete_paste
        ...
        with patch("robo_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.robo/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``robo debug``
    invocation.
    """


    def test_records_pending_to_json(self, robo_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from robo_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_dedupes_same_url(self, robo_home):
        """Same URL recorded twice → one entry with the later expire_at."""
        from robo_cli.debug import _schedule_auto_delete, _load_pending
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_deletes_expired_entries(self, robo_home):
        from robo_cli.debug import (
        ...
        with patch("robo_cli.debug.delete_paste", side_effect=fake_delete):
    ...
    def test_sweep_leaves_future_entries_alone(self, robo_home):
        from robo_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("robo_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, robo_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, robo_home):
        from robo_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("robo_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)
...
class TestRunDebugDelete:
    ...
    def test_handles_delete_failure(self, capsys):
        from robo_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("robo_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_shows_privacy_notice(self, robo_home, capsys):
        from robo_cli.debug import run_debug_share
        ...
        with patch("robo_cli.dump.run_dump"), \
             patch("robo_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("robo_cli.debug._schedule_auto_delete"):
            run_debug_share(args)
...
class TestBuildDebugShare:
    ...
    Backs both ``robo debug share`` (CLI) and ``POST /api/ops/debug-share``
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, robo_home):
        from robo_cli.debug import build_debug_share

        secret = "sk-proj-SUPERSECRETtoken1234567890"
        (robo_home / "logs" / "agent.log").write_text(
        ...
        with patch("robo_cli.dump.run_dump"), patch(
            "robo_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("robo_cli.debug._schedule_auto_delete"):
    ...
    def test_optional_log_failure_is_collected_not_raised(self, robo_home):
        from robo_cli.debug import build_debug_share
...
# ---------------------------------------------------------------------------
# Shared bundle collection + Ignitee Now-S3 path
# ---------------------------------------------------------------------------

class TestCollectShareBundle:

    def test_no_redact_omits_banner(self, robo_home):
        from robo_cli.debug import collect_share_bundle
    ...
    def test_redaction_keeps_secrets_out(self, robo_home):
        from robo_cli.debug import collect_share_bundle
...
class TestBuildIgniteeNowBundle:
    def test_envelope_shape_and_gzip(self, robo_home):
        import gzip
        import json as _json

        from robo_cli.debug import build_igniteenow_bundle

        files = {"report": "hello", "agent.log": "log line"}
        blob = build_igniteenow_bundle(files, redact=True)

        # It's gzip — magic bytes.
        assert blob[:2] == b"\x1f\x8b"
        envelope = _json.loads(gzip.decompress(blob).decode())
        assert envelope["format"] == "robo-debug-share/1"
    ...
    def test_redacted_false_recorded(self):
        ...
        blob = build_igniteenow_bundle({"report": "x"}, redact=False)
        envelope = _json.loads(gzip.decompress(blob).decode())
        assert envelope["redacted"] is False


class TestRunDebugShareIgniteeNow:
    def _args(self, **over):
        class _A:
            lines = 50
            expire = 7
    ...
    def test_igniteenow_success_prints_view_url(self, robo_home, capsys):
        from robo_cli.debug import run_debug_share
        ...
        with patch("robo_cli.dump.run_dump"), patch(
            "robo_cli.diagnostics_upload.share_to_igniteenow", return_value=res
        ) as share:
    ...
    def test_igniteenow_failure_suggests_local(self, robo_home, capsys):
        from robo_cli.debug import run_debug_share

        with patch("robo_cli.dump.run_dump"), patch(
            "robo_cli.diagnostics_upload.share_to_igniteenow",
        ...
        assert "Ignitee Now upload failed" in err
        assert "--local" in err

    def test_igniteenow_does_not_touch_pastebin(self, robo_home):
        from robo_cli.debug import run_debug_share

        res = {"id": "id-1", "viewUrl": "https://v"}
        with patch("robo_cli.dump.run_dump"), patch(
            "robo_cli.diagnostics_upload.share_to_igniteenow", return_value=res
        ), patch("robo_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
        paste.assert_not_called()


class TestDebugSlashCommand:
    """`/debug [igniteenow|local]` parsing in the CLI/TUI handler.

    The classic CLI and the TUI slash worker both dispatch through
    ``RoboCLI.process_command`` → ``_handle_debug_command(cmd_original)``,
    ...
    def _captured(self, cmd_original):
        ...
        with patch("robo_cli.debug.run_debug_share", _fake_run):
            self._handler()(cmd_original)
        return captured

    def test_bare_debug_defaults_to_paste(self):
        c = self._captured("/debug")
        assert c["igniteenow"] is False and c["local"] is False
    ...
    def test_word_parsing_is_case_insensitive(self):
        c = self._captured("/debug IGNITEENOW")
        assert c["igniteenow"] is True
...
class TestShareConsentGate:
    """`robo debug share` requires explicit consent before uploading.
    ...
    def _args(self, **over):
        ...
        base = dict(lines=50, expire=7, local=False, igniteenow=False,
    ...
    def test_non_interactive_requires_yes(self, robo_home, capsys, monkeypatch):
        ...
        with patch("robo_cli.dump.run_dump"), \
             patch("robo_cli.debug.upload_to_pastebin") as mock_upload:
            with pytest.raises(SystemExit) as exc:
                run_debug_share(self._args())
    ...
    def test_local_never_prompts(self, robo_home, capsys, monkeypatch):

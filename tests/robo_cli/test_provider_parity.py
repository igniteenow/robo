...
the SAME provider universe as ``robo model`` (the CLI/TUI picker).
...
def test_every_robo_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `robo model` universe."""
    ...
    assert not missing, (
        "providers shown in `robo model` but not configurable in the desktop "

"""Tests for the Robo plugin system (robo_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<robo_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<robo_home>/plugins/``; we derive
    ``<robo_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <ROBO_HOME>/config.yaml.
        # Config is always read from ROBO_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        robo_home_str = os.environ.get("ROBO_HOME")
        if robo_home_str:
            robo_home = Path(robo_home_str)
        else:
            robo_home = base.parent
        robo_home.mkdir(parents=True, exist_ok=True)
        cfg_path = robo_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_plugin_can_register_and_invoke_middleware(self, tmp_path, monkeypatch):
        plugins_dir = tmp_path / "robo_test" / "plugins"
        _make_plugin_dir(
            plugins_dir,
            "mw_plugin",
            register_body=(
        ...
        monkeypatch.setenv("ROBO_HOME", str(tmp_path / "robo_test"))
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under robo_plugins.<name>."""
        plugins_dir = tmp_path / "robo_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("ROBO_HOME", str(tmp_path / "robo_test"))

        # Clean up any prior namespace module
        sys.modules.pop("robo_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "robo_plugins.ns_plugin" in sys.modules
...
class TestPreToolCallDirective:
    ...
    def test_first_party_observer_receives_pre_tool_call(self, monkeypatch):
        from robo_cli import observability
        from robo_cli.plugins import get_pre_tool_call_directive
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("robo_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("robo_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        from robo_cli.plugins import PluginToolOverrideError
        ...
        try:
            ...
            from robo_cli.plugins import PluginContext, PluginManifest
    ...
    def test_register_tool_override_blocked_via_delayed_callback(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            import sys as _sys
            mod = _sys.modules.get("robo_plugins.delayed_override_plugin")
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        """
        import robo_cli.plugins as plugins_mod
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `robo plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_register_command_empty_name_rejected(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="robo_cli.plugins"):
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("robo_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("robo_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("robo_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("robo_cli.plugins.PluginContext.dispatch_tool.__module__", "robo_cli.plugins"):
...
class TestPluginDebugLogging:
    """ROBO_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("ROBO_PLUGINS_DEBUG", raising=False)
        from robo_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from ROBO_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """ROBO_HOME at the root resolves to 'default'."""
        home = tmp_path / ".robo"
        home.mkdir()
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """ROBO_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".robo" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ROBO_HOME", str(prof))

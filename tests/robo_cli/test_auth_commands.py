...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from robo_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "robo" / "auth.json").read_text())
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``robo auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``robo auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("robo_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from robo_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from robo_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """robo auth add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``robo auth add xai-oauth`` runs must produce independent pool entries.
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`robo logout --provider openai-codex` must still clear model.provider.
    ...
    config_text = (robo_home / "config.yaml").read_text()
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from robo_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
# =============================================================================
# Unified credential-source stickiness — every source Robo reads from has a
...
def test_seed_from_singletons_respects_robo_pkce_suppression(tmp_path, monkeypatch):
    """anthropic robo_pkce must not re-seed from ~/.robo/.anthropic_oauth.json when suppressed."""
    robo_home = tmp_path / "robo"
    robo_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("ROBO_HOME", str(robo_home))

    import yaml
    (robo_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (robo_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["robo_pkce"]},
    }))

    # Stub the readers so only robo_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_robo_oauth_credentials", lambda: {
    ...
    # robo_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "robo_pkce" not in active
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from robo_cli.auth import is_source_suppressed
    from robo_cli.auth_commands import auth_remove_command

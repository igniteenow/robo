...
from robo_cli import relaunch as relaunch_mod


class TestResolveRoboBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/robo"
        ...
        assert relaunch_mod.resolve_robo_bin() == fake
...
class TestBuildRelaunchArgv:
    def test_uses_bin_when_available(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_robo_bin", lambda: "/usr/bin/robo")
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv[0] == "/usr/bin/robo"
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/robo", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/robo", ["/usr/bin/robo", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        robo).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead."""
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_robo_bin", lambda: r"C:\Users\test\robo.exe")
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\robo.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_robo_bin", lambda: r"C:\robo.exe")
...
class TestResolveRoboBinWindowsPyGuard:
    """On Windows, resolve_robo_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a robo.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\robo.exe" if name == "robo" else None,
        )

        bin_path = relaunch_mod.resolve_robo_bin()
        # Must NOT be the .py — must be the robo.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\robo.exe"
    ...
    def test_posix_still_accepts_py_argv0(self, monkeypatch, tmp_path):
        ...
        script = tmp_path / "robo"
        script.write_text("#!/usr/bin/env python3\n")
        script.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(script), "chat"])
        assert relaunch_mod.resolve_robo_bin() == str(script)

    def test_windows_py_argv0_with_no_robo_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND robo.exe
        isn't on PATH, return None so the caller falls back to
        python -m robo_cli.main."""
        script = tmp_path / "main.py"
        script.write_text("# stub")

"""Tests for the unified provider catalog (robo_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``robo model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from robo_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,

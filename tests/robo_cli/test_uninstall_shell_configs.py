...
This rewrites files Robo does not own (``~/.bashrc``, ``~/.zshrc``, ...) and
...
from robo_cli import uninstall
...
def fake_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Point both ``Path.home()`` and ``ROBO_HOME`` at a throwaway dir."""
    home = tmp_path / "home"
    home.mkdir()
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))
    monkeypatch.setenv("ROBO_HOME", str(home / ".robo"))
    return home


class TestHappyPath:
    def test_robo_path_block_is_removed(self, fake_home: Path):
        rc = fake_home / ".zshrc"
...
class TestCrashDurability:
    def test_shell_config_survives_an_interrupted_rewrite(self, fake_home: Path):
        ...
        # Path.home()/ROBO_HOME patches the fake_home fixture installed.
    ...
    def test_symlinked_shell_config_stays_a_symlink(self, fake_home: Path):
        ...
        assert "# Robo Agent" not in real.read_text(encoding="utf-8")

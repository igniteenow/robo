...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    project_root = tmp_path / "robo-engineer"
    ...
    monkeypatch.setattr(uninstall, "get_robo_home", lambda: robo_home)
    monkeypatch.setattr(uninstall, "_is_default_robo_home", lambda home: False)
    ...
    assert str(robo_home) in output
    assert project_root.exists()
    assert robo_home.exists()

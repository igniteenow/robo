...
class TestConfigWriting:
    ...
    def test_plugin_provider_active_overrides_managed_igniteenow_active_label(self, monkeypatch):
        from robo_cli import tools_config
        ...
        monkeypatch.setattr(
            ...
            lambda config, **kwargs: SimpleNamespace(
                features={"image_gen": SimpleNamespace(managed_by_igniteenow=True)}
            ),
        )

        config = {"image_gen": {"provider": "openai", "use_gateway": False}}
        igniteenow_row = {
            "name": "Ignitee Now Subscription",
            "managed_igniteenow_feature": "image_gen",
        }
        openai_row = {
            "name": "OpenAI",
        ...
        assert tools_config._is_provider_active(igniteenow_row, config) is False

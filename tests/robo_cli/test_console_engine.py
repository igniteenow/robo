...
from robo_cli.console_engine import RoboConsoleEngine, run_console_repl
...
MUTATING_CONFIRMATION_SMOKE_COMMANDS = [
    ...
    "mcp picker",
    "backup --quick -o /tmp/robo-console-test.zip",
    "import /tmp/robo-console-test.zip",
...
def test_sessions_list_and_stats_use_isolated_session_store(_isolate_robo_home):
...
def test_cron_pause_resume_and_run_require_confirmation(_isolate_robo_home):
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_robo_home):
    ...
    assert code == 0
    assert "Robo Console" in stdout.getvalue()
    assert "robo>" not in stdout.getvalue()
    assert stderr.getvalue() == ""


def test_capture_output_surfaces_string_exit_code_as_command_error():
    from robo_cli.console_engine import ConsoleCommandError, _capture_output
...
def test_execute_handler_string_exit_returns_error_not_crash(_isolate_robo_home):
    result = RoboConsoleEngine().execute(

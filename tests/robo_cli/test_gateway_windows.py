"""Tests for robo_cli.gateway_windows."""

from pathlib import Path

import pytest

import robo_cli.gateway as gateway
import robo_cli.gateway_windows as gateway_windows
import robo_cli.setup as setup
...
def test_build_gateway_argv_keeps_venv_console_python_for_uv_venv(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda robo_home: "")
    monkeypatch.setattr("robo_cli.config.get_robo_home", lambda: str(robo_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(venv_python), "-m", "robo_cli.main"]
    assert cwd == str(robo_home.resolve())
    assert env_overlay["VIRTUAL_ENV"] == str(project / "venv")
    assert str(project) in env_overlay["PYTHONPATH"].split(gateway_windows.os.pathsep)


class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_robo_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".robo"
        home.mkdir()
        monkeypatch.setattr("robo_cli.config.get_robo_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        missing = tmp_path / "missing" / ".robo"
        project = tmp_path / "checkout"
        monkeypatch.setattr("robo_cli.config.get_robo_home", lambda: missing)
...
def _arrange_startup_fallback(monkeypatch, tmp_path, running_pids):
    script_path = tmp_path / "Robo_Gateway_alice.cmd"
    startup_entry = tmp_path / "Startup" / "Robo_Gateway_alice.cmd"
    calls = []

    monkeypatch.setattr(gateway_windows, "_prompt_install_choices", lambda *args, **kwargs: (False, True))
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Robo_Gateway_alice")
...
def test_elevated_gateway_command_uses_hidden_console_python(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Robo\venv\Scripts\python.exe")
    ...
    assert verb == "runas"
    assert executable == r"C:\Robo\venv\Scripts\python.exe"
...
def test_install_scheduled_task_recreates_instead_of_change(monkeypatch, tmp_path):
    ...
    ok, detail = gateway_windows._install_scheduled_task("Robo_Gateway_alice", script_path)

    assert ok is True
    assert "/Change" not in [arg for call in calls for arg in call]
    assert calls[0][:4] == ("/Delete", "/F", "/TN", "Robo_Gateway_alice")
    ...
    assert "Robo_Gateway_alice.vbs" in xml_seen["text"]
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    assert "robo_cli.main" in content
    assert "gateway run" in content
    assert ", 0, False" in content  # hidden window, detached/async
    for var in ("ROBO_HOME", "PYTHONIOENCODING", "ROBO_GATEWAY_DETACHED", "VIRTUAL_ENV", "PYTHONPATH"):
...
# resume_pending=True) never fires when `robo gateway stop` kills the

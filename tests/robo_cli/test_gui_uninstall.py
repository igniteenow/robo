"""Tests for robo_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import robo_cli.gui_uninstall as gu


def _make_agent(robo_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = robo_home / "robo-engineer"
    (agent_root / "robo_cli").mkdir(parents=True)
    (agent_root / "robo_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(robo_home: Path) -> None:
    """Create the source-built GUI artifacts a `robo desktop` run produces."""
    desktop = robo_home / "robo-engineer" / "apps" / "desktop"
    ...
    (robo_home / "robo-engineer" / "node_modules").mkdir(parents=True)
    (robo_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(robo_home: Path) -> None:
    (robo_home / "config.yaml").write_text("x: 1\n")
    (robo_home / ".env").write_text("KEY=secret\n")
    (robo_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(robo_home)
    _make_gui_build(robo_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(robo_home)
    ...
    assert summary["robo_home"] == str(robo_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `robo desktop` installs is removable."""
    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.setenv("XDG_DATA_HOME", str(tmp_path / "xdg"))

    from robo_cli import linux_desktop_entry as lde
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    robo_home = tmp_path / ".robo"
    _make_agent(robo_home)
    icon = lde.icon_path(robo_home / "robo-engineer")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(robo_home)
    ...
    assert lde.icon_path(robo_home / "robo-engineer").exists()
    # The agent itself survives a GUI uninstall.
    assert (robo_home / "robo-engineer" / "robo_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".robo")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import robo_cli.uninstall as uninstall

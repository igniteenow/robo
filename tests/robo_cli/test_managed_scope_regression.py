...
def robo_home(tmp_path, monkeypatch):
    ...
    # /etc/robo on the dev/CI box can't influence the test.
    monkeypatch.setenv("ROBO_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(robo_home, monkeypatch):
    from robo_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(robo_home, monkeypatch):
    from robo_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_robo_dotenv(robo_home=str(home))

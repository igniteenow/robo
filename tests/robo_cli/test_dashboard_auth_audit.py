...
Profile-aware location: ``$ROBO_HOME/logs/dashboard-auth.log``.
...
from robo_cli.dashboard_auth.audit import audit_log, AuditEvent


@pytest.fixture
def profile_home(tmp_path, monkeypatch):
    """Redirect $ROBO_HOME and ~ to a tmp dir for the duration of the test."""
    home = tmp_path / ".robo"
    home.mkdir()
...
def test_audit_writes_jsonlines(profile_home):
    audit_log(AuditEvent.LOGIN_START, provider="igniteenow", ip="1.2.3.4")
    audit_log(
        AuditEvent.LOGIN_SUCCESS,
        provider="igniteenow", user_id="u1",
    ...
    assert second["provider"] == "igniteenow"
...
def test_audit_redacts_token_like_fields(profile_home):
    audit_log(
        AuditEvent.LOGIN_SUCCESS,
        provider="igniteenow", access_token="should-not-appear",

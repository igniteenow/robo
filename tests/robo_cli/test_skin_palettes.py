...
fallbacks (a partial skin silently inherits the brand skin's colours and ends
up looking like two skins at once), and every palette must be a
...
from robo_cli.skin_engine import LIGHT_AUTHORED_SKINS, _BUILTIN_SKINS
...
LIGHT_AUTHORED = LIGHT_AUTHORED_SKINS
...
# --- Light palettes are held to the same floors, against a light terminal ------
# The checks above only hold dark-authored bases to the STRONG/SOFT floors.
# Light text that is too pale to read is just as much a bug, so light-authored
# bases and every `light_colors` overlay are audited against LIGHT_POLE too.
LIGHT_BLOCKS = [(n, "colors", p) for n, p, is_light in BASE_PALETTES if is_light] + [
    (n, block, p) for n, block, p, is_light in OVERLAYS if is_light
]


@pytest.mark.parametrize(("skin", "block", "palette"), LIGHT_BLOCKS, ids=[f"{n}:{b}" for n, b, _ in LIGHT_BLOCKS])
def test_light_foregrounds_are_readable_on_a_light_terminal(skin, block, palette):
    problems = []
    for keys, floor in ((STRONG_FG, STRONG_MIN), (SOFT_FG, SOFT_MIN)):
        for key in keys:
            if key in palette:
                ratio = contrast(palette[key], LIGHT_POLE)
                if ratio < floor:
                    problems.append(f"{key}={palette[key]} contrast {ratio:.2f} < {floor} vs {LIGHT_POLE}")
    assert not problems, f"{skin}.{block}:\n  " + "\n  ".join(problems)


def test_every_builtin_is_an_ignitee_now_skin():
    """No inherited skins may come back in through a merge."""
    assert set(_BUILTIN_SKINS) == {"robo", "ember", "midnight", "paper", "contrast"}

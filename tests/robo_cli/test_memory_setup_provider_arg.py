"""Tests for `robo memory setup [provider]` routing.
...
``robo memory setup honcho``) without the interactive picker — which matters
because the per-provider ``robo <provider>`` subcommand is only registered
...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect, run_behavior=None):
        ...
        # The hermetic conftest sets ROBO_DISABLE_LAZY_INSTALLS=1 so no test
        ...
        with _patch.dict(os.environ, {"ROBO_DISABLE_LAZY_INSTALLS": "0"}), \
             patch("plugins.memory.find_provider_dir", return_value=tmp_path), \
             patch("robo_cli.tools_config.shutil.which", side_effect=which_side_effect), \
             patch("robo_cli.tools_config.subprocess.run", fake_run):

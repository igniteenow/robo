...
2. ``robo_cli.main._refresh_windows_gateway_launchers`` — ``robo
...
def test_restart_spec_normalizes_legacy_pythonw_argv(tmp_path):
    ...
    argv = [str(pythonw), "-m", "robo_cli.main", "gateway", "run"]
    ...
    ), mock.patch("robo_cli.config.get_robo_home", return_value=str(tmp_path)):
...
# ---------------------------------------------------------------------------
# _refresh_windows_gateway_launchers: robo update regenerates launchers
# ---------------------------------------------------------------------------

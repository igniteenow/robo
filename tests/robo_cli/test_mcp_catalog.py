"""Tests for robo_cli.mcp_catalog and robo_cli.mcp_picker.
...
def catalog_dir(tmp_path, monkeypatch):
    ...
    cat.mkdir()
    monkeypatch.setenv("ROBO_OPTIONAL_MCPS", str(cat))
    return cat


@pytest.fixture(autouse=True)
def _isolate_robo_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp ROBO_HOME."""
    hh = tmp_path / "robo-home"
    hh.mkdir()
    monkeypatch.setenv("ROBO_HOME", str(hh))
    monkeypatch.setattr(
        "robo_cli.config.get_robo_home", lambda: hh
    )
    monkeypatch.setattr(
        "robo_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "robo_cli.config.get_env_path", lambda: hh / ".env"
    )
    # mcp_catalog grabs get_robo_home() lazily through robo_constants
    monkeypatch.setattr(
        "robo_constants.get_robo_home", lambda: hh
    )
    return hh
...
def _entry(name: str):
    ...
    from robo_cli.mcp_catalog import get_entry
...
class TestManifestParsing:
    def test_minimal_valid(self, catalog_dir):
        ...
        from robo_cli.mcp_catalog import list_catalog
    ...
    def test_http_api_key_builds_bearer_headers_template(self, catalog_dir):
        ...
        _write_manifest(catalog_dir, "demo", body)
        from robo_cli.mcp_catalog import _build_server_config
    ...
    def test_http_api_key_requires_matching_env_declaration(self, catalog_dir):
        ...
        from robo_cli.mcp_catalog import CatalogError, _parse_manifest
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from robo_cli.mcp_catalog import install_entry
    ...
    def test_install_with_api_key_prompts_and_saves(self, catalog_dir, monkeypatch):
        ...
        from robo_cli import mcp_catalog

        monkeypatch.setattr(mcp_catalog, "_prompt_input", lambda *a, **kw: "secret-val")

        from robo_cli.mcp_catalog import install_entry
        from robo_cli.config import get_env_value, load_config
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from robo_cli.mcp_catalog import install_entry, uninstall_entry
        from robo_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from robo_cli.mcp_catalog import uninstall_entry
...
class TestCatalogDiagnostics:
    def test_future_manifest_version_skipped_with_diagnostic(self, catalog_dir):
        ...
        from robo_cli.mcp_catalog import list_catalog, catalog_diagnostics
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from robo_cli import mcp_catalog
        from robo_cli.mcp_catalog import _do_git_install
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "robo_cli.curses_ui.curses_checklist",
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        """
        # Use the actual repo's optional-mcps directory (no ROBO_OPTIONAL_MCPS
        # override) so this test catches real manifests.
        monkeypatch.delenv("ROBO_OPTIONAL_MCPS", raising=False)
        from robo_cli.mcp_catalog import _catalog_root, _parse_manifest
    ...
    def test_all_shipped_manifests_are_version_locked(self, monkeypatch):
        ...
        pyproject dependencies — everything Robo fetches/launches is pinned
        to an exact version.

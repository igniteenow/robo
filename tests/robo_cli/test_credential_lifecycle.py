...
ROBO_HOME (tests/conftest.py isolation) and assert every store agrees
afterwards.
...
@pytest.fixture
def robo_home(monkeypatch, tmp_path):
    """Fresh ROBO_HOME with .env + auth.json + config.yaml fixtures."""
    home = tmp_path / "cred_home"
    home.mkdir()
    monkeypatch.setenv("ROBO_HOME", str(home))
    from robo_cli.config import invalidate_env_cache
...
def test_delete_clears_provider_models_cache(robo_home):
    _write_env(robo_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(robo_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = robo_home / "provider_models_cache.json"
...
def test_update_rotates_config_yaml_model_mirror(robo_home):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(robo_home, OPENAI_API_KEY=old)
    _write_config(
        robo_home,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = robo_home.joinpath("config.yaml").read_text(encoding="utf-8")

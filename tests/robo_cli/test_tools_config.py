"""Tests for robo_cli.tools_config platform tool persistence."""
...
from robo_cli.igniteenow_account import IgniteeNowPortalAccountInfo, IgniteeNowToolAccessInfo
from robo_cli.igniteenow_subscription import IgniteeNowSubscriptionFeatures
...
def test_all_invalid_platform_toolsets_logs_runtime_warning(caplog):
    ...
    (e.g. 'robo' instead of 'robo-cli') must warn at resolve time so an
    already-corrupted config is caught at runtime, not just during migration."""
    import robo_cli.tools_config as _tc
    ...
    config = {"platform_toolsets": {"cli": ["robo"]}}

    with caplog.at_level(logging.WARNING, logger="robo_cli.tools_config"):
        _get_platform_tools(config, "cli")

    warnings = [r.getMessage() for r in caplog.records if r.levelno >= logging.WARNING]
    assert any("#38798" in m and "robo" in m for m in warnings), warnings
...
def test_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["robo-cli"]}}
...
def test_partially_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["robo-cli", "bogus"]}}
...
# via the platform's native composite. The composite ``robo-discord``
...
def test_discord_toolsets_do_not_leak_to_other_platforms():
    ...
    config = {"platform_toolsets": {"telegram": ["robo-telegram", "discord"]}}
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    with patch("robo_cli.tools_config.save_config"):
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from robo_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """robo-gateway includes list should cover all messaging platforms."""
        from robo_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["robo-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from robo_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from robo_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from robo_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        """Both Ignitee Now Subscription and FAL.ai providers must carry the
        imagegen_backend tag so _configure_provider fires the picker."""
        from robo_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from robo_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("robo_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("robo_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("robo_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `robo tools` → "reconfigure existing" shows
    the same toolset two rows in a row.
    """
    from robo_cli.tools_config import _get_effective_configurable_toolsets
    ...
    assert spotify_rows[0][1] == "Spotify"  # built-in label (Robo design: no emoji glyphs)
...
# ---------------------------------------------------------------------------
# Inline Ignitee Now Portal login gate on managed-provider selection
# ---------------------------------------------------------------------------
...
#    reported as added/removed by `robo tools` ──────────────────────────




def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `robo tools` printed ``- kanban``
...
def test_vision_picker_custom_endpoint(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ROBO_HOME", str(tmp_path))
    import robo_cli.tools_config as tc
...
# `robo tools post-setup <key>` child. On Windows each console child (npm,
...
# ── Toolsets that shipped after a platform's last `robo tools` save ────────
#
# Saving the picker (or one toggle in the desktop Toolsets UI) replaces a
# platform's composite (``[robo-cli]``) with a frozen explicit list, and
...
def test_saved_list_gains_toolsets_that_shipped_after_it_was_written():
    """The bug: a frozen list never gained bfl, so composite users got Ignitee Now
    Portal video generation on upgrade and picker users silently did not."""
    on_composite = _get_platform_tools(
        {"platform_toolsets": {"cli": ["robo-cli"]}},
        "cli",
...
def test_platforms_whose_composite_excludes_it_are_left_narrow():
    ...
    composite (robo-acp, robo-webhook) that never carried the toolset."""
    from toolsets import TOOLSETS, resolve_toolset

    narrow = [
        platform
        for platform in ("acp", "webhook")
        if f"robo-{platform}" in TOOLSETS
        and not any(
            set(resolve_toolset(ts, include_registry=False))
            <= set(resolve_toolset(f"robo-{platform}"))

"""Tests for `_can_open_graphical_browser()` in robo_cli.auth.

Guards the fix for the May 2026 report where `robo auth add xai-oauth`
...
from robo_cli.auth import _can_open_graphical_browser
...
def _force_platform_linux(monkeypatch):
    monkeypatch.setattr("robo_cli.auth.sys.platform", "linux")

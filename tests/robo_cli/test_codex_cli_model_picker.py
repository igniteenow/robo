...
Covers:
 - Normal path (tokens already in Robo auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Robo
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `robo auth openai-codex`. The old
...
@pytest.fixture()
def robo_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Robo auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(robo_auth_only_env):
    """openai-codex appears when tokens are already in Robo auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Robo
    auth store."""
    robo_home = tmp_path / ".robo"
    robo_home.mkdir()
    ...
    (robo_home / "auth.json").write_text(

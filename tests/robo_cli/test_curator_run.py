"""Tests for `robo curator run` CLI behavior."""
...
def test_run_defaults_to_synchronous(monkeypatch, capsys):
    ...
    import robo_cli.curator as curator_cli
...
def test_dry_run_default_reports_synchroigniteenow_wording(monkeypatch, capsys):
    import agent.curator as curator_state
    import robo_cli.curator as curator_cli
    ...
    assert "Read the report with `robo curator status`" in out

"""Tests for the top-level `./robo` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./robo` should use `robo_cli.main`, not the legacy Fire wrapper.

    `./robo` itself is a POSIX ``sh`` shim that ``exec``s ``robo.py``; run the
    Python launcher it delegates to.
    """
    launcher_path = Path(__file__).resolve().parents[2] / "robo.py"
    called = []

    fake_main_module = types.ModuleType("robo_cli.main")

    def fake_main():
        called.append("robo_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "robo_cli.main", fake_main_module)

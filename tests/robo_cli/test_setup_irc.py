"""Tests for IRC gateway configuration via `robo setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Robo install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Robo install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#robo")
            monkeypatch.setenv("IRC_NICKNAME", "robo-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `robo setup gateway` experience with IRC present."""

"""Tests for the ``robo send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``robo_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
def test_load_robo_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``robo send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``robo config set``) lives
    ...
    robo_home.mkdir()
    (robo_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_robo_home() to re-resolve under the patched env.
    from importlib import reload

    import robo_cli.config as _hc_config
    reload(_hc_config)

    send_cmd._load_robo_env()

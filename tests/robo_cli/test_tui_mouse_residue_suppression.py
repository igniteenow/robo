...
The Python launcher (`robo --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Robo session
...
from robo_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        ...
        monkeypatch.setenv("ROBO_TUI_NO_EARLY_DISABLE", "1")

"""Tests for the per-profile Projects store (robo_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Robo Agent", folders=["/tmp/robo"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "robo-engineer"
    assert proj.name == "Robo Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/robo"
    assert [f.path for f in proj.folders] == ["/tmp/robo"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "robo-engineer").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' ROBO_HOME.

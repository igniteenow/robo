...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_robo_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import robo_cli.oneshot as oneshot
        from robo_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """robo -z bypasses RoboCLI, but recall still needs SessionDB."""
    from robo_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "robo_state", mod("robo_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "robo_cli.config",
        mod("robo_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "robo_cli.models",
        mod("robo_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        sys.modules,
        "robo_cli.tools_config",
        mod("robo_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["ROBO_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="igniteenow/robo-test", provider="igniteenow", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["ROBO_MODEL"] == "igniteenow/robo-test"
    assert env["ROBO_INFERENCE_MODEL"] == "igniteenow/robo-test"
    assert env["ROBO_TUI_PROVIDER"] == "igniteenow"
    assert env["ROBO_INFERENCE_PROVIDER"] == "igniteenow"
    assert env["ROBO_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["ROBO_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("robo-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_robo_ink(monkeypatch, main_mod, tmp_path):

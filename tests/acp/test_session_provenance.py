...
def test_root_session_no_compression(db):
    ...
    assert prov["currentRoboSessionId"] == "root1"
    assert prov["rootRoboSessionId"] == "root1"
    assert prov["parentRoboSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_robo_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentRoboSessionId"] == "old"
    assert prov["rootRoboSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousRoboSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootRoboSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"robo"}
    assert "sessionProvenance" in meta["robo"]
    assert meta["robo"]["sessionProvenance"]["currentRoboSessionId"] == "root1"

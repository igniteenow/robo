...
class TestApprovalBridge:
    ...
    def test_allow_always_maps_correctly(self):
        # Direct callback path, not use_prompt_path — prompt_dangerous_approval
        # unconditionally forces allow_permanent=False on every callback it
        # drives ("Robo's mission checkpoint intentionally exposes only
        # once/session/deny", tools/approval.py), so "allow_always" is never
        # an offered option on that path and this outcome is unreachable
        # there by design. It IS a real, offered option on the direct
        # callback path (see test_bridge_schedules_request_on_the_given_loop),
        # which is what this test actually verifies the mapping for.
    ...
    def test_prompt_path_never_offers_allow_always(self):
        """The mission-checkpoint path must never permanently allow-list a
        dangerous command in one click — only once/session/deny.

        This pins tools.approval.prompt_dangerous_approval's documented
        "intentionally exposes only once/session/deny" invariant at the ACP
        bridge boundary: even a client that answers with "allow_always" (and
        even when the caller asked for allow_permanent=True) must fall
        through to the unknown-option-id path and deny, because
        _build_permission_options never received that option to offer in
        the first place.
        """

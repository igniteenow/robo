...
(/opt/robo/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable ROBO_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_robo_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable ROBO_HOME."""
    ...
    monkeypatch.setattr(
        "robo_constants.get_robo_home", lambda: robo_home
    )
    ...
    expected = robo_home / "scripts" / "whatsapp-bridge"

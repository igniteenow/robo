...
class TestSlackExecApproval:
    ...
    async def test_sends_blocks_with_buttons(self):
        ...
        assert "robo_approve_once" in action_ids
        assert "robo_approve_session" in action_ids
        assert "robo_approve_always" in action_ids
        assert "robo_deny" in action_ids
...
class TestSlackApprovalAction:
    ...
    async def test_truncates_inflated_original_text(self):
        ...
        action = {"action_id": "robo_approve_once", "value": "session-key"}
...
class TestSlackReactionForwarding:
    ...
    async def test_reaction_synthesizes_message_in_thread(self):
        ...
        assert synth["_robo_reaction"]["name"] == "thumbsup"
        assert synth["_robo_reaction"]["action"] == "added"
        assert synth["_robo_reaction"]["reacted_to_ts"] == "2000.0"
        # Pre-authorized as addressed-to-the-bot (skips mention gate only).
        assert synth["_robo_force_process"] is True
    ...
    async def test_target_channel_handoff_routes_top_level(self):
        ...
        assert synth["_robo_no_thread_response"] is True
        assert synth["_robo_reaction_source_channel"] == "C1"
    ...
    async def test_target_channel_with_thread_routes_to_thread(self):
        ...
        assert "_robo_no_thread_response" not in forwarded[0]

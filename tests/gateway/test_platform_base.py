...
def test_media_delivery_denies_encrypted_bitwarden_cache(tmp_path, monkeypatch):
    ...
    robo_home.mkdir()
    monkeypatch.setattr(base, "_ROBO_HOME", robo_home)
    monkeypatch.setattr(base, "_ROBO_ROOT", robo_home)
    path = robo_home / "cache" / "bws_cache.enc.json"
    path.parent.mkdir()
...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.robo/media/'
...
class TestExtensionlessMediaDelivery:
    ...
    def _patch_allow_root(self, monkeypatch, root):
        ...
        monkeypatch.delenv("ROBO_MEDIA_DELIVERY_STRICT", raising=False)
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("ROBO_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("ROBO_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        monkeypatch.setenv("ROBO_KANBAN_HOME", str(tmp_path / "robo"))
        monkeypatch.setenv("ROBO_MEDIA_TRUST_RECENT_FILES", "0")
        board_root = tmp_path / "robo" / "kanban" / "boards" / "research"
    ...
    def test_recency_trust_denies_system_paths_even_when_fresh(self, tmp_path, monkeypatch):
        ...
        """
        self._patch_roots(monkeypatch)
        monkeypatch.delenv("ROBO_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("ROBO_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("ROBO_MEDIA_TRUST_RECENT_SECONDS", "600")
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.robo/mcp-tokens/ must
        ...
        fake_home = tmp_path / "home"
        robo_dir = fake_home / ".robo"
        (robo_dir / "mcp-tokens").mkdir(parents=True)
        secret = robo_dir / rel
        secret.write_text('{"access_token": "live-bearer-abc123"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._ROBO_HOME",
            robo_dir,
        )
        monkeypatch.setattr(
            "gateway.platforms.base._ROBO_ROOT",
            robo_dir,
        )
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the ROBO_HOME root (google_token.json)
        ...
        token = robo_dir / "google_token.json"
        token.write_text('{"access_token": "***", "refresh_token": "***"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._ROBO_HOME", robo_dir)
        monkeypatch.setattr("gateway.platforms.base._ROBO_ROOT", robo_dir)

        assert BasePlatformAdapter.validate_media_delivery_path(str(token)) is None


    def test_denylist_blocks_non_cache_file_under_robo_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.robo
        ...
        robo_dir = fake_home / ".robo"
        robo_dir.mkdir(parents=True)
        artifact = robo_dir / "adhoc_report.pdf"
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting ROBO_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose ROBO_HOME is symlinked
        under a denied prefix (e.g. /opt/data -> /root/.robo) emits
        ...
        robo_root = denied_root / ".robo"
        prof_cache = robo_root / "profiles" / "myprof" / "cache" / "images"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._ROBO_ROOT", robo_root
        )
...
class TestGetHumanDelay:
    ...
    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "ROBO_HUMAN_DELAY_MODE": "natural",
            "ROBO_HUMAN_DELAY_MIN_MS": "oops",
            "ROBO_HUMAN_DELAY_MAX_MS": "still-bad",
        }
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"ROBO_MEDIA_DELIVERY_STRICT": "1",
                                     "ROBO_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ROBO_MEDIA_DELIVERY_STRICT", "0")
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Robo
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../robo/cache/video/abc.mp4``
    — a host filesystem path with no actionable information.
    """

    SENSITIVE_PATH = "/home/jayne/.robo/cache/media/sensitive_host_path_abc123.bin"

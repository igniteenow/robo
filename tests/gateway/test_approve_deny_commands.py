...
class TestApproveCommand:
    ...
    def test_session_choice_enables_broad_session_yolo(self):
        """Choosing 'session' via resolve_gateway_approval — what the TUI's
        answerApproval('session') actually calls through tui_gateway's
        approval.respond handler — must enable session-wide yolo, not just
        resolve the one pending entry. Otherwise the very next, unrelated
        command re-prompts, defeating the point of granting session trust
        (the actual bug this test pins down).
        """
        from tools.approval import (
            _ApprovalEntry,
            _gateway_queues,
        ...
        session_key = "test-session-session-choice"
        entry = _ApprovalEntry({"command": "npm install"})
        _gateway_queues[session_key] = [entry]

        assert is_session_yolo_enabled(session_key) is False

        resolved = resolve_gateway_approval(session_key, "session")

        assert resolved == 1
        assert entry.result == "session"
        assert entry.event.is_set()
        # This is the actual fix: session-wide trust, not just this one command.
        assert is_session_yolo_enabled(session_key) is True

    def test_once_choice_does_not_enable_session_yolo(self):
        """'once' must stay scoped to the single command — only 'session'
        grants broader trust. Guards against over-applying the fix above."""
        from tools.approval import (
            _ApprovalEntry,
            _gateway_queues,
            is_session_yolo_enabled,
        ...
        session_key = "test-session-once-choice"
    ...
    def test_deny_choice_does_not_enable_session_yolo(self):
        """'deny' must never grant session trust — sanity check on the
        exact-match condition (only 'session', nothing else)."""
        from tools.approval import (
            _ApprovalEntry,
            _gateway_queues,
            is_session_yolo_enabled,
        ...
        session_key = "test-session-deny-choice"
        entry = _ApprovalEntry({"command": "rm file.txt"})
        _gateway_queues[session_key] = [entry]
    ...
    def test_session_yolo_from_one_command_bypasses_a_different_later_command(self):
        """End-to-end proof of the actual user-facing bug: approve one
        command with 'session', then confirm check_all_command_guards lets
        a completely different, later command through without prompting.
        """
        from tools.approval import (
            _ApprovalEntry,
            _gateway_queues,
        ...
        session_key = "test-session-e2e-bypass"
        first_entry = _ApprovalEntry({"command": "npm install"})
        _gateway_queues[session_key] = [first_entry]
        resolve_gateway_approval(session_key, "session")

        with patch("tools.approval.get_current_session_key", return_value=session_key):
            # A different, unrelated command — must not prompt again.
            result = check_all_command_guards("pip install requests", "local")

        assert result["approved"] is True

    def test_session_yolo_never_bypasses_the_hardline_floor(self):
        """Session trust must still stop at genuinely catastrophic commands
        — exactly the 'except when it tries to delete anything or run a
        very dangerous command' carve-out the behavior is supposed to have.
        """
        from tools.approval import (
            _ApprovalEntry,
            _gateway_queues,
        ...
        session_key = "test-session-hardline-floor"
        entry = _ApprovalEntry({"command": "npm install"})
        _gateway_queues[session_key] = [entry]
        resolve_gateway_approval(session_key, "session")
...
class TestBlockingApprovalE2E:
    ...
    def setup_method(self):
        ...
        os.environ.pop("ROBO_GATEWAY_SESSION", None)
        os.environ.pop("ROBO_EXEC_ASK", None)
        os.environ.pop("ROBO_SESSION_KEY", None)
    ...
    def test_blocking_approval_uses_canonical_timeout(self, approval_config, monkeypatch):
        ...
        def agent_thread():
            ...
            os.environ["ROBO_SESSION_KEY"] = session_key
            try:
                with patch(
...
class TestFallbackNoCallback:
    ...
    def test_no_callback_returns_approval_required(self):
        ...
        os.environ["ROBO_EXEC_ASK"] = "1"
        os.environ["ROBO_SESSION_KEY"] = "no-callback-test"
        try:
...
class TestCrossSessionApprovalIsolation:
    ...
    process-global ``os.environ["ROBO_SESSION_KEY"]`` inside
    ...
    def teardown_method(self):
        ...
        os.environ.pop("ROBO_SESSION_KEY", None)
        # clear_session_vars() (used by test_unset_contextvar_does_not_fall_
        # back_to_clobbered_environ) deliberately leaves its ContextVars set
        # to "" rather than _UNSET — correct for a real handler's cleanup
        # (get_session_env must not fall back to stale os.environ), but that
        # "" is itself a real value to get_session_env, so it silently
        # persists into every later test in this pytest process (contextvars
        # only get a fresh copy at a new thread/task boundary, not between
        # plain function calls in the same thread) and makes
        # get_current_session_key() resolve to "" instead of falling through
        # to "default". Restore the true _UNSET sentinel here.
        reset_session_vars()
    ...
    def test_contextvar_wins_over_clobbered_environ(self):
        ...
        os.environ["ROBO_SESSION_KEY"] = "session-B"
    ...
    def test_unset_contextvar_does_not_fall_back_to_clobbered_environ(self):
        ...
        os.environ["ROBO_SESSION_KEY"] = "session-B-stale"
    ...
    def test_approval_prompt_routes_to_originating_session(self):
        ...
        def worker_a():
            ...
            # gateway, which no longer writes ROBO_SESSION_KEY).
            token = set_current_session_key("session-A")
            try:
                result_holder[0] = check_all_command_guards(
                    "rm -rf /important", "local"
    ...
    def test_two_concurrent_sessions_route_to_own_queue_contextvar_only(self):
        ...
        ``os.environ["ROBO_SESSION_KEY"]``. This proves the contextvar is
        ...
        # No ROBO_SESSION_KEY in os.environ at all — pure contextvar routing.
        os.environ.pop("ROBO_SESSION_KEY", None)
        os.environ["ROBO_GATEWAY_SESSION"] = "1"
        os.environ["ROBO_EXEC_ASK"] = "1"

...
class TestGatewayPidState:
    def test_write_pid_file_records_gateway_metadata(self, tmp_path, monkeypatch):
        ...
        assert payload["kind"] == "robo-gateway"
    ...
    def test_get_running_pid_cached_invalidates_when_pid_file_changes(self, tmp_path, monkeypatch):
        ...
        def _write_record(pid: int, start_time: int) -> None:
            record = {
                "pid": pid,
                "kind": "robo-gateway",
                "argv": ["python", "-m", "robo_cli.main", "gateway"],
    ...
    def test_gateway_identity_files_use_process_home_not_context_override(
        ...
        """Regression: pid/lock/state files must use process-level ROBO_HOME.
        ...
        the process-level ROBO_HOME, not the profile's directory.  See #56986.
        """
        ...
        monkeypatch.setenv("ROBO_HOME", str(process_home))
...
class TestGatewayRuntimeStatus:
    ...
    def test_runtime_status_running_pid_rejects_pid_reused_by_other_profile(self, monkeypatch):
        ...
        onto the live *default* gateway (``robo gateway run``).  The recorded
        ...
        monkeypatch.setattr(
            status, "_read_process_cmdline", lambda pid: "robo gateway run --replace"
        )
    ...
    def test_runtime_status_running_pid_accepts_matching_profile_cmdline(self, monkeypatch):
        ...
        for cmdline in (
            "robo -p coder gateway run --replace",
            "/opt/robo/.venv/bin/robo --profile coder gateway run --replace",
            "robo_home=/opt/data/profiles/coder robo gateway run --replace",
        ):
    ...
    def test_command_line_belongs_to_profile_normalizes_separators(self):
        """A Windows argv renders ROBO_HOME with backslashes while the
        ...
        cmdline = r"robo_home=c:\opt\data\profiles\coder robo gateway run --replace"
...
class TestScopedLocks:
    ...
    def test_acquire_scoped_lock_rejects_live_other_process(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ROBO_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
    ...
    def test_acquire_scoped_lock_replaces_pid_reused_by_unrelated_process(self, tmp_path, monkeypatch):
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "robo-gateway",
            "argv": ["/Users/user/.robo/robo-engineer/robo_cli/main.py", "gateway", "run", "--replace"],
        }))
...
class TestTakeoverMarker:
    ...
    def test_write_marker_records_replacer_robo_home(self, tmp_path, monkeypatch):
        """The marker stamps the replacer's ROBO_HOME for cross-profile guard (#29092)."""
        monkeypatch.setenv("ROBO_HOME", str(tmp_path))
        monkeypatch.setattr(status, "_get_process_start_time", lambda pid: 42)
        ...
        payload = json.loads((tmp_path / ".gateway-takeover.json").read_text())
        assert payload["replacer_robo_home"] == str(tmp_path)
    ...
    def test_consume_rejects_marker_from_different_profile(self, tmp_path, monkeypatch):
        ...
        ROBO_HOME must be rejected even when PID + start_time coincidentally
        match — otherwise two profile services sharing a default ~/.robo flap
        ...
        marker_path.write_text(json.dumps({
            ...
            "replacer_pid": 99999,
            "replacer_robo_home": str(tmp_path / "profiles" / "other"),
    ...
    def test_consume_accepts_legacy_marker_without_robo_home(self, tmp_path, monkeypatch):
        """Back-compat (#29092): markers written by older Robo versions have no
        ``replacer_robo_home`` field; an absent field is treated as same-home so
...
class TestScopedLockTakeover:
    ...
    def _owner_record(target_home: Path, *, pid: int = 4242, start_time: int = 123):
        ...
        record = {
            "pid": pid,
            "kind": "robo-gateway",
            "argv": ["python", "-m", "robo_cli.main", "gateway", "run"],
            "start_time": start_time,
            "robo_home": str(target_home),
        }
    ...
    def test_verified_distinct_home_handoff_marks_target_before_sigterm(
        ...
        replacer_home.mkdir()
        monkeypatch.setenv("ROBO_HOME", str(replacer_home))
        ...
        monkeypatch.setattr(
            ...
            lambda _pid: "python -m robo_cli.main gateway run",
        )
        calls = []
        ...
        def terminate(pid, *, force=False):
            ...
            assert payload["target_robo_home"] == str(target_home)
            assert payload["replacer_robo_home"] == str(replacer_home)
...
class TestPlannedStopMarker:
    ...
    def test_consume_returns_true_on_windows_when_start_time_unavailable(
        ...
        it would always return False — and ``robo gateway stop`` would be
...
class TestReadProcessCmdlinePsFallback:
    ...
    def test_proc_cmdline_takes_priority_over_ps(self, monkeypatch):
        ...
        def fake_read_bytes(self):
            calls.append("proc")
            return b"python\x00robo_cli/main.py\x00gateway\x00"

        monkeypatch.setattr(status.Path, "read_bytes", fake_read_bytes)
        result = status._read_process_cmdline(12345)
        assert "robo_cli/main.py" in result
        assert calls == ["proc"]
...
class TestLaunchdPlistRespawnGovernance:
    def test_plist_has_throttle_interval(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ROBO_HOME", str(tmp_path))
        from robo_cli.gateway import generate_launchd_plist
...
class TestResolveGatewayLiveness:
    ...
    def test_profile_dir_scopes_every_read_to_that_profile(self, tmp_path):
        ...
        ignore the ROBO_HOME contextvar override (#56986), so the profile

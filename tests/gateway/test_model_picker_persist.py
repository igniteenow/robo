...
closure the PR changed, against a real temp ``ROBO_HOME``.
"""
...
def _fake_switch_result():
    ...
    from robo_cli.model_switch import ModelSwitchResult
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "robo_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "robo_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(gateway_run, "_robo_home", robo_home)
    _stub_picker_dependencies(monkeypatch)
    # save_config writes to ``get_robo_home() / config.yaml`` — point it here.
    monkeypatch.setattr("robo_constants.get_robo_home", lambda: robo_home)
    monkeypatch.setattr("robo_cli.config.get_robo_home", lambda: robo_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_robo_home", default_home)
    _stub_picker_dependencies(monkeypatch)

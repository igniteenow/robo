...
substrings like 'foo@robo_bot.example'.
...
def _make_adapter():
    ...
    adapter._bot = SimpleNamespace(id=999, username="robo_bot")
    return adapter
...
class TestSubstringFalsePositivesAreRejected:
    ...
    def test_email_like_substring(self):
        """bug #12545 exact repro: 'foo@robo_bot.example'."""
        adapter = _make_adapter()
        msg = _message(text="email me at foo@robo_bot.example")
    ...
    def test_substring_inside_url_without_entity(self):
        ...
        adapter = _make_adapter()
        msg = _message(text="see https://example.com/@robo_bot for details")
        assert adapter._message_mentions_bot(msg) is False


    def test_email_substring_in_caption(self):
        adapter = _make_adapter()
        msg = _message(caption="foo@robo_bot.example")
        assert adapter._message_mentions_bot(msg) is False
...
class TestCaseInsensitivity:
    ...
    def test_uppercase_mention(self):
        adapter = _make_adapter()
        text = "hi @ROBO_BOT"
        msg = _message(text=text, entities=[_mention_entity(text, mention="@ROBO_BOT")])
        assert adapter._message_mentions_bot(msg) is True

...
def gateway_home(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("ROBO_PREFILL_MESSAGES_FILE", raising=False)
    monkeypatch.delenv("ROBO_EPHEMERAL_SYSTEM_PROMPT", raising=False)
    monkeypatch.delenv("ROBO_GATEWAY_BUSY_INPUT_MODE", raising=False)
    monkeypatch.delenv("ROBO_RESTART_DRAIN_TIMEOUT", raising=False)
    monkeypatch.delenv("ROBO_BACKGROUND_NOTIFICATIONS", raising=False)
    return tmp_path

"""Unit tests for the generic-OIDC / Ignitee Now-Portal caller-identity token resolver.

Covers gateway.relay._resolve_relay_identity_token() — the canonical resolver
shared by the runtime self-provision path and the `robo gateway enroll` CLI.
...
Two modes:
  ...
  2. Ignitee Now Portal (resolve_igniteenow_access_token) otherwise — the default.

The HTTP POST and the Ignitee Now resolver are monkeypatched; these prove the mode

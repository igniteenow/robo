...
def _arm(monkeypatch, *, url="wss://connector.example/relay", token="nas-token"):
    """Arm the real trigger: a relay URL + a resolvable relay identity token.
...
def test_no_nas_token_is_non_fatal(monkeypatch):
    """A self-hosted box with a relay URL but no configured IdP skips
    ...
    # No gateway.idp.token_url configured — _resolve_relay_identity_token
    # raises by default now that there is no managed fallback.
...
def test_relay_display_name_suppresses_stock_brand(monkeypatch):
    """The default 'Robo Agent' brand is identical on every install — forwarding
    ...
    class _Skin:
        def get_branding(self, key, fallback=""):
            return "Robo Agent" if key == "agent_name" else fallback

    monkeypatch.setattr("robo_cli.skin_engine.get_active_skin", lambda: _Skin())

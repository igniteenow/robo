...
def test_get_flush_dir_uses_get_robo_home(tmp_path, monkeypatch):
    """Flush dir must use get_robo_home(), not hardcoded Path.home()."""
    import gateway.shutdown_flush as mod

    captured = {}

    def fake_get_robo_home():
        from pathlib import Path
        captured["called"] = True
        return tmp_path

    monkeypatch.setattr(
        "robo_constants.get_robo_home", fake_get_robo_home
    )

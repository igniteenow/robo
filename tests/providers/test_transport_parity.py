...
# robo_cli/igniteenow_account.py); these tests only apply when a profile for it
# is registered, e.g. via an external provider plugin.
_requires_igniteenow_profile = pytest.mark.skipif(
    get_provider_profile("igniteenow") is None,
...
@_requires_igniteenow_profile
class TestIgniteeNowParity:
    """Ignitee Now: product tags, reasoning, omit when disabled."""

    def test_tags(self, transport):
        from agent.portal_tags import igniteenow_portal_tags
        kw = transport.build_kwargs(
            model="hermes-3-llama-3.1-405b",
            messages=_simple_messages(),
            tools=None,
            provider_profile=get_provider_profile("igniteenow"),
        )
        assert kw["extra_body"]["tags"] == igniteenow_portal_tags()

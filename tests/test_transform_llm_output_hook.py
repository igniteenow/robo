...
from robo_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(robo_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <robo_home>/plugins/<name> and opt it in."""
    plugin_dir = robo_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ROBO_HOME", str(tmp_path / "robo_empty"))

...
def _clear_terminal_env(monkeypatch):
    ...
    # Default: no Ignitee Now subscription — patch tool_backend_helpers (used by
    # resolve_modal_backend_state).
    import tools.tool_backend_helpers as _tbh
    monkeypatch.setattr(_tbh, "managed_igniteenow_tools_enabled", lambda: False)


def test_local_terminal_requirements(monkeypatch, caplog):
    """Local backend uses Robo' own LocalEnvironment wrapper."""
...
def test_modal_backend_managed_mode_without_feature_flag_logs_clear_error(monkeypatch, caplog, tmp_path):
    ...
    assert ok is False
    assert any(
        "managed Modal execution is no longer available" in record.getMessage()
        for record in caplog.records
    )

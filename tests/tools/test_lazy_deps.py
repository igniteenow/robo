...
class TestEnsure:
    ...
    def test_openwakeword_uses_onnx_only_install_path(self, monkeypatch):
        installed = set()
        monkeypatch.setattr(
            ld,
            "_is_satisfied",
        ...
        def fake_install(specs, **kwargs):
            ...
            installed.update(ld._pkg_name_from_spec(spec) for spec in specs)
        ...
        assert calls[0] == (("openwakeword==0.6.0",), {"no_deps": True})
        assert all(
            ld._pkg_name_from_spec(spec) != "openwakeword"
            for spec in calls[1][0]
        )
        assert calls[1][1] == {}
...
class TestIsSatisfiedVersionAware:
    ...
    def test_trace_upload_hub_at_core_locked_version_is_current(self, monkeypatch):
        ...
        core installs satisfies the trace-upload spec, so the `robo update`
...
# ---------------------------------------------------------------------------
# active_features + refresh_active_features (Piece A — robo update wiring)
# ---------------------------------------------------------------------------
...
class TestActiveFeatures:
    ...
    def test_shared_dependency_does_not_activate_feature(self, monkeypatch):
        ...
        # reasons. It must not make robo update try to refresh Matrix unless
...
class TestRefreshActiveFeatures:
    ...
    def test_windows_matrix_refresh_is_skipped_before_pip(self, monkeypatch):
        ...
        # path. `robo update` must not retry that doomed install every run.

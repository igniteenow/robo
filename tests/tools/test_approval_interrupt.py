...
class TestApprovalInterrupt:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("ROBO_GATEWAY_SESSION", "ROBO_YOLO_MODE",
                      "ROBO_SESSION_KEY")
        }
        os.environ.pop("ROBO_YOLO_MODE", None)
        os.environ["ROBO_GATEWAY_SESSION"] = "1"
        os.environ["ROBO_SESSION_KEY"] = self.SESSION_KEY
        # Each test method below replaces this module attribute directly
        # (not via unittest.mock.patch, which would auto-restore), so it must
        # be saved/restored by hand or it leaks into every later test in the
        # same pytest process — see teardown_method.
    ...
    def teardown_method(self):
        ...
        for k, v in self._saved_env.items():
            if v is None:
                ...
                os.environ[k] = v
        _clear_approval_state()
        # Undo the per-test `mod._get_approval_config = lambda: ...`
        # monkeypatch. Without this, later tests in the same pytest process
        # see a permanently mocked config (e.g. a fixed `{"timeout": 1}`
        # missing "mode" entirely), which silently forces
        # `_get_approval_mode()` to its "manual" fallback and
        # `_get_approval_timeout()` to whatever this file last set — see the
        # cross-file failures this caused in test_approval_mode_parity.py.
    ...
    def test_interrupt_unblocks_pending_approval_quickly(self):
        ...
        # Compare the original contract keys; the decision dict also carries
        # ``interrupted``/``surface_closed`` flags so callers can tell a Stop
        # (or a redirect) apart from a real user deny.
        assert {k: result_holder["result"][k] for k in ("resolved", "choice", "reason")} == {
    ...
    def test_unrelated_thread_interrupt_does_not_unblock(self):
        ...
        assert {k: result_holder["result"][k] for k in ("resolved", "choice", "reason")} == {
            "resolved": False, "choice": None, "reason": None}

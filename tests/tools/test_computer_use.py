...
def _reset_backend():
    ...
    with patch.dict(os.environ, {"ROBO_COMPUTER_USE_BACKEND": "noop"}, clear=False):
        yield
    reset_backend_for_tests()
...
class TestRegistration:
    ...
    def test_cua_driver_cmd_env_override_is_resolved_dynamically(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ROBO_CUA_DRIVER_CMD", str(driver))
...
class TestCuaEnvironmentScrubbing:
    ...
    def test_cua_session_sanitizes_provider_env_vars(self):
        ...
        The cua-driver MCP subprocess should not inherit Robo-managed
...
class TestClickButtonPassthrough:
    """Surface 5 (IgniteeNow/robo-engineer#47072) — `middle_click` must
...
class TestImageMimeTypePropagation:
    """Surface 7 (IgniteeNow/robo-engineer#47072): trycua/cua#1961 made
...
class TestMcpInvocationResolution:
    """Surface 8 (IgniteeNow/robo-engineer#47072): instead of hardcoding
    ...
    the MCP subcommand doesn't require a Robo patch.
    ...
    def test_falls_back_when_manifest_missing_command(self):
        ...
        resolved driver path (so ROBO_CUA_DRIVER_CMD still wins)."""
        from unittest.mock import patch
        from tools.computer_use.cua_backend import _resolve_mcp_invocation
...
class TestStructuredElementsConsumption:
    """Surface 2 (IgniteeNow/robo-engineer#47072): trycua/cua#1961 made
...
class TestCapabilityDiscovery:
    """Surface 4 (IgniteeNow/robo-engineer#47072): the wrapper learns
...
class TestElementTokenAttachment:
    """Surface 6 (IgniteeNow/robo-engineer#47072): trycua/cua#1961 added
...
class TestSessionLifecycle:
    """Surface gap (audit June 2026): Robo never declared a cua-driver

...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects ROBO_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("ROBO_INTERACTIVE", "ROBO_GATEWAY_SESSION", "ROBO_EXEC_ASK", "ROBO_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ROBO_INTERACTIVE"] = "1"
        session_key = os.getenv("ROBO_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ROBO_INTERACTIVE or ROBO_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
...
class TestCombinedWarnings:
    ...
    def test_combined_cli_deny(self, mock_tirith):
        ...
        assert result["approved"] is False
        cb.assert_called_once()
        # The public checkpoint is always once/session/deny. The legacy wire
        # field stays false even when more than one warning source is present.
        assert cb.call_args[1]["allow_permanent"] is False
    ...
    @patch(_TIRITH_PATCH,
           return_value=_tirith_result("warn",
                                       [{"rule_id": "homograph_url"}],
                                       "homograph URL"))
    def test_legacy_always_alias_unlocks_the_live_session(self, mock_tirith):
        """A legacy Always response maps to the modern live-session grant."""
        os.environ["ROBO_INTERACTIVE"] = "1"
        ...
        assert _mod.is_session_yolo_enabled(session_key)
...
# ---------------------------------------------------------------------------
# Dangerous-only warnings → three-choice mission checkpoint
# ---------------------------------------------------------------------------

class TestMissionApprovalVisibility:
    @patch(_TIRITH_PATCH, return_value=_tirith_result("allow"))
    def test_dangerous_only_uses_three_choice_contract(self, mock_tirith):
        os.environ["ROBO_INTERACTIVE"] = "1"
        cb = MagicMock(return_value="always")
...
class TestGatewayApprovalAllowPermanent:
    ...
    def test_dangerous_only_uses_session_contract(self):

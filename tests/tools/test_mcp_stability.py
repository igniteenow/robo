...
# ---------------------------------------------------------------------------
#
# When a stdio MCP wrapper (e.g. ``some-agent mcp serve``) itself spawns a
...
class TestMCPReloadTimeout:
    ...
    def test_reload_timeout_does_not_block_forever(self, tmp_path, monkeypatch):
        ...
        # Create a mock RoboCLI-like object with the needed attributes
        class FakeCLI:
            _config_mtime = 0.0
            _config_mcp_servers = {}
        ...
        source = inspect.getsource(RoboCLI._check_config_mcp_changes)

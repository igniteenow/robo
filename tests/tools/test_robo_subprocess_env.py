"""Tests for robo_subprocess_env() — the centralized credential-safe env
...
full credential environment. Two tiers:
  ...
  * Tier 2 (_build_provider_env_blocklist()): LLM provider/tool keys — stripped
...
_TIER1_SAMPLE = {
    ...
    "ROBO_DASHBOARD_SESSION_TOKEN": "dash-secret",
}
...
def _build(extra=None, *, inherit_credentials=False):
    ...
    with patch.dict(os.environ, env, clear=True):
        return robo_subprocess_env(inherit_credentials=inherit_credentials)
...
class TestDelegatedChildMarker:
    def test_delegated_child_context_scrubs_parent_kanban_keys_and_sets_marker(self):
        ...
        with patch.dict(
            ...
            {
                **_SAFE_SAMPLE,
                "ROBO_KANBAN_TASK": "t_parent",
                "ROBO_KANBAN_RUN_ID": "123",
                "ROBO_KANBAN_DB": "/tmp/parent-kanban.db",
                "ROBO_KANBAN_WORKSPACE": "/tmp/parent-workspace",
            },
            clear=True,
        ):

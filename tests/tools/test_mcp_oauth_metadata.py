...
Covers:
- :class:`RoboTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.RoboMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _ROBO_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# RoboTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = RoboTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ROBO_HOME", str(tmp_path))
        storage = RoboTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (RoboMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: RoboTokenStorage, **context_attrs):
    ...
    provider = _ROBO_PROVIDER_CLS.__new__(_ROBO_PROVIDER_CLS)
    provider._robo_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ROBO_HOME", str(tmp_path))
        storage = RoboTokenStorage("mgr-srv")
        ...
        with patch.object(
            _ROBO_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ROBO_HOME", str(tmp_path))
        storage = RoboTokenStorage("flow-srv")
        ...
        with patch.object(
            _ROBO_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",

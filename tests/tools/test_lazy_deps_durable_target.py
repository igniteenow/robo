...
class TestGatingWithTarget:
    """``ROBO_DISABLE_LAZY_INSTALLS=1`` must STOP blocking once a durable
...
class TestInstallArgConstruction:
    ...
    def test_no_deps_flag_is_forwarded_to_pip(self, monkeypatch):
        monkeypatch.delenv(ld._LAZY_TARGET_ENV, raising=False)
        monkeypatch.setattr(ld.shutil, "which", lambda _: None)
        captured = {}
        ...
        monkeypatch.setattr(ld.subprocess, "run", fake_run)
        result = ld._venv_pip_install(("openwakeword==0.6.0",), no_deps=True)

        assert result.success
        assert "--no-deps" in captured["cmd"]
        assert captured["cmd"][-1] == "openwakeword==0.6.0"


@pytest.mark.skipif(
    os.environ.get("ROBO_RUN_NETWORK_TESTS") != "1",
    reason="opt-in real-install test (set ROBO_RUN_NETWORK_TESTS=1); CI runs "

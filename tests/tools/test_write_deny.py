...
class TestWriteDenyExactPaths:
    ...
    def test_robo_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.robo/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "robo_root"
        ...
        # Sanity check: ROBO_HOME does point to the profile dir, not the root.
        from robo_constants import get_robo_home, get_default_robo_root
        assert get_robo_home() == profile_home
        assert get_default_robo_root() == root
...
class TestWriteAllowed:
    ...
    def test_robo_control_files_requested_writable(self):

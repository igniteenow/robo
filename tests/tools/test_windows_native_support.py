...
code paths Robo takes on native Windows.
...
class TestConfigureWindowsStdio:
    """``robo_cli.stdio.configure_windows_stdio`` wiring.
    ...
    - respect ROBO_DISABLE_WINDOWS_UTF8 opt-out
    """
    ...
    def _reset_configured(self, monkeypatch):
        ...
        sys.modules.pop("robo_cli.stdio", None)
        # Fresh import now; tests import from robo_cli.stdio themselves,
        # but this guarantees the module they get is a brand-new copy.
        import robo_cli.stdio as _s
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("robo_cli.kanban_db", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "robo_cli" / "web_server.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """cli.py, robo_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "robo_cli/main.py", "gateway/run.py"],
    )
    def test_entry_point_calls_configure_stdio(self, relpath):
        root = Path(__file__).resolve().parents[2]
        source = (root / relpath).read_text(encoding="utf-8")
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call robo_cli.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    """robo_cli/_subprocess_compat.py POSIX + Windows behaviour."""

    def test_is_windows_matches_sys_platform(self):
        from robo_cli import _subprocess_compat as sc
    ...
    def test_resolve_node_command_returns_absolute_on_posix(self):
        ...
        from robo_cli._subprocess_compat import resolve_node_command
    ...
    def test_windows_detach_flags_includes_breakaway_from_job(self, monkeypatch):
        ...
        Without it, the gateway-respawn watcher spawned by ``robo update``
        (which runs under robo-setup.exe, itself a grandchild of the
...
# ---------------------------------------------------------------------------
# robo_cli/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "robo_cli" / "kanban_db.py").read_text(encoding="utf-8")
...
class TestCodeExecutionTransportTcpFallback:
    ...
    AF_INET endpoints based on the ROBO_RPC_SOCKET format.
    """
...
class TestNpmBareSpawnsResolved:
    ...
    shutil.which / robo_cli._subprocess_compat.resolve_node_command
    so Windows can execute the .cmd batch shims."""

    @pytest.mark.parametrize(
        "relpath",
        [
            "robo_cli/tools_config.py",
            "robo_cli/doctor.py",
...
class TestLocalEnvironmentWindowsTempDir:
    ...
    def test_source_has_windows_branch_using_robo_home(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "tools" / "environments" / "local.py").read_text(encoding="utf-8")
        assert "if _IS_WINDOWS:" in source
        assert "get_robo_home" in source
        assert 'cache_dir = get_robo_home() / "cache" / "terminal"' in source
...
class TestGitBashPathNormalization:
    ...
    def test_posix_noop(self):
        ...
        if sys.platform != "win32":
            assert _normalize_git_bash_path("/home/robodev/foo") == "/home/robodev/foo"
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_robo_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "robo_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "robo_cli/gateway.py must use the platform-aware detach helper"
        )
    ...
    def test_launch_detached_profile_gateway_restart_inlined_watcher_uses_breakaway(self):
        ...
        robo_cli._subprocess_compat; the watcher must call that helper from
        ...
        """
        root = Path(__file__).resolve().parents[2]
        text = (root / "robo_cli" / "gateway.py").read_text(encoding="utf-8")
        ...
        block = text[idx:end]
        assert "from robo_cli._subprocess_compat import" in block
    ...
    def test_watcher_threads_hidden_console_spec_into_respawn(self):
        ...
        The spec supplies the stable cwd + env overlay (ROBO_HOME,
        ...
        assert '_popen_kwargs["env"]' in block, (
            ...
            "ROBO_HOME) from the restart spec."
        )
...
class TestWindowlessGatewayRestartSpec:
    ...
    def test_noop_on_non_windows(self):
        import robo_cli.gateway_windows as gw

        argv = ["/path/venv/bin/python", "-m", "robo_cli.main", "gateway", "run"]
    ...
    def test_windows_keeps_console_python_and_preserves_tail(self):
        ...
        # ``from robo_cli.gateway import PROJECT_ROOT`` resolves from
        ...
        import robo_cli.config  # noqa: F401
        import robo_cli.gateway  # noqa: F401
        ...
        # Mock get_robo_home too: the real one calls Path.resolve(), which
        ...
        with mock.patch.object(gw.sys, "platform", "win32"), mock.patch.object(
            gw, "_stable_gateway_working_dir", return_value="C:/robo"
        ), mock.patch(
            "robo_cli.config.get_robo_home", return_value="C:/robo"
        ):
            new_argv, cwd, env = gw.windowless_gateway_restart_spec(list(argv))
...
class TestGatewayRunRestartWatcherOuterPopenFallback:
    ...
    def test_outer_watcher_retries_without_breakaway_on_oserror(self, monkeypatch):
        ...
        monkeypatch.setattr(gr, "_resolve_robo_bin", lambda: ["robo"])
        ...
        assert "_ROBO_GATEWAY" not in kw1["env"]
    ...
    def test_outer_watcher_dual_failure_warns_without_leaking_secrets(
        ...
        monkeypatch.setenv("ROBO_TEST_SECRET", secret)
        ...
        assert secret in (kwargs_used.get("env") or {}).get("ROBO_TEST_SECRET", "")

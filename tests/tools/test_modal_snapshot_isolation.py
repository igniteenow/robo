...
def _restore_tool_modules():
    original_robo_home = os.environ.get("ROBO_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_robo_home is None:
            os.environ.pop("ROBO_HOME", None)
        else:
            os.environ["ROBO_HOME"] = original_robo_home
        _reset_modules(("tools", "robo_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    robo_cli = types.ModuleType("robo_cli")
    robo_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["robo_cli"] = robo_cli
    robo_home = tmp_path / "robo-home"
    os.environ["ROBO_HOME"] = str(robo_home)
    sys.modules["robo_cli.config"] = types.SimpleNamespace(
        get_robo_home=lambda: robo_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="robo-engineer", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": robo_home / "modal_snapshots.json",

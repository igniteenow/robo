"""Cross-session ROBO_SESSION_ID leak via the shared bash snapshot.
...
``ROBO_SESSION_ID`` into the snapshot, so every LATER session ``source``d that
stale value and its ``echo $ROBO_SESSION_ID`` reported a FOREIGN session's id
— overriding the correct per-command Popen env injected by
``_inject_session_context_env``.

The fix strips the per-session bridged vars (ROBO_SESSION_* / UI /
...
def test_export_snippet_shape():
    snippet = _export_dump_excluding_session_vars('"$__robo_snap_tmp"')
    ...
    assert "${!ROBO_SESSION_*}" in snippet
    assert "${!ROBO_CRON_AUTO_DELIVER_*}" in snippet
    assert "ROBO_UI_SESSION_ID" in snippet
    assert "grep -vE" not in snippet
    assert '"$__robo_snap_tmp"' in snippet
    ...
    assert snippet.rstrip().endswith('> "$__robo_snap_tmp"')
    ...
    try:
        def run_as(sid):
            ...
            def worker():
                ...
                out["r"] = env.execute('echo "[$ROBO_SESSION_ID]"')
        ...
        if os.path.exists(snap):
            with open(snap) as f:
                assert "ROBO_SESSION_ID" not in f.read()
    finally:
        env.cleanup()

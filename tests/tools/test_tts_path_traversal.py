...
def test_output_path_rejects_robo_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    robo_home = tmp_path / "robo-home"
    robo_home.mkdir()
    monkeypatch.setattr(file_safety, "_robo_home_path", lambda: robo_home)
    monkeypatch.setattr(file_safety, "_robo_root_path", lambda: robo_home)

    target = robo_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    robo_home = tmp_path / "robo-home"
    token_dir = robo_home / "mcp-tokens"

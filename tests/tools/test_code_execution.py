...
UDS socket lifecycle, robo_tools generation, timeout enforcement,
...
class TestRoboToolsGeneration(unittest.TestCase):
    ...
    def test_file_transport_uses_tempfile_fallback_for_rpc_dir(self):
        src = generate_robo_tools_module(["terminal"], transport="file")
        self.assertIn("import json, os, shlex, tempfile, threading, time", src)
        self.assertIn("os.path.join(tempfile.gettempdir(), \"robo_rpc\")", src)
        self.assertNotIn('os.environ.get("ROBO_RPC_DIR", "/tmp/robo_rpc")', src)
    ...
    def test_uds_transport_serializes_concurrent_calls(self):
        ...
        src = generate_robo_tools_module(["terminal"], transport="uds")
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/robo_exec_", mkdir_cmd)
        self.assertIn("ROBO_RPC_DIR=/data/data/com.termux/files/usr/tmp/robo_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/robo_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/robo_exec_", mkdir_cmd)

    def test_timezone_shell_quoted_in_remote_execution(self):
        """ROBO_TIMEZONE must be shell-quoted in remote env_prefix to prevent injection."""
        class FakeEnv:
            def __init__(self):
                self.commands = []
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch.dict(os.environ, {"ROBO_TIMEZONE": malicious_tz}):
            result = json.loads(_execute_remote("print('hello')", "task-1", ["terminal"]))
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import robo_constants; print(robo_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("robo_constants.py", result["output"])

    def test_single_tool_call(self):
        """Script calls terminal and prints the result."""
        code = """
from robo_tools import terminal
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from robo_tools import shell_quote
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_robo_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("ROBO_RPC_SOCKET", child_env)


    def test_timezone_injected_when_set(self):
        env_backup = os.environ.copy()
        try:
            os.environ["ROBO_TIMEZONE"] = "America/New_York"
            child_env = self._get_child_env()
    ...
    def test_timezone_not_set_when_empty(self):
        env_backup = os.environ.copy()
        try:
            os.environ.pop("ROBO_TIMEZONE", None)
            child_env = self._get_child_env()
            if "TZ" in child_env:
...
class TestRpcTokenAuthorization(unittest.TestCase):
    ...
    request without the matching ROBO_RPC_TOKEN must be rejected before
    ...
    def test_generated_module_sends_token(self):
        """The generated robo_tools module reads ROBO_RPC_TOKEN and sends it."""
        src = generate_robo_tools_module(["terminal"], transport="uds")
        self.assertIn("ROBO_RPC_TOKEN", src)

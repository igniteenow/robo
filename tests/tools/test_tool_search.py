...
Coverage targets — these mirror known failure modes of deferred tool
search. Every test that names an issue is the regression
...
class TestClassification:
    def test_core_tools_never_defer(self):
        ...
        # Sample of core tools from _ROBO_CORE_TOOLS.
    ...
    def test_unknown_tool_not_deferrable(self):
        ...
        not be claimed as deferrable. This protects against the
    ...
    def test_classify_keeps_unknown_in_visible(self):
        ...
        This is the cron-tool-loss regression guard (cron lost ``exec``
...
class TestRegression_CronToolLoss:
    """Regression guard for the cron-tool-loss class of bug.

    Scenario: ``toolsAllow: ["exec"]`` on an isolated cron turn

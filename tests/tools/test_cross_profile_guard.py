...
def fake_robo(tmp_path, monkeypatch):
    """Build a two-profile Robo layout and point ROBO_HOME at
    the robo-security profile (matching the original-incident shape).
    """
    ...
    monkeypatch.setenv("ROBO_HOME", str(sec_home))

    import robo_constants
    monkeypatch.setattr(robo_constants, "get_default_robo_root", lambda: root)

    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_robo_home_path", lambda: sec_home)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_robo):
        from tools.file_tools import write_file_tool
        target = fake_robo["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_robo):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_robo["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "robo-security" in result["error"]
        # File untouched.
        assert target.read_text() == original


    def test_non_robo_path_unaffected(self, fake_robo, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_robo):
    ...
    def test_cross_profile_patch_bypass(self, fake_robo):
        from tools.file_tools import patch_tool
        target = fake_robo["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_robo):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_robo, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_robo["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up ROBO_HOME (set in
        ...
        assert "not found in active profile 'robo-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.robo/profiles/<name>/."""
        # Don't set ROBO_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_robo_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_robo_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_robo):
        """When active profile is 'robo-security', the prompt warns
        ...
        assert "Active Robo profile" in src
        assert "cross_profile=True" in src
        assert "~/.robo/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Robo profile: default" in src
        assert "Active Robo profile: {active_profile}" in src

...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad ROBO_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ROBO_GATEWAY_SESSION set, a bound session
...
def test_terminal_smart_deny_session_choice_unlocks_mission(gw_session, monkeypatch):
    """The operator's session choice overrides the reviewer for this mission."""
    with A._lock:
    ...
    assert A.is_session_yolo_enabled(gw_session) is True
...
def test_execute_code_smart_deny_session_choice_unlocks_mission(gw_session, monkeypatch):
    with A._lock:
        A._permanent_approved.discard("execute_code")
...
def test_smart_escalate_still_persists_session_choice(gw_session, monkeypatch):
    ...
    assert result["approved"] is True
    assert shown["approval_data"]["allow_permanent"] is False
    assert shown["approval_data"]["choices"] == ["once", "session", "deny"]
...
def test_env_scrub_robo_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "ROBO_HOME": "/h", "ROBO_PROFILE": "p",
        "ROBO_CONFIG": "/c.yaml", "ROBO_ENV": "/e",
        "ROBO_DELEGATED_CHILD_CONTEXT": "1",
        # other ROBO_* → dropped (broad prefix removed)
        "ROBO_BASE_URL": "https://x", "ROBO_INTERACTIVE": "1",
        "ROBO_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "ROBO_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ROBO_BASE_URL", "ROBO_INTERACTIVE", "ROBO_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ROBO_* vars."""
    import logging

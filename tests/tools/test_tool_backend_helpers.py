...
Tests cover:
- managed_igniteenow_tools_enabled() subscription-based gate
...
from robo_cli.igniteenow_account import IgniteeNowPortalAccountInfo
from tools.tool_backend_helpers import (
    coerce_modal_mode,
    has_direct_modal_credentials,
    managed_igniteenow_tools_enabled,
...
# managed_igniteenow_tools_enabled
# ---------------------------------------------------------------------------
class TestManagedIgniteeNowToolsEnabled:
    """Subscription-based gate: True for paid Ignitee Now subscribers."""

    def test_disabled_when_not_logged_in(self, monkeypatch):
        monkeypatch.setattr(
            "robo_cli.igniteenow_account.get_igniteenow_portal_account_info",
            lambda: IgniteeNowPortalAccountInfo(logged_in=False, source="none", fresh=False),
        )
        assert managed_igniteenow_tools_enabled() is False
...
class TestResolveModalBackendState:
    ...
    @staticmethod
    def _resolve(monkeypatch, mode, *, has_direct, managed_ready, igniteenow_enabled=False):
        """Helper to call resolve_modal_backend_state with feature flag control."""
        monkeypatch.setattr(
            "tools.tool_backend_helpers.managed_igniteenow_tools_enabled",
            lambda: igniteenow_enabled,
        )
    ...
    def test_auto_prefers_managed_when_available(self, monkeypatch):
        result = self._resolve(monkeypatch, "auto", has_direct=True, managed_ready=True, igniteenow_enabled=True)
    ...
    def test_direct_selects_direct_when_available(self, monkeypatch):
        result = self._resolve(monkeypatch, "direct", has_direct=True, managed_ready=True, igniteenow_enabled=True)
        assert result["selected_backend"] == "direct"

    def test_direct_none_when_no_credentials(self, monkeypatch):
        result = self._resolve(monkeypatch, "direct", has_direct=False, managed_ready=True, igniteenow_enabled=True)
        assert result["selected_backend"] is None

    # --- managed mode ---


    def test_managed_blocked_when_igniteenow_disabled(self, monkeypatch):
        result = self._resolve(monkeypatch, "managed", has_direct=True, managed_ready=True, igniteenow_enabled=False)
        assert result["selected_backend"] is None

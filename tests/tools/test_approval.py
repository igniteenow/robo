...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("robo_cli.config.load_config_readonly", return_value={"approvals": {"mode": False}}):
...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("ROBO_SESSION_KEY", session_key)
        monkeypatch.setenv("ROBO_EXEC_ASK", "1")
        monkeypatch.delenv("ROBO_CRON_SESSION", raising=False)
        monkeypatch.setattr(
            approval_module,
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("robo-verify-", "robo-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        try:
            ...
            raise
        basename = "robo-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/robo-verify-example.py",
            "rm -f /tmp/robo-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/../robo-verify-example.py",
            "rm -f /tmp/a/../../tmp/robo-verify-example.py",
            "rm -f /var/tmp/robo-verify-example.py",
            "rm -f /tmp/robo-verify-*",
            "rm -f /tmp/robo-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/robo-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/robo-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\robo-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\robo-victim", "Windows cmd destructive delete"),
            ...
            (r"powershell Remove-Item -Recurse -Force C:\tmp\robo-victim",
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"ROBO_SESSION_KEY": "bob"}, clear=False):
...
class TestRoboConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.robo/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.robo/config.yaml",
            "echo '  mode: off' >> ~/.robo/config.yaml",
            "echo x | tee ~/.robo/config.yaml",
            "echo x | tee $ROBO_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.robo/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-Robo absolute config.yaml is
        # handled by the project patterns, not the Robo-home rule.
        for cmd in (
            "cat ~/.robo/config.yaml",
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.robo/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Robo-home prefixes must fold to ~/ and
    ~/.robo/ in dangerous-command detection.
    ...
    SSH, and Robo config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_robo_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/ROBO_HOME so the fold is verified on the POSIX CI runner too."""
...
class TestFullCommandAlwaysShown:
    ...
    def test_choice_with_long_command(self):
        ...
        for keystroke, expected in (
            ("o", "once"), ("s", "session"), ("a", "session"), ("d", "deny"), ("v", "deny"),
        ):
...
class TestSmartDeniedPrompt:
    def test_callback_keeps_standard_three_choice_contract(self):
        captured = {}
        ...
        assert result == "deny"
        assert captured == {"allow_permanent": False}

    def test_short_prompt_smart_deny_accepts_session_input(self):
    ...
    def test_smart_deny_offers_once_session_and_deny(self, capsys):
        ...
        assert "[o]nce" in rendered and "[s]ession" in rendered and "[d]eny" in rendered
        assert "[a]lways" not in rendered

    def test_smart_deny_uses_locale_specific_three_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("ROBO_LANGUAGE", "tr")
        ...
        assert "[o]turum" in rendered
        assert "[r]eddet" in rendered
        assert "b/o/R" in prompts[0]
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.robo/robo-engineer && source venv/bin/activate && python -m robo_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc
        for variant in (
            "python -m robo_cli.main gateway run --replace &",
            "nohup python -m robo_cli.main gateway run --replace",
        ):
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart robo-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the Robo security config via IFS.
            "sed${IFS}-i ~/.robo/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep robo) bypasses the pkill/killall name-matching
    ...
    def test_kill_pidof_expansion_detected(self):
        """`kill $(pidof robo)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof robo_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof robo`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Robo service
    label achieves the same effect as `robo gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_against_robo_label_detected(self):
        for cmd in (
            "launchctl stop ai.robo.gateway",
            "launchctl kickstart -k system/ai.robo.gateway",
            "launchctl bootout system/ai.robo.gateway",
            "launchctl unload ~/Library/LaunchAgents/ai.robo.gateway.plist",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd

    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-Robo labels, are
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("ROBO_GATEWAY_SESSION", "ROBO_CRON_SESSION",
                      "ROBO_YOLO_MODE",
                      "ROBO_SESSION_KEY", "ROBO_INTERACTIVE")
        }
        os.environ.pop("ROBO_YOLO_MODE", None)
        os.environ.pop("ROBO_INTERACTIVE", None)
        # ROBO_CRON_SESSION takes priority over ROBO_GATEWAY_SESSION in
        ...
        os.environ.pop("ROBO_CRON_SESSION", None)
        os.environ["ROBO_GATEWAY_SESSION"] = "1"
        os.environ["ROBO_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("robo_cli.config.load_config_readonly", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"ROBO_INTERACTIVE": "1"}, clear=False):

...
The ``robo update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m robo_cli.main gateway run`` — image name
    ``pythonw``, not ``robo.exe`` — so the ``taskkill /IM robo.exe`` guard
    ...
    # The robo.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM robo.exe' in text

"""Shared fixtures for the robo-engineer test suite.
...
2. **Isolated ROBO_HOME.** ROBO_HOME points to a per-test tempdir so
   code reading ``~/.robo/*`` via ``get_robo_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".robo"`` instead of the canonical
   ``get_robo_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ROBO_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox ROBO_HOME before ANY test module is imported ──────────────────
# `robo_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_robo_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.robo/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes ROBO_HOME, but fixtures run
...
# the REAL Robo root — capture it BEFORE the sandbox rewires ROBO_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.robo (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("ROBO_KANBAN_HOME", "").strip()
_PRE_SANDBOX_ROBO_HOME = os.environ.get("ROBO_HOME", "")


def _robo_home_points_at_production(value: str) -> bool:
    """True when a pre-set ROBO_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    ROBO_HOME=~/.robo``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``robo_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.robo`` — the escape vector that landed pytest
    ...
    ROBO_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".robo").resolve()
    except Exception:
        return True
...
if _robo_home_points_at_production(os.environ.get("ROBO_HOME", "")):
    _SESSION_ROBO_HOME = tempfile.mkdtemp(prefix="robo-test-home-")
...
ROBO_HOME_AT_CONFTEST_IMPORT = os.environ.get("ROBO_HOME", "")
...
# ROBO_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ROBO_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``robo_cli.voice.speak_text`` — real synthesis,
    ...
    "ROBO_MAX_ITERATIONS",
    "ROBO_SESSION_PLATFORM",
    "ROBO_SESSION_CHAT_ID",
    "ROBO_SESSION_CHAT_NAME",
    "ROBO_SESSION_CHAT_TYPE",
    "ROBO_SESSION_THREAD_ID",
    "ROBO_SESSION_SOURCE",
    "ROBO_SESSION_KEY",
    "ROBO_GATEWAY_SESSION",
    "ROBO_CRON_SESSION",
    "_ROBO_GATEWAY",
    "ROBO_PLATFORM",
    "ROBO_MODEL",
    "ROBO_INFERENCE_MODEL",
    "ROBO_INFERENCE_PROVIDER",
    "ROBO_TUI_PROVIDER",
    "ROBO_MANAGED",
    "ROBO_MANAGED_DIR",
    "ROBO_DEV",
    ...
    "ROBO_KANBAN_BOARD",
    "ROBO_KANBAN_HOME",
    "ROBO_KANBAN_WORKSPACES_ROOT",
    "ROBO_KANBAN_LOGS_ROOT",
    "ROBO_KANBAN_TASK",
    "ROBO_KANBAN_WORKSPACE",
    "ROBO_KANBAN_RUN_ID",
    "ROBO_KANBAN_CLAIM_LOCK",
    "ROBO_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    # otherwise-unrelated config tests away from the default "robo" host.
    "ROBO_HONCHO_HOST",
    # Dashboard OAuth auth gate (PR #30156). When set, the bundled
    # dashboard-auth `igniteenow` plugin auto-registers itself on plugin discovery,
    ...
    "ROBO_DASHBOARD_OAUTH_CLIENT_ID",
    "ROBO_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ROBO_HOME to per-test tempdirs so code that
    reads ``~/.robo/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ROBO_* vars that could change test semantics.
    for name in _ROBO_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("ROBO_HONCHO_HOST", "robo")

    # 3. Redirect ROBO_HOME to a per-test tempdir. Code that reads
    #    ``~/.robo/*`` via ``get_robo_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.robo/*`` via
    #    ``Path.home() / ".robo"`` instead of ``get_robo_home()``
    #    is a bug to fix at the callsite.
    fake_robo_home = tmp_path / "robo_test"
    fake_robo_home.mkdir()
    (fake_robo_home / "sessions").mkdir()
    (fake_robo_home / "cron").mkdir()
    (fake_robo_home / "memories").mkdir()
    (fake_robo_home / "skills").mkdir()
    monkeypatch.setenv("ROBO_HOME", str(fake_robo_home))

    # 3b. robo_state computes ``DEFAULT_DB_PATH = get_robo_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from robo_state import ...``) that
    ...
    robo_state_mod = sys.modules.get("robo_state")
    if robo_state_mod is not None and hasattr(robo_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            robo_state_mod, "DEFAULT_DB_PATH", fake_robo_home / "state.db"
        )
    ...
    monkeypatch.setenv("ROBO_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.robo/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import robo_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_robo_home(_hermetic_environment):
...
# invocation), kanban writes silently pollute the real ~/.robo. This autouse
...
# allow-list because test-level fixtures legitimately move ROBO_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session ROBO_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``ROBO_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Robo root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_ROBO_HOME and not _robo_home_points_at_production(
        _PRE_SANDBOX_ROBO_HOME
    ):
        # ROBO_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_robo_root().resolve()
    # No pre-existing ROBO_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".robo").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.robo`` captured at import time. Hermetic tests that legitimately
    move ROBO_HOME to sibling tempdirs are unaffected.

    Only patches when ``robo_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("robo_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import robo_cli.kanban_db`
...
# ``robo_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Robo root. This fixture wires the
# test-side knobs:
...
#     installs where ROBO_HOME is not ~/.robo) into the guard's
...
# so subprocess children that import robo_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("robo_state")
    ...
    if _PRE_SANDBOX_ROBO_HOME and not _robo_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_ROBO_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Robo home override redirects every later
    # ``get_robo_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from robo_constants import get_robo_home_override, set_robo_home_override

        if get_robo_home_override() is not None:
            set_robo_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal robo config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``robo-gateway`` process
...
# correlated with a ``tests/robo_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> robo-gateway``
...
def _wal_is_usable() -> bool:
    """True when Robo will actually put a database into WAL mode here.

    Robo refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Robo can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Robo managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``robo_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_robo_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_robo_home`` fixture
    redirects ``ROBO_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.robo/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``robo_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["ROBO_VOICE_TTS"] = "1"``. The
...
#      ``robo_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``robo_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``robo_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``robo_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Robo falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Robo uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart robo-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting robo/python patterns are also blocked.
    """
    ...
    def _matches_robo_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ROBO_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_robo_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch robo-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m robo_cli.main".
                if (
                    "robo" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live robo-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting robo/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `robo update` (or the
        # equivalent `python -m robo_cli.main update`).  These commands
        ...
        if "update" in low and (
            # robo update / robo update --gateway / setsid bash -c ... robo update
            ("robo" in low and "update" in low.split())
            or
            # python -m robo_cli.main update --gateway
            ("robo_cli" in low and "update" in low.split())
            or
            # venv/bin/robo update  (absolute path variant used in tests)
            (".venv/bin/robo" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`robo update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_ROBO_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import robo_cli.voice as _voice
    except Exception:

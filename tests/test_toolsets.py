...
class TestToolsetConsistency:
    ...
    def test_robo_platforms_share_core_tools(self):
        """All robo-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on robo-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["robo-cli", "robo-telegram", "robo-discord", "robo-whatsapp", "robo-slack", "robo-signal", "robo-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_robo_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("robo-whatsapp")

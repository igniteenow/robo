...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"robo-engineer[termux]"' in text
    assert '"robo-engineer[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"robo-engineer[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]

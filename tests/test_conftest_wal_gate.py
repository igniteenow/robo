"""The conftest WAL gate must agree with robo_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``robo_state``. That is deliberate: importing
``robo_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.robo`` before the per-test ``ROBO_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_robo_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and robo_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_robo_state_at_collection():
    """The gate must stay import-free of robo_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.robo, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``robo_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "robo_state" or name.startswith("robo_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported robo_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.robo during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("robo_state", None)

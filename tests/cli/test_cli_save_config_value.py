...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # save_config_value resolves the target live via get_robo_home(), so
        # point ROBO_HOME at the temp dir (the _robo_home import-time
        # constant is no longer consulted).
        monkeypatch.setenv("ROBO_HOME", str(robo_home))
        monkeypatch.setattr("cli._robo_home", robo_home)
        return config_path
    ...
    def test_calls_roundtrip_yaml_update(self, config_env, monkeypatch):
        ...
        save_config_value("display.skin", "contrast")

        mock_update.assert_called_once_with(config_env, "display.skin", "contrast")
    ...
    def test_model_write_runs_shared_cron_drift_warning(self, config_env, monkeypatch):
        warning = MagicMock()
        monkeypatch.setattr(
            "robo_cli.config.warn_unpinned_cron_jobs_after_model_config_change",
            warning,
        )
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in ROBO_HOME/config.yaml
    ...
    install whose ROBO_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read ROBO_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh ROBO_HOME with NO config.yaml (managed/desktop first launch).
        robo_home = tmp_path / ".robo"
        robo_home.mkdir()
        monkeypatch.setenv("ROBO_HOME", str(robo_home))
    ...
    def test_does_not_write_repo_cli_config(self, tmp_path, monkeypatch):
        ...
        result = yaml.safe_load((robo_home / "config.yaml").read_text())
        assert result["wake_word"]["enabled"] is True

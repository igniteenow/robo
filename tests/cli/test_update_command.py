...
Verifies that ``RoboCLI._handle_update_command`` correctly:
...
Also verifies that ``robo_cli.main._launch_tui`` correctly handles exit
...
def _make_self(modal_response):
    ...
    so we don't need a full ``RoboCLI`` construction.
...
def _call(self_):
    ...
    return RoboCLI._handle_update_command(self_)
...
def test_managed_install_refuses_and_does_not_set_pending_relaunch(capsys):
    ...
    with (
        patch("robo_cli.config.is_managed", return_value=True),
        patch(
            "robo_cli.config.format_managed_message",

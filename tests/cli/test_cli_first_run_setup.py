...
def test_credentials_ready_false_when_no_provider(monkeypatch):
    ...
    monkeypatch.setattr("robo_cli.runtime_provider.resolve_runtime_provider", _raise)
...
def test_offer_first_run_setup_routes_into_shared_picker(monkeypatch):
    ...
    monkeypatch.setattr("robo_cli.main.select_provider_and_model", _fake_picker)
    monkeypatch.setattr("builtins.input", lambda *a, **k: "y")
    # After the picker "runs", config has a provider and creds resolve.
    monkeypatch.setattr(
        "robo_cli.config.load_config",
        lambda: {"model": {"provider": "igniteenow", "default": "hermes-4-405b"}},
    )
    monkeypatch.setattr(
        "robo_cli.runtime_provider.resolve_runtime_provider",
        lambda **kw: {
            ...
            "base_url": "https://inference-api.igniteenow.com/v1",
            "source": "oauth",
        },
    )
    ...
    assert shell.requested_provider == "igniteenow"
...
def test_offer_first_run_setup_declined(monkeypatch):
    ...
    monkeypatch.setattr("robo_cli.main.select_provider_and_model", _fail_picker)
...
def test_offer_first_run_setup_picker_cancel_is_graceful(monkeypatch):
    ...
    monkeypatch.setattr("robo_cli.main.select_provider_and_model", _cancel_picker)
...
def test_empty_key_error_names_actual_provider(monkeypatch, capsys):
    ...
    assert "robo model" in out or "robo setup" in out

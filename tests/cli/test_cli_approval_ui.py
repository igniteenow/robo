...
class TestCliApprovalUi:
    def test_smart_denied_callback_keeps_session_choice(self):
        cli = _make_cli_stub()
        result = {}
    ...
    def test_background_task_registers_thread_local_approval_callbacks(self):
        ...
        assert seen["approval"].__func__ is RoboCLI._approval_callback
        assert seen["sudo"].__self__ is cli
        assert seen["sudo"].__func__ is RoboCLI._sudo_password_callback
...
def _make_real_paint_cli_stub():
    ...
    cli._paint_now = RoboCLI._paint_now.__get__(cli, RoboCLI)
    cli._invalidate = RoboCLI._invalidate.__get__(cli, RoboCLI)

...
def _isolate(tmp_path, monkeypatch):
    """Isolate ROBO_HOME so tests don't touch real config."""
    home = tmp_path / ".robo"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal RoboCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = RoboCLI.__new__(RoboCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["robo-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Robo' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Robo' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Robo' minimum context length."""

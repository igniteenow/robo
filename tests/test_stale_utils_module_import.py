...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.robo/robo-engineer/utils.py)
...
2. ``robo update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``robo_cli/main.py`` (the ``robo update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``robo_constants`` after the

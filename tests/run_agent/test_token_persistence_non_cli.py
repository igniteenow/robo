...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    robo_state = ModuleType("robo_state")
    robo_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "robo_state", robo_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Robo"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Robo"

"""Regression guard: a genuine Ignitee Now 429 must re-enter the retry loop so the
top-of-loop Ignitee Now rate-limit guard can activate the fallback chain.
...
``record_igniteenow_rate_limit()`` moments earlier and either activates a fallback

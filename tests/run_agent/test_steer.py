...
class TestActiveTurnRedirect:
    ...
    def test_redirect_during_tool_execution_interrupts_batch_by_default(self, monkeypatch):
        # display.busy_input_mode: interrupt (default) — the running batch is
        # cancelled cooperatively and the text is applied as a real redirect
        # on the next loop iteration, so the model reads it immediately.
        agent = _bare_agent()
        agent._executing_tools = True
        monkeypatch.setattr(type(agent), "_busy_correction_interrupts_tools", lambda self: True)

        assert agent.redirect("also check migrations") is True
        assert agent._pending_redirect == "also check migrations"
        assert agent._pending_steer is None
        assert agent._interrupt_requested is True

    def test_redirect_during_tool_execution_uses_safe_steer_boundary(self, monkeypatch):
        # display.busy_input_mode: redirect — gentle mode keeps the batch
        # running and delivers the text on the last tool result.
        agent = _bare_agent()
        agent._executing_tools = True
        monkeypatch.setattr(type(agent), "_busy_correction_interrupts_tools", lambda self: False)
...
class TestSteerCommandRegistry:
    ...
    def test_steer_in_bypass_set(self):
        ...
        """
        from robo_cli.commands import ACTIVE_SESSION_BYPASS_COMMANDS, should_bypass_active_session

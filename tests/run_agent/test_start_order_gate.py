...
def _make_agent(monkeypatch):
    ...
    stub._apply_pending_claim_verification = lambda *a, **kw: None
...
class TestSafeCallWaitsForPendingApproval:
    """A safe tool call, dispatched in the same concurrent batch as one that
    needs human approval, must now WAIT for that approval to clear before it
    is allowed to do its own real work — not run freely alongside it. This is
    a deliberate, explicit behavior change: previously a safe call never
    reached any approval-related check at all (see
    tools/approval.py's is_human_wait_pending docstring history). The user
    found running-alongside-a-pending-prompt confusing (it looked like the
    prompted command itself was running unanswered) and asked for a hard
    pause across the whole batch instead.
    """

    def test_safe_tool_holds_until_pending_approval_clears(self, monkeypatch):
        import agent.tool_executor as te
        import tools.approval as approval_mod

        agent = _make_agent(monkeypatch)
        monkeypatch.setattr(te, "_resolve_concurrent_tool_timeout", lambda: 10.0)

        # Simulate "some sibling call currently has a human approval pending"
        # without needing a real approval round-trip — a controllable Event
        # standing in for is_human_wait_pending's real signal, released only
        # by this test, not by the gate under test.
        pending = threading.Event()
        pending.set()
        monkeypatch.setattr(
            approval_mod, "is_human_wait_pending", lambda *a, **kw: pending.is_set()
        )
        monkeypatch.setattr(approval_mod, "human_wait_ceiling", lambda: 5.0)
        ...
        msg = _FakeAssistantMsg([_FakeToolCall("safe_tool", "tc_safe")])
        messages: list = []
        ...
        assert dispatched, "safe_tool never ran at all"
        waited = dispatched[0][1] - started
        assert waited >= 0.4, (
            f"safe_tool ran after only {waited:.2f}s — it did not wait for the "
            "pending approval signal to clear before doing its real work"
        )

    def test_safe_tool_does_not_wait_when_nothing_is_pending(self, monkeypatch):
        """Sanity check on the other direction: with no pending approval at
        all, a safe call must run immediately — this change must not add a
        blanket delay to every tool call."""
        import agent.tool_executor as te
        import tools.approval as approval_mod

        agent = _make_agent(monkeypatch)
        monkeypatch.setattr(te, "_resolve_concurrent_tool_timeout", lambda: 10.0)
        monkeypatch.setattr(approval_mod, "is_human_wait_pending", lambda *a, **kw: False)
        ...
        assert dispatched, "safe_tool never ran"
        assert elapsed < 0.3, (
            f"safe_tool took {elapsed:.2f}s with nothing pending — the new "
            "check added a delay even when there's nothing to wait for"
        )

    def test_expires_and_proceeds_rather_than_hanging_forever(self, monkeypatch):
        """If the pending signal never clears (a stuck/leaked state), the
        wait must expire via human_wait_ceiling() and proceed anyway — same
        fail-open tradeoff used by every other bounded wait in this file,
        ...
        agent = _make_agent(monkeypatch)
        monkeypatch.setattr(te, "_resolve_concurrent_tool_timeout", lambda: 10.0)
        monkeypatch.setattr(approval_mod, "is_human_wait_pending", lambda *a, **kw: True)
        monkeypatch.setattr(approval_mod, "human_wait_ceiling", lambda: 0.3)
        ...
        assert dispatched, "safe_tool never ran even after the ceiling expired"
        assert elapsed < 3.0, (
            f"took {elapsed:.2f}s — did not respect human_wait_ceiling() and "
            "hung well past it instead of proceeding on expiry"
        )

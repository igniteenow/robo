...
class TestFallbackChainAdvancement:
    ...
    def test_igniteenow_anthropic_fallback_uses_the_messages_wire(self):
        """Portal Claude fallbacks must not stay on chat_completions.

        ``resolve_provider_client`` still returns an OpenAI client for Ignitee Now;
        ...
        """
        portal = "https://inference-api.igniteenow.com/v1"
        fbs = [
            {
                "provider": "igniteenow",
        ...
        with (
            ...
            patch(
                "robo_cli.model_normalize.normalize_model_for_provider",
    ...
    def test_igniteenow_non_anthropic_fallback_stays_on_chat_completions(self):
        portal = "https://inference-api.igniteenow.com/v1"
        fbs = [{"provider": "igniteenow", "model": "hermes-4-405b"}]
        agent = _make_agent(fallback_model=fbs)
        with (
            patch(
                "agent.chat_completion_helpers._fallback_entry_unavailable_without_network",
...
class TestFallbackChainDedup:
    ...
    def test_skips_entry_matching_current_provider_and_model(self):
        ...
        with patch("agent.auxiliary_client.resolve_provider_client", side_effect=_resolve):
            with patch("robo_cli.model_normalize.normalize_model_for_provider", side_effect=lambda m, p: m):

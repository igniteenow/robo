...
def test_aiagent_reuses_existing_errors_log_handler():
    ...
    error_log_path = (run_agent._robo_home / "logs" / "errors.log").resolve()
...
class TestSessionJsonSnapshotOptIn:
    ...
    def test_traversal_session_id_cannot_escape_logs_dir(self, agent, tmp_path):
        ...
        # originate from the untrusted X-Robo-Session-Id API header) must not
...
class TestSaveSessionLogRedactsSecrets:
    ...
    def _ensure_redaction_enabled(self, monkeypatch):
        """Force redaction on regardless of host ROBO_REDACT_SECRETS state.
...
class TestInit:
    ...
    def test_prompt_caching_cache_ttl_defaults_without_config(self):
        ...
        with (
            ...
            patch("run_agent.check_toolset_requirements", return_value={}),
            patch("run_agent.OpenAI"),
            patch("robo_cli.config.load_config", return_value={}), patch("robo_cli.config.load_config_readonly", return_value={}),
        ):
            a = AIAgent(
                api_key="test-k...7890",
                model="anthropic/claude-sonnet-4-20250514",
    ...
    def test_prompt_caching_disabled_by_falsy_cache_ttl(self, falsy_value):
        ...
        with (
            ...
            patch(
                "robo_cli.config.load_config_readonly",
                return_value={"prompt_caching": {"cache_ttl": falsy_value}},
            ),
        ):
            a = AIAgent(
...
class TestBuildSystemPrompt:
    ...
    def test_includes_igniteenow_subscription_prompt(self, agent, monkeypatch):
        monkeypatch.setattr(run_agent, "build_igniteenow_subscription_prompt", lambda tool_names: "IGNITEENOW SUBSCRIPTION BLOCK")
        prompt = agent._build_system_prompt()
        assert "IGNITEENOW SUBSCRIPTION BLOCK" in prompt
...
class TestBuildApiKwargs:
    ...
    def test_core_responses_preserves_supported_xhigh(self, agent, monkeypatch):
        ...
        monkeypatch.setattr(
            "robo_cli.models.github_model_reasoning_efforts",
...
class TestExecuteToolCalls:
    ...
    def test_keyboard_interrupt_emits_cancelled_post_tool_hook(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("robo_cli.lifecycle.invoke_hook", _capture_hook)
        monkeypatch.setattr("robo_cli.lifecycle.has_hook", lambda name: True)
    ...
    def test_interrupt_skips_remaining(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("robo_cli.lifecycle.has_hook", lambda name: True)
        monkeypatch.setattr(
            "robo_cli.lifecycle.invoke_hook",
...
class TestConcurrentToolExecution:
    ...
    def test_sequential_registry_tool_forwards_request_middleware_trace(
        ...
        from robo_cli.middleware import RequestMiddlewareResult
        ...
        mock_msg = _mock_assistant_msg(content="", tool_calls=[tool_call])
        monkeypatch.setattr(
            "robo_cli.middleware.apply_tool_request_middleware",
        ...
        monkeypatch.setattr(
            "robo_cli.middleware.run_tool_execution_middleware",
            lambda _name, args, callback, **_kwargs: callback(args),
        )
        monkeypatch.setattr(
            "robo_cli.plugins.resolve_pre_tool_block",
    ...
    def test_tool_execution_middleware_replacement_emits_one_terminal_hook(
        ...
        monkeypatch.setattr("robo_cli.plugins.get_plugin_manager", lambda: manager)
        monkeypatch.setattr(
            "robo_cli.lifecycle.invoke_hook",
            lambda hook_name, **kwargs: hook_calls.append((hook_name, kwargs)) or [],
        )
    ...
    def test_managed_tool_pipeline_rejects_second_dispatch(self, agent, monkeypatch):
        ...
        assert duplicate_errors == [
            "Robo tool execution callback invoked more than once"
        ]
    ...
    def test_managed_tool_pipeline_allows_one_concurrent_dispatch(
        ...
        assert outcome.result == "ok"
        assert dispatched == [{"command": "true"}]
        assert errors == ["Robo tool execution callback invoked more than once"]
        assert outcome.blocked is False
...
class TestHandleMaxIterations:
    ...
    def test_api_sanitizer_repairs_tool_call_with_empty_function_name(self, agent):
        ...
        call, because robo' dispatch loop keeps empty-name calls paired with
...
class TestRunConversation:
    ...
    def test_task_start_failure_closes_relay_turn_and_lease(self, agent):
        ...
        with (
            ...
            patch(
                "robo_cli.observability.relay_shared_metrics.start_task_run",
                side_effect=start_error,
            ),
            patch(
                "robo_cli.observability.relay_shared_metrics.finish_task_run"
            ) as finish_task_run,
    ...
    def test_ollama_small_runtime_context_fails_before_api_call(self, agent, caplog):
        ...
        assert "Ollama runtime context too small for Robo tool use" in caplog.text
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            ...
            patch(
                "robo_cli.lifecycle.has_hook",
                side_effect=lambda name: name in {"pre_api_request", "post_api_request"},
            ),
            patch("robo_cli.lifecycle.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_api_request_error_hook_skips_payload_work_without_listener(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("robo_cli.lifecycle.has_hook", lambda name: False)
        monkeypatch.setattr("robo_cli.lifecycle.invoke_hook", _invoke_hook)
    ...
    def test_request_scoped_api_hooks_skip_payload_work_without_listeners(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("robo_cli.lifecycle.has_hook", _has_hook)
        ...
        with (
            patch("robo_cli.lifecycle.invoke_hook", return_value=[]),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_engine_preflight_fires_below_threshold(self, agent):
        ...
        should_compress_preflight() hook so engines like robo-lcm can
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("ROBO_KANBAN_TASK", "t_test_task_123")
        ...
        with (
            ...
            patch("robo_cli.kanban_db._record_task_failure",
                  mock_record_failure),
            patch("robo_cli.kanban_db.connect", mock_connect),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_no_kanban_block_when_not_in_kanban_mode(self, agent, monkeypatch):
        """The exhaustion bridge must NOT fire when ROBO_KANBAN_TASK
...
class TestRetryExhaustion:
    ...
    def test_content_filter_refusal_surfaced_not_retried(self, agent):
        ...
        portal (Ignitee Now Portal fronting Anthropic) returns ``message.refusal``
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "Robo Agent" in agent._cached_system_prompt

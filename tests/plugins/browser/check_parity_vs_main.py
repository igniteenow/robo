...
Run from the PR worktree:
    ...
    cd ~/.robo/robo-engineer/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.robo/robo-engineer``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.robo/robo-engineer
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a robo-engineer checkout"
)
...
# Isolated ROBO_HOME for the config write.

...
def _load_plugin_router():
    ...
    spec = importlib.util.spec_from_file_location(
        "robo_dashboard_plugin_kanban_test", plugin_file,
    )
...
def test_dashboard_markdown_html_is_sanitized_before_render():
    ...
    if not bundle.is_file():
        pytest.skip("kanban dashboard frontend bundle is not shipped in this repo")
    js = bundle.read_text()
...
def test_add_comment(client):
    ...
    r = client.post(
        ...
        json={"body": "how's progress?", "author": "robodev"},
    )
    assert r.status_code == 200
    ...
    assert comments[0]["author"] == "robodev"
...
def test_ws_events_rejects_when_token_required(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "robo_cli.web_server", stub)
    monkeypatch.setattr(robo_cli, "web_server", stub, raising=False)
...
# tests/robo_cli/test_kanban_db.py. The OUTER fallback here is not, which

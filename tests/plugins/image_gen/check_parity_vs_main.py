...
# worktree), ``MAIN_DIR`` falls back to a sibling ``robo-engineer-main``
...
def _resolve_main_dir() -> Path:
    ...
    if (candidate / "tools" / "image_generation_tool.py").exists() and candidate != REPO_ROOT:
        return candidate
    sibling = REPO_ROOT.parent / "robo-engineer-main"
...
assert (PR_DIR / "tools" / "image_generation_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a robo-engineer checkout"
)
...
# Isolated ROBO_HOME so the config write is hermetic.
...
SCENARIOS: list[tuple[str, str, dict[str, str]]] = [
    ...
    (
        ...
        "",
        {
            "TOOL_GATEWAY_DOMAIN": "igniteenow.com",
            "TOOL_GATEWAY_USER_TOKEN": "igniteenow-token",
        },
    ),
]
...
def main() -> int:
    ...
    if MAIN_DIR == PR_DIR:
        print(
            ...
            "      Set up a sibling 'robo-engineer-main' checkout pinned to "

...
# The Chronos cron provider is an optional external plugin that is not part of
# this tree; skip the module rather than fail collection when it is absent.
pytest.importorskip(
    "plugins.cron_providers.chronos",
    reason="chronos cron provider plugin not installed",
)
...
AUD = "agent:inst-123"
ISS = "https://portal.igniteenow.com"
...
def test_jwks_url_path_resolves_key(rsa_keys, monkeypatch):
    ...
    class FakeJWKClient:
        def __init__(self, url, **kwargs):
            assert url == "https://portal.igniteenow.com/.well-known/jwks.json"
    ...
    claims = verify_nas_fire_token(
        ...
        jwks_or_key="https://portal.igniteenow.com/.well-known/jwks.json",
        issuer=ISS,
    )
...
def test_jwks_client_sends_explicit_http_headers(monkeypatch):
    ...
    the dashboard-auth igniteenow/self_hosted providers)."""
    from plugins.cron_providers.chronos import verify as verify_mod
    ...
    url = "https://portal.igniteenow.com/.well-known/jwks.json"

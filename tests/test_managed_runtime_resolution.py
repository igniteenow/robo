"""Guard: Robo-owned subprocesses must not resolve managed runtimes by bare PATH.

Robo installs runtimes for itself — ``uv`` at ``$ROBO_HOME/bin/uv``, Node at
``$ROBO_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Robo's own
...
* when a system copy also exists, the one Robo does not own wins — which is
...
``iter_robo_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Robo provisions into ROBO_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Robo-owned subprocess code: plugins ship their own
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Robo-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$ROBO_HOME/bin (uv) and $ROBO_HOME/node are not on an "
        ...
        "  node/npm -> robo_constants.find_node_executable()\n"
        "  PATH env -> robo_constants.iter_robo_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(robo_constants, helper))


def test_managed_uv_helpers_exist():
    from robo_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv

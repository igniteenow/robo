...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("robo.gateway.up"), _M("robo.gateway.active_agents"),
        _M("robo.gateway.busy"), _M("robo.gateway.drainable"),
        _M("robo.gateway.restart_requested"),
        _M("robo.platform.up"), _M("robo.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("robo.cron.scheduler.heartbeat_age_seconds"),
        _M("robo.cron.scheduler.last_success_age_seconds"),
        _M("robo.cron.scheduler.catch_up_occurrences"),
        _M("robo.cron.jobs.enabled"), _M("robo.cron.jobs.running"),
        _M("robo.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Robo Agent-owned Relay transport health" in text

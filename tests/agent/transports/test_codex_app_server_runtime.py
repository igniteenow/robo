...
class TestSpawnEnvIsolation:
    ...
    Other agent runtimes have hit this footgun — they were
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Robo passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("ROBO_HOME", "/users/alice/.robo/profiles/backend-worker")
        monkeypatch.setenv("ROBO_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "ROBO_KANBAN_DB",
            "/users/alice/.robo/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.robo/kanban/boards/smoke"]'
            in cmd
        )
...
class TestSpawnEnvSecretStripping:
    """codex app-server routes its spawn env through robo_subprocess_env(
    ...
    credentials to authenticate, but it must NOT inherit Tier-1 Robo secrets
    ...
    def test_tier1_and_internal_secrets_stripped_from_spawn_env(self, monkeypatch):
        ...
        for var in (
            ...
            "ROBO_DASHBOARD_SESSION_TOKEN", "AUXILIARY_VISION_API_KEY",

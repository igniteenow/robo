...
@pytest.fixture
def robo_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.robo/`` with a config.yaml referencing one external dir."""
    home = tmp_path / ".robo"
    home.mkdir()
...
def test_cache_invalidates_on_mtime_change(robo_home_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = robo_home_with_config
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different ROBO_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".robo"
    ...
    monkeypatch.setenv("ROBO_HOME", str(home_a))
    assert get_external_skills_dirs() == [ext_a.resolve()]

    monkeypatch.setenv("ROBO_HOME", str(home_b))

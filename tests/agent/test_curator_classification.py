...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"robo-engineer-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "robo-engineer-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "robo-engineer-dev"
...
# `robo curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `robo curator pin <umbrella>` in the rename block so

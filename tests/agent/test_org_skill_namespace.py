...
class TestOrgPullIsWiredIn:
    ...
    def test_sync_pull_command_refreshes_org_mirror(self):
        ...
        assert "maybe_pull_org_skills" in main_src, (
            "`robo sync pull` must also refresh the org mirror."
        )
    ...
    def test_no_internal_jargon_in_user_facing_strings(self):
        ...
        targets = [
            root / "robo_cli" / "subcommands" / "sync.py",
            root / "robo_cli" / "subcommands" / "skills.py",
        ]
        banned = re.compile(
...
class TestSkillSyncIsOneCommand:
    """Every Skill Sync verb lives under `robo sync` for launch.
    ...
    belong elsewhere. `propose` in particular used to sit under `robo
    ...
    def test_propose_is_a_sync_subcommand(self):
        sync_src = self._src("robo_cli", "subcommands", "sync.py")
        assert '"propose"' in sync_src, (
            "`propose` must be a `robo sync` subcommand."
        )

    def test_propose_is_not_under_skills(self):
        skills_src = self._src("robo_cli", "subcommands", "skills.py")
        assert '"propose"' not in skills_src, (
            "`propose` must NOT remain under `robo skills` — Skill Sync is "
            "one command for launch."
        )

    def test_sync_usage_lists_propose(self):
        main_src = self._src("robo_cli", "main.py")
        usage_start = main_src.index("usage: robo sync ")
        usage_block = main_src[usage_start : usage_start + 1400]
        assert "propose" in usage_block, (
            "`robo sync` usage must list the propose verb."
        )
...
class TestLocalEditsSurviveOrgUpdates:
    ...
    def test_auto_propose_defaults_off(self, monkeypatch):
        ...
        monkeypatch.delenv("ROBO_SYNC_ORG_AUTO_PROPOSE", raising=False)
        monkeypatch.setattr(
            "robo_cli.config.load_config", lambda: {}, raising=False
        )
    ...
    def test_auto_propose_can_be_enabled_by_env(self, monkeypatch):
        ...
        monkeypatch.setenv("ROBO_SYNC_ORG_AUTO_PROPOSE", "1")

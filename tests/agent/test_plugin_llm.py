...
""",
        ...
        from robo_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        robo_home = tmp_path / ".robo"
        robo_home.mkdir()
        (robo_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("ROBO_HOME", str(robo_home))
        from robo_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from robo_cli.plugins import PluginContext, PluginManifest, PluginManager

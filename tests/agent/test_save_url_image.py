...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ROBO_HOME under tmp_path."""
    monkeypatch.setenv("ROBO_HOME", str(tmp_path / ".robo"))
    (tmp_path / ".robo").mkdir()

    # Force the constants/image cache helpers to re-read ROBO_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("robo_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_robo_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under ROBO_HOME — gateway cleanup

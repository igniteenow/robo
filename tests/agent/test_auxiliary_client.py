...
def _clean_env(monkeypatch):
    ...
    for key in (
        ...
        "OPENAI_MODEL", "LLM_MODEL", "IGNITEENOW_INFERENCE_BASE_URL",
...
class TestResolveTaskProviderModel:
    ...
    def test_explicit_provider_moa_unwraps_to_aggregator(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "robo_cli.moa_config.resolve_moa_preset",
            lambda cfg, name: preset,
        )
        monkeypatch.setattr("robo_cli.config.load_config", lambda: {"moa": {}})
        monkeypatch.setattr("robo_cli.config.load_config_readonly", lambda: {"moa": {}})
...
class TestMoaAggregatorSharedResolution:
    ...
    Real-config tests: write an actual config.yaml under a temp ROBO_HOME
                    ...
                    "moa": {
                        ...
                        "presets": {
                            ...
                            "igniteenow-mix": {
                                "enabled": True,
                                "reference_models": [
                                    {"provider": "igniteenow", "model": "hermes-4-70b"}
                                ],
                                "aggregator": {
                                    "provider": "igniteenow",
                                    "model": "hermes-4-405b",
...
class TestBuildCallKwargsMaxTokens:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("igniteenow", "hermes-4", "https://inference-api.igniteenow.com/v1", "max_tokens"),
        ],
    )
...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(self, tmp_path, monkeypatch):
        ...
        ``robo auth status`` already reports these as logged in; compression
    ...
    def test_pool_backed_credentials_honor_base_url_env_override(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ROBO_HOME", str(robo_home))
        monkeypatch.setenv("ROBO_XAI_BASE_URL", "https://example.x.ai/v1/")
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``robo model``, no per-task override is
    ...
    fallback chain (OpenRouter / Ignitee Now / etc.), so aux tasks billed
...
class TestOpenRouterPaidLaneGuard:
    ...
    def test_free_only_skips_paid_default_model(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("robo_cli.config.load_config_readonly", return_value={"auxiliary": {"free_only": True}}), \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            client, model = _try_openrouter()
        assert client is None
        assert model is None
    ...
    def test_free_only_allows_free_model(self, monkeypatch):
        ...
        monkeypatch.setenv("OPENROUTER_API_KEY", "or-key")
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("robo_cli.config.load_config_readonly",
...
class TestGetTextAuxiliaryClient:
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            ...
            patch("robo_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from agent.auxiliary_client import _build_codex_client
...
class TestIsModelNotFoundError:
    ...
    def test_igniteenow_openrouter_catalog_404(self):
        """The exact incident error: a Portal-recommended model dropped from
        the Ignitee Now → OpenRouter catalog."""
        exc = Exception(
...
class TestAuxiliaryFallbackLayering:
    ...
    def test_fallback_entry_openai_codex_uses_oauth_pool_without_inline_key(self):
        """Configured Codex fallback resolves through Robo auth / credential pool."""
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    from robo_cli.auth import ProviderConfig
    ...
    monkeypatch.setattr("robo_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
...
class TestAuxClientNoSdkRetries:
    ...
    disabled so Robo owns the retry/timeout budget (issue #54465). The SDK
...
class TestAuxiliaryTaskExtraBody:
    def test_sync_call_merges_task_extra_body_from_config(self):
        ...
        with patch("robo_cli.config.load_config", return_value=config), patch("robo_cli.config.load_config_readonly", return_value=config), patch(
            "agent.auxiliary_client._get_cached_client",
    ...
    def test_moa_tasks_reject_task_level_reasoning_effort(self, moa_task, caplog):
        ...
        with patch("robo_cli.config.load_config", return_value=config), patch("robo_cli.config.load_config_readonly", return_value=config), \
             caplog.at_level(logging.WARNING, logger="agent.auxiliary_client"):
...
class TestAnthropicAuxiliaryReasoningTranslation:
    """Native Anthropic aux adapters must receive normalized Robo reasoning.

    MoA slot reasoning is carried through call_llm as a Robo
...
class TestNvidiaBillingHeaders:
    ...
    def test_resolve_provider_client_cloud_adds_billing_origin_header(self, monkeypatch):
        ...
        assert headers["X-BILLING-INVOKE-ORIGIN"] == "RoboAgent"
...
class TestAuxUnhealthyCache:
    ...
    def test_payment_fallback_skips_unhealthy(self):
        ...
        # unhealthy. The chain should still find api-key.
        ...
        with patch("agent.auxiliary_client._read_main_provider", return_value="openrouter"), \
             ...
             patch("agent.auxiliary_client._resolve_api_key_provider", return_value=(api_key_client, "k-model")):
            client, model, label = _try_payment_fallback("openrouter", task="compression")
    ...
    def test_call_llm_marks_provider_unhealthy_on_402(self, monkeypatch):
        ...
        igniteenow_client = MagicMock()
        igniteenow_resp = MagicMock()
        igniteenow_resp.choices = [MagicMock(message=MagicMock(content="ok"))]
        igniteenow_client.chat.completions.create.return_value = igniteenow_resp
        ...
        with patch("agent.auxiliary_client._get_cached_client",
             ...
             patch("agent.auxiliary_client._try_payment_fallback",
                    return_value=(igniteenow_client, "n-model", "igniteenow")), \
...
class TestAuxiliaryMaxTokensParam:
    ...
    def test_openrouter_api_key_present_keeps_max_tokens_without_model_hint(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._current_custom_base_url",
...
class TestCustomEndpointApiKeyInheritance:
    ...
    def test_inherits_main_api_key_when_aux_key_empty(self, monkeypatch):
        ...
        with patch("robo_cli.config.load_config", return_value=fake_config), patch("robo_cli.config.load_config_readonly", return_value=fake_config), \
    ...
    def test_runtime_override_key_is_used(self, monkeypatch):
        ...
        with patch.object(ac, "_RUNTIME_MAIN_API_KEY", "sk-runtime-key"), \
             ...
             patch("robo_cli.config.load_config", return_value={"model": {}}), patch("robo_cli.config.load_config_readonly", return_value={"model": {}}), \
             patch.object(ac, "_create_openai_client", side_effect=_capture_create):
            client, model = resolve_provider_client(
                "custom",
                model="test-model",

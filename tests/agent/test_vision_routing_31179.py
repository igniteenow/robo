...
def isolated_home(monkeypatch):
    """Temp ROBO_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="robo_test_31179_")
    robo_home = os.path.join(test_home, ".robo")
    os.makedirs(robo_home)
...
def _fresh_modules():
    """Drop cached robo modules so each test reloads against current env."""
    _drop_reload_targets()

...
from robo_cli.igniteenow_subscription import IgniteeNowFeatureState, IgniteeNowSubscriptionFeatures


@pytest.fixture(autouse=True)
def _reset_truncation_queue():
    """Module-wide: reset the truncation-warning queue before AND after every
    test in this file, not just the one class that first needed it.

    _record_truncation_warning()/drain_truncation_warnings() is a shared,
    process-global queue (agent/prompt_builder.py). Several classes in this
    file trigger it directly (_truncate_content) or indirectly
    (build_context_files_prompt on an oversized fixture file) — a single
    class-scoped fixture only protected its own class, so a warning left
    behind by, say, TestDynamicContextFileCap or TestBuildContextFilesPrompt
    still leaked into whatever ran next in the same pytest process and
    happened to call drain_truncation_warnings() (build_system_prompt does,
    on every call) — a real, observed cross-file failure, not hypothetical.
    """
    drain_truncation_warnings()
    yield
    drain_truncation_warnings()
...
class TestTruncateContent:
    @pytest.fixture(autouse=True)
    def _mock_config_for_truncation_tests(self, monkeypatch):
        def default_load_config():
            return {}

        monkeypatch.setattr("robo_cli.config.load_config", default_load_config)
        monkeypatch.setattr("robo_cli.config.load_config_readonly", default_load_config)
    ...
    def test_truncation_warning_points_to_config_key(self, monkeypatch):
        ...
        monkeypatch.setattr("robo_cli.config.load_config", fake_load_config)
        monkeypatch.setattr("robo_cli.config.load_config_readonly", fake_load_config)
...
class TestDynamicContextFileCap:
    ...
    def _no_explicit_config(self, monkeypatch):
        ...
        monkeypatch.setattr("robo_cli.config.load_config", lambda: {})
        monkeypatch.setattr("robo_cli.config.load_config_readonly", lambda: {})
...
class TestBuildIgniteeNowSubscriptionPrompt:
    def test_feature_flag_off_returns_empty_prompt(self, monkeypatch):
        monkeypatch.setattr("tools.tool_backend_helpers.managed_igniteenow_tools_enabled", lambda: False)

        prompt = build_igniteenow_subscription_prompt({"web_search"})
...
class TestBuildContextFilesPrompt:
    def test_empty_dir_loads_seeded_global_soul(self, tmp_path):
        ...
        assert "You are Robo, an autonomous engineering robot" in result
    ...
    def test_empty_soul_md_adds_nothing(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ROBO_HOME", str(tmp_path / "robo_home"))
        robo_home = tmp_path / "robo_home"
        robo_home.mkdir()
        (robo_home / "SOUL.md").write_text("\n\n", encoding="utf-8")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert result == ""
...
# =========================================================================
# .robo.md helper functions
# =========================================================================


class TestFindRoboMd:
    def test_finds_in_cwd(self, tmp_path):
        (tmp_path / ".robo.md").write_text("rules")
        assert _find_robo_md(tmp_path) == tmp_path / ".robo.md"



    def test_walks_to_git_root(self, tmp_path):
        (tmp_path / ".git").mkdir()
        (tmp_path / ".robo.md").write_text("root rules")
        sub = tmp_path / "a" / "b"
        sub.mkdir(parents=True)
        assert _find_robo_md(sub) == tmp_path / ".robo.md"
    ...
    def test_no_git_root_checks_cwd_only(self, tmp_path):
        ...
        the way to / and pick up a .robo.md planted in /tmp, /home, or /
...
class TestEnvironmentHints:
    ...
    def test_environment_hint_from_env_var_is_appended(self, monkeypatch):
        """ROBO_ENVIRONMENT_HINT lets an embedder describe the runtime env."""
        import agent.prompt_builder as _pb
        monkeypatch.setattr(_pb, "is_wsl", lambda: False)
        monkeypatch.delenv("TERMINAL_ENV", raising=False)
        monkeypatch.setenv("ROBO_ENVIRONMENT_HINT", "Running inside an OpenShell sandbox.")
        _pb._clear_backend_probe_cache()
        result = _pb.build_environment_hints()

...
path via ``robo chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp ROBO_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point ROBO_HOME at a temp dir and turn moa.save_traces on."""
    robo_home = tmp_path / ".robo"
    robo_home.mkdir()
    monkeypatch.setenv("ROBO_HOME", str(robo_home))

    # save_moa_turn reads config via robo_cli.config.load_config; stub it to

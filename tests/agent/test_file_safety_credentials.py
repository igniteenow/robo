"""Tests for ROBO_HOME credential-file read blocking in file_safety.

Regression for —
``read_file`` was previously only sandboxed against ``ROBO_HOME`` itself,
...
for the credential stores while leaving arbitrary ``ROBO_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_robo_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "robo_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_robo_home_path", lambda: home)
    return home
...
def test_arbitrary_robo_home_file_not_blocked(fake_home):
    """Non-credential files inside ROBO_HOME stay readable."""
...
def test_identically_named_robo_files_outside_home_not_blocked(
    fake_home, tmp_path
):
    """Robo-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside ROBO_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside ROBO_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside ROBO_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, ROBO_HOME = <root>/profiles/<name>, but
    ...
    monkeypatch.setattr(fs, "_robo_home_path", lambda: profile)

...
class TestPrologueStamping:
    ...
    def test_no_stamp_without_injections(self):
        agent = _FakeAgent()
        with patch("robo_cli.plugins.invoke_hook", return_value=[]):
            ctx = _build(agent)
...
@pytest.fixture()
def wire_env():
    """Mock provider + isolated ROBO_HOME + a shared SessionDB.
    ...
    test_home = tempfile.mkdtemp(prefix="robo_api_content_")
    os.makedirs(os.path.join(test_home, ".robo"))
    prev_home = os.environ.get("ROBO_HOME")

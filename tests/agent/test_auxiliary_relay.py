...
from robo_cli.observability.shared_metrics_contract import MODEL_ROUTE_METRIC
from robo_cli.observability.shared_metrics_subscriber import SharedMetricsSubscriber


@pytest.fixture()
def relay_turn(tmp_path, monkeypatch):
    monkeypatch.setenv("ROBO_HOME", str(tmp_path / "profile"))
...
def test_auxiliary_provider_fallback_records_one_terminal_model_route(
    ...
    assert snapshot[0]["resource"]["robo_version"] == "test-version"

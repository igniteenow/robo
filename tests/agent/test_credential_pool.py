...
# Ignitee Now Portal was removed as a provider option (see
# robo_cli/igniteenow_account.py), so the credential pool no longer seeds
# ``igniteenow`` entries from the auth.json singleton. These tests apply only
# when a profile for it is registered again (e.g. via an external plugin).


def _igniteenow_provider_available() -> bool:
    try:
        from providers import get_provider_profile

        return get_provider_profile("igniteenow") is not None
    except Exception:
        return False


_requires_igniteenow_provider = pytest.mark.skipif(
    not _igniteenow_provider_available(),
    reason="no 'igniteenow' provider profile registered (portal provider removed)",
)
...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (robo_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_explicit_reset_timestamp_overrides_default_429_ttl(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "robo_cli.auth._import_codex_cli_tokens",
        lambda: None,
    )
    _write_auth_store(
        tmp_path,
...
def test_unmatched_api_key_hint_rotates_without_benching_innocent_key(tmp_path, monkeypatch):
    ...
    auth_payload = json.loads((tmp_path / "robo" / "auth.json").read_text())
...
def test_load_pool_does_not_persist_env_seeded_secret_value(tmp_path, monkeypatch):
    ...
    auth_text = (tmp_path / "robo" / "auth.json").read_text()
...
def test_load_pool_collapses_duplicate_env_rows_to_active_key(tmp_path, monkeypatch):
    ...
    persisted = json.loads((tmp_path / "robo" / "auth.json").read_text())
...
def test_load_pool_persists_bitwarden_origin_metadata_without_secret(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENROUTER_API_KEY", sentinel)
    monkeypatch.setattr(
        "robo_cli.env_loader.get_secret_source",
...
def test_write_credential_pool_sanitizes_borrowed_payload_at_disk_boundary(tmp_path, monkeypatch):
    ...
    from robo_cli.auth import write_credential_pool
    ...
    write_credential_pool("openrouter", [
        {
            ...
            "auth_type": "api_key",
            "priority": 0,
            "source": "systemd://robo/openrouter",
            "access_token": sentinel,
            "refresh_token": f"refresh-{sentinel}",
            "agent_key": f"agent-{sentinel}",
            "api_key": f"extra-{sentinel}",
    ...
    assert borrowed["source"] == "systemd://robo/openrouter"
...
def test_write_credential_pool_preserves_known_provider_owned_oauth_state(tmp_path, monkeypatch):
    ...
    write_credential_pool("igniteenow", [
        {
            "id": "igniteenow-device",
            "label": "device-code",
            "auth_type": "oauth",
            "priority": 0,
            "source": "device_code",
    ...
    persisted = json.loads((tmp_path / "robo" / "auth.json").read_text())["credential_pool"]["igniteenow"][0]
...
def test_load_pool_prefers_dotenv_over_stale_os_environ(tmp_path, monkeypatch):
    ...
    from a parent shell) must NOT shadow the fresh key in ~/.robo/.env when
    ...
    # User edited ~/.robo/.env with the fresh key
    (robo_home / ".env").write_text(
...
def test_load_pool_falls_back_to_os_environ_when_dotenv_empty(tmp_path, monkeypatch):
    """When ~/.robo/.env does not define OPENROUTER_API_KEY (typical Docker /
    ...
    (robo_home / ".env").write_text("SOME_OTHER_VAR=unrelated\n")
...
@_requires_igniteenow_provider
def test_load_pool_mirrors_igniteenow_invoke_jwt_agent_key_runtime_api_key(tmp_path, monkeypatch):
    monkeypatch.setenv("ROBO_HOME", str(tmp_path / "robo"))
    ...
    auth_payload = json.loads((tmp_path / "robo" / "auth.json").read_text())
    pool_entry = auth_payload["credential_pool"]["igniteenow"][0]
    assert pool_entry["agent_key"] == token
    assert pool_entry["agent_key_expires_at"] == expires_at


@_requires_igniteenow_provider
def test_igniteenow_runtime_api_key_rejects_opaque_agent_key():
    from agent.credential_pool import PooledCredential

    entry = PooledCredential(
        provider="igniteenow",
        id="igniteenow-opaque",
        label="opaque",
        auth_type="oauth",
        priority=0,
        source="device_code",
...
def test_load_pool_api_key_path_skips_oauth_autodiscovery(tmp_path, monkeypatch):
    ...
    When the user picks "Anthropic API key" at `robo setup`,
    ...
    + claude-cli user-agent).  Autodiscovered Claude Code / Robo PKCE
    ...
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    monkeypatch.setattr("robo_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    ...
    monkeypatch.setattr("agent.anthropic_adapter.read_robo_oauth_credentials", _fake_pkce)
...
def test_load_pool_api_key_path_prunes_stale_oauth_entries(tmp_path, monkeypatch):
    ...
    `robo_pkce` into auth.json) and later switches to the API key at
    `robo setup` would still have those OAuth entries dormant on disk.
    ...
    monkeypatch.setattr("robo_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    monkeypatch.setattr("agent.anthropic_adapter.read_robo_oauth_credentials", lambda: None)
    monkeypatch.setattr("agent.anthropic_adapter.read_claude_code_credentials", lambda: None)
...
def test_load_pool_oauth_path_still_autodiscovers(tmp_path, monkeypatch):
    ...
    OAuth path at `robo setup`.  When ANTHROPIC_TOKEN is set (and
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_robo_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_custom_endpoint_pool_seeds_from_config(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "robo" / "config.yaml"
    import yaml
...
def test_load_pool_seeds_copilot_via_gh_auth_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "robo_cli.copilot_auth.resolve_copilot_token",
...
def test_load_pool_skips_exchange_for_suppressed_copilot(tmp_path, monkeypatch):
    ...
    removed with ``robo auth remove copilot gh_cli``.  The gate must run
    ...
    monkeypatch.setattr(
        "robo_cli.copilot_auth.get_copilot_api_token",
        _boom,
    )
...
def test_load_pool_skips_resolve_when_all_copilot_sources_suppressed(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ROBO_HOME", str(tmp_path / "robo"))
    from robo_cli.copilot_auth import COPILOT_ENV_VARS
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    monkeypatch.setattr("robo_cli.copilot_auth.resolve_copilot_token", _boom)
...
def test_load_pool_seeds_qwen_oauth_via_cli_tokens(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "robo_cli.auth.resolve_qwen_runtime_credentials",
        lambda **kw: {
            "provider": "qwen-oauth",
...
@_requires_igniteenow_provider
def test_igniteenow_seed_from_singletons_preserves_obtained_at_timestamps(tmp_path, monkeypatch):
    """Regression test for #15099 secondary issue.

    When ``_seed_from_singletons`` materialises a device_code pool entry from
    the ``providers.igniteenow`` singleton, it must carry the mint/refresh
        ...
        tmp_path,
        {
            ...
            "providers": {
                "igniteenow": {
                    ...
                    "client_id": "robo-cli",
                    "portal_base_url": "https://portal.igniteenow.com",
                    "inference_base_url": "https://inference.igniteenow.com/v1",
                    "token_type": "Bearer",
                    "scope": "openid profile",
                    "obtained_at": "2026-04-24T10:00:00+00:00",
                    "expires_at": "2026-04-24T11:00:00+00:00",
                    "expires_in": 3600,
                    "agent_key": "sk-igniteenow-AAAA",
                    "agent_key_id": "ak_123",
    ...
    assert e.agent_key == "sk-igniteenow-AAAA"
...
# ── PR #10160 salvage: Ignitee Now OAuth cross-process sync tests ─────────────────
...
def test_persist_preserves_concurrent_disk_only_entry(tmp_path, monkeypatch):
    ...
    final = json.loads((tmp_path / "robo" / "auth.json").read_text())
...
def _make_anthropic_claude_code_pool(tmp_path, monkeypatch, *, access_token, refresh_token, expires_at_ms=9_999_999_999_000):
    ...
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    _write_auth_store(tmp_path, {"version": 1, "credential_pool": {}})
    monkeypatch.setattr("robo_cli.auth.is_provider_explicitly_configured", lambda pid: pid == "anthropic")
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_robo_oauth_credentials",
        lambda: None,
    )

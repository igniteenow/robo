...
class TestAuthFailureAborts:
    ...
    Real incident: a igniteenow token pointed at a stale staging inference URL
    ...
    def test_missing_provider_api_key_preserves_original_messages(self):
        ...
        err = RuntimeError(
            ...
            "to a different provider with robo model."
        )
        with patch(
...
class TestTruncateToolCallArgsJson:
    ...
    def test_shrunken_args_remain_valid_json(self):
        ...
        original = _json.dumps({
            "path": "~/.robo/skills/shopping/browser-setup-notes.md",
        ...
        assert parsed["path"] == "~/.robo/skills/shopping/browser-setup-notes.md"
...
class TestPreLlmFeasibilityCheck:
    ...
    def test_boundary_accounting_skip_does_not_feed_fallback_streak(self, compressor):
        """The interaction the maintainer's sweeper review flagged on #68334: the

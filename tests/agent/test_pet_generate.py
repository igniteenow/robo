...
pytestmark = pytest.mark.skipif(
    os.environ.get("ROBO_RUN_SLOW_PET_TESTS") != "1",
    reason=(
        "pet generation image-processing suite is opt-in; run with "
        "ROBO_RUN_SLOW_PET_TESTS=1 scripts/run_tests.sh tests/agent/test_pet_generate.py"
    ),
)
...
def test_list_sprite_providers_marks_default(monkeypatch):
    ...
    registry = {"openai": _FakeImgProvider("openai"), "igniteenow": _FakeImgProvider("igniteenow")}
    ...
    assert names == {"openai", "igniteenow"}
    ...
    # Listed in preference order: Ignitee Now Portal before OpenAI.
    assert [p["name"] for p in listed] == ["igniteenow", "openai"]

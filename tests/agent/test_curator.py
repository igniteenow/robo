...
def curator_env(tmp_path, monkeypatch):
    """Isolated ROBO_HOME + freshly reloaded curator + skill_usage modules."""
    home = tmp_path / ".robo"
    ...
    # save_state() resolves the state path from ROBO_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.robo once monkeypatch
    ...
    # thread here, while ROBO_HOME is still pinned to this test's tmp home
...
def test_run_review_synchroigniteenow_invokes_llm_stub(curator_env, monkeypatch):
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
    _write_skill(skills_dir, "a")
...
def test_cli_pin_refuses_bundled_skill(curator_env, capsys):
    from robo_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `robo model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from robo_cli.main import _AUX_TASKS
    from robo_cli.web_server import _AUX_TASK_SLOTS
    ...
    # 2. robo_cli/main.py _AUX_TASKS — CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. robo_cli/web_server.py _AUX_TASK_SLOTS — REST API allowlist

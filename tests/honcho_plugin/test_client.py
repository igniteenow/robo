...
from robo_cli.profiles import _get_default_robo_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "robo"
        assert config.workspace_id == "robo"
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="robo-engineer"
        ):
            result = config.resolve_session_name("/home/user/robo-engineer/subdir")
        assert result == "robo-engineer"


class TestResolveConfigPath:
    def test_prefers_robo_home_when_exists(self, tmp_path):
        robo_home = tmp_path / "robo"
        robo_home.mkdir()
        local_cfg = robo_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: ROBO_HOME points at ~/.robo/profiles/<name>, so
        # _get_default_robo_home() must resolve back to ~/.robo — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".robo"
        ...
        assert _get_default_robo_home() == default_home
        assert result == default_cfg


class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "robo_coder"
        assert profile_host_key("default") == "robo"


    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"ROBO_HONCHO_HOST": "robo.coder"}):
            assert resolve_active_host() == "robo.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            os.environ.pop("ROBO_HONCHO_HOST", None)
            # Temporarily remove robo_cli.profiles to simulate import failure
            saved = sys.modules.get("robo_cli.profiles")
            sys.modules["robo_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "robo"
            finally:
                if saved is not None:
                    sys.modules["robo_cli.profiles"] = saved
                else:
                    sys.modules.pop("robo_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="robo_coder")
        assert config.host == "robo_coder"
        assert config.workspace_id == "robo"  # shared workspace
        assert config.ai_peer == "robo_coder"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"robo": {"enabled": True, "aiPeer": "robo"}},
        }))
...
class TestGetHonchoClient:
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_robo_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ROBO_MANAGED_DIR", str(managed_dir))
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_local_base_url_with_v3_suffix_stripped(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("robo_cli.config.load_config", return_value={}):
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=True), \
             patch("robo_cli.profiles.get_active_profile_name", return_value="default"), \

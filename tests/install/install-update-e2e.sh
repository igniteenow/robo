...
#
# Installs a real, earlier Robo the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `robo`.
#
...
# Keep sandbox state out of the default .robo-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".robo-sandbox-e2e-$ROUTE"
export ROBO_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/robo/.robo/robo-engineer"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/robo-engineer.git"
...
LOG_DIR="${ROBO_E2E_LOG_DIR:-$(mktemp -d -t robo-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED robo accept FLAG on `robo update`?
#
...
# that silently rots. `robo update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "robo update --help 2>&1" | grep -qF -- "$flag"
}
...
require_robo_works() {
  local when="$1" out
  out="$(in_sandbox "robo --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "robo --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "robo runs $when"
}

# ── install the earlier Robo ─────────────────────────────────────────────
...
require_robo_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: robo update'
    ...
    # robo which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="robo update --yes"
    else
      update_cmd="robo update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "robo update failed ($update_cmd)"
    fi
    require_landed_on_target 'robo update'
    require_robo_works 'after robo update'
    ;;
  installer)
    ...
    require_robo_works 'after installer re-run'
    ;;
esac

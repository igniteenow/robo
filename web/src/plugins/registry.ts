 ...
 *
 * Plugins call window.__ROBO_PLUGINS__.register(name, Component)
...
import { Card, CardHeader, CardTitle, CardContent } from "@igniteenow/ui/ui/components/card";
import { Input } from "@igniteenow/ui/ui/components/input";
import { Label } from "@igniteenow/ui/ui/components/label";
import { Separator } from "@igniteenow/ui/ui/components/separator";
import { Tabs, TabsList, TabsTrigger } from "@igniteenow/ui/ui/components/tabs";
 ...
 * Exposed at runtime as ``window.__ROBO_PLUGIN_SDK__.sdkVersion`` so a
...
export function exposePluginSDK() {
  window.__ROBO_PLUGINS__ = {
    register: registerPlugin,
    registerSlot,
  };

  window.__ROBO_PLUGIN_SDK__ = {
    ...
    // window.__ROBO_SESSION_TOKEN__ directly.
    authedFetch,
    ...
    // UI components — Ignitee Now DS where available, shadcn/ui primitives elsewhere.
    components: {
      Card,
      CardHeader,
      CardTitle,

...
import { api, ROBO_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "robo:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${ROBO_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${ROBO_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?robo_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-robo-plugin", manifest.name);

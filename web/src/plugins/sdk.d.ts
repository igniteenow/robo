/**
 * Robo Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__ROBO_PLUGIN_SDK__`` and
 * ``window.__ROBO_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__ROBO_PLUGIN_SDK__`` because:
 *   1. The runtime object is assembled from many internal modules
 *      (``@/lib/api``, ``@igniteenow/ui``, …). Deriving the type would
 ...
 *
 * Versioning: bump ``RoboPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@robo/dashboard-plugin-sdk`` types package, or
 ...
 *     (pulls @igniteenow/ui types into the contract) or is the loose
 ...
 * of calling ``fetch`` with a hand-read ``window.__ROBO_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__ROBO_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__ROBO_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface RoboPluginSDK {
  ...
  /**
   * Shared UI primitives (Ignitee Now DS / shadcn). Typed permissively at the
...
declare global {
  interface Window {
    __ROBO_PLUGIN_SDK__?: RoboPluginSDK;
    __ROBO_PLUGINS__?: PluginRegistry;
  }
}

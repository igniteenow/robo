...
import { SelectionSwitcher } from "@igniteenow/ui/ui/components/selection-switcher";
import { Spinner } from "@igniteenow/ui/ui/components/spinner";
import { Typography } from "@igniteenow/ui/ui/components/typography/index";
import { ConfirmDialog } from "@igniteenow/ui/ui/components/confirm-dialog";
import { cn } from "@/lib/utils";
import { SidebarFooter } from "@/components/SidebarFooter";
import { SidebarStatusStrip, gatewayLine } from "@/components/SidebarStatusStrip";
import { useBelowBreakpoint } from "@igniteenow/ui/hooks/use-below-breakpoint";
...
const SIDEBAR_COLLAPSED_KEY = "robo-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="robo-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="robo-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateRobo = status?.can_update_robo === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'robo update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "robo update";
    return (
      t.status.updateRoboConfirmMessage ??
      `This will run 'robo update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateRoboConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateRobo) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateRobo,
      runningLabel: t.status.updatingRobo,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Robo gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateRoboConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateRoboConfirmTitle ?? `${t.status.updateRobo}?`}
    />
    </>
  );
}

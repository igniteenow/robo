...
interface RoboConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function RoboConsoleModal({ open, onClose }: RoboConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("robo> ");
  const inputPromptRef = useRef("robo> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "robo> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Robo Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="robo-console-title"
    >
      <div
        className={cn(
          themedBody,

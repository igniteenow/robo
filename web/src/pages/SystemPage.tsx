...
import { H2 } from "@igniteenow/ui/ui/components/typography/h2";
import { Card, CardContent } from "@igniteenow/ui/ui/components/card";
...
import { useConfirmDelete } from "@igniteenow/ui/hooks/use-confirm-delete";
import { ConfirmDialog } from "@igniteenow/ui/ui/components/confirm-dialog";
import { useModalBehavior } from "@/hooks/useModalBehavior";
import { DeleteConfirmDialog } from "@/components/DeleteConfirmDialog";
import { RoboConsoleModal } from "@/components/RoboConsoleModal";
...
export default function SystemPage() {
  ...
  // spawned `robo import` runs non-interactively (stdin is /dev/null), so
  ...
  const checkForUpdate = useCallback(
    async (force = false) => {
      if (status?.can_update_robo === false) return;
      setCheckingUpdate(true);
      try {
        const info = await api.checkRoboUpdate(force);
        setUpdateInfo(info);
        if (force) {
    ...
    [showToast, status?.can_update_robo],
  );
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_robo === false) {
      showToast(
        "Robo updates are managed outside this dashboard.",
        "success",
      );
      return;
    }
    try {
      const resp = await api.updateRobo();
      if (!resp.ok) {
        showToast(
          resp.message ??
      ...
      setActiveAction(resp.name ?? "robo-update");
  ...
  const canUpdateRobo = status?.can_update_robo !== false;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <ConfirmDialog
        open={canUpdateRobo && updateConfirmOpen}
        onCancel={() => setUpdateConfirmOpen(false)}
        onConfirm={() => void applyUpdate()}
        title="Update Robo?"
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'robo update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'robo update' (${updateInfo?.update_command ?? "robo update"}) and restart the gateway when it finishes.`
        }
        ...
        >
          <div className={cn(themedBody, "relative w-full max-w-lg border border-border bg-card shadow-2xl flex flex-col")}>
            ...
            <header className="p-5 pb-3 border-b border-border">
              <h2 className="font-display text-display text-base tracking-wider">
                New shell hook
              </h2>
            </header>
        ...
        <Card>
          <CardContent className="py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 text-sm">
              ...
              <div>
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Robo</div>
                <div className="flex items-center gap-2">
                  <span>v{stats?.robo_version}</span>
                  {canUpdateRobo &&
                    updateInfo &&
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            ...
            <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-end">
              <div className="grid min-w-0 flex-1 gap-2">
                ...
                <Input
                  ...
                  placeholder="$ROBO_HOME/backups/robo-backup.zip"
                />
              </div>
              <Button
                size="sm"
            ...
            <ConfirmDialog
              ...
              title="Restore full Robo backup?"
              description={`This will overwrite your current Robo configuration, skills, sessions, and data with the contents of ${backupImportLabel(importConfirmTarget)}. This cannot be undone.`}
              destructive
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    ...
                    returns links to send the Robo team. Pastes auto-delete
                    after 6 hours.
                  </span>
                </div>
              </div>

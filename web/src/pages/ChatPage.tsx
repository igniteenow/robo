/**
 * ChatPage — embeds `robo --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (robo_cli/web_server.py)                          .
 *          ▼                                                              .
...
const PTY_ATTACH_TOKEN_KEY = "robo.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__ROBO_SESSION_TOKEN__ &&
    !window.__ROBO_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `robo dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__ROBO_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Robo TUI has enabled xterm mouse-events
    ...
    //      payloads upload to ROBO_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to ROBO_HOME/images, then drive `/image`
    ...
    // WebSocket. In gated mode (``window.__ROBO_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets ROBO_HOME pointed at the
    ...
    // forwarding them into Robo. Keyboard input, paste, and resize still
    // behave normally.
    ...
    createPortal(
      <>
        ...
        <div
          ...
          className={cn(
            "font-display fixed top-0 right-0 z-[60] flex h-dvh max-h-dvh w-64 min-w-0 flex-col antialiased",

import { buildRoboWebSocketUrl } from "@robo/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/robo/). The Python backend
// injects ``window.__ROBO_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__ROBO_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const ROBO_BASE_PATH = readBasePath();
const BASE = ROBO_BASE_PATH;
...
declare global {
  interface Window {
    __ROBO_SESSION_TOKEN__?: string;
    __ROBO_BASE_PATH__?: string;
    ...
    __ROBO_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Robo-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__ROBO_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``robo update``, ``robo gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__ROBO_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__ROBO_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__ROBO_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``robo_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__ROBO_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Robo-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``robo_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__ROBO_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateRobo: () =>
    fetchJSON<ActionResponse>("/api/robo/update", { method: "POST" }),
  checkRoboUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/robo/update/check${force ? "?force=true" : ""}`,
    ),
 ...
 * the dashboard auth contract V1 (the access token has no email/name
...
export interface SkillHubSource {
  ...
  /** robo-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
  ...
  /** Phase 7: registered ``DashboardAuthProvider`` names (e.g. ``["self-hosted"]``).
   ...
   * updates are handled by the outer launcher instead of ``robo update``. */
  can_update_robo?: boolean;
  config_path: string;

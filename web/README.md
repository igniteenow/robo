...
Browser-based dashboard for managing Robo Agent configuration, API keys, and monitoring active sessions.
...
cd ../
python -m robo_cli.main web --no-open
...
`robo dashboard` on port 9119 serves the **built** bundle from `robo_cli/web_dist/`, not the Vite dev server — changes in `web/src/` will not appear there until you run `npm run build` and restart the dashboard (or use `web --no-open` + Vite as above).
...
This outputs to `../robo_cli/web_dist/`, which the FastAPI server serves as a static SPA. The built assets are included in the Python package via `pyproject.toml` package-data.
...
- Use the **semantic text tokens** from `@igniteenow/ui`'s `globals.css`:
...
- The dashboard preserves the Ignitee Now brand uppercase aesthetic, but it is **opt-in per element, not global**.
...
- **Do not introduce new `uppercase`** (the literal Tailwind class) in `robo-engineer/web/src`. Prefer `text-display` for new brand chrome. Legacy `uppercase` call sites (e.g. `components/ui/label.tsx`, `card.tsx`) remain until migrated.
...
> **No brand font files ship in this repo.** The `font-mondwest` / `font-expanded`
> tiers below name typefaces inherited from the upstream design system; their
> binaries were removed because Ignitee Now holds no redistribution licence for
> them, so these classes currently fall back to the system sans stack. Choose an
> Ignitee Now typeface (open-licence, or one you hold a web-font licence for),
> drop its `.woff2` files in `web/public/fonts/`, and point the `--font-*`
> variables at it. `web/public/fonts/` is git-ignored by design; remove that
> rule if your font's licence allows committing it.
...
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “Robo Agent” — mixed case, no Mondwest, no `text-display` |

...
How to build Robo well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Robo surface can also
...
- A **runtime home change** (switching the underlying `ROBO_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Robo — do not assume

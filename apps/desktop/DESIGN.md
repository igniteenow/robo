...
2. **Borderless elevation for floating panels.** Overlays float on
   `shadow-igniteenow` + a `--stroke-igniteenow` hairline, not thick framed boxes. In-panel
...
3. **One primitive per concern.** One `Button`, one set of control variants,
   ...
   fork.
4. **Tokens, not literals.** Reference CSS vars (`--ui-*`, `--shadow-igniteenow`,
...
```
shadow-igniteenow           /* downward-weighted, layered contact→ambient falloff */
border-(--stroke-igniteenow) /* currentColor hairline, theme-adaptive */
```
...
| `--stroke-igniteenow` | the overlay hairline (pairs with `shadow-igniteenow`) |
...
- **`BrandMark`** (`src/components/brand-mark.tsx`) is the brand glyph — the
  Robo face mark on a dark tile, softly rounded, identical in light/dark.
...
- [ ] Tokens (`--ui-*`, `shadow-igniteenow`, `--stroke-igniteenow`) — zero raw colors /
      one-off shadows?
...
- [ ] Overlay uses `shadow-igniteenow` + `border-(--stroke-igniteenow)`, no hard border?

...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Robo E2E'], { cwd: repo })
...
function configureRepoCwd(roboHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(roboHome, mockUrl)
  fs.appendFileSync(path.join(roboHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(roboHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.roboHome, mock.url, repo)

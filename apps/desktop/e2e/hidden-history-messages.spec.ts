...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(sandbox.roboHome, mock.url)
  fs.appendFileSync(
    path.join(sandbox.roboHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.roboHome)
  const builder = await RealSessionBuilder.start(sandbox.roboHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.roboHome, mock.url)
  fs.appendFileSync(path.join(sandbox.roboHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.roboHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))

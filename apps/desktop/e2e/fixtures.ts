/**
 * Shared E2E fixtures for the Robo desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → robo serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `robo desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `robo-e2e-${prefix}-${Math.random()}`))
  const roboHome = path.join(root, 'robo-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(roboHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(roboHome, 'config.yaml')
 ...
 */
export function writeEnvFile(roboHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(roboHome, '.env')
 ...
 */
function writeEmptyConfig(roboHome: string): void {
  const configPath = path.join(roboHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - ROBO_HOME → sandbox robo-home (isolated config/sessions)
 *  - ROBO_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - ROBO_DESKTOP_IGNORE_EXISTING=1 → don't pick up `robo` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - ROBO_DESKTOP_ROBO_ROOT → REPO_ROOT (dev checkout resolution)
 *  - ROBO_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    ROBO_HOME: sandbox.roboHome,
    ROBO_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    ROBO_DESKTOP_IGNORE_EXISTING: '1',
    ROBO_DESKTOP_ROBO_ROOT: REPO_ROOT,
    ROBO_DESKTOP_APP_NAME: `RoboE2E-${Date.now()}`,
    ...
    ROBO_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated ROBO_HOME + userData
 * @param env      - the process environment (already has ROBO_HOME etc.)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.roboHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via ROBO_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`robo serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.roboHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { ROBO_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Robo backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Robo.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Robo.app', 'Contents', 'MacOS', 'Robo')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'robo')
}
 ...
 * progress without spawning a real Robo backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * ROBO_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    ROBO_DESKTOP_BOOT_FAKE: '1',
    ROBO_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + robo-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).ROBO_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).ROBO_DESKTOP_ROBO
  delete (env as Record<string, string | undefined>).ROBO_DESKTOP_ROBO_ROOT

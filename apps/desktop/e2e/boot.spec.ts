 ...
 * packaged binary) with a real `robo serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → robo serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Robo title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Robo')
  })
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → robo serve is spawned → WS connects → config

 ...
 * AIAgent loop, using the E2E mock provider configured in `roboHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(roboHome: string) {
  ...
  static async start(roboHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(roboHome)

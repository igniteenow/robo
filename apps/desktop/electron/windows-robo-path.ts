/**
 * windows-robo-path.ts
 *
 * Pure, dependency-injected pieces of Windows `robo` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvRoboCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `robo` shim shadowed the real
 *      robo.cmd/robo.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\robo.exe (the console-script shim, written at the END
 ...
 *   3. resolveVenvRoboCommand() — unwrapWindowsVenvRoboCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `robo`) must
 * not shadow `robo.cmd`/`robo.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv robo shim, bootstrap-complete
 * marker) — gating on just the robo.exe console-script shim alone is the
...
export interface ResolveVenvRoboCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportRoboCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `robo`/`robo.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/robo(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m robo_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportRoboCli(). A
 * venv whose update died mid-`pip install` still has python.exe + robo.exe
 ...
 *
 * Returns null when `command` is not a venv robo shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvRoboCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvRoboCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^robo(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Robo at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Robo Python at ${python}`,
    command: python,

 ...
 * the three user-facing uninstall modes to the `robo uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m robo_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `robo_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'robo_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Robo.app/Contents/MacOS/Robo  → …/Robo.app
 *   Windows: …\Robo\Robo.exe                 → …\Robo  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Robo.app/Contents/MacOS/Robo → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Robo.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Robo.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Robo$/i.test(dir) || /[\\/]robo-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/robo
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import robo_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, roboHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import robo_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Robo install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "ROBO_HOME=${String(roboHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]

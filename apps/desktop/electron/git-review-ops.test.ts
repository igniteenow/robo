...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'robo-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'robo-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Robo Test'], { cwd: dir })

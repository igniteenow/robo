...
//   ROBO_WIN_SSH_HOST   ssh alias/host of the Windows box
//   ROBO_WIN_SSH_USER   remote user
//   ROBO_WIN_SSH_ROBO absolute path to the remote robo.exe under test
const liveHost = process.env.ROBO_WIN_SSH_HOST || ''
const liveUser = process.env.ROBO_WIN_SSH_USER || ''
const configuredRobo = process.env.ROBO_WIN_SSH_ROBO || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Robo-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredRobo)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredRobo.replace('robo.exe', 'python.exe')}' -m robo_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredRobo.replace('robo.exe', 'python.exe')
        const terminate = `& '${python}' -m robo_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.roboPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m robo_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m robo_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }

// Profile-delete routing logic for the `robo:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_robo_home() and
...
/**
 * Parse a `robo:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `robo:api` request away from the primary/window backend
 ...
 * ensure_robo_home() recreates the directory the delete just removed.
 */

// Backend subcommand routing for the desktop-managed Robo process.
//
// The desktop app launches its own headless backend via `robo serve` — it
...
// app hasn't updated yet, or an older `robo` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Robo profile to pin via `--profile`.
 */
 ...
 * `-m robo_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `robo_cli/subcommands/dashboard.py` source registers

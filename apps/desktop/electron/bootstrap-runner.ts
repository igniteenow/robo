 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Robo by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.robo-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(roboHome) {
  return path.join(roboHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.robo/robo-engineer. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(roboHome) {
  if (!roboHome) {
    return null
  }

  const candidate = path.join(roboHome, 'robo-engineer', 'scripts', installScriptName())
...
function cachedScriptPath(roboHome, commit) {
  return path.join(bootstrapCacheDir(roboHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const releaseBase = String(process.env.ROBO_INSTALL_BASE_URL ?? '').trim().replace(/\/$/, '')

  if (!releaseBase) {
    return Promise.reject(
      new Error('Robo setup requires the installer bundled with the matching Robo release package')
    )
  }

  const url = `${releaseBase}/${ref}/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(roboHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(roboHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, roboHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass ROBO_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          ROBO_HOME: roboHome || process.env.ROBO_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, roboHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, roboHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--robo-home', roboHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, roboHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, roboHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, roboHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(roboHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, roboHome, emit })

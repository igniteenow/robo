...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'robo-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/robo-runner-test',
    sourceRepoRoot: null,
    roboHome: '/tmp/robo-runner-test',
    logRoot: '/tmp/robo-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'robo-engineer', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'robo-engineer')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/robo-engineer',
      roboHome: '/tmp/robo'
    }),
    ['--dir', '/tmp/robo-engineer', '--robo-home', '/tmp/robo', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/robo-engineer', '--robo-home', '/tmp/robo', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/robo', '--robo-home', '/tmp/home', '--branch', 'main']
  )
})

...
const READY_RE = /^ROBO_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitRoboPath = '') {
  const explicit = psLiteral(explicitRoboPath)

  const script = [
    '$ErrorActionPreference="Stop"',
    `$explicit=${explicit}`,
    '$roboHome=$env:ROBO_HOME',
    'if(-not $roboHome){$roboHome=Join-Path $env:LOCALAPPDATA "robo"}',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command robo.exe -ErrorAction SilentlyContinue',
    'if($cmd){$candidates+=$cmd.Source}',
    '$candidates+=(Join-Path $roboHome "robo-engineer\\venv\\Scripts\\robo.exe")',
    '$candidates+=(Join-Path $HOME "robo-engineer\\.venv\\Scripts\\robo.exe")',
    '$robo=$candidates|Where-Object{Test-Path -LiteralPath $_ -PathType Leaf}|Select-Object -First 1',
    'if(-not $robo){throw "Robo is not installed on the remote Windows host."}',
    'if($explicit -and $robo -ne $explicit){throw "The configured Robo path is not an executable file."}',
    '$python=Join-Path (Split-Path $robo) "python.exe"',
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote Robo Python runtime was not found."}',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;roboHome=$roboHome;roboPath=$robo;python=$python}|ConvertTo-Json -Compress'
  ].join(';')
...
async function detectRemotePlatform(ssh, explicitRoboPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitRoboPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'robo_cli.windows_ssh_runtime', operation, ...args]
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.roboPath === 'string' &&
    typeof lock.roboHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.roboPath === runtime.roboPath &&
    lock.roboHome === runtime.roboHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertCurrent(signal)
  const runtime = await probeWindowsRemote(ssh, remoteRoboPath)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.roboPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Robo on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.roboPath = inspection.path
  const roboVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located robo at ${runtime.roboPath}`)
  ...
  try {
    spawned = await helper(
      ...
      'spawn',
      [],
      JSON.stringify({ ownershipId, spawnNonce, profile, roboPath: runtime.roboPath })
    )
  } catch (error) {
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForRobo(baseUrl, token)
    assertCurrent(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Robo SSH"', 'powershell.exe -NoLogo')

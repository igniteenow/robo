...
export interface RoboReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForRoboReady`).
   */
...
export async function waitForRoboReady(baseUrl: string, options: RoboReadyOptions): Promise<void> {
  ...
  throw new Error(`Robo backend did not become ready: ${detail}`)
}

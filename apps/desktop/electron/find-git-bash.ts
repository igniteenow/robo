 ...
 *   1. ROBO_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\robo\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect ROBO_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.ROBO_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'robo', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'robo', 'git', 'usr', 'bin', 'bash.exe'))
  }

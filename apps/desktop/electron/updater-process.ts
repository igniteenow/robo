 ...
 *
 * The Tauri installer self-copies into ROBO_HOME on *every* platform
 * (`robo-setup.exe` on Windows, `robo-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_robo_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `robo update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(roboHome, 'robo-setup.exe')
 ...
 *
 * `copy_self_to_robo_home` deliberately no-ops during `--update`
 ...
 * "Another Robo update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `robo update`, which
 ...
 * the installer is written to ROBO_HOME at install/repair time, making mtime

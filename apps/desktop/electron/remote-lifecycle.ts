 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Robo dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Robo install on the remote (login-shell probe),
 ...
 *     `ROBO_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForRobo in.
 *
 * The minted ROBO_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^ROBO_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.robo/desktop-ssh'
...
// Resolve the remote robo executable. An EXPLICIT path is honored strictly
...
async function locateRobo(ssh, remoteRoboPath) {
  ...
  if (remoteRoboPath) {
    if (await isExecutable(remoteRoboPath)) {
      return resolveLauncher(remoteRoboPath)
    }

    const err: any = new Error(
      `The Robo path you set is not an executable on the remote host: "${remoteRoboPath}". ` +
        'Check the path (it must be the full path to the `robo` binary), ' +
        'or clear it to auto-detect.'
    )

    err.kind = 'robo-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v robo')}`)).trim()
  ...
  candidates.push('~/.local/bin/robo')
  candidates.push('~/.robo/bin/robo')
  candidates.push('/usr/local/bin/robo')
  candidates.push('~/.robo/robo-engineer/venv/bin/robo')
  ...
  const err: any = new Error(
    'Robo is not installed on the remote host (could not find a `robo` executable). ' +
      'Install Robo from its release directory, or set the Robo path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<robo> --version`,
// e.g. "Robo v0.18.2 ..."), or '' on failure. Surfaces WHICH robo a
// connection uses, so a stale/unexpected install is visible.
async function probeRoboVersion(ssh, roboPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(roboPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Robo SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The ROBO_HOME the remote dashboard will use (explicit env wins, else
// ~/.robo). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteRoboHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${ROBO_HOME:-$HOME/.robo}"')).trim().split('\n').pop()

    return out || '~/.robo'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Robo runtime home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'roboPath', 'roboHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(ssh, pid, spawnNonce, roboPath = '') {
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !roboPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(roboPath)})\n` +
...
async function cleanupStale(ssh, ownershipId, lock, pidAlive = true) {
  if (pidAlive && lock && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.roboPath))) {
    try {
      const result = (
        await ssh.exec(
...
function buildSpawnCommand(roboPath, profile, opts: any = {}) {
  const robo = expandRemotePath(roboPath)
  ...
  const dashCmd = `env ROBO_DESKTOP=1 ${robo} ${profileArgs}${subCmd}`
...
async function remoteSupportsSshOwnership(ssh, roboPath) {
  const robo = expandRemotePath(roboPath)

  const out = await ssh.exec(
    `help="$(${robo} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(ssh, { roboPath, profile, token, ownershipId }) {
  if (!(await remoteSupportsSshOwnership(ssh, roboPath))) {
    const err: any = new Error(
      'The remote Robo runtime does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Robo on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    out = await ssh.exec(buildSpawnCommand(roboPath, profile, { spawnNonce, tokenFilePath, logPath }))
  } catch (error) {
    try {
      await ssh.exec(`rm -f ${expandRemotePath(tokenFilePath)}`)
    } catch {
 ...
 * opened SshConnection, forward/pickLocalPort/waitForRobo, a token-gated
...
async function connect(deps) {
  ...
  const roboPath = await locateRobo(ssh, remoteRoboPath)
  log(`located Robo runtime at ${roboPath}`)
  const roboVersion = await probeRoboVersion(ssh, roboPath)

  if (roboVersion) {
    log(`remote Robo runtime version: ${roboVersion}`)
  }

  const reuseToken = deps.reuseToken || ''
  const roboHome = await probeRemoteRoboHome(ssh)
  const lock = await readLockfile(ssh, ownershipId)

  if (lock) {
    const pidAlive = await remotePidAlive(ssh, lock.pid)
    const owned = pidAlive && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.roboPath))
    ...
    const reusable =
      ...
      lock.roboPath === roboPath &&
      lock.roboHome === roboHome
  ...
  try {
    ...
    assertNotAborted(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForRobo(baseUrl, spawnToken)
    assertNotAborted(signal)

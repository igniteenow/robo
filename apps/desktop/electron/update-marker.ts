...
export function markerPath(roboHome) {
  return path.join(roboHome, '.robo-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(roboHome)
  let raw
 ...
 *
 * The Tauri-based robo-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(roboHome, pid, { now = Date.now } = {}) {
  const file = markerPath(roboHome)
 ...
 * Robo to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(roboHome, opts)

...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(ROBO_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Robo-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.robo/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'robo-desktop-ssh')
  }

  return path.join(os.homedir(), '.robo', 'desktop-ssh')
}

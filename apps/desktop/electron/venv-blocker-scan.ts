...
const SCAN_MODULE = 'robo_cli._scan_venv_blockers'
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Robo process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the Robo installation is free.\n' +
    '\n' +
    'Close other Robo windows and terminals, then retry.  If the problem\n' +
    'persists, run `robo update` in a terminal for detailed diagnostics.'
  )
}

 ...
 * `iter_robo_node_dirs()` in robo_constants.py, which the Electron main
 * process cannot import.
 */
function roboManagedNodePathEntries(
  roboHome,
  ...
  const root = pathModule.join(roboHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const roboNodeDirs = roboManagedNodePathEntries(roboHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([roboNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeRoboHomeRoot(roboHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!roboHome) {
    return roboHome
  }

  const resolved = pathModule.resolve(String(roboHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). robo_bootstrap sets this inside the child too, but only

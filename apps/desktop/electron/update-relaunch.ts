 ...
 * Background
 * ----------
 * After `robo update` + `robo desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything ROBO_DESKTOP_* is preserved
// plus ROBO_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['ROBO_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['ROBO_DESKTOP_']

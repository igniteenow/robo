...
const USER_DATA_OVERRIDE = process.env.ROBO_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.ROBO_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.ROBO_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with ROBO_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[robo] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[robo] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. ROBO_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[robo] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[robo] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// never go through `robo desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Robo sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[robo] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[robo] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[robo] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[robo] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[robo] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('robo:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Robo burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[robo] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[robo] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[robo] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[robo] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// ROBO_HOME — the user-facing root for everything Robo-related. Mirrors
// scripts/install.ps1's $RoboHome and scripts/install.sh's $ROBO_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\robo (matches install.ps1)
//   macOS / Linux: ~/.robo (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.robo directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\robo yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// ROBO_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// ROBO_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.robo / %LOCALAPPDATA%\robo.
function resolveRoboHome() {
  if (process.env.ROBO_HOME) {
    return normalizeRoboHomeRoot(process.env.ROBO_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'robo-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a ROBO_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\robo and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('ROBO_HOME')

    if (fromRegistry) {
      return normalizeRoboHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'robo')
    const legacy = path.join(app.getPath('home'), '.robo')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.robo setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.robo')
}

const ROBO_HOME = resolveRoboHome()

function pathWithRoboManagedNode(...entries) {
  const managed = roboManagedNodePathEntries(ROBO_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_ROBO_ROOT — the canonical mutable Robo install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_ROBO_ROOT = path.join(ROBO_HOME, 'robo-engineer')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_ROBO_ROOT, 'venv')
...
// boots, going straight to `resolveRoboBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_ROBO_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_ROBO_ROOT, '.robo-bootstrap-complete')
...
// active-profile.json records which Robo profile the desktop launches its
// local backend as. When set, startRobo() passes `robo --profile <name>
// dashboard …`, which deterministically pins ROBO_HOME (see
// _apply_profile_override in robo_cli/main.py) and bypasses the sticky
// ~/.robo/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors robo_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// roboDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under ROBO_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by robo_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(ROBO_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (robo_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.ROBO_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.ROBO_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.ROBO_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.ROBO_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.ROBO_DESKTOP_APP_NAME || 'Robo'
...
// 'robo:native-theme' ('dark' | 'light' | 'system'); we pin
...
// an in-place `robo update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveRoboVersion(),
  copyright: 'Copyright © 2026 Ignitee Now'
})
...
const MEDIA_PROTOCOL = 'robo-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(robo-media://…)`.
...
// startRobo(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.ROBO_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.ROBO_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startRobo() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning robo serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Robo backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (roboLog.length > 300) {
    roboLog.splice(0, roboLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-robo-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('robo:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the robo:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('robo:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_ROBO_ROOT
  })
}
...
// The Tauri updater writes ROBO_HOME/.robo-update-in-progress for the whole
...
// (`force_kill_other_robo`, taskkill /IM robo.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(ROBO_HOME)),
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Robo will start automatically when it completes…',
        12
      )
    },
...
function findOnPath(command) {
  ...
  // shell-script shim named `robo` — must not shadow `robo.cmd`/`robo.exe`.
...
function unwrapWindowsVenvRoboCommand(command, backendArgs) {
  return resolveVenvRoboCommand(command, backendArgs, {
...
// spawns `robo serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `robo` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'robo_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `robo --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, ROBO_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveRoboBackend step 4); execFileSync of a .cmd without
...
function isRoboSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'robo_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.ROBO_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Robo deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the robo-import
...
// findGitBash — locate bash.exe on Windows. Resolves ROBO_GIT_BASH_PATH
...
// normal ROBO_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\robo\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'robo', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'robo', 'git', 'bin', 'git.exe'))
  }
...
function recentRoboLog() {
  return roboLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's robo root) ──────
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('robo:updates:progress', merged)
  }
}
...
// Windows that binary owns ALL repo mutation — running `robo update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(ROBO_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `robo update` can write
...
// only signals the direct child, so on Windows a backend `robo.exe` that
// spawned its own grandchildren (a `robo` REPL, a pty terminal session, the
...
// `roboProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `robo.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a robo
// REPL, a pty terminal, the gateway) keep `robo.exe` and other files in the
...
// `robo update`). Instead we spawn the staged Robo-Setup binary with
// --update and quit, so it can run `robo update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // `robo update` (backend) + `robo desktop --build-only` (OS-aware GUI
    ...
    if (!updater) {
      ...
      return { ok: true, manual: true, command, roboRoot: updateRoot }
    }
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Robo — this window will close and the updater will open. Don’t reopen Robo yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // robo.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running robo, an unkillable child). Handing off anyway
    ...
    // Robo processes running from this venv.  The updater normally refuses
    ...
    // `robo update` will run (the venv shim is locked while we live).
    const child = spawnUpdaterProcess(updater, updaterArgs, {
      cwd: ROBO_HOME,
      env: {
        ...process.env,
        ROBO_HOME,
        PATH: pathWithRoboManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore'
    })
    ...
    // abort with "Another Robo update is already running (PID <itself>)" —
    ...
    if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
      writeUpdateMarker(ROBO_HOME, child.pid)
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvRobo = path.join(venvBin, IS_WINDOWS ? 'robo.exe' : 'robo')
  ...
  // not just the `robo.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvRobo) || fileExists(path.join(updateRoot, '.robo-bootstrap-complete'))
...
// Resolve the Robo CLI used to drive an in-app update. Support both common
// virtual-environment directory names without retaining an old product alias.
function resolveRoboCliBinary(updateRoot) {
  const venvRobo = path.join(updateRoot, '.venv', 'bin', 'robo')
  const alternateVenvRobo = path.join(updateRoot, 'venv', 'bin', 'robo')

  if (fileExists(venvRobo)) {
    return venvRobo
  }

  if (fileExists(alternateVenvRobo)) {
    return alternateVenvRobo
  }

  return findOnPath('robo') || null
}
...
// Python-level pre-update snapshot inside `robo update`.
function preflightStateDb(roboHome, rememberLog) {
  const stateDbPath = path.join(roboHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(roboHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(roboHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(roboHome, old))
            } catch {
              void 0
            }
          }
...
// macOS/Linux in-app update: backend (`robo update`) + OS-aware GUI rebuild
// (`robo desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const robo = resolveRoboCliBinary(updateRoot)

  if (!robo) {
    emitUpdateProgress({ stage: 'manual', message: 'robo update', percent: null })

    return { ok: true, manual: true, command: 'robo update', roboRoot: updateRoot }
  }

  // ── Pre-flight state.db integrity guard (#68474) ──
  preflightStateDb(ROBO_HOME, rememberLog)

  // Put the Robo-managed Node and the venv on PATH so `robo desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\\robo\\node, not node\\bin.
  // PYTHONUNBUFFERED: `robo update` writes to a pipe here, so CPython
  ...
  const env: Record<string, string> = {
    ROBO_HOME,
    PYTHONUNBUFFERED: '1',
    PATH: pathWithRoboManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `robo update` reaps stale `robo serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads ROBO_DESKTOP_CHILD_PID and excludes
  ...
  if (roboProcess && Number.isInteger(roboProcess.pid)) {
    desktopChildPids.push(roboProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.ROBO_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Robo (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(robo, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'robo update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'robo update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(robo, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Robo to retry.',
  ...
  // Linux in-app update terminal state (#45205). `robo desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Robo…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `robo-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Robo to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Robo desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Robo to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Robo.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Robo.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Robo to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.robo-update-new"; then
    rm -rf "$DST.robo-update-old" 2>/dev/null || true
    mv "$DST" "$DST.robo-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.robo-update-new" "$DST"
    rm -rf "$DST.robo-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Robo to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_ROBO_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isRoboSourceRoot(ACTIVE_ROBO_ROOT) &&
    fileExists(venvPython) &&
    canImportRoboCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_ROBO_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `robo
...
function resolveWebDist() {
  const override = process.env.ROBO_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set ROBO_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: robo desktop --force-build`
  )
...
function resolveRoboCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Robo.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveRoboCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'robo_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_ROBO_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Robo at ${ACTIVE_ROBO_ROOT}`,
    command,
    args: ['-m', 'robo_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      roboHome: ROBO_HOME,
      pythonPathEntries: [ACTIVE_ROBO_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_ROBO_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveRoboBackend(backendArgs) {
  // 1. Explicit override -- ROBO_DESKTOP_ROBO_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.ROBO_DESKTOP_ROBO_ROOT && path.resolve(process.env.ROBO_DESKTOP_ROBO_ROOT)

  if (overrideRoot && isRoboSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Robo source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `robo` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isRoboSourceRoot.)
  if (!IS_PACKAGED && isRoboSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Robo source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_ROBO_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\robo\\robo-engineer (Windows) or ~/.robo/robo-engineer.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Robo runtime at ${ACTIVE_ROBO_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `robo` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    ROBO_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.ROBO_DESKTOP_IGNORE_EXISTING !== '1') {
    let roboCommand = null
    const roboOverride = process.env.ROBO_DESKTOP_ROBO

    if (roboOverride) {
      const resolvedOverride = findOnPath(roboOverride)

      if (resolvedOverride) {
        roboCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(roboOverride, { isWsl: IS_WSL })) {
        roboCommand = roboOverride
      } else {
        rememberLog(`Ignoring Windows Robo override under WSL: ${roboOverride}`)
      }
    } else {
      roboCommand = findOnPath('robo')
    }

    if (roboCommand) {
      if (looksLikeDesktopAppBinary(roboCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Robo CLI: ${roboCommand}`)
        roboCommand = null
      }
    }

    if (roboCommand) {
      const unwrapped = unwrapWindowsVenvRoboCommand(roboCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `robo` shim
      ...
      const shellForProbe = isCommandScript(roboCommand)

      // ROBO_DESKTOP_ROBO is an explicit deployment override (used by
      ...
      if (shouldTrustRoboOverride(roboOverride) || verifyRoboCli(roboCommand, { shell: shellForProbe })) {
      ...
      rememberLog(
        `Ignoring existing Robo CLI at ${roboCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed robo_cli module via system Python.
  ...
  if (python) {
    ...
    // having robo_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\robo\robo-engineer\venv.
    if (canImportRoboCli(python)) {
      return {
        kind: 'python',
        label: `installed robo_cli module via ${python}`,
        command: python,
        args: ['-m', 'robo_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: robo_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveRoboBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Robo is not installed yet; setup required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveRoboBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Robo install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Robo recovery was handed off to Robo Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      roboHome: ROBO_HOME,
      logRoot: path.join(ROBO_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Robo install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Robo bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(ROBO_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startRobo() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // robo:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveRoboBackend(backend.args))
  }
  ...
  if (!isRoboSourceRoot(ACTIVE_ROBO_ROOT)) {
    throw new Error(
      `Robo install at ${ACTIVE_ROBO_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Robo' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\robo\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `robo` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Robo on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isRoboSourceRoot()
    // plus an importable robo_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Robo venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Robo at ${ACTIVE_ROBO_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Robo runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----robo-${crypto.randomBytes(12).toString('hex')}`
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Robo backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Robo backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Robo backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Robo-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('robo:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveRoboCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('robo:preview-file-changed', payload)
}
...
// waitForRoboReady how to read a 401 — rejected session vs gated route.
...
function sendBackendExit(payload) {
  ...
  webContents.send('robo:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('robo:close-preview-requested')
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('robo:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Robo...".
function sendPowerResume() {
  ...
  webContents.send('robo:power-resume')
}
...
// 'robo:power-battery' push below.
ipcMain.handle('robo:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('robo:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('robo:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('robo:window-state-changed', state)
}
...
//
// Hosted Robo gateways gate the dashboard behind an OAuth provider (e.g. a
// self-hosted OIDC provider) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``robo_session_at``),
...
//     alongside the ~15-min access token (robo #37247).
//     Both are set as HttpOnly cookies (``robo_session_at`` ~15 min,
//     ``robo_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:robo-remote-oauth'
...
// "not signed in", which on the initial boot path (startRobo → the renderer's
// single-shot boot() with no retry) surfaced as the "Robo couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Robo
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Robo Cloud agent…' : 'Sign in to Robo gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Robo backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in robo_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Robo Cloud org tag on cloud-mode entries so Settings can
...
// a valid stored value (pins the root ROBO_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.ROBO_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.ROBO_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Robo Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteRoboPath: (ssh || savedSsh)?.remoteRoboPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by ROBO_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Robo Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Robo Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteRoboPath: input.sshRemoteRoboPath ?? existingBlock.remoteRoboPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (robo #37247), and the gateway middleware transparently
    ...
    if (
      ...
      const err = new Error(
        'Remote Robo gateway uses OAuth, but you are not signed in. ' +
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Robo gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Robo gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget() {
  ...
  if (process.env.ROBO_DESKTOP_REMOTE_URL) {
    return null
  }
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  try {
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteRoboPath || '')
    ...
    result = await lifecycle({
      ...
      remoteRoboPath: sshConfig.remoteRoboPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForRobo: (baseUrl, token) => waitForRobo(baseUrl, token, lease.signal, 'token'),
  ...
  sshConnections.set(scope, {
    ...
    host: sshConfig.host,
    hostLabel,
    roboVersion: result.roboVersion || '',
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.roboVersion || 'robo (version unknown)'} at ${result.roboPath || '?'}`
  )
  ...
  return { ...connection, remoteRoboVersion: result.roboVersion || '' }
}
...
//   2. env override (ROBO_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.ROBO_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.ROBO_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'ROBO_DESKTOP_REMOTE_URL is set but ROBO_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Robo backend.'
      )
    }
...
// env → global) so a startRobo() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Robo
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with <provider>" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteRoboPath: input.sshRemoteRoboPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteRoboPath || '')
          let roboPath
          let roboVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            roboPath = runtime.roboPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.roboPath])
            roboVersion = inspection.version
            supported = inspection.supported
          } else {
            roboPath = await remoteLifecycle.locateRobo(ssh, sshConfig.remoteRoboPath || '')
            roboVersion = await remoteLifecycle.probeRoboVersion(ssh, roboPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, roboPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Robo on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteRoboPath: roboPath,
            remoteRoboVersion: roboVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startRobo())
  ...
  // connect to Robo gateway". Mirror the renderer's connect here so the test
...
function resetRoboConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const roboProcess = backendConnectionState.invalidate()
  stopBackendChild(roboProcess)
...
// startRobo() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetRoboConnection() invalidates it.
  const roboProcess = backendConnectionState.getProcess()
  const dying = roboProcess && !roboProcess.killed ? roboProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('robo:connection:applied')
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startRobo()
...
// local-spawn portion of startRobo() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForRobo(remote.baseUrl, remote.token, undefined, remote.authMode)
  ...
  // --profile wins over the inherited ROBO_HOME env (see _apply_profile_override
  // step 3 in robo_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveRoboBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const roboCwd = resolveRoboCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Robo backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: roboCwd,
        ROBO_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        ROBO_DESKTOP: '1',
        ROBO_WEB_DIST: webDist,
        ...(readyFile ? { ROBO_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Robo backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Robo backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Robo backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForRobo(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Robo backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Robo backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_robo_home() recreates the deleted profile directory.
//
...
async function startRobo() {
  ...
  // process, every subsequent startRobo() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Robo backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      await advanceBootProgress('backend.remote', `Connecting to remote Robo backend at ${remote.baseUrl}`, 24)
      await waitForRobo(remote.baseUrl, remote.token, undefined, remote.authMode)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Robo backend is ready',
        progress: 94,
        running: true,
        error: null
      })
      ...
      return {
        ...
        remoteRoboVersion: remote.remoteRoboVersion,
        token: remote.token,
        wsUrl: remote.wsUrl,
        logs: roboLog.slice(-80),
        ...getWindowState()
    ...
    // deterministic (it wins over the sticky ~/.robo/active_profile file) and
    // resolves ROBO_HOME the same way `robo -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Robo runtime', 28)

        return resolveRoboBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Robo backend via ${backend.label}`, 84)
    rememberLog(`Starting Robo backend via ${backend.label}`)

    const roboProcess = spawn(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: roboCwd,
        env: {
          ...process.env,
          // Explicitly pin ROBO_HOME for the child so Python's get_robo_home()
          // resolves to the SAME location our resolveRoboHome() picked. Without
          // this pin, Python falls back to ~/.robo on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\robo, which differs from C:\Users\<u>\.robo.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets ROBO_HOME via setx; the desktop
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, roboProcess)

    if (!processOwner) {
      stopBackendChild(roboProcess)
      throw new Error('Robo backend start was superseded by a newer connection attempt.')
    }

    roboProcess.stdout.on('data', rememberLog)
    roboProcess.stderr.on('data', rememberLog)
    ...
    roboProcess.once('error', error => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Robo backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Robo backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Robo backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Robo backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    roboProcess.once('exit', (code, signal) => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Robo backend exit (${signal || code})`)
      ...
      rememberLog(`Robo backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Robo backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Robo backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentRoboLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Robo backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(roboProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Robo backend to become ready', 90)
    await Promise.race([waitForRobo(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => roboProcess.exitCode === null && !roboProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Robo backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Robo backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Robo is
...
// pushes pet state over IPC (robo:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via robo:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Robo app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Robo icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see robo:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Robo in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('robo:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('robo:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('robo:quick-entry:shown')
}
...
function createWindow() {
  ...
  startRobo().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('robo:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Robo…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('robo:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: resetRoboConnection,
...
ipcMain.handle('robo:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('robo:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('robo:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('robo:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('robo:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('robo:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('robo:zoom:get', event => {
...
ipcMain.on('robo:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('robo:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('robo:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('robo:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('robo:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('robo:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('robo:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('robo:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('robo:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('robo:pet-overlay:control', payload)
})
ipcMain.handle('robo:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startRobo() call restarts the
...
ipcMain.handle('robo:bootstrap:repair', async () => {
  // Forceful repair: force the next startRobo() through the full installer
  ...
  // "bypass active runtime" path inside startRobo() and fall through
...
ipcMain.handle('robo:boot-progress:get', async () => bootProgressState)
ipcMain.handle('robo:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('robo:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('robo:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('robo:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('robo:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('robo:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('robo:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startRobo() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('robo:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('robo:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('robo:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('robo:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('robo:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new ROBO_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('robo:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('robo:requestMicrophoneAccess', async () => {
...
ipcMain.handle('robo:api', async (_event, request) => {
  ...
  // backend calls ensure_robo_home() which recreates the profile directory,
...
ipcMain.handle('robo:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('robo:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('robo:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('robo:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('robo:data-url-read-max:get', () => ({
...
ipcMain.handle('robo:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('robo:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('robo:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('robo:readFileText', async (_event, filePath) => {
...
ipcMain.handle('robo:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('robo:writeClipboard', (_event, text) => {
...
ipcMain.handle('robo:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('robo:readClipboard', () => clipboard.readText())

ipcMain.handle('robo:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('robo:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('robo:saveClipboardImage', async () => {
...
ipcMain.handle('robo:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('robo:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('robo:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('robo:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('robo:titlebar-theme', (_event, payload) => {
...
ipcMain.on('robo:native-theme', (_event, mode) => {
...
ipcMain.on('robo:translucency', (_event, payload) => {
...
ipcMain.on('robo:keep-awake', (_event, on) => {
...
ipcMain.handle('robo:quick-entry:settings:get', async () => {
...
ipcMain.handle('robo:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('robo:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('robo:quick-entry:submit', {
...
ipcMain.on('robo:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('robo:quick-entry:state', payload)
  }
})

ipcMain.on('robo:quick-entry:dismiss', () => hideQuickEntryWindow())

ipcMain.handle('robo:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `robo:window:openSession`).
...
ipcMain.handle('robo:find-in-page', (event, query, options) => {
...
ipcMain.handle('robo:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('robo:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveRoboCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('robo:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveRoboCwd()
}))

ipcMain.handle('robo:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('robo:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('robo:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('robo:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('robo:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('robo:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: roboLog.slice(-200) }))
...
function terminalShellCommand() {
  // ROBO_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.ROBO_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Robo' TUI into
  ...
  env.TERM_PROGRAM = 'Robo'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a robo/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from ROBO_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.ROBO_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `robo:terminal:${id}:${suffix}`
}
...
ipcMain.handle('robo:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('robo:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('robo:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('robo:fs:openDir', async (_event, dirPath) => {
...
// The LOCAL Desktop runtime-plugin root: `<ROBO_HOME>/desktop-plugins`,
// resolved from the main-process ROBO_HOME (see resolveRoboHome) — NOT from
// the connected backend. A remote backend reports its own `robo_home` over
...
ipcMain.handle('robo:fs:desktopPluginsRoot', async () => {
  // Profile-aware: a named Desktop profile gets its own plugin root under
  // profiles/<name>/, matching the profile-scoped robo_home the backend
  // reported before this resolver existed. 'default'/unset pins the global root.
  const profile = readActiveDesktopProfile()
  const base = profile && profile !== 'default' ? path.join(ROBO_HOME, 'profiles', profile) : ROBO_HOME
...
ipcMain.handle('robo:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('robo:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('robo:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('robo:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('robo:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('robo:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('robo:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('robo:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))

ipcMain.handle('robo:git:baseBranchList', async (_event, repoPath) => listBaseBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('robo:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('robo:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('robo:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('robo:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('robo:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('robo:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('robo:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('robo:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('robo:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('robo:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('robo:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('robo:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('robo:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('robo:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('robo:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('robo:terminal:write', (_event, id, data) => {
...
ipcMain.handle('robo:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('robo:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('robo:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('robo:updates:check', async () =>
...
ipcMain.handle('robo:updates:apply', async (_event, payload) =>
...
ipcMain.handle('robo:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('robo:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Robo version (the one `release.py` bumps in
// robo_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Robo version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveRoboVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'robo_cli', '__init__.py')
...
// Re-resolve the live Robo version and push it into the native About panel
// just before showing it, so an in-place `robo update` is reflected without
...
ipcMain.handle('robo:version', async () => ({
  appVersion: resolveRoboVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  roboRoot: resolveUpdateRoot()
}))
...
// the actual removal via `robo uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (robo_cli/uninstall.py + robo_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_ROBO_ROOT
  ...
  const fallback = () => ({
    robo_home: ROBO_HOME,
    agent_installed: isRoboSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'robo_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, ROBO_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Robo runtime environment at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import robo_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_ROBO_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `robo-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `robo-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('robo:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('robo:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('robo:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('robo:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// robo:// deep links (e.g. robo://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const ROBO_PROTOCOL = 'robo'
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${ROBO_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // robo://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('robo:deep-link', payload)
...
ipcMain.handle('robo:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${ROBO_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(ROBO_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(ROBO_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `robo://` launch spawns a
...
app.on('before-quit', event => {
  ...
  // quitting Robo never keeps another app's chord hostage.

 ...
 *     `X-Robo-Session-Token` header; WS uses `?token=`.
...
// robo_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - robo_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - robo_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / robo #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-robo_session_at', '__Secure-robo_session_at', 'robo_session_at']
const RT_COOKIE_VARIANTS = ['__Host-robo_session_rt', '__Secure-robo_session_rt', 'robo_session_rt']

// Keep this aligned with robo_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['robo', 'test', 'tmp', 'root', 'sudo'])
...
// True for connection modes that resolve to a REMOTE backend.
...
function normalizeSshConfig(entry) {
  ...
  const remoteRoboPath = String(entry.remoteRoboPath || '').trim()

  if (remoteRoboPath) {
    out.remoteRoboPath = remoteRoboPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Robo installation. Preserve an explicit mapping
  // when it is a valid Robo profile identifier; otherwise fall back to the
 ...
 *     `--profile`, so its `ROBO_HOME` scopes it.
 *
...
function pathWithGlobalRemoteProfile(path, profile, opts: ProfileRouteOptions = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://robo.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a robo session ACCESS-token cookie

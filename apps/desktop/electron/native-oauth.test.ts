...
test('buildNativeAuthorizeUrl encodes params and honours a path prefix', () => {
  ...
  assert.equal(parsed.searchParams.get('provider'), 'xai')
})

test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/robo', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/robo/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/robo'), 'https://gw.example.com/robo/auth/native/refresh')
})
...
test('parseTokenResponse cannot read a persisted set (the reload bug #73271)', () => {
  ...
  const persisted = JSON.parse(
    JSON.stringify({ accessToken: 'AT', refreshToken: 'RT', expiresAt: 1, provider: 'xai', userId: 'u' })
  )

...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env ROBO_DASHBOARD_SESSION_TOKEN=abc123deadbeef ROBO_DESKTOP=1 robo dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /ROBO_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /ROBO_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-Robo-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-Robo-Session-Token: hdr_888'), /X-Robo-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-Robo-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('robo', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v robo', 15000)
  assert.equal(args[args.length - 1], 'command -v robo')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `robo-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `robo-ssh-test-${process.pid}-${Date.now()}`)

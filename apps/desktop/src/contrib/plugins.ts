 ...
 *    `RoboPlugin` registers automatically (vite glob — drop a folder in).
 *    None ship in-tree today; reference/demo plugins live in the companion
 *    `robo-example-plugins` repo.
 *  - RUNTIME: the on-disk door (`<robo home>/desktop-plugins/<name>/plugin.js`)
...
const modules = import.meta.glob<{ default: RoboPlugin }>('../plugins/*/plugin.{ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default RoboPlugin export — skipped`)

 ...
 * `RoboPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/robo'
...
export type { PluginRestOptions } from '@/robo'
 ...
 *  under `robo.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.roboDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from Robo — use `host.notify` for the in-app toast.
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `robo.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.roboDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.roboDesktop

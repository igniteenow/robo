 ...
 *   (`@robo/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default RoboPlugin -> register(ctx)
 *
 ...
 * and `<robo home>/desktop-plugins/<name>/plugin.js` on disk — the door the
...
import { createPluginContext, type RoboPlugin } from './plugin'
...
/** Rewrite ONLY mapped import specifiers (@robo/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default RoboPlugin export`)
    }
...
// ---------------------------------------------------------------------------
// The on-disk plugin door: `<robo home>/desktop-plugins/<name>/plugin.js`
...
async function scanDiskPlugins(): Promise<void> {
  ...
  try {
    ...
    // connected backend — a remote backend's robo_home is a remote path and
...
export function watchRuntimePlugins(): void {
  ...
  const startDirWatch = async (): Promise<boolean> => {
    ...
    try {
      ...
      // robo_home, which is a remote path in remote mode (#66899).
      const root = await desktop.desktopPluginsRoot?.()

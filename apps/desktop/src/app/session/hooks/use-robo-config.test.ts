...
import { getRoboConfig } from '@/robo'
import {
  $currentCwd,
  $currentFastMode,
...
import { useRoboConfig } from './use-robo-config'

vi.mock('@/robo', () => ({
  getRoboConfig: vi.fn(),
  getRoboConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'robo.desktop.workspace-cwd'
...
const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getRoboConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getRoboConfig>>)

describe('useRoboConfig refreshRoboConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Robo' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useRoboConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshRoboConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useRoboConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getRoboConfig>>>()
    vi.mocked(getRoboConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useRoboConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshRoboConfig(true)
    })
    expect(getRoboConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getRoboConfig>>)
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getRoboConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getRoboConfig>>>()
    vi.mocked(getRoboConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshRoboConfig(true)
      refreshC = result.current.refreshRoboConfig(true)
    })

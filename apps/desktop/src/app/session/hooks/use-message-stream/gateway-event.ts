import type { BillingBlock } from '@robo/shared'
import type { RoboSkin } from '@robo/shared/skin'
 ...
 * billing-specific toast — never the generic "Robo error" — with a smart CTA
 * (a provider with a billing URL deep-links out, otherwise Settings → Providers
 * opens in-app).
 */
function surfaceBillingBlock(sessionId: string, raw: unknown): void {
  ...
  notify({
    ...
    kind: 'warning',
    icon: 'credit-card',
    title: block.is_igniteenow
      ? translateNow('billingBlock.titleIgniteeNow')
...
interface GatewayEventDeps {
  ...
  refreshRoboConfig: () => Promise<void>
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  ...
  // refreshRoboConfig — two REST calls (config + defaults) per event, per
  ...
  return useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        ingestBackendSkin((payload as { skin?: RoboSkin } | undefined)?.skin, { apply: false })
        ...
        // A runtime skin switch (Robo activating an authored skin, or `/skin`
        ...
        if (fromActiveProfile) {
          ingestBackendSkin(payload as RoboSkin | undefined, { apply: true })
        }
        ...
        if (apply) {
          ...
          // everything refreshRoboConfig applies is either active-session
        ...
        const failure =
          payload?.status === 'error'
            ? {
                error: coerceGatewayText(payload.error).trim() || finalText || 'Robo reported an error',
        ...
        // native OS notification when Robo is backgrounded; dispatch is gated
        ...
        const errorMessage = payload?.message || 'Robo reported an error'

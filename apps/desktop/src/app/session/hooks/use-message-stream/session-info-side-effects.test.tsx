...
let refreshRoboConfig: ReturnType<typeof vi.fn<() => Promise<void>>>
...
beforeEach(() => {
  handleEvent = null
  refreshRoboConfig = vi.fn<() => Promise<void>>(async () => undefined)
...
describe('session.info config refetch gating', () => {
  it('coalesces active-session bursts into one trailing config fetch', async () => {
    ...
    expect(refreshRoboConfig).not.toHaveBeenCalled()

    await act(async () => {
      await vi.advanceTimersByTimeAsync(400)
    })

    expect(refreshRoboConfig).toHaveBeenCalledTimes(1)
  })

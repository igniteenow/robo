...
import { getRoboConfig, getRoboConfigDefaults } from '@/robo'
import {
...
interface RoboConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useRoboConfig({ activeSessionIdRef }: RoboConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshRoboConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        const [config, defaults] = await Promise.all([getRoboConfig(), getRoboConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Robo' built-in medium over the user's config.
  ...
  return { refreshRoboConfig, sttEnabled, voiceMaxRecordingSeconds }
}

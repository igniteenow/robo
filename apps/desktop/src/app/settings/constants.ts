...
export const EMPTY_SELECT_VALUE = '__robo_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    ...
    // Slot #1 — mirrors CANONICAL_PROVIDERS.
  ...
  { prefix: 'ROBO_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // robo_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  agent: {
    ...
    maxTurns: 'Upper bound for tool-calling turns before Robo stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Robo can read from one file request.',
  approvals: {
    mode: 'How Robo handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Robo updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})

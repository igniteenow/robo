...
const getRoboConfigRecord = vi.fn()
const saveRoboConfig = vi.fn()
...
vi.mock('@/robo', () => ({
  ...
  getRoboConfigRecord: () => getRoboConfigRecord(),
  saveRoboConfig: (config: unknown) => saveRoboConfig(config),
...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'xai', model: 'grok-4' })
    ...
    providers: [
      {
        ...
        authenticated: true,
        capabilities: { 'grok-4': { reasoning: true, fast: true } }
      }
    ]
  })
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'xai', model: 'grok-4' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  getMoaModels.mockResolvedValue(null)
  setModelAssignment.mockResolvedValue({ provider: 'xai', model: 'grok-4', gateway_tools: [] })
  getRecommendedDefaultModel.mockResolvedValue({ provider: 'xai', model: 'grok-4', free_tier: null })
  setEnvVar.mockResolvedValue({ ok: true })
  getRoboConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
  saveRoboConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  it('loads the current main model and lists configured providers only', async () => {
    ...
    // "xAI" shows in both the trigger and the open list.
    expect((await screen.findAllByText('xAI')).length).toBeGreaterThan(0)
  ...
  it('replaces the selected provider and model when the active profile changes', async () => {
    getGlobalModelInfo
      ...
      .mockResolvedValueOnce({ provider: 'xai', model: 'grok-4' })
    ...
    await waitFor(() => expect(screen.getAllByRole('combobox')[0].textContent).toContain('xAI'))
  ...
  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getRoboConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(saveRoboConfig).toHaveBeenCalledWith(
      ...
      providers: [
        {
          ...
          models: ['grok-4'],
          authenticated: true,
          capabilities: { 'grok-4': { reasoning: false, fast: false } }
        }
      ]
    })
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'xai', model: 'grok-4' }]
    })
        ...
        expect.objectContaining({
          presets: expect.objectContaining({
            default: expect.objectContaining({
              reference_models: [
                expect.objectContaining({ provider: 'xai', model: 'grok-4', enabled: false }),

...
import type { ToolsetConfig } from '@/types/robo'
...
const getRoboConfigSchema = vi.fn()
...
vi.mock('@/robo', () => ({
  ...
  getRoboConfigSchema: () => getRoboConfigSchema(),
...
beforeEach(() => {
  ...
  getRoboConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getRoboConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveRoboConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveRoboConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
  ...
  describe('readiness pills', () => {
    it('renders the server status instead of assuming keyless rows are Ready', async () => {
      // The false-Ready bug: a signed-out OAuth-gated row and a

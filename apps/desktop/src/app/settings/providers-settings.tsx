...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.roboDesktop?.terminal)
...
//      in robo_cli/provider_catalog.py) — the SAME provider identity
//      `robo model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Robo can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Robo can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Robo never deletes creds another tool owns behind a silent API call.

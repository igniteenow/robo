...
function reveal(file: string) {
  void window.roboDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // robo_home breaks against a remote backend (#66899).
    const dir = await window.roboDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.roboDesktop?.openDir?.(dir)

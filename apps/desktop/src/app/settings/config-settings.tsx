...
import type { ConfigFieldSchema, RoboConfigRecord } from '@/types/robo'
...
export function voiceFieldVisible(key: string, config: RoboConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<RoboConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useRoboConfigRecord()
  ...
  const {
    ...
    queryKey: ['robo-config-schema'],
    queryFn: getRoboConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveRoboConfig(config)
  ...
  const updateConfig = (next: RoboConfigRecord) => {

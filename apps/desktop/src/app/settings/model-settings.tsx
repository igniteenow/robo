...
import { invalidateRoboConfig, setRoboConfigCache, useRoboConfigRecord } from '../hooks/use-config-record'
...
// backend now surfaces the full `robo model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in robo_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useRoboConfigRecord()
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `robo model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Robo runs the flow for you.`}
          </p>
        )}

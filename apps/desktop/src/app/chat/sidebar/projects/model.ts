...
import type { SessionInfo } from '@/robo'
...
// zero Robo sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, RoboGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, RoboGitWorktree[]>>({})

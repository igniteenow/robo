...
describe('detectTrigger', () => {
  ...
  it('only treats a KNOWN kind as a scope', () => {
    // `@robodev1:` is a handle with a colon, not a directive — inventing a
    // scope for it would make Backspace eat the whole word.
    expect(detectTrigger('@robodev1:')?.scope).toBeUndefined()
    expect(detectTrigger('@robodev1:')?.value).toBe('robodev1:')

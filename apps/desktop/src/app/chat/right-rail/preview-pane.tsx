        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.roboDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Robo to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.roboDesktop?.watchPreviewFile ||
      !window.roboDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.roboDesktop.onPreviewFileChanged(payload => {
    ...
    void window.roboDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.roboDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.roboDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:robo-preview')

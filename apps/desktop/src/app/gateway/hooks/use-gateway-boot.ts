import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@robo/shared'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.roboDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: RoboGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: RoboConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Robo...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Robo…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Robo gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshRoboConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshRoboConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startRobo re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new RoboGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshRoboConfig(),
          ...
          // retry it), not brick boot behind the "Robo couldn't start"

...
// bug that drives the REAL useGatewayBoot hook + REAL RoboGateway through a
...
function Harness({
  ...
  useGatewayBoot({
    ...
    refreshRoboConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { roboDesktop?: unknown }).roboDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { roboDesktop?: unknown }).roboDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForRobo) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startRobo()'s remote branch awaits waitForRobo() for 45s before it
    ...
    ;(window as { roboDesktop?: unknown }).roboDesktop = desktop
    ...
    // After ~45s waitForRobo gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Robo backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "Robo couldn't start" overlay — the socket is open

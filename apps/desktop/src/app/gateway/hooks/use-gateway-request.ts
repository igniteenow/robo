...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as RoboGateway | null
  const gatewayRef = useRef<RoboGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.roboDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<RoboGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as RoboGateway | null
      }),
    []
  )

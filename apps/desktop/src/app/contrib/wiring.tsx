...
import { getSessionMessages, triggerCronJob } from '@/robo'
...
import { useRoboConfig } from '../session/hooks/use-robo-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshRoboConfig, sttEnabled, voiceMaxRecordingSeconds } = useRoboConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('robo:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('robo:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshRoboConfig(true)
    void refreshActiveProfile()
  }, [activeGatewayProfile, refreshCurrentModel, refreshRoboConfig])

...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { roboDesktop: { readDir: typeof readDir } }).roboDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): RoboReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { roboDesktop: unknown }).roboDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: RoboReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<RoboReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { roboDesktop: unknown }).roboDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.roboDesktop is missing', async () => {
    delete (window as unknown as { roboDesktop?: unknown }).roboDesktop

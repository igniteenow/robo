...
import type { RoboReadDirEntry, RoboReadDirResult } from '@/global'
...
export type ProjectTreeEntry = RoboReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: RoboReadDirEntry) {
...
async function filterIgnored(entries: RoboReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<RoboReadDirResult> {
  if (!window.roboDesktop) {

...
import { getRoboConfigRecord, type RoboConfigRecord, saveRoboConfig } from '@/robo'
...
export interface I18nConfigClient {
  getConfig: () => Promise<RoboConfigRecord>
  saveConfig: (config: RoboConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.roboDesktop?.api) {
      return Promise.resolve({})
    }

    return getRoboConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.roboDesktop?.api) {
...
export function getConfigDisplayLanguage(config: RoboConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: RoboConfigRecord, locale: Locale): RoboConfigRecord {

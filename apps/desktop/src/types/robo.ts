...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Robo doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in robo_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `robo model` uses — instead of
...
export type RoboConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `robo model` CLI / onboarding OAuth flow. */
  auth_type?: string
  ...
  free_tier?: boolean
  /** Reserved: always empty on current backends. */
...
// non-desktop) Robo simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (robo.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors robo_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Robo, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `robo tools` active-provider detection). */
  is_active: boolean
  /** Honest readiness computed server-side (keys ∧
 ...
 *  not Robo); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/robo/update/check` — the backend's own update state.
...
/** One MCP catalog entry from `GET /api/mcp/catalog`. */

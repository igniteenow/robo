...
export function setZoomPercent(percent: number): void {
  window.roboDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.roboDesktop?.zoom) {
  void window.roboDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.roboDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}

...
const { getProfiles } = await import('@/robo')
...
const remoteConn = (over: Partial<RoboConnection> = {}): RoboConnection =>
  ({ baseUrl: 'https://robo-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as RoboConnection

const localConn = (over: Partial<RoboConnection> = {}): RoboConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as RoboConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<RoboConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { roboDesktop: { getConnection } })

...
import type { RoboGitWorktree, RoboRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: RoboGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, RoboRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, RoboGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<RoboRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<RoboGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<RoboRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<RoboGitWorktree[]>>()
const $noRepoStatus = atom<RoboRepoStatus | null>(null)
const $noWorktrees = atom<RoboGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<RoboRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<RoboGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: RoboRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: RoboGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<RoboRepoStatus | null>
  seq: number
  target: string
}

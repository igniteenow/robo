...
const isOpen = (gateway: RoboGateway | null): boolean => gateway?.connectionState === 'open'
...
interface GatewayRegistryState {
  ...
  primaryGateway: RoboGateway | null
  primaryProfile: string
  activeKey: string
  secondaries: Map<string, Secondary>
  $gateway: ReturnType<typeof atom<RoboGateway | null>>
}

const STATE_KEY = Symbol.for('robo.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<RoboGateway | null>(null)
  }
}
...
export function setPrimaryGateway(gateway: RoboGateway | null, profile = 'default'): void {
...
export function activeGateway(): RoboGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<RoboGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }

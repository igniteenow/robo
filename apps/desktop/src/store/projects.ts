...
import type { RoboGitBaseBranch, RoboGitBranch } from '@/global'
import { translateNow } from '@/i18n'
import { desktopDefaultCwd, isDesktopFsRemoteMode, selectDesktopPaths, writeDesktopFileText } from '@/lib/desktop-fs'
import { desktopGit } from '@/lib/desktop-git'
import { isMissingRpcMethod } from '@/lib/gateway-rpc'
import { persistentAtom } from '@/lib/persisted'
import { getRoboConfig, type RoboGateway } from '@/robo'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/robo'
...
const PROJECT_SCOPE_KEY = 'robo.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Robo gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== ($activeGatewayProfile.get() || 'default')) {
    throw new Error('Active Robo profile changed while connecting')
  }
...
async function refreshProjectTreeOn(gateway: RoboGateway): Promise<void> {
...
const repoScanStates = new WeakMap<RoboGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<RoboGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getRoboConfig(context.profile))
...
// repo, returning where Robo should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<RoboGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<RoboGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.roboDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.roboDesktop?.writeClipboard?.(path)
  }
}

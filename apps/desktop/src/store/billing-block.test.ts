...
test('billingCtaLabel picks the right verb per route', () => {
  const copy = { addCredits: 'Add credits', openBilling: 'Manage provider' }
  expect(billingCtaLabel(makeBlock({ billing_url: 'https://example.com/billing' }), copy)).toBe('Add credits')
  expect(billingCtaLabel(makeBlock({ billing_url: null }), copy)).toBe('Manage provider')
})

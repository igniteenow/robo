...
const STORAGE_KEY = 'robo:native-notifications'
...
// "Backgrounded" = the user isn't on Robo. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.roboDesktop?.notify({
 ...
 *  user is away from Robo — the in-app toast (`host.notify`) covers the
 *  foreground case. */
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.roboDesktop

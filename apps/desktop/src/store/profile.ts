...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/robo'
...
// /api/profiles/active `current`). "default" is the root ~/.robo. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'robo.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'robo.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.roboDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new ROBO_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.roboDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.roboDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'robo.desktop.showAllProfiles'
...
// Switch to the default (root ~/.robo) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.roboDesktop?.touchBackend?.(target).catch(() => undefined)
}

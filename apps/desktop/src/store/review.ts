...
import type { RoboReviewFile, RoboReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Robo' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'robo.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'robo.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'robo.desktop.reviewTreeMode'
const SELECTED_KEY = 'robo.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<RoboReviewFile[]>([])
...
export const $reviewShipInfo = atom<RoboReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['roboDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: RoboReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly RoboReviewFile[], path: string): RoboReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.roboDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.roboDesktop?.openExternal?.(url)
    }

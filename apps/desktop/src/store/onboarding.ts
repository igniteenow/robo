...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/robo'
...
const CONFIGURED_CACHE_KEY = 'robo-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'robo-onboarding-skipped-v1'
const POLL_MS = 2000
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Robo is ready', message: `${provider} connected.` })
}

// Human-friendly labels for tools auto-routed through a managed tool gateway.
// Reserved: gateway_tools is always empty on current backends.
...
// Reserved for a future managed tool gateway. Always a no-op on current
// backends since gateway_tools is always empty.
function notifyGatewayTools(tools: string[] | undefined) {
  ...
  notify({
    durationMs: 8000,
    kind: 'info',
    message: `${list} now run through your managed subscription — no separate API keys needed.`,
...
async function fetchProviderDefaultModel(
  ...
  // `robo model` does. Fall back to the first curated model if the endpoint
  // can't resolve one.
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Robo still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Robo still cannot resolve a usable provider.'
}
...
export async function refreshOnboarding(ctx: OnboardingContext) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'Robo could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
...
async function openSignInUrl(url: string) {
  if (window.roboDesktop?.openExternal) {
    try {
      await window.roboDesktop.openExternal(url)
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Robo still cannot reach ${url}.` }
    }

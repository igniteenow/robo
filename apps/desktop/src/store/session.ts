import type { ConnectionState } from '@robo/shared'
...
import type { SessionInfo, UsageStats } from '@/types/robo'
...
const COMPOSER_MODEL_KEY = 'robo.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'robo.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'robo.desktop.composer.model-source'
...
const LAST_SESSION_KEY = 'robo.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'robo.desktop.lastRoute'
...
function workspaceCwdKey(connection: RoboConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.roboDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.roboDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<RoboConnection | null>(null)
...
export const setConnection = (next: Updater<RoboConnection | null>) => updateAtom($connection, next)
...
// until config loads, and re-seeded on every profile switch by useRoboConfig.

...
// reaches the gateway); everything else routes through window.roboDesktop.git,
...
function file(path: string, over: Partial<RoboReviewFile> = {}): RoboReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as RoboReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.roboDesktop. Any op not supplied defaults
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as RoboReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.roboDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: RoboReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as RoboReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { roboDesktop?: unknown }).roboDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as RoboReviewShipInfo)

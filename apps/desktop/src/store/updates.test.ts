...
const checkRoboUpdateSpy = vi.fn()
const updateRoboSpy = vi.fn()
const getActionStatusSpy = vi.fn()

vi.mock('@/robo', () => ({
  checkRoboUpdate: (...args: unknown[]) => checkRoboUpdateSpy(...args),
  updateRobo: (...args: unknown[]) => updateRoboSpy(...args),
...
describe('checkBackendUpdates', () => {
  ...
  it('maps the backend /update/check onto the backend status, including commits', async () => {
    setRemote(true)
    checkRoboUpdateSpy.mockResolvedValue({
      install_method: 'git',
    ...
    expect(checkRoboUpdateSpy).toHaveBeenCalled()
  ...
  it('is a no-op in local mode (backend check only runs when remote)', async () => {
    ...
    expect(checkRoboUpdateSpy).not.toHaveBeenCalled()
  })
})

// The ⌘K "Update Robo" row. It used to call applyBackendUpdate() flat, which
...
describe('requestActiveUpdate', () => {
  ...
  beforeEach(() => {
    ...
    updateRoboSpy.mockReset().mockResolvedValue({ ok: true, name: 'update' })
    checkRoboUpdateSpy.mockReset().mockResolvedValue({
      install_method: 'git',
    ...
    ;(globalThis as unknown as { window: unknown }).window = {
      roboDesktop: { updates: { apply: applyClientMock, check: checkClientMock } }
    }
    vi.useRealTimers()
  })
  ...
  it('applies the CLIENT update in local mode, never the backend', async () => {
    ...
    expect(updateRoboSpy).not.toHaveBeenCalled()
  ...
  it('applies the BACKEND update in remote mode', async () => {
    ...
    requestActiveUpdate()
    await vi.waitFor(() => expect(updateRoboSpy).toHaveBeenCalled())
...
describe('applyUpdates terminal state', () => {
  ...
  beforeEach(() => {
    ...
    ;(globalThis as unknown as { window: unknown }).window = {
      roboDesktop: { updates: { apply: applyMock } }
    }
    vi.useRealTimers()
  })
  ...
  it('keeps the manual command state for CLI installs with no staged updater', async () => {
    applyMock.mockResolvedValue({ ok: true, manual: true, command: 'robo update' })

    await applyUpdates()

    expect($updateApply.get().stage).toBe('manual')
    expect($updateApply.get().command).toBe('robo update')
    expect($updateOverlayOpen.get()).toBe(true)
    expect(notifySpy).not.toHaveBeenCalled()
  })
  ...
  it('lands on a closeable manual-restart state when the rebuilt sandbox blocks auto-relaunch', async () => {
    ...
    applyMock.mockResolvedValue({
      ...
      manualRestart: true,
      sandboxBlocked: true,
      message: 'Backend updated. Quit and reopen Robo to finish.'
    })
...
describe('applyBackendUpdate recovery', () => {
  ...
  it('waits for the backend to return after the restart drops the connection, then clears the overlay', async () => {
    ...
    updateRoboSpy.mockResolvedValue({ action_id: actionId, ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValueOnce(new Error('ECONNREFUSED')).mockResolvedValueOnce({
      exit_code: null,
      lines: [`=== robo-update completed ${actionId} ===`],
      name: 'update',
      pid: null,
      running: false
    })
  ...
  it('keeps waiting past the old 45-second cutoff while the update action is running', async () => {
    ...
    updateRoboSpy.mockResolvedValue({ action_id: actionId, ok: true, name: 'robo-update', pid: 1 })

    for (let attempt = 0; attempt < 31; attempt += 1) {
      getActionStatusSpy.mockResolvedValueOnce({
        exit_code: null,
        lines: ['=== robo-update started now ===', `step ${attempt}`],
        name: 'robo-update',
        pid: 1,
        running: true
      })
  ...
  it('treats a successful no-op as complete without waiting for a restart', async () => {
    updateRoboSpy.mockResolvedValue({ ok: true, name: 'robo-update', pid: 1 })
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['stale output from another run', '=== robo-update started now ===', '✓ Already up to date!'],
      name: 'robo-update',
      pid: 1,
      running: false
    })
  ...
  it('treats a successful dependency repair as complete without waiting for a restart', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['=== robo-update started now ===', '✓ Dependencies repaired!', '✓ Update complete!'],
      name: 'robo-update',
      pid: 1,
      running: false
    })
  ...
  it('waits for current-action completion proof after the backend restarts', async () => {
    ...
    getActionStatusSpy
      ...
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['Update complete!', `=== robo-update completed ${'c'.repeat(32)} ===`],
        name: 'robo-update',
        pid: null,
        running: false
      })
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['Update complete!', `=== robo-update completed ${actionId} ===`],
        name: 'robo-update',
        pid: null,
        running: false
      })
  ...
  it('accepts its terminal receipt when a verbose update pushes the start marker out of the log tail', async () => {
    ...
    getActionStatusSpy.mockRejectedValueOnce(new Error('ECONNREFUSED')).mockResolvedValueOnce({
      exit_code: null,
      lines: ['final build output', 'Update complete!', `=== robo-update completed ${actionId} ===`],
      name: 'robo-update',
      pid: null,
      running: false
    })
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(getActionStatusSpy).toHaveBeenCalledWith('robo-update', 2000)
  })

  it('proves a pre-action-ID backend reached its requested commit after restart', async () => {
    $backendUpdateStatus.set({
      behind: 2,
      commits: [{ at: 1, author: 'Robo', sha: 'requested-target', summary: 'target' }],
      fetchedAt: 1,
      supported: true,
    ...
    checkRoboUpdateSpy
      ...
      .mockResolvedValueOnce({
        behind: 1,
        can_apply: true,
        commits: [{ at: 2, author: 'Robo', sha: 'newer-commit', summary: 'newer' }],
        current_version: '0.18.2',
        install_method: 'git',
        message: null,
        update_available: true,
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(checkRoboUpdateSpy).toHaveBeenCalledTimes(2)
  })
  ...
  it('proves a fast pre-action-ID packaged update by its changed version', async () => {
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(checkRoboUpdateSpy).toHaveBeenCalledWith(true)
  })
  ...
  it('resumes action polling after a transient status failure', async () => {
    ...
    getActionStatusSpy
      ...
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['=== robo-update started now ===', 'still running'],
        name: 'robo-update',
        pid: 1,
        running: true
      })
      .mockResolvedValueOnce({
        exit_code: 0,
        lines: ['=== robo-update started now ===', 'Update complete!'],
        name: 'robo-update',
        pid: 1,
        running: false
      })
  ...
  it('restores the fixed action deadline after reconnecting', async () => {
    updateRoboSpy.mockResolvedValue({ action_id: 'a'.repeat(32), ok: true, name: 'robo-update', pid: 1 })
  ...
  it('shares one in-flight update between concurrent apply requests', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['=== robo-update started now ===', '✓ Already up to date!'],
      name: 'robo-update',
      pid: 1,
      running: false
    })
    ...
    expect(updateRoboSpy).toHaveBeenCalledTimes(1)
  })
  ...
  it('fails immediately when the update action exits nonzero', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 1,
      lines: ['=== robo-update started now ===', 'update failed'],
      name: 'robo-update',
      pid: 1,
      running: false
    })
  ...
  it('surfaces an error when the backend never comes back after the restart', async () => {
    updateRoboSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkRoboUpdateSpy.mockRejectedValue(new Error('ECONNREFUSED'))
...
describe('startUpdatePoller', () => {
  ...
  beforeEach(() => {
    ...
    $updateStatus.set(null)
    ;(globalThis as unknown as { window: unknown }).window = {
      roboDesktop: { updates: { check: checkMock, onProgress: onProgressMock } },

 ...
 *
 * The archive is the CLI's own `robo profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/robo'
...
import type { ProfileDesktopOverlay } from '@/types/robo'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Robo profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.roboDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.roboDesktop?.selectPaths?.({

 ...
 * app's bounds and stays visible while Robo is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'robo.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'robo.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.roboDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.roboDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.roboDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.roboDesktop?.petOverlay?.close()
}

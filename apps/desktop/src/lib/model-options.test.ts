...
const globalOptions = { model: 'grok-4', provider: 'xai', providers: [] }
...
describe('manualPickRemoved', () => {
  const providers = [
    ...
    { name: 'xAI', slug: 'xai', models: [] } // present but unconfigured / re-auth
  ]
  ...
  it('never clobbers when the provider has an empty model list (re-auth)', () => {
    expect(manualPickRemoved(providers, 'xai', 'grok-4')).toBe(false)
  })

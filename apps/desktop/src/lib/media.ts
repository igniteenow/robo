...
export async function resolveMediaDisplaySrc(path: string): Promise<string> {
  ...
  if (window.roboDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.roboDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.roboDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
export async function resolveMediaPlaybackSrc(path: string): Promise<string> {
  ...
  if (window.roboDesktop && ['audio', 'video'].includes(mediaKind(path))) {
...
export function mediaStreamUrl(path: string): string {
  return `robo-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// (workspace, skills, ~/.robo/cache, etc.); /api/media is intentionally

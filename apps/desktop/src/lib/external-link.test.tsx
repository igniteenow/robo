...
function installDesktopBridge(partial: Partial<Window['roboDesktop']> = {}) {
...
function installTitleBridge(title: string) {
  ...
  installDesktopBridge({ fetchLinkTitle: bridge as unknown as Window['roboDesktop']['fetchLinkTitle'] })
...
afterEach(() => {
  ...
  if (initialRoboDesktop) {
    ...
    delete desktopWindow.roboDesktop
  }
})
...
describe('external link helpers', () => {
  ...
  it('opens links via the desktop bridge', () => {
    ...
    installDesktopBridge({ openExternal: openExternal as unknown as Window['roboDesktop']['openExternal'] })
  ...
  it('prefixes a pretty link to a known host with its brand glyph', () => {
    ...
    const url = 'https://github.com/IgniteeNow/robo-engineer/pull/123'

...
describe('deriveRemoteAuthProviderShape', () => {
  ...
  it('keeps OAuth copy for redirect providers and mixed deployments', () => {
    expect(
      deriveRemoteAuthProviderShape([{ name: 'xai', displayName: 'xAI', supportsPassword: false }])
    ).toEqual({
      isPassword: false,
      providerLabel: 'xAI'
    })
    ...
    expect(
      deriveRemoteAuthProviderShape([
        ...
        { name: 'xai', displayName: 'xAI', supportsPassword: false }
      ])
    ).toEqual({
      isPassword: false,
      providerLabel: 'Username & Password / xAI'
    })
  })

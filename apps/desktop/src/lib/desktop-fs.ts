import type {
  RoboConnection,
  RoboReadDirResult,
  RoboReadFileTextResult,
  RoboSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: RoboConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: RoboConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<RoboReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<RoboReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<RoboReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<RoboReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: RoboSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()

...
//
// When someone is in a hands-free "Hey Robo" voice chat, the natural way to
...
// phrase (optionally addressed to Robo), so a real turn that merely contains
...
// Optional address prefixes so "robo stop" / "ok stop" / "hey robo, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey robo', 'hey robo,', 'robo', 'robo,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("robo") is not a stop command on its own.
      continue
    }
 ...
 * to Robo). Returns false for anything that merely contains "stop" as part of

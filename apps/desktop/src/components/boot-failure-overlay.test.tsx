...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Robo gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.roboDesktop
  Object.defineProperty(window, 'roboDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'roboDesktop', { configurable: true, value: original })
}

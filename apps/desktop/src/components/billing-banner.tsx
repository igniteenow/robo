 ...
 * disables the composer — slash commands (`/model`, `/login`) stay usable —
 * it only offers recovery: a provider with a billing URL deep-links out,
 * otherwise Settings → Providers opens in-app. The sticky toast is the loud
 * surface; this is the calm reminder that outlives it.
 */
export function BillingBanner({ sessionId }: { sessionId: null | string }) {
  ...
  const title = block.is_igniteenow ? copy.titleIgniteeNow : copy.titleProvider(block.provider_label)

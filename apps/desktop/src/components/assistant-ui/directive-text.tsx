...
const ROBO_REF_TYPES = WIRE_REFERENCE_KINDS
type RoboRefType = (typeof ROBO_REF_TYPES)[number]
...
const ROBO_DIRECTIVE_RE = referenceRe()
...
export const roboDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(ROBO_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...roboDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Robo directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.roboDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.roboDesktop?.readFileDataUrl(id)

...
import { getGlobalModelOptions } from '@/robo'
...
import type { ModelOptionProvider, OAuthProvider } from '@/types/robo'
...
// Curated order mirrors CANONICAL_PROVIDERS: Fireworks sits #1 overall,
...
// onboarding / Providers key form lists every `api_key` provider `robo model`
    ...
    >
      <div
        className={cn(
          ...
          bare
            ? ''
            : 'overflow-hidden rounded-xl border border-(--stroke-igniteenow) bg-(--ui-chat-bubble-background) shadow-igniteenow',
...
const SHOW_ALL_KEY = 'robo-onboarding-show-all-v1'
...
export function Picker({ ctx }: { ctx: OnboardingContext }) {
  ...
  // Collapse the secondary providers behind a disclosure only when a
  ...
  return (
    <div className="grid gap-2">
      <div className="grid max-h-[60dvh] gap-2 overflow-y-auto p-1">
        ...
        {/* Always visible, matching CANONICAL_PROVIDERS (Fireworks is #1). */}

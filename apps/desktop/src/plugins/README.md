...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `RoboPlugin` and
...
`$ROBO_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`robo-desktop-plugins` skill.

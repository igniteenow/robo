import type { GatewayWsUrlResult } from '@robo/shared'
...
declare global {
  interface Window {
    roboDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<RoboConnection>
      ...
      profile: {
        ...
        // backend under the new ROBO_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: RoboApiRequest) => Promise<T>
      notify: (payload: RoboNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<RoboReadFileTextResult>
      selectPaths: (options?: RoboSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<RoboPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<RoboPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<RoboPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: RoboActiveWork) => void
      setTitleBarTheme?: (payload: RoboTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<RoboReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<ROBO_HOME>/desktop-plugins),
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<RoboGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<RoboGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<RoboGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<RoboRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: RoboReviewScope, baseRef?: null | string) => Promise<RoboReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: RoboReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<RoboReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: RoboTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<RoboTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: RoboWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: RoboPreviewFileChanged) => void) => () => void
...
export interface RoboTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface RoboTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `robo update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  roboRoot?: string
...
export interface RoboConnection {
  baseUrl: string
...
export interface RoboTitleBarTheme {
  background: string
  foreground: string
}
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected Robo Cloud org (slug or id) to
...
export interface RoboApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface RoboPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface RoboReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface RoboPreviewWatch {
  id: string
  path: string
}
...
export interface RoboGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface RoboGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface RoboGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface RoboRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface RoboRepoStatus {
  branch: null | string
...
// turn began.
export type RoboReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface RoboReviewFile {
  path: string
  added: number
  removed: number
...
export interface RoboReviewList {
  files: RoboReviewFile[]
...
export interface RoboReviewShipInfo {
  ghReady: boolean
  pr: RoboReviewPr | null
}

export interface RoboReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface RoboReadDirResult {
  entries: RoboReadDirEntry[]
  error?: string
}

export interface RoboPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface RoboSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean

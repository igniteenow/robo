...
const SKIN_KEY = 'robo-desktop-theme-v2'
const MODE_KEY = 'robo-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'robo-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'robo-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'robo-desktop-active-profile-v1'
const RETIRED_SKINS = new Set(['igniteenow-light', 'default', 'gold'])
...
// An exact match wins (so a user theme that happens to share a retired name is
// respected); otherwise a retired or terminal-skin name migrates to its successor.
const normalizeSkin = (name: string | null): string => {
  if (name && resolveTheme(name) && !RETIRED_SKINS.has(name)) {
    return name
  }

  const successor = canonicalSkinName(name)

  return successor && resolveTheme(successor) ? successor : DEFAULT_SKIN_NAME
}
...
export function getBaseColors(skinName: string, mode: 'light' | 'dark'): DesktopThemeColors {
  const seed = resolveTheme(skinName) ?? igniteenowTheme
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  const typo = { ...DEFAULT_TYPOGRAPHY, ...igniteenowTheme.typography, ...theme.typography }
  ...
  root.dataset.roboTheme = skinName
  root.dataset.roboMode = rendered
  ...
  window.roboDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('robo-boot-background', chromeBg)
    window.localStorage.setItem('robo-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.roboThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.roboDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Robo authoring/activating a skin from a

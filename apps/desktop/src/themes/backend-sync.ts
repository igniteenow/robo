/**
 * Live skin sync from the Robo backend.
 *
 * The backend resolves the active skin (built-in or `$ROBO_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Robo
 ...
 */
export function ingestBackendSkin(skin: RoboSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  // (igniteenow), so we never register a converted theme under `default`. It is still a
  // valid apply TARGET though: a runtime switch back to `default` must repaint the
  // desktop to its own default (setTheme normalizes `default` → igniteenow). So we only
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as RoboSkin)

 ...
 *
 * Stamps the Robo icon + identity onto the packed Windows Robo.exe via
 * rcedit (delegated to set-exe-identity.mjs). This runs for EVERY packed build
 * — first install, `robo desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Robo')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Robo'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Robo.exe keeps the stock Electron icon`)
  }
}

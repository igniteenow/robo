...
// no longer bundles the Robo Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Robo.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Robo'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Robo.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Robo'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default ROBO_HOME for non-sandboxed runs -- matches main.ts's
// resolveRoboHome(). On Windows it's %LOCALAPPDATA%\robo; elsewhere
// it's ~/.robo. The fresh-install sandbox launchFresh() sets its own
// ROBO_HOME and never touches this.
const DEFAULT_ROBO_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'robo')
  }
  return path.join(os.homedir(), '.robo')
})()
const VENV_ROOT = path.join(DEFAULT_ROBO_HOME, 'robo-engineer', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'robo-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.ROBO_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Robo-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Robo-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Robo-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Robo-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.ROBO_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.ROBO_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const roboHome = path.join(sandbox, 'robo-home')
  ...
  env.ROBO_DESKTOP_CWD = cwd
  env.ROBO_DESKTOP_IGNORE_EXISTING = '1'
  env.ROBO_DESKTOP_TEST_MODE = 'fresh-install'
  env.ROBO_DESKTOP_USER_DATA_DIR = userDataDir
  env.ROBO_HOME = roboHome
  delete env.ROBO_DESKTOP_ROBO
  delete env.ROBO_DESKTOP_ROBO_ROOT
  ...
  console.log(`  ROBO_HOME: ${roboHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(roboHome, 'robo-engineer', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Robo Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of robo_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'robo-engineer', 'robo_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Robo
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + ROBO_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ROBO_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}

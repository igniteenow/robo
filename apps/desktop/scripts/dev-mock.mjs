...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `robo-dev-mock-${Date.now()}`))
  ...
  return { root, roboHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(roboHome, mockUrl) {
  fs.writeFileSync(
    path.join(roboHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(roboHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.roboHome, mock.url)
  console.log(`  ROBO_HOME: ${sandbox.roboHome}`)
  ...
  const env = {
    ...
    ROBO_DESKTOP_APP_NAME: `RoboDevMock-${Date.now()}`,
  }

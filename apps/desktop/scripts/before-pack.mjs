 ...
 * product name (`Robo`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Robo'
 *
 * This is a hard failure with no obvious cause for the user — `robo desktop`
 ...
 * (the mac-unpacked Robo.app bundle) and Windows (win-unpacked), so we
 ...
 * Robo.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `robo desktop --build-only` (robo_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Robo.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Robo'}.exe`

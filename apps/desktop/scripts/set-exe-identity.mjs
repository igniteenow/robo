#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Robo icon + version metadata onto the
// built Robo.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Robo.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `robo desktop`, the installer's
...
// path (which rebuilds via `robo desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Robo.exe>
//
...
// Stamp the Robo icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Robo',
      FileDescription: 'Robo Desktop Robot',
      InternalName: 'Robo',
      OriginalFilename: 'Robo.exe',
      CompanyName: 'Ignitee Now',
      LegalCopyright: 'Copyright © 2026 Ignitee Now. MIT licensed.'
    }
  })

  console.log('[set-exe-identity] done — Robo icon + identity stamped')
}

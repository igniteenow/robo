...
/// desktop app handing off via `Robo-Setup.exe --update`).
///
...
pub fn run() {
    // Tracing → bootstrap-installer.log under ROBO_HOME/logs/ so install
    ...
    // Robo is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Robo installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Robo is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Robo" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Robo" shortcuts that install.ps1 creates, and a
            // reliable detached relaunch there needs the DETACHED_PROCESS +
            // startup-grace handling used by launch_robo_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::robo_home().join("robo-engineer");
                if bootstrap::robo_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_robo_desktop).
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            tracing::info!(
                                "robo already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
        ...
        .expect("error while running Robo Setup");
}

...
//!
//! Mirrors `robo_constants.get_robo_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\robo
//!   macOS:   ~/.robo
//!   Linux:   ~/.robo  (override via $ROBO_HOME)
//!
//! NOTE (macOS): Python's get_robo_home(), scripts/install.sh, and the
//! Electron desktop's resolveRoboHome() ALL use ~/.robo on macOS — there
...
/// Returns the canonical Robo home directory, respecting $ROBO_HOME if set.
pub fn robo_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("ROBO_HOME") {
    ...
    {
        // %LOCALAPPDATA%\robo — matches scripts/install.ps1's $RoboHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("robo");
        }
    }

    // macOS + Linux + fallback: ~/.robo (matches Python get_robo_home(),
    // install.sh, and the Electron desktop's resolveRoboHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".robo");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    robo_home().join("bootstrap-cache")
}
...
/// ROBO_HOME so it survives repo checkout deletion (unlike anything under
/// robo-engineer/).
///
/// On Windows this is `%LOCALAPPDATA%\robo\robo-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_robo`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under ROBO_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves ROBO_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    robo_home().join(".robo-update-in-progress")
}
...
pub fn copy_self_to_robo_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to ROBO_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_ROBO_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".robo-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under ROBO_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[robo-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("ROBO_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_robo_home() -> String {
    robo_home().to_string_lossy().into_owned()
}

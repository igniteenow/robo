...
export const $roboHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.robo/logs/bootstrap-installer.log')
    $roboHome.set('~/.robo')
  ...
  try {
    const [logPath, roboHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_robo_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $roboHome.set(roboHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Robo" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Robo-Setup.exe --update);
...
export async function launchRoboDesktop(): Promise<void> {
  ...
  await invoke('launch_robo_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Robo repository' },

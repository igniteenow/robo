...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the ROBO_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Robo runtime / deployment
    "ROBO_HOME", "ROBO_PROFILE", "ROBO_GATEWAY_LOCK_DIR",
    "ROBO_MAX_ITERATIONS", "ROBO_MAX_TOKENS", "ROBO_API_TIMEOUT",
    "ROBO_REDACT_SECRETS",
    "_ROBO_GATEWAY",
    # OS / interpreter
    ...
    "ROBO_KANBAN_DB", "ROBO_KANBAN_WORKSPACES_ROOT", "ROBO_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "ROBO_KANBAN_",
    "ROBO_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Robo writes
    ...
    ``robo_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Robo parser: save_env_value
    ...
    from robo_cli.config import _parse_env_value
...
def build_profile_secret_scope(robo_home: Path) -> Dict[str, str]:
    ...
    try:
        from robo_cli.env_loader import get_secret_source_values

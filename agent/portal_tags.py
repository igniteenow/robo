"""Centralized Ignitee Now Portal request tags.

Every Robo request that hits the Ignitee Now Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Ignitee Now can attribute usage to Robo Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=robo-engineer",
        "client=robo-client-v<__version__>",
    ]

The version is sourced live from ``robo_cli.__version__`` so it auto-aligns
to whatever release is installed; bumping that single string at release time
means every Portal request picks up the new tag on the next process start.
...
``robo_cli.__version__`` is the canonical source of truth.
"""
...
# active conversation id here and ``igniteenow_portal_tags()`` picks it up as a
...
_conversation_id: ContextVar[Optional[str]] = ContextVar(
    "igniteenow_portal_conversation_id", default=None
)
...
def _robo_version() -> str:
    """Return the current Robo release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``robo_cli`` cannot be imported (should
...
def robo_client_tag() -> str:
    """Return the ``client=...`` tag for Ignitee Now Portal requests.

    Format: ``client=robo-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=robo-client-v{_robo_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Robo session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Robo conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.robo/sessions/`` storage, session logs, and lineage.
...
def igniteenow_portal_tags(session_id: str | None = None) -> List[str]:
    """Return the canonical list of Ignitee Now Portal product tags.

    Always returns a fresh list so callers can mutate it freely
    (e.g. ``merged_extra.setdefault("tags", []).extend(igniteenow_portal_tags())``).

    When ``session_id`` is provided, a ``conversation=<session_id>`` tag is
    appended so Portal usage can be attributed to a specific Robo
    ...
    """
    tags = ["product=robo-engineer", robo_client_tag()]

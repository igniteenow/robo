...
as ``kind: backend``) or ``~/.robo/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``robo tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$ROBO_HOME/cache/videos/``, creating parents as needed."""
    from robo_constants import get_robo_home

    path = get_robo_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$ROBO_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$ROBO_HOME/cache/videos/``.

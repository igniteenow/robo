...
class BillingBlock:
    ...
    ``is_igniteenow`` is the routing bit: Ignitee Now has a first-class in-app billing surface
...
def is_igniteenow_inference_route(provider: str, base_url: str) -> bool:
    """True when the failing route is the Ignitee Now-managed inference gateway."""
    if (provider or "").strip().lower() == "igniteenow":
        return True
    return base_url_host_matches(str(base_url or ""), "inference-api.igniteenow.com")


def _igniteenow_billing_url() -> Optional[str]:
    """Best-effort Ignitee Now portal billing URL (text-surface fallback; Ignitee Now prefers the in-app flow)."""
    try:
        from robo_cli.igniteenow_account import igniteenow_portal_billing_url

        return igniteenow_portal_billing_url(None)
    except Exception:
        return "https://portal.igniteenow.com/billing"
...
def build_billing_block(
    *,
    ...
    if is_igniteenow_inference_route(slug, base_url):
        return BillingBlock(slug or "igniteenow", "Ignitee Now Portal", model, _igniteenow_billing_url(), True, message or "")

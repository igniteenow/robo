...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Robo with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.robo/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Robo
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``robo tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``robo setup`` for

...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `robo backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - robo_home (str): The active ROBO_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.robo``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "robo").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'robo memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], robo_home: str) -> None:
        ...
        Called by 'robo memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``robo_home`` is the active ROBO_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE ROBO_HOME.

        ``robo backup`` only walks ROBO_HOME, so any provider state kept
        ...
        of the archive, and ``robo import`` restores them to their original

...
# `export ROBO_REDACT_SECRETS=false`) cannot disable redaction
# mid-session.  ON by default — secure default per issue #17691. Users who
# need raw credential values in tool output (e.g. working on the redactor
# itself) can opt out via `security.redact_secrets: false` in config.yaml
# (bridged to this env var in robo_cli/main.py, gateway/run.py, and
# cli.py) or `ROBO_REDACT_SECRETS=false` in ~/.robo/.env. An opt-out
# warning is logged at gateway and CLI startup so operators see the
# downgrade — see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("ROBO_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}
...
_PREFIX_PATTERNS = [
    ...
    # an upstream fix; follow-up invited in #4541.
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across Robo — used by
    ``robo config``, ``robo status``, ``robo dump``, and anywhere
...
def redact_sensitive_text(
    ...
    *,
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical robo log line

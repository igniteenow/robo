...
Resolution order for text tasks (auto mode):
  ...
  3. Ignitee Now Portal (~/.robo/auth.json active provider)
...
from robo_constants import OPENROUTER_BASE_URL
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``ROBO_CA_BUNDLE`` /
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `robo update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Robo owns auxiliary retry + provider/model fallback policy (the
    ...
    # Robo sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Robo control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="robo-protected-aux-provider",
        daemon=True,
    ).start()
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Robo triggers summarization.  Higher values delay
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``ROBO_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``ROBO_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("ROBO_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("ROBO_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "RoboAgent",
}
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Robo Agent)",
...
def _pool_runtime_api_key(entry: Any) -> str:
    ...
    # provider-specific fallback (e.g. agent_key for igniteenow).
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    # runtime_base_url handles provider-specific logic (e.g. igniteenow prefers inference_base_url).
...
class _AnthropicCompletionsAdapter:
    ...
    def __init__(
        ...
        # pre-strip Portal ``.../v1`` form). No blanket fallback to the SDK
        # client's host — that would flip MiniMax/Zhipu/etc. aux adapters
        # from "unknown host = native Anthropic" to third-party (stripping
        # thinking signatures).
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Robo plumbing (_reasoning_config
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``robo_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``robo auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``robo_cli.auth``'s singleton runtime resolver for older
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Robo auth store.
    ...
    try:
        from robo_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from robo_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from robo_cli.auth import is_provider_explicitly_configured
...
# Default OpenRouter model for the auxiliary auto-chain fallback when
# auxiliary.openrouter_model is unset. A PAID model by design — this
# preserves the historical paid fallback; see config_defaults.py's
# own documentation of the openrouter_model key for the full rationale
# and how to switch to a :free SKU via auxiliary.openrouter_model.
...
def _aux_openrouter_settings() -> Tuple[bool, str]:
    ...
    """
    try:
        from robo_cli.config import cfg_get, load_config_readonly
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `robo setup` or `robo model` and enter a valid http(s) base URL."
        ) from exc


# The Codex backend-api base URL — matches robo_cli.auth.DEFAULT_CODEX_BASE_URL,
# the single canonical value used throughout the codebase for this endpoint.
from robo_cli.auth import DEFAULT_CODEX_BASE_URL as _CODEX_AUX_BASE_URL
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = _create_openai_client(
        api_key=api_key,
        base_url=base_url,
        default_headers=robo_xai_default_headers(),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _try_azure_foundry(
    *,
    ...
    :func:`robo_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from robo_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# The official Anthropic API base URL — matches the same default used
# throughout robo_cli/auth.py and robo_cli/runtime_provider.py.
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second robo
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``robo config aux reset``)."""
...
def _is_model_not_found_error(exc: Exception) -> bool:
    ...
    that has since been dropped from the Ignitee Now → OpenRouter catalog. The Ignitee Now
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_robo_fallback_destination", None)
...
def _replan_synchroigniteenow_cache_sections(
    messages: list,
    tools: Optional[list],
    *,
...
def _call_fallback_candidate_sync(
    ...
    *,
    ...
    fallback_messages, fallback_tools = _replan_synchroigniteenow_cache_sections(
        messages,
        tools,
    ...
    try:
        ...
        if fb_provider not in {"auto", "", None} and _refresh_provider_credentials(fb_provider):
            ...
            if retry_client is not None:
                ...
                retry_messages, retry_tools = _replan_synchroigniteenow_cache_sections(
                    messages,
                    tools,
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``robo_cli.fallback_config.resolve_entry_api_key``).
    """
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        try:
            client._robo_fallback_destination = _fallback_destination_from_entry(
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Robo' built-in discovery
...
def _resolve_auto(
    ...
    Priority:
      1. User's main provider + main model, regardless of provider type.
         ...
         chat.  Users on OpenRouter/Ignitee Now get their chosen chat model; users
    ...
    #    scenario where a user switches providers via `robo model` but the
    #    old OPENAI_BASE_URL lingers in ~/.robo/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: robo model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.robo/.env",
    ...
    # on aggregators (OpenRouter, Ignitee Now) who previously got routed to a
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        async_kwargs["default_headers"] = robo_xai_default_headers()
    else:
    ...
    # See _create_openai_client: disable SDK-internal retries so Robo owns
...
def resolve_provider_client(
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: robo model)")
                return None, None
    ...
    # OpenRouter / Ignitee Now bills for side tasks they thought were running on
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: robo model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        ...
        # entries that coincidentally match a canonical provider (e.g. ``igniteenow``)
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: robo doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("robo_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            headers.update(robo_xai_default_headers())
        else:
            # Fall back to profile.default_headers for providers that declare
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #      :func:`_resolve_provider_vision_default`). Ignitee Now is the
        ...
        #   3. Ignitee Now Portal (vision-capable aggregator fallback)
...
def get_auxiliary_extra_body() -> dict:
    ...
    Always empty now — this previously carried Ignitee Now Portal product tags
    when the auxiliary client was backed by Ignitee Now Portal, which is no
    longer an available provider.
    """
    return {}
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`robo_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def _build_call_kwargs(
    ...
    # body fields (Gemini ``thinking_config``), and OpenRouter/Ignitee Now use
    ...
    # Anthropic Messages adapters translate Robo reasoning into native
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Robo reasoning config for direct model calls
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `robo model`."
                    )
    ...
    try:
        ...
        # When the resolved provider returns 401 and neither the Ignitee Now

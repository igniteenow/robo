...
def strip_think_blocks(agent, content: str) -> str:
    ...
    Ported from an upstream fix. The ``<function>`` variant is
    ...
    # 1b. Tool-call XML blocks (an upstream fix). Handle the
...
def try_recover_primary_transport(
    ...
    Skipped for proxy/aggregator providers (OpenRouter, Ignitee Now) which
    ...
    if (
        provider_lower in {"igniteenow", "igniteenow-portal", "igniteenow"}
...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-Robo-Session-Id header), and an
        ...
        if env_var_enabled("ROBO_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            from robo_cli.config import load_config as _load_moa_cfg
    ...
    # Ignitee Now Portal proxies to OpenRouter behind the scenes — identical
    # OpenAI-wire envelope cache_control semantics. Treat it as an
    # OpenRouter-equivalent endpoint for caching layout purposes.
    is_igniteenow_portal = "igniteenow" in eff_base_url.lower()
    ...
    if (
        (is_openrouter or is_igniteenow_portal)
        and (is_claude or is_kimi)
        and not is_anthropic_wire
    ):
        return True, False
    # Ignitee Now Portal Qwen (e.g. qwen3.6-plus) takes the same envelope-layout
    ...
    if is_igniteenow_portal and "qwen" in model_lower:
        return True, False
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Delegate all rate-limit / 5xx retry to robo's outer conversation loop,
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    turn-scoped).
    """
    from robo_cli.providers import determine_api_mode

    # ── Determine api_mode if not provided ──
    # Pass model so dual-wire providers (Ignitee Now Portal anthropic/* → Messages)
    ...
    try:
        ...
        from robo_cli.config import load_config as _sm_load_config
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    try:
        from robo_cli.middleware import apply_tool_request_middleware
    ...
    if not pre_tool_block_checked:
        try:
            from robo_cli.plugins import resolve_pre_tool_block
    ...
    from robo_cli.middleware import run_tool_execution_middleware
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: robo' own dispatch loop intentionally keeps an
...
def _iter_httpx_pool_objects(http_client: Any):
    ...
    Robo' keepalive client (#10324 / ``_build_keepalive_http_client``) and
...
def apply_pending_claim_verification(
    agent, assistant_message, messages: list, num_tool_msgs: int
) -> None:
    """Check the assistant's own text for a risky/definitive claim and, if
    an auxiliary-LLM second opinion flags it, append a note to the last
    tool result — same delivery slot as steer, same reasoning (see
    apply_pending_steer_to_tool_results above), different, distinctly
    labeled source (agent/prompt_builder.py's CLAIM_VERIFICATION_MARKER,
    not the user's own STEER_MARKER).

    Off by default (agent._claim_verification); this is an active check
    that spends a real auxiliary-LLM call, not free prompt guidance, so it
    only runs at all when explicitly enabled.

    Unlike steer, there is no external producer accumulating a pending
    queue on a separate thread — detection and the auxiliary check both
    happen synchronously, right here, driven entirely by this call. If
    there's no tool result in this batch to attach to (e.g. everything was
    skipped by an interrupt), the note is dropped rather than queued for
    next-turn delivery: that's a rare edge case, and if the agent did no
    tool work this turn its next response is likely the turn's own final
    answer anyway, where there is no further "next step" for a warning to
    usefully precede.

    Args:
        assistant_message: The message whose .content is scanned.
        messages: The running messages list.
        num_tool_msgs: Number of tool results appended in this batch;
            used to locate the tail slice safely.
    """
    if not getattr(agent, "_claim_verification", False):
        return
    if num_tool_msgs <= 0 or not messages:
        return
    ...
    try:
        ...
        check = detect_risky_claim(text)
        if not check.triggered:
            return
        note = verify_claim(check.claim_text, check.trigger_kind, messages)
        if not note:
            return
    except Exception as e:
        # Fails open: a claim-verification error must never break the turn.
        _ra().logger.debug("Claim verification: check failed (%s), skipping", e)
        return
    ...
    if target_idx is None:
        _ra().logger.debug(
            "Claim verification flagged a claim but no tool result was "
            "available to attach the note to this turn; dropping it: %s",
            note[:120],
        )
        return

    from agent.prompt_builder import format_claim_verification_marker

    marker = format_claim_verification_marker(note)
    existing_content = messages[target_idx].get("content", "")
    if not isinstance(existing_content, str):
        try:
            blocks = list(existing_content) if existing_content else []
        ...
        messages[target_idx]["content"] = existing_content + marker
    _ra().logger.info(
        "Claim verification flagged a %s claim and delivered a note (%d chars): %s",
        check.trigger_kind,
        len(note),
        note[:120] + ("..." if len(note) > 120 else ""),
    )

...
# un-throttled log fires on *every* selection. On Windows several Robo
...
# readiness handshake ("Timed out connecting to Robo backend after
...
def _write_through_provider_state_to_global_root(
    ...
    (#48415 / #43589): igniteenow, openai-codex, and xai-oauth rotate the
    ...
    ``robo_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``robo model`` / ``robo auth`` which
        ...
        Mirrors the Ignitee Now/Anthropic resync paths above.  Only applies to
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Robo process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently Ignitee Now, OpenAI Codex, and xAI Grok OAuth).
        ...
        here would mean every Ignitee Now/Codex/xAI refresh in a multi-provider
        setup silently flips the ``active_provider`` flag — the next
        ``robo`` invocation that defaults to the active provider
        (e.g. setup wizard, ``robo auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Robo
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``ROBO_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "ROBO_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "ROBO_XAI_REFRESH_TIMEOUT_SECONDS"
        )
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("robo_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Robo
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("robo_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
                    ...
                    return synced
            # For xai-oauth: same race as igniteenow — another process may have
            ...
            if self.provider == "xai-oauth":
                ...
                # pool. Mirrors the Ignitee Now quarantine path above.
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/igniteenow — another Robo process
            ...
            if self.provider == "openai-codex":
                ...
                # in-memory pool.  Mirrors the xAI and Ignitee Now quarantine paths.
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Robo profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `robo model` / `robo auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `robo model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``robo auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `robo auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `robo auth remove <provider> <N>` is stable across all source types.
    try:
        from robo_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Robo PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `robo setup` ("Claude
        ...
        # Claude Code CLI, robo_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.robo/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `robo setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"robo_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_robo_oauth_credentials

        for source_name, creds in (
            ("robo_pkce", read_robo_oauth_credentials()),
        ...
        try:
            ...
            # (robo auth remove copilot gh_cli) must not pay either on
        ...
        try:
            from robo_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.robo/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `robo auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `robo auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Robo auth store.  Without this gate the removal is instantly
        ...
        # Robo owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``robo model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``robo auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            # always surfaced as ``device_code`` (consistent with igniteenow/codex).
            source = "device_code"
            if _is_suppressed(provider, source):
                return changed, active_sources
            active_sources.add(source)
            from robo_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
# Prefer ~/.robo/.env over os.environ — the user's config file is the
# authoritative source for Robo credentials. Stale env vars from parent
...
def get_env_prefer_dotenv(key: str) -> str:
    ...
    # load_robo_dotenv() -> apply_onepassword_secrets()).  The raw
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `robo auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.robo/.env.
    ...
    try:
        from robo_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from robo_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.robo/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `robo auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Robo

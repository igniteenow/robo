...
def _ensure_file_checkpoint(
    ...
    # which can differ from the Robo process cwd (notably in Docker).  Resolve
...
def _resolve_concurrent_tool_timeout() -> float | None:
    raw = os.getenv("ROBO_CONCURRENT_TOOL_TIMEOUT_S", "").strip()
    if not raw:
    ...
    try:
        ...
        logger.warning(
            "invalid ROBO_CONCURRENT_TOOL_TIMEOUT_S=%r; using %.0fs",
            raw,
...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    current Robo process before the normal turn-end persistence path runs.
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    """Run Relay rewrites before Robo policy and dispatch exactly once."""
    from agent import relay_tools
    from robo_cli.middleware import (
    ...
    def _authorized_dispatch(final_args: dict[str, Any]) -> Any:
        ...
        # Approval barrier (shared by the sequential and concurrent paths):
        # never dispatch a tool while a human approval prompt is open for
        # this session. The concurrent path also checks inside its worker
        # closure; this covers the sequential path, delegated children and
        # every other caller that funnels through this middleware.
        try:
    ...
    def _robo_pipeline(relay_args: dict[str, Any]) -> Any:
...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    for tool_call in tool_calls:
        ...
        # the reference lesson: hooks must observe the real tool name.
        #
        ...
        try:
            try:
                def _execute(next_args: dict[str, Any]) -> Any:
                    # Full stop while ANY sibling in this session has a human
                    # approval decision pending — not just other calls that
                    # also need approval. Previously a safe, unrelated call
                    # ran freely alongside a pending prompt; the user found
                    # that confusing (it looked like the prompted command was
                    # itself running unanswered) and asked for a hard pause
                    # instead. Same bounded-wait shape as the approval system's
                    # own sibling-coordination wait (tools/approval.py's
                    # _await_gateway_decision): bounded by human_wait_ceiling()
                    # so a stuck/leaked pending state can't park this call
                    # forever, and interrupt-aware so Ctrl+C/cancel doesn't
                    # sit through it. On expiry it proceeds anyway — same
                    # tradeoff used everywhere else in this file (start-order
                    # gate, authorization gate): worst case is one call
                    # running earlier than intended, strictly better than a
                    # permanent hang.
                    try:
                        from tools.approval import (
                            get_current_session_key,
                            human_wait_ceiling,
                            is_human_wait_pending,
                        )
                        from tools.interrupt import is_interrupted

                        _session_key = get_current_session_key()
                        _deadline = time.monotonic() + human_wait_ceiling()
                        while is_human_wait_pending(_session_key):
                            if is_interrupted() or time.monotonic() >= _deadline:
                                break
                            time.sleep(0.1)
                    except Exception:
                        # Never let this coordination check itself be the
                        # reason a tool call fails to run.
                        pass
    ...
    if finalize and num_tools > 0:
        ...
        agent._apply_pending_claim_verification(assistant_message, messages, num_tools)
...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    if finalize and num_tools_seq > 0:
        ...
        agent._apply_pending_claim_verification(assistant_message, messages, num_tools_seq)
...
def execute_tool_calls_segmented(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, segments=None) -> None:
    ...
    *k+1..n* without executing them while preserving one result per
    ...
    if total_tools > 0:
        ...
        agent._apply_pending_claim_verification(assistant_message, messages, total_tools)

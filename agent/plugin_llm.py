...
Plugins built on Robo Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Robo
...
:class:`~robo_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Robo' main config
...
handles every provider, fallback chain, and per-task override Robo
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`robo_cli.plugins.PluginContext`

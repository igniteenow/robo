...
upstream prefix cache warm.  See ``robo-engineer-dev``'s
...
* ``stable``   — identity (SOUL.md or DEFAULT_AGENT_IDENTITY), tool
  guidance, computer-use guidance, igniteenow subscription block, tool-use
...
def _ra():
    ...
    ``build_context_files_prompt``, ``build_igniteenow_subscription_prompt``,
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``ROBO_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("ROBO_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Robo never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want ROBO_HOME persona while keeping
    ...
    # Pointer to the robo-engineer skill + docs for user questions about Robo itself.
    stable_parts.append(ROBO_AGENT_HELP_GUIDANCE)
    ...
    # Universal task-focus / long-horizon guidance. Complements the block
    # above — see agent/prompt_builder.py's TASK_FOCUS_GUIDANCE docstring
    # for why this is a separate block rather than folded into it.
    if getattr(agent, "_task_focus_guidance", True) and agent.valid_tool_names:
        stable_parts.append(TASK_FOCUS_GUIDANCE)

    # Universal verification / anti-premature-conclusion guidance. See
    # agent/prompt_builder.py's VERIFICATION_GUIDANCE docstring for why this
    # is a third, distinct block rather than folded into either above.
    if getattr(agent, "_verification_guidance", True) and agent.valid_tool_names:
        stable_parts.append(VERIFICATION_GUIDANCE)
    ...
    # ROBO_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Claim verification uses the same delivery channel, but — unlike
    # steer, which is always live — this feature has its own toggle
    # (default off; see agent/claim_verification.py). Gated so a disabled
    # feature doesn't spend prompt tokens explaining a marker the model
    # will never actually see.
    if agent.valid_tool_names and getattr(agent, "_claim_verification", False):
        stable_parts.append(CLAIM_VERIFICATION_CHANNEL_NOTE)
    ...
    igniteenow_subscription_prompt = _r.build_igniteenow_subscription_prompt(agent.valid_tool_names)
    if igniteenow_subscription_prompt:
        stable_parts.append(igniteenow_subscription_prompt)
    ...
    # Coding posture (base Robo, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Robo profile the agent is running
    # under so it doesn't conflate ~/.robo/skills/ (default profile) with
    # ~/.robo/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        post_workspace_parts.append(
            ...
            "under " + str(get_robo_home()) + "/profiles/<name>/. Each profile has its own "
        ...
        post_workspace_parts.append(
            f"Active Robo profile: {active_profile}. This session reads "
            f"and writes {get_robo_home()}/profiles/{active_profile}/. The default "
            f"profile's data lives at {get_robo_home()}/skills/, {get_robo_home()}/plugins/, "
            f"{get_robo_home()}/cron/, {get_robo_home()}/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Robo). Every other surface (desktop chat panel,

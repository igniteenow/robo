"""Unified removal contract for every credential source Robo reads from.

Robo seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.robo/.env
    claude_code   — ~/.claude/.credentials.json
    robo_pkce   — ~/.robo/.anthropic_oauth.json
...
unify here is **removal**:
    ...
    ``robo auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, robo_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.robo/.env  → clear it
    ...
    """
    from robo_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Robo.",
            f"  The pool entry is now suppressed — Robo will ignore "
            f"{env_var} until you run `robo auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Robo stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `robo auth add anthropic` to re-enable if needed.",
    ])


def _remove_robo_pkce(provider: str, removed) -> RemovalResult:
    """~/.robo/.anthropic_oauth.json is ours — delete it outright."""
    from robo_constants import get_robo_home

    result = RemovalResult()
    oauth_file = get_robo_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Robo Anthropic OAuth credentials")
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    ...
    Single-source OAuth state with refresh tokens; suppression of the
    `oauth` source ensures the pool reseed path (_seed_from_singletons)
    doesn't instantly undo the removal.
    """
    result = RemovalResult()
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``robo auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `robo model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Robo auth store is not enough — _seed_from_singletons() would
    ...
    via ``robo auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `robo auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `robo auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Robo stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `robo auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="robo_pkce",
        remove_fn=_remove_robo_pkce,
        description="~/.robo/.anthropic_oauth.json",
    ))
    register(RemovalStep(

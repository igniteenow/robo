"""Cross-session rate limit guard for Ignitee Now Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether Ignitee Now Portal is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 from Ignitee Now triggers up to 9 API calls per conversation turn
(3 SDK retries x 3 Robo retries), and every one of those calls counts
...
_STATE_FILENAME = "igniteenow.json"


def _state_path() -> str:
    """Return the path to the Ignitee Now rate limit state file."""
    try:
        from robo_constants import get_robo_home
        base = get_robo_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".robo")
...
def record_igniteenow_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
) -> None:
    """Record that Ignitee Now Portal is rate-limited.
    ...
    try:
        ...
        logger.info(
            "Ignitee Now rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write Ignitee Now rate limit state: %s", exc)


def igniteenow_rate_limit_remaining() -> Optional[float]:
    """Check if Ignitee Now Portal is currently rate-limited.
...
def clear_igniteenow_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful Ignitee Now request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear Ignitee Now rate limit state: %s", exc)
...
def is_genuine_igniteenow_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    last_known_state: Optional[Any] = None,
) -> bool:
    """Decide whether a 429 from Ignitee Now Portal is a real account rate limit.

    Ignitee Now Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Robo, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on Ignitee Now is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on Ignitee Now.

    Tripping the cross-session breaker on (b) blocks ALL Ignitee Now requests
    (and all models, since Ignitee Now is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  Ignitee Now emits

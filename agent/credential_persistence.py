...
``auth.json``.  They intentionally have no dependency on ``robo_cli.auth`` so
...
# Sources Robo owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Robo-owned OAuth/device-code state)

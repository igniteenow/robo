...
projection into Robo' display, approval bridging, transcript projection into
...
"codex_app_server"`. Robo' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Robo secret — gateway
        ...
        spawn_env = robo_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("ROBO_KANBAN_TASK"):
            kanban_db = spawn_env.get("ROBO_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "ROBO_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("ROBO_HOME", os.path.expanduser("~/.robo")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "robo",
        client_title: str = "Robo Agent",

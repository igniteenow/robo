...
Owns one Codex thread per Robo session. Drives `turn/start`, consumes
...
# Robo' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ROBO_TO_CODEX_PERMISSION_PROFILE = {
...
class TurnResult:
    ...
    # of riding a CPU-spinning or auth-broken process. Retires timed-out
    # app-server clients rather than letting them linger.
...
# normal completion path fires. Mirrors an earlier implementation fix.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Robo/OpenAI rich content into app-server text input.
...
# original error verbatim. Mirrors an earlier implementation's auth-refresh
# classification.
...
class CodexAppServerSession:
    """One Codex thread per Robo session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ROBO_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ROBO_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="robo",
            client_title="Robo Agent",
            client_version=_get_robo_version(),
        )
        ...
        # serialized this under either key. Mirrors an earlier implementation's
    ...
    def run_turn(
        ...
        *,
        ...
        Mirrors an earlier implementation's post-tool completion watchdog (#81697)
        ...
        # supports rich content but Robo' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Robo' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own robo-tools callback we
            # auto-accept — the user already approved Robo' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Robo
        approval-mode/timeout logic. The Robo-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Robo approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Robo approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Robo returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Robo approval mode/timeout resolution
    ...
    # "prompt expired"; the Robo-side messaging already distinguishes them.
    return "decline"
...
def _has_turn_aborted_marker(text: str) -> bool:
    ...
    an earlier implementation's terminal-marker fix so we don't burn the full turn
...
def _get_robo_version() -> str:
    """Best-effort Robo version string for codex's userAgent line."""
    try:

...
appropriate.  All installs go to a Robo-owned bin staging dir,
``<ROBO_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``robo lsp
  status``.
...
# ``<ROBO_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `robo lsp status` reports the host's presence.
...
def robo_lsp_bin_dir() -> Path:
    """Return the Robo-owned bin staging dir for LSP servers."""
    from robo_constants import get_robo_home

    p = get_robo_home() / "lsp" / "bin"
...
def _install_npm(
    ...
    """
    # Managed npm first: $ROBO_HOME/node is not on an arbitrary process's
    # PATH, so a bare which() misses the Node that Robo installed and
    ...
    if npm is None:
        ...
        return None
    staging = robo_lsp_bin_dir().parent  # <ROBO_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a robo-owned target dir.
    ...
    console script.
    """
    pip_target = robo_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = robo_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``robo lsp status`` CLI to give users a quick

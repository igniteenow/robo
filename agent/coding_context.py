"""Coding-context awareness — base Robo, every interactive surface.

When the user runs Robo inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Robo shifts into a **coding posture**. This module is the
...
from robo_cli._subprocess_compat import bounded_git_probe

logger = logging.getLogger("robo.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         # "hermes" is a MODEL family needle (nousresearch/hermes-*), not the
         # product name — keep it as-is through any rebrand.
...
# in _ROBO_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``ROBO_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Robo should operate in its coding posture right now."""

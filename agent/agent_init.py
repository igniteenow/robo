...
from robo_cli.timeouts import get_provider_request_timeout
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from robo_cli.providers import ROBO_OVERLAYS, get_provider

        overlay = ROBO_OVERLAYS.get(provider)
    ...
    try:
        ...
        from robo_cli.models import normalize_provider as normalize_model_provider
        from robo_cli.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: robo config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$ROBO_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_robo_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$ROBO_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .robo.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / ROBO_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.robo/SOUL.md as the primary
    ...
    if api_mode in {"chat_completions", "codex_responses", "anthropic_messages", "bedrock_converse", "codex_app_server"}:
    ...
    elif agent.provider in {"igniteenow", "igniteenow-portal", "igniteenow"}:
        ...
        from robo_cli.providers import igniteenow_api_mode

        agent.api_mode = igniteenow_api_mode(agent.model)
    else:
    ...
    agent._cache_ttl = "5m"
    try:
        from robo_cli.config import load_config_readonly as _load_pc_cfg
    ...
    # x-igniteenow-credits-* response headers after each API call.  Session-start
    # remaining is latched the first time a header is ever seen so we can
    # report cumulative micros spent.  Surfaced behind ROBO_DEV_CREDITS.
    ...
    # both live under ~/.robo/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from robo_logging import setup_logging, setup_verbose_logging
    setup_logging(robo_home=_ra()._robo_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that robo_logging.setup_logging() does not
        ...
        # handler level inside robo_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from robo_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from robo_cli.config import load_config_readonly as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if base_url_host_matches(effective_base, "openrouter.ai"):
                ...
                client_kwargs["default_headers"] = robo_xai_default_headers()
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `robo model` to "
                        "select a provider, or run `robo setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff ROBO_KANBAN_TASK is set).
    ...
    try:
        ...
        if not delegated_child:
            os.environ["ROBO_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.robo/sessions/ alongside gateway sessions
    robo_home = get_robo_home()
    agent.logs_dir = robo_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.robo/sessions/session_{sid}.json)
    ...
    try:
        from robo_cli.config import load_config_readonly as _load_sess_cfg
    ...
    # `robo --resume <id>` can restore the bypass (CLI resume paths read
    ...
    try:
        from robo_cli.config import load_config_readonly as _load_agent_config
    ...
    # management API (the historical Robo behavior) or left to LM Studio's
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "robo"
                    except Exception:
                        pass
    ...
    # Universal task-focus / long-horizon guidance toggle.  Default True.
    # Separate flag from task_completion_guidance — a user may want one but
    # not the other. Steers the model to finish the assigned task instead of
    # context-switching to tangents, and to treat a genuinely long task as
    # normal rather than a reason to wrap it up early.
    agent._task_focus_guidance = bool(_agent_section.get("task_focus_guidance", True))

    # Universal verification guidance toggle.  Default True.  A third,
    # separate block from the two above — steers the model to verify claims
    # about code/system behavior before stating them, and to try a concrete
    # alternative before concluding a task can't be done.
    agent._verification_guidance = bool(_agent_section.get("verification_guidance", True))

    # Active claim verification toggle.  Default FALSE — unlike the three
    # guidance blocks above, which are free prompt text, this spends a real
    # auxiliary-LLM call whenever it triggers (agent/claim_verification.py).
    # Off by default so it never surprises anyone with per-turn cost;
    # explicitly opt in via config.yaml agent.claim_verification: true.
    agent._claim_verification = bool(_agent_section.get("claim_verification", False))
    ...
    # Lets an enterprise admin append to or replace Robo' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "robo", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, robo, off.",
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from robo_cli.auth import resolve_provider as resolve_auth_provider
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from robo_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from robo_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Robo Agent.  Choose a model with at least "
    ...
    # Ignitee Now Robo 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from robo_cli.model_switch import _check_robo_model_warning

            _robo_warn = _check_robo_model_warning(agent.model or "")
            if _robo_warn:
                _user_msg = (
                    "⚠ Ignitee Now Robo 3 & 4 models are NOT agentic — they "

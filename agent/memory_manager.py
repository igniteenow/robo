...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_ROBO_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Robo expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``robo_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_robo_home()`` themselves.
        """
        if "robo_home" not in kwargs:
            from robo_constants import get_robo_home
            kwargs["robo_home"] = str(get_robo_home())
        for provider in self._providers:
            try:

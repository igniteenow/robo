...
cache problem must never block Robo startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Robo home used for cache paths.

    ``home_path`` is whatever ``load_robo_dotenv()`` already resolved;
    falling back to ``$ROBO_HOME`` / ``~/.robo`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from robo_constants import get_robo_home

        home_path = get_robo_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<robo_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if ROBO_HOME is ever made traversable.
            try:

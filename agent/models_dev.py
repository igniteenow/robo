...
Data resolution order:
  ...
  2. Disk cache (~/.robo/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Robo ↔ models.dev
# ---------------------------------------------------------------------------

# Robo provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Robo (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_robo_home() / "models_dev_cache.json"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``robo config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Robo provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Robo agent traffic.
...
def get_provider_info(
    ...
    Accepts either a Robo provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Robo ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Robo or models.dev provider ID.  Tries exact match then

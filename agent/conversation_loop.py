...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in robo_state.py — kept local
# to avoid importing robo_state at module load time (its module-level
# DEFAULT_DB_PATH = get_robo_home() / "state.db" breaks tests that
# monkeypatch get_robo_home to return a str).
...
from robo_constants import PARTIAL_STREAM_STUB_ID
from robo_logging import set_session_context
...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    mechanism as the ~/.robo/prefill.json incident, 20/20 blocked with
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Robo tool use: "
    ...
    return (
        ...
        f"context, but Robo needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Robo config, set `model.ollama_num_ctx: 65536` "
...
def _igniteenow_entitlement_message(capability: str) -> str:
    try:
        from robo_cli.igniteenow_account import (
            format_igniteenow_portal_entitlement_message,
            get_igniteenow_portal_account_info,
        )

        account_info = get_igniteenow_portal_account_info(force_fresh=True)
...
def _print_igniteenow_entitlement_guidance(agent, capability: str) -> bool:
    message = _igniteenow_entitlement_message(capability)
    if not message:
        return False
...
def _is_igniteenow_inference_route(provider: str, base_url: str) -> bool:
    provider = (provider or "").strip().lower()
    if provider == "igniteenow":
        return True
    base = str(base_url or "")
    return (
        base_url_host_matches(base, "inference-api.igniteenow.com")
    )
...
def _billing_or_entitlement_message(
    *,
    ...
    if _is_igniteenow_inference_route(provider, base_url):
        return _igniteenow_entitlement_message(capability)
...
def _try_refresh_igniteenow_paid_entitlement_credentials(agent) -> bool:
    """Refresh Ignitee Now runtime credentials after a fresh paid-entitlement check."""
    try:
        from robo_cli.igniteenow_account import get_igniteenow_portal_account_info

        account_info = get_igniteenow_portal_account_info(force_fresh=True)
        if account_info.paid_service_access is not True:
            return False
        return agent._try_refresh_igniteenow_client_credentials(
            force=True,
        )
    except Exception:
        return False
...
def _stored_prompt_matches_runtime(agent, prompt: str) -> bool:
    ...
    def host_info_value(label: str) -> str:
        ...
        occurrence, so only Robo' own emitted block can satisfy the read.
        """
        prefix = f"{label}:"
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `robo fallback add`."
)
...
def run_conversation(
    ...
    if moa_config is None:
        try:
            from robo_cli.moa_config import decode_moa_turn
    ...
    # Adopt any ~/.robo/.env credential/base-url edits made since the last
    ...
    # all run inside Codex). Default Robo path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        # Approval barrier: if a human approval prompt is open for this
        # session (raised by a tool on another thread — a concurrent sibling,
        # a delegated child, an execute_code RPC), do not start another model
        # call. The prompt on screen must mean the agent is stopped, not that
        # one command is stopped while the model keeps planning around it.
        try:
            from tools.approval import wait_for_approval_barrier
            wait_for_approval_barrier()
        except Exception:
            pass
        ...
        if _redirect_text:
            ...
            # A redirect that landed while tools were executing rides on the
            # interrupt machinery (it cancels the running batch so the model
            # reads the new instruction now instead of after the batch). The
            # correction is applied; clear the cancellation bit so the loop
            # continues with a fresh model call instead of breaking out.
        ...
        # cache prefix.  The system prompt is reserved for Robo internals.
        #
        # Robo invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Robo tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            # ── Ignitee Now Portal rate limit guard ──────────────────────
            # If another session already recorded that Ignitee Now is rate-
            # limited, skip the API call entirely.  Each attempt
            # (including SDK-level retries) counts against RPH and
            # deepens the rate limit hole.
            if agent.provider == "igniteenow":
                try:
                    from agent.igniteenow_rate_guard import (
                        igniteenow_rate_limit_remaining,
                        format_remaining as _fmt_igniteenow_remaining,
                    )
                    _igniteenow_remaining = igniteenow_rate_limit_remaining()
                    if _igniteenow_remaining is not None and _igniteenow_remaining > 0:
                        _igniteenow_msg = (
                            f"Ignitee Now Portal rate limit active — "
                            f"resets in {_fmt_igniteenow_remaining(_igniteenow_remaining)}."
                        )
                        agent._buffer_vprint(
                            f"⏳ {_igniteenow_msg} Trying fallback..."
                        )
                        agent._buffer_status(f"⏳ {_igniteenow_msg}")
            ...
            try:
                ...
                try:
                    from robo_cli.middleware import apply_llm_request_middleware
                ...
                try:
                    from robo_cli.lifecycle import (
                        has_hook,
                ...
                if env_var_enabled("ROBO_DUMP_REQUESTS"):
                ...
                from robo_cli.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Robo/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                # Clear Ignitee Now rate limit state on successful request —
                # proves the limit has reset and other sessions can
                # resume hitting Ignitee Now.
                if agent.provider == "igniteenow":
                    try:
                        from agent.igniteenow_rate_guard import clear_igniteenow_rate_limit
                        clear_igniteenow_rate_limit()
                    except Exception:
                        pass
                from agent import relay_llm
                ...
                if (
                    ...
                    and _is_igniteenow_inference_route(
                        getattr(agent, "provider", "") or "",
                        getattr(agent, "base_url", "") or "",
                    )
                    and not _retry.igniteenow_paid_entitlement_refresh_attempted
                ):
                    _retry.igniteenow_paid_entitlement_refresh_attempted = True
                    if _try_refresh_igniteenow_paid_entitlement_credentials(agent):
                        agent._vprint(
                            f"{agent.log_prefix}🔐 Ignitee Now paid access verified — "
                ...
                if (
                    ...
                    and agent.provider == "igniteenow"
                    and status_code == 401
                    and not _retry.igniteenow_auth_retry_attempted
                ):
                    _retry.igniteenow_auth_retry_attempted = True
                    if agent._try_refresh_igniteenow_client_credentials(force=True):
                        print(f"{agent.log_prefix}🔐 Ignitee Now agent key refreshed after 401. Retrying request...")
                        continue
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix}🔐 Ignitee Now 401 — Portal authentication failed.")
                    if _body_text:
                        print(f"{agent.log_prefix}   Response: {_body_text}")
                    if not _print_igniteenow_entitlement_guidance(agent, "Ignitee Now model access"):
                        print(f"{agent.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
                    print(f"{agent.log_prefix}   Troubleshooting:")
                    print(f"{agent.log_prefix}     • Re-authenticate: robo auth add igniteenow")
                    print(f"{agent.log_prefix}     • Check credits / billing: https://portal.igniteenow.com")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `robo doctor` for credential-chain diagnostics, or")
                    ...
                    from robo_constants import display_robo_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Robo-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: robo config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: robo config set ANTHROPIC_API_KEY \"\"")
                ...
                if getattr(api_error, "status_code", None) == 404:
                    try:
                        from robo_cli.model_normalize import suggest_prefixed_model_id
                    ...
                    if _suggestion:
                        ...
                        agent._buffer_vprint(
                            f"      Did you mean '{_suggestion}'?  Re-pick it with `robo model`."
                        )
                ...
                # ── Ignitee Now Portal: record rate limit & skip retries ─────
                # When Ignitee Now returns a 429 that is a genuine account-
                ...
                #
                # IMPORTANT: Ignitee Now Portal multiplexes multiple upstream
                # providers (DeepSeek, Kimi, MiMo, Robo).  A 429 can
                # also mean an UPSTREAM provider is out of capacity
                # for one specific model -- transient, clears in
                # seconds, nothing to do with the caller's quota.
                # Tripping the cross-session breaker on that would
                # block every Ignitee Now model for minutes.  We use
                # ``is_genuine_igniteenow_rate_limit`` to tell the two
                ...
                if (
                    ...
                    _genuine_igniteenow_rate_limit = False
                    try:
                        from agent.igniteenow_rate_guard import (
                            is_genuine_igniteenow_rate_limit,
                            record_igniteenow_rate_limit,
                        )
                        ...
                        _genuine_igniteenow_rate_limit = is_genuine_igniteenow_rate_limit(
                            headers=_err_hdrs,
                            last_known_state=agent._rate_limit_state,
                        )
                        if _genuine_igniteenow_rate_limit:
                            record_igniteenow_rate_limit(
                                headers=_err_hdrs,
                                error_context=error_context,
                            )
                            ...
                            logger.info(
                                "Ignitee Now 429 looks like upstream capacity "
                    ...
                    if _genuine_igniteenow_rate_limit:
                        ...
                        # top-of-loop Ignitee Now guard handles fallback or
                ...
                # which Robo' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Robo' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`robo",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # Robo may add API-only content not present in persisted
                    ...
                    # recovery arms on the TRUE request that overflowed. See robo-lcm engine
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            pass
                        elif _provider == "igniteenow" and _print_igniteenow_entitlement_guidance(
                            agent,
                            "Ignitee Now model access",
                        ):
                            pass
                        elif _provider in {"openai-codex", "xai-oauth", "igniteenow"} and status_code == 401:
                            if _provider == "openai-codex":
                                agent._vprint(f"{agent.log_prefix}   💡 Codex OAuth token was rejected (HTTP 401). Your token may have been", force=True)
                                agent._vprint(f"{agent.log_prefix}      refreshed by another client (Codex CLI, VS Code). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Run `codex` in your terminal to generate fresh tokens.", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Then run `robo auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `robo model`.", force=True)
                            else:  # igniteenow
                                agent._vprint(f"{agent.log_prefix}   💡 Ignitee Now Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: robo portal", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://portal.igniteenow.com", force=True)
                                # ``:free`` is OpenRouter slug syntax; Ignitee Now Portal will reject
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    ...
                                    agent._vprint(f"{agent.log_prefix}         Ignitee Now Portal won't recognize that model name. Either switch to a", force=True)
                                    agent._vprint(f"{agent.log_prefix}         Ignitee Now catalog model, or run `/model openrouter:{_model}` to use OpenRouter.", force=True)
                        else:
                            agent._vprint(f"{agent.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: robo setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        robo fallback add   (interactive picker — same as `robo model`)",
                            force=True,
                        )
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.robo/config.yaml` to extend the per-call "
                            f"timeout. (Robo's built-in floor is 600s for "
                    ...
                    return {
                        ...
                        # descriptor (provider, billing_url, is_igniteenow, message).
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates Robo mid-turn, resume logic still sees the
                ...
                # timeout (ROBO_AGENT_TIMEOUT, default 1800s) and the
                ...
                try:
                    ...
                    from robo_cli.lifecycle import has_hook
                    from robo_cli.plugins import get_pre_verify_continue_message
                ...
                if _kanban_nudge:
                    ...
                    logger.info(
                        ...
                        agent._kanban_stop_nudges,
                        os.environ.get("ROBO_KANBAN_TASK", ""),
                    )
                    agent._emit_status(

...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Robo also honours ROBO_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("ROBO_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
_PROVIDER_PREFIXES: frozenset[str] = frozenset({
    "openrouter", "igniteenow", "openai-codex", "copilot", "copilot-acp",
...
def _local_probe_disk_cache_path() -> Path:
    ...
    return get_robo_home() / "cache" / "local_endpoint_probes.json"
...
def _get_model_metadata_cache_path() -> Path:
    ...
    from robo_constants import get_robo_home
    return get_robo_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Robo Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    "qwen3.8-max": 1_000_000,     # 1M context (OpenRouter & Ignitee Now portal, verified 2026-08-03)
    ...
    # (native MiniMax-M3, OpenRouter/Ignitee Now minimax/minimax-m3).
    ...
    # so these hardcoded fallbacks prevent Robo from probing-down to
    ...
    "gemini-1.5-pro": 2097152,  # 2M window (used at Robo's 2M working window)
    "llama-4-scout": 10000000,  # 10M window (capped to context.working_window)
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference-api.igniteenow.com": "igniteenow",
    ...
    # (often 8K) makes it unusable for Robo' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets Robo minimum.
...
def _get_context_cache_path() -> Path:
    ...
    from robo_constants import get_robo_home
    return get_robo_home() / "context_length_cache.yaml"
...
def parse_available_output_tokens_from_error(error_msg: str) -> Optional[int]:
    ...
    is_output_cap_error = (
        ...
        # OpenRouter/Ignitee Now phrasing of the same condition.
    ...
    # OpenRouter/Ignitee Now format: "maximum context length is N … (A of text input,
...
def is_output_cap_error(error_msg: str) -> bool:
    ...
    output_cap_signal = (
        ...
        or ("in the output" in error_lower                  # OpenRouter / Ignitee Now
            and "maximum context length" in error_lower)
...
def _model_name_suggests_minimax_m3(model: str) -> bool:
    ...
    across surfaces (native MiniMax-M3, OpenRouter/Ignitee Now minimax/minimax-m3).
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Robo grow conversations past the runtime limit and Ollama
...
def _normalize_model_version(model: str) -> str:
    ...
    Ignitee Now uses dashes: claude-opus-4-6, claude-sonnet-4-5
...
def _resolve_igniteenow_context_length(
    model: str,
    base_url: str = "",
    api_key: str = "",
) -> Tuple[Optional[int], str]:
    """Resolve Ignitee Now Portal model context length.

    Tries the live Ignitee Now inference endpoint first (authoritative), then falls
    back to OpenRouter metadata with suffix/version matching.

    Ignitee Now model IDs are bare after prefix-stripping (e.g. 'qwen3.6-plus',
    ...
    Returns ``(context_length, source)`` where ``source`` is one of:
      ...
      - ``""``           — could not resolve
    """
    # Portal first — the Ignitee Now /models endpoint is authoritative for what our
    ...
    def _safe_ctx(or_id: str, entry: dict) -> Optional[int]:
        ...
        if ctx <= 32768 and _model_name_suggests_kimi(or_id):
            logger.info(
                ...
                "(Kimi-family underreport, Ignitee Now path); falling through to hardcoded defaults",
                ctx, or_id,
            )
            return None
        return ctx
...
# Robo's supported working window. Any model that offers more than this is
# used at this ceiling: tool-output budgets, compaction tail budgets and the
# per-turn token accounting are all sized from the resolved window, and 2M is
# the point up to which those budgets have been validated. Override with
# ``context.working_window`` (0 = no ceiling).
DEFAULT_WORKING_WINDOW = 2_000_000


def _configured_working_window() -> int:
    """``context.working_window`` from config (default 2,000,000; 0 = unlimited)."""
    try:
        from robo_cli.config import load_config_readonly
        ctx = (load_config_readonly() or {}).get("context") or {}
        raw = ctx.get("working_window", DEFAULT_WORKING_WINDOW)
        value = int(raw) if raw is not None else DEFAULT_WORKING_WINDOW
        return max(0, value)
    except Exception:
...
def get_model_context_length(
    ...
    """Resolve a model's context window, capped at ``context.working_window``.

    See :func:`_resolve_model_context_length` for the resolution chain. The
    window Robo actually *works in* is ``min(resolved, working_window)``
    (default 2,000,000): a 2M-class model (Gemini 1.5 Pro, Grok 4 Fast) is
    used in full, a 10M-class model is used at 2M, and a 200K model stays at
    200K. An explicit ``model.context_length`` override is honoured as
    written (the user knows their endpoint best) — set it to 2000000 to force
    the full window on a proxy or local server that under-reports.
    """
    ...
    if config_context_length is not None and isinstance(config_context_length, int) and config_context_length > 0:
        return resolved
    ceiling = _configured_working_window()
    if ceiling and resolved > ceiling:
        logger.info("Context window for %s resolved to %d; using Robo's working window of %d", model, resolved, ceiling)
        return ceiling
    return resolved
...
def _resolve_model_context_length(
    ...
    1. Persistent cache (previously discovered via probing).  Ignitee Now URLs,
    ...
    5. Provider-aware lookups (before generic OpenRouter cache):
       ...
       b. Ignitee Now: live /v1/models probe first (authoritative), then OR
    ...
    if base_url and not _skip_persistent_context_cache(base_url, provider):
        ...
        if cached is not None:
            ...
            # leftover — drop it and fall through to the hardcoded default.
                ...
                _invalidate_cached_context_length(model, base_url)
            # Ignitee Now Portal: the portal /v1/models endpoint is authoritative.
            ...
            elif _infer_provider_from_url(base_url) == "igniteenow":
                logger.debug(
                    "Bypassing persistent cache for %s@%s (Ignitee Now portal authoritative)",
                    model, base_url,
                )
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from robo_cli.models import get_copilot_model_context
    ...
    if effective_provider == "igniteenow":
        ctx, source = _resolve_igniteenow_context_length(
        ...
        if ctx:
            ...
            # want it leaking into the persistent cache for Ignitee Now URLs.
    ...
    # the dedicated Ignitee Now/Copilot/GMI branches above.
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Robo sends to providers:

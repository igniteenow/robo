...
Storage
-------
Bundles live in ``~/.robo/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ROBO_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``robo bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ROBO_HOME.

    Honors ``ROBO_BUNDLES_DIR`` for tests; falls back to
    ``<ROBO_HOME>/skill-bundles``.
    """
    override = os.environ.get("ROBO_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_robo_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `robo bundles` CLI subcommand.
# ---------------------------------------------------------------------------

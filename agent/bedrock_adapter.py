"""AWS Bedrock Converse API adapter for Robo Agent.
...
Reference: ``extensions/amazon-bedrock/`` plugin, which implements
...
def _require_boto3():
    ...
    try:
        ...
        raise ImportError(
            ...
            "Or install Robo with Bedrock support: pip install -e '.[bedrock]'"
        )
    # converse() / converse_stream() were added in boto3 1.34.59.
    # When Robo is installed editable into system Python, the system boto3
...
# Priority order matches resolveAwsSdkEnvVarName():
...
def resolve_aws_auth_env_var(env: Optional[Dict[str, str]] = None) -> Optional[str]:
    ...
    This mirrors ``resolveAwsSdkEnvVarName()`` — used to detect
...
def has_aws_credentials(env: Optional[Dict[str, str]] = None) -> bool:
    ...
    This two-tier approach mirrors the pattern from an upstream PR:
...
def discover_bedrock_models(
    ...
    Mirrors ``discoverBedrockModels()`` in
...
# ---------------------------------------------------------------------------
# Mirrors classifyFailoverReason() and matchesContextOverflowError()

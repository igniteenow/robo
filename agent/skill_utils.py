...
from robo_constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.robo/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    ROBO_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``ROBO_KANBAN_TASK`` / ``ROBO_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("ROBO_KANBAN_TASK") or os.getenv("ROBO_KANBAN_BOARD"):
        ...
        # The Robo Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``robo_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ROBO_PLATFORM`` or
            ``ROBO_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("ROBO_PLATFORM")
        or get_session_env("ROBO_SESSION_PLATFORM")
    )
...
# which becomes the dominant cost of ``robo`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.robo/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``robo`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ROBO_HOME, not cwd
        if not p.is_absolute():
            p = (robo_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.robo/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.robo/skills/`` (including via symlinks) or configured
    ...
    # ~/.robo/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Robo can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    robo = metadata.get("robo") or {}
    if not isinstance(robo, dict):
        robo = {}
    return {
        "fallback_for_toolsets": robo.get("fallback_for_toolsets", []),
        "requires_toolsets": robo.get("requires_toolsets", []),
        "fallback_for_tools": robo.get("fallback_for_tools", []),
        "requires_tools": robo.get("requires_tools", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Robo metadata, VCS, virtualenv/dependency, cache, and skill

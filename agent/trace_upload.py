"""Upload a Robo session transcript to Hugging Face as an agent trace.

Robo stores sessions in its own SQLite store (``robo_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``robo trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Robo' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "robo-traces"
_ROBO_VERSION = "robo-engineer"
...
# ---------------------------------------------------------------------------
# Conversion: Robo OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Robo' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Robo conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.robo/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `robo trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/robo-traces`` dataset. Returns a

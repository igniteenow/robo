"""Anthropic Messages API adapter for Robo Agent.

Translates between Robo's internal OpenAI-style message format and
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in robo_cli/runtime_provider.py.
    """
...
# Robo effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _resolve_positive_anthropic_max_tokens(value) -> Optional[int]:
    ...
    finite positive number. Ported from an upstream fix.
...
def _resolve_anthropic_messages_max_tokens(
    ...
    Ported from an upstream fix (resolveAnthropicMessagesMaxTokens).
    """
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to robo's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to robo's outer conversation
    ...
    # from ``ANTHROPIC_API_KEY`` (Robo loads that into the process env from
    # ``~/.robo/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Robo notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Robo historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``robo models``) never mutates ``~/.robo/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Robo)
      ...
      5. Anthropic credential_pool OAuth entry (~/.robo/auth.json)
    ...
    # 1. Robo-managed OAuth/setup token env var
    ...
    # 5. Robo credential_pool OAuth entry.
...
# ── Robo-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.robo/.anthropic_oauth.json (our own file).
...
def _get_robo_oauth_file() -> Path:
    return get_robo_home() / ".anthropic_oauth.json"
...
def run_robo_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Robo-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Robo with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from robo_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_robo_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Robo-managed OAuth credentials from ~/.robo/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Robo OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See robo-engineer#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See robo-engineer: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See robo-engineer#13848 (Kimi) and
    robo-engineer#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    # rather than 400-ing at the Anthropic API. See an upstream fix.
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Robo Agent", "Claude Code")
                text = text.replace("Robo agent", "Claude Code")
                text = text.replace("robo-engineer", "claude-code")
                text = text.replace("Ignitee Now", "Anthropic")
                block["text"] = text
        ...
        #      a) bare Robo-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Robo surfaces in its CLI. We
...
def create_anthropic_message(
    ...
    *,
    ...
    parsed ``Message`` drops — Ignitee Now Portal's ``x-igniteenow-credits-*`` balance family

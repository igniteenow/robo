...
row taxonomy is inferred from the atlas shape so Robo can render both legacy
...
# rows (1536x1872), while older Robo/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Robo' activity state names. They are not always identical to the
...
# Legacy Robo/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Robo activity names -> accepted row-name aliases in descending

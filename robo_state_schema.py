...
``robo_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import robo_state (cycle) — shared
module-level constants live in robo_state_common.
"""
...
# Moved methods logged under the "robo_state" logger before the split;
# keep that logger identity so log filtering/capture behavior is unchanged.
logger = logging.getLogger("robo_state")
...
def schema_read_probe_statements() -> tuple:
    ...
    ``robo_cli/web_server.py``) run these probes right after a read-only
    ...
    served an empty session list after `robo update` until the user's
...
class SessionSchemaMixin:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._robo_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._robo_fts5_probe")
            return True
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `robo sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `robo db optimize`) must keep its EXISTING

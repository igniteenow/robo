...
load_robo_dotenv(
    robo_home=_robo_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.robo/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_robo_home, "logs", "tui_gateway_crash.log")
...
try:
    _slash_timeout = float(os.environ.get("ROBO_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("ROBO_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
_LONG_HANDLERS = frozenset(
    {
        ...
        # pool checks, pricing fetch, optional custom-provider
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("ROBO_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 8
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = (
    contextvars.ContextVar("robo_gateway_runtime_session_record", default=None)
)
...
class _SlashWorker:
    """Persistent RoboCLI subprocess for slash commands."""
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Robo agent → needs provider credentials.
        ...
        # gateway's launch ROBO_HOME (#40677). The override goes through the
        # build_subprocess_env factory's `extra` (applied last, always wins)
        # instead of a hand-rolled env["ROBO_HOME"] assignment.
        from tools.environments.local import build_subprocess_env
        env = build_subprocess_env(
            robo_subprocess_env(inherit_credentials=True),
            scrub_secrets=False,
            inherit_profile_home=False,  # base already carries the HOME contract
            extra={"ROBO_HOME": str(profile_home)} if profile_home else None,
        )
...
def _load_busy_input_mode() -> str:
    ...
    return raw if raw in {"queue", "steer", "interrupt", "redirect"} else "interrupt"
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from robo_cli.active_sessions import transfer_active_session
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `robo --tui` keeps real _stdio.
...
try:
    _SESSION_TTL_S = float(os.environ.get("ROBO_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `robo --tui`, so it must NOT count as dead here (doing
...
def _reap_idle_sessions() -> None:
    ...
    try:
        from robo_cli.mem_trim import trim_memory
...
def _reclaim_orphaned_leases() -> None:
    ...
    try:
        from robo_cli.active_sessions import release_orphaned_leases
...
def _max_live_sessions() -> int:
    try:
        from robo_cli.active_sessions import coerce_max_concurrent_sessions
...
# ``profile`` on those calls; we open that profile's db and bind its ROBO_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_robo_home).resolve():
        return None
    return home if (home / "state.db").exists() or home.exists() else None


def _profile_scoped(handler):
    """Bind ``params['profile']``'s ROBO_HOME around a pet RPC handler.

    Pets are per-profile: ``display.pet.*`` lives in the profile's config.yaml and
    sprites install under its ``pets/`` dir (both resolve via ``get_robo_home``).
    ...
    def wrapper(rid, params):
        ...
        token = set_robo_home_override(home)
        try:
            return handler(rid, params)
        finally:
...
def _profile_configured_cwd(profile_home: Path | None) -> str | None:
    ...
    if profile_home is None:
        return None
    try:
        from robo_cli.config import _expand_env_vars, read_user_config_raw
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``robo dashboard`` was launched) instead of the configured
...
def _inside_compute_host_child() -> bool:
    return os.environ.get("ROBO_COMPUTE_HOST_CHILD") == "1"
...
def _approval_policy_from_config() -> tuple[bool, bool]:
    """Resolve the interactive approval-barrier policy from config.

    ``approvals.interactive_timeout`` — seconds an approval prompt on an
    interactive surface (TUI/desktop) waits for an answer. ``0`` (default)
    means *until answered*: the agent stays parked on the prompt instead of
    expiring into a BLOCKED result it then reasons around. A positive value
    restores a bounded wait (the messaging-platform ``approvals.timeout`` is
    unaffected either way).

    ``approvals.deny_stops_turn`` — when true (default) a deny, an expired
    prompt, or a prompt torn down by session close also stops the turn, so
    the agent never keeps working in the background after the user said no.
    """
    wait_forever = True
    halt_on_deny = True
    try:
        ...
        if isinstance(approvals, dict):
            raw_timeout = approvals.get("interactive_timeout", 0)
            try:
                wait_forever = float(raw_timeout or 0) <= 0
            except (TypeError, ValueError):
                wait_forever = True
            halt_on_deny = is_truthy_value(approvals.get("deny_stops_turn", True))
    except Exception:
        pass
    return wait_forever, halt_on_deny


def _register_tui_approval_notify(sid: str, key: str) -> None:
    """Register the approval notifier for a TUI/desktop session with the
    interactive barrier policy (wait until answered; deny stops the turn).

    Every place that binds a session key to an approval notifier goes
    through here so the policy cannot drift between the deferred build,
    the eager init, and the post-compression re-anchor.
    """
    from tools.approval import register_gateway_notify

    wait_forever, halt_on_deny = _approval_policy_from_config()

    def _on_halt(reason: str, approval_data: dict, _sid: str = sid) -> None:
        session = _sessions.get(_sid)
        if not session:
            return
        agent = session.get("agent")
        # The user typed a new instruction instead of answering: that is a
        # redirect, not a deny — the loop applies it and continues.
        if agent is not None and getattr(agent, "_has_pending_redirect", None):
            try:
                ...
                pass
        if reason == "denied":
            note = "Command denied — Robo stopped. Tell it what to do next."
        elif reason == "timeout":
            note = "Approval expired unanswered — Robo stopped."
        else:
            note = "Approval prompt closed unanswered — Robo stopped."
        try:
            _emit("status.update", _sid, {"kind": "approval", "text": note})
        except Exception:
            pass
        if agent is not None and hasattr(agent, "interrupt"):
            try:
                # Plain (soft) interrupt: the blocked tool result is already
                # on its way into the transcript; this only stops the loop
                # from taking another step. Runs on a helper thread so the
                # agent thread returning the BLOCKED result never waits on
                # provider abort plumbing.
                threading.Thread(
                    target=lambda: agent.interrupt(),
                    daemon=True,
                    name=f"approval-halt-{_sid}",
                ).start()
            except Exception:
                pass
    ...
    register_gateway_notify(
        ...
        halt_on_deny=halt_on_deny,
        on_halt=_on_halt,
    )


def _reemit_pending_approval(sid: str, session: dict | None) -> bool:
    """Re-send the oldest still-pending approval prompt for *session*.

    The agent thread is parked on it; if the client missed or lost the
    original ``approval.request`` frame (reconnect, session switch, hot
    reload) it would otherwise see a silent, stuck turn. Returns True when
    a prompt was re-emitted.
    """
    if not session:
        return False
    ...
    try:
        ...
        pending = get_pending_gateway_approvals(key)
    except Exception:
        return False
    if not pending:
        return False
    _emit_approval_request(sid, pending[0])
    return True
...
def _emit_approval_request(sid: str, data: dict | None) -> None:
    ...
    # Robo exposes one consistent contract on every interactive surface.
    # "session" means the rest of this live Robo session, not merely the
    # current detector key; permanent hidden allowlists are intentionally not
    # offered from the mission checkpoint.
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `robo` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # ROBO_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_robo_home_override(profile_home)
                try:
            ...
            try:
                ...
                _register_tui_approval_notify(sid, key)
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    * A terminal session (``robo`` / ``robo --tui`` / CLI) is started from a
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from robo_cli.runtime_provider import canonical_custom_identity
    ...
    try:
        ...
        from robo_state import is_disk_full_error
...
def _set_session_cwd(session: dict, cwd: str) -> str:
    from robo_constants import translate_cwd_for_wsl_backend
...
def _load_dashboard_process_isolation_config(cfg: dict | None = None) -> dict[str, Any]:
    ...
    ``robo_cli.config.DEFAULT_CONFIG``. Keep
...
def _load_cfg_raw() -> dict:
    ...
    try:
        ...
        # launch profile's _robo_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_robo_home_override()
        home = override if isinstance(override, str) and override else _robo_home
...
def _load_cfg() -> dict:
    ...
    ``robo_cli.config.load_config_readonly`` — managed-scope overlay and
...
def _apply_managed(cfg: dict) -> dict:
    ...
    robo_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def _save_cfg(cfg: dict):
    ...
    from robo_cli.config import atomic_config_write

    path = _robo_home / "config.yaml"
...
def _set_session_context(
    ...
    *,
    ...
    try:
        ...
        # can read ROBO_SESSION_ID. Without this, set_session_vars leaves the
        ...
        # ROBO_SESSION_ID even though agent_init set it via
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["ROBO_GATEWAY_SESSION"] = "1"
    os.environ["ROBO_EXEC_ASK"] = "1"
...
def resolve_skin() -> dict:
    try:
        from robo_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _skin_sig() -> tuple[str, float | None]:
    ...
    name = str((_load_cfg().get("display") or {}).get("skin") or "robo")
    override = get_robo_home_override()
    home = override if isinstance(override, str) and override else _robo_home
    try:
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``robo config set display.skin``) OR edited the active skin's colors in
...
def _watcher_home() -> Path:
    ...
    override = get_robo_home_override()
    return Path(override if isinstance(override, str) and override else _robo_home)
...
def _ensure_skin_watcher() -> None:
    ...
    Robo activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
    ...
    threading.Thread(target=_loop, name="robo-change-watcher", daemon=True).start()


def _resolve_model() -> str:
    env = (
        os.environ.get("ROBO_MODEL", "")
        or os.environ.get("ROBO_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
    ...
    try:
        from robo_cli.models import get_preferred_silent_default_model
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``ROBO_DESKTOP=1`` and ``ROBO_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``ROBO_DESKTOP_TERMINAL=1`` → "tui"
        (``robo --tui`` running in the desktop's embedded terminal pane;
        it IS a TUI, just embedded. The clarifier attached to the tui hint
        in system_prompt.py tells the agent about the embedding.)
      * neither set → "tui"
        (standalone ``robo --tui``.)
    """
    if is_truthy_value(os.environ.get("ROBO_DESKTOP")) and not is_truthy_value(
        os.environ.get("ROBO_DESKTOP_TERMINAL")
    ):
        return "desktop"
    return "tui"
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads ROBO_MODEL /
    ROBO_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    (`robo --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # No fallback to _resolve_model() here: that reads ROBO_MODEL /
    # ROBO_INFERENCE_MODEL, which `robo --tui -m <model>` sets as a
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("ROBO_TUI_PROVIDER", "").strip()
    ...
    try:
        from robo_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("ROBO_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _runtime_model_config(agent, existing: dict | None = None) -> dict:
    ...
    if provider:
        if provider.strip().lower() == "custom":
            ...
            try:
                from robo_cli.runtime_provider import (
...
def _load_approval_mode() -> str:
    ...
    canonical ``robo_cli.config.load_config`` path applies managed-scope
...
def _load_reasoning_config(model: str = "") -> dict | None:
    ...
    :func:`robo_constants.resolve_reasoning_config` (per-model override >
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("ROBO_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    ...
    Both entries are deliberately off ``_ROBO_CORE_TOOLS`` — every other
    ...
    paths run with ``ROBO_DESKTOP=1``. Keying GUI capability off that env var
    silently stripped every pane/browser tool from URL and cloud gateways while
    the same backend told the model it was "chatting inside the Robo desktop
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    explicit = [
        item.strip()
        for item in os.environ.get("ROBO_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base Robo): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `robo --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] ROBO_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from robo_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown ROBO_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in ROBO_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid ROBO_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if not enabled:
            return None
        # The client-surface toolsets are off _ROBO_CORE_TOOLS (every other
        # platform would carry their schema for nothing), so the platform
        # recovery above — which keys off robo-cli's tool universe — can't
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid ROBO_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    from robo_cli.runtime_provider import resolve_runtime_provider
    ...
    cfg = None
    try:
        from robo_cli.config import get_compatible_custom_providers, load_config
    ...
    #
    # We deliberately do NOT write process-global env vars (ROBO_MODEL /
    # ROBO_INFERENCE_MODEL / ROBO_TUI_PROVIDER / ROBO_INFERENCE_PROVIDER)
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `robo --tui -m`
    ...
    try:
        ...
        if yolo_was_on:
            try:
                ...
                pass
        try:
            _register_tui_approval_notify(sid, new_session_id)
        except Exception:
            pass
    except Exception:
...
def _get_usage(agent) -> dict:
    ...
    # ROBO_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("ROBO_DEV_CREDITS")):
        try:
...
def _project_info_for_cwd(cwd: str) -> dict | None:
    ...
    if not str(cwd or "").strip():
        return None
    try:
        from robo_cli import projects_db as pdb
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if agent is not None and not (session or {}).get("_compute_host_active"):
        try:
            ...
            pass
        try:
            from robo_cli.banner import get_available_skills
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from robo_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from robo_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("ROBO_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("ROBO_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``RoboCLI.__init__`` and ``gateway/run.py``:
    ...
    try:
        ...
        for entry in fb_chain:
            ...
            if not fb_provider or not fb_model:
                continue
            try:
                from robo_cli.fallback_config import resolve_entry_api_key
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses robo_cli.mcp_startup; TUI stdio keeps
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("ROBO_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("ROBO_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("ROBO_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("ROBO_IGNORE_RULES")),
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "edit_snapshots": {},
            "tool_started_at": {},
            # Profile-scoped ROBO_HOME for app-global remote mode; None =
...
def _build_persist_message_with_image_refs(user_text: str, image_paths: list[str]) -> str:
    ...
    format the desktop client (directive-text.tsx / ROBO_DIRECTIVE_RE)
...
def _session_home(session: dict) -> Path:
    """The ROBO_HOME the session's durable state lives in (profile-aware)."""
    profile_home = session.get("profile_home")
    return Path(profile_home) if profile_home else Path(_robo_home)
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Robo has to support both the legacy 8-row petdex atlas and the current
...
def _pet_gen_root():
    ...
    root = get_robo_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("ROBO_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
def _spawn_trees_root():
    ...
    root = get_robo_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _collect_kanban_notifications(session: dict) -> list:
    ...
    ``platform="tui"`` and ``chat_id=ROBO_SESSION_KEY`` (see
    ...
    # DB when ROBO_KANBAN_DB pins the board path (same guard as the gateway
    # notifier).
...
def _wire_desktop_ui() -> None:
    ...
    Idempotent. The tool hands back the turn's ``ROBO_UI_SESSION_ID`` as
...
def _run_prompt_submit(
    ...
    *,
    ...
    def run():
        ...
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn ROBO_HOME override for a resumed remote profile
        secret_token = None
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_robo_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from robo_cli.config import load_config as _tui_load_config
            ...
            # billing_url, message) so the TUI/desktop render a
            ...
            if (
                ...
                try:
                    ...
                    logger.warning("voice TTS skipped: robo_cli.voice unavailable")
                except Exception as e:
            ...
            # Run while any profile-specific ROBO_HOME override is still active
...
def _session_images_dir(session: dict) -> Path:
    ...
    run BEFORE ``prompt.submit`` installs the session's profile ROBO_HOME
    override, so ``get_robo_home()`` here would return the gateway's launch
    ...
    """
    profile_home = session.get("profile_home")
    base = Path(profile_home) if profile_home else _robo_home
    return base / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".robo" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.robo/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.robo/desktop-attachments/``.
    ...
    _index_attachment_in_background(target)
    return target.resolve(), True


def _index_attachment_in_background(path: Path) -> None:
    """Add a freshly attached document to the knowledge base on a daemon
    thread, so a 500 MB PDF neither blocks the upload nor needs to fit in the
    agent's context. Images are skipped (they go to vision); any failure is
    logged and ignored, because indexing is a convenience, not the upload."""
    import threading
    ...
    def work() -> None:
        try:
            ...
            if path.suffix.lower() in IMAGE_SUFFIXES:
                return
            KnowledgeStore().add_file(path)
        except KnowledgeError as exc:
            logger.debug("attachment not indexed: %s", exc)
        except Exception as exc:  # pragma: no cover - never let indexing surface as an upload error
            logger.debug("attachment indexing failed for %s: %s", path, exc)
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                from robo_cli.model_switch import parse_model_switch_args
    ...
    if key == "fast":
        ...
        overrides = None
        if nv == "fast":
            from robo_cli.models import resolve_fast_mode_overrides
    ...
    if key == "busy":
        ...
        if raw not in {"queue", "steer", "interrupt", "redirect"}:
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("ROBO_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
                    os.environ["ROBO_YOLO_MODE"] = "1"
                    nv = "1"
                else:
                    os.environ.pop("ROBO_YOLO_MODE", None)
                    nv = "0"
...
def _is_repo_junk(root: str) -> bool:
    ...
    anything under ROBO_HOME (~/.robo by default) — config/sessions/skills,
    ...
    robo_home = os.path.realpath(str(get_robo_home()))

    return real == home or real == robo_home or real.startswith(robo_home + os.sep)
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    ROBO_HOME may be an intentional prose/data workspace. The pre-Projects
    ...
    robo_home = os.path.normcase(os.path.realpath(str(get_robo_home())))
    return real == home or real == robo_home
...
def _discover_repos_payload(
    ...
    repos even with zero robo sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (robo home subtree + bare
    ...
    try:
        ...
        def _read(c) -> None:
            for entry in pdb.list_discovered_repos(c):
                ...
                # i.e. "just now" — so a git checkout with zero Robo sessions
...
def _rank_slash_completions(
    ...
    *,
    ...
    of skills someone invokes daily lead the ones that shipped with Robo
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `robo` is interactive — use `/robo chat -q …` or run `robo` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`robo setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`robo gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`robo sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`robo config edit` needs $EDITOR in a real terminal"
    return None
...
def _model_picker_context(agent):
    ...
    from robo_cli.inventory import load_picker_context
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Robo" if role == "assistant" else role.title()
...
def _any_session_running() -> bool:
    ...
    Registered as the voice busy-probe (``robo_cli.voice.set_voice_busy_probe``)
        ...
        try:
            ...
            if text:
                ...
                if _is_stop:
                    ...
                    try:
                        from robo_cli.voice import stop_continuous
...
def _speak_text_with_barge(text: str) -> None:
    """Speak *text* via robo_cli.voice.speak_text with spoken barge-in.
    ...
    """
    from robo_cli.voice import speak_text
...
def _(rid, params: dict) -> dict:
    """Index a file into the knowledge base: the `/attach` command.

    Runs on the gateway so the TUI, the dashboard's chat and any other client
    share one store. Returns the document record, or a clear error message."""
    raw = str(params.get("path") or "").strip()
    if not raw:
        return _err(rid, 4001, "knowledge.add requires a path")
    try:
        from robo_cli.knowledge_store import KnowledgeError, KnowledgeStore, to_json

        target = _resolve_gateway_attachment_path(raw) or Path(raw).expanduser()
        if not target.is_file():
            return _err(rid, 4004, f"no such file: {raw}")
        doc = KnowledgeStore().add_file(target, title=str(params.get("title") or "") or None)
        return _ok(rid, {"indexed": to_json(doc), "path": str(target)})
    except KnowledgeError as e:
        return _err(rid, 4005, str(e))
    except Exception as e:
        logger.exception("knowledge.add failed")
...
def _(rid, params: dict) -> dict:
    try:
        from robo_cli.knowledge_store import KnowledgeStore, to_json

        store = KnowledgeStore()
        return _ok(rid, {"documents": [to_json(d) for d in store.documents()], "stats": store.stats()})
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["ROBO_VOICE"] = "1" if enabled else "0"
    ...
    if action == "tts":
        ...
        os.environ["ROBO_VOICE_TTS"] = "1" if new_value else "0"
        if not new_value:
            _tts_stream_stop(user_barge=False)
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from robo_cli.voice import start_continuous
            ...
            # are tolerated.
            try:
                from robo_cli.voice import set_voice_busy_probe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        import robo_cli.voice  # noqa: F401
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from robo_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from robo_cli.browser_connect import DEFAULT_BROWSER_CDP_URL

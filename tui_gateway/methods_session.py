...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind ROBO_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    with _profile_db(params) as db:
        ...
        try:
            ...
            # custom `ROBO_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_robo_home_override(str(profile_home)) if profile_home is not None else None
    )
    secret_token = (
    ...
    with _session_resume_lock:
        ...
        try:
            init_home_token = (
                set_robo_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            init_secret_token = (
            ...
            try:
                ...
                if init_home_token is not None:
                    reset_robo_home_override(init_home_token)
            ...
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds ROBO_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # ``robo --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    # A turn parked on an approval prompt must show that prompt again to the
    # client that just attached — otherwise the switch lands on a silent,
    # stuck session with no way to answer.
    try:
        _reemit_pending_approval(sid, session)
    except Exception:
        pass
    return _ok(rid, payload)
...
def _(rid, params: dict) -> dict:
    ...
    with _profile_db(params) as db:
        ...
        if profile_home is not None:
            ...
            sessions_dir = get_robo_home() / "sessions"
        try:
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``robo gateway`` process, whose
...
def _(rid, params: dict) -> dict:
    ...
    # Ignitee Now credits block — agent-independent (a portal fetch), so it shows even
    # with zero API calls or on a resumed session. The TUI /usage panel renders
    # these lines regardless of `calls`. Fail-open: [] when not logged into Ignitee Now
    # or on any portal hiccup.
    try:
        from agent.account_usage import igniteenow_credits_lines

        credits = igniteenow_credits_lines()
        if credits:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from robo_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from robo_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from robo_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from robo_cli.pets import set_pet_scale
...
def _(rid, params: dict) -> dict:
    ...
    True only when a reference-capable image backend (Ignitee Now Portal / OpenRouter /
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Robo profile home
    # (~/.robo/sessions/saved/) rather than the project/workspace CWD, and
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        home_token = (
            set_robo_home_override(parent_home) if parent_home else None
        )

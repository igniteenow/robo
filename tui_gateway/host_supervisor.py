...
def _default_registry_path() -> Path:
    return get_robo_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_robo_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_robo_home = expected_robo_home if expected_robo_home is not None else str(get_robo_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["ROBO_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("robo_home") or "")
        if got_home and got_home != self.expected_robo_home:
            raise RuntimeError(f"compute host ROBO_HOME mismatch: {got_home} != {self.expected_robo_home}")

...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from robo_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_robo_home), "display": display_robo_home()})
    if key == "project":
    ...
    if key == "mtime":
        cfg_path = _robo_home / "config.yaml"
        try:
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Robo provider is configured.",
                },
            )

...
def _is_dispatcher_owned_worker() -> bool:
    """False when ROBO_KANBAN_* is present but this execution does not own it
...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_robo_home() under the active profile.
...
#   - cli.py, robo_cli/*      -> inline on startup (no event loop)
    ...
    if quiet_mode:
        ...
        if profile_scope != CHECK_FN_CACHE_BYPASS:
            cache_key = (
                ...
                cfg_fp,
                bool(os.environ.get("ROBO_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if (
            ...
            # Dispatcher-spawned workers are scoped by ROBO_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like robo-cli)
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("robo-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (robo-*) include _ROBO_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("robo-")
    ...
    # window). Core Robo tools (toolsets._ROBO_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from robo_cli.config import load_config as _load
        cfg = _load() or {}
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to robo-engineer's coercion layer.

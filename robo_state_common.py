...
Extracted verbatim from robo_state.py so the SessionDB mixin modules
(robo_state_search / robo_state_schema / robo_state_portability) can
reference them without importing robo_state (which would be a cycle).
robo_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``robo sessions optimize-storage``. A legacy DB sits at
...
# `robo sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `robo db optimize`. This is

#!/usr/bin/env python3
"""Google Workspace API CLI for Robo Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Robo-facing JSON contract and falls back to the Python client
...
from _robo_home import get_robo_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ROBO_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Robo Agent")

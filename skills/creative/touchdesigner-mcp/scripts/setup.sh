...
ROBO_HOME_DIR="${ROBO_HOME:-$HOME/.robo}"
ROBO_CFG="${ROBO_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Robo config has twozero_td MCP entry ──
if [[ ! -f "$ROBO_CFG" ]]; then
    echo -e " ${FAIL} Robo config not found at ${ROBO_CFG}"
    manual_steps+=("Create ${ROBO_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ROBO_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Robo config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Robo config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ROBO_CFG} manually"); }
    manual_steps+=("Restart Robo session to pick up config change")
fi

...
## 5. Robo Configuration

### Config location

`$ROBO_HOME/config.yaml` (defaults to `~/.robo/config.yaml` when `ROBO_HOME` is unset)
...
Restart the Robo session for changes to take effect. The MCP connection is

...
Config: `~/.robo/config.yaml` (settings), `~/.robo/.env` (API keys) — both under `$ROBO_HOME` when it is set.
...
`_ROBO_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_robo_home()` for paths,
never hardcode `~/.robo`. For custom/local-only tools, write a plugin in
`~/.robo/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `robo_cli/commands.py`
...
- Tests auto-redirect `ROBO_HOME` to temp dirs — never touch real `~/.robo/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/robo_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_robo_home()` from `robo_constants` for all paths (profile-safe)

...
Write plugins for the Robo desktop app: statusbar items, layout panes,
...
- The Robo desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$ROBO_HOME/desktop-plugins/` (usually
  `~/.robo/desktop-plugins/`).

## How to Run

1. Create `$ROBO_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.robo/...` by default, or `~/.robo/profiles/<profile>/...` under a
...
The ONLY import surface is `@robo/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  Fires only while the user is away from Robo (use `host.notify` for the
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.robo/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@robo/plugin-sdk`, `react`, and

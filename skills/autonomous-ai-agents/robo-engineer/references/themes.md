# Themes / Skins — Author a Robo Color Theme

Author a Robo **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`robo_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.robo/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Robo itself.
...
- Write access to the Robo home dir — `~/.robo` by default, or `$ROBO_HOME`
  / the active profile's dir. Skins live in `<robo-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `robo config set`).
...
3. `write_file` it to `<robo-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `robo config set display.skin default` (or they can `/skin default`).
...
```
robo skin set <key> <hex>      # e.g. robo skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `robo skin set background "#08201f"` changes
only the background; `robo skin use <name>` / `robo skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.robo`** when a profile is active — resolve the real home
  from `$ROBO_HOME` first, falling back to `~/.robo`.
...
- **Name collisions**: a skin named like a desktop built-in (`igniteenow`,
  `ember`, `midnight`, `contrast`) won't override that built-in on the GUI. Also
  avoid the retired names `mono`, `slate`, `cyberpunk`, `ares`, `gold` and
  `default`: on the desktop they migrate to a built-in unless a user theme with
  exactly that name exists. Pick a fresh name.
- **Never hand-edit `config.yaml` to activate.** Use `robo config set
...
- **You apply it, not the user.** `robo config set display.skin <name>` is
...
- `read_file` the written `<robo-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `robo config get display.skin` and confirm it reports `<name>`.

...
the `cronjob` tool, the `robo cron` CLI (`list`, `add`, `edit`,
...
- **CLI:** `robo curator <verb>` — `status`, `usage`, `run`, `pause`,
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  `robo curator run --consolidate`. Routine background curation costs
  zero tokens.
- **Telemetry:** sidecar at `~/.robo/skills/.usage.json` holds
...
Users drive it via `robo kanban <verb>`; dispatcher-spawned workers
see a focused `kanban_*` toolset gated by `ROBO_KANBAN_TASK`, and

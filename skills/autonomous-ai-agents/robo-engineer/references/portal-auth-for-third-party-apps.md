# Ignitee Now Portal — authenticating third-party apps against the subscription

Recurring user question: "Can app X (Karakeep, OpenWebUI, LibreChat, OpenViking,
LangChain pipeline, n8n flow, etc.) use my Ignitee Now Portal subscription without me
...
## Layer 1 — Is this thing a Robo plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Robo process**. Its LLM calls go through Robo's already-configured provider. | Already uses Portal if user's Robo is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
| **Karakeep, n8n, LibreChat, OpenWebUI, any self-hosted app** | Different process, often different machine. Makes its own HTTPS calls to `inference-api.igniteenow.com`. | Layer 2/3 below. |

**Pitfall to avoid**: do not pitch "OAuth into Portal" as the solution for a
plugin that already runs inside Robo. That LLM call is already authenticated
via Robo's provider config. The plugin's own server auth (e.g.
...
Portal at `https://inference-api.igniteenow.com/v1` is an OpenAI-compatible
inference endpoint. It accepts **bearer-token authentication only**: either

1. **A static API key** from `portal.igniteenow.com → API Keys`, or
...
"Sign in with Ignitee Now Portal" SSO that third-party apps can register as clients
against. There is no shared cookie or session that browser-Portal-login
extends to other apps on the same machine.

What Robo Agent has that *feels* like OAuth — `robo login --provider igniteenow`
opening a browser, user signs in, token lands in `~/.robo/auth.json` — is a
**Robo-specific browser flow**. Under the hood it produces a credential
Robo uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Robo's existing Portal credential out of `~/.robo/auth.json`.
2. Expose a local OpenAI-compatible endpoint at `http://localhost:NNNN/v1`.
3. Forward incoming requests to `inference-api.igniteenow.com/v1` with that
   bearer attached.
...
rides on the credential Robo already holds.

Where this could live in Robo:
...
Only when the consumer is *another first-party Ignitee Now thing* (a future SDK, a
Ignitee Now-branded extension, a Discord-bot integration that needs per-user
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Robo itself.
...
1. First decide: Robo plugin (runs inside Robo) or separate app? If plugin,
   it already uses Portal via Robo's provider config — done.
2. If separate app: today, paste the static API key from Portal → API Keys.
   Base URL `https://inference-api.igniteenow.com/v1`. Rate limits are
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Robo agent itself") is fully supported. Robo treats agent
...
pitch in the Tool Gateway announcement is specifically about Robo Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Robo is configured to use Portal as its provider. It is

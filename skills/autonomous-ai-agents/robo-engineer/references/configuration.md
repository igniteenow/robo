...
`robo config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `robo tools` (interactive) or `robo tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_ROBO_CORE_TOOLS` is the default bundle most platforms inherit).

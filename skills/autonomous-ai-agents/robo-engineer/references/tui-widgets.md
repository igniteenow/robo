...
Author widget apps for the Robo TUI (`robo --tui`): glanceable ambient
...
- The TUI must be in use (`robo --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.robo/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.robo/logs/tui_gateway_crash.log` if a widget never appears.

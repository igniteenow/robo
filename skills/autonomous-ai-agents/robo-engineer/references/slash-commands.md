...
Registry of record: `robo_cli/commands.py` (`COMMAND_REGISTRY`) — every
...
/snapshot [sub]          Create/restore Robo config+state snapshots (CLI)
...
/subscription (/upgrade) Ignitee Now plan info (CLI)
/topup                   Ignitee Now balance / billing
/copy [N]                Copy last response to clipboard (CLI)
/paste                   Attach clipboard image (CLI)
/image <path>            Attach a local image file (CLI)
/update                  Update Robo to latest
/version (/v)            Show version
/debug [igniteenow|local]      Upload debug report, get shareable links
```

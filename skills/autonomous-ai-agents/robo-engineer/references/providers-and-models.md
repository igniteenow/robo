...
| openai-codex | OAuth | `robo auth add openai-codex` |
| qwen-oauth | OAuth | `robo auth add qwen-oauth` |
| minimax-oauth | OAuth | `robo auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`robo auth`).
Fallback chain when the primary fails: `robo fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`robo_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   robo config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:

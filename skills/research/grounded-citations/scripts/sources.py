...
Ledger path resolution (first wins):
  --ledger PATH
  $ROBO_CITATION_LEDGER
  $ROBO_HOME/cache/citations/ledger.json
"""
...
from _robo_home import get_robo_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("ROBO_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_robo_home() / "cache" / "citations" / "ledger.json"

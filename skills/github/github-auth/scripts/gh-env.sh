#!/usr/bin/env bash
# GitHub environment detection helper for Robo Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _robo_env="${ROBO_HOME:-$HOME/.robo}/.env"; [ -f "$_robo_env" ] && grep -q "^GITHUB_TOKEN=" "$_robo_env" 2>/dev/null; then

...
# 5. Symlinks the 'robo' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}◆ Robo Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/robo-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink robo into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up robo command..."

ROBO_BIN="$SCRIPT_DIR/venv/bin/robo"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ROBO_BIN" "$COMMAND_LINK_DIR/robo"
echo -e "${GREEN}✓${NC} Symlinked robo → $COMMAND_LINK_DISPLAY_DIR/robo"
...
# ============================================================================
# Seed bundled skills into ~/.robo/skills/
# ============================================================================

ROBO_SKILLS_DIR="${ROBO_HOME:-$HOME/.robo}/skills"
mkdir -p "$ROBO_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.robo/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ROBO_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  robo status        # Check configuration"
if is_termux; then
    echo "  robo gateway       # Run gateway in foreground"
else
    echo "  robo gateway install # Install gateway service (messaging + cron)"
fi
echo "  robo cron list     # View scheduled jobs"
echo "  robo doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m robo_cli.main setup
fi

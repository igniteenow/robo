# Contributing to Robo Agent

Thank you for contributing to Robo Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Robo to work everywhere.
...
Related: #38284 covers the agent-side analog — Robo itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Robo install. They should be **broadly useful to most users**:
...
**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.robo/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(robo_home, config)` for setup-wizard integration
...
- Integrate with `robo memory setup` via `post_setup()` — no need to touch core code
...
The installer creates the Robo venv, wires the `robo` command, stamps the
install method for `robo update`, and clones the full git project into
`$ROBO_HOME/robo-engineer` (usually `~/.robo/robo-engineer`). That keeps your
...
Use this only if you intentionally do not want Robo' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `robo` entrypoint from this venv; running the
system `python3 -m robo_cli.main` can pick up unrelated system Python
packages.
...
export VIRTUAL_ENV="$HOME/.robo/venvs/robo-dev"
...
```bash
mkdir -p ~/.robo/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.robo/config.yaml
touch ~/.robo/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.robo/.env
```

### Run

```bash
# The standard installer already put `robo` on PATH.
robo doctor
robo chat -q "Hello"
```

If you used the manual clone fallback, run `./robo` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/robo" ~/.local/bin/robo
```
...
├── cli.py                    # RoboCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (robo-cli, robo-telegram, etc.)
├── robo_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── robo_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── config.py                 # Config management, migration, env var definitions
│   ├── setup.py                  # Interactive setup wizard
│   ├── auth.py                   # Provider resolution, OAuth device-code flows
...
│
├── skills/                   # Bundled skills (copied to ~/.robo/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation (Docusaurus site; source in website/docs)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.robo/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.robo/`)

| Path | Purpose |
|------|---------|
| `~/.robo/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.robo/.env` | API keys and secrets |
| `~/.robo/auth.json` | OAuth credentials (Codex, Qwen, MiniMax, xAI …) |
| `~/.robo/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.robo/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.robo/state.db` | SQLite session database |
| `~/.robo/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.robo/cron/` | Scheduled job data |
| `~/.robo/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`robo_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.robo/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
- **Ephemeral injection**: System prompts and prefill messages are injected at API call time, never persisted to the database or logs.
- **Provider abstraction**: The agent works with any OpenAI-compatible API. Provider resolution happens at init time (OAuth device-code providers, OpenRouter API key, or custom endpoint).
...
(for example `_ROBO_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.robo`:
...
The user may skip setup and keep loading the skill. Robo only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `robo setup` or update `~/.robo/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Robo tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Robo Agent" is the secondary collaborator. If the contributor's commit shows "Robo Agent" as author because they used Robo to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Robo tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `robo --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Robo uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.robo/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `robo_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `robo_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Robo runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the robo wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `robo_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `robo_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `robo_cli/gateway_windows.py::
...
Robo has terminal access. Security matters.
...
2. **Test manually**: Run `robo` and exercise the code path you changed

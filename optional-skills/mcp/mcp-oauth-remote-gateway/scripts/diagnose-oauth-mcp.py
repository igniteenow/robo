...
Usage:
  ...
  <server_name> matches the files in $ROBO_HOME/mcp-tokens/<server>.json etc.
...
def _robo_home():
    # Prefer Robo' own resolver (profile-safe); fall back to env then ~/.robo.
    try:
        from robo_constants import get_robo_home
        return str(get_robo_home())
    except Exception:
        return os.environ.get("ROBO_HOME") or os.path.expanduser("~/.robo")


def _tokens_dir():
    return os.path.join(_robo_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "robo-diag", "version": "1.0"}},
        },
        headers={

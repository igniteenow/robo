...
metadata:
  robo:
    ...
    related_skills: [claude-code, codex, opencode, robo-engineer]
---
...
- User wants a coding agent that can run on a non-Anthropic / non-OpenAI provider (DeepSeek, Qwen, Ollama, vLLM, Ignitee Now, etc.) — sibling skills `claude-code` and `codex` are tied to one vendor.

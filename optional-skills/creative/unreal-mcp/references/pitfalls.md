...
### 1. Start order: editor first, Robo second

Robo probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Robo session.
...
restart the editor, restart the Robo session.
...
server fails to bind (Output Log) or Robo' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.robo/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Robo

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Robo' connection
lives in `~/.robo/config.yaml` via `robo mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Robo — just don't
mistake it for the Robo setup step.
...
explicitly warns against overlapping calls. Robo executes same-turn tool
...
### 9. Timeouts: Robo gives up before Unreal does

Robo' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Robo runs elsewhere (SSH backend,

#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `robo kanban list` and
...
def robo_available() -> bool:
    return shutil.which("robo") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed robo CLI."""
    try:
        out = subprocess.run(
            ["robo", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `robo kanban list`
    out = subprocess.run(
        ["robo", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
...
def main():
    ...
    if not robo_available():
        print("ERROR: 'robo' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)

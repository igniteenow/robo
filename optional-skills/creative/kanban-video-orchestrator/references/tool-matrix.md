...
Maps each role archetype to the Robo skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public robo-engineer
...
## Robo skills relevant to video production

### Visual / rendering skills (`robo-engineer/skills/creative/`)
...
### Design / pre-production skills (`robo-engineer/skills/creative/`)
...
### Audio / media skills (`robo-engineer/skills/creative/` + `skills/media/`)
...
These are **not** Robo skills but external CLIs / APIs that profiles invoke.
...
## Built-in Robo tools for media review

These are native Robo tools — not invoked via terminal but through their own
...
key is present in `${ROBO_HOME:-~/.robo}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Robo profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${ROBO_HOME:-~/.robo}/.env` → environment variable.
...
When a new Robo-public video skill ships:

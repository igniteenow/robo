...
robo kanban watch --tenant <project-slug>

# Snapshot of the board
robo kanban list --tenant <project-slug>
robo kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
robo kanban stats --tenant <project-slug>

# Visual dashboard (browser)
robo dashboard

# Inspect a specific task (includes comments + events)
robo kanban show <task-id>

# Follow a single task's event stream
robo kanban tail <task-id>
```

Verify available subcommands with `robo kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `robo kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `robo kanban block <id>` then `robo kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `robo kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `robo kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `robo profile list` (profile exists?), `robo status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(robo kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
robo kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
robo kanban block <task-id>
robo kanban archive <task-id>

# Diagnose what happened
robo kanban show <task-id>      # task body, comments, recent events
robo kanban tail <task-id>      # follow the live event stream
robo kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `robo kanban create` for the director
...
while true; do
    clear
    robo kanban list --tenant <slug>
    echo "---"
    robo kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `robo kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `robo profile list` and start

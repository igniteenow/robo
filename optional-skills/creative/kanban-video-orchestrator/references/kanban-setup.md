...
the actual `setup.sh` that creates the project workspace, configures Robo
...
> [IgniteeNow/kanban-video-pipeline](https://github.com/IgniteeNow/kanban-video-pipeline).
...
2. **Create profiles** — `robo profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.robo/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `robo kanban create` assigned to the director
...
```bash
robo profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.robo/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.robo/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Robo
...
Each profile gets a `SOUL.md` at `~/.robo/profiles/<name>/SOUL.md` that
...
```bash
robo kanban create "Direct production of <video title>" \
    --assignee director \
...
the Robo `.env` (`${ROBO_HOME:-$HOME/.robo}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _robo_env="${ROBO_HOME:-$HOME/.robo}/.env"
    if grep -q "^${var}=" "$_robo_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_robo_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_robo_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY robo ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY robo OPENROUTER_API_KEY || exit 1
# ...
```

...
Usage (via cron with --no-agent):
    ...
    robo cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ROBO_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Robo-Watcher/1.0"})

#!/usr/bin/env python3
"""Canvas LMS API CLI for Robo Agent.
...
def _check_config():
    ...
    if missing:
        robo_env = os.path.join(
            os.environ.get("ROBO_HOME", os.path.expanduser("~/.robo")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {robo_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Robo Agent"
    )

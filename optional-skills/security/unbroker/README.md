...
The design is **Robo-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
robo skills install official/security/unbroker
```

Then start a new Robo session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Robo home directory (`0600`, optionally

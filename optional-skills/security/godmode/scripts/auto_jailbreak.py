...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ROBO_HOME", os.path.expanduser("~/.robo")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ROBO_HOME", Path.home() / ".robo")) / "skills" / "red-teaming" / "godmode"
...
ROBO_HOME = Path(os.getenv("ROBO_HOME", Path.home() / ".robo"))
CONFIG_PATH = ROBO_HOME / "config.yaml"
PREFILL_PATH = ROBO_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Ignitee Now/Robo models — already uncensored, just needs clean prompt
    "robo": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "robo" in model_lower or "igniteenow" in model_lower:
        return "robo"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Robo config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.robo/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Robo for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Robo for changes to take effect.")

...
/**
 * User widget apps — Robo authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$ROBO_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.robo/plugins/`: files under ROBO_HOME execute
...
const widgetsDir = () => join(process.env.ROBO_HOME?.trim() || join(homedir(), '.robo'), 'tui-widgets')
 ...
 *  every change, so a widget Robo writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Robo) ever writes must hot-load too — a 10s poll here read

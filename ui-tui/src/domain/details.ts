...
//
//   - thinking / tools: collapsed — the turn reads like Claude Code /
//     ChatGPT: one chevron row per section ("▸ Tools (7)  running terminal…")
//     that ticks over in place while Robo works, and a single click (or
//     `/details expanded`, or Shift+click for everything) opens the full
//     reasoning / every tool call and result. Streaming them open by default
//     turned every turn into a wall of tool output that buried the answer.
...
//
// Opt back into the live wall with `display.sections.tools: expanded` (and
// `thinking: expanded`) in config.yaml, or at runtime via

...
import { clipboardUnavailableHint, isUsableClipboardText, readClipboardText, writeClipboardText } from '../lib/clipboard.js'

describe('clipboardUnavailableHint', () => {
  it('gives a generic message on macOS (pbpaste is always present)', () => {
    expect(clipboardUnavailableHint('darwin', {})).toBe('Clipboard is empty or unreadable.')
  })

  it('gives a generic message on Windows (PowerShell is always present)', () => {
    expect(clipboardUnavailableHint('win32', {})).toBe('Clipboard is empty or unreadable.')
  })

  it('mentions the WSL/Windows bridge when running under WSL', () => {
    const msg = clipboardUnavailableHint('linux', { WSL_DISTRO_NAME: 'Ubuntu' })

    expect(msg).toContain('WSL')
  })

  it('points at wl-clipboard on Wayland', () => {
    const msg = clipboardUnavailableHint('linux', { WAYLAND_DISPLAY: 'wayland-0' })

    expect(msg).toContain('wl-clipboard')
    expect(msg).not.toContain('xclip')
  })

  it('points at xclip on plain X11 Linux (the common case: nothing preinstalled)', () => {
    const msg = clipboardUnavailableHint('linux', {})

    expect(msg).toContain('xclip')
    expect(msg).not.toContain('wl-clipboard')
  })

  it('defaults to the real process.platform/env when not given', () => {
    // Just confirms it doesn't throw when called with no arguments — the
    // signature's defaults are what every real call site actually uses.
    expect(() => clipboardUnavailableHint()).not.toThrow()
  })
})

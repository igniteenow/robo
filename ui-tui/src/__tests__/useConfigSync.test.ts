...
describe('normalizeBusyInputMode', () => {
  it('passes through the canonical CLI parity values', () => {
    ...
    expect(normalizeBusyInputMode('redirect')).toBe('redirect')
  })
  ...
  it('defaults to interrupt for missing/unknown values (framework parity)', () => {
    // The TUI follows the framework default (robo_cli/config_defaults.py +
    // tui_gateway/server.py::_load_busy_input_mode). The old TUI-only `queue`
    // default parked mid-turn messages in a client-side queue: never sent,
    // not shown in the transcript, read only after the whole turn ended.
...
describe('applyDisplay → busy_input_mode', () => {
  ...
  it('falls back to interrupt when value is missing or invalid (framework default)', () => {
    const setBell = vi.fn()

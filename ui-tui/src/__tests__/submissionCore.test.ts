...
describe('submissionCore.submitPrompt — synchronous busy (queue-race fix)', () => {
  ...
  it('appends the user message bubble SYNCHRONOUSLY, before input.detect_drop resolves', () => {
    const { gw, resolveDrop } = makeDeferredGateway()
    const appendMessage = vi.fn()

    submitPrompt('hello while agent is busy', makeDeps(gw, { appendMessage }))

    // The actual bug this pins down: previously appendMessage lived inside
    // startSubmit, only reachable after detect_drop resolved. When the
    // gateway's event loop is busy running the agent's active turn, that
    // RPC can sit unresolved for a real, noticeable stretch — during which
    // the user's own message never appeared on screen at all, even though
    // it had already been accepted for submission (markSubmitting already
    // ran). The agent would eventually pick it up and respond; the user
    // just never saw their own message land.
    expect(appendMessage).toHaveBeenCalledTimes(1)
    expect(appendMessage).toHaveBeenCalledWith({ role: 'user', text: 'hello while agent is busy' })

    resolveDrop()
  })

  it('uses displayOverride for the synchronous bubble when provided', () => {
    const { gw, resolveDrop } = makeDeferredGateway()
    const appendMessage = vi.fn()

    submitPrompt('/skill full-expansion-body', makeDeps(gw, { appendMessage }), true, '/skill')

    expect(appendMessage).toHaveBeenCalledWith({ role: 'user', text: '/skill' })

    resolveDrop()
  })

  it('does not append a bubble when showUserMessage is false', () => {
    const { gw, resolveDrop } = makeDeferredGateway()
    const appendMessage = vi.fn()

    submitPrompt('silent resubmit', makeDeps(gw, { appendMessage }), false)

...
describe('approvalAction — pure key dispatch for ApprovalPrompt', () => {
  ...
  it('maps number keys 1..3 to once/session/deny in registration order', () => {
  ...
  it('keeps the session option available after a Smart DENY review', () => {
  ...
  it('filters explicit gateway choices to Robo approval choices', () => {
    expect(
      approvalOptions({
        allowPermanent: true,
        choices: ['once', 'deny'],

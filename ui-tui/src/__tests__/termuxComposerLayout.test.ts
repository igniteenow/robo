...
describe('Termux composer prompt + width guards', () => {
  ...
  it('reserves fewer columns for gutter on narrow Termux widths', () => {
    ...
    // Termux keeps those 2 columns for the active composer, minus 1 more so
    // the GoodVibesHeart overlay (which the scrollbar gutter isn't reserved
    // here to cover) has its own cell instead of sharing the last input column.
    expect(stableComposerColumns(40, 8, false)).toBe(28)
    expect(stableComposerColumns(40, 8, true)).toBe(29)

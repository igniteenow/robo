...
describe('createGatewayEventHandler', () => {
  ...
  it('opens a billing confirm dialog routing Ignitee Now to /topup', () => {
    const appended: Msg[] = []
    ...
    onEvent({
      payload: {
        billing: {
          ...
          model: 'm',
          provider: 'igniteenow',
          provider_label: 'Ignitee Now Portal'
        },
    ...
    expect(confirm?.title).toContain('Ignitee Now')
  ...
  it('surfaces self-improvement review summaries as a persistent system line', () => {
    ...
    onEvent({
      payload: { text: "💾 Self-improvement review: Skill 'robo-release' patched" },
      type: 'review.summary'
    } as any)

    expect(ctx.system.sys).toHaveBeenCalledWith("💾 Self-improvement review: Skill 'robo-release' patched")
  })
  ...
  it('annotates gateway.start_timeout with stderr tail lines so users can diagnose without /logs', () => {
    ...
    onEvent({
      payload: {
        ...
        stderr_tail:
          '[startup] timed out\nModuleNotFoundError: No module named openai\nFileNotFoundError: ~/.robo/config.yaml'
      },
  ...
  it('prefers raw text over Rich-rendered ANSI on message.complete (#16391)', () => {
    ...
    const rendered = '\u001b[33mRobo here.\u001b[0m\n\n\u001b[2mLine two.\u001b[0m'
  ...
  it('shows setup panel for missing provider startup error', () => {
    ...
    onEvent({
      payload: {
        message:
          'agent init failed: No LLM provider configured. Run `robo model` to select a provider, or run `robo setup` for first-time configuration.'
      },
      type: 'error'
    } as any)
  ...
  it('picks the polarity-matching paired palette from gateway.ready skins', async () => {
    ...
    vi.stubEnv('ROBO_TUI_BACKGROUND', '')
    ...
    vi.stubEnv('ROBO_TUI_BACKGROUND', '#ffffff')
    createGatewayEventHandler(buildCtx(appended))({ payload: skin, type: 'skin.changed' } as any)
  ...
  it('routes a spoken choice to the active approval instead of submitting a chat turn', async () => {
    const ctx = buildCtx([])
    ctx.gateway.rpc = vi.fn(async () => ({ ok: true }))
    patchUiState({ sid: 'approval-session' })
    patchOverlayState({
      approval: {
        allowPermanent: true,
        command: 'apt install nmap',
        description: 'install the requested tool'
      }
    })

    createGatewayEventHandler(ctx)({ payload: { text: 'allow for this session' }, type: 'voice.transcript' } as any)

    await vi.waitFor(() =>
      expect(ctx.gateway.rpc).toHaveBeenCalledWith('approval.respond', {
        choice: 'session',
        session_id: 'approval-session'
      })
    )
    await vi.waitFor(() => expect(getOverlayState().approval).toBeNull())
    expect(ctx.submission.submitRef.current).not.toHaveBeenCalled()
    expect(getTurnState().outcome).toBe('approved (session)')
  })

  it('keeps a spoken wait pending and never sends it to the model', () => {
    const ctx = buildCtx([])
    patchOverlayState({ approval: { command: 'rm x', description: 'delete a file' } })

    createGatewayEventHandler(ctx)({ payload: { text: 'wait' }, type: 'voice.transcript' } as any)

    expect(getOverlayState().approval).not.toBeNull()
    expect(ctx.gateway.rpc).not.toHaveBeenCalledWith('approval.respond', expect.anything())
    expect(ctx.submission.submitRef.current).not.toHaveBeenCalled()
  })

  it('does not guess when speech is ambiguous while approval is pending', () => {
    const ctx = buildCtx([])
    patchOverlayState({ approval: { command: 'python task.py', description: 'run code' } })

    createGatewayEventHandler(ctx)({
      payload: { text: 'do not allow this until you explain it' },
  ...
  it('opens a fresh session before starting voice after wake detection', async () => {
    ...
    createGatewayEventHandler(ctx)({
      payload: { phrase: 'hey robo', start_new_session: true },
      type: 'wake.detected'
    } as any)
  ...
  it('keeps the current session when wake detection disables session creation', async () => {
    ...
    createGatewayEventHandler(ctx)({
      payload: { phrase: 'hey robo', start_new_session: false },
      type: 'wake.detected'
    } as any)
  ...
  it('defaults approval overlays to the session-only mission contract', () => {
  ...
  it('normalizes every approval overlay to Robo mission choices', () => {
    const onEvent = createGatewayEventHandler(buildCtx([]))

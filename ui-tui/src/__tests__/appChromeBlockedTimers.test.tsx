...
// ── AppLayout harness ────────────────────────────────────────────────
//
// The maintainer's review of this file was right that mounting StatusRule alone
...
describe('status-chrome timers under an occluding overlay', () => {
  ...
  it('re-syncs the elapsed read-outs from the wall clock on reveal instead of resuming stale', async () => {
    ...
    expect(rule.output()).toContain('idle 5s')
    ...
    expect(resumed).toContain('idle 5m 5s')
...
// The maintainer's review of #12463 called out that its test asserted on a `picker`
...
// The visibility gate a maintainer asked for: mount the REAL AppLayout so the

...
/**
 * A short, platform-aware hint for when readClipboardText()/writeClipboardText()
 * come back empty across every backend. macOS (pbpaste/pbcopy) and Windows
 * (PowerShell) are always present, so this is mainly useful on Linux, where
 * xclip/wl-clipboard are frequently *not* preinstalled — a missing binary and
 * a genuinely empty clipboard otherwise look identical to the caller.
 */
export function clipboardUnavailableHint(
  ...
  if (env.WSL_INTEROP || env.WSL_DISTRO_NAME) {
    return 'Clipboard is empty, or the WSL/Windows clipboard bridge is unavailable.'
  }

  if (env.WAYLAND_DISPLAY) {
    return 'Clipboard is empty, or wl-clipboard is not installed (try: sudo apt install wl-clipboard).'
  }

  return 'Clipboard is empty, or xclip is not installed (try: sudo apt install xclip).'
}

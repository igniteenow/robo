...
describe('billingDialogCopy', () => {
  it('routes Ignitee Now to the /topup flow', () => {
    const copy = billingDialogCopy(makeBlock({ is_igniteenow: true, provider: 'igniteenow', provider_label: 'Ignitee Now Portal' }))
    expect(copy.title).toContain('Ignitee Now')

...
const logDir = join(process.env.ROBO_HOME?.trim() || join(homedir(), '.robo'), 'logs')
...
// a real ~/.robo (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('robo-tui: parent lifecycle log unavailable\n')
    }
  }
}

...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ROBO_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Robo on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ROBO_TUI_TRUECOLOR=1.

 ...
 * `robo-boot-background` / `robo-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports ROBO_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.robo matches get_robo_home()'s default.
const bootFilePath = () => join(process.env.ROBO_HOME ?? join(homedir(), '.robo'), 'tui-theme-boot.json')

// Never touch the user's real ~/.robo from test runs (the TS suite has no
// ROBO_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into ROBO_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.ROBO_TUI_THEME || env.ROBO_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.ROBO_TUI_BACKGROUND
  ) {
    env.ROBO_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.ROBO_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.ROBO_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into ROBO_TUI_THEME.

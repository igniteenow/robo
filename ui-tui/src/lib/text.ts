...
import { THINKING_STATUS_VERBS } from '../content/verbs.js'
...
const THINKING_STATUS_RE = new RegExp(`^(?:${THINKING_STATUS_VERBS.join('|')})\\.{0,3}$`, 'i')

const THINKING_STATUS_CHUNK_RE = new RegExp(
  // A status fragment begins with a face/glyph, never a Markdown emphasis
  // marker. Excluding `*` keeps headings such as **Planning execution** intact.
  `[^A-Za-z\n*]+\\s*(?:${THINKING_STATUS_VERBS.join('|')})\\.{0,3}\\s*`,
  'giu'
)

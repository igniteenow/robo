import { stringWidth, wrapAnsi } from '@robo/ink'
...
export function stableComposerColumns(totalCols: number, promptWidth: number, termuxMode = false) {
  ...
  // `GoodVibesHeart` (appChrome.tsx) is drawn `position="absolute" right={0}`
  // inside the same composer row (appLayout.tsx), and briefly renders a glyph
  // over whichever cell that is. The scrollbar gutter above already leaves 2
  // spare columns at the row's right edge when it's reserved, which is enough
  // room for the heart too. When it *isn't* reserved (narrow terminals /
  // Termux), nothing else keeps that corner clear, so a full-width input line
  // would have the heart flash land directly on the last typed character.
  // Reserve 1 dedicated column for it in that case.
  const heartGutter = reserveScrollbar > 0 ? 0 : 1

  return Math.max(1, totalCols - promptWidth - 2 - reserveScrollbar - heartGutter)
}

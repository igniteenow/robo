...
// Mutates `queue` in place (shifts the head off). Returns the FULL item
// (text and display), not just .text — a caller that only forwards .text
// silently drops the original typed text for any item whose display differs
// from its submit text (a /skill expansion, a file-drop rewrite), showing
// the wrong thing (or nothing recognizable) once the item is actually sent.
export function dequeueItem(queue: QueueItem[]): QueueItem | undefined {
  return queue.shift()
}
...
export function useQueue() {
  ...
  const dequeue = useCallback(() => {
    const head = dequeueItem(queueRef.current)
    syncQueue()

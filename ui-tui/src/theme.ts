import type { SkinBranding, SkinColors } from '@robo/shared/skin'
...
// Mirrors @robo/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @robo/ink is bundled separately from packages/.
...
const BRAND: ThemeBrand = {
  ...
  icon: '◆',
  prompt: '›',
  welcome: 'Assign a mission, ask a question, or type /help.',
  goodbye: 'Robo standing by. ◆  Built by Ignitee Now.',
  tool: 'ROBO',
  helpHeader: 'ROBO // COMMAND INDEX'
}
...
// ── Ignitee identity ────────────────────────────────────────────────
// Sampled from the Ignitee Now logo (see assets/brand/BRAND.md): deep navy
// base, the indigo of the logo disc for structure and interactive rows, and
// the flame (red -> ember orange) as the accent. Semantic hues stay
// conventional (green ok / amber warn / red error) so status is never
// ambiguous; warn is yellower than the ember accent so the two never blur.
...
// Light-terminal seeds: the same indigo/flame family, deepened to hold
 ...
 *  answer when known (cached in ROBO_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.ROBO_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit ROBO_TUI_THEME / COLORFGBG signals above still win,
...
// `ROBO_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `ROBO_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `ROBO_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `ROBO_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Robo palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.ROBO_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.ROBO_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.ROBO_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into ROBO_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into ROBO_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  // The product palette is already authored as a coherent fixed point. An
  // empty skin should preserve it byte-for-byte instead of deriving it again
  // against a generic reference canvas (which introduces one-channel colour
  // drift and makes startup differ from the first gateway frame).
  if (!hasSkinColors) {
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Robo navy
  ...
  // the derived value already carries the identity seeds (e.g. Robo navy).
  const activeRow =

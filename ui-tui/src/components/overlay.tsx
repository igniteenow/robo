...
export function Dialog({ children, hint, title, width }: DialogProps) {
  ...
  // Robo dialog: single-line frame, accent gutter, title set as a rule
  // (`▍ ROBO · MODEL ───────`) so every overlay reads as one product.
  const ruleWidth = Math.max(4, (innerWidth ?? 40) - (title ? title.length + 10 : 0))
      ...
      {title && (
        <Box marginBottom={1} width={innerWidth}>
          <Text wrap="truncate-end">
            <Text bold color={theme.color.accent}>{'▍ '}</Text>
            <Text bold color={theme.color.text}>{`ROBO · ${title.toUpperCase()}`}</Text>
            <Text color={theme.color.border}>{' ' + '─'.repeat(ruleWidth)}</Text>
          </Text>
        </Box>
      )}

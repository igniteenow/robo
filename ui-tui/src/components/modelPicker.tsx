import { Box, Text, useInput, useStdout } from '@robo/ink'
import { useEffect, useMemo, useState } from 'react'

import { CLI_NAME, PRODUCT_ENV_FILE } from '../brand.js'
...
import { chipRowProps, clampOverlayWidth, OverlayTitle } from './overlayPrimitives.js'
...
export function ModelPicker({
  ...
  useInput((ch, key) => {
                ...
                prev.map(p =>
                  p.slug === provider.slug
                    ? {
                        ...
                        models: [],
                        total_models: 0,
                        warning: p.key_env ? `paste ${p.key_env} to activate` : `run \`${CLI_NAME} model\` to configure`
                      }
                    : p
                )
              )
    ...
    if (key.return) {
      if (stage === 'provider') {
        ...
        if (provider.authenticated === false) {
          ...
          // Other auth types: no-op (warning shown tells them to run robo model)
          return
        }
    ...
    // filter. With Ctrl held, @robo/ink reports `ch` as the key name ('g'),
  ...
  if (stage === 'key' && provider) {
    ...
    return (
      <Box flexDirection="column" width={width}>
        <OverlayTitle section="model" t={t} width={width}>
          Configure {provider.name}
        </OverlayTitle>

        <Text color={t.color.muted} wrap="truncate-end">
          Paste your API key below (saved to {PRODUCT_ENV_FILE})
        </Text>

import type { InputEvent, Key } from '@robo/ink'
import * as Ink from '@robo/ink'
 ...
 * but that accepts `key.meta` on macOS — and robo-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.ROBO_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (
        ...
        // readClipboardText() (lib/clipboard.js) already covers every
        // platform (pbpaste/pbcopy on macOS, PowerShell on Windows/WSL,
        // wl-paste on Wayland, xclip on X11) — this fallback exists for
        // TextInput instances that don't wire up onPaste, and it must not
        // be macOS-only.

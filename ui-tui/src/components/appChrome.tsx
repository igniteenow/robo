...
import { Box, type ScrollBoxHandle, stringWidth, Text } from '@robo/ink'
...
function SessionDuration({ startedAt }: { startedAt: number }) {
  ...
  return `up ${fmtDuration(now - startedAt)}`
}
...
function IdleSince({ endedAt }: { endedAt: number }) {
  ...
  return `idle ${fmtDuration(now - endedAt)}`
}
...
export function StatusRule({
  ...
  // Dev-only readout (ROBO_DEV_CREDITS). The server omits the key entirely unless the
  ...
  const showDuration = segs.duration && !!sessionStartedAt && fits(SEP + stringWidth('up ') + MAX_DURATION_WIDTH)
  ...
  const showIdle =
    segs.duration && !busy && lastTurnEndedAt != null && fits(SEP + stringWidth('idle ') + MAX_DURATION_WIDTH)
  ...
  // Dev-gated readout (ROBO_DEV_CREDITS), lowest priority,

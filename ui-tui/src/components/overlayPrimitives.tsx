import type { Key } from '@robo/ink'
import { Box, Text, useInput } from '@robo/ink'
...
export function MenuRow({ active, index, label, t }: { active: boolean; index: number; label: string; t: Theme }) {
  ...
  return (
    <Text>
      ...
      >
        {active ? '● ' : '○ '}
        <Text color={t.color.muted}>{String(index).padStart(2)} </Text>
        {label}
      </Text>
    </Text>
  )
...
export const footer = (extra: string, t: Theme) => (
...
/** Robo overlay title: `▍ ROBO · SECTION` + subtitle, then a rule. Every
 *  overlay (model picker, sessions, settings, skills, plugins) uses it so
 *  the pickers read as one product rather than a stack of bold captions. */
export function OverlayTitle({ children, section, t, width }: { children?: ReactNode; section: string; t: Theme; width?: number }) {
  const ruleWidth = Math.max(8, (width ?? 60) - 2)

  return (
    <Box flexDirection="column" marginBottom={1}>
      <Text wrap="truncate-end">
        <Text bold color={t.color.accent}>{'▍ '}</Text>
        <Text bold color={t.color.text}>{`ROBO · ${section.toUpperCase()}`}</Text>
        {children ? <Text color={t.color.muted}>{'  ·  '}{children}</Text> : null}
      </Text>
      <Text color={t.color.border}>{'─'.repeat(ruleWidth)}</Text>
    </Box>
  )
}

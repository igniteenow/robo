...
export function useSubmission(opts: UseSubmissionOptions) {
  ...
  //                   tool window).
  //   - 'interrupt' (default): submit immediately; the message is echoed into
  //                   the transcript now and the backend redirects the live
  //                   turn (cancelling a running tool batch so the model reads
  //                   it right away), with legacy interrupt + queue as its
  //                   compatibility path.
  //   - 'redirect'  : same submit path; the backend lets a running tool batch
  //                   finish before applying the correction.
  //
    ...
    (value: string) => {
      ...
      if (!value.trim() && !composerState.inputBuf.length) {
        ...
        if (doubleTap && live.sid && composerRefs.queueRef.current.length) {
          ...
          if (next) {
            composerActions.setQueueEdit(null)
            dispatchSubmission(next.display)
          }
        }

...
const BUSY_MODES = new Set<BusyInputMode>(['interrupt', 'queue', 'redirect', 'steer'])

// The TUI follows the framework default (`robo_cli/config_defaults.py`):
// `interrupt`. It used to default to `queue`, which silently parked a message
// typed mid-turn in the client-side queue — never sent to the gateway, never
// shown in the transcript (only in the small queued-messages panel), and only
// read by the agent after the whole turn finished. That is the "my message
// didn't show up and Robo took ages to read it" symptom. With `interrupt` the
// message is echoed into the transcript immediately and delivered to the live
// turn. Set `display.busy_input_mode: queue` (or `redirect` / `steer`)
// explicitly to opt out.
const TUI_BUSY_DEFAULT: BusyInputMode = 'interrupt'
...
export function useConfigSync({
  ...
  useEffect(() => {
    ...
    setVoiceEnabled(process.env.ROBO_VOICE === '1')

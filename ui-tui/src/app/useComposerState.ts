...
import { useStdin, withInkSuspended } from '@robo/ink'
...
import { clipboardUnavailableHint, isUsableClipboardText, readClipboardText } from '../lib/clipboard.js'
...
/**
 * Decide what to tell the user when clipboard.paste comes back without an
 * attached image. `overrideMessage` (when the caller supplies one) always
 * wins over the backend's own `r.message` — the backend's clipboard.paste
 * RPC is fundamentally an image-attach endpoint, so its message ("No image
 * found in clipboard" / "Clipboard has image but extraction failed") only
 * makes sense for an explicit "attach an image" request. A caller that
 * passes overrideMessage (the hotkey-triggered text-paste fallback) already
 * knows text-reading failed first, so it has strictly more context than the
 * backend's generic image-only response and must not have that context
 * silently discarded in favor of it.
 */
export function resolveClipboardPasteMessage(
  overrideMessage: string | undefined,
  backendMessage: string | undefined
): string {
  return overrideMessage || backendMessage || 'No image found in clipboard'
}
...
export function useComposerState({ gw, submitRef, sys }: UseComposerStateOptions): UseComposerStateResult {
  ...
  const pasteClipboardImage = useCallback(
    async (
      ...
      if (!quiet) {
        sys(resolveClipboardPasteMessage(overrideMessage, r?.message))
      }
    ...
    ({ bracketed, cursor, hotkey, text, value }: PasteEvent): MaybePromise<ComposerPasteResult | null> => {
      if (hotkey) {
        ...
        return readPreferredText.then(async preferredText => {
          ...
          // Text reading already failed via both readClipboardText() and OSC 52
          // above — unlike the bracketed-paste caller (where an empty-text paste
          // genuinely does suggest an image), we know this was a Ctrl+V/Cmd+V
          // attempt that found no usable text, so "no image found" alone is a
          // misleading message. Give a platform-aware hint instead: on Linux,
          // a missing xclip/wl-clipboard install is the single most common
          // real-world cause of clipboard reads silently coming back empty.
          return pasteClipboardImage(value, cursor, false, clipboardUnavailableHint())
        })
      }
  ...
  const openEditor = useCallback(async () => {
    const dir = mkdtempSync(join(tmpdir(), 'robo-'))

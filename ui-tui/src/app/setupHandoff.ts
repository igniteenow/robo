import type { RunExternalProcess } from '@robo/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`${CLI_NAME} ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching ${CLI_NAME}: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`${CLI_NAME} ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })

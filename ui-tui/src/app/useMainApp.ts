...
import { PRODUCT_SHORT_NAME } from '../brand.js'
...
import { composeTabTitle, shortCwd } from '../domain/paths.js'
...
export function useMainApp(gw: GatewayClient) {
  ...
  useEffect(() => {
    ...
    if (next) {
      ...
      sendQueued(next.text, next.display)
    }
  ...
  const appStatus = useMemo(
    () => ({
      // No path is ever displayed here. The status bar shows nothing for
      // this segment, and statusRuleWidths correctly reserves zero width

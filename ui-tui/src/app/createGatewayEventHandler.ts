...
const commitTheme = (theme: Theme) => {
  ...
  const pin = configPinnedTheme ? process.env.ROBO_TUI_THEME : undefined

  writeBootTheme(theme, process.env.ROBO_TUI_BACKGROUND, pin === 'light' || pin === 'dark' ? pin : undefined)
 ...
 * to ROBO_TUI_THEME — the priority-2 signal `detectLightMode` already
 * honors (only an explicit ROBO_TUI_LIGHT env var outranks it); 'auto'
 ...
 */
// True once CONFIG (via light/dark) owns the ROBO_TUI_THEME env pin, so an
...
export function applyConfiguredTuiTheme(raw: unknown): void {
  ...
  const current = process.env.ROBO_TUI_THEME ?? ''
  ...
  if (mode === 'light' || mode === 'dark') {
    ...
    process.env.ROBO_TUI_THEME = mode
  } else {
    // 'auto' clears only a pin CONFIG set — never a ROBO_TUI_THEME the user
    ...
    delete process.env.ROBO_TUI_THEME
  }
 ...
 * is cached into ROBO_TUI_BACKGROUND — the slot `detectLightMode` already
 ...
 * ROBO_TUI_LIGHT / ROBO_TUI_THEME overrides still win inside
...
export function syncThemeToTerminalBackground(): void {
  ...
  onTerminalBackground(hex => {
    ...
    resolved = true
    process.env.ROBO_TUI_BACKGROUND = hex
    reapplyTheme()
  })
  ...
  onTerminalForeground(hex => {
    if (resolved || process.env.ROBO_TUI_THEME || process.env.ROBO_TUI_LIGHT) {
      return
    }
    ...
    resolved = true
    process.env.ROBO_TUI_BACKGROUND = inferred
    reapplyTheme()
  })
  ...
  setTimeout(() => {
    if (
      ...
      process.env.ROBO_TUI_BACKGROUND ||
      process.env.ROBO_TUI_THEME ||
      process.env.ROBO_TUI_LIGHT ||
    ...
    execFile('defaults', ['read', '-g', 'AppleInterfaceStyle'], (error, stdout) => {
      if (resolved || process.env.ROBO_TUI_BACKGROUND || process.env.ROBO_TUI_THEME) {
        return
      }
      ...
      // inference).
      resolved = true
      process.env.ROBO_TUI_BACKGROUND = dark ? '#1e1e1e' : '#ffffff'
      reapplyTheme()
    })
  }, 1500).unref?.()
}
...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: GatewayEvent) => void {
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // Arm "Hey Robo" if this surface owns it (server gates on config).
    ...
    // forging a brand-new one.  Mirrors classic CLI's `robo -c` /
    // `robo --tui` muscle memory and addresses the audit's "session
  ...
  return (ev: GatewayEvent) => {
    ...
    switch (ev.type) {
      ...
      case 'voice.transcript': {
        ...
        // If Robo is visibly waiting for approval, a short spoken choice is
        // control input for that prompt — it must not be sent to the model as
        // another chat turn. The parser accepts only exact, deliberate phrases
        // so normal conversation cannot accidentally authorize a command.
        const approval = getOverlayState().approval

        if (approval) {
          const action = parseSpokenApproval(text)

          if (action === 'wait') {
            setStatus('approval paused · waiting for you')
            sys('approval: waiting · say allow once, allow for this session, deny, or show command')

            return
          }

          if (action === 'show') {
            panel('Approval command', [
              { text: approval.description, title: 'Why Robo is asking' },
              { text: approval.command, title: 'Full command' }
            ])
          ...
          if (action) {
            const available = approvalOptions(approval)

            if (!available.includes(action)) {
              sys(`approval: '${action}' is not available for this command · choose ${available.join(', ')}`)

              return
            }

            void rpc('approval.respond', { choice: action, session_id: getUiState().sid })
              .then(result => {
                if (!result) {
                  sys('approval: Robo could not deliver the spoken response; use the numbered approval menu')

                  return
                }

                patchOverlayState({ approval: null })
                patchTurnState({ outcome: action === 'deny' ? 'denied' : `approved (${action})` })
                setStatus('running…')
                sys(`approval: ${action === 'deny' ? 'denied' : `allowed ${action}`}`)
              })
              .catch(error => sys(`approval: ${rpcErrorMessage(error)}`))
      ...
      case 'wake.detected': {
        // "Hey Robo": optionally open a fresh session (start_new_session),
        ...
        void (async () => {
          ...
          if (wakeProfile && wakeProfile !== ownProfile) {
            const cli = process.env.ROBO_CLI_NAME?.trim() || 'robo'
            sys(`wake phrase for profile '${wakeProfile}' — run: ${cli} -p ${wakeProfile} --tui`)

...
import { terminalBackgroundHex } from '@robo/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $ROBO_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see ROBO_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['ROBO_TUI_BACKGROUND', process.env.ROBO_TUI_BACKGROUND ?? '(unset)'],
            ['ROBO_TUI_THEME', process.env.ROBO_TUI_THEME ?? '(unset)'],

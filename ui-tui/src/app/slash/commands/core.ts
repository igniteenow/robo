import { forceRedraw, type MouseTrackingMode } from '@robo/ink'

import { CLI_NAME, PRODUCT_NAME, PRODUCT_SHORT_NAME } from '../../../brand.js'
...
export const coreCommands: SlashCommand[] = [
...

  {
    ...
    run: (_arg, ctx) => {
      ...
      // Exit code 42 signals the Python wrapper to exec `robo update`.
    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys('clipboard copy failed — try ROBO_TUI_FORCE_OSC52=1 to force the escape sequence')
        }
      }
...

  {
    ...
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `${PRODUCT_SHORT_NAME} #${i + 1}`

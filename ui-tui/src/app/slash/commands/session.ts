...
export const sessionCommands: SlashCommand[] = [
  ...
  {
    help: 'control busy enter mode [interrupt|redirect|queue|steer|status]',
    name: 'busy',
    run: (arg, ctx) => {
      const mode = arg.trim().toLowerCase()
      const valid = new Set(['', 'status', 'queue', 'steer', 'interrupt', 'redirect'])

      if (!valid.has(mode)) {
        return ctx.transcript.sys('usage: /busy [interrupt|redirect|queue|steer|status]')
      }
  ...
  {
    help: 'session usage + Ignitee Now credits',
    name: 'usage',
    run: (_arg, ctx) => {
      ctx.gateway.rpc<SessionUsageResponse>('session.usage', { session_id: ctx.sid }).then(r => {
        ...
        // Ignitee Now balance block is agent-independent (a portal fetch), so it shows
        ...
        if (usageModel?.available && (barLines.length || usageModel.status === 'free')) {
          ...
          if (creditsLines.length) {
            ctx.transcript.panel('Ignitee Now balance', [{ text: creditsLines.join('\n') }])
            showedBalance = true
          }
        }

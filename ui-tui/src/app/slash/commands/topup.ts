import { driveChargeSettlement, type SettlementOutcome } from '@robo/shared/charge-settlement'
...
const renderBillingError = (
  ...
  switch (env.error) {
    ...
    case 'remote_spending_disabled':
      ...
      // turn it on from the portal's Robo page; this is NOT a per-terminal stop.
      sys(
        "Remote spending is off for this account — a billing administrator can enable it in the provider portal."
      )
      ...
      ctx.gateway
        ...
        .then(
          ctx.guarded<BillingStateResponse>(s => {
            if (!s.logged_in) {
              sys('💳 Not logged into Ignitee Now Portal — run /portal to log in, then /topup.')

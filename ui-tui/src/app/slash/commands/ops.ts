import { PRODUCT_ENV_FILE } from '../../../brand.js'
import type {
...
export const opsCommands: SlashCommand[] = [
  ...
  {
    help: `re-read ${PRODUCT_ENV_FILE} into the running gateway (CLI parity)`,
    name: 'reload',
    run: (_arg, ctx) => {
      ctx.gateway
...

  {
    ...
    run: (arg, ctx, cmd) => {
      ...
      // text slash worker so it stays at parity with `robo plugins`.
      if (!arg.trim()) {

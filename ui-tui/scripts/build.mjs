...
await build({
  ...
  // Keep npm workspace junctions on the build drive. This matters on
  // hardened Windows installations where the repository is readable but
  // enumerating the user's home-directory root is intentionally denied.
  preserveSymlinks: true,
  // Skip the prebuilt @robo/ink bundle and inline the source instead:
  ...
  alias: { '@robo/ink': resolve(root, 'packages/robo-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The robo_cli launcher

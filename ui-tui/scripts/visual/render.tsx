 ...
 * across theme x background scenes to <tmpdir>/robo-tui-visual/tui-visual.html,
...
// The regenerated slate light_colors block from robo_cli/skin_engine.py
// (relight recipe: vivid hue-preserved accents, airy capped-saturation text,
// darker calm dims).

const info: SessionInfo = {
  cwd: '/Users/brooklyn/www/robo-engineer',
...
const setup = (bgHex: string) => {
  process.env.ROBO_TUI_BACKGROUND = bgHex
  resetOverlayState()
  resetUiState()
}
...
addScene('default · dark terminal', '#071018', {})
addScene('default · light terminal (Cursor)', '#ffffff', {})
addScene('slate · dark terminal', '#071018', SLATE)
  ...
  const intro = renderAnsi(
    <Box flexDirection="column">
      ...
      <RoboCommandDeck
        busy
        cols={88}
        cwdLabel="~/projects/network-hardening"
        model="claude-opus-4.8-fast"
        profile="engineer"
        sessionId="d2a6ecf8"
        sessionTitle="Harden the lab switch and verify controls"
      ...
      <Box flexDirection="column" paddingX={2} paddingTop={1}>
        ...
        <Text color={scene.theme.color.text}>I’m checking management access, VLAN boundaries, and logging now.</Text>
        <Text color={scene.theme.color.muted}>  ⚙ running: configuration inventory</Text>
      </Box>
    </Box>,
    88
  )

...
React + Ink terminal UI for Robo. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ROBO_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `robo setup`, suspends Ink while it runs, opens a new session on success
...
- `commands/billing.ts` — `/billing`: manage Ignitee Now remote spending — buy credits, auto-reload, limits
...
- Input history is stored in `~/.robo/.robo_history` or under `ROBO_HOME`.
...
### Billing (`billing.ts`)
`/billing` — manage Ignitee Now remote spending — buy credits, auto-reload, limits
...
### Credits (`credits.ts`)
`/credits` — Ignitee Now credit balance and browser top-up

### Setup (`setup.ts`)
`/setup` — launches external `robo setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/robo-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external robo setup, suspends Ink while it runs
      ...
      slash/
        ...
        commands/
          billing.ts                /billing — manage Ignitee Now remote spending
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.robo/tui-parent.log
    ...
    types/
      robo-ink.d.ts            type declarations for @robo/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent RoboCLI subprocess for slash commands
```

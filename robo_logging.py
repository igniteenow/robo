"""Centralized logging setup for Robo Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.robo/logs/`` (profile-aware via ``get_robo_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``robo_cli.web_server``,
    ``robo_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Robo (TUI, gateway, ``hy_memory`` server, MCP
...
from robo_constants import get_config_path, get_robo_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_robo_session_injector", False):
    ...
    _session_record_factory._robo_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``robo logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``robo logs --component gateway``.
    "gateway": ("gateway", "robo_plugins", "plugins.platforms"),
...
def setup_logging(
    *,
    ...
    """Configure the Robo logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    robo_home
        Override for the Robo home directory.  Falls back to
        ``get_robo_home()`` (profile-aware).
    log_level
    ...
    home = robo_home or get_robo_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_robo_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._robo_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the robo group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from robo_cli.config import is_managed
...
# the module header): when several Robo processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._robo_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="robo-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_robo_queue", False):
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse robo_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when robo_cli.config isn't importable (bare
        # robo_logging consumers).
        try:
            from robo_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
